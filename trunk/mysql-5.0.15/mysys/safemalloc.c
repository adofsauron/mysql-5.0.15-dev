...
static int check_ptr(const char *where, byte *ptr, const char *sFile, uint uLine);
...
gptr _mymalloc(uint size, const char *filename, uint lineno, myf MyFlags)
{
  ...
  else
  {
    ...
    irem = (struct st_irem *)malloc(ALIGN_SIZE(sizeof(struct st_irem)) + sf_malloc_prehunc + size + /* size requested */
                                    4 +                                                             /* overrun mark */
                                    sf_malloc_endhunc);
  }
  ...
  if (!irem)
  {
    ...
    if (MyFlags & (MY_FAE + MY_WME))
    {
      ...
      sprintf(buff, "needed %d byte (%ldk), memory in use: %ld bytes (%ldk)", size, (size + 1023L) / 1024L,
              sf_malloc_max_memory, (sf_malloc_max_memory + 1023L) / 1024L);
      my_message(EE_OUTOFMEMORY, buff, MYF(ME_BELL + ME_WAITTANG));
    }
    DBUG_PRINT("error", ("Out of memory, in use: %ld at line %d, '%s'", sf_malloc_max_memory, lineno, filename));
    if (MyFlags & MY_FAE)
      exit(1);
    DBUG_RETURN((gptr)0);
  }
  ...
  data = (((char *)irem) + ALIGN_SIZE(sizeof(struct st_irem)) + sf_malloc_prehunc);
...
gptr _myrealloc(register gptr ptr, register uint size, const char *filename, uint lineno, myf MyFlags)
{
  ...
  irem = (struct st_irem *)(((char *)ptr) - ALIGN_SIZE(sizeof(struct st_irem)) - sf_malloc_prehunc);
  if (*((uint32 *)(((char *)ptr) - sizeof(uint32))) != MAGICKEY)
  {
    fprintf(stderr, "Error: Reallocating unallocated data at line %d, '%s'\n", lineno, filename);
    DBUG_PRINT("safe", ("Reallocating unallocated data at line %d, '%s'", lineno, filename));
    (void)fflush(stderr);
    DBUG_RETURN((gptr)NULL);
  }
...
void _myfree(gptr ptr, const char *filename, uint lineno, myf myflags)
{
  ...
  if ((!ptr && (myflags & MY_ALLOW_ZERO_PTR)) || check_ptr("Freeing", (byte *)ptr, filename, lineno))
    DBUG_VOID_RETURN;
...
static int check_ptr(const char *where, byte *ptr, const char *filename, uint lineno)
{
  if (!ptr)
  {
  ...
  if ((long)ptr & (ALIGN_SIZE(1) - 1))
  {
    ...
    DBUG_PRINT("safe", ("Wrong aligned pointer at line %d, '%s'", lineno, filename));
    (void)fflush(stderr);
    return 1;
  }
#endif
  if (ptr < sf_min_adress || ptr > sf_max_adress)
  {
    ...
    DBUG_PRINT("safe", ("Pointer out of range at line %d '%s'", lineno, filename));
    (void)fflush(stderr);
    return 1;
  }
  return 0;
...
void TERMINATE(FILE *file)
{
  ...
  */
  ...
  if ((irem = sf_malloc_root))
  {
    ...
    DBUG_PRINT("safe", ("Memory that was not free'ed (%ld bytes):", sf_malloc_cur_memory));
    while (irem)
    {
      char *data = (((char *)irem) + ALIGN_SIZE(sizeof(struct st_irem)) + sf_malloc_prehunc);
      if (file)
      {
        fprintf(file, "\t%6u bytes at 0x%09lx, allocated at line %4u in '%s'", irem->datasize, (long)data,
                irem->linenum, irem->filename);
        fprintf(file, "\n");
        (void)fflush(file);
      }
      DBUG_PRINT("safe", ("%6u bytes at 0x%09lx, allocated at line %4d in '%s'", irem->datasize, data, irem->linenum,
                          irem->filename));
      irem = irem->next;
    }
  }
  ...
  DBUG_PRINT("safe",
             ("Maximum memory usage: %ld bytes (%ldk)", sf_malloc_max_memory, (sf_malloc_max_memory + 1023L) / 1024L));
  pthread_mutex_unlock(&THR_LOCK_malloc);
  DBUG_VOID_RETURN;
}
...
static int _checkchunk(register struct st_irem *irem, const char *filename, uint lineno)
{
  int flag = 0;
  char *magicp, *data;
  ...
  {
    fprintf(stderr, "Error: Memory allocated at %s:%d was underrun,", irem->filename, irem->linenum);
    fprintf(stderr, " discovered at %s:%d\n", filename, lineno);
    (void)fflush(stderr);
    DBUG_PRINT("safe", ("Underrun at 0x%lx, allocated at %s:%d", data, irem->filename, irem->linenum));
    flag = 1;
  }

  /* Check for a possible overrun */
  magicp = data + irem->datasize;
  if (*magicp++ != MAGICEND0 || *magicp++ != MAGICEND1 || *magicp++ != MAGICEND2 || *magicp++ != MAGICEND3)
  {
    fprintf(stderr, "Error: Memory allocated at %s:%d was overrun,", irem->filename, irem->linenum);
    fprintf(stderr, " discovered at '%s:%d'\n", filename, lineno);
    (void)fflush(stderr);
    DBUG_PRINT("safe", ("Overrun at 0x%lx, allocated at %s:%d", data, irem->filename, irem->linenum));
    flag = 1;
  }
  return (flag);
}
...
int _sanity(const char *filename, uint lineno)
{
  ...
  for (irem = sf_malloc_root; irem != NULL && count--; irem = irem->next) flag += _checkchunk(irem, filename, lineno);
  pthread_mutex_unlock(&THR_LOCK_malloc);
  if (count || irem)
  {
    const char *format = "Error: Safemalloc link list destroyed, discovered at '%s:%d'";
    fprintf(stderr, format, filename, lineno);
    fputc('\n', stderr);
...
char *_my_strdup_with_length(const byte *from, uint length, const char *filename, uint lineno, myf MyFlags)
{
  gptr ptr;
