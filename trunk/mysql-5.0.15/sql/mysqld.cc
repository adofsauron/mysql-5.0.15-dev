...
/* We have HAVE_purify below as this speeds up the shutdown of MySQL */
    ...
    defined(HAVE_purify) && defined(__linux__)
...
char mysql_real_data_home[FN_REFLEN], language[FN_REFLEN], reg_ext[FN_EXTLEN],
    mysql_charsets_dir[FN_REFLEN], *opt_init_file, *opt_tc_log_file,
...
pthread_mutex_t LOCK_mysql_create_db, LOCK_Acl, LOCK_open, LOCK_thread_count,
    LOCK_mapped_file, LOCK_status, LOCK_global_read_lock, LOCK_error_log,
    LOCK_uuid_generator, LOCK_delayed_insert, LOCK_delayed_status,
    LOCK_delayed_create, LOCK_crypt, LOCK_bytes_sent, LOCK_bytes_received,
...
/* kill connection thread */
  ...
  if (hPipe != INVALID_HANDLE_VALUE && opt_enable_named_pipe) {
    ...
    if ((temp = CreateFile(pipe_name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  */
  ...
  while ((tmp = it++)) {
    DBUG_PRINT("quit",
               ("Informing thread %ld that it's time to die", tmp->thread_id));
  ...
  for (;;) {
    ...
    if (tmp->vio_ok()) {
      if (global_system_variables.log_warnings)
        sql_print_warning(
            ER(ER_FORCING_CLOSE), my_progname, tmp->thread_id,
...
#endif
{
  ...
  if (sig == MYSQL_KILL_SIGNAL || sig == 0)
    ...
    sql_print_error(ER(ER_GOT_SIGNAL), my_progname,
                    sig); /* purecov: inspected */
...
#if defined(__NETWARE__) ||                                                    \
    (defined(USE_ONE_SIGNAL_HAND) && !defined(__WIN__) && !defined(OS2))
  ...
  RETURN_FROM_KILL_SERVER;
    ...
    (defined(__NETWARE__) && defined(SIGNALS_DONT_BREAK_READ))
...
static struct passwd *check_user(const char *user) {
  ...
  if (!user) {
    if (!opt_bootstrap) {
      sql_print_error("Fatal error: Please read \"Security\" section of the "
...
err:
  sql_print_error("Fatal error: Can't change to run as user '%s' ;  Please "
...
static void network_init(void) {
  ...
  if (mysqld_port != 0 && !opt_disable_networking && !opt_bootstrap) {
    ...
    */
    (void)setsockopt(ip_sock, SOL_SOCKET, SO_REUSEADDR, (char *)&arg,
                     sizeof(arg));
#endif /* __WIN__ */
  ...
  if (Service.IsNT() && mysqld_unix_port[0] && !opt_bootstrap &&
    ...
    if ((hPipe =
             CreateNamedPipe(pipe_name, PIPE_ACCESS_DUPLEX,
  ...
  */
  if (mysqld_unix_port[0] && !opt_bootstrap) {
    ...
    if (bind(unix_sock, my_reinterpret_cast(struct sockaddr *)(&UNIXaddr),
             sizeof(UNIXaddr)) < 0) {
      sql_perror(
          "Can't start server : Bind on unix socket"); /* purecov: tested */
      sql_print_error(
          "Do you already have another mysqld server running on socket: %s ?",
          mysqld_unix_port);
      unireg_abort(1); /* purecov: tested */
    }
    umask(((~my_umask) & 0666));
...
void close_connection(THD *thd, uint errcode, bool lock) {
  ...
  DBUG_PRINT("enter",
             ("fd: %s  error: '%s'",
              thd->net.vio ? vio_description(thd->net.vio) : "(not connected)",
              errcode ? ER(errcode) : ""));
  if (lock)
    (void)pthread_mutex_lock(&LOCK_thread_count);
...
void mysql_cb_init() {
  ...
  rtag_t rt =
      AllocateResourceTag(handle, "MySQL Down Server Callback", EventSignature);
  NX_WRAP_INTERFACE((void *)mysql_down_server_cb, 2, (void **)&ref);
  eh = RegisterForEventNotification(
      rt, EVENT_PRE_DOWN_SERVER, EVENT_PRIORITY_APPLICATION, NULL, ref, NULL);
...
static void getvolumeID(BYTE *volumeName) {
  ...
  */
  ...
  if ((status = zGetInfo(fileKey, getInfoMask, sizeof(info), zINFO_VERSION_A,
                         &info)) != zOK) {
    consoleprintf(
...
pthread_handler_t signal_hand(void *arg __attribute__((unused))) {
  ...
  */
  ...
  for (;;) {
    ...
    default:
#ifdef EXTRA_DEBUG
      sql_print_warning("Got signal: %d  error: %d", sig,
                        error); /* purecov: tested */
#endif
...
static int my_message_sql(uint error, const char *str, myf MyFlags) {
  ...
  */
  if ((thd = current_thd)) {
    ...
    */
    if (thd->lex->current_select && thd->lex->current_select->no_error &&
        !thd->is_fatal_error) {
      DBUG_PRINT(
          "error",
          ("Error converted to warning: current_select: no_error %d  "
           "fatal_error: %d",
           (thd->lex->current_select ? thd->lex->current_select->no_error : 0),
  ...
  */
  ...
  /* connections and databases needs lots of files */
  {
    ...
    if (files < wanted_files) {
      if (!open_files_limit) {
        ...
        sql_print_warning("Could not increase number of max_open_files to more "
                          "than %u (request: %u)",
                          files, wanted_files);
    }
  ...
  if (default_collation_name) {
    ...
    if (!my_charset_same(default_charset_info, default_collation)) {
      sql_print_error(ER(ER_COLLATION_CHARSET_MISMATCH), default_collation_name,
...
You should consider changing lower_case_table_names to 1 or 2",
  ...
  table_alias_charset =
      (lower_case_table_names ? files_charset_info : &my_charset_bin);
...
server.");
  ...
  tc_log = (total_ha_2pc > 1 ? (opt_bin_log ? (TC_LOG *)&mysql_bin_log
                                            : (TC_LOG *)&tc_log_mmap)
                             : (TC_LOG *)&tc_log_dummy);
...
static void handle_connections_methods() {
  ...
  if (hPipe == INVALID_HANDLE_VALUE &&
      (!have_tcpip || opt_disable_networking) && !opt_enable_shared_memory) {
    sql_print_error("TCP/IP, --shared-memory, or --named-pipe should be "
                    "configured on NT OS");
  ...
  if (have_tcpip && !opt_disable_networking) {
    handler_count++;
    if (pthread_create(&hThread, &connection_attrib, handle_connections_sockets,
                       0)) {
...
we force server id to 2, but this MySQL server will not act as a slave.");
  ...
  */
  ...
  sql_print_information(
      ER(ER_STARTUP), my_progname, server_version,
...
#ifdef __WIN__
  if (!have_tcpip || opt_disable_networking) {
    sql_print_error("TCP/IP unavailable or disabled with --skip-networking; no "
...
int main(int argc, char **argv) {
  ...
  */
  int10_to_str((int)GetCurrentProcessId(),
               strmov(shutdown_event_name, "MySQLShutdown"), 10);
...
static void create_new_thread(THD *thd) {
  ...
  if (thread_count - delayed_insert_threads >= max_connections + 1 ||
      abort_loop) {
...
/* Is this a new connection request ? */
    ...
    */
    ...
    if (!(vio_tmp = vio_new(new_sock, sock == unix_sock ? VIO_TYPE_SOCKET
                                                        : VIO_TYPE_TCPIP,
...
pthread_handler_t handle_connections_shared_memory(void *arg) {
  ...
  if (my_security_attr_create(&sa_event, &errmsg, GENERIC_ALL,
                              SYNCHRONIZE | EVENT_MODIFY_STATE))
    goto error;

  if (my_security_attr_create(&sa_mapping, &errmsg, GENERIC_ALL,
  ...
  */
  ...
  if ((smem_event_connect_request = CreateEvent(sa_event, FALSE, FALSE, tmp)) ==
      0) {
  ...
  if ((handle_connect_file_map =
           CreateFileMapping(INVALID_HANDLE_VALUE, sa_mapping, PAGE_READWRITE,
                             0, sizeof(connect_number), tmp)) == 0) {
    errmsg = "Could not create file mapping";
    goto error;
  }
  if ((handle_connect_map =
           (char *)MapViewOfFile(handle_connect_file_map, FILE_MAP_WRITE, 0, 0,
                                 sizeof(DWORD))) == 0) {
  ...
  while (!abort_loop) {
    ...
    */
    ...
    if ((handle_client_file_map =
             CreateFileMapping(INVALID_HANDLE_VALUE, sa_mapping, PAGE_READWRITE,
                               0, smem_buffer_length, tmp)) == 0) {
      errmsg = "Could not create file mapping";
      goto errorconn;
    }
    if ((handle_client_map =
             (char *)MapViewOfFile(handle_client_file_map, FILE_MAP_WRITE, 0, 0,
    ...
    if (!(thd->net.vio = vio_new_win32shared_memory(
              &thd->net, handle_client_file_map, handle_client_map,
    ...
    thd->security_ctx->host =
        my_strdup(my_localhost, MYF(0)); /* Host is unknown */
    create_new_thread(thd);
    connect_number++;
    continue;
...
struct my_option my_long_options[] = {
  { "help",            '?', "Display this help and exit.", (gptr *)&opt_help,
    (gptr *)&opt_help, 0,   GET_BOOL,                      NO_ARG,
    0,                 0,   0,                             0,
    0,                 0 },
#ifdef HAVE_REPLICATION
  ...
  { "ansi", 'a', "Use ANSI SQL syntax instead of MySQL syntax. This mode will "
  ...
  { "allow-suspicious-udfs", OPT_ALLOW_SUSPICIOUS_UDFS,
    ...
    "from libc.so",
    (gptr *)&opt_allow_suspicious_udfs, (gptr *)&opt_allow_suspicious_udfs, 0,
    GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0 },
  { "auto-increment-increment",
    OPT_AUTO_INCREMENT,
    "Auto-increment columns are incremented by this",
    (gptr *)&global_system_variables.auto_increment_increment,
    (gptr *)&max_system_variables.auto_increment_increment,
    0,
    GET_ULONG,
    OPT_ARG,
    1,
  ...
  { "automatic-sp-privileges", OPT_SP_AUTOMATIC_PRIVILEGES,
    ...
    (gptr *)&sp_automatic_privileges, (gptr *)&sp_automatic_privileges, 0,
    GET_BOOL, NO_ARG, 1, 0, 0, 0, 0, 0 },
  { "basedir", 'b', "Path to installation directory. All paths are usually "
...
Disable with --skip-bdb (will save memory).",
    ...
    0 },
#ifdef HAVE_BERKELEY_DB
  { "bdb-home",             OPT_BDB_HOME,           "Berkeley home directory.",
    (gptr *)&berkeley_home, (gptr *)&berkeley_home, 0,
  ...
  { "bdb-lock-detect",
    ...
    0,
    0 },
  { "bdb-logdir",                      OPT_BDB_LOG,
    "Berkeley DB log file directory.", (gptr *)&berkeley_logdir,
    (gptr *)&berkeley_logdir,          0,
    GET_STR,                           REQUIRED_ARG,
    0,                                 0,
    0,                                 0,
    0,                                 0 },
  { "bdb-no-recover",                                    OPT_BDB_NO_RECOVER,
    "Don't try to recover Berkeley DB tables on start.", 0,
    0,                                                   0,
    GET_NO_ARG,                                          NO_ARG,
    0,                                                   0,
    0,                                                   0,
  ...
  { "bdb-shared-data",                          OPT_BDB_SHARED,
    "Start Berkeley DB in multi-process mode.", 0,
    0,                                          0,
    GET_NO_ARG,                                 NO_ARG,
    0,                                          0,
    0,                                          0,
    0,                                          0 },
  { "bdb-tmpdir",                 OPT_BDB_TMP,
    "Berkeley DB tempfile name.", (gptr *)&berkeley_tmpdir,
    (gptr *)&berkeley_tmpdir,     0,
    GET_STR,                      REQUIRED_ARG,
    0,                            0,
    0,                            0,
    0,                            0 },
#endif /* HAVE_BERKELEY_DB */
  { "big-tables", OPT_BIG_TABLES, "Allow big result sets by saving all "
                                  "temporary sets on file (Solves most 'table "
                                  "full' errors).",
    0, 0, 0, GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0 },
  { "bind-address",            OPT_BIND_ADDRESS,
    "IP address to bind to.",  (gptr *)&my_bind_addr_str,
    (gptr *)&my_bind_addr_str, 0,
    GET_STR,                   REQUIRED_ARG,
    0,                         0,
    0,                         0,
    0,                         0 },
  { "binlog-do-db", OPT_BINLOG_DO_DB, "Tells the master it should log updates "
                                      "for the specified database, and exclude "
                                      "all others not explicitly mentioned.",
    0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  { "binlog-ignore-db", OPT_BINLOG_IGNORE_DB, "Tells the master that updates "
                                              "to the given database should "
                                              "not be logged tothe binary log.",
    0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  { "bootstrap", OPT_BOOTSTRAP, "Used by mysql installation scripts.", 0, 0,
    0,           GET_NO_ARG,    NO_ARG,                                0, 0,
    0,           0,             0,                                     0 },
  ...
  { "character-set-server",              'C',
    "Set the default character set.",    (gptr *)&default_character_set_name,
    (gptr *)&default_character_set_name, 0,
    GET_STR,                             REQUIRED_ARG,
    0,                                   0,
    0,                                   0,
    0,                                   0 },
  { "character-sets-dir",                  OPT_CHARSETS_DIR,
    ...
    0,                                     0 },
  { "chroot",                               'r',
    "Chroot mysqld daemon during startup.", (gptr *)&mysqld_chroot,
    (gptr *)&mysqld_chroot,                 0,
    GET_STR,                                REQUIRED_ARG,
    0,                                      0,
    0,                                      0,
    0,                                      0 },
  { "collation-server",              OPT_DEFAULT_COLLATION,
    "Set the default collation.",    (gptr *)&default_collation_name,
    (gptr *)&default_collation_name, 0,
    GET_STR,                         REQUIRED_ARG,
    0,                               0,
    0,                               0,
    0,                               0 },
  { "completion-type",
    ...
    (gptr *)&max_system_variables.completion_type,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    0,
  ...
  { "core-file", OPT_WANT_CORE, "Write core on errors.", 0, 0, 0, GET_NO_ARG,
    NO_ARG,      0,             0,                       0, 0, 0, 0 },
  { "datadir",                    'h',
    "Path to the database root.", (gptr *)&mysql_data_home,
    (gptr *)&mysql_data_home,     0,
    GET_STR,                      REQUIRED_ARG,
    0,                            0,
    0,                            0,
    0,                            0 },
#ifndef DBUG_OFF
  { "debug",                      '#',                          "Debug log.",
    (gptr *)&default_dbug_option, (gptr *)&default_dbug_option, 0,
    GET_STR,                      OPT_ARG,                      0,
    0,                            0,                            0,
    0,                            0 },
#endif
  { "default-character-set", 'C', "Set the default character set (deprecated "
                                  "option, use --character-set-server "
                                  "instead).",
    (gptr *)&default_character_set_name, (gptr *)&default_character_set_name, 0,
    GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  { "default-collation", OPT_DEFAULT_COLLATION, "Set the default collation "
                                                "(deprecated option, use "
                                                "--collation-server instead).",
    (gptr *)&default_collation_name, (gptr *)&default_collation_name, 0,
    GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  ...
  { "default-table-type",                         OPT_STORAGE_ENGINE,
    "(deprecated) Use --default-storage-engine.", 0,
    0,                                            0,
    GET_STR,                                      REQUIRED_ARG,
    0,                                            0,
    0,                                            0,
  ...
  { "default-time-zone",          OPT_DEFAULT_TIME_ZONE,
    "Set the default time zone.", (gptr *)&default_tz_name,
    (gptr *)&default_tz_name,     0,
    GET_STR,                      REQUIRED_ARG,
    0,                            0,
    0,                            0,
    0,                            0 },
  { "delay-key-write", OPT_DELAY_KEY_WRITE, "Type of DELAY_KEY_WRITE.", 0, 0,
    0,                 GET_STR,             OPT_ARG,                    0, 0,
    0,                 0,                   0,                          0 },
  ...
  { "enable-locking",
    ...
    0 },
#ifdef __NT__
  { "enable-named-pipe",            OPT_HAVE_NAMED_PIPE,
    "Enable the named pipe (NT).",  (gptr *)&opt_enable_named_pipe,
    (gptr *)&opt_enable_named_pipe, 0,
    GET_BOOL,                       NO_ARG,
    0,                              0,
    0,                              0,
    0,                              0 },
  ...
  { "enable-pstack",                            OPT_DO_PSTACK,
    "Print a symbolic stack trace on failure.", (gptr *)&opt_do_pstack,
    (gptr *)&opt_do_pstack,                     0,
    GET_BOOL,                                   NO_ARG,
    0,                                          0,
    0,                                          0,
    0,                                          0 },
  { "engine-condition-pushdown",
    ...
    0,
    0 },
  { "exit-info", 'T',      "Used for debugging;  Use at your own risk!", 0, 0,
    0,           GET_LONG, OPT_ARG,                                      0, 0,
    0,           0,        0,                                            0 },
  { "external-locking", OPT_USE_LOCKING,
    ...
    (gptr *)&opt_external_locking, (gptr *)&opt_external_locking, 0, GET_BOOL,
    NO_ARG, 0, 0, 0, 0, 0, 0 },
  { "flush", OPT_FLUSH,  "Flush tables to disk between SQL commands.", 0, 0,
    0,       GET_NO_ARG, NO_ARG,                                       0, 0,
    0,       0,          0,                                            0 },
  /* We must always support the next option to make scripts like mysqltest
     easier to do */
  { "gdb",                                 OPT_DEBUGGING,
    "Set up signals usable for debugging", (gptr *)&opt_debugging,
    (gptr *)&opt_debugging,                0,
    GET_BOOL,                              NO_ARG,
    0,                                     0,
    0,                                     0,
    0,                                     0 },
...
Disable with --skip-large-pages.", (gptr *)&opt_large_pages,
    (gptr *)&opt_large_pages,           0,
    GET_BOOL,                           NO_ARG,
    0,                                  0,
    0,                                  0,
    0,                                  0 },
  ...
  { "init-file",                                    OPT_INIT_FILE,
    "Read SQL commands from this file at startup.", (gptr *)&opt_init_file,
    (gptr *)&opt_init_file,                         0,
    GET_STR,                                        REQUIRED_ARG,
    0,                                              0,
    0,                                              0,
    0,                                              0 },
  { "init-rpl-role", OPT_INIT_RPL_ROLE, "Set the replication role.", 0, 0,
    0,               GET_STR,           REQUIRED_ARG,                0, 0,
    0,               0,                 0,                           0 },
  { "init-slave",
    OPT_INIT_SLAVE,
...
Disable with --skip-innodb-checksums.",
  ...
  { "innodb_data_file_path",                     OPT_INNODB_DATA_FILE_PATH,
    "Path to individual files and their sizes.", 0,
    0,                                           0,
    GET_STR,                                     REQUIRED_ARG,
    0,                                           0,
    0,                                           0,
...
*/
...
#endif
    ".",
    (gptr *)&innobase_fast_shutdown, (gptr *)&innobase_fast_shutdown, 0,
    GET_ULONG, OPT_ARG, 1, 0, IF_NETWARE(1, 2), 0, 0, 0 },
  ...
  { "innodb_flush_log_at_trx_commit", OPT_INNODB_FLUSH_LOG_AT_TRX_COMMIT,
    ...
    (gptr *)&innobase_flush_log_at_trx_commit, 0, GET_UINT, OPT_ARG, 1, 0, 2, 0,
    0, 0 },
  { "innodb_flush_method",
    OPT_INNODB_FLUSH_METHOD,
  ...
  { "innodb_locks_unsafe_for_binlog", OPT_INNODB_LOCKS_UNSAFE_FOR_BINLOG,
    ...
    (gptr *)&innobase_locks_unsafe_for_binlog, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0,
    0, 0 },
  { "innodb_log_arch_dir",                 OPT_INNODB_LOG_ARCH_DIR,
    "Where full logs should be archived.", (gptr *)&innobase_log_arch_dir,
    (gptr *)&innobase_log_arch_dir,        0,
    GET_STR,                               REQUIRED_ARG,
    0,                                     0,
    0,                                     0,
    0,                                     0 },
  { "innodb_log_archive",                          OPT_INNODB_LOG_ARCHIVE,
    "Set to 1 if you want to have logs archived.", 0,
    0,                                             0,
    GET_LONG,                                      OPT_ARG,
    0,                                             0,
    0,                                             0,
  ...
  { "innodb_log_group_home_dir",          OPT_INNODB_LOG_GROUP_HOME_DIR,
    ...
    (gptr *)&innobase_log_group_home_dir, 0,
    GET_STR,                              REQUIRED_ARG,
    0,                                    0,
    0,                                    0,
    0,                                    0 },
  ...
  { "isam",                                                  OPT_ISAM,
    "Obsolete. ISAM storage engine is no longer supported.", (gptr *)&opt_isam,
    (gptr *)&opt_isam,                                       0,
    GET_BOOL,                                                NO_ARG,
    0,                                                       0,
    0,                                                       0,
  ...
  { "local-infile",
    ...
    0 },
  { "log",                                  'l',
    "Log connections and queries to file.", (gptr *)&opt_logname,
    (gptr *)&opt_logname,                   0,
    GET_STR,                                OPT_ARG,
    0,                                      0,
    0,                                      0,
  ...
  */
  ...
  { "log-error",                 OPT_ERROR_LOG_FILE,          "Error log file.",
    (gptr *)&log_error_file_ptr, (gptr *)&log_error_file_ptr, 0,
    GET_STR,                     OPT_ARG,                     0,
    0,                           0,                           0,
    0,                           0 },
  { "log-isam",                        OPT_ISAM_LOG,
    "Log all MyISAM changes to file.", (gptr *)&myisam_log_filename,
    (gptr *)&myisam_log_filename,      0,
    GET_STR,                           OPT_ARG,
    0,                                 0,
    0,                                 0,
    0,                                 0 },
  { "log-long-format", '0', "Log some extra information to update log. Please "
  ...
  { "log-queries-not-using-indexes", OPT_LOG_QUERIES_NOT_USING_INDEXES,
    ...
    "log if it is open.",
    (gptr *)&opt_log_queries_not_using_indexes,
    (gptr *)&opt_log_queries_not_using_indexes, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0,
    0, 0 },
  { "log-short-format",
    OPT_SHORT_LOG_FORMAT,
    "Don't log extra information to update and slow-query logs.",
    (gptr *)&opt_short_log_format,
    (gptr *)&opt_short_log_format,
    0,
    GET_BOOL,
    NO_ARG,
  ...
  { "log-slow-admin-statements", OPT_LOG_SLOW_ADMIN_STATEMENTS,
    ...
    (gptr *)&opt_log_slow_admin_statements, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
    0 },
  ...
  { "log-tc-size",                          OPT_LOG_TC_SIZE,
    "Size of transaction coordinator log.", (gptr *)&opt_tc_log_size,
    (gptr *)&opt_tc_log_size,               0,
    GET_ULONG,                              REQUIRED_ARG,
    TC_LOG_MIN_SIZE,                        TC_LOG_MIN_SIZE,
    ~0L,                                    0,
    TC_LOG_PAGE_SIZE,                       0 },
#endif
...
log and this option justs turns on --log-bin instead.",
  ...
  { "log-warnings",
    ...
    (gptr *)&max_system_variables.log_warnings,
    0,
    GET_ULONG,
    OPT_ARG,
    1,
...
thread is in the master's binlogs.",
  ...
  { "master-password", OPT_MASTER_PASSWORD,
    ...
    (gptr *)&master_password, (gptr *)&master_password, 0, GET_STR,
    REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  ...
  { "master-ssl",                                           OPT_MASTER_SSL,
    "Enable the slave to connect to the master using SSL.", (gptr *)&master_ssl,
    (gptr *)&master_ssl,                                    0,
    GET_BOOL,                                               NO_ARG,
    0,                                                      0,
    0,                                                      0,
...
#endif /* HAVE_REPLICATION */
  { "memlock",                 OPT_MEMLOCK,
    "Lock mysqld in memory.",  (gptr *)&locked_in_memory,
    (gptr *)&locked_in_memory, 0,
    GET_BOOL,                  NO_ARG,
    0,                         0,
    0,                         0,
    0,                         0 },
...
Disable with --skip-ndbcluster (will save memory).",
  ...
  { "ndb-connectstring",              OPT_NDB_CONNECTSTRING,
    "Connect string for ndbcluster.", (gptr *)&opt_ndb_connectstring,
    (gptr *)&opt_ndb_connectstring,   0,
    GET_STR,                          REQUIRED_ARG,
    0,                                0,
    0,                                0,
    0,                                0 },
  ...
  { "ndb-nodeid",                                  OPT_NDB_NODEID,
    "Nodeid for this mysqlserver in the cluster.", (gptr *)&opt_ndb_nodeid,
    (gptr *)&opt_ndb_nodeid,                       0,
  ...
  { "ndb-force-send", OPT_NDB_FORCE_SEND,
    ...
    (gptr *)&global_system_variables.ndb_force_send, 0, GET_BOOL, OPT_ARG, 1, 0,
    0, 0, 0, 0 },
  { "ndb_force_send",
    OPT_NDB_FORCE_SEND,
  ...
  { "ndb-use-exact-count", OPT_NDB_USE_EXACT_COUNT,
    ...
    (gptr *)&global_system_variables.ndb_use_exact_count, 0, GET_BOOL, OPT_ARG,
    1, 0, 0, 0, 0, 0 },
  { "ndb_use_exact_count",
    OPT_NDB_USE_EXACT_COUNT,
    ...
    0 },
  { "ndb-shm",                                       OPT_NDB_SHM,
    "Use shared memory connections when available.", (gptr *)&opt_ndb_shm,
    (gptr *)&opt_ndb_shm,                            0,
    GET_BOOL,                                        OPT_ARG,
  ...
  { "ndb-cache-check-time", OPT_NDB_CACHE_CHECK_TIME,
    ...
    GET_ULONG, REQUIRED_ARG, 0, 0, LONG_TIMEOUT, 0, 1, 0 },
#endif
  { "new",
    'n',
  ...
  { "old-passwords",
    ...
    (gptr *)&max_system_variables.old_passwords,
    0,
    GET_BOOL,
    NO_ARG,
    0,
  ...
  { "one-thread",                                       OPT_ONE_THREAD,
    "Only use one thread (for debugging under Linux).", 0,
    0,                                                  0,
    GET_NO_ARG,                                         NO_ARG,
    0,                                                  0,
    0,                                                  0,
  ...
  { "old-style-user-limits", OPT_OLD_STYLE_USER_LIMITS,
    ...
    (gptr *)&opt_old_style_user_limits, (gptr *)&opt_old_style_user_limits, 0,
    GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0 },
  { "pid-file",                      OPT_PID_FILE,
    "Pid file used by safe_mysqld.", (gptr *)&pidfile_name_ptr,
    (gptr *)&pidfile_name_ptr,       0,
    GET_STR,                         REQUIRED_ARG,
    0,                               0,
    0,                               0,
    0,                               0 },
  { "port",                               'P',
    "Port number to use for connection.", (gptr *)&mysqld_port,
    (gptr *)&mysqld_port,                 0,
  ...
  { "relay-log",                                    OPT_RELAY_LOG,
    "The location and name to use for relay logs.", (gptr *)&opt_relay_logname,
    (gptr *)&opt_relay_logname,                     0,
...
Can't be set to 1 if --log-slave-updates is used.",
  ...
  { "report-password",        OPT_REPORT_PASSWORD,      "Undocumented.",
    (gptr *)&report_password, (gptr *)&report_password, 0,
    GET_STR,                  REQUIRED_ARG,             0,
    0,                        0,                        0,
    0,                        0 },
  { "report-port", OPT_REPORT_PORT,
    ...
    MYSQL_PORT, 0, 0, 0, 0, 0 },
  { "report-user",        OPT_REPORT_USER,      "Undocumented.",
    (gptr *)&report_user, (gptr *)&report_user, 0,
    GET_STR,              REQUIRED_ARG,         0,
    0,                    0,                    0,
    0,                    0 },
  { "rpl-recovery-rank",        OPT_RPL_RECOVERY_RANK,      "Undocumented.",
    (gptr *)&rpl_recovery_rank, (gptr *)&rpl_recovery_rank, 0,
    GET_ULONG,                  REQUIRED_ARG,               0,
    0,                          0,                          0,
    0,                          0 },
  { "safe-mode", OPT_SAFE,   "Skip some optimize stages (for testing).", 0, 0,
    0,           GET_NO_ARG, NO_ARG,                                     0, 0,
    0,           0,          0,                                          0 },
#ifndef TO_BE_DELETED
  { "safe-show-database",                                     OPT_SAFE_SHOW_DB,
    "Deprecated option; use GRANT SHOW DATABASES instead...", 0,
    0,                                                        0,
    GET_NO_ARG,                                               NO_ARG,
    0,                                                        0,
    0,                                                        0,
  ...
  { "secure-auth",
    ...
    0,
    0 },
  { "server-id", OPT_SERVER_ID, "Uniquely identifies the server instance in "
                                "the community of replication partners.",
    (gptr *)&server_id, (gptr *)&server_id, 0, GET_ULONG, REQUIRED_ARG, 0, 0, 0,
    0, 0, 0 },
  { "set-variable", 'O', "Change the value of a variable. Please note that "
                         "this option is deprecated;you can set variables "
                         "directly with --variable-name=value.",
    0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
#ifdef HAVE_SMEM
  { "shared-memory",                   OPT_ENABLE_SHARED_MEMORY,
    "Enable the shared memory.",       (gptr *)&opt_enable_shared_memory,
    (gptr *)&opt_enable_shared_memory, 0,
    GET_BOOL,                          NO_ARG,
    0,                                 0,
    0,                                 0,
    0,                                 0 },
  ...
  { "show-slave-auth-info",
    ...
    0,
    0 },
  { "skip-grant-tables", OPT_SKIP_GRANT, "Start without grant tables. This "
                                         "gives all users FULL ACCESS to all "
                                         "tables!",
    (gptr *)&opt_noacl, (gptr *)&opt_noacl, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
    0 },
  { "skip-host-cache", OPT_SKIP_HOST_CACHE, "Don't cache host names.", 0, 0,
    0,                 GET_NO_ARG,          NO_ARG,                    0, 0,
    0,                 0,                   0,                         0 },
  { "skip-locking",                                            OPT_SKIP_LOCK,
    "Deprecated option, use --skip-external-locking instead.", 0,
    0,                                                         0,
    GET_NO_ARG,                                                NO_ARG,
    0,                                                         0,
    0,                                                         0,
  ...
  { "skip-networking",                     OPT_SKIP_NETWORKING,
    "Don't allow connection with TCP/IP.", 0,
    0,                                     0,
    GET_NO_ARG,                            NO_ARG,
    0,                                     0,
    0,                                     0,
    0,                                     0 },
  { "skip-new", OPT_SKIP_NEW, "Don't use new, possible wrong routines.", 0, 0,
    0,          GET_NO_ARG,   NO_ARG,                                    0, 0,
    0,          0,            0,                                         0 },
#ifndef DBUG_OFF
#ifdef SAFEMALLOC
  { "skip-safemalloc",                           OPT_SKIP_SAFEMALLOC,
    "Don't use the memory allocation checking.", 0,
    0,                                           0,
    GET_NO_ARG,                                  NO_ARG,
    0,                                           0,
    0,                                           0,
  ...
  { "skip-show-database",                    OPT_SKIP_SHOW_DB,
    "Don't allow 'SHOW DATABASE' commands.", 0,
    0,                                       0,
    GET_NO_ARG,                              NO_ARG,
    0,                                       0,
    0,                                       0,
  ...
  { "skip-slave-start",                  OPT_SKIP_SLAVE_START,
    ...
    (gptr *)&opt_skip_slave_start,       0,
    GET_BOOL,                            NO_ARG,
    0,                                   0,
    0,                                   0,
    0,                                   0 },
  { "skip-stack-trace",                      OPT_SKIP_STACK_TRACE,
    "Don't print a stack trace on failure.", 0,
    0,                                       0,
    GET_NO_ARG,                              NO_ARG,
    0,                                       0,
    0,                                       0,
  ...
  { "skip-thread-priority",                     OPT_SKIP_PRIOR,
    "Don't give threads different priorities.", 0,
    0,                                          0,
    GET_NO_ARG,                                 NO_ARG,
    0,                                          0,
    0,                                          0,
...
log and this option does nothing anymore.",
  ...
  { "sql-mode", OPT_SQL_MODE, "Syntax: sql-mode=option[,option[,option...]] "
...
#include "sslopt-longopts.h"
  ...
  { "standalone",                                          OPT_STANDALONE,
    "Dummy option to start as a standalone program (NT).", 0,
    0,                                                     0,
    GET_NO_ARG,                                            NO_ARG,
    0,                                                     0,
    0,                                                     0,
...
#endif
  { "symbolic-links",                's',
    "Enable symbolic link support.", (gptr *)&my_use_symdir,
    (gptr *)&my_use_symdir,          0,
    GET_BOOL,                        NO_ARG,
    IF_PURIFY(0, 1),                 0,
    0,                               0,
    0,                               0 },
  { "tc-heuristic-recover", OPT_TC_HEURISTIC_RECOVER,
    ...
    "or ROLLBACK",
    (gptr *)&opt_tc_heuristic_recover, (gptr *)&opt_tc_heuristic_recover, 0,
    GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  { "temp-pool", OPT_TEMP_POOL, "Using this option will cause most temporary "
                                "files created to use a small set of names, "
                                "rather than a unique name for each new file.",
    (gptr *)&use_temp_pool, (gptr *)&use_temp_pool, 0, GET_BOOL, NO_ARG, 1, 0,
    0, 0, 0, 0 },
  { "timed_mutexes", OPT_TIMED_MUTEXES, "Specify whether to time mutexes (only "
                                        "InnoDB mutexes are currently "
                                        "supported)",
    (gptr *)&timed_mutexes, (gptr *)&timed_mutexes, 0, GET_BOOL, NO_ARG, 0, 0,
    0, 0, 0, 0 },
  { "tmpdir", 't',
    ...
    (gptr *)&opt_mysql_tmpdir, (gptr *)&opt_mysql_tmpdir, 0, GET_STR,
    REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  ...
  { "user",       'u', "Run mysqld daemon as user.", 0, 0, 0, GET_STR,
    REQUIRED_ARG, 0,   0,                            0, 0, 0, 0 },
  { "verbose",                                   'v',
    "Used with --help option for detailed help", (gptr *)&opt_verbose,
    (gptr *)&opt_verbose,                        0,
    GET_BOOL,                                    NO_ARG,
    0,                                           0,
    0,                                           0,
  ...
  { "warnings",
    ...
    0,
    0 },
  { "back_log", OPT_BACK_LOG, "The number of outstanding connection requests "
                              "MySQL can have. This comes into play when the "
                              "main MySQL thread gets very many connection "
                              "requests in a very short time.",
    (gptr *)&back_log, (gptr *)&back_log, 0, GET_ULONG, REQUIRED_ARG, 50, 1,
    65535, 0, 1, 0 },
#ifdef HAVE_BERKELEY_DB
  { "bdb_cache_size",
    OPT_BDB_CACHE_SIZE,
    "The buffer that is allocated to cache index and rows for BDB tables.",
    (gptr *)&berkeley_cache_size,
    (gptr *)&berkeley_cache_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
  ...
  { "bdb_lock_max",              OPT_BDB_MAX_LOCK,
    "Synonym for bdb_max_lock.", (gptr *)&berkeley_max_lock,
    (gptr *)&berkeley_max_lock,  0,
    GET_ULONG,                   REQUIRED_ARG,
    10000,                       0,
    (long)~0,                    0,
    1,                           0 },
  ...
  { "bulk_insert_buffer_size", OPT_BULK_INSERT_BUFFER_SIZE,
    ...
    (gptr *)&max_system_variables.bulk_insert_buff_size, 0, GET_ULONG,
    REQUIRED_ARG, 8192 * 1024, 0, ~0L, 0, 1, 0 },
  ...
  { "expire_logs_days", OPT_EXPIRE_LOGS_DAYS,
    ...
    (gptr *)&expire_logs_days, (gptr *)&expire_logs_days, 0, GET_ULONG,
  ...
  { "group_concat_max_len",
    ...
    (gptr *)&max_system_variables.group_concat_max_len,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    1024,
  ...
  { "innodb_buffer_pool_awe_mem_mb", OPT_INNODB_BUFFER_POOL_AWE_MEM_MB,
    ...
    (gptr *)&innobase_buffer_pool_awe_mem_mb, 0, GET_LONG, REQUIRED_ARG, 0, 0,
    63000, 0, 1, 0 },
  { "innodb_buffer_pool_size", OPT_INNODB_BUFFER_POOL_SIZE,
    "The size of the memory buffer InnoDB uses to cache data and indexes of "
    "its tables.",
    (gptr *)&innobase_buffer_pool_size, (gptr *)&innobase_buffer_pool_size, 0,
    GET_LONG, REQUIRED_ARG, 8 * 1024 * 1024L, 1024 * 1024L, ~0L, 0,
    1024 * 1024L, 0 },
  ...
  { "innodb_file_io_threads",                OPT_INNODB_FILE_IO_THREADS,
    ...
    (gptr *)&innobase_file_io_threads,       0,
    GET_LONG,                                REQUIRED_ARG,
    4,                                       4,
    64,                                      0,
    1,                                       0 },
  ...
  { "innodb_lock_wait_timeout", OPT_INNODB_LOCK_WAIT_TIMEOUT,
    ...
    "rolled back.",
    (gptr *)&innobase_lock_wait_timeout, (gptr *)&innobase_lock_wait_timeout, 0,
    GET_LONG, REQUIRED_ARG, 50, 1, 1024 * 1024 * 1024, 0, 1, 0 },
  ...
  { "innodb_thread_sleep_delay", OPT_INNODB_THREAD_SLEEP_DELAY,
    ...
    " disable a sleep",
    (gptr *)&srv_thread_sleep_delay, (gptr *)&srv_thread_sleep_delay, 0,
    GET_LONG, REQUIRED_ARG, 10000L, 0L, ~0L, 0, 1L, 0 },
#endif /* HAVE_INNOBASE_DB */
  { "interactive_timeout", OPT_INTERACTIVE_TIMEOUT,
    "The number of seconds the server waits for activity on an interactive "
    "connection before closing it.",
    (gptr *)&global_system_variables.net_interactive_timeout,
    (gptr *)&max_system_variables.net_interactive_timeout, 0, GET_ULONG,
    REQUIRED_ARG, NET_WAIT_TIMEOUT, 1, LONG_TIMEOUT, 0, 1, 0 },
  { "join_buffer_size",
    OPT_JOIN_BUFF_SIZE,
    "The size of the buffer that is used for full joins.",
    (gptr *)&global_system_variables.join_buff_size,
    (gptr *)&max_system_variables.join_buff_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    128 * 1024L,
    IO_SIZE * 2 + MALLOC_OVERHEAD,
    ~0L,
    MALLOC_OVERHEAD,
    IO_SIZE,
    0 },
  { "key_buffer_size", OPT_KEY_BUFFER_SIZE,
    ...
    "is quite common.",
    (gptr *)&dflt_key_cache_var.param_buff_size, (gptr *)0, 0,
    (GET_ULL | GET_ASK_ADDR), REQUIRED_ARG, KEY_CACHE_SIZE, MALLOC_OVERHEAD,
    ~(ulong)0, MALLOC_OVERHEAD, IO_SIZE, 0 },
  { "key_cache_age_threshold", OPT_KEY_CACHE_AGE_THRESHOLD,
    "This characterizes the number of hits a hot block has to be untouched "
    "until it is considered aged enough to be downgraded to a warm block. This "
    "specifies the percentage ratio of that number of hits to the total number "
    "of blocks in key cache",
    (gptr *)&dflt_key_cache_var.param_age_threshold, (gptr *)0, 0,
    (GET_ULONG | GET_ASK_ADDR), REQUIRED_ARG, 300, 100, ~0L, 0, 100, 0 },
  ...
  { "key_cache_division_limit",
    ...
    1,
    0 },
  { "long_query_time", OPT_LONG_QUERY_TIME, "Log all queries that have taken "
                                            "more than long_query_time seconds "
                                            "to execute to file.",
    (gptr *)&global_system_variables.long_query_time,
    (gptr *)&max_system_variables.long_query_time, 0, GET_ULONG, REQUIRED_ARG,
    10, 1, LONG_TIMEOUT, 0, 1, 0 },
  ...
  { "max_allowed_packet",
    ...
    (gptr *)&max_system_variables.max_allowed_packet,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    1024 * 1024L,
...
The minimum value for this variable is 4096.",
  ...
  { "max_delayed_threads", OPT_MAX_DELAYED_THREADS,
    ...
    (gptr *)&max_system_variables.max_insert_delayed_threads, 0, GET_ULONG,
    REQUIRED_ARG, 20, 0, 16384, 0, 1, 0 },
  { "max_error_count",
    OPT_MAX_ERROR_COUNT,
  ...
  { "max_heap_table_size",
    ...
    (gptr *)&max_system_variables.max_heap_table_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    16 * 1024 * 1024L,
    ...
    1024,
    0 },
  { "max_join_size", OPT_MAX_JOIN_SIZE, "Joins that are probably going to read "
  ...
  { "max_length_for_sort_data",
    ...
    (gptr *)&max_system_variables.max_length_for_sort_data,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    1024,
  ...
  { "max_seeks_for_key",
    ...
    (gptr *)&max_system_variables.max_seeks_for_key,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    ~0L,
  ...
  { "max_sort_length", OPT_MAX_SORT_LENGTH,
    ...
    (gptr *)&max_system_variables.max_sort_length, 0, GET_ULONG, REQUIRED_ARG,
    1024, 4, 8192 * 1024L, 0, 1, 0 },
  { "max_tmp_tables",
    OPT_MAX_TMP_TABLES,
  ...
  { "multi_range_count",
    ...
    (gptr *)&max_system_variables.multi_range_count,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    256,
  ...
  { "myisam_max_sort_file_size", OPT_MYISAM_MAX_SORT_FILE_SIZE,
    ...
    (gptr *)&max_system_variables.myisam_max_sort_file_size, 0, GET_ULL,
    REQUIRED_ARG, (longlong)LONG_MAX, 0, (ulonglong)MAX_FILE_SIZE, 0,
    1024 * 1024, 0 },
  { "myisam_repair_threads", OPT_MYISAM_REPAIR_THREADS,
    "Number of threads to use when repairing MyISAM tables. The value of 1 "
    "disables parallel repair.",
    (gptr *)&global_system_variables.myisam_repair_threads,
    (gptr *)&max_system_variables.myisam_repair_threads, 0, GET_ULONG,
    REQUIRED_ARG, 1, 1, ~0L, 0, 1, 0 },
  { "myisam_sort_buffer_size", OPT_MYISAM_SORT_BUFFER_SIZE,
    "The buffer that is allocated when sorting the index when doing a REPAIR "
    "or when creating indexes with CREATE INDEX or ALTER TABLE.",
    (gptr *)&global_system_variables.myisam_sort_buff_size,
    (gptr *)&max_system_variables.myisam_sort_buff_size, 0, GET_ULONG,
    REQUIRED_ARG, 8192 * 1024, 4, ~0L, 0, 1, 0 },
  ...
  { "net_buffer_length",
    ...
    (gptr *)&max_system_variables.net_buffer_length,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    16384,
    ...
    1024,
    0 },
  { "net_read_timeout", OPT_NET_READ_TIMEOUT, "Number of seconds to wait for "
                                              "more data from a connection "
                                              "before aborting the read.",
    (gptr *)&global_system_variables.net_read_timeout,
    (gptr *)&max_system_variables.net_read_timeout, 0, GET_ULONG, REQUIRED_ARG,
    NET_READ_TIMEOUT, 1, LONG_TIMEOUT, 0, 1, 0 },
  { "net_retry_count", OPT_NET_RETRY_COUNT, "If a read on a communication port "
                                            "is interrupted, retry this many "
                                            "times before giving up.",
    (gptr *)&global_system_variables.net_retry_count,
    (gptr *)&max_system_variables.net_retry_count, 0, GET_ULONG, REQUIRED_ARG,
    MYSQLD_NET_RETRY_COUNT, 1, ~0L, 0, 1, 0 },
  { "net_write_timeout", OPT_NET_WRITE_TIMEOUT,
    "Number of seconds to wait for a block to be written to a connection  "
    "before aborting the write.",
    (gptr *)&global_system_variables.net_write_timeout,
    (gptr *)&max_system_variables.net_write_timeout, 0, GET_ULONG, REQUIRED_ARG,
    NET_WRITE_TIMEOUT, 1, LONG_TIMEOUT, 0, 1, 0 },
  ...
  { "optimizer_prune_level", OPT_OPTIMIZER_PRUNE_LEVEL,
    ...
    (gptr *)&max_system_variables.optimizer_prune_level, 0, GET_ULONG, OPT_ARG,
    1, 0, 1, 0, 1, 0 },
  { "optimizer_search_depth", OPT_OPTIMIZER_SEARCH_DEPTH,
    ...
    (gptr *)&max_system_variables.optimizer_search_depth, 0, GET_ULONG, OPT_ARG,
    MAX_TABLES + 1, 0, MAX_TABLES + 2, 0, 1, 0 },
  { "preload_buffer_size",
    OPT_PRELOAD_BUFFER_SIZE,
    "The size of the buffer that is allocated when preloading indexes",
    (gptr *)&global_system_variables.preload_buff_size,
    (gptr *)&max_system_variables.preload_buff_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    32 * 1024L,
  ...
  { "query_alloc_block_size",
    ...
    (gptr *)&max_system_variables.query_alloc_block_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    QUERY_ALLOC_BLOCK_SIZE,
  ...
  { "query_cache_min_res_unit", OPT_QUERY_CACHE_MIN_RES_UNIT,
    ...
    (gptr *)&query_cache_min_res_unit, (gptr *)&query_cache_min_res_unit, 0,
    GET_ULONG, REQUIRED_ARG, QUERY_CACHE_MIN_RESULT_DATA_SIZE, 0,
    (longlong)ULONG_MAX, 0, 1, 0 },
#endif /*HAVE_QUERY_CACHE*/
  { "query_cache_size",
    OPT_QUERY_CACHE_SIZE,
    "The memory allocated to store results from old queries.",
    (gptr *)&query_cache_size,
    (gptr *)&query_cache_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
  ...
  { "query_cache_type", OPT_QUERY_CACHE_TYPE,
    ...
    (gptr *)&max_system_variables.query_cache_type, 0, GET_ULONG, REQUIRED_ARG,
    1, 0, 2, 0, 1, 0 },
  ...
  { "query_prealloc_size",
    ...
    (gptr *)&max_system_variables.query_prealloc_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
  ...
  { "range_alloc_block_size",
    ...
    (gptr *)&max_system_variables.range_alloc_block_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    RANGE_ALLOC_BLOCK_SIZE,
  ...
  { "read_buffer_size", OPT_RECORD_BUFFER,
    ...
    0 },
  { "read_only", OPT_READONLY, "Make all tables readonly, with the exception "
                               "for replication (slave) threads and users with "
                               "the SUPER privilege",
    (gptr *)&opt_readonly, (gptr *)&opt_readonly, 0, GET_BOOL, NO_ARG, 0, 0, 1,
    0, 1, 0 },
  { "read_rnd_buffer_size", OPT_RECORD_RND_BUFFER,
    ...
    (gptr *)&max_system_variables.read_rnd_buff_size, 0, GET_ULONG,
    REQUIRED_ARG, 256 * 1024L, IO_SIZE * 2 + MALLOC_OVERHEAD, ~0L,
    MALLOC_OVERHEAD, IO_SIZE, 0 },
  { "div_precision_increment",
    OPT_DIV_PRECINCREMENT,
    "Precision of the result of '/' operator will be increased on that value.",
    (gptr *)&global_system_variables.div_precincrement,
    (gptr *)&max_system_variables.div_precincrement,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    4,
  ...
  { "record_buffer",
    ...
    (gptr *)&global_system_variables.read_buff_size,
    (gptr *)&max_system_variables.read_buff_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    128 * 1024L,
    ...
    0 },
#ifdef HAVE_REPLICATION
  { "relay_log_purge", OPT_RELAY_LOG_PURGE, "0 = do not purge relay logs. 1 = "
                                            "purge them as soon as they are no "
                                            "more needed.",
    (gptr *)&relay_log_purge, (gptr *)&relay_log_purge, 0, GET_BOOL, NO_ARG, 1,
    0, 1, 0, 1, 0 },
  { "relay_log_space_limit",                    OPT_RELAY_LOG_SPACE_LIMIT,
    "Maximum space to use for all relay logs.", (gptr *)&relay_log_space_limit,
    (gptr *)&relay_log_space_limit,             0,
    GET_ULL,                                    REQUIRED_ARG,
    0L,                                         0L,
    (longlong)ULONG_MAX,                        0,
    1,                                          0 },
  ...
  { "slave_net_timeout", OPT_SLAVE_NET_TIMEOUT,
    ...
    "before aborting the read.",
    (gptr *)&slave_net_timeout, (gptr *)&slave_net_timeout, 0, GET_ULONG,
    REQUIRED_ARG, SLAVE_NET_TIMEOUT, 1, LONG_TIMEOUT, 0, 1, 0 },
  { "slave_transaction_retries", OPT_SLAVE_TRANS_RETRIES,
    "Number of times the slave SQL thread will retry a transaction in case "
    "it failed with a deadlock or elapsed lock wait timeout, "
    "before giving up and stopping.",
    (gptr *)&slave_trans_retries, (gptr *)&slave_trans_retries, 0, GET_ULONG,
    REQUIRED_ARG, 10L, 0L, (longlong)ULONG_MAX, 0, 1, 0 },
#endif /* HAVE_REPLICATION */
  ...
  { "sort_buffer_size",
    ...
    (gptr *)&max_system_variables.sortbuff_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    MAX_SORT_MEMORY,
    MIN_SORT_MEMORY + MALLOC_OVERHEAD * 2,
    ~0L,
    MALLOC_OVERHEAD,
    1,
    0 },
  ...
  { "sync-frm",                                         OPT_SYNC_FRM,
    "Sync .frm to disk on create. Enabled by default.", (gptr *)&opt_sync_frm,
    (gptr *)&opt_sync_frm,                              0,
    GET_BOOL,                                           NO_ARG,
    1,                                                  0,
    0,                                                  0,
  ...
  { "table_lock_wait_timeout", OPT_TABLE_LOCK_WAIT_TIMEOUT,
    ...
    (gptr *)&table_lock_wait_timeout, (gptr *)&table_lock_wait_timeout, 0,
    GET_ULONG, REQUIRED_ARG, 50, 1, 1024 * 1024 * 1024, 0, 1, 0 },
  { "thread_cache_size",
    OPT_THREAD_CACHE_SIZE,
    "How many threads we should keep in a cache for reuse.",
    (gptr *)&thread_cache_size,
    (gptr *)&thread_cache_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
  ...
  { "thread_stack",                    OPT_THREAD_STACK,
    ...
    (gptr *)&thread_stack,             0,
    GET_ULONG,                         REQUIRED_ARG,
    DEFAULT_THREAD_STACK,              1024L * 128L,
    ~0L,                               0,
    1024,                              0 },
  { "time_format",
    OPT_TIME_FORMAT,
  ...
  { "tmp_table_size", OPT_TMP_TABLE_SIZE,
    ...
    (gptr *)&max_system_variables.tmp_table_size, 0, GET_ULONG, REQUIRED_ARG,
    32 * 1024 * 1024L, 1024, ~0L, 0, 1, 0 },
  { "transaction_alloc_block_size",
    OPT_TRANS_ALLOC_BLOCK_SIZE,
    "Allocation block size for transactions to be stored in binary log",
    (gptr *)&global_system_variables.trans_alloc_block_size,
    (gptr *)&max_system_variables.trans_alloc_block_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
    QUERY_ALLOC_BLOCK_SIZE,
  ...
  { "transaction_prealloc_size",
    ...
    (gptr *)&max_system_variables.trans_prealloc_size,
    0,
    GET_ULONG,
    REQUIRED_ARG,
  ...
  { "updatable_views_with_limit", OPT_UPDATABLE_VIEWS_WITH_LIMIT,
    ...
    (gptr *)&max_system_variables.updatable_views_with_limit, 0, GET_ULONG,
    REQUIRED_ARG, 1, 0, 1, 0, 1, 0 },
  { "wait_timeout", OPT_WAIT_TIMEOUT, "The number of seconds the server waits "
                                      "for activity on a connection before "
                                      "closing it.",
    (gptr *)&global_system_variables.net_wait_timeout,
    (gptr *)&max_system_variables.net_wait_timeout, 0, GET_ULONG, REQUIRED_ARG,
    NET_WAIT_TIMEOUT, 1, IF_WIN(INT_MAX32 / 1000, LONG_TIMEOUT), 0, 1, 0 },
...
struct show_var_st status_vars[] = {
  ...
  { "Com_admin_commands", (char *)offsetof(STATUS_VAR, com_other),
    SHOW_LONG_STATUS },
  { "Com_alter_db",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_ALTER_DB]),
    SHOW_LONG_STATUS },
  { "Com_alter_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_ALTER_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_analyze", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_ANALYZE]),
    SHOW_LONG_STATUS },
  { "Com_backup_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_BACKUP_TABLE]),
    SHOW_LONG_STATUS },
  ...
  { "Com_change_db",
    ...
    SHOW_LONG_STATUS },
  { "Com_change_master",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CHANGE_MASTER]),
    SHOW_LONG_STATUS },
  { "Com_check", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CHECK]),
    SHOW_LONG_STATUS },
  { "Com_checksum",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CHECKSUM]),
    SHOW_LONG_STATUS },
  { "Com_commit", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_COMMIT]),
    SHOW_LONG_STATUS },
  { "Com_create_db",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CREATE_DB]),
    SHOW_LONG_STATUS },
  { "Com_create_function",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CREATE_FUNCTION]),
    SHOW_LONG_STATUS },
  { "Com_create_index",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CREATE_INDEX]),
    SHOW_LONG_STATUS },
  { "Com_create_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CREATE_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_dealloc_sql",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DEALLOCATE_PREPARE]),
    SHOW_LONG_STATUS },
  { "Com_delete", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DELETE]),
    SHOW_LONG_STATUS },
  { "Com_delete_multi",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DELETE_MULTI]),
    SHOW_LONG_STATUS },
  { "Com_do", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DO]),
    SHOW_LONG_STATUS },
  { "Com_drop_db", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DROP_DB]),
    SHOW_LONG_STATUS },
  { "Com_drop_function",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DROP_FUNCTION]),
    SHOW_LONG_STATUS },
  { "Com_drop_index",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DROP_INDEX]),
    SHOW_LONG_STATUS },
  { "Com_drop_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DROP_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_drop_user",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_DROP_USER]),
    SHOW_LONG_STATUS },
  { "Com_execute_sql",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_EXECUTE]),
    SHOW_LONG_STATUS },
  { "Com_flush", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_FLUSH]),
    SHOW_LONG_STATUS },
  { "Com_grant", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_GRANT]),
    SHOW_LONG_STATUS },
  { "Com_ha_close",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_HA_CLOSE]),
    SHOW_LONG_STATUS },
  { "Com_ha_open", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_HA_OPEN]),
    SHOW_LONG_STATUS },
  { "Com_ha_read", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_HA_READ]),
    SHOW_LONG_STATUS },
  { "Com_help", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_HELP]),
    SHOW_LONG_STATUS },
  { "Com_insert", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_INSERT]),
    SHOW_LONG_STATUS },
  { "Com_insert_select",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_INSERT_SELECT]),
    SHOW_LONG_STATUS },
  { "Com_kill", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_KILL]),
    SHOW_LONG_STATUS },
  { "Com_load", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_LOAD]),
    SHOW_LONG_STATUS },
  { "Com_load_master_data",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_LOAD_MASTER_DATA]),
    SHOW_LONG_STATUS },
  { "Com_load_master_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_LOAD_MASTER_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_lock_tables",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_LOCK_TABLES]),
    SHOW_LONG_STATUS },
  { "Com_optimize",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_OPTIMIZE]),
    SHOW_LONG_STATUS },
  { "Com_preload_keys",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_PRELOAD_KEYS]),
    SHOW_LONG_STATUS },
  { "Com_prepare_sql",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_PREPARE]),
    SHOW_LONG_STATUS },
  { "Com_purge", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_PURGE]),
    SHOW_LONG_STATUS },
  { "Com_purge_before_date",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_PURGE_BEFORE]),
    SHOW_LONG_STATUS },
  { "Com_rename_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RENAME_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_repair", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_REPAIR]),
    SHOW_LONG_STATUS },
  { "Com_replace", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_REPLACE]),
    SHOW_LONG_STATUS },
  { "Com_replace_select",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_REPLACE_SELECT]),
    SHOW_LONG_STATUS },
  { "Com_reset", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RESET]),
    SHOW_LONG_STATUS },
  { "Com_restore_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RESTORE_TABLE]),
    SHOW_LONG_STATUS },
  { "Com_revoke", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_REVOKE]),
    SHOW_LONG_STATUS },
  { "Com_revoke_all",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_REVOKE_ALL]),
    SHOW_LONG_STATUS },
  { "Com_rollback",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_ROLLBACK]),
    SHOW_LONG_STATUS },
  { "Com_savepoint",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SAVEPOINT]),
    SHOW_LONG_STATUS },
  ...
  { "Com_show_binlog_events",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_BINLOG_EVENTS]),
    SHOW_LONG_STATUS },
  { "Com_show_binlogs",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_BINLOGS]),
    SHOW_LONG_STATUS },
  { "Com_show_charsets",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_CHARSETS]),
    SHOW_LONG_STATUS },
  { "Com_show_collations",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_COLLATIONS]),
    SHOW_LONG_STATUS },
  { "Com_show_column_types",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_COLUMN_TYPES]),
    SHOW_LONG_STATUS },
  { "Com_show_create_db",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_CREATE_DB]),
    SHOW_LONG_STATUS },
  { "Com_show_create_table",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_CREATE]),
    SHOW_LONG_STATUS },
  { "Com_show_databases",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_DATABASES]),
    SHOW_LONG_STATUS },
  { "Com_show_errors",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_ERRORS]),
    SHOW_LONG_STATUS },
  { "Com_show_fields",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_FIELDS]),
    SHOW_LONG_STATUS },
  { "Com_show_grants",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_GRANTS]),
    SHOW_LONG_STATUS },
  { "Com_show_innodb_status",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_INNODB_STATUS]),
    SHOW_LONG_STATUS },
  { "Com_show_keys",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_KEYS]),
    SHOW_LONG_STATUS },
  { "Com_show_logs",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_LOGS]),
    SHOW_LONG_STATUS },
  { "Com_show_master_status",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_MASTER_STAT]),
    SHOW_LONG_STATUS },
  { "Com_show_ndb_status",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_NDBCLUSTER_STATUS]),
    SHOW_LONG_STATUS },
  { "Com_show_new_master",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_NEW_MASTER]),
    SHOW_LONG_STATUS },
  { "Com_show_open_tables",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_OPEN_TABLES]),
    SHOW_LONG_STATUS },
  { "Com_show_privileges",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_PRIVILEGES]),
    SHOW_LONG_STATUS },
  { "Com_show_processlist",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_PROCESSLIST]),
    SHOW_LONG_STATUS },
  { "Com_show_slave_hosts",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_SLAVE_HOSTS]),
    SHOW_LONG_STATUS },
  { "Com_show_slave_status",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_SLAVE_STAT]),
    SHOW_LONG_STATUS },
  { "Com_show_status",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_STATUS]),
    SHOW_LONG_STATUS },
  { "Com_show_storage_engines",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_STORAGE_ENGINES]),
    SHOW_LONG_STATUS },
  { "Com_show_tables",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_TABLES]),
    SHOW_LONG_STATUS },
  { "Com_show_triggers",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_TRIGGERS]),
    SHOW_LONG_STATUS },
  { "Com_show_variables",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_VARIABLES]),
    SHOW_LONG_STATUS },
  { "Com_show_warnings",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_WARNS]),
    SHOW_LONG_STATUS },
  { "Com_slave_start",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SLAVE_START]),
    SHOW_LONG_STATUS },
  { "Com_slave_stop",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SLAVE_STOP]),
    SHOW_LONG_STATUS },
  ...
  { "Com_stmt_send_long_data",
    (char *)offsetof(STATUS_VAR, com_stmt_send_long_data), SHOW_LONG_STATUS },
  { "Com_truncate",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_TRUNCATE]),
    SHOW_LONG_STATUS },
  { "Com_unlock_tables",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_UNLOCK_TABLES]),
    SHOW_LONG_STATUS },
  { "Com_update", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_UPDATE]),
    SHOW_LONG_STATUS },
  { "Com_update_multi",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_UPDATE_MULTI]),
    SHOW_LONG_STATUS },
  { "Com_xa_commit",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_COMMIT]),
    SHOW_LONG_STATUS },
  { "Com_xa_end", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_END]),
    SHOW_LONG_STATUS },
  { "Com_xa_prepare",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_PREPARE]),
    SHOW_LONG_STATUS },
  { "Com_xa_recover",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_RECOVER]),
    SHOW_LONG_STATUS },
  { "Com_xa_rollback",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_ROLLBACK]),
    SHOW_LONG_STATUS },
  { "Com_xa_start",
    (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_XA_START]),
    SHOW_LONG_STATUS },
  { "Connections", (char *)&thread_id, SHOW_LONG_CONST },
  { "Created_tmp_disk_tables",
    (char *)offsetof(STATUS_VAR, created_tmp_disk_tables), SHOW_LONG_STATUS },
  { "Created_tmp_files", (char *)&my_tmp_file_created, SHOW_LONG },
  { "Created_tmp_tables", (char *)offsetof(STATUS_VAR, created_tmp_tables),
    SHOW_LONG_STATUS },
  ...
  { "Handler_read_rnd_next",
    (char *)offsetof(STATUS_VAR, ha_read_rnd_next_count), SHOW_LONG_STATUS },
  ...
  { "Handler_savepoint_rollback",
    (char *)offsetof(STATUS_VAR, ha_savepoint_rollback_count),
    SHOW_LONG_STATUS },
  ...
  { "Key_writes", (char *)&dflt_key_cache_var.global_cache_write,
    SHOW_KEY_CACHE_LONGLONG },
  { "Last_query_cost", (char *)offsetof(STATUS_VAR, last_query_cost),
    SHOW_DOUBLE_STATUS },
  ...
  { "Select_full_range_join",
    (char *)offsetof(STATUS_VAR, select_full_range_join_count),
    SHOW_LONG_STATUS },
  { "Select_range", (char *)offsetof(STATUS_VAR, select_range_count),
    SHOW_LONG_STATUS },
  { "Select_range_check",
    (char *)offsetof(STATUS_VAR, select_range_check_count), SHOW_LONG_STATUS },
  ...
  { "Sort_merge_passes", (char *)offsetof(STATUS_VAR, filesort_merge_passes),
    SHOW_LONG_STATUS },
  { "Sort_range", (char *)offsetof(STATUS_VAR, filesort_range_count),
    SHOW_LONG_STATUS },
  { "Sort_rows", (char *)offsetof(STATUS_VAR, filesort_rows),
    SHOW_LONG_STATUS },
  { "Sort_scan", (char *)offsetof(STATUS_VAR, filesort_scan_count),
    SHOW_LONG_STATUS },
#ifdef HAVE_OPENSSL
  { "Ssl_accept_renegotiates", (char *)0,
  ...
  { "Ssl_connect_renegotiates", (char *)0,
  ...
  { "Uptime", (char *)0, SHOW_STARTTIME }, { NullS, NullS, SHOW_LONG }
};
...
static void mysql_init_variables(void) {
  ...
  opt_date_time_formats[0] = opt_date_time_formats[1] =
      opt_date_time_formats[2] = 0;
  ...
  master_ssl_key = master_ssl_cert = master_ssl_ca = master_ssl_capath =
      master_ssl_cipher = 0;
...
static my_bool get_one_option(int optid, const struct my_option *opt
  ...
  case 'u':
    if (!mysqld_user || !strcmp(mysqld_user, argument))
      ...
      sql_print_warning("Ignoring user change to '%s' because the user was set "
...
#include <sslopt-case.h>
...
#if defined(__NETWARE__)
    sql_perror("Can't start server: skip-networking option is currently not "
  ...
  case OPT_STORAGE_ENGINE: {
    if ((enum db_type)((global_system_variables.table_type = ha_resolve_by_name(
                            argument, strlen(argument)))) == DB_TYPE_UNKNOWN) {
  ...
  case OPT_BDB_LOCK: {
    ...
    if ((type = find_type(argument, &berkeley_lock_typelib, 2)) > 0)
      ...
      long value =
          my_strntol(&my_charset_latin1, argument, length, 10, &end, &err);
  ...
  case OPT_NDB_NODEID: {
    int len = my_snprintf(opt_ndb_constrbuf + opt_ndb_constrbuf_len,
                          ...
                          optid == OPT_NDB_NODEID ? "nodeid=" : "", argument);
  ...
  case OPT_NDB_CONNECTSTRING:
    if (opt_ndb_connectstring && opt_ndb_connectstring[0])
      my_snprintf(opt_ndb_constrbuf + opt_ndb_constrbuf_len,
                  sizeof(opt_ndb_constrbuf) - opt_ndb_constrbuf_len, "%s%s",
                  opt_ndb_constrbuf_len > 0 ? "," : "", opt_ndb_connectstring);
    else
  ...
  case OPT_TC_HEURISTIC_RECOVER: {
    if ((tc_heuristic_recover =
             find_type(argument, &tc_heuristic_recover_typelib, 2)) <= 0) {
...
static gptr *mysql_getopt_value(const char *keyname, uint key_length,
...
static void create_pid_file() {
  File file;
  if ((file = my_create(pidfile_name, 0664, O_WRONLY | O_TRUNC, MYF(MY_WME))) >=
      0) {
    char buff[21], *end;
    end = int10_to_str((long)getpid(), buff, 10);
    *end++ = '\n';
    if (!my_write(file, (byte *)buff, (uint)(end - buff),
                  MYF(MY_WME | MY_NABP))) {
