...
void init_alloc_root(MEM_ROOT *mem_root, uint block_size, uint pre_alloc_size __attribute__((unused)))
{
  ...
  if (pre_alloc_size)
  {
    if ((mem_root->free = mem_root->pre_alloc =
             (USED_MEM *)my_malloc(pre_alloc_size + ALIGN_SIZE(sizeof(USED_MEM)), MYF(0))))
    {
...
gptr alloc_root(MEM_ROOT *mem_root, unsigned int Size)
{
  ...
  {
    if ((*prev)->left < Size && mem_root->first_block_usage++ >= ALLOC_MAX_BLOCK_USAGE_BEFORE_DROP &&
    ...
    for (next = *prev; next && next->left < Size; next = next->next) prev = &next->next;
  }
  if (!next)
...
char *strdup_root(MEM_ROOT *root, const char *str) { return strmake_root(root, str, (uint)strlen(str)); }
