...
Item_func::Item_func(THD *thd, Item_func *item)
    : Item_result_field(thd, item), allowed_arg_cols(item->allowed_arg_cols),
      arg_count(item->arg_count), used_tables_cache(item->used_tables_cache),
...
Field *Item_func::tmp_table_field(TABLE *t_arg) {
  ...
  case DECIMAL_RESULT:
    res =
        new Field_new_decimal(my_decimal_precision_to_length(
                                  decimal_precision(), decimals, unsigned_flag),
...
void Item_num_op::find_num_type(void) {
  ...
  if (r0 == REAL_RESULT || r1 == REAL_RESULT || r0 == STRING_RESULT ||
      r1 == STRING_RESULT) {
    count_real_length();
    max_length = float_length(decimals);
...
void Item_func_mul::result_precision() {
  ...
  int precision =
      min(args[0]->decimal_precision() + args[1]->decimal_precision(),
...
void Item_func_div::result_precision() {
  uint precision =
      min(args[0]->decimal_precision() + prec_increment, DECIMAL_MAX_PRECISION);
  /* Integer operations keep unsigned_flag if one of arguments is unsigned */
  if (result_type() == INT_RESULT)
    unsigned_flag = args[0]->unsigned_flag | args[1]->unsigned_flag;
  else
...
longlong Item_func_shift_left::val_int() {
  ...
  uint shift;
  ulonglong res =
      ((ulonglong)args[0]->val_int() << (shift = (uint)args[1]->val_int()));
...
void Item_func_int_val::fix_num_length_and_dec() {
  max_length =
      args[0]->max_length - (args[0]->decimals ? args[0]->decimals + 1 : 0) + 2;
  uint tmp = float_length(decimals);
  set_if_smaller(max_length, tmp);
  decimals = 0;
}
...
void Item_func_round::fix_length_and_dec() {
  ...
  case STRING_RESULT:
    ...
    break;
  case INT_RESULT:
    if (!decimals_to_set && (truncate || (args[0]->decimal_precision() <
...
void Item_func_min_max::fix_length_and_dec() {
  ...
  if (cmp_type == STRING_RESULT)
    ...
    max_length = my_decimal_precision_to_length(max_int_part + decimals,
...
longlong Item_func_locate::val_int() {
  ...
  if (!cmp_collation.collation->coll->instr(
           cmp_collation.collation, a->ptr() + start, a->length() - start,
           b->ptr(), b->length(), &match, 1))
    return 0;
...
longlong Item_func_ord::val_int() {
  ...
  if (use_mb(res->charset())) {
    ...
    register uint32 n = 0,
                    l = my_ismbchar(res->charset(), str, str + res->length());
    if (!l)
      ...
      if ((symbol_len = cs->cset->mb_wc(cs, &wc, (uchar *)str_end,
        ...
        if (is_separator || is_last_item) {
          ...
          if (is_last_item && !is_separator)
            str_end = substr_end;
          if (!my_strnncoll(cs, (const uchar *)str_begin, str_end - str_begin,
...
bool udf_handler::fix_fields(THD *thd, Item_result_field *func, uint arg_count,
  ...
  if ((f_args.arg_count = arg_count)) {
    if (!(f_args.arg_type = (Item_result *)sql_alloc(f_args.arg_count *
                                                     sizeof(Item_result)))) {
      free_udf(u_d);
      DBUG_RETURN(TRUE);
    }
  ...
  if (u_d->func_init) {
    ...
    my_bool (*init)(UDF_INIT *, UDF_ARGS *, char *) =
        (my_bool (*)(UDF_INIT *, UDF_ARGS *, char *))u_d->func_init;
...
String *udf_handler::val_str(String *str, String *save_str) {
  ...
  char *(*func)(UDF_INIT *, UDF_ARGS *, char *, ulong *, uchar *, uchar *) =
      (char *
       (*)(UDF_INIT *, UDF_ARGS *, char *, ulong *, uchar *, uchar *))u_d->func;
...
void item_user_lock_init(void) {
  ...
  hash_init(&hash_user_locks, system_charset_info, 16, 0, 0,
            (hash_get_key)ull_get_key, NULL, 0);
...
longlong Item_master_pos_wait::val_int() {
  ...
  if ((event_count =
           active_mi->rli.wait_for_pos(thd, log_name, pos, timeout)) == -2) {
    null_value = 1;
    event_count = 0;
  }
#endif
...
longlong Item_func_get_lock::val_int() {
  ...
  */
  ...
  if (!(ull = ((User_level_lock *)hash_search(
            &hash_user_locks, (byte *)res->ptr(), res->length())))) {
...
longlong Item_func_release_lock::val_int() {
  ...
  pthread_mutex_lock(&LOCK_user_locks);
  if (!(ull = ((User_level_lock *)hash_search(
            &hash_user_locks, (const byte *)res->ptr(), res->length())))) {
    null_value = 1;
  } else {
...
static user_var_entry *get_variable(HASH *hash, LEX_STRING &name,
  ...
  if (!(entry = (user_var_entry *)hash_search(hash, (byte *)name.str,
    ...
    if (!(entry = (user_var_entry *)my_malloc(size, MYF(MY_WME))))
      return 0;
    entry->name.str =
        (char *)entry + ALIGN_SIZE(sizeof(user_var_entry)) + extra_size;
...
static bool update_hash(user_var_entry *entry, bool set_null, void *ptr,
                        uint length, Item_result type, CHARSET_INFO *cs,
                        Derivation dv) {
  if (set_null) {
...
bool Item_func_set_user_var::update_hash(void *ptr, uint length,
  ...
  */
  ...
  if (::update_hash(entry, (null_value = args[0]->null_value), ptr, length,
                    type, cs, dv)) {
...
bool Item_func_set_user_var::update() {
  ...
  case DECIMAL_RESULT: {
    if (!save_result.vdec) // Null value
      ...
      res = update_hash((void *)save_result.vdec, sizeof(my_decimal),
                        DECIMAL_RESULT, &my_charset_bin, DERIVATION_IMPLICIT);
    break;
  }
  case ROW_RESULT:
  default:
  ...
  if (!var_entry) {
    ...
    */
    ...
    tmp_var_list.push_back(
        new set_var_user(new Item_func_set_user_var(name, new Item_null())));
    /* Create the variable */
...
my_decimal *
Item_user_var_as_out_param::val_decimal(my_decimal *decimal_buffer) {
  DBUG_ASSERT(0);
  return 0;
}
...
bool Item_func_match::fix_fields(THD *thd, Item **ref) {
  ...
  */
  ...
  return agg_arg_collations_for_comparison(cmp_collation, args + 1,
                                           arg_count - 1);
}
...
err:
  ...
  my_message(ER_FT_MATCHING_KEY_NOT_FOUND, ER(ER_FT_MATCHING_KEY_NOT_FOUND),
             MYF(0));
  return 1;
}
...
double Item_func_match::val_real() {
  ...
  if (key == NO_SUCH_KEY) {
    ...
    if ((null_value = (a == 0)))
      DBUG_RETURN(0);
    DBUG_RETURN(ft_handler->please->find_relevance(ft_handler, (byte *)a->ptr(),
                                                   a->length()));
  } else
    DBUG_RETURN(
...
Item *get_system_var(THD *thd, enum_var_type var_type, LEX_STRING name,
  ...
  if (component.str == 0 &&
    ...
    return new Item_string(NULL, server_version, (uint)strlen(server_version),
...
const char *Item_func_sp::func_name() const {
  ...
  uint len =
      ((m_name->m_db.length + m_name->m_name.length) * 2 + // characters*quoting
       2 +             // ` and `
       1 +             // .
       1 +             // end of string
...
bool Item_func_sp::find_and_check_access(THD *thd, ulong want_access,
  ...
  *save = 0; // Safety if error
  ...
  if (check_routine_access(thd, want_access, m_sp->m_db.str, m_sp->m_name.str,
                           0, FALSE))
    goto error;
  ...
  */
  if (*save && check_routine_access(thd, want_access, m_sp->m_db.str,
                                    m_sp->m_name.str, 0, FALSE)) {
