...
bool mysql_create_view(THD *thd, enum_view_create_mode mode) {
  ...
  if (lex->proc_list.first || lex->result) {
    my_error(ER_VIEW_SELECT_CLAUSE, MYF(0),
             (lex->result ? "INTO" : "PROCEDURE"));
    res = TRUE;
    goto err;
  }
  if (lex->derived_tables || lex->variables_used || lex->param_list.elements) {
    int err = (lex->derived_tables ? ER_VIEW_SELECT_DERIVED
  ...
  */
  if (strcmp(lex->create_view_definer->user.str,
      ...
      my_strcasecmp(system_charset_info, lex->create_view_definer->host.str,
    ...
    if (!(thd->security_ctx->master_access & SUPER_ACL)) {
      ...
      if (!is_acl_user(lex->create_view_definer->host.str,
        ...
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_NO_SUCH_USER,
                            ER(ER_NO_SUCH_USER),
                            lex->create_view_definer->user.str,
                            lex->create_view_definer->host.str);
      }
  ...
  */
  if ((check_access(thd, CREATE_VIEW_ACL, view->db, &view->grant.privilege, 0,
                    0, is_schema_db(view->db)) ||
       grant_option && check_grant(thd, CREATE_VIEW_ACL, view, 0, 1, 0)) ||
      (mode != VIEW_CREATE_NEW &&
       (check_access(thd, DROP_ACL, view->db, &view->grant.privilege, 0, 0,
                     is_schema_db(view->db)) ||
  ...
  if (&lex->select_lex != lex->all_selects_list) {
    ...
    for (tbl = tables; tbl; tbl = tbl->next_global) {
      if (!tbl->table_in_first_from_clause) {
        if (check_access(thd, SELECT_ACL, tbl->db, &tbl->grant.privilege, 0, 0,
  ...
  */
  for (tbl = lex->query_tables; tbl; tbl = tbl->next_global) {
    ...
    if (tbl->view && strcmp(tbl->view_db.str, view->db) == 0 &&
  ...
  */
  for (sl = select_lex; sl; sl = sl->next_select()) {
    ...
    while ((item = it++)) {
      Item_field *fld;
      uint priv = (get_column_grant(thd, &view->grant, db, view->table_name,
                                    item->name) &
                   VIEW_ANY_ACL);
      if ((fld = item->filed_for_view_update())) {
        ...
        */
        if (!fld->field->table->s->tmp_table &&
            (~fld->have_privileges & priv)) {
          /* VIEW column has more privileges */
          my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), "create view",
                   thd->security_ctx->priv_user, thd->security_ctx->priv_host,
                   item->name, view->table_name);
          res = TRUE;
          goto err;
        }
      }
...
static File_option view_parameters[] = {
  { {(char *)STRING_WITH_LEN("query") }, offsetof(TABLE_LIST, query),
    FILE_OPTIONS_STRING },
  { {(char *)STRING_WITH_LEN("md5") }, offsetof(TABLE_LIST, md5),
    FILE_OPTIONS_STRING },
  { {(char *)STRING_WITH_LEN("updatable") },
    offsetof(TABLE_LIST, updatable_view), FILE_OPTIONS_ULONGLONG },
  { {(char *)STRING_WITH_LEN("algorithm") }, offsetof(TABLE_LIST, algorithm),
    FILE_OPTIONS_ULONGLONG },
  { {(char *)STRING_WITH_LEN("definer_user") },
    offsetof(TABLE_LIST, definer.user), FILE_OPTIONS_STRING },
  { {(char *)STRING_WITH_LEN("definer_host") },
    offsetof(TABLE_LIST, definer.host), FILE_OPTIONS_STRING },
  { {(char *)STRING_WITH_LEN("suid") }, offsetof(TABLE_LIST, view_suid),
    FILE_OPTIONS_ULONGLONG },
  { {(char *)STRING_WITH_LEN("with_check_option") },
    offsetof(TABLE_LIST, with_check), FILE_OPTIONS_ULONGLONG },
  { {(char *)STRING_WITH_LEN("revision") }, offsetof(TABLE_LIST, revision),
    FILE_OPTIONS_REV },
  { {(char *)STRING_WITH_LEN("timestamp") }, offsetof(TABLE_LIST, timestamp),
    FILE_OPTIONS_TIMESTAMP },
  { {(char *)STRING_WITH_LEN("create-version") },
    offsetof(TABLE_LIST, file_version), FILE_OPTIONS_ULONGLONG },
  { {(char *)STRING_WITH_LEN("source") }, offsetof(TABLE_LIST, source),
...
static int mysql_register_view(THD *thd, TABLE_LIST *view,
  ...
  file.str = file_buff;
  file.length =
      (strxnmov(file_buff, FN_REFLEN, view->table_name, reg_ext, NullS) -
       file_buff);
  /* init timestamp */
  ...
  view->source.length =
      (thd->query_length - (thd->lex->create_view_select_start - thd->query));
  view->file_version = 1;
  view->calc_md5(md5);
  view->md5.str = md5;
  view->md5.length = 32;
  ...
  if ((view->updatable_view =
           (can_be_merged && view->algorithm != VIEW_ALGORITHM_TMPTABLE))) {
    /* TODO: change here when we will support UNIONs */
    for (TABLE_LIST *tbl = (TABLE_LIST *)lex->select_lex.table_list.first; tbl;
         tbl = tbl->next_local) {
...
loop_out:
  ...
  */
  ...
  if (sql_create_definition_file(&dir, &file, view_file_type, (gptr)view,
                                 view_parameters, num_view_backups)) {
...
my_bool mysql_make_view(File_parser *parser, TABLE_LIST *table) {
  ...
  */
  if (!table->definer.user.str) {
    DBUG_ASSERT(!table->definer.host.str && !table->definer.user.length &&
                !table->definer.host.length);
    push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_VIEW_FRM_NO_USER,
                        ER(ER_VIEW_FRM_NO_USER), table->db, table->table_name);
  ...
  */
  ...
  if (!res && !thd->is_fatal_error) {
    ...
    */
    ...
    old_lex->safe_to_cache_query =
        (old_lex->safe_to_cache_query && lex->safe_to_cache_query);
    ...
    */
    if (table->algorithm != VIEW_ALGORITHM_TMPTABLE && lex->can_be_merged() &&
      ...
      */
      ...
      for (SELECT_LEX_UNIT *unit = lex->select_lex.first_inner_unit(); unit;
           unit = next_unit) {
        if (unit == end_unit)
          break;
    ...
    if (access(path, F_OK) || (type = (mysql_frm_type(path) != FRMTYPE_VIEW))) {
      ...
      if (thd->lex->drop_if_exists) {
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE,
                            ER_BAD_TABLE_ERROR, ER(ER_BAD_TABLE_ERROR), name);
        VOID(pthread_mutex_unlock(&LOCK_open));
        continue;
      }
      if (type)
...
int view_checksum(THD *thd, TABLE_LIST *view) {
  ...
  view->calc_md5(md5);
  return (strncmp(md5, view->md5.str, 32) ? HA_ADMIN_WRONG_CHECKSUM
                                          : HA_ADMIN_OK);
}
...
bool mysql_rename_view(THD *thd, const char *new_name, TABLE_LIST *view) {
  ...
  if ((parser = sql_parse_prepare(&pathstr, thd->mem_root, 1)) &&
    ...
    */
    ...
    file.str = file_buff;
    file.length =
        (strxnmov(file_buff, FN_REFLEN, new_name, reg_ext, NullS) - file_buff);
