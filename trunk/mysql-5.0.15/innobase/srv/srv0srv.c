...
byte srv_latin1_ordering[256] /* The sort order table of the latin1
    ...
    = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12,
       ...
       0xBE, 0xBF, 0x41, 0x41, 0x41, 0x41, 0x5C, 0x5B, 0x5C, 0x43, 0x45, 0x45, 0x45, 0x45, 0x49, 0x49, 0x49, 0x49, 0x44,
       0x4E, 0x4F, 0x4F, 0x4F, 0x4F, 0x5D, 0xD7, 0xD8, 0x55, 0x55, 0x55, 0x59, 0x59, 0xDE, 0xDF, 0x41, 0x41, 0x41, 0x41,
       0x5C, 0x5B, 0x5C, 0x43, 0x45, 0x45, 0x45, 0x45, 0x49, 0x49, 0x49, 0x49, 0x44, 0x4E, 0x4F, 0x4F, 0x4F, 0x4F, 0x5D,
       0xF7, 0xD8, 0x55, 0x55, 0x55, 0x59, 0x59, 0xDE, 0xFF};
...
srv_conc_queue;                  /* queue of threads
                                 waiting to get in */
...
table. */
static srv_slot_t *srv_table_get_nth_slot(
    /*===================*/
...
static ulint srv_table_reserve_slot(
    /*===================*/
...
/* out: event for the calling thread to wait */
{
  ...
  {
    fprintf(stderr, "Suspending thread %lu to slot %lu meter %lu\n", (ulong)os_thread_get_curr_id(), (ulong)slot_no,
    ...
    ulint n)    /* in: number of threads to release */
{
  ...
  for (i = 0; i < OS_THREAD_MAX_N; i++)
  {
    ...
    if (slot->in_use && slot->type == type && slot->suspended)
    {
      ...
      if (srv_print_thread_releases)
      {
        fprintf(stderr, "Releasing thread %lu type %lu from slot %lu meter %lu\n", (ulong)slot->id, (ulong)type,
                (ulong)i, (ulong)srv_meter[SRV_RECOVERY]);
      }
...
/*==========*/
{
  ...
  table = dict_mem_table_create("SYS_DUMMY1", DICT_HDR_SPACE, 1, FALSE);
  dict_mem_table_add_col(table, "DUMMY", DATA_CHAR, DATA_ENGLISH | DATA_NOT_NULL, 8, 0);

  srv_sys->dummy_ind1 = dict_mem_index_create("SYS_DUMMY1", "SYS_DUMMY1", DICT_HDR_SPACE, 0, 1);
  dict_index_add_col(srv_sys->dummy_ind1, dict_table_get_nth_col(table, 0), 0, 0);
  srv_sys->dummy_ind1->table = table;
  /* create dummy table and index for new-style infimum and supremum */
  table = dict_mem_table_create("SYS_DUMMY2", DICT_HDR_SPACE, 1, TRUE);
  dict_mem_table_add_col(table, "DUMMY", DATA_CHAR, DATA_ENGLISH | DATA_NOT_NULL, 8, 0);
  srv_sys->dummy_ind2 = dict_mem_index_create("SYS_DUMMY2", "SYS_DUMMY2", DICT_HDR_SPACE, 0, 1);
  dict_index_add_col(srv_sys->dummy_ind2, dict_table_get_nth_col(table, 0), 0, 0);
    ...
    if (i >= OS_THREAD_MAX_N)
    {
      ...
      for (i = 0; i < OS_THREAD_MAX_N; i++)
      {
        ...
        fprintf(stderr, "Slot %lu: thread id %lu, type %lu, in use %lu, susp %lu, time %lu\n", (ulong)i,
                (ulong)os_thread_pf(slot->id), (ulong)slot->type, (ulong)slot->in_use, (ulong)slot->suspended,
...
void srv_release_mysql_thread_if_suspended(
    /*==================================*/
...
void srv_printf_innodb_monitor(
    /*======================*/
                      ...
                      the list of active transactions */
{
  ...
  fputs(
      ...
      "----------------------\n",
      file);
  fprintf(file, "Total memory allocated " ULINTPF "; in additional pool allocated " ULINTPF "\n",
          ut_total_allocated_memory, mem_pool_get_reserved(mem_comm_pool));

  if (srv_use_awe)
  {
    fprintf(file, "In addition to that %lu MB of AWE memory allocated\n",
  ...
  if (n_reserved > 0)
  {
    fprintf(file, "%lu tablespace extents now reserved for B-tree split operations\n", (ulong)n_reserved);
  }
  ...
  fprintf(file, "%.2f inserts/s, %.2f updates/s, %.2f deletes/s, %.2f reads/s\n",
...
void srv_export_innodb_status(void)
{
  ...
  export_vars.innodb_buffer_pool_pages_misc =
      buf_pool->max_size - UT_LIST_GET_LEN(buf_pool->LRU) - UT_LIST_GET_LEN(buf_pool->free);
  ...
  {
    export_vars.innodb_row_lock_time_avg = (ulint)(srv_n_lock_wait_time / 10000 / srv_n_lock_wait_count);
  }
  else
  {
...
loop:
  ...
  for (i = 0; i < OS_THREAD_MAX_N; i++)
  {
    ...
    if (slot->in_use)
    {
      ...
      if (srv_lock_wait_timeout < 100000000 && (wait_time > (double)srv_lock_wait_timeout || wait_time < 0))
      {
  ...
  if (some_waits || srv_print_innodb_monitor || srv_print_innodb_lock_monitor || srv_print_innodb_tablespace_monitor ||
...
loop:
  ...
  if (ut_dulint_cmp(new_lsn, old_lsn) < 0)
  {
    ...
    fprintf(stderr,
            ...
            (ulong)ut_dulint_get_high(old_lsn), (ulong)ut_dulint_get_low(old_lsn), (ulong)ut_dulint_get_high(new_lsn),
...
loop:
  ...
  n_ios_very_old = log_sys->n_log_ios + buf_pool->n_pages_read + buf_pool->n_pages_written;
  mutex_enter(&kernel_mutex);
