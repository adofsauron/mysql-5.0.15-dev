...
my_bool dynstr_set(DYNAMIC_STRING *str, const char *init_str)
{
  ...
  {
    str->max_length = ((length + str->alloc_increment - 1) / str->alloc_increment) * str->alloc_increment;
    if (!str->max_length)
...
my_bool dynstr_realloc(DYNAMIC_STRING *str, ulong additional_size)
{
  ...
  {
    str->max_length =
        ((str->length + additional_size + str->alloc_increment - 1) / str->alloc_increment) * str->alloc_increment;
    if (!(str->str = (char *)my_realloc(str->str, str->max_length, MYF(MY_WME))))
      DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
...
my_bool dynstr_append_mem(DYNAMIC_STRING *str, const char *append, uint length)
{
  ...
  {
    uint new_length = (str->length + length + str->alloc_increment) / str->alloc_increment;
