...
Item *sp_eval_func_item(THD *thd, Item **it_addr, enum enum_field_types type,
...
void sp_name::init_qname(THD *thd) {
  ...
  sprintf(m_qname.str, "%.*s.%.*s", m_db.length, (m_db.length ? m_db.str : ""),
...
void sp_head::operator delete(void *ptr, size_t size) {
  ...
  DBUG_PRINT("info", ("mem_root 0x%lx moved to 0x%lx", (ulong) & sp->mem_root,
                      (ulong) & own_root));
...
sp_head::sp_head()
    : Query_arena(&main_mem_root, INITIALIZED_FOR_SP), m_flags(0),
      m_returns_cs(NULL) {
...
void sp_head::init(LEX *lex) {
  ...
  */
  ...
  m_qname.str = m_db.str = m_name.str = m_params.str = m_body.str =
      m_defstr.str = 0;
...
TYPELIB *sp_head::create_typelib(List<String> *src) {
  ...
  if (src->elements) {
    ...
    result->name = "";
    if (!(result->type_names = (const char **)alloc_root(
              mem_root, (sizeof(char *) + sizeof(int)) * (result->count + 1))))
      return 0;
    result->type_lengths =
        (unsigned int *)(result->type_names + result->count + 1);
    ...
    for (uint i = 0; i < result->count; i++) {
      ...
      if (String::needs_conversion(tmp->length(), tmp->charset(), cs, &dummy)) {
        ...
        length = conv.length();
        result->type_names[i] =
            (char *)strmake_root(mem_root, conv.ptr(), length);
      } else {
        length = tmp->length();
...
int sp_head::create(THD *thd) {
  ...
  DBUG_PRINT("info", ("type: %d name: %s params: %s body: %s", m_type,
                      m_name.str, m_params.str, m_body.str));
...
Field *sp_head::make_field(uint max_length, const char *name, TABLE *dummy) {
  ...
  field = ::make_field((char *)0, !m_returns_len ? max_length : m_returns_len,
                       (uchar *)"", 0, m_returns_pack, m_returns, m_returns_cs,
                       m_geom_returns, Field::NONE, m_returns_typelib,
...
int sp_head::execute(THD *thd) {
  ...
  */
  ...
  DBUG_PRINT("info", ("ret=%d killed=%d query_error=%d", ret, thd->killed,
                      thd->query_error));
...
int sp_head::execute_function(THD *thd, Item **argp, uint argcount,
                              Item **resp) {
  Item **param_values;
...
int sp_head::execute_procedure(THD *thd, List<Item> *args) {
  ...
  if (!octx) { // Create a temporary old context
...
void sp_head::set_info(char *definer, uint definerlen, longlong created,
                       longlong modified, st_sp_chistics *chistics,
                       ulong sql_mode) {
...
bool check_show_routine_access(THD *thd, sp_head *sp, bool *full_access) {
  ...
  *full_access =
      (!check_table_access(thd, SELECT_ACL, &tables, 1) ||
...
int sp_lex_keeper::reset_lex_and_exec_core(THD *thd, uint *nextp,
...
int sp_instr_stmt::execute(THD *thd, uint *nextp) {
  ...
  if (!(res = alloc_query(thd, m_query.str, m_query.length + 1)) &&
    ...
    */
    if (query_cache_send_result_to_client(thd, thd->query, thd->query_length) <=
        0) {
...
void sp_instr_hpop::backpatch(uint dest, sp_pcontext *dst_ctx) {
...
void sp_instr_cpop::backpatch(uint dest, sp_pcontext *dst_ctx) {
...
bool sp_change_security_context(THD *thd, sp_head *sp,
      ...
      */
      if ((tab = (SP_TABLE *)hash_search(&m_sptabs, (byte *)tname, tlen))) {
        if (tab->lock_type < table->lock_type)
          tab->lock_type =
              table->lock_type; // Use the table with the highest lock type
...
TABLE_LIST *sp_add_to_query_tables(THD *thd, LEX *lex, const char *db,
                                   const char *name, thr_lock_type locktype) {
  TABLE_LIST *table;
