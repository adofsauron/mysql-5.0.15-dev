...
my_bool bitmap_init(MY_BITMAP *map, uchar *buf, uint bitmap_size, my_bool thread_safe)
{
  ...
  if (!(map->bitmap = buf) &&
      !(map->bitmap =
            (uchar *)my_malloc(bitmap_size + (thread_safe ? sizeof(pthread_mutex_t) : 0), MYF(MY_WME | MY_ZEROFILL))))
    return 1;
...
void bitmap_set_prefix(MY_BITMAP *map, uint prefix_size)
{
  ...
  DBUG_ASSERT(map->bitmap && (prefix_size <= map->bitmap_size * 8 || prefix_size == (uint)~0));
  bitmap_lock(map);
...
void bitmap_clear_all(MY_BITMAP *map) { bitmap_set_prefix(map, 0); }

void bitmap_set_all(MY_BITMAP *map) { bitmap_set_prefix(map, ~0); }

my_bool bitmap_is_prefix(const MY_BITMAP *map, uint prefix_size)
{
  uint prefix_bits = prefix_size & 7, res = 0;
  uchar *m = map->bitmap, *end_prefix = map->bitmap + prefix_size / 8, *end = map->bitmap + map->bitmap_size;
...
my_bool bitmap_is_clear_all(const MY_BITMAP *map) { return bitmap_is_prefix(map, 0); }

my_bool bitmap_is_set_all(const MY_BITMAP *map) { return bitmap_is_prefix(map, map->bitmap_size * 8); }
...
my_bool bitmap_is_subset(const MY_BITMAP *map1, const MY_BITMAP *map2)
{
  ...
  DBUG_ASSERT(map1->bitmap && map2->bitmap && map1->bitmap_size == map2->bitmap_size);
...
void bitmap_subtract(MY_BITMAP *map, const MY_BITMAP *map2)
{
  ...
  DBUG_ASSERT(map->bitmap && map2->bitmap && map->bitmap_size == map2->bitmap_size);
  bitmap_lock(map);
  bitmap_lock((MY_BITMAP *)map2);
