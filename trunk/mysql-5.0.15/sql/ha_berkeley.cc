...
const char *berkeley_lock_names[] = { "DEFAULT",  "OLDEST", "RANDOM",
                                      "YOUNGEST", 0 };
u_int32_t berkeley_lock_types[] = { DB_LOCK_DEFAULT, DB_LOCK_OLDEST,
                                    DB_LOCK_RANDOM };
TYPELIB berkeley_lock_typelib = {
  array_elements(berkeley_lock_names) - 1, "", berkeley_lock_names, NULL
};
...
handlerton berkeley_hton = {
  "BerkeleyDB",                                   SHOW_OPTION_YES,
  "Supports transactions and page-level locking", DB_TYPE_BERKELEY_DB,
  berkeley_init,                                  0,    /* slot */
  0,                                                    /* savepoint size */
  berkeley_close_connection,                      NULL, /* savepoint_set */
  NULL,                                                 /* savepoint_rollback */
  NULL,                                                 /* savepoint_release */
  berkeley_commit,                                berkeley_rollback,
  NULL, /* prepare */
  NULL, /* recover */
  NULL, /* commit_by_xid */
...
bool berkeley_init(void) {
  ...
  */
  ...
  if (opt_endinfo)
    db_env->set_verbose(
        db_env, DB_VERB_CHKPOINT | DB_VERB_DEADLOCK | DB_VERB_RECOVERY, 1);
  ...
  if (db_env->open(db_env, berkeley_home,
                   berkeley_init_flags | DB_INIT_LOCK | DB_INIT_LOG |
                       DB_INIT_MPOOL | DB_INIT_TXN | DB_CREATE | DB_THREAD,
                   0666)) {
...
bool berkeley_flush_logs() {
  ...
  if ((error = db_env->log_flush(db_env, 0))) {
    my_error(ER_ERROR_DURING_FLUSH_LOGS, MYF(0),
             error); /* purecov: inspected */
    result = 1;      /* purecov: inspected */
  }
  if ((error = db_env->txn_checkpoint(db_env, 0, 0, 0))) {
    my_error(ER_ERROR_DURING_CHECKPOINT, MYF(0),
             error); /* purecov: inspected */
    result = 1;      /* purecov: inspected */
  }
  DBUG_RETURN(result);
...
static int berkeley_commit(THD *thd, bool all) {
  ...
  berkeley_trx_data *trx =
      (berkeley_trx_data *)thd->ha_data[berkeley_hton.slot];
...
int berkeley_show_logs(Protocol *protocol) {
  ...
  *root_ptr = &show_logs_root;
  ...
  if ((error =
           db_env->log_archive(db_env, &all_logs, DB_ARCH_ABS | DB_ARCH_LOG)) ||
...
// by HF. Sometimes it crashes. TODO - find out why
  ...
  if ((error = db_env->log_archive(db_env, &names, DB_ARCH_ABS)) != 0) {
    DBUG_PRINT("error", ("log_archive failed (error %d)",
                         error)); /* purecov: inspected */
    db_env->err(db_env, error,
                "log_archive: DB_ARCH_ABS"); /* purecov: inspected */
    DBUG_VOID_RETURN;                        /* purecov: inspected */
  }

  if (names) {                     /* purecov: tested */
...
ha_berkeley::ha_berkeley(TABLE *table_arg)
    : handler(&berkeley_hton, table_arg), alloc_ptr(0), rec_buff(0), file(0),
      int_table_flags(HA_REC_NOT_IN_SEQ | HA_FAST_KEY_READ | HA_NULL_IN_KEY |
                      HA_CAN_INDEX_BLOBS | HA_NOT_EXACT_COUNT |
...
static int berkeley_cmp_hidden_key(DB *file, const DBT *new_key,
                                   const DBT *saved_key) {
...
static int berkeley_cmp_packed_key(DB *file, const DBT *new_key,
                                   const DBT *saved_key) {
  ...
  for (; key_part != end && (int)key_length > 0; key_part++) {
    ...
    key_length -= length;
    saved_key_ptr +=
        key_part->field->packed_col_length(saved_key_ptr, key_part->length);
  }
...
#ifdef NOT_YET
static int berkeley_cmp_fix_length_key(DB *file, const DBT *new_key,
                                       const DBT *saved_key) {
  KEY *key = (new_key->app_private ? (KEY *)new_key->app_private
                                   : (KEY *)(file->app_private));
  char *new_key_ptr = (char *)new_key->data;
...
static bool berkeley_key_cmp(TABLE *table, KEY *key_info, const char *key,
                             uint key_length) {
  ...
  for (; key_part != end && (int)key_length > 0; key_part++) {
    ...
    if (key_part->null_bit) {
      ...
      */
      if (*key != (table->record[0][key_part->null_offset] & key_part->null_bit)
              ? 0
              : 1)
        return 1;
      if (!*key++) // Null value
...
int ha_berkeley::open(const char *name, int mode, uint test_if_locked) {
  ...
  if ((primary_key = table_share->primary_key) >= MAX_KEY) { // No primary key
  ...
  if (!(alloc_ptr = my_multi_malloc(
            MYF(MY_WME), &key_buff, max_key_length, &key_buff2, max_key_length,
            &primary_key_buff,
            (hidden_primary_key ? 0 : table->key_info[table_share->primary_key]
                                          .key_length),
            NullS)))
    DBUG_RETURN(1); /* purecov: inspected */
  if (!(rec_buff = (byte *)my_malloc(
...
int ha_berkeley::pack_row(DBT *row, const byte *record, bool new_row) {
  ...
  for (Field **field = table->field; *field; field++)
    ptr = (byte *)(*field)
              ->pack((char *)ptr, (char *)record + (*field)->offset());
...
void ha_berkeley::unpack_key(char *record, DBT *key, uint index) {
  ...
  for (; key_part != end; key_part++) {
    ...
    pos = (char *)key_part->field->unpack_key(
        record + key_part->field->offset(), pos, key_part->length);
  }
}
...
DBT *ha_berkeley::pack_key(DBT *key, uint keynr, char *buff,
  ...
  for (; key_part != end && (int)key_length > 0; key_part++) {
    ...
    buff = key_part->field->pack_key_from_key_image(
        buff, (char *)key_ptr + offset, key_part->length);
    key_ptr += key_part->store_length;
    key_length -= key_part->store_length;
  }
  key->size = (buff - (char *)key->data);
...
int ha_berkeley::write_row(byte *record) {
  ...
  if (table->s->keys + test(hidden_primary_key) == 1) {
    error = file->put(file, transaction,
      ...
      if (!(error =
        ...
        for (uint keynr = 0; keynr < table->s->keys; keynr++) {
          if (keynr == primary_key)
            continue;
          if ((error = key_file[keynr]
                           ->put(key_file[keynr], sub_trans,
      ...
      if (error) {
        ...
        if (using_ignore) {
          ...
          if (!changed_keys.is_clear_all()) {
            new_error = 0;
            for (uint keynr = 0;
                 keynr < table->s->keys + test(hidden_primary_key); keynr++) {
              if (changed_keys.is_set(keynr)) {
                if ((new_error =
                         remove_key(sub_trans, keynr, record, &prim_key)))
...
int ha_berkeley::restore_keys(DB_TXN *trans, key_map *changed_keys,
                              uint primary_key, const byte *old_row,
                              DBT *old_key, const byte *new_row, DBT *new_key) {
  int error;
  DBT tmp_key;
  uint keynr;
  ...
  if ((error = update_primary_key(trans, TRUE, new_row, new_key, old_row,
                                  old_key, FALSE)))
  ...
  if (!changed_keys->is_clear_all()) {
    for (keynr = 0; keynr < table->s->keys + test(hidden_primary_key);
         keynr++) {
      if (changed_keys->is_set(keynr)) {
        if (changed_keys->is_prefix(1) &&
          ...
          break; /* purecov: inspected */
        if ((error = key_file[keynr]
                         ->put(key_file[keynr], trans,
                               create_key(&tmp_key, keynr, key_buff2, old_row),
...
int ha_berkeley::update_row(const byte *old_row, byte *new_row) {
  ...
  for (uint retry = 0; retry < berkeley_trans_retry; retry++) {
    ...
    if (!(error = update_primary_key(sub_trans, primary_key_changed, old_row,
                                     &old_prim_key, new_row, &prim_key,
                                     using_ignore))) {
      ...
      for (uint keynr = 0; keynr < table->s->keys; keynr++) {
        ...
        if (key_cmp(keynr, old_row, new_row) || primary_key_changed) {
          ...
          if ((error = key_file[keynr]
                           ->put(key_file[keynr], sub_trans,
                                 create_key(&key, keynr, key_buff2, new_row),
                                 &prim_key, key_type[keynr]))) {
            last_dup_key = keynr;
            break;
          }
  ...
  if (keynr == active_index && cursor)
    error = cursor->c_del(cursor, 0);
           ...
           ((table->key_info[keynr].flags & (HA_NOSAME | HA_NULL_PART_KEY)) ==
            HA_NOSAME)) { // Unique key
    DBUG_ASSERT(keynr == primary_key || prim_key->data != key_buff2);
    error = key_file[keynr]->del(
        key_file[keynr], trans,
        keynr == primary_key ? prim_key
                             : create_key(&key, keynr, key_buff2, record),
        0);
  } else {
    /*
    ...
    */
    ...
    DBC *tmp_cursor;
    if (!(error = key_file[keynr]
                      ->cursor(key_file[keynr], trans, &tmp_cursor, 0))) {
      if (!(error = tmp_cursor->c_get(
                tmp_cursor, create_key(&key, keynr, key_buff2, record),
                prim_key, DB_GET_BOTH | DB_RMW))) { // This shouldn't happen
...
int ha_berkeley::remove_keys(DB_TXN *trans, const byte *record, DBT *new_record,
                             DBT *prim_key, key_map *keys) {
  int result = 0;
  for (uint keynr = 0; keynr < table->s->keys + test(hidden_primary_key);
       keynr++) {
...
int ha_berkeley::index_init(uint keynr) {
  ...
  */
  ...
  active_index = keynr;
  if ((error = key_file[keynr]->cursor(
           key_file[keynr], transaction, &cursor,
...
int ha_berkeley::read_row(int error, char *buf, uint keynr, DBT *row,
  ...
  if (keynr != primary_key) {
    ...
    if ((error = file->get(file, transaction, &key, &current_row, 0))) {
      ...
      DBUG_RETURN(error == DB_NOTFOUND ? HA_ERR_CRASHED
                                       : error); /* purecov: inspected */
    }
    row = &current_row;
  }
...
int ha_berkeley::index_read_idx(byte *buf, uint keynr, const byte *key,
  ...
  DBUG_RETURN(read_row(
      key_file[keynr]->get(key_file[keynr], transaction,
                           pack_key(&last_key, keynr, key_buff, key, key_len),
                           &current_row, 0),
      (char *)buf, keynr, &current_row, &last_key, 0));
}

int ha_berkeley::index_read(byte *buf, const byte *key, uint key_len,
  ...
  if (key_len == key_info->key_length &&
    ...
    error = read_row(
        cursor->c_get(
            cursor, pack_key(&last_key, active_index, key_buff, key, key_len),
            &row, (find_flag == HA_READ_KEY_EXACT ? DB_SET : DB_SET_RANGE)),
    ...
    /*
      ...
      matching key.
    */
    key_info->handler.bdb_return_if_eq =
        (find_flag == HA_READ_AFTER_KEY ? 1 : -1);
...
int ha_berkeley::index_read_last(byte *buf, const byte *key, uint key_len) {
  ...
  key_info->handler.bdb_return_if_eq = 0;
  bzero((char *)&row, sizeof(row));
  if (read_row(cursor->c_get(cursor, &last_key, &row, DB_PREV), (char *)buf,
               active_index, &row, &last_key, 1) ||
...
int ha_berkeley::rnd_pos(byte *buf, byte *pos) {
  ...
  active_index = MAX_KEY;
  DBUG_RETURN(read_row(
      file->get(file, transaction, get_pos(&db_pos, pos), &current_row, 0),
...
int ha_berkeley::external_lock(THD *thd, int lock_type) {
  ...
  if (!trx) {
    thd->ha_data[berkeley_hton.slot] = trx =
        (berkeley_trx_data *)my_malloc(sizeof(*trx), MYF(MY_ZEROFILL));
    if (!trx)
      DBUG_RETURN(1);
  }
...
static int create_sub_table(const char *table_name, const char *sub_name,
  ...
  if (!(error = db_create(&file, db_env, 0))) {
    ...
    if (error) {
      DBUG_PRINT("error", ("Got error: %d when opening table '%s'",
                           error,                    /* purecov: inspected */
    ...
    DBUG_PRINT("error", ("Got error: %d when creting table",
                         error)); /* purecov: inspected */
  }
  if (error)
    my_errno = error; /* purecov: inspected */
...
int ha_berkeley::create(const char *name, register TABLE *form,
  ...
  for (uint i = 0; i < form->s->keys; i++) {
    if (i != primary_key) {
      ...
      if ((error = create_sub_table(
               name_buff, part, DB_BTREE,
               (table->key_info[i].flags & HA_NOSAME) ? 0 : DB_DUP)))
        DBUG_RETURN(error); /* purecov: inspected */
    }
  }
  ...
  if (!(error = (db_create(&status_block, db_env, 0)))) {
    if (!(error = (status_block->open(status_block, NULL, name_buff, "status",
...
int ha_berkeley::delete_table(const char *name) {
  ...
  if ((error = db_create(&file, db_env, 0)))
    ...
    error = file->remove(
        file, fn_format(name_buff, name, "", ha_berkeley_ext, 2 | 4), NULL, 0);
  file = 0; // Safety
  DBUG_RETURN(error);
}
...
int ha_berkeley::rename_table(const char *from, const char *to) {
  ...
  if ((error = db_create(&file, db_env, 0)))
    ...
    error = file->rename(
        file, fn_format(from_buff, from, "", ha_berkeley_ext, 2 | 4), NULL,
...
ha_rows ha_berkeley::records_in_range(uint keynr, key_range *start_key,
  ...
  key_info->handler.bdb_return_if_eq = -1;
  error = ((start_key &&
            kfile->key_range(kfile, transaction,
                             pack_key(&key, keynr, key_buff, start_key->key,
                                      start_key->length),
                             &start_range, 0)));
  if (error) {
    key_info->handler.bdb_return_if_eq = 0;
...
ulonglong ha_berkeley::get_auto_increment() {
  ...
  if (!table->s->next_number_key_offset) { // Autoincrement at key-start
    ...
    ha_berkeley::create_key(&last_key, active_index, key_buff, table->record[0],
...
int ha_berkeley::analyze(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  */
  ...
  if (!db_env->txn_stat(db_env, &txn_stat_ptr, 0) && txn_stat_ptr &&
      txn_stat_ptr->st_nactive >= 2) {
  ...
  for (i = 0; i < table->s->keys; i++) {
    ...
    if ((key_file[i]->stat)(key_file[i], (void *)&stat, 0))
      goto err; /* purecov: inspected */
    share->rec_per_key[i] =
        (stat->bt_ndata / (stat->bt_nkeys ? stat->bt_nkeys : 1));
  }
...
int ha_berkeley::check(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  */
  ...
  tmp_file->set_bt_compare(
      tmp_file,
      (hidden_primary_key ? berkeley_cmp_hidden_key : berkeley_cmp_packed_key));
...
static BDB_SHARE *get_share(const char *table_name, TABLE *table) {
  ...
  if (!(share = (BDB_SHARE *)hash_search(&bdb_open_tables, (byte *)table_name,
    ...
    if ((share = (BDB_SHARE *)my_multi_malloc(
             MYF(MY_WME | MY_ZEROFILL), &share, sizeof(*share), &rec_per_key,
             keys * sizeof(ha_rows), &tmp_name, length + 1, &key_file,
             (keys + 1) * sizeof(*key_file), &key_type,
...
void ha_berkeley::get_status() {
  if (!test_all_bits(share->status,
    ...
    if (!share->status_block) {
      char name_buff[FN_REFLEN];
      uint open_mode =
          (((table->db_stat & HA_READ_ONLY) ? DB_RDONLY : 0) | DB_THREAD);
      fn_format(name_buff, share->table_name, "", ha_berkeley_ext, 2 | 4);
      if (!db_create(&share->status_block, db_env, 0)) {
        if (share->status_block->open(share->status_block, NULL, name_buff,
          ...
          share->status_block->close(share->status_block,
                                     0); /* purecov: inspected */
  ...
  if (share->rows != share->org_rows || (share->status & STATUS_BDB_ANALYZE)) {
    ...
    if (!share->status_block) {
      ...
      */
      ...
      share->status_block->set_flags(share->status_block,
                                     0); /* purecov: inspected */
      if (share->status_block->open(share->status_block, NULL,
                                    fn_format(name_buff, share->table_name, "",
                                              ha_berkeley_ext, 2 | 4),
                                    "status", DB_BTREE, DB_THREAD | DB_CREATE,
                                    my_umask)) /* purecov: inspected */
        goto end;                              /* purecov: inspected */
    }
    {
