...
const char *join_type_str[] = {
  ...
  "ref_or_null", "unique_subquery", "index_subquery", "index_merge"
};
...
static bool update_ref_and_keys(THD *thd, DYNAMIC_ARRAY *keyuse,
                                JOIN_TAB *join_tab, uint tables, COND *conds,
                                COND_EQUAL *cond_equal, table_map table_map,
...
static void greedy_search(JOIN *join, table_map remaining_tables, uint depth,
                          uint prune_level);
...
static store_key *get_store_key(THD *thd, KEYUSE *keyuse, table_map used_tables,
...
static COND *substitute_for_best_equal_field(COND *cond, COND_EQUAL *cond_equal,
                                             void *table_join_idx);
static COND *simplify_joins(JOIN *join, List<TABLE_LIST> *join_list,
                            COND *conds, bool top);
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list,
                           Item::cond_result *cond_value);
static bool resolve_nested_join(TABLE_LIST *table);
static COND *remove_eq_conds(THD *thd, COND *cond,
                             Item::cond_result *cond_value);
static bool const_expression_in_where(COND *conds, Item *item,
                                      Item **comp_item);
...
static enum_nested_loop_state end_send(JOIN *join, JOIN_TAB *join_tab,
                                       bool end_of_records);
static enum_nested_loop_state end_send_group(JOIN *join, JOIN_TAB *join_tab,
                                             bool end_of_records);
static enum_nested_loop_state end_write(JOIN *join, JOIN_TAB *join_tab,
                                        bool end_of_records);
static enum_nested_loop_state end_update(JOIN *join, JOIN_TAB *join_tab,
                                         bool end_of_records);
static enum_nested_loop_state end_unique_update(JOIN *join, JOIN_TAB *join_tab,
                                                bool end_of_records);
static enum_nested_loop_state end_write_group(JOIN *join, JOIN_TAB *join_tab,
                                              bool end_of_records);
...
static int remove_dup_with_hash_index(THD *thd, TABLE *table, uint field_count,
                                      Field **first_field, ulong key_length,
                                      Item *having);
  ...
  if (select_lex->next_select())
    ...
    */
    res = mysql_select(
        ...
        (TABLE_LIST *)select_lex->table_list.first, select_lex->with_wild,
        select_lex->item_list, select_lex->where,
        ...
        select_lex->options | thd->options | setup_tables_done_option, result,
        unit, select_lex);
  }
...
inline int setup_without_group(THD *thd, Item **ref_pointer_array,
                               TABLE_LIST *tables, TABLE_LIST *leaves,
  ...
  res = res ||
        setup_order(thd, ref_pointer_array, tables, fields, all_fields, order);
  thd->allow_sum_func = 0;
...
/*
  ...
  return -1 on error
          0 on success
*/
int JOIN::prepare(Item ***rref_pointer_array, TABLE_LIST *tables_init,
                  uint wild_num, COND *conds_init, uint og_num,
                  ORDER *order_init, ORDER *group_init, Item *having_init,
  ...
  */
  ...
  if ((!(select_options & OPTION_SETUP_TABLES_DONE) &&
       setup_tables(thd, &select_lex->context, join_list, tables_list, &conds,
                    &select_lex->leaf_tables, FALSE)) ||
      setup_wild(thd, tables_list, fields_list, &all_fields, wild_num) ||
      select_lex->setup_ref_array(thd, og_num) ||
      setup_fields(thd, (*rref_pointer_array), fields_list, 1, &all_fields,
                   1) ||
      setup_without_group(thd, (*rref_pointer_array), tables_list,
                          select_lex->leaf_tables, fields_list, all_fields,
  ...
  if (having) {
    ...
    bool having_fix_rc = (!having->fixed && (having->fix_fields(thd, &having) ||
                                             having->check_cols(1)));
  ...
  */
  {
    ...
    for (table_ptr = select_lex->leaf_tables; table_ptr;
  ...
  if (procedure) {
    if (setup_new_fields(thd, fields_list, all_fields, procedure->param_fields))
      goto err; /* purecov: inspected */
    if (procedure->group) {
      if (!test_if_subpart(procedure->group,
                           group_list)) { /* purecov: inspected */
...
#endif
    if (order && (procedure->flags & PROC_NO_SORT)) { /* purecov: inspected */
...
int JOIN::optimize() {
  ...
  if (cond_value == Item::COND_FALSE ||
      (!unit->select_limit_cnt &&
       !(select_options & OPTION_FOUND_ROWS))) { /* Impossible cond */
  ...
  if (!(thd->options & OPTION_BIG_SELECTS) &&
      ...
      !(select_options & SELECT_DESCRIBE)) { /* purecov: inspected */
  ...
  select =
      make_select(*table, const_table_map, const_table_map, conds, 1, &error);
  if (error) {  /* purecov: inspected */
  ...
  */
  for (JOIN_TAB *tab = join_tab + const_tables; tab < join_tab + tables;
       tab++) {
    if (*tab->on_expr_ref) {
      *tab->on_expr_ref = substitute_for_best_equal_field(
          *tab->on_expr_ref, tab->cond_equal, map2table);
  ...
  if (group_list || tmp_table_param.sum_func_count) {
    ...
    */
    ...
    if ((group_list =
             create_distinct_group(thd, select_lex->ref_pointer_array, order,
                                   fields_list, &all_order_fields_used))) {
      bool skip_group =
          (skip_sort_order &&
           test_if_skip_sort_order(tab, group_list, select_limit, 1) != 0);
      if ((skip_group && all_order_fields_used) ||
          select_limit == HA_POS_ERROR || (order && !skip_sort_order)) {
  ...
  simple_group = 0;
  {
    ...
    group_list =
        remove_const(this, (old_group_list = group_list), conds,
                     rollup.state == ROLLUP::STATE_NONE, &simple_group);
  ...
  if (procedure && procedure->group) {
    group_list = procedure->group =
        remove_const(this, procedure->group, conds, 1, &simple_group);
    calc_group_buffer(this, group_list);
  }
  ...
  need_tmp =
      (const_tables != tables &&
       ((select_distinct || !simple_order || !simple_group) ||
        (group_list && order) || test(select_options & OPTION_BUFFER_RESULT)));

  // No cache for MATCH
  make_join_readinfo(
      this, (select_options & (SELECT_DESCRIBE | SELECT_NO_JOIN_CACHE)) |
  ...
  */
  if (!group_list && !order && unit->item &&
      unit->item->substype() == Item_subselect::IN_SUBS && tables == 1 &&
    ...
    if (!having) {
      ...
      if (join_tab[0].type == JT_EQ_REF &&
        ...
        if (test_in_subselect(&where)) {
          ...
          error = 0;
          DBUG_RETURN(
              unit->item->change_engine(new subselect_uniquesubquery_engine(
                  thd, join_tab, unit->item, where)));
        }
        ...
        if (test_in_subselect(&where)) {
          ...
          error = 0;
          DBUG_RETURN(
              unit->item->change_engine(new subselect_indexsubquery_engine(
                  thd, join_tab, unit->item, where, 0)));
        }
      }
      ...
      DBUG_RETURN(unit->item->change_engine(new subselect_indexsubquery_engine(
          thd, join_tab, unit->item, conds, 1)));
    }
  }
/*
  ...
  if (need_tmp) {
    ...
    if (!(exec_tmp_table1 = create_tmp_table(
              ...
              ((!simple_group && !procedure &&
                !(test_flags & TEST_NO_KEY_GROUP))
                   ? group_list
                   : (ORDER *)0),
              group_list ? 0 : select_distinct, group_list && simple_group,
              select_options,
              (order == 0 || skip_sort_order) ? select_limit : HA_POS_ERROR,
              (char *)"")))
      DBUG_RETURN(1);
    ...
    */
    ...
    if (group_list && simple_group) {
      ...
      if (create_sort_index(thd, this, group_list, HA_POS_ERROR,
                            HA_POS_ERROR) ||
    ...
    */
    ...
    if (exec_tmp_table1->distinct) {
      ...
      if (order && skip_sort_order) {
        ...
        if (test_if_skip_sort_order(&join_tab[const_tables], order,
                                    unit->select_limit_cnt, 0))
          order = 0;
      }
    }
...
void JOIN::exec() {
  ...
  if (!tables_list) { // Only test of functions
    if (select_options & SELECT_DESCRIBE)
      select_describe(
      ...
      */
      if (cond_value != Item::COND_FALSE && (!conds || conds->val_int()) &&
        ...
        if (do_send_rows && (procedure ? (procedure->send_row(fields_list) ||
          ...
          send_records =
              ((select_options & OPTION_FOUND_ROWS) ? 1 : thd->sent_row_count);
        }
      } else {
        error = (int)result->send_eof();
        send_records = 0;
  ...
  if (zero_result_cause) {
    (void)return_zero_rows(this, result, select_lex->leaf_tables, fields_list,
                           send_row_on_empty_set(), select_options,
  ...
  if (select_options & SELECT_DESCRIBE) {
    ...
    */
    ...
    if (order && (const_tables == tables ||
                  ((simple_order || skip_sort_order) &&
                   test_if_skip_sort_order(&join_tab[const_tables], order,
                                           select_limit, 0))))
      order = 0;
    having = tmp_having;
    select_describe(this, need_tmp, order != 0 && !skip_sort_order,
                    select_distinct);
    DBUG_VOID_RETURN;
  }
  ...
  if (need_tmp) {
    ...
    if ((tmp_error =
             do_select(curr_join, (List<Item> *)0, curr_tmp_table, 0))) {
      error = tmp_error;
      DBUG_VOID_RETURN;
    }
    ...
    if (!items1) {
      ...
      if (sort_and_group || curr_tmp_table->group) {
        if (change_to_use_tmp_fields(thd, items1, tmp_fields_list1,
    ...
    if (curr_tmp_table->group) { // Already grouped
    ...
    */
    ...
    if (curr_join->group_list &&
      ...
      if (exec_tmp_table2)
        ...
        if (!(curr_tmp_table = exec_tmp_table2 = create_tmp_table(
                  thd, &curr_join->tmp_table_param, *curr_all_fields,
                  (ORDER *)0,
                  curr_join->select_distinct && !curr_join->group_list, 1,
                  curr_join->select_options, HA_POS_ERROR, (char *)"")))
          DBUG_VOID_RETURN;
      ...
      if (curr_join->make_sum_func_list(*curr_all_fields, *curr_fields_list, 1,
                                        TRUE))
        DBUG_VOID_RETURN;
      curr_join->group_list = 0;
      ...
      if (!items2) {
        ...
        if (change_to_use_tmp_fields(thd, items2, tmp_fields_list2,
                                     tmp_all_fields2, fields_list.elements,
                                     tmp_all_fields1))
          DBUG_VOID_RETURN;
    ...
    if (curr_join->select_distinct && !curr_join->group_list) {
      ...
      if (remove_duplicates(curr_join, curr_tmp_table, *curr_fields_list,
                            curr_join->tmp_having))
        DBUG_VOID_RETURN;
  ...
  if (curr_join->group || curr_join->tmp_table_param.sum_func_count ||
    ...
    if (!items3) {
      ...
      setup_copy_fields(thd, &curr_join->tmp_table_param, items3,
                        tmp_fields_list3, tmp_all_fields3,
  ...
  if (curr_join->group_list || curr_join->order) {
    ...
    if (curr_join->tmp_having && !curr_join->group_list &&
      ...
      table_map used_tables =
          (curr_join->const_table_map | curr_table->table->map);

      Item *sort_table_cond =
          make_cond_for_table(curr_join->tmp_having, used_tables, used_tables);
      if (sort_table_cond) {
        ...
        {
          if (!(curr_table->select->cond = new Item_cond_and(
                    curr_table->select->cond, sort_table_cond)))
            DBUG_VOID_RETURN;
          /*
        ...
        DBUG_EXECUTE("where", print_where(curr_table->select->cond,
                                          "select and having"););
        curr_join->tmp_having = make_cond_for_table(
            curr_join->tmp_having, ~(table_map)0, ~used_tables);
        DBUG_EXECUTE("where",
                     print_where(curr_join->tmp_having, "having after sort"););
      }
    }
    {
      if (group)
        ...
        */
        JOIN_TAB *curr_table =
            &curr_join->join_tab[curr_join->const_tables + 1];
      ...
      */
      if (create_sort_index(
              thd, curr_join,
              curr_join->group_list ? curr_join->group_list : curr_join->order,
              curr_join->select_limit,
              (select_options & OPTION_FOUND_ROWS ? HA_POS_ERROR
                                                  : unit->select_limit_cnt)))
        DBUG_VOID_RETURN;
    }
  }
...
bool mysql_select(THD *thd, Item ***rref_pointer_array, TABLE_LIST *tables,
                  uint wild_num, List<Item> &fields, COND *conds, uint og_num,
    ...
    */
    if (select_lex->linkage != DERIVED_TABLE_TYPE ||
      ...
      if (select_lex->linkage != GLOBAL_OPTIONS_TYPE) {
        ...
        if (err = join->prepare(rref_pointer_array, tables, wild_num, conds,
                                og_num, order, group, having, proc_param,
                                select_lex, unit)) {
          goto err;
        }
      }
    ...
    if (err =
            join->prepare(rref_pointer_array, tables, wild_num, conds, og_num,
                          order, group, having, proc_param, select_lex, unit)) {
      goto err;
    }
  }
...
static ha_rows get_quick_record_count(THD *thd, SQL_SELECT *select,
                                      TABLE *table, const key_map *keys,
                                      ha_rows limit) {
  int error;
...
static bool make_join_statistics(JOIN *join, TABLE_LIST *tables, COND *conds,
  ...
  table_vector =
      (TABLE **)join->thd->alloc(sizeof(TABLE *) * (table_count * 2));
  ...
  for (s = stat, i = 0; tables; s++, tables = tables->next_leaf, i++) {
    ...
    bzero((char *)table->const_key_parts,
    ...
    if (*s->on_expr_ref) {
      ...
      if (!table->file->records && !embedding) { // Empty table
  ...
  if (conds || outer_join)
    if (update_ref_and_keys(join->thd, keyuse_array, stat, join->tables, conds,
                            join->cond_equal, ~outer_join, join->select_lex))
      DBUG_RETURN(1);
    ...
    for (JOIN_TAB **pos = stat_vector + const_count; (s = *pos); pos++) {
      ...
      if (s->dependent) // If dependent on some table
      {
        ...
        if (table->file->records <= 1L &&
            !(table->file->table_flags() & HA_NOT_EXACT_COUNT) &&
            !table->pos_in_table_list->embedding) { // system table
          int tmp = 0;
          s->type = JT_SYSTEM;
          join->const_table_map |= table->map;
          set_position(join, const_count++, s, (KEYUSE *)0);
          if ((tmp = join_read_const_table(s, join->positions + const_count -
                                                  1))) {
            if (tmp > 0)
              DBUG_RETURN(1); // Fatal error
          } else
  ...
  for (s = stat; s < stat_end; s++) {
    ...
    */
    s->worst_seeks =
        min((double)s->found_records / 10, (double)s->read_time * 3);
...
static void add_key_field(KEY_FIELD **key_fields, uint and_level,
                          Item_func *cond, Field *field, bool eq_func,
    ...
    while ((item = it++)) {
      if (!field->eq(item->field)) {
        add_key_field(key_fields, and_level, cond, item->field, eq_func, val,
                      num_values, usable_tables);
      }
    }
  }
}

static void add_key_fields(KEY_FIELD **key_fields, uint *and_level, COND *cond,
                           table_map usable_tables) {
...
static void add_ft_keys(DYNAMIC_ARRAY *keyuse_array, JOIN_TAB *stat, COND *cond,
                        table_map usable_tables) {
...
static bool update_ref_and_keys(THD *thd, DYNAMIC_ARRAY *keyuse,
                                JOIN_TAB *join_tab, uint tables, COND *cond,
                                COND_EQUAL *cond_equal, table_map normal_tables,
                                SELECT_LEX *select_lex) {
  ...
  if (!(key_fields = (KEY_FIELD *)thd->alloc(
            sizeof(key_fields[0]) * (thd->lex->current_select->cond_count + 1) *
            2 * m)))
...
static void optimize_keyuse(JOIN *join, DYNAMIC_ARRAY *keyuse_array) {
  ...
  for (end = keyuse + keyuse_array->elements; keyuse < end; keyuse++) {
    ...
    */
    ...
    if (keyuse->used_tables &(map = (keyuse->used_tables &
...
static void add_group_and_distinct_keys(JOIN *join, JOIN_TAB *join_tab) {
  ...
  if (join->group_list) { /* Collect all query fields referenced in the GROUP
    ...
    for (cur_group = join->group_list; cur_group; cur_group = cur_group->next)
      (*cur_group->item)
          ->walk(&Item::collect_item_field_processor, (byte *)&indexed_fields);
...
static void best_access_path(JOIN *join, JOIN_TAB *s, THD *thd,
      ...
      if (ft_key) {
        ...
        */
        if (found_part == PREV_BITS(uint, keyinfo->key_parts) &&
            !found_ref_or_null) { /* use eq key */
          max_key_part = (uint) ~0;
          if ((keyinfo->flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME) {
            tmp = prev_record_reads(join, found_ref);
            records = 1.0;
          } else {
            if (!found_ref) { /* We found a const key */
              ...
              if (!(records =
                        keyinfo->rec_per_key[keyinfo->key_parts -
                                             1])) { /* Prefer longer keys */
                records = ((double)s->records / (double)rec *
                           (1.0 + ((double)(table->s->max_key_length -
                                            keyinfo->key_length) /
            ...
            if (table->used_keys.is_set(key)) {
              ...
              uint keys_per_block =
                  table->file->block_size / 2 /
                      (keyinfo->key_length + table->file->ref_length) +
                  1;
  ...
  */
  if ((records >= s->found_records || best > s->read_time) &&
      ...
      !(s->table->force_index && best_key)) { // Check full join
      ...
      */
      ...
      if (s->table->map & join->outer_join) // Can't use join cache
      {
        ...
        */
        ...
        tmp *=
            (1.0 + floor((double)cache_record_length(join, idx) * record_count /
...
static void optimize_straight_join(JOIN *join, table_map join_tables) {
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++) {
    ...
    best_access_path(join, s, join->thd, join_tables, idx, record_count,
                     read_time);
...
static void greedy_search(JOIN *join, table_map remaining_tables,
                          uint search_depth, uint prune_level) {
  double record_count = 1.0;
  double read_time = 0.0;
...
static void best_extension_by_limited_search(
    JOIN *join, table_map remaining_tables, uint idx, double record_count,
  ...
  */
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++) {
    ...
    if ((remaining_tables & real_table_bit) &&
      ...
      /* Find the best access method from 's' to the current partial plan */
      best_access_path(join, s, thd, remaining_tables, idx, record_count,
                       read_time);
      ...
      */
      ...
      if ((search_depth > 1) &&
          (remaining_tables &
           ~real_table_bit)) { /* Recursively expand the current partial plan */
        swap_variables(JOIN_TAB *, join->best_ref[idx], *pos);
        best_extension_by_limited_search(
            join, remaining_tables & ~real_table_bit, idx + 1,
            current_record_count, current_read_time, search_depth - 1,
            prune_level);
        if (thd->killed)
          DBUG_VOID_RETURN;
        swap_variables(JOIN_TAB *, join->best_ref[idx], *pos);
        ...
        DBUG_EXECUTE("opt",
                     print_plan(join, current_read_time, current_record_count,
                                idx, "full_plan"););
      }
    }
  }
...
/*
  ...
  tested and accepted.
*/
static void find_best(JOIN *join, table_map rest_tables, uint idx,
                      double record_count, double read_time) {
  ha_rows rec;
  double tmp;
  THD *thd = join->thd;
  ...
  if (!rest_tables) {
    DBUG_PRINT("best",
               ("read_time: %g  record_count: %g", read_time, record_count));
          ...
          do {
            ...
            do {
              if (!(rest_tables & keyuse->used_tables) &&
                ...
                found_part |= keyuse->keypart_map;
                double tmp =
                    prev_record_reads(join, (found_ref | keyuse->used_tables));
                ...
                if (table->used_keys.is_set(key)) {
                  ...
                  uint keys_per_block =
                      table->file->block_size / 2 /
                          (keyinfo->key_length + table->file->ref_length) +
                      1;
                  tmp = record_count * (tmp + keys_per_block - 1) /
                        keys_per_block;
                } else
                  tmp = record_count * min(tmp, s->worst_seeks);
              }
...
static bool create_ref_for_key(JOIN *join, JOIN_TAB *j, KEYUSE *org_keyuse,
  ...
  if (!(j->ref.key_buff = (byte *)thd->calloc(ALIGN_SIZE(length) * 2)) ||
      !(j->ref.key_copy =
            (store_key **)thd->alloc((sizeof(store_key *) *(keyparts + 1)))) ||
  ...
  if (ftkey) {
    ...
    for (i = 0; i < keyparts; keyuse++, i++) {
      ...
      if (!keyuse->used_tables &&
          !(join->select_options &
            SELECT_DESCRIBE)) { // Compare against constant
        ...
        *ref_key++ =
            get_store_key(thd, keyuse, join->const_table_map,
                          &keyinfo->key_part[i], (char *)key_buff, maybe_null);
      /*
  ...
  *ref_key = 0; // end_marker
  ...
  else if (((keyinfo->flags &
             (HA_NOSAME | HA_NULL_PART_KEY | HA_END_SPACE_KEY)) != HA_NOSAME) ||
...
static store_key *get_store_key(THD *thd, KEYUSE *keyuse, table_map used_tables,
  ...
  {
    return new store_key_const_item(thd, key_part->field, key_buff + maybe_null,
                                    maybe_null ? key_buff : 0, key_part->length,
                                    keyuse->val);
  } else if (keyuse->val->type() == Item::FIELD_ITEM)
    return new store_key_field(thd, key_part->field, key_buff + maybe_null,
                               maybe_null ? key_buff : 0, key_part->length,
                               ((Item_field *)keyuse->val)->field,
                               keyuse->val->full_name());
  return new store_key_item(thd, key_part->field, key_buff + maybe_null,
                            maybe_null ? key_buff : 0, key_part->length,
                            keyuse->val);
}
        ...
        if (tab->ref.null_rejecting & (1 << keypart)) {
          ...
          */
          if (notnull->fix_fields(join->thd, &notnull))
            DBUG_VOID_RETURN;
          DBUG_EXECUTE("where",
                       print_where(notnull, referred_tab->table->alias););
...
static COND *add_found_match_trig_cond(JOIN_TAB *tab, COND *cond,
                                       JOIN_TAB *root_tab) {
  COND *tmp;
      ...
      { // Check const tables
        ...
        for (JOIN_TAB *tab = join->join_tab + join->const_tables;
          ...
          if (*tab->on_expr_ref) {
            ...
            COND *tmp = make_cond_for_table(
                *tab->on_expr_ref, join->const_table_map, (table_map)0);
            if (!tmp)
              continue;
    ...
    for (uint i = join->const_tables; i < join->tables; i++) {
      ...
      */
      ...
      if (tmp || !cond) {
        ...
        SQL_SELECT *sel = tab->select =
            (SQL_SELECT *)thd->memdup((gptr)select, sizeof(SQL_SELECT));
        if (!sel)
...
*/
          ...
          if (i != join->const_tables &&
              tab->use_quick != 2) { /* Read with cache */
            if (cond &&
                (tmp = make_cond_for_table(
                     cond, join->const_table_map | current_map, current_map))) {
              DBUG_EXECUTE("where", print_where(tmp, "cache"););
              tab->cache.select =
                  (SQL_SELECT *)thd->memdup((gptr)sel, sizeof(SQL_SELECT));
      ...
      */
      ...
      while (first_inner_tab && first_inner_tab->last_inner == last_tab) {
        ...
        table_map used_tables =
            join->const_table_map | OUTER_REF_TABLE_BIT | RAND_TABLE_BIT;
        for (tab = join->join_tab + join->const_tables; tab <= last_tab;
             tab++) {
          ...
          if (tmp) {
            ...
            /*
              ...
              all embedding outer join operations.
                */
            if (!(tmp = add_found_match_trig_cond(cond_tab->first_inner, tmp,
                                                  first_inner_tab)))
              DBUG_RETURN(1);
            /*
            ...
            tmp =
                new Item_func_trig_cond(tmp, &first_inner_tab->not_null_compl);
            ...
            cond_tab->select_cond =
                !cond_tab->select_cond
                    ...
                    : new Item_cond_and(cond_tab->select_cond, tmp);
            DBUG_PRINT("info",
                       ("Item_cond_and 0x%lx", (ulong)cond_tab->select_cond));
            if (!cond_tab->select_cond)
              DBUG_RETURN(1);
            cond_tab->select_cond->quick_fix_field();
          }
...
static void make_join_readinfo(JOIN *join, uint options) {
  ...
  for (i = join->const_tables; i < join->tables; i++) {
      ...
      if (tab->use_quick == 2) {
        ...
        if (i == join->const_tables) {
          ...
          if (tab->select && tab->select->quick) {
            if (statistics)
              statistic_increment(
                  join->thd->status_var.select_full_range_join_count,
                  &LOCK_status);
          } else {
            join->thd->server_status |= SERVER_QUERY_NO_INDEX_USED;
            if (statistics)
        ...
        if (!table->no_keyread) {
          if (tab->select && tab->select->quick &&
            ...
            table->file->extra(HA_EXTRA_KEYREAD);
                     ...
                     !(tab->select &&
                       tab->select->quick)) { // Only read index tree
    ...
    default:
      DBUG_PRINT("error",
                 ("Table type %d found", tab->type)); /* purecov: deadcode */
...
void JOIN::join_free() {
  ...
  */
  if (can_unlock && lock && thd->lock && !(select_options & SELECT_NO_UNLOCK) &&
...
static void update_depend_map(JOIN *join) {
  ...
  for (; join_tab != end; join_tab++) {
    ...
    for (JOIN_TAB **tab = join->map2table; depend_map;
         tab++, depend_map >>= 1) {
      if (depend_map & 1)
      ...
      *simple_order = 0; // Must do a temp table to sort
      ...
      if (order_tables & (RAND_TABLE_BIT | OUTER_REF_TABLE_BIT))
        *simple_order = 0;
        ...
        Item *comp_item = 0;
        if (cond &&
            const_expression_in_where(cond, order->item[0], &comp_item)) {
          DBUG_PRINT("info", ("removing: %s", order->item[0]->full_name()));
          continue;
        }
...
static int return_zero_rows(JOIN *join, select_result *result,
                            TABLE_LIST *tables, List<Item> &fields,
                            bool send_row, uint select_options,
    ...
    if (left_item->type() == Item::FIELD_ITEM &&
      ...
      if (left_item_equal) {
        ...
        if (right_item_equal)
          ...
          Item_equal *item =
              new Item_equal((Item_field *)left_item, (Item_field *)right_item);
...

    {
      ...
      if (const_item &&
        ...
        Item_equal *item_equal =
            find_item_equal(cond_equal, field_item->field, &copyfl);
        if (copyfl) {
...
static void change_cond_ref_to_const(THD *thd, I_List<COND_CMP> *save_list,
...
static void propagate_cond_constants(THD *thd, I_List<COND_CMP> *save_list,
...
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list,
  ...
  if (!conds)
    ...
    */
    ...
    conds =
        build_equal_items(join->thd, conds, NULL, join_list, &join->cond_equal);
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    *cond_value = Item::COND_UNDEF;
    ...
    if (!((Item_cond *)cond)->argument_list()->elements ||
      ...
      return (COND *)0;
    if (((Item_cond *)cond)->argument_list()->elements == 1) { // Remove list
    ...
    */
    ...
    if (args[0]->type() == Item::FIELD_ITEM) {
      ...
      if (field->flags & AUTO_INCREMENT_FLAG && !field->table->maybe_null &&
          (thd->options & OPTION_AUTO_IS_NULL) && thd->insert_id()) {
#ifdef HAVE_QUERY_CACHE
        query_cache_abort(&thd->net);
#endif
        COND *new_cond;
        if ((new_cond = new Item_func_eq(
                 args[0],
                 new Item_int("last_insert_id()", thd->insert_id(), 21)))) {
          cond = new_cond;
          /*
    ...
    *cond_value = eval_const_cond(cond) ? Item::COND_TRUE : Item::COND_FALSE;
    return (COND *)0;
  } else if ((*cond_value = cond->eq_cmp_result()) !=
             Item::COND_OK) { // boolan compare function
...
static bool const_expression_in_where(COND *cond, Item *comp_item,
                                      Item **const_item) {
  if (cond->type() == Item::COND_ITEM) {
    bool and_level =
        (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC);
...
Field *create_tmp_field_from_field(THD *thd, Field *org_field, const char *name,
  ...
  if (convert_blob_length && (org_field->flags & BLOB_FLAG))
    new_field =
        new Field_varstring(convert_blob_length, org_field->maybe_null(),
                            org_field->field_name, table, org_field->charset());
  else
...
static Field *create_tmp_field_from_item(THD *thd, Item *item, TABLE *table,
  ...
  case REAL_RESULT:
    new_field = new Field_double(item->max_length, maybe_null, item->name,
                                 table, item->decimals);
    break;
  case INT_RESULT:
    new_field = new Field_longlong(item->max_length, maybe_null, item->name,
                                   table, item->unsigned_flag);
    break;
  case STRING_RESULT:
    ...
    */
    if ((type = item->field_type()) == MYSQL_TYPE_DATETIME ||
      ...
      new_field = item->tmp_table_field_from_field_type(table);
             ...
             convert_blob_length)
      new_field =
          new Field_varstring(convert_blob_length, maybe_null, item->name,
  ...
  case DECIMAL_RESULT:
    new_field =
        new Field_new_decimal(item->max_length, maybe_null, item->name, table,
...
Field *create_tmp_field_for_schema(THD *thd, Item *item, TABLE *table) {
  if (item->field_type() == MYSQL_TYPE_VARCHAR) {
    if (item->max_length >
        MAX_FIELD_VARCHARLENGTH / item->collation.collation->mbmaxlen)
      return new Field_blob(item->max_length, item->maybe_null, item->name,
                            table, item->collation.collation);
...
Field *create_tmp_field(THD *thd, TABLE *table, Item *item, Item::Type type,
                        Item ***copy_func, Field **from_field, bool group,
                        bool modify_item, bool table_cant_handle_bit_fields,
                        uint convert_blob_length) {
  ...
  case Item::SUM_FUNC_ITEM: {
    ...
    Field *result =
        item_sum->create_tmp_field(group, table, convert_blob_length);
    if (!result)
      thd->fatal_error();
    return result;
  }
  ...
  case Item::DEFAULT_VALUE_ITEM: {
    ...
    */
    if (field->maybe_null && !field->field->maybe_null()) {
      result = create_tmp_field_from_item(thd, item, table, NULL, modify_item,
                                          convert_blob_length);
      ...
      *from_field = field->field;
      ...
      result = create_tmp_field_from_field(
          thd, (*from_field = field->field), item->name, table,
...
TABLE *create_tmp_table(THD *thd, TMP_TABLE_PARAM *param, List<Item> &fields,
  ...
  if (temp_pool_slot != MY_BIT_NONE) // we got a slot
    ...
    sprintf(path, "%s%lx_%lx_%x", tmp_file_prefix, current_pid, thd->thread_id,
            thd->tmp_table++);
  }
  ...
  */
  ...
  if (!multi_alloc_root(
           &own_root, &table, sizeof(*table), &reg_field,
           sizeof(Field *) * (field_count + 1), &blob_field,
           sizeof(uint) * (field_count + 1), &from_field,
           sizeof(Field *) * field_count, &copy_func,
           sizeof(*copy_func) * (param->func_count + 1), &param->keyinfo,
  ...
  while ((item = li++)) {
    ...
    if (type == Item::SUM_FUNC_ITEM && !group &&
        ...
        if (!arg->const_item()) {
          Field *new_field =
              create_tmp_field(thd, table, arg, arg->type(), &copy_func,
                               ...
                               distinct, param->convert_blob_length);
          if (!new_field)
  ...
  *blob_field = 0; // End marker
  ...
  if (blob_count ||
      string_total_length >= STRING_TOTAL_LENGTH_TO_PACK_ROWS &&
          (reclength / string_total_length <= RATIO_TO_PACK_ROWS ||
           string_total_length / string_count >=
  ...
  table->s->reclength = reclength;
  {
    ...
    if (!(table->record[0] =
              (byte *)alloc_root(&table->mem_root, alloc_length * 3)))
      goto err;
  ...
  if (group) {
    ...
    for (; group; group = group->next, key_part_info++) {
      ...
      if (!using_unique_constraint) {
        ...
        if (!(group->field = field->new_key_field(
                  thd->mem_root, table, (char *)group_buff + test(maybe_null),
        ...
        if (maybe_null) {
          ...
          */
          ...
          key_part_info->null_offset =
              (uint)(field->null_ptr - (uchar *)table->record[0]);
  ...
  if (distinct) {
    ...
    */
    ...
    null_pack_length -= hidden_null_pack_length;
    keyinfo->key_parts =
        ((field_count - param->hidden_field_count) + test(null_pack_length));
    ...
    if (!(key_part_info = (KEY_PART_INFO *)alloc_root(
              &table->mem_root, keyinfo->key_parts * sizeof(KEY_PART_INFO))))
      goto err;
    ...
    if (null_pack_length) {
      ...
      key_part_info->field = new Field_string(
          (char *)table->record[0], (uint32)key_part_info->length, (uchar *)0,
...
TABLE *create_virtual_tmp_table(THD *thd, List<create_field> &field_list) {
  ...
  while ((cdef = it++)) {
    *field = make_field(
        0, cdef->length, (uchar *)(f_maybe_null(cdef->pack_flag) ? "" : 0),
        f_maybe_null(cdef->pack_flag) ? 1 : 0, cdef->pack_flag, cdef->sql_type,
        cdef->charset, cdef->geom_type, cdef->unireg_check, cdef->interval,
        cdef->field_name, table);
    if (!*field)
      goto error;
...
static bool create_myisam_tmp_table(TABLE *table, TMP_TABLE_PARAM *param,
  ...
  if (table->s->keys) { // Get keys for ni_create
    bool using_unique_constraint = 0;
    ...
    for (uint i = 0; i < keyinfo->key_parts; i++, seg++) {
      ...
      if (field->flags & BLOB_FLAG) {
        seg->type = ((keyinfo->key_part[i].key_type & FIELDFLAG_BINARY)
                         ...
                         : HA_KEYTYPE_VARTEXT2);
        seg->bit_start =
            (uint8)(field->pack_length() - table->s->blob_ptr_size);
  ...
  if ((error = mi_create(table->s->table_name, table->s->keys, &keydef,
                         ...
                         param->start_recinfo, table->s->uniques, &uniquedef,
                         &create_info, HA_CREATE_TMP_TABLE))) {
    table->file->print_error(error, MYF(0)); /* purecov: inspected */
    table->db_stat = 0;
    goto err;
  }
...
static int do_select(JOIN *join, List<Item> *fields, TABLE *table,
                     Procedure *procedure) {
  int rc = 0;
  ...
  if (join->tables == join->const_tables) {
    ...
    */
    if (!join->conds || join->conds->val_int()) {
      Next_select_func end_select =
          join->join_tab[join->tables - 1].next_select;
...
enum_nested_loop_state sub_select_cache(JOIN *join, JOIN_TAB *join_tab,
                                        bool end_of_records) {
...
enum_nested_loop_state sub_select(JOIN *join, JOIN_TAB *join_tab,
                                  bool end_of_records) {
  ...
  if (rc == NESTED_LOOP_NO_MORE_ROWS && join_tab->last_inner &&
      !join_tab->found)
...
flush_cached_records(JOIN *join, JOIN_TAB *join_tab, bool skip_last) {
  ...
  if (join_tab->use_quick == 2) {
    if (join_tab->select->quick) { /* Used quick select last. reset it */
...
int report_error(TABLE *table, int error) {
  ...
  */
  if (error != HA_ERR_LOCK_DEADLOCK && error != HA_ERR_LOCK_WAIT_TIMEOUT)
    sql_print_error("Got error %d when reading table '%s'", error,
                    table->s->path);
  table->file->print_error(error, MYF(0));
  return 1;
}
...
int safe_index_read(JOIN_TAB *tab) {
  int error;
  TABLE *table = tab->table;
  if ((error = table->file->index_read(table->record[0], tab->ref.key_buff,
...
static int join_read_const_table(JOIN_TAB *tab, POSITION *pos) {
  ...
  if (tab->type == JT_SYSTEM) {
    if ((error = join_read_system(tab))) { // Info for DESCRIBE
...
static int join_read_const(JOIN_TAB *tab) {
  ...
  if (table->status & STATUS_GARBAGE) // If first read
  {
    ...
    if (cp_buffer_from_ref(tab->join->thd, &tab->ref))
      ...
      error = table->file->index_read_idx(
          table->record[0], tab->ref.key, (byte *)tab->ref.key_buff,
          tab->ref.key_length, HA_READ_KEY_EXACT);
    }
    if (error) {
...
static int join_read_key(JOIN_TAB *tab) {
  ...
  if (cmp_buffer_with_ref(tab) ||
    ...
    error = table->file->index_read(table->record[0], tab->ref.key_buff,
                                    tab->ref.key_length, HA_READ_KEY_EXACT);
...
static int join_read_always_key(JOIN_TAB *tab) {
  ...
  if (cp_buffer_from_ref(tab->join->thd, &tab->ref))
    return -1;
  if ((error =
           table->file->index_read(table->record[0], tab->ref.key_buff,
                                   tab->ref.key_length, HA_READ_KEY_EXACT))) {
    if (error != HA_ERR_KEY_NOT_FOUND)
      return report_error(table, error);
...
static int join_read_last_key(JOIN_TAB *tab) {
  ...
  if (cp_buffer_from_ref(tab->join->thd, &tab->ref))
    return -1;
  if ((error = table->file->index_read_last(table->record[0], tab->ref.key_buff,
                                            tab->ref.key_length))) {
    if (error != HA_ERR_KEY_NOT_FOUND)
      return report_error(table, error);
    return -1; /* purecov: inspected */
...
static int join_read_next_same(READ_RECORD *info) {
  ...
  if ((error = table->file->index_next_same(table->record[0], tab->ref.key_buff,
                                            tab->ref.key_length))) {
    if (error != HA_ERR_END_OF_FILE)
      return report_error(table, error);
    table->status = STATUS_GARBAGE;
...
static int test_if_quick_select(JOIN_TAB *tab) {
  delete tab->select->quick;
  tab->select->quick = 0;
  return tab->select->test_quick_select(tab->join->thd, tab->keys, (table_map)0,
                                        HA_POS_ERROR, 0);
}
...
static int join_init_read_record(JOIN_TAB *tab) {
  if (tab->select && tab->select->quick && tab->select->quick->reset())
    return 1;
  init_read_record(&tab->read_record, tab->join->thd, tab->table, tab->select,
                   1, 1);
...
static enum_nested_loop_state end_update(JOIN *join, JOIN_TAB *join_tab
  ...
  if (!table->file->index_read(table->record[1],
                               ...
                               HA_READ_KEY_EXACT)) { /* Update old record */
  ...
  */
  ...
  copy_funcs(join->tmp_table_param.items_to_copy);
  if ((error = table->file->write_row(table->record[0]))) {
    if (create_myisam_from_heap(join->thd, table, &join->tmp_table_param, error,
                                0))
      DBUG_RETURN(NESTED_LOOP_ERROR); // Not a table_is_full error
    ...
    if (join->first_record || (end_of_records && !join->group)) {
      ...
      if (idx < send_group_parts) {
        ...
        if (!join->having || join->having->val_int()) {
          ...
          if (error && create_myisam_from_heap(
                           join->thd, table, &join->tmp_table_param, error, 0))
            DBUG_RETURN(NESTED_LOOP_ERROR);
        }
  ...
  if (!field->table->const_table && !field->table->maybe_null) {
    ...
    if (ref_item && ref_item->eq(right_item, 1)) {
      ...
      if (right_item->const_item() && !(right_item->is_null())) {
        ...
        */
        if (field->binary() && field->real_type() != MYSQL_TYPE_STRING &&
...
static COND *make_cond_for_table(COND *cond, table_map tables,
                                 table_map used_table) {
...
static int test_if_order_by_key(ORDER *order, TABLE *table, uint idx,
  ...
  for (; order; order = order->next, const_key_parts >>= 1) {
    ...
    */
    ...
    flag =
        ((order->asc == !(key_part->key_part_flag & HA_REVERSE_SORT)) ? 1 : -1);
...
static uint test_if_subkey(ORDER *order, TABLE *table, uint ref,
...
static bool test_if_skip_sort_order(JOIN_TAB *tab, ORDER *order,
  ...
  if (ref_key >= 0) {
    ...
    */
    ...
    if (usable_keys.is_set(ref_key) &&
        (order_direction =
             test_if_order_by_key(order, table, ref_key, &used_key_parts))) {
        ...
        if (select && select->quick) {
          ...
          */
          if (!select->quick->reverse_sorted()) {
            ...
            QUICK_SELECT_DESC *tmp = new QUICK_SELECT_DESC(
                (QUICK_RANGE_SELECT *)(select->quick), used_key_parts);
            if (!tmp || tmp->error) {
              delete tmp;
        ...
        if ((flag = test_if_order_by_key(order, table, nr, &not_used))) {
          if (!no_changes) {
            ...
            tab->read_first_record =
                (flag > 0 ? join_read_first : join_read_last);
            tab->type = JT_NEXT; // Read with index_first(), index_next()
...
static int create_sort_index(THD *thd, JOIN *join, ORDER *order,
  ...
  table->sort.io_cache =
      (IO_CACHE *)my_malloc(sizeof(IO_CACHE), MYF(MY_WME | MY_ZEROFILL));
  ...
  table->sort.found_records = filesort(thd, table, sortorder, length, select,
                                       filesort_limit, &examined_rows);
...
static bool fix_having(JOIN *join, Item **having) {
  ...
  Item *sort_table_cond =
      make_cond_for_table(*having, used_tables, used_tables);
  if (sort_table_cond) {
    if (!table->select)
    ...
    if (!table->select->cond)
      table->select->cond = sort_table_cond;
        ...
        if (!(table->select->cond =
                  new Item_cond_and(table->select->cond, sort_table_cond)) ||
            table->select->cond->fix_fields(join->thd, &table->select->cond))
      return 1;
    table->select_cond = table->select->cond;
    table->select_cond->top_level_item();
    DBUG_EXECUTE("where",
                 print_where(table->select_cond, "select and having"););
...
static int remove_duplicates(JOIN *join, TABLE *entry, List<Item> &fields,
  ...
  if (!field_count &&
      !(join->select_options &
        OPTION_FOUND_ROWS)) { // only const items with no OPTION_FOUND_ROWS
    join->unit->select_limit_cnt = 1; // Only send first row
    DBUG_RETURN(0);
  }
  Field **first_field = entry->field + entry->s->fields - field_count;
  offset =
      field_count ? entry->field[entry->s->fields - field_count]->offset() : 0;
  ...
  if (entry->s->db_type == DB_TYPE_HEAP ||
    ...
    error = remove_dup_with_hash_index(join->thd, entry, field_count,
                                       first_field, reclength, having);
  else
    error =
        remove_dup_with_compare(join->thd, entry, first_field, offset, having);
...
static int remove_dup_with_hash_index(THD *thd, TABLE *table, uint field_count,
  ...
  if (!my_multi_malloc(
           MYF(MY_WME), &key_buffer,
           (uint)((key_length + extra_length) * (long)file->records),
  ...
  if (hash_init(&hash, &my_charset_bin, (uint)file->records, 0, key_length,
                (hash_get_key)0, 0, 0)) {
...
SORT_FIELD *make_unireg_sortorder(ORDER *order, uint *length) {
  ...
  for (; order; order = order->next, pos++) {
    ...
    else if (order->item[0]->type() == Item::COPY_STR_ITEM) { // Blob patch
...
static int join_init_cache(THD *thd, JOIN_TAB *tables, uint table_count) {
  ...
  if (!(cache->field = (CACHE_FIELD *)sql_alloc(
            sizeof(CACHE_FIELD) * (cache->fields + table_count * 2) +
  ...
  copy = cache->field;
  blob_ptr = cache->blob_ptr =
      (CACHE_FIELD **)(cache->field + cache->fields + table_count * 2);
  ...
  for (i = 0; i < table_count; i++) {
    ...
    if (null_fields &&
        tables[i].table->s->null_fields) { /* must copy null bits */
...
static bool find_order_in_list(THD *thd, Item **ref_pointer_array,
  ...
  */
  ...
  if (!order_item->fixed &&
      (order_item->fix_fields(thd, order->item) ||
       (order_item = *order->item)->check_cols(1) || thd->is_fatal_error)) {
...
*/
    ...
    ref_pointer_array
        [((i < border) ? all_fields.elements - i - 1 : i - border)] = pos;
  }
  param->copy_field_end = copy;
...
bool JOIN::alloc_func_list() {
  ...
  */
  ...
  sum_funcs =
      (Item_sum **)thd->calloc(sizeof(Item_sum **) * (func_count + 1) +
...
static bool change_to_use_tmp_fields(THD *thd, Item **ref_pointer_array,
  ...
  for (i = 0; (item = it++); i++) {
    ...
    ref_pointer_array
        [((i < border) ? all_fields.elements - i - 1 : i - border)] =
            item_field;
  }
...
static bool change_refs_to_tmp_fields(THD *thd, Item **ref_pointer_array,
  ...
  for (i = 0; (item = it++); i++) {
    ...
    ref_pointer_array
        [((i < border) ? all_fields.elements - i - 1 : i - border)] = new_item;
  }
...
static bool add_ref_to_table_cond(THD *thd, JOIN_TAB *join_tab) {
  ...
  for (uint i = 0; i < join_tab->ref.key_parts; i++) {
    Field *field =
        table->field
            [table->key_info[join_tab->ref.key].key_part[i].fieldnr - 1];
  ...
  } else if ((join_tab->select =
                  make_select(join_tab->table, 0, 0, cond, 0, &error)))
...
static bool change_group_ref(THD *thd, Item_func *expr, ORDER *group_list,
  ...
  if (expr->arg_count) {
    ...
    for (arg = expr->arguments(), arg_end = expr->arguments() + expr->arg_count;
         arg != arg_end; arg++) {
      Item *item = *arg;
      if (item->type() == Item::FIELD_ITEM || item->type() == Item::REF_ITEM) {
        ...
        for (group_tmp = group_list; group_tmp; group_tmp = group_tmp->next) {
          if (item->eq(*group_tmp->item, 0)) {
            Item *new_item;
            if (!(new_item =
                      new Item_ref(context, group_tmp->item, 0, item->name)))
...
bool JOIN::rollup_init() {
  ...
  */
  ...
  if (!(rollup.null_items = (Item_null_result **)thd->alloc(
            (sizeof(Item *) + sizeof(Item **) + sizeof(List<Item>) +
          ...
          if (*group_tmp->item == item) {
            ...
            */
            Item_null_result *null_item =
                new (thd->mem_root) Item_null_result();
            if (!null_item)
              return 1;
...
int JOIN::rollup_send_data(uint idx) {
  ...
  for (i = send_group_parts; i-- > idx;) {
    ...
    memcpy((char *)ref_pointer_array, (char *)rollup.ref_pointer_arrays[i],
           ref_pointer_array_size);
    if ((!having || having->val_int())) {
...
static void select_describe(JOIN *join, bool need_tmp_table, bool need_order,
  ...
  DBUG_PRINT("info",
             ("Select 0x%lx, type %s, message %s", (ulong)join->select_lex,
              join->select_lex->type, message ? message : "NULL"));
    ...
    */
    ...
    /* table */
    {
      ...
      for (; sl && len + lastop + 5 < NAME_LEN; sl = sl->next_select()) {
        len += lastop;
        lastop = my_snprintf(table_name_buffer + len, NAME_LEN - len, "%u,",
                             sl->select_number);
      }
    ...
    for (uint i = 0; i < join->tables; i++) {
      ...
      /* id */
      item_list.push_back(
          new Item_uint((uint32)join->select_lex->select_number));
      /* select_type */
      item_list.push_back(new Item_string(join->select_lex->type,
                                          strlen(join->select_lex->type), cs));
      if (tab->type == JT_ALL && tab->select && tab->select->quick) {
      ...
      if (table->derived_select_number) {
        ...
        item_list.push_back(
            new Item_string(table->alias, strlen(table->alias), cs));
      /* type */
      item_list.push_back(new Item_string(
          join_type_str[tab->type], strlen(join_type_str[tab->type]), cs));
        ...
        for (j = 0; j < table->s->keys; j++) {
          if (tab->keys.is_set(j)) {
            ...
            tmp1.append(table->key_info[j].name,
                        strlen(table->key_info[j].name), system_charset_info);
          }
        }
      }
      if (tmp1.length())
      ...
      if (tab->ref.key_parts) {
        ...
        item_list.push_back(new Item_string(
            key_info->name, strlen(key_info->name), system_charset_info));
        ...
        register uint length;
        item_list.push_back(
            new Item_string(key_info->name, strlen(key_info->name), cs));
      ...
      item_list.push_back(new Item_int(
          (longlong)(ulonglong)join->best_positions[i].records_read, 21));
      ...
      if (quick_type == QUICK_SELECT_I::QS_TYPE_ROR_INTERSECT &&
          !((QUICK_ROR_INTERSECT_SELECT *)tab->select->quick)
               ->need_to_fetch_row)
        key_read = 1;
  ...
  for (SELECT_LEX_UNIT *unit = join->select_lex->first_inner_unit(); unit;
       unit = unit->next_unit()) {
...
bool mysql_explain_union(THD *thd, SELECT_LEX_UNIT *unit,
  ...
  for (SELECT_LEX *sl = first; sl; sl = sl->next_select()) {
    ...
    sl->type =
        (((&thd->lex->select_lex) == sl)
             ? ((thd->lex->all_selects_list != sl) ? primary_key_name
                                                   : "SIMPLE")
             : ((sl == first)
                    ...
                    : ((uncacheable & UNCACHEABLE_DEPENDENT)
                           ? "DEPENDENT UNION"
                           : uncacheable ? "UNCACHEABLE UNION" : "UNION")));
  ...
  if (first->next_select()) {
    ...
    unit->set_limit(unit->global_parameters);
    res = mysql_select(
        thd, &first->ref_pointer_array, (TABLE_LIST *)first->table_list.first,
        first->with_wild, first->item_list, first->where,
        first->order_list.elements + first->group_list.elements,
        (ORDER *)first->order_list.first, (ORDER *)first->group_list.first,
        first->having, (ORDER *)thd->lex->proc_list.first,
        first->options | thd->options | SELECT_DESCRIBE, result, unit, first);
  }
...
static void print_join(THD *thd, String *str, List<TABLE_LIST> *tables) {
  ...
  TABLE_LIST **table =
      (TABLE_LIST **)thd->alloc(sizeof(TABLE_LIST *) * tables->elements);
  if (table == 0)
