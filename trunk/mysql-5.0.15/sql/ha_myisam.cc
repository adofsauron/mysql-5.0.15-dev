...
TYPELIB myisam_recover_typelib = {
  array_elements(myisam_recover_names) - 1, "", myisam_recover_names, NULL
};
...
const char *ha_myisam::index_type(uint key_number) {
  return (
      (table->key_info[key_number].flags & HA_FULLTEXT)
          ? "FULLTEXT"
...
int ha_myisam::restore(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  if (my_copy(src_path,
              fn_format(dst_path, table->s->path, "", MI_NAME_DEXT, 4),
              MYF(MY_WME))) {
...
int ha_myisam::backup(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  if (my_copy(
          fn_format(src_path, table->s->path, "", reg_ext, MY_UNPACK_FILENAME),
          dst_path,
...
int ha_myisam::repair(THD *thd, MI_CHECK &param, bool optimize) {
  ...
  if (!error) {
    ...
    */
    ...
    if (optimize_done)
      error = update_state_info(
          &param, file, UPDATE_TIME | UPDATE_OPEN_COUNT |
  ...
  DBUG_RETURN(error ? HA_ADMIN_FAILED : !optimize_done ? HA_ADMIN_ALREADY_DONE
                                                       : HA_ADMIN_OK);
}
...
int ha_myisam::assign_to_keycache(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  if ((error = mi_assign_to_key_cache(file, map, new_key_cache))) {
    ...
    my_snprintf(buf, sizeof(buf), "Failed to flush to index file (errno: %d)",
                error);
    errmsg = buf;
...
int ha_myisam::preload_keys(THD *thd, HA_CHECK_OPT *check_opt) {
  ...
  if ((error = mi_preload(file, map, ignore_leaves))) {
    ...
    default:
      ...
      my_snprintf(buf, ERRMSGSIZE, "Failed to read from index file (errno: %d)",
                  my_errno);
      errmsg = buf;
    }
    error = HA_ADMIN_FAILED;
...
int ha_myisam::enable_indexes(uint mode) {
  ...
  if (mode == HA_KEY_SWITCH_ALL) {
    ...
    */
    ...
    param.stats_method =
        (enum_mi_stats_method)thd->variables.myisam_stats_method;
    param.tmpdir = &mysql_tmpdir_list;
...
void ha_myisam::start_bulk_insert(ha_rows rows) {
  ...
  THD *thd = current_thd;
  ulong size =
      min(thd->variables.read_buff_size, table->s->avg_row_length * rows);
  DBUG_PRINT("info",
...
int ha_myisam::end_bulk_insert() {
  ...
  return err ? err : can_enable_indexes
                         ? enable_indexes(HA_KEY_SWITCH_NONUNIQ_SAVE)
                         : 0;
}
...
int ha_myisam::index_read(byte *buf, const byte *key, uint key_len,
...
int ha_myisam::index_read_last(byte *buf, const byte *key, uint key_len) {
  ...
  statistic_increment(table->in_use->status_var.ha_read_key_count,
                      &LOCK_status);
  int error =
      mi_rkey(file, buf, active_index, key, key_len, HA_READ_PREFIX_LAST);
  table->status = error ? STATUS_NOT_FOUND : 0;
  return error;
}
...
THR_LOCK_DATA **ha_myisam::store_lock(THD *thd, THR_LOCK_DATA **to,
...
int ha_myisam::create(const char *name, register TABLE *table_arg,
  ...
  if (!(my_multi_malloc(MYF(MY_WME), &recinfo,
                        (share->fields * 2 + 2) * sizeof(MI_COLUMNDEF), &keydef,
  ...
  for (i = 0; i < share->keys; i++, pos++) {
    ...
    keydef[i].key_alg =
        pos->algorithm == HA_KEY_ALG_UNDEF
    ...
    for (j = 0; j < pos->key_parts; j++) {
      ...
      if (field->null_ptr) {
        ...
        keydef[i].seg[j].null_pos =
            (uint)(field->null_ptr - (uchar *)table_arg->record[0]);
      } else {
      ...
      if (field->type() == FIELD_TYPE_BLOB ||
        ...
        keydef[i].seg[j].bit_start =
            (uint)(field->pack_length() - share->blob_ptr_size);
  ...
  while (recpos < (uint)share->reclength) {
    ...
    if (found->null_ptr) {
      ...
      recinfo_pos->null_pos =
          (uint)(found->null_ptr - (uchar *)table_arg->record[0]);
    } else {
  ...
  create_info.auto_increment =
      (info->auto_increment_value ? info->auto_increment_value - 1
                                  : (ulonglong)0);
  create_info.data_file_length =
      ((ulonglong)share->max_rows * share->avg_row_length);
  create_info.raid_type = info->raid_type;
  create_info.raid_chunks =
      (info->raid_chunks ? info->raid_chunks : RAID_DEFAULT_CHUNKS);
  create_info.raid_chunksize =
      (info->raid_chunksize ? info->raid_chunksize : RAID_DEFAULT_CHUNKSIZE);
  ...
  error = mi_create(fn_format(buff, name, "", "", 2 + 4), share->keys, keydef,
                    (uint)(recinfo_pos - recinfo), recinfo, 0,
                    (MI_UNIQUEDEF *)0, &create_info, create_flags);
...
ulonglong ha_myisam::get_auto_increment() {
  ...
  error = mi_rkey(file, table->record[1], (int)table->s->next_number_index, key,
                  table->s->next_number_key_offset, HA_READ_PREFIX_LAST);
  if (error)
    nr = 1;
  else {
