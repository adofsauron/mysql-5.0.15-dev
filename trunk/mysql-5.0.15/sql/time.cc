...
timestamp_type str_to_datetime_with_warn(const char *str, uint length,
  ...
  ts_type = str_to_datetime(
      str, length, l_time, (flags | (thd->variables.sql_mode &
...
bool parse_date_time_format(timestamp_type format_type, const char *format,
                            uint format_length,
...
DATE_TIME_FORMAT
*date_time_format_make(timestamp_type format_type, const char *format_str,
                       uint format_length) {
  DATE_TIME_FORMAT tmp;

  if (format_length && format_length < 255 &&
      !parse_date_time_format(format_type, format_str, format_length, &tmp)) {
...
void make_truncated_value_warning(THD *thd, const char *str_val,
  ...
  if (field_name)
    cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff),
                       ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), type_str,
                       str.c_ptr(), field_name, (ulong)thd->row_count);
  else
    cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff),
                       ER(ER_TRUNCATED_WRONG_VALUE), type_str, str.c_ptr());
  push_warning(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE,
               warn_buff);
}
