...
int init_slave() {
  ...
  */
  ...
  if (master_host && !opt_skip_slave_start) {
    if (start_slave_threads(1 /* need mutex */, 0 /* no wait for start*/,
    ...
    while (look_for_description_event) {
      ...
      */
      ...
      /*
        ...
        event
      */
      if (!(ev = Log_event::read_log_event(
                rli->cur_log, 0, rli->relay_log.description_event_for_exec))) {
...
void init_slave_skip_errors(const char *arg) {
  ...
  for (; my_isspace(system_charset_info, *arg); ++arg)
    /* empty */;
  if (!my_strnncoll(system_charset_info, (uchar *)arg, 4, (const uchar *)"all",
                    4)) {
...
int terminate_slave_threads(MASTER_INFO *mi, int thread_mask, bool skip_lock) {
  ...
  if ((thread_mask & (SLAVE_IO | SLAVE_FORCE_ALL)) && mi->slave_running) {
    ...
    mi->abort_slave = 1;
    if ((error = terminate_slave_thread(mi->io_thd, io_lock, io_cond_lock,
                                        &mi->stop_cond, &mi->slave_running)) &&
        !force_all)
      DBUG_RETURN(error);
  }
  if ((thread_mask & (SLAVE_SQL | SLAVE_FORCE_ALL)) && mi->rli.slave_running) {
    ...
    if ((error = terminate_slave_thread(mi->rli.sql_thd, sql_lock,
                                        sql_cond_lock, &mi->rli.stop_cond,
  ...
  if (start_cond && cond_lock) // caller has cond_lock
  {
    ...
    while (start_id == *slave_run_id) {
      ...
      const char *old_msg = thd->enter_cond(
          start_cond, cond_lock, "Waiting for slave thread to start");
...
int start_slave_threads(bool need_slave_mutex, bool wait_for_start,
  ...
  pthread_mutex_t *lock_io = 0, *lock_sql = 0, *lock_cond_io = 0,
                  *lock_cond_sql = 0;
  ...
  if (thread_mask & SLAVE_IO)
    error = start_slave_thread(handle_slave_io, lock_io, lock_cond_io, cond_io,
                               &mi->slave_running, &mi->slave_run_id, mi,
                               1); // high priority, to read the most possible
  if (!error && (thread_mask & SLAVE_SQL)) {
    error = start_slave_thread(handle_slave_sql, lock_sql, lock_cond_sql,
                               cond_sql, &mi->rli.slave_running,
...
void init_table_rule_hash(HASH *h, bool *h_inited) {
  hash_init(h, system_charset_info, TABLE_RULE_HASH_SIZE, 0, 0,
            (hash_get_key)get_table_key, (hash_free_key)free_table_ent, 0);
  *h_inited = 1;
}
...
static TABLE_RULE_ENT *find_wild(DYNAMIC_ARRAY *a, const char *key, int len) {
  ...
  for (i = 0; i < a->elements; i++) {
    ...
    if (!my_wildcmp(system_charset_info, key, key_end, (const char *)e->db,
  ...
  for (; tables; tables = tables->next_global) {
    ...
    *end++ = '.';
    ...
    if (wild_do_table_inited &&
        find_wild(&replicate_wild_do_table, hash_key, len))
      DBUG_RETURN(1);
    if (wild_ignore_table_inited &&
        find_wild(&replicate_wild_ignore_table, hash_key, len))
      DBUG_RETURN(0);
  }
...
int add_table_rule(HASH *h, const char *table_spec) {
  ...
  TABLE_RULE_ENT *e =
      (TABLE_RULE_ENT *)my_malloc(sizeof(TABLE_RULE_ENT) + len, MYF(MY_WME));
  if (!e)
    return 1;
    ...
    */
    ...
    case '3':
      mi->rli.relay_log.description_event_for_queue =
          new Format_description_log_event(1, mysql->server_version);
      break;
    case '4':
      mi->rli.relay_log.description_event_for_queue =
          new Format_description_log_event(3, mysql->server_version);
      break;
    default:
      /*
      ...
      */
      mi->rli.relay_log.description_event_for_queue =
          new Format_description_log_event(4, mysql->server_version);
      break;
    }
  }
...
static int create_table_from_dump(THD *thd, MYSQL *mysql, const char *db,
  ...
  {
    char *err_msg;
    err_msg = (char *)net->read_pos +
              ((mysql->server_capabilities & CLIENT_PROTOCOL_41)
                   ? 3 + SQLSTATE_LENGTH + 1
                   : 3);
  ...
  */
  {
    ...
    */
    if (rli->relay_log.open_index_file(opt_relaylog_index_name, ln) ||
        rli->relay_log.open(
            ln, LOG_BIN, 0, SEQ_READ_APPEND, 0,
  ...
  if (access(fname, F_OK)) {
    ...
    */
    ...
    if ((info_fd = my_open(fname, O_CREAT | O_RDWR | O_BINARY, MYF(MY_WME))) <
        0) {
...
file '%s', errno %d)",
    ...
    if (init_relay_log_pos(rli, NullS, BIN_LOG_HEADER_SIZE,
                           0 /* no data lock */, &msg, 0)) {
...
Failed to open the existing relay log info file '%s' (errno %d)",
                        fname, my_errno);
        error = 1;
      } else if (init_io_cache(&rli->info_file, info_fd, IO_SIZE * 2,
    ...
    if (init_strvar_from_file(rli->group_relay_log_name,
                              ...
                              &rli->info_file, "") ||
        init_intvar_from_file(&relay_log_pos, &rli->info_file,
                              BIN_LOG_HEADER_SIZE) ||
    ...
    if (init_relay_log_pos(rli, rli->group_relay_log_name,
                           rli->group_relay_log_pos, 0 /* no data lock*/, &msg,
                           0)) {
      char llbuf[22];
...
static void write_ignored_events_info_to_relay_log(THD *thd, MASTER_INFO *mi) {
  ...
  if (rli->ign_master_log_name_end[0]) {
    ...
    Rotate_log_event *ev = new Rotate_log_event(
        thd, rli->ign_master_log_name_end, 0, rli->ign_master_log_pos_end,
...
file '%s')",
    ...
    */
    if (init_strvar_from_file(mi->master_log_name, sizeof(mi->master_log_name),
                              &mi->file, ""))
      goto errwithmsg;

    lines = strtoul(mi->master_log_name, &first_non_digit, 10);

    if (mi->master_log_name[0] != '\0' && *first_non_digit == '\0' &&
        lines >= LINES_IN_MASTER_INFO_WITH_SSL) { // Seems to be new format
    ...
    if (lines >= LINES_IN_MASTER_INFO_WITH_SSL &&
        (init_intvar_from_file(&ssl, &mi->file, master_ssl) ||
         init_strvar_from_file(mi->ssl_ca, sizeof(mi->ssl_ca), &mi->file,
                               master_ssl_ca) ||
         init_strvar_from_file(mi->ssl_capath, sizeof(mi->ssl_capath),
                               &mi->file, master_ssl_capath) ||
         init_strvar_from_file(mi->ssl_cert, sizeof(mi->ssl_cert), &mi->file,
                               master_ssl_cert) ||
         init_strvar_from_file(mi->ssl_cipher, sizeof(mi->ssl_cipher),
                               &mi->file, master_ssl_cipher) ||
         init_strvar_from_file(mi->ssl_key, sizeof(mi->ssl_key), &mi->file,
                               master_ssl_key)))
      goto errwithmsg;
#ifndef HAVE_OPENSSL
    if (ssl)
...
int register_slave_on_master(MYSQL *mysql) {
  ...
  if (simple_command(mysql, COM_REGISTER_SLAVE, (char *)buf, (uint)(pos - buf),
                     0)) {
    sql_print_error("Error on COM_REGISTER_SLAVE: %d '%s'", mysql_errno(mysql),
                    mysql_error(mysql));
    return 1;
  }
  return 0;
...
bool show_master_info(THD *thd, MASTER_INFO *mi) {
  ...
  field_list.push_back(
      new Item_empty_string("Master_SSL_CA_File", sizeof(mi->ssl_ca)));
  field_list.push_back(
      new Item_empty_string("Master_SSL_CA_Path", sizeof(mi->ssl_capath)));
  field_list.push_back(
      new Item_empty_string("Master_SSL_Cert", sizeof(mi->ssl_cert)));
  field_list.push_back(
      new Item_empty_string("Master_SSL_Cipher", sizeof(mi->ssl_cipher)));
  field_list.push_back(
      new Item_empty_string("Master_SSL_Key", sizeof(mi->ssl_key)));
  field_list.push_back(
  ...
  if (mi->host[0]) {
    ...
    */
    ...
    protocol->store(mi->slave_running == MYSQL_SLAVE_RUN_CONNECT ? "Yes" : "No",
                    &my_charset_bin);
    ...
    */
    if ((mi->slave_running == MYSQL_SLAVE_RUN_CONNECT) &&
        mi->rli.slave_running) {
      long tmp =
          (long)((time_t)time((time_t *)0) - mi->rli.last_master_timestamp) -
          mi->clock_diff_with_master;
      /*
      ...
      */
      protocol->store(
          (longlong)(mi->rli.last_master_timestamp ? max(0, tmp) : 0));
    } else
      protocol->store_null();
...
bool flush_master_info(MASTER_INFO *mi, bool flush_relay_log_cache) {
  ...
  */
  ...
  my_b_seek(file, 0L);
  my_b_printf(file, "%u\n%s\n%s\n%s\n%s\n%s\n%d\n%d\n%d\n%s\n%s\n%s\n%s\n%s\n",
              LINES_IN_MASTER_INFO_WITH_SSL, mi->master_log_name,
              llstr(mi->master_log_pos, lbuf), mi->host, mi->user, mi->password,
              mi->port, mi->connect_retry, (int)(mi->ssl), mi->ssl_ca,
              mi->ssl_capath, mi->ssl_cert, mi->ssl_cipher, mi->ssl_key);
  flush_io_cache(file);
  DBUG_RETURN(0);
}
...
st_relay_log_info::st_relay_log_info()
    : info_fd(-1), cur_log_fd(-1), save_temporary_tables(0),
      ...
      slave_skip_counter(0), abort_pos_wait(0), slave_run_id(0), sql_thd(0),
      last_slave_errno(0), inited(0), abort_slave(0), slave_running(0),
      until_condition(UNTIL_NONE), until_log_pos(0), retried_trans(0) {
...
int st_relay_log_info::wait_for_pos(THD *thd, String *log_name,
                                    longlong log_pos, longlong timeout) {
  if (!inited)
    return -1;
  int event_count = 0;
  ...
  */
  ...
  strmake(log_name_tmp, log_name->ptr(),
          min(log_name->length(), FN_REFLEN - 1));
...
improper_arguments: %d  timed_out: %d",
              thd->killed_errno(), (int)(init_abort_pos_wait != abort_pos_wait),
...
void set_slave_thread_options(THD *thd) {
  thd->options =
      ((opt_log_slave_updates) ? OPTION_BIN_LOG : 0) | OPTION_AUTO_IS_NULL;
...
static int init_slave_thread(THD *thd, SLAVE_THD_TYPE thd_type) {
  ...
  thd->system_thread = (thd_type == SLAVE_THD_SQL) ? SYSTEM_THREAD_SLAVE_SQL
...
bool st_relay_log_info::is_until_satisfied() {
  ...
  } else { /* until_condition == UNTIL_RELAY_POS */
  ...
  if (ev) {
    ...
    if ((ev->server_id == (uint32)::server_id && !replicate_same_server_id &&
      ...
      */
      if (rli->slave_skip_counter &&
          !((type_code == INTVAR_EVENT || type_code == RAND_EVENT ||
    ...
    */
    ...
    if (slave_trans_retries) {
      if (exec_res && (thd->net.last_errno == ER_LOCK_DEADLOCK ||
        ...
        */
        if (rli->trans_retries < slave_trans_retries) {
          if (init_master_info(rli->mi, 0, 0, 0, SLAVE_SQL))
            sql_print_error("Failed to initialize the master info structure");
          else if (init_relay_log_pos(rli, rli->group_relay_log_name,
                                      rli->group_relay_log_pos, 1, &errmsg, 1))
...
slave_begin:
  ...
  DBUG_PRINT("master_info",
             ("log_file_name: '%s'  position: %s", mi->master_log_name,
  ...
  replication started in log '%s' at position %s",
                          mi->user, mi->host, mi->port, IO_RPL_LOG_NAME,
                          llstr(mi->master_log_pos, llbuff));
  else {
...
slave_begin:
  ...
  */
  ...
  if (init_relay_log_pos(rli, rli->group_relay_log_name,
                         rli->group_relay_log_pos, 1 /*need data lock*/,
                         &errmsg, 1 /*look for a description_event*/)) {
...
*/
  ...
  DBUG_PRINT("master_info",
             ("log_file_name: %s  position: %s", rli->group_master_log_name,
...
static int process_io_rotate(MASTER_INFO *mi, Rotate_log_event *rev) {
  ...
  DBUG_PRINT("info", ("master_log_pos: '%s' %d", mi->master_log_name,
 ...
 master could be corrupt but a more likely cause of this is a bug",
  ...
  if (likely(!ignore_event)) {
    if (ev->log_pos)
      ...
      */
      ev->log_pos +=
          event_len; /* make log_pos be the pos of the end of the event */
    if (unlikely(rli->relay_log.append(ev))) {
      delete ev;
      pthread_mutex_unlock(&mi->data_lock);
      DBUG_RETURN(1);
...
int queue_event(MASTER_INFO *mi, const char *buf, ulong event_len) {
  ...
  case STOP_EVENT:
    ...
    */
    goto err;
  case ROTATE_EVENT: {
    Rotate_log_event rev(buf, event_len,
                         mi->rli.relay_log.description_event_for_queue);
...
Error: '%s'  errno: %d  retry-time: %d  retries: %d",
                      (reconnect ? "reconnecting" : "connecting"), mi->user,
                      mi->host, mi->port, mysql_error(mysql), last_errno,
...
replication resumed in log '%s' at position %s",
      ...
      mysql_log.write(thd, COM_CONNECT_OUT, "%s@%s:%d", mi->user, mi->host,
                      mi->port);
    }
#ifdef SIGNAL_WITH_VIO_CLOSE
...
static IO_CACHE *reopen_relay_log(RELAY_LOG_INFO *rli, const char **errmsg) {
  ...
  if ((rli->cur_log_fd =
           open_binlog(cur_log, rli->event_relay_log_name, errmsg)) < 0)
    DBUG_RETURN(0);
  /*
...
Log_event *next_event(RELAY_LOG_INFO *rli) {
  ...
  */
  ...
  while (!sql_slave_killed(thd, rli)) {
    ...
    */
    if ((ev = Log_event::read_log_event(
             cur_log, 0, rli->relay_log.description_event_for_exec))) {
      ...
      if (hot_log) {
        ...
        */
        ...
        if (rli->ign_master_log_name_end[0]) {
          ...
          ev = new Rotate_log_event(thd, rli->ign_master_log_name_end, 0,
      ...
      if (relay_log_purge) {
        ...
        */
        if (rli->relay_log.purge_first_log(
                rli, rli->group_relay_log_pos == rli->event_relay_log_pos &&
...
*/
      ...
      if ((rli->cur_log_fd =
               open_binlog(cur_log, rli->linfo.log_file_name, &errmsg)) < 0)
        goto err;
    } else {
      /*
