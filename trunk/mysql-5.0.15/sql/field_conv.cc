...
static void do_cut_string(Copy_field *copy) {
  ...
  if (cs->cset->scan(cs, copy->from_ptr + copy->to_length,
      ...
      */
      ...
      if (to->real_type() == FIELD_TYPE_ENUM ||
        ...
        if (to_length != from_length)
          return (((Field_varstring *)to)->length_bytes == 1 ? do_varstring1
                                                             : do_varstring2);
        ...
        return do_expand_string;
               ...
               to_length != from_length || !compatible_db_low_byte_first) {
...
void field_conv(Field *to, Field *from) {
  if (to->real_type() == from->real_type()) {
    if (to->pack_length() == from->pack_length() &&
        ...
        (from->real_type() != MYSQL_TYPE_VARCHAR ||
         ((Field_varstring *)from)->length_bytes ==
             ((Field_varstring *)to)->length_bytes)) { // Identical fields
#ifdef HAVE_purify
  ...
  if (to->type() == FIELD_TYPE_BLOB) { // Be sure the value is stored
    Field_blob *blob = (Field_blob *)to;
    from->val_str(&blob->value);
    if (!blob->value.is_alloced() && from->real_type() != MYSQL_TYPE_STRING &&
        from->real_type() != MYSQL_TYPE_VARCHAR)
      blob->value.copy();
