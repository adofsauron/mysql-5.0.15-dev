...
#else
#define my_safe_alloca(size, min_length)                                       \
  ((size <= min_length) ? my_alloca(size) : my_malloc(size, MYF(0)))
...
static int check_insert_fields(THD *thd, TABLE_LIST *table_list,
  ...
  if (fields.elements == 0 && values.elements != 0) {
    ...
    if (grant_option) {
      ...
      if (check_grant_all_columns(thd, INSERT_ACL, &table->grant, table->s->db,
                                  table->s->table_name, &fields))
        return -1;
    }
#endif
    ...
    */
    ...
    if (table_list->effective_algorithm == VIEW_ALGORITHM_MERGE) {
      ...
      if (table_list->check_single_table(&tbl, map, table_list) || tbl == 0) {
        my_error(ER_VIEW_MULTIUPDATE, MYF(0), table_list->view_db.str,
                 table_list->view_name.str);
        return -1;
      }
...
// For the values we need select_priv
  ...
  if (check_key_in_view(thd, table_list) ||
      (table_list->view && check_view_insertability(thd, table_list))) {
    my_error(ER_NON_UPDATABLE_TABLE, MYF(0), table_list->alias, "INSERT");
    return -1;
  }
...
bool mysql_insert(THD *thd, TABLE_LIST *table_list, List<Item> &fields,
                  List<List_item> &values_list, List<Item> &update_fields,
 ...
 */
  ...
  if (lock_type == TL_WRITE_DELAYED) {
    if (thd->locked_tables) {
      if (find_locked_table(thd, table_list->db ? table_list->db : thd->db,
  ...
  */
  thd->count_cuted_fields =
      ((values_list.elements == 1 && !ignore) ? CHECK_FIELD_ERROR_FOR_NULL
                                              : CHECK_FIELD_WARN);
  thd->cuted_fields = 0L;
  ...
  while ((values = its++)) {
    if (fields.elements || !value_count) {
      ...
      if (fill_record_n_invoke_before_triggers(
              thd, fields, *values, 0, table->triggers, TRG_EVENT_INSERT)) {
    ...
    if ((res = table_list->view_check_option(
             thd, (values_list.elements == 1 ? 0 : ignore))) == VIEW_CHECK_SKIP)
      continue;
...
#ifndef EMBEDDED_LIBRARY
    if (lock_type == TL_WRITE_DELAYED) {
      error = write_delayed(thd, table, duplic, ignore, query,
                            thd->query_length, log_on);
      query = 0;
    } else
#endif
    ...
    */
    if (!id && thd->insert_id_used) { // Get auto increment value
...
*/
  ...
  if (values_list.elements == 1 &&
      (!(thd->options & OPTION_WARNINGS) || !thd->cuted_fields)) {
...
static bool mysql_prepare_insert_check_table(THD *thd, TABLE_LIST *table_list,
  ...
  if (setup_tables(thd, &thd->lex->select_lex.context,
                   &thd->lex->select_lex.top_join_list, table_list, where,
...
bool mysql_prepare_insert(THD *thd, TABLE_LIST *table_list, TABLE *table,
  ...
  DBUG_PRINT("enter", ("table_list 0x%lx, table 0x%lx, view %d",
                       (ulong)table_list, (ulong)table, (int)insert_into_view));
  ...
  */
  ...
  if (values && !(res = check_insert_fields(thd, context->table_list, fields,
                                            *values, !insert_into_view) ||
    ...
    while ((error = table->file->write_row(table->record[0]))) {
      ...
      */
      ...
      if (table->file->table_flags() & HA_DUPP_POS) {
        ...
        if ((error = (table->file->index_read_idx(
                 table->record[1], key_nr, (byte *)key,
      ...
      if (info->handle_duplicates == DUP_UPDATE) {
        ...
        */
        ...
        if (fill_record_n_invoke_before_triggers(
                thd, *info->update_fields, *info->update_values, 0,
                table->triggers, TRG_EVENT_UPDATE))
          goto before_trg_err;

        /* CHECK OPTION for VIEW ... ON DUPLICATE KEY UPDATE ... */
        if (info->view && (res = info->view->view_check_option(
                               current_thd, info->ignore)) == VIEW_CHECK_SKIP)
    ...
    if ((*field)->query_id != thd->query_id &&
      ...
      if (view) {
        ...
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_WARN,
                            ...
                            ER(ER_NO_DEFAULT_FOR_FIELD), (*field)->field_name);
      }
      err = 1;
    }
  }
...
public:
  ...
  ~delayed_insert() {
    ...
    VOID(pthread_cond_broadcast(
        &COND_thread_count)); /* Tell main we are ready */
  }
    ...
    if (!(tmp = find_handler(thd, table_list))) {
      ...
      */
      ...
      if ((error = pthread_create(&tmp->thd.real_id, &connection_attrib,
        ...
        DBUG_PRINT(
            "error",
            ("Can't create thread to handle delayed insert (error %d)", error));
...
static void end_delayed_insert(THD *thd) {
  ...
  if (!--di->tables_in_use || di->thd.killed) { // Unlock table
    di->status = 1;
    pthread_cond_signal(&di->cond);
  }
  pthread_mutex_unlock(&di->mutex);
    ...
    if (!di->status && !di->stacked_inserts) {
      ...
      while (!thd->killed) {
        ...
        if (error && error != EINTR && error != ETIMEDOUT) {
          ...
          DBUG_PRINT("error",
                     ("Got error %d from pthread_cond_timedwait", error));
        }
#endif
#endif
...
bool delayed_insert::handle_inserts(void) {
  ...
  */
  ...
  while ((row = rows.get())) {
    ...
    */
    if (group_count++ >= max_rows && (row = rows.head()) &&
        (!(row->log_query & using_bin_log) || row->query)) {
      group_count = 0;
  ...
  pthread_mutex_unlock(&mutex);
  if ((error =
           table->file->extra(HA_EXTRA_NO_CACHE))) { // This shouldn't happen
    table->file->print_error(error, MYF(0));
    sql_print_error("%s", thd.net.last_error);
    goto err;
  }
...
bool mysql_insert_select_prepare(THD *thd) {
  ...
  */
  ...
  if (mysql_prepare_insert(thd, lex->query_tables, lex->query_tables->table,
                           lex->field_list, 0, lex->update_list,
                           lex->value_list, lex->duplicates, &select_lex->where,
                           TRUE))
    DBUG_RETURN(TRUE);
...
int select_insert::prepare(List<Item> &values, SELECT_LEX_UNIT *u) {
  ...
  */
  ...
  if (info.handle_duplicates == DUP_UPDATE) {
    ...
    res = res ||
          check_update_fields(thd, context->table_list, *info.update_fields);
  ...
  */
  ...
  thd->no_trans_update = 0;
  thd->abort_on_warning =
      (!info.ignore && (thd->variables.sql_mode &
                        (MODE_STRICT_TRANS_TABLES | MODE_STRICT_ALL_TABLES)));
  res = ((fields->elements &&
          check_that_all_fields_are_given_values(thd, table, table_list)) ||
...
int select_create::prepare(List<Item> &values, SELECT_LEX_UNIT *u) {
  ...
  unit = u;
  table = create_table_from_items(thd, create_info, create_table, extra_fields,
                                  keys, &values, &lock);
  if (!table)
...
void select_create::store_values(List<Item> &values) {
  fill_record_n_invoke_before_triggers(thd, field, values, 1, table->triggers,
                                       TRG_EVENT_INSERT);
}
