...
#define AES_USE_KEY_BYTES (AES_USE_KEY_BITS / 8)
  ...
  */
  for (ptr = rkey + AES_USE_KEY_BYTES, sptr = rkey; ptr < rkey_end; ptr++, sptr++)
  {
...
int my_aes_encrypt(const char *source, int source_length, char *dest, const char *key, int key_length)
{
  ...
  {
    rijndaelEncrypt(aes_key.rk, aes_key.nr, (const uint8 *)source, (uint8 *)dest);
...
int my_aes_decrypt(const char *source, int source_length, char *dest, const char *key, int key_length)
{
  ...
  {
    rijndaelDecrypt(aes_key.rk, aes_key.nr, (const uint8 *)source, (uint8 *)dest);
    source += AES_BLOCK_SIZE;
    dest += AES_BLOCK_SIZE;
  }
...
int my_aes_get_size(int source_length) { return AES_BLOCK_SIZE * (source_length / AES_BLOCK_SIZE) + AES_BLOCK_SIZE; }
