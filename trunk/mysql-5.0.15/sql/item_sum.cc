...
Item_sum::Item_sum(THD *thd, Item_sum *item)
    : Item_result_field(thd, item), arg_count(item->arg_count), quick_group(item->quick_group)
{
  if (arg_count <= 2)
    args = tmp_args;
...
void Item_sum::make_field(Send_field *tmp_field)
{
  if (args[0]->type() == Item::FIELD_ITEM && keep_field_type())
  {
    ...
    if (maybe_null) tmp_field->flags &= ~NOT_NULL_FLAG;
  }
  else
...
Field *Item_sum::create_tmp_field(bool group, TABLE *table, uint convert_blob_length)
{
  switch (result_type())
  {
    case REAL_RESULT:
    ...
    case STRING_RESULT:
      if (max_length > 255 && convert_blob_length)
        return new Field_varstring(convert_blob_length, maybe_null, name, table, collation.collation);
      return make_string_field(table);
    case DECIMAL_RESULT:
      return new Field_new_decimal(max_length, maybe_null, name, table, decimals, unsigned_flag);
    case ROW_RESULT:
    default:
...
bool Item_sum_num::fix_fields(THD *thd, Item **ref)
{
  ...
  {
    my_message(ER_INVALID_GROUP_FUNC_USE, ER(ER_INVALID_GROUP_FUNC_USE), MYF(0));
    return TRUE;
  }
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    if (args[i]->fix_fields(thd, args + i) || args[i]->check_cols(1)) return TRUE;
...
bool Item_sum_hybrid::fix_fields(THD *thd, Item **ref)
{
  ...
  if (!item->fixed && item->fix_fields(thd, args) || (item = args[0])->check_cols(1)) return TRUE;
...
Field *Item_sum_hybrid::create_tmp_field(bool group, TABLE *table, uint convert_blob_length)
{
  if (args[0]->type() == Item::FIELD_ITEM)
  {
    Field *field = ((Item_field *)args[0])->field;

    if ((field = create_tmp_field_from_field(current_thd, field, name, table, NULL, convert_blob_length)))
...
Item_sum_sum::Item_sum_sum(THD *thd, Item_sum_sum *item)
    : Item_sum_num(thd, item), hybrid_type(item->hybrid_type), curr_dec_buff(item->curr_dec_buff)
{
...
bool Item_sum_sum::add()
{
  ...
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    {
      my_decimal_add(E_DEC_FATAL_ERROR, dec_buffs + (curr_dec_buff ^ 1), val, dec_buffs + curr_dec_buff);
      curr_dec_buff ^= 1;
      null_value = 0;
    }
  }
...
longlong Item_sum_sum::val_int()
{
  ...
  {
    longlong result;
    my_decimal2int(E_DEC_FATAL_ERROR, dec_buffs + curr_dec_buff, unsigned_flag, &result);
    return result;
  }
...
{
  DBUG_ASSERT(fixed == 1);
  if (hybrid_type == DECIMAL_RESULT) my_decimal2double(E_DEC_FATAL_ERROR, dec_buffs + curr_dec_buff, &sum);
  return sum;
}
...
Item_sum_distinct::Item_sum_distinct(Item *item_arg) : Item_sum_num(item_arg), tree(0)
{
  /*
...
Item_sum_distinct::Item_sum_distinct(THD *thd, Item_sum_distinct *original)
    : Item_sum_num(thd, original), val(original->val), tree(0), table_field_type(original->table_field_type)
{
  quick_group = 0;
}
...
/*
  ...
  values.
*/
struct Hybrid_type_traits_fast_decimal : public Hybrid_type_traits_integer
{
...
const Hybrid_type_traits_fast_decimal *Hybrid_type_traits_fast_decimal::instance()
{
...
void Item_sum_distinct::fix_length_and_dec()
{
  ...
  switch (args[0]->result_type())
  {
    ...
    case REAL_RESULT:
      ...
      if (table_field_type != MYSQL_TYPE_FLOAT) table_field_type = MYSQL_TYPE_DOUBLE;
      break;
    case INT_RESULT:
      /*
    ...
    case DECIMAL_RESULT:
      val.traits = Hybrid_type_traits_decimal::instance();
      if (table_field_type != MYSQL_TYPE_LONGLONG) table_field_type = MYSQL_TYPE_NEWDECIMAL;
      break;
    case ROW_RESULT:
    default:
      DBUG_ASSERT(0);
...
bool Item_sum_distinct::setup(THD *thd)
{
  ...
  */
  ...
  if (!(table = create_virtual_tmp_table(thd, field_list))) return TRUE;
  ...
  */
  tree = new Unique(simple_raw_key_cmp, &tree_key_length, tree_key_length, thd->variables.max_heap_table_size);
...
void Item_sum_avg_distinct::fix_length_and_dec()
{
...
void Item_sum_avg_distinct::calculate_val_and_count()
{
  Item_sum_distinct::calculate_val_and_count();
  if (count) val.traits->div(&val, count);
}
...
Field *Item_sum_avg::create_tmp_field(bool group, TABLE *table, uint convert_blob_len)
{
  if (group)
  {
    /*
    ...
    */
    return new Field_string(((hybrid_type == DECIMAL_RESULT) ? dec_bin_size : sizeof(double)) + sizeof(longlong), 0,
  ...
  switch (args[0]->result_type())
  {
    ...
    case DECIMAL_RESULT:
    {
      ...
      */
      ...
      f_precision0 = min(args[0]->decimal_precision() + DECIMAL_LONGLONG_DIGITS, DECIMAL_MAX_PRECISION);
      f_scale1 = min(args[0]->decimals * 2, DECIMAL_MAX_SCALE);
      f_precision1 = min(args[0]->decimal_precision() * 2 + DECIMAL_LONGLONG_DIGITS, DECIMAL_MAX_PRECISION);
...
Field *Item_sum_variance::create_tmp_field(bool group, TABLE *table, uint convert_blob_len)
{
  if (group)
  {
    /*
    ...
    */
    return new Field_string(
        ((hybrid_type == DECIMAL_RESULT) ? dec_bin_size0 + dec_bin_size1 : sizeof(double) * 2) + sizeof(longlong), 0,
        name, table, &my_charset_bin);
  }
  if (hybrid_type == DECIMAL_RESULT)
    return new Field_new_decimal(max_length, maybe_null, name, table, decimals, unsigned_flag);
...
bool Item_sum_variance::add()
{
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    if (!args[0]->null_value)
    {
      ...
      my_decimal_add(E_DEC_FATAL_ERROR, dec_sqr + next_dec, dec_sqr + cur_dec, &sqr_buf);
      my_decimal_add(E_DEC_FATAL_ERROR, dec_sum + next_dec, dec_sum + cur_dec, dec);
      cur_dec = next_dec;
    }
  }
  else
...
my_decimal *Item_sum_variance::val_decimal(my_decimal *dec_buf)
{
  ...
  my_decimal_mul(E_DEC_FATAL_ERROR, &sum_sqr_buf, dec_sum + cur_dec, dec_sum + cur_dec);
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &sum_sqr_buf, &count_buf, prec_increment);
  my_decimal_sub(E_DEC_FATAL_ERROR, &sum_sqr_buf, dec_sqr + cur_dec, dec_buf);
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &sum_sqr_buf, &count1_buf, prec_increment);
  return dec_buf;
}
...
void Item_sum_variance::reset_field()
{
  ...
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    my_decimal2binary(E_DEC_FATAL_ERROR, arg_dec, res, f_precision0, f_scale0);
    my_decimal2binary(E_DEC_FATAL_ERROR, arg2_dec, res + dec_bin_size0, f_precision1, f_scale1);
...
void Item_sum_variance::update_field()
{
  ...
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    if (!args[0]->null_value)
    {
      binary2my_decimal(E_DEC_FATAL_ERROR, res, dec_sum + 1, f_precision0, f_scale0);
      binary2my_decimal(E_DEC_FATAL_ERROR, res + dec_bin_size0, dec_sqr + 1, f_precision1, f_scale1);
      ...
      field_count++;
      my_decimal2binary(E_DEC_FATAL_ERROR, dec_sum, res, f_precision0, f_scale0);
      my_decimal2binary(E_DEC_FATAL_ERROR, dec_sqr, res + dec_bin_size0, f_precision1, f_scale1);
      res += dec_bin_size0 + dec_bin_size1;
...
Item *Item_sum_min::copy_or_same(THD *thd) { return new (thd->mem_root) Item_sum_min(thd, this); }
...
bool Item_sum_min::add()
{
  switch (hybrid_type)
  {
    case STRING_RESULT:
    {
      ...
      if (!args[0]->null_value && (null_value || sortcmp(&value, result, collation.collation) > 0))
      {
        value.copy(*result);
        null_value = 0;
      }
    ...
    case INT_RESULT:
    {
      ...
      if (!args[0]->null_value &&
          (null_value || (unsigned_flag && (ulonglong)nr < (ulonglong)sum_int) || (!unsigned_flag && nr < sum_int)))
      {
        sum_int = nr;
        null_value = 0;
      }
    ...
    {
      my_decimal value, *val = args[0]->val_decimal(&value);
      if (!args[0]->null_value && (null_value || (my_decimal_cmp(&sum_dec, val) > 0)))
      {
...
Item *Item_sum_max::copy_or_same(THD *thd) { return new (thd->mem_root) Item_sum_max(thd, this); }
...
bool Item_sum_max::add()
{
  switch (hybrid_type)
  {
    ...
    {
      my_decimal value, *val = args[0]->val_decimal(&value);
      if (!args[0]->null_value && (null_value || (my_decimal_cmp(val, &sum_dec) > 0)))
      {
        my_decimal2decimal(val, &sum_dec);
        null_value = 0;
      }
...
bool Item_sum_or::add()
{
  ...
  if (!args[0]->null_value) bits |= value;
  return 0;
}
...
void Item_sum_sum::update_field()
{
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    if (!args[0]->null_value)
    {
      ...
      {
        my_decimal field_value, *field_val = result_field->val_decimal(&field_value);
...
void Item_sum_avg::update_field()
{
  ...
  if (hybrid_type == DECIMAL_RESULT)
  {
    ...
    if (!args[0]->null_value)
    {
      binary2my_decimal(E_DEC_FATAL_ERROR, res, dec_buffs + 1, f_precision, f_scale);
...
void Item_sum_hybrid::min_max_update_str_field()
{
  ...
  if (!args[0]->null_value)
  {
    ...
    if (result_field->is_null() || (cmp_sign * sortcmp(res_str, &tmp_value, collation.collation)) < 0)
...
void Item_sum_hybrid::min_max_update_real_field()
{
  ...
  if (!args[0]->null_value)
  {
    if (result_field->is_null(0) || (cmp_sign > 0 ? old_nr > nr : old_nr < nr)) old_nr = nr;
    result_field->set_notnull();
  }
...
void Item_sum_hybrid::min_max_update_int_field()
{
  ...
  if (!args[0]->null_value)
  {
    ...
    {
      bool res = (unsigned_flag ? (ulonglong)old_nr > (ulonglong)nr : old_nr > nr);
...
longlong Item_avg_field::val_int() { return (longlong)val_real(); }
...
my_decimal *Item_avg_field::val_decimal(my_decimal *dec_buf)
{
  ...
  binary2my_decimal(E_DEC_FATAL_ERROR, field->ptr, &dec_field, f_precision, f_scale);
  int2my_decimal(E_DEC_FATAL_ERROR, count, 0, &dec_count);
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &dec_field, &dec_count, prec_increment);
  return dec_buf;
}
...
my_decimal *Item_variance_field::val_decimal(my_decimal *dec_buf)
{
  ...
  if ((null_value = (count <= sample))) return 0;

  my_decimal dec_count, dec1_count, dec_sum, dec_sqr, tmp;
  int2my_decimal(E_DEC_FATAL_ERROR, count, 0, &dec_count);
  int2my_decimal(E_DEC_FATAL_ERROR, count - sample, 0, &dec1_count);
  binary2my_decimal(E_DEC_FATAL_ERROR, field->ptr, &dec_sum, f_precision0, f_scale0);
  binary2my_decimal(E_DEC_FATAL_ERROR, field->ptr + dec_bin_size0, &dec_sqr, f_precision1, f_scale1);
  ...
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &dec_sum, &dec1_count, prec_increment);
  return dec_buf;
}
...
Item_sum_count_distinct::~Item_sum_count_distinct() { cleanup(); }
...
bool Item_sum_count_distinct::setup(THD *thd)
{
  ...
  */
  ...
  if (!(tmp_table_param = new TMP_TABLE_PARAM)) return TRUE;
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    ...
    {
      (void)item->val_int();
      if (item->null_value) always_null = 1;
    }
  }
  if (always_null) return FALSE;
  count_field_types(tmp_table_param, list, 0);
  DBUG_ASSERT(table == 0);
  if (!(table = create_tmp_table(thd, tmp_table_param, list, (ORDER *)0, 1, 0, (select_lex->options | thd->options),
                                 HA_POS_ERROR, (char *)"")))
    return TRUE;
  ...
  if (table->s->db_type == DB_TYPE_HEAP)
  {
    ...
    */
    ...
    for (tree_key_length = 0; field < field_end; ++field)
    {
      ...
      if (!f->binary() && (type == MYSQL_TYPE_STRING || type == MYSQL_TYPE_VAR_STRING || type == MYSQL_TYPE_VARCHAR))
      {
        all_binary = FALSE;
        break;
      }
    ...
    else
    {
      ...
      else
      {
        ...
        for (tree_key_length = 0, length = field_lengths, field = table->field; field < field_end; ++field, ++length)
        {
    ...
    DBUG_ASSERT(tree == 0);
    tree = new Unique(compare_key, cmp_arg, tree_key_length, thd->variables.max_heap_table_size);
    /*
...
Item *Item_sum_count_distinct::copy_or_same(THD *thd) { return new (thd->mem_root) Item_sum_count_distinct(thd, this); }
...
bool Item_sum_count_distinct::add()
{
  ...
  for (Field **field = table->field; *field; field++)
    if ((*field)->is_real_null(0)) return 0;  // Don't count NULL
  ...
  if ((error = table->file->write_row(table->record[0])) && error != HA_ERR_FOUND_DUPP_KEY &&
...
longlong Item_sum_count_distinct::val_int()
{
  ...
  if (tree)
  {
    ...
    if (tree->elements == 0) return (longlong)tree->elements_in_tree();  // everything fits in memory
    count = 0;
...
int group_concat_key_cmp_with_distinct(void *arg, byte *key1, byte *key2)
{
  ...
  for (field_item = grp_item->args, end = field_item + grp_item->arg_count_field; field_item < end; field_item++)
  {
    /*
    ...
    */
    if (field && !(*field_item)->const_item())
    {
      ...
      if ((res = field->cmp((char *)key1 + offset, (char *)key2 + offset))) return res;
    }
  }
  return 0;
}
...
int group_concat_key_cmp_with_order(void *arg, byte *key1, byte *key2)
{
  ...
  for (order_item = grp_item->order, end = order_item + grp_item->arg_count_order; order_item < end; order_item++)
  {
    ...
    */
    if (field && !item->const_item())
    {
      int res;
      uint offset = field->offset() - table->s->null_bytes;
      if ((res = field->cmp((char *)key1 + offset, (char *)key2 + offset))) return (*order_item)->asc ? res : -res;
    }
  }
  /*
...
{
  if (!group_concat_key_cmp_with_distinct(arg, key1, key2)) return 0;
...
int dump_leaf_key(byte *key, element_count count __attribute__((unused)), Item_func_group_concat *item)
{
  ...
  String tmp((char *)table->record[1], table->s->reclength, default_charset_info);
  String tmp2;
...
Item_func_group_concat::Item_func_group_concat(Name_resolution_context *context_arg, bool distinct_arg,
                                               List<Item> *select_list, SQL_LIST *order_list, String *separator_arg)
    : tmp_table_param(0),
      warning(0),
      ...
      original(0)
{
  ...
  */
  if (!(args = (Item **)sql_alloc(sizeof(Item *) * arg_count + sizeof(ORDER *) * arg_count_order))) return;
  ...
  if (arg_count_order)
  {
    ...
    for (ORDER *order_item = (ORDER *)order_list->first; order_item != NULL; order_item = order_item->next)
    {
...
Item_func_group_concat::Item_func_group_concat(THD *thd, Item_func_group_concat *item)
    : Item_sum(thd, item),
...
bool Item_func_group_concat::add()
{
  ...
  for (uint i = 0; i < arg_count_field; i++)
  {
    ...
    if (!show_item->const_item())
    {
      ...
      if (f->is_null_in_record((const uchar *)table->record[0])) return 0;  // Skip row if it contains null
    }
  }
  ...
  if (tree) el = tree_insert(tree, table->record[0] + table->s->null_bytes, 0, tree->custom_arg);
  /*
    If the row is not a duplicate (el->count == 1)
    we can dump the row here in case of GROUP_CONCAT(DISTINCT...)
    instead of doing tree traverse later.
  */
  if (result.length() <= max_length && !warning_for_row && (!tree || (el->count == 1 && distinct && !arg_count_order)))
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  ...
  {
    if ((!args[i]->fixed && args[i]->fix_fields(thd, args + i)) || args[i]->check_cols(1)) return TRUE;
  }

  if (agg_item_charsets(collation, func_name(), args, arg_count, MY_COLL_ALLOW_CONV)) return 1;
...
bool Item_func_group_concat::setup(THD *thd)
{
  ...
  */
  if (table || tree) DBUG_RETURN(FALSE);

  if (!(tmp_table_param = new TMP_TABLE_PARAM)) DBUG_RETURN(TRUE);
  ...
  {
    Item *item = args[i];
    if (list.push_back(item)) DBUG_RETURN(TRUE);
    if (item->const_item())
    {
      if (item->is_null())
      {
  ...
  /*
    ...
    tmp table columns.
  */
  if (arg_count_order && setup_order(thd, args, context->table_list, list, all_fields, *order)) DBUG_RETURN(TRUE);
  ...
  */
  if (!(table = create_tmp_table(thd, tmp_table_param, all_fields, (ORDER *)0, 0, TRUE,
                                 (select_lex->options | thd->options), HA_POS_ERROR, (char *)"")))
    DBUG_RETURN(TRUE);
  ...
  if (distinct || arg_count_order)
  {
    ...
    */
    init_tree(tree, min(thd->variables.max_heap_table_size, thd->variables.sortbuff_size / 16), 0, tree_key_length,
              compare_key, 0, NULL, (void *)this);
  }
...
String *Item_func_group_concat::val_str(String *str)
{
  ...
  if (count_cut_values && !warning)
  {
    ...
    */
    DBUG_ASSERT(table);
    warning = push_warning(table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN, ER_CUT_VALUE_GROUP_CONCAT,
                           ER(ER_CUT_VALUE_GROUP_CONCAT));
  }
  if (result.length()) return &result;
  if (tree) tree_walk(tree, (tree_walk_action)&dump_leaf_key, (void *)this, left_root_right);
  return &result;
}
...
void Item_func_group_concat::print(String *str)
{
  ...
  if (distinct) str->append("distinct ", 9);
  for (uint i = 0; i < arg_count_field; i++)
  {
    if (i) str->append(',');
    args[i]->print(str);
