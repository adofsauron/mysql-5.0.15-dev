                              ...
                              side */
{
  ...
  ut_a((search_cond->func == '<') || (search_cond->func == '>') || (search_cond->func == '=') ||
       (search_cond->func == PARS_GE_TOKEN) || (search_cond->func == PARS_LE_TOKEN));
  ...
  else if ((cmp_type == OPT_COMPARISON) && (search_cond->func != '<') && (search_cond->func != '>') &&
           (search_cond->func != PARS_GE_TOKEN) && (search_cond->func != PARS_LE_TOKEN))
  {
    return (NULL);
  }
  ...
  if (que_node_get_type(arg) == QUE_NODE_SYMBOL)
  {
    ...
    if ((sym_node->token_type == SYM_COLUMN) && (sym_node->table == table) && (sym_node->col_no == col_no))
    {
                              ...
                              PARS_GE_TOKEN, ... ) */
{
  ...
  if (search_cond->func == PARS_AND_TOKEN)
  {
    ...
    exp = opt_look_for_col_in_cond_before(cmp_type, col_no, new_cond, sel_node, nth_table, op);
    if (exp)
    {
      return (exp);
    }
  ...
  exp = opt_look_for_col_in_comparison_before(cmp_type, col_no, search_cond, sel_node, nth_table, op);
  if (exp == NULL)
  {
    return (NULL);
  }
                             ...
                             goodness > 1 */
{
  ...
  for (j = 0; j < n_fields; j++)
  {
    ...
    exp = opt_look_for_col_in_cond_before(OPT_EQUAL, col_no, sel_node->search_cond, sel_node, nth_table, &op);
    ...
    else
    {
      ...
      exp = opt_look_for_col_in_cond_before(OPT_COMPARISON, col_no, sel_node->search_cond, sel_node, nth_table, &op);
      if (exp)
      {
        index_plan[j] = exp;
        *last_op = op;
...
UNIV_INLINE
ulint opt_calc_n_fields_from_goodness(
    /*============================*/
    ...
    dict_table_t *table)  /* in: table */
{
  ...
  else
  {
    ...
    plan->tuple_exps = mem_heap_alloc(pars_sym_tab_global->heap, n_fields * sizeof(void *));
  ...
  if ((best_index->type & DICT_CLUSTERED) && (plan->n_exact_match >= dict_index_get_n_unique(best_index)))
  {
  ...
  if ((table->type != DICT_TABLE_ORDINARY) && (best_index->type & DICT_CLUSTERED))
  {
    ...
    if (mix_id_pos < n_fields)
    {
      ...
      plan->tuple_exps[mix_id_pos] = sym_tab_add_str_lit(pars_sym_tab_global, table->mix_id_buf, table->mix_id_len);
    }
  }
  else
  {
    ...
    func_node_t *cond)    /* in: comparison condition */
{
  ...
  if ((n_fields > plan->n_exact_match) && opt_is_arg(plan->tuple_exps[n_fields - 1], cond))
  {
  ...
  if ((dict_index_get_n_fields(plan->index) > plan->n_exact_match) &&
      opt_look_for_col_in_comparison_before(OPT_COMPARISON, dict_index_get_nth_col_no(plan->index, plan->n_exact_match),
                                            cond, sel_node, i, &op))
  {
    if (sel_node->asc && ((op == '<') || (op == PARS_LE_TOKEN)))
    {
                               ...
                               NULL */
{
  ...
  sym_node->field_nos[SYM_CLUST_FIELD_NO] =
      dict_index_get_nth_col_pos(dict_table_get_first_index(index->table), sym_node->col_no);
  if (!(index->type & DICT_CLUSTERED))
  {
    ut_a(plan);
    ...
    ulint n)              /* in: nth table in select */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    {
      fprintf(stderr, "InnoDB: Error in pars0opt.c: table %s has prefix_len != 0\n", index->table_name);
    }

    *(plan->clust_map + i) = pos;

    ut_ad((pos != ULINT_UNDEFINED) || ((table->type == DICT_TABLE_CLUSTER_MEMBER) && (i == table->mix_len)));
  }
    ...
    sel_node_t *sel_node) /* in: parsed select node */
{
  ...
  sel_node->plans = mem_heap_alloc(pars_sym_tab_global->heap, sel_node->n_tables * sizeof(plan_t));
    ...
    sel_node_t *sel_node) /* in: select node */
{
  ...
  for (i = 0; i < sel_node->n_tables; i++)
  {
    ...
    fprintf(stderr, "; exact m. %lu, match %lu, end conds %lu\n", (unsigned long)plan->n_exact_match,
            (unsigned long)n_fields, (unsigned long)UT_LIST_GET_LEN(plan->end_conds));
  }
}
