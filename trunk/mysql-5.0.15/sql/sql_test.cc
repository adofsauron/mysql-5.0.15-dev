...
static const char *lock_descriptions[] = {"No lock",
...
void TEST_filesort(SORT_FIELD *sortorder, uint s_length)
{
  ...
  for (sep = ""; s_length--; sortorder++, sep = " ")
  {
    ...
    if (sortorder->field)
    {
      ...
      out.append(sortorder->field->field_name ? sortorder->field->field_name : "tmp_table_column");
    }
    else
    {
      str.length(0);
...
static void push_locks_into_array(DYNAMIC_ARRAY *ar, THR_LOCK_DATA *data, bool wait, const char *text)
{
  if (data)
  {
    ...
    if (table && table->s->tmp_table == NO_TMP_TABLE)
    {
      ...
      memcpy(table_lock_info.table_name, table->s->table_cache_key, table->s->key_length);
...
r_requests:     %10s\n\
reads:          %10s\n\n",
        name, (ulong)key_cache->param_buff_size, key_cache->param_block_size, key_cache->param_division_limit,
        key_cache->param_age_threshold, key_cache->blocks_used, key_cache->global_blocks_changed,
        llstr(key_cache->global_cache_w_requests, llbuff1), llstr(key_cache->global_cache_write, llbuff2),
        llstr(key_cache->global_cache_r_requests, llbuff3), llstr(key_cache->global_cache_read, llbuff4));
  }
  return 0;
}
...
Open files:    %10lu\n\
Open streams:  %10lu\n",
      tmp.opened_tables, (ulong)cached_tables(), (ulong)my_file_opened, (ulong)my_stream_opened);
...
Estimated memory (with thread stack):    %ld\n",
      (int)info.arena, (int)info.ordblks, (int)info.smblks, (int)info.hblks, (int)info.hblkhd, (int)info.usmblks,
