...
int heap_delete(HP_INFO *info, const byte *record)
{
  ...
  if (--(share->records) < share->blength >> 1)
    share->blength >>= 1;
...
int hp_rb_delete_key(HP_INFO *info, register HP_KEYDEF *keyinfo, const byte *record, byte *recpos, int flag)
{
...
int hp_delete_key(HP_INFO *info, register HP_KEYDEF *keyinfo, const byte *record, byte *recpos, int flag)
{
  ...
  pos = hp_find_hash(&keyinfo->block, hp_mask(hp_rec_hashnr(keyinfo, record), blength, share->records + 1));
  gpos = pos3 = 0;
  ...
  if (flag)
  {
    ...
    DBUG_PRINT("info", ("Corrected current_ptr to point at: 0x%lx", info->current_ptr));
  }
  empty = pos;
  if (gpos)
  ...
  pos = hp_find_hash(&keyinfo->block, hp_mask(lastpos_hashnr, share->blength, share->records));
  ...
  pos3 = hp_find_hash(&keyinfo->block, hp_mask(pos_hashnr, share->blength, share->records));
  if (pos != pos3)
