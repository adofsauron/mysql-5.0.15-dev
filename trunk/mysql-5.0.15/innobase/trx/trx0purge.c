    ...
    dulint *undo_no)     /* out: undo number */
{
  ...
  for (i = 0;; i++)
  {
    ...
    if (cell->in_use)
    {
      ...
      if ((trx_cmp > 0) || ((trx_cmp == 0) && (ut_dulint_cmp(cell->undo_no, pair_undo_no) >= 0)))
      {
...
/*===========================*/
{
  ...
  while (rseg)
  {
    ...
    if (rseg->last_page_no != FIL_NULL)
    {
      if ((min_rseg == NULL) || (ut_dulint_cmp(min_trx_no, rseg->last_trx_no) > 0))
      {
        min_rseg = rseg;
    ...
    mem_heap_t *heap)      /* in: memory heap where copied */
{
  ...
  if (!purge_sys->next_stored)
  {
    ...
    if (!purge_sys->next_stored)
    {
      ...
      {
        fprintf(stderr, "Purge: No logs left in the history list; pages handled %lu\n",
...
/*=====================*/
{
  ...
  fprintf(stderr,
          ...
          (ulong)purge_sys->next_stored, (ulong)purge_sys->page_no, (ulong)purge_sys->offset,
          (ulong)purge_sys->hdr_page_no, (ulong)purge_sys->hdr_offset);
}
