...
WF_PACK *wf_comp(my_string str)
{
  ...
  for (pos = str; *pos; pos++) ant += test(*pos == ' ' || *pos == ',');

  if ((ret = (WF_PACK *)my_malloc((uint)ant * (sizeof(my_string *) + 2) + sizeof(WF_PACK) + (uint)strlen(str) + 1,
                                  MYF(MY_WME))) == 0)
    DBUG_RETURN((WF_PACK *)NULL);
  ...
  {
    ret->wild[ant++] = buffer;
    while (*pos != ' ' && *pos != ',' && *pos != '!' && *pos) *buffer++ = *pos++;
