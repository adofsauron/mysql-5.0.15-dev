...
public:
  field_info(Item *a, analyse *b)
      : treemem(0), tree_elements(0), empty(0), nulls(0), min_length(0),
        max_length(0), room_in_tree(1), found(0), item(a), pc(b) {};
...
public:
  field_str(Item *a, analyse *b)
      : field_info(a, b), min_arg("", default_charset_info),
        max_arg("", default_charset_info), sum(0), must_be_blob(0),
        was_zero_fill(0), was_maybe_zerofill(0), can_be_still_num(1) {
    init_tree(&tree, 0, 0, sizeof(String), (qsort_cmp2)sortcmp2, 0,
...
public:
  field_decimal(Item *a, analyse *b) : field_info(a, b) {
    ...
    init_tree(&tree, 0, 0, bin_size, (qsort_cmp2)compare_decimal2, 0, 0,
              (void *)&bin_size);
  };
...
public:
  field_real(Item *a, analyse *b)
      : field_info(a, b), min_arg(0), max_arg(0), sum(0), sum_sqr(0),
        max_notzero_dec_len(0) {
    init_tree(&tree, 0, 0, sizeof(double), (qsort_cmp2)compare_double2, 0, NULL,
              NULL);
  }
...
public:
  field_longlong(Item *a, analyse *b)
      : field_info(a, b), min_arg(0), max_arg(0), sum(0), sum_sqr(0) {
    init_tree(&tree, 0, 0, sizeof(longlong), (qsort_cmp2)compare_longlong2, 0,
              NULL, NULL);
  }
...
public:
  field_ulonglong(Item *a, analyse *b)
      : field_info(a, b), min_arg(0), max_arg(0), sum(0), sum_sqr(0) {
    init_tree(&tree, 0, 0, sizeof(ulonglong), (qsort_cmp2)compare_ulonglong2, 0,
              NULL, NULL);
  }
  void add();
  void get_opt_type(String *, ha_rows);
...
public:
  ...
  analyse(select_result *res)
      : Procedure(res, PROC_NO_SORT), f_info(0), rows(0), output_str_length(0) {
  }
