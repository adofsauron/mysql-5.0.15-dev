  ...
  if (sensitive_cursor->is_open()) {
    ...
    *pcursor = sensitive_cursor;
    ...
    if (!(materialized_cursor =
              new (mem_root) Materialized_cursor(result, table))) {
      rc = 1;
      goto err_open;
    }
...
Sensitive_cursor::Sensitive_cursor(THD *thd, select_result *result_arg)
    : Server_side_cursor(&main_mem_root, result_arg), stmt_arena(0), join(0),
...
int Sensitive_cursor::open(JOIN *join_arg) {
  ...
  */
  ...
  for (JOIN_TAB *tab = join_tab; tab != join->join_tab + join->tables - 1;
       tab++) {
...
Materialized_cursor::Materialized_cursor(select_result *result_arg,
                                         TABLE *table_arg)
    : Server_side_cursor(&table_arg->mem_root, result_arg), table(table_arg),
      fetch_limit(0), fetch_count(0) {
...
bool Select_materialize::send_fields(List<Item> &list, uint flags) {
  bool rc;
  DBUG_ASSERT(table == 0);
  if (create_result_table(unit->thd, unit->get_unit_column_types(), FALSE,
