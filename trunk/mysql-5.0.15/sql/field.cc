...
void Field_num::prepend_zeros(String *value) {
  ...
  if ((diff = (int)(field_length - value->length())) > 0) {
    bmove_upp((char *)value->ptr() + field_length,
              value->ptr() + value->length(), value->length());
...
bool Field::check_int(const char *str, int length, const char *int_end,
  ...
  if (str == int_end) {
    ...
    push_warning_printf(table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN,
                        ...
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "integer",
                        tmp.c_ptr(), field_name,
...
static bool test_if_real(const char *str, int length, CHARSET_INFO *cs) {
  ...
  while (length && my_isspace(cs, *str)) { // Allow start space
    length--;
    str++;
  }
  if (!length)
  ...
  for (; length; length--, str++) { // Allow end space
    if (!my_isspace(cs, *str))
      return 0;
  }
  return 1;
...
String *Field::val_int_as_str(String *val_buffer, my_bool unsigned_val) {
  ...
  if (val_buffer->alloc(length))
    return 0;
  length = (uint) (*cs->cset->longlong10_to_str)(
      cs, (char *)val_buffer->ptr(), length, unsigned_val ? 10 : -10, value);
...
Field::Field(char *ptr_arg, uint32 length_arg, uchar *null_ptr_arg,
             uchar null_bit_arg, utype unireg_check_arg,
             const char *field_name_arg, struct st_table *table_arg)
    : ptr(ptr_arg), null_ptr(null_ptr_arg), table(table_arg),
      orig_table(table_arg),
      table_name(table_arg ? &table_arg->alias : &unknown_table_name),
      field_name(field_name_arg), query_id(0), key_start(0), part_of_key(0),
...
longlong Field::convert_decimal2longlong(const my_decimal *val,
  ...
  if (unsigned_flag) {
    if (val->sign()) {
      ...
      i = 0;
      *err = 1;
    } else if (warn_if_overflow(my_decimal2int(E_DEC_ERROR & ~E_DEC_OVERFLOW &
                                                   ~E_DEC_TRUNCATED,
                                               val, TRUE, &i))) {
      i = ~(longlong)0;
      *err = 1;
...
Field_new_decimal::Field_new_decimal(char *ptr_arg, uint32 len_arg,
...
Field_new_decimal::Field_new_decimal(uint32 len_arg, bool maybe_null,
                                     const char *name, struct st_table *t_arg,
                                     uint8 dec_arg, bool unsigned_arg)
    : Field_num((char *)0, len_arg, maybe_null ? (uchar *)"" : 0, 0, NONE, name,
                t_arg, dec_arg, 0, unsigned_arg) {
  precision = my_decimal_length_to_precision(len_arg, dec_arg, unsigned_arg);
  DBUG_ASSERT((precision <= DECIMAL_MAX_PRECISION) &&
              (dec <= DECIMAL_MAX_SCALE));
  bin_size = my_decimal_get_binary_size(precision, dec);
...
bool Field_new_decimal::store_value(const my_decimal *decimal_value) {
  ...
  DBUG_EXECUTE("info",
               print_decimal_buff(decimal_value, (byte *)ptr, bin_size););
  DBUG_RETURN(error);
}
...
int Field_new_decimal::store(const char *from, uint length,
  ...
  if ((err =
    ...
    push_warning_printf(table->in_use, MYSQL_ERROR::WARN_LEVEL_ERROR,
                        ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "decimal", from,
                        field_name, (ulong)table->in_use->row_count);
    DBUG_RETURN(err);
  }
...
int Field_new_decimal::store(longlong nr, bool unsigned_val) {
  ...
  if ((err = int2my_decimal(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, nr,
...
double Field_tiny::val_real(void) {
  int tmp =
      unsigned_flag ? (int)((uchar *)ptr)[0] : (int)((signed char *)ptr)[0];
  return (double)tmp;
}
...
int Field_longlong::cmp(const char *a_ptr, const char *b_ptr) {
  ...
  if (unsigned_flag)
    return ((ulonglong)a < (ulonglong)b) ? -1 : ((ulonglong)a > (ulonglong)b)
                                                    ? 1
                                                    : 0;
  return (a < b) ? -1 : (a > b) ? 1 : 0;
}

void Field_longlong::sort_string(char *to,
                                 uint length __attribute__((unused))) {
#ifdef WORDS_BIGENDIAN
  if (!table->s->db_low_byte_first) {
    if (unsigned_flag)
...
int Field_timestamp::store(const char *from, uint len, CHARSET_INFO *cs) {
  ...
  have_smth_to_conv =
      (str_to_datetime(from, len, &l_time,
  ...
  if (have_smth_to_conv && l_time.month) {
    if (!(tmp = TIME_to_timestamp(thd, &l_time, &in_dst_time_gap))) {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN,
                           ER_WARN_DATA_OUT_OF_RANGE, from, len,
                           MYSQL_TIMESTAMP_DATETIME, !error);
      error = 1;
    } else if (in_dst_time_gap) {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN,
                           ER_WARN_INVALID_TIMESTAMP, from, len,
                           MYSQL_TIMESTAMP_DATETIME, !error);
      error = 1;
    }
  }
...
int Field_timestamp::store(longlong nr, bool unsigned_val) {
  ...
  /* We don't want to store invalid or fuzzy datetime values in TIMESTAMP */
  longlong tmp = number_to_datetime(
      nr, &l_time,
  ...
  if (!error && tmp) {
    ...
    if (in_dst_time_gap) {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN,
                           ER_WARN_INVALID_TIMESTAMP, nr,
                           MYSQL_TIMESTAMP_DATETIME, 1);
      error = 1;
    }
  } else if (error)
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, nr,
                         MYSQL_TIMESTAMP_DATETIME, 1);
...
String *Field_timestamp::val_str(String *val_buffer, String *val_ptr) {
  ...
  if (temp == 0L) { /* Zero time is "000000" */
...
void Field_timestamp::sort_string(char *to,
                                  uint length __attribute__((unused))) {
#ifdef WORDS_BIGENDIAN
  if (!table->s->db_low_byte_first) {
    to[0] = ptr[0];
...
int Field_time::store(const char *from, uint len, CHARSET_INFO *cs) {
  ...
  if (str_to_time(from, len, &ltime, &error)) {
    ...
    if (ltime.month)
      ltime.day = 0;
    tmp = (ltime.day * 24L + ltime.hour) * 10000L +
          (ltime.minute * 100 + ltime.second);
    if (tmp > 8385959) {
      tmp = 8385959;
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN,
                           ER_WARN_DATA_OUT_OF_RANGE, from, len,
                           MYSQL_TIMESTAMP_TIME, !error);
      error = 1;
    }
    if (error > 1)
      error = 2;
...
bool Field_time::get_date(TIME *ltime, uint fuzzydate) {
  ...
  if (!(fuzzydate & TIME_FUZZY_DATE)) {
    push_warning_printf(
        table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE,
...
int Field_date::store(const char *from, uint len, CHARSET_INFO *cs) {
  ...
  if (str_to_datetime(
    ...
    tmp = (uint32)l_time.year * 10000L +
          (uint32)(l_time.month * 100 + l_time.day);
...
int Field_date::store(double nr) {
  ...
  if (nr < 0.0 || nr > 99991231.0) {
    tmp = LL(0);
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN,
                         ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_DATE);
    error = 1;
  } else
...
int Field_datetime::store_time(TIME *ltime, timestamp_type type) {
  ...
  */
  if (type == MYSQL_TIMESTAMP_DATE || type == MYSQL_TIMESTAMP_DATETIME)
    tmp = ((ltime->year * 10000L + ltime->month * 100 + ltime->day) *
               LL(1000000) +
...
bool Field_datetime::get_date(TIME *ltime, uint fuzzydate) {
  ...
  return (!(fuzzydate & TIME_FUZZY_DATE) && (!ltime->month || !ltime->day)) ? 1
                                                                            : 0;
}
...
int Field_datetime::cmp(const char *a_ptr, const char *b_ptr) {
  ...
  return ((ulonglong)a < (ulonglong)b) ? -1 : ((ulonglong)a > (ulonglong)b) ? 1
                                                                            : 0;
}

void Field_datetime::sort_string(char *to,
                                 uint length __attribute__((unused))) {
#ifdef WORDS_BIGENDIAN
  if (!table->s->db_low_byte_first) {
    to[0] = ptr[0];
...
int Field_string::store(const char *from, uint length, CHARSET_INFO *cs) {
  ...
  */
  copy_length = field_charset->cset->well_formed_len(
      field_charset, from, from + length,
  ...
  if ((copy_length < length) &&
      table->in_use->count_cuted_fields) { // Check if we loosed some info
...
int Field_str::store(double nr) {
  ...
  */
  ...
  if (char_length > 4 && char_length < 32 &&
      (anr < 1.0
           ? anr > 1 / (log_10[max(0, (int)char_length - neg - 2)]) /* -2 for
                                                                       "0." */
  ...
  length = (uint)my_sprintf(
      buff, (buff, "%-.*g",
             (use_scientific_notation ? max(0, (int)char_length - neg - 5)
                                      : char_length),
             nr));
  /*
...
int Field_string::cmp(const char *a_ptr, const char *b_ptr) {
  ...
  if (field_charset->strxfrm_multiply > 1) {
    ...
    */
    return field_charset->coll->strnncollsp(field_charset, (const uchar *)a_ptr,
                                            field_length, (const uchar *)b_ptr,
                                            field_length, 0);
  }
...
void Field_string::sort_string(char *to, uint length) {
  uint tmp = my_strnxfrm(field_charset, (unsigned char *)to, length,
...
int Field_string::pack_cmp(const char *a, const char *b, uint length,
  ...
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)a,
                                          a_length, (const uchar *)b, b_length,
                                          insert_or_update);
}
...
int Field_string::pack_cmp(const char *key, uint length,
  ...
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)ptr,
                                          row_length, (const uchar *)key,
                                          key_length, insert_or_update);
}
...
Field *Field_string::new_field(MEM_ROOT *root, struct st_table *new_table) {
  ...
  */
  return new Field_varstring(field_length, maybe_null(), field_name, new_table,
                             charset());
}
...
int Field_varstring::cmp(const char *a_ptr, const char *b_ptr) {
  ...
  diff = field_charset->coll->strnncollsp(
      field_charset, (const uchar *)a_ptr + length_bytes, a_length,
...
int Field_varstring::key_cmp(const byte *key_ptr, uint max_key_length) {
  ...
  set_if_smaller(length, char_length);
  return field_charset->coll->strnncollsp(
      field_charset, (const uchar *)ptr + length_bytes, length,
...
int Field_varstring::key_cmp(const byte *a, const byte *b) {
  return field_charset->coll->strnncollsp(
      field_charset, (const uchar *)a + HA_KEY_BLOB_LENGTH, uint2korr(a),
...
void Field_varstring::sort_string(char *to, uint length) {
  ...
  tot_length = my_strnxfrm(field_charset, (uchar *)to, length,
                           (uchar *)ptr + length_bytes, tot_length);
...
char *Field_varstring::pack_key(char *to, const char *key, uint max_length) {
  ...
  uint char_length =
      ((field_charset->mbmaxlen > 1) ? max_length / field_charset->mbmaxlen
                                     : max_length);
  key += length_bytes;
  if (length > char_length) {
...
int Field_varstring::pack_cmp(const char *b, uint key_length,
  ...
  uint b_length;
  uint char_length =
      ((field_charset->mbmaxlen > 1) ? key_length / field_charset->mbmaxlen
                                     : key_length);
...
Field *Field_varstring::new_key_field(MEM_ROOT *root,
                                      struct st_table *new_table, char *new_ptr,
...
int Field_blob::store(const char *from, uint length, CHARSET_INFO *cs) {
  ...
  if (!length) {
    ...
    /* Convert character set if necessary */
    if ((was_conversion =
             String::needs_conversion(length, cs, field_charset, &not_used))) {
      uint conv_errors;
    ...
    */
    copy_length = field_charset->cset->well_formed_len(
        field_charset, from, from + min(length, copy_length), copy_length,
        &well_formed_error);
    if (copy_length < length)
      error = 2;
    Field_blob::store_length(copy_length);
    if (was_conversion || table->copy_blobs ||
        copy_length <= MAX_FIELD_WIDTH) { // Must make a copy
...
int Field_blob::cmp(const char *a, uint32 a_length, const char *b,
                    uint32 b_length) {
  return field_charset->coll->strnncollsp(
      field_charset, (const uchar *)a, a_length, (const uchar *)b, b_length, 0);
}
...
void Field_blob::get_key_image(char *buff, uint length, imagetype type) {
  ...
  if (type == itMBR) {
    ...
    get_ptr(&blob);
    gobj = Geometry::create_from_wkb(&buffer, blob + SRID_SIZE,
                                     blob_length - SRID_SIZE);
  ...
  uint char_length = length / field_charset->mbmaxlen;
  char_length =
      my_charpos(field_charset, blob, blob + blob_length, char_length);
...
void Field_blob::sort_string(char *to, uint length) {
  ...
  if (!blob_length)
    ...
    blob_length = my_strnxfrm(field_charset, (uchar *)to, length, (uchar *)blob,
                              blob_length);
...
const char *Field_blob::unpack_key(char *to, const char *from,
                                   uint max_length) {
  /* get length of the blob key */
...
int Field_geom::store(double nr) {
  my_message(ER_CANT_CREATE_GEOMETRY_OBJECT, ER(ER_CANT_CREATE_GEOMETRY_OBJECT),
             MYF(0));
  return -1;
}
...
int Field_set::store(longlong nr, bool unsigned_val) {
  int error = 0;
  if ((ulonglong)nr >
      (ulonglong)(((longlong)1 << typelib->count) - (longlong)1)) {
...
bool Field_enum::eq_def(Field *field) {
  ...
  for (uint i = 0; i < from_lib->count; i++)
    if (my_strnncoll(field_charset, (const uchar *)typelib->type_names[i],
...
Field_bit::Field_bit(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg,
    ...
    : Field(ptr_arg, len_arg >> 3, null_ptr_arg, null_bit_arg, unireg_check_arg,
            field_name_arg, table_arg),
...
Field *Field_bit::new_key_field(MEM_ROOT *root, struct st_table *new_table,
                                char *new_ptr, uchar *new_null_ptr,
                                uint new_null_bit) {
  Field_bit *res;
...
void Field_bit::sql_type(String &res) const {
  ...
  ulong length = cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(),
                                    "bit(%d)", (int)field_length * 8 + bit_len);
...
void create_field::create_length_to_internal_length(void) {
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    key_length = pack_length = my_decimal_get_binary_size(
        my_decimal_length_to_precision(length, decimals, flags & UNSIGNED_FLAG),
        decimals);
    break;
  default:
...
Field *make_field(char *ptr, uint32 field_length, uchar *null_pos,
                  ...
                  CHARSET_INFO *field_charset, Field::geometry_type geom_type,
    ...
    if (!f_is_packed(pack_flag)) {
      ...
      if (field_type == MYSQL_TYPE_VARCHAR)
        return new Field_varstring(
            ...
            null_bit, unireg_check, field_name, table, field_charset);
      return 0; // Error
    }

    uint pack_length =
        calc_pack_length((enum_field_types)f_packtype(pack_flag), field_length);

#ifdef HAVE_SPATIAL
    if (f_is_geom(pack_flag))
      return new Field_geom(ptr, null_pos, null_bit, unireg_check, field_name,
                            table, pack_length, geom_type);
#endif
    if (f_is_blob(pack_flag))
      return new Field_blob(ptr, null_pos, null_bit, unireg_check, field_name,
                            table, pack_length, field_charset);
    if (interval) {
      if (f_is_enum(pack_flag))
        return new Field_enum(ptr, field_length, null_pos, null_bit,
                              unireg_check, field_name, table, pack_length,
  ...
  case FIELD_TYPE_DECIMAL:
    return new Field_decimal(
        ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
        f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0,
  ...
  case FIELD_TYPE_NEWDECIMAL:
    return new Field_new_decimal(
        ...
        f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_FLOAT:
    return new Field_float(ptr, field_length, null_pos, null_bit, unireg_check,
                           field_name, table, f_decimals(pack_flag),
                           f_is_zerofill(pack_flag) != 0,
                           f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_DOUBLE:
    return new Field_double(ptr, field_length, null_pos, null_bit, unireg_check,
                            field_name, table, f_decimals(pack_flag),
                            f_is_zerofill(pack_flag) != 0,
                            f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_TINY:
    return new Field_tiny(ptr, field_length, null_pos, null_bit, unireg_check,
                          field_name, table, f_is_zerofill(pack_flag) != 0,
                          f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_SHORT:
    return new Field_short(ptr, field_length, null_pos, null_bit, unireg_check,
                           field_name, table, f_is_zerofill(pack_flag) != 0,
                           f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_INT24:
    return new Field_medium(ptr, field_length, null_pos, null_bit, unireg_check,
                            field_name, table, f_is_zerofill(pack_flag) != 0,
                            f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_LONG:
    return new Field_long(ptr, field_length, null_pos, null_bit, unireg_check,
                          field_name, table, f_is_zerofill(pack_flag) != 0,
                          f_is_dec(pack_flag) == 0);
  case FIELD_TYPE_LONGLONG:
  ...
  case FIELD_TYPE_TIMESTAMP:
    return new Field_timestamp(ptr, field_length, null_pos, null_bit,
                               unireg_check, field_name, table, field_charset);
  case FIELD_TYPE_YEAR:
    return new Field_year(ptr, field_length, null_pos, null_bit, unireg_check,
                          field_name, table);
  case FIELD_TYPE_DATE:
    return new Field_date(ptr, null_pos, null_bit, unireg_check, field_name,
                          table, field_charset);
  case FIELD_TYPE_NEWDATE:
    return new Field_newdate(ptr, null_pos, null_bit, unireg_check, field_name,
                             table, field_charset);
  case FIELD_TYPE_TIME:
    return new Field_time(ptr, null_pos, null_bit, unireg_check, field_name,
                          table, field_charset);
  case FIELD_TYPE_DATETIME:
    return new Field_datetime(ptr, null_pos, null_bit, unireg_check, field_name,
                              table, field_charset);
  case FIELD_TYPE_NULL:
  ...
  case FIELD_TYPE_BIT:
    return f_bit_as_char(pack_flag)
               ? new Field_bit_as_char(ptr, field_length, null_pos, null_bit,
                                       bit_ptr, bit_offset, unireg_check,
                                       field_name, table)
...
create_field::create_field(Field *old_field, Field *orig_field) {
  ...
  if (!(flags & (NO_DEFAULT_VALUE_FLAG | BLOB_FLAG)) && old_field->ptr &&
      orig_field &&
...
bool Field::set_warning(MYSQL_ERROR::enum_warning_level level, uint code,
                        int cuted_increment) {
  THD *thd = table->in_use;
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level,
                                 uint code, const char *str, uint str_length,
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level,
                                 uint code, longlong nr, timestamp_type ts_type,
                                 int cuted_increment) {
  if (table->in_use->really_abort_on_warning() ||
      set_warning(level, code, cuted_increment)) {
    char str_nr[22];
    ...
    make_truncated_value_warning(table->in_use, str_nr,
                                 (uint)(str_end - str_nr), ts_type, field_name);
  }
}
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level,
                                 uint code, double nr, timestamp_type ts_type) {
