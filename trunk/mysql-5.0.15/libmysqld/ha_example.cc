...
*/
static byte *example_get_key(EXAMPLE_SHARE *share, uint *length, my_bool not_used __attribute__((unused)))
{
  ...
  */
  if (!example_init)
  {
    ...
    if (!example_init)
    {
      ...
      (void)hash_init(&example_open_tables, system_charset_info, 32, 0, 0, (hash_get_key)example_get_key, 0, 0);
    }
  ...
  if (!(share = (EXAMPLE_SHARE *)hash_search(&example_open_tables, (byte *)table_name, length)))
  {
    if (!(share = (EXAMPLE_SHARE *)my_multi_malloc(MYF(MY_WME | MY_ZEROFILL), &share, sizeof(*share), &tmp_name,
                                                   length + 1, NullS)))
    {
...
const char **ha_example::bas_ext() const { return ha_example_exts; }
...
/*
  ...
  index.
*/
int ha_example::index_read(byte *buf, const byte *key, uint key_len __attribute__((unused)),
...
*/
int ha_example::index_read_idx(byte *buf, uint index, const byte *key, uint key_len __attribute__((unused)),
                               enum ha_rkey_function find_flag __attribute__((unused)))
{
