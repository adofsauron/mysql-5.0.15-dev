...
int sp_rcontext::set_item_eval(THD *thd, uint idx, Item **item_addr,
  ...
  while (i--) {
    ...
    case sp_cond_type_t::exception:
      if ((sqlstate[0] != '0' || sqlstate[1] > '2') &&
          level == MYSQL_ERROR::WARN_LEVEL_ERROR && found < 0)
        found = i;
      break;
    }
  }
...
int sp_cursor::fetch(THD *thd, List<struct sp_pvar> *vars) {
  ...
  if (vars->elements != result.get_field_count()) {
    my_message(ER_SP_WRONG_NO_OF_FETCH_ARGS, ER(ER_SP_WRONG_NO_OF_FETCH_ARGS),
               MYF(0));
    return -1;
  }
