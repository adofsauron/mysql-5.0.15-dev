...
class Item_sum : public Item_result_field
{
 public:
  ...
  Item_sum() : arg_count(0), quick_group(1) { mark_as_sum_func(); }
  ...
  */
  ...
  virtual Field *create_tmp_field(bool group, TABLE *table, uint convert_blob_length);
...
{
 private:
  Item_sum_sum_distinct(THD *thd, Item_sum_sum_distinct *item) : Item_sum_distinct(thd, item) {}
...
{
 private:
  Item_sum_avg_distinct(THD *thd, Item_sum_avg_distinct *original) : Item_sum_distinct(thd, original) {}
...
class Item_sum_count : public Item_sum_int
{
 ...
 public:
  Item_sum_count(Item *item_par) : Item_sum_int(item_par), count(0), used_table_cache(~(table_map)0) {}
  Item_sum_count(THD *thd, Item_sum_count *item)
      : Item_sum_int(thd, item), count(item->count), used_table_cache(item->used_table_cache)
  {
  }
...
class Item_sum_count_distinct : public Item_sum_int
{
 ...
 public:
  Item_sum_count_distinct(List<Item> &list)
      : Item_sum_int(list), table(0), field_lengths(0), tmp_table_param(0), tree(0), original(0), always_null(FALSE)
  {
    quick_group = 0;
  }
...
class Item_avg_field : public Item_result_field
{
 public:
  ...
  {
    return hybrid_type == DECIMAL_RESULT ? MYSQL_TYPE_NEWDECIMAL : MYSQL_TYPE_DOUBLE;
  }
  void fix_length_and_dec() {}
  enum Item_result result_type() const { return hybrid_type; }
};
...
class Item_sum_avg : public Item_sum_sum
{
 public:
  ...
  Item_sum_avg(THD *thd, Item_sum_avg *item)
      : Item_sum_sum(thd, item), count(item->count), prec_increment(item->prec_increment)
  {
  }
  ...
  void reset_field();
  void update_field();
  Item *result_item(Field *field) { return new Item_avg_field(hybrid_type, this); }
  void no_rows_in_result() {}
...
class Item_sum_variance : public Item_sum_num
{
 ...
 public:
  ...
  Item_sum_variance(Item *item_par, uint sample_arg)
      : Item_sum_num(item_par), hybrid_type(REAL_RESULT), cur_dec(0), count(0), sample(sample_arg)
  {
  }
  ...
  void reset_field();
  void update_field();
  Item *result_item(Field *field) { return new Item_variance_field(this); }
  void no_rows_in_result() {}
  const char *func_name() const { return sample ? "var_samp(" : "variance("; }
  Item *copy_or_same(THD *thd);
  Field *create_tmp_field(bool group, TABLE *table, uint convert_blob_length);
  enum Item_result result_type() const { return hybrid_type; }
};
...
{
 public:
  Item_sum_std(Item *item_par, uint sample_arg) : Item_sum_variance(item_par, sample_arg) {}
  Item_sum_std(THD *thd, Item_sum_std *item) : Item_sum_variance(thd, item) {}
  enum Sumfunctype sum_func() const { return STD_FUNC; }
  double val_real();
  Item *result_item(Field *field) { return new Item_std_field(this); }
 ...
 public:
  Item_sum_hybrid(Item *item_par, int sign)
      : Item_sum(item_par),
        ...
        hybrid_field_type(FIELD_TYPE_LONGLONG),
        cmp_sign(sign),
...
class Item_sum_bit : public Item_sum_int
{
 ...
 public:
  Item_sum_bit(Item *item_par, ulonglong reset_arg) : Item_sum_int(item_par), reset_bits(reset_arg), bits(reset_arg) {}
  Item_sum_bit(THD *thd, Item_sum_bit *item) : Item_sum_int(thd, item), reset_bits(item->reset_bits), bits(item->bits)
  {
  }
...
class Item_udf_sum : public Item_sum
{
 ...
 public:
  ...
  Item_udf_sum(udf_func *udf_arg, List<Item> &list) : Item_sum(list), udf(udf_arg) { quick_group = 0; }
  Item_udf_sum(THD *thd, Item_udf_sum *item) : Item_sum(thd, item), udf(item->udf) { udf.not_original = TRUE; }
...
class Item_sum_udf_float : public Item_udf_sum
{
 public:
  ...
  Item_sum_udf_float(udf_func *udf_arg, List<Item> &list) : Item_udf_sum(udf_arg, list) {}
  Item_sum_udf_float(THD *thd, Item_sum_udf_float *item) : Item_udf_sum(thd, item) {}
  longlong val_int()
  {
    DBUG_ASSERT(fixed == 1);
...
class Item_sum_udf_int : public Item_udf_sum
{
 public:
  ...
  Item_sum_udf_int(udf_func *udf_arg, List<Item> &list) : Item_udf_sum(udf_arg, list) {}
  Item_sum_udf_int(THD *thd, Item_sum_udf_int *item) : Item_udf_sum(thd, item) {}
  longlong val_int();
  double val_real()
  {
    DBUG_ASSERT(fixed == 1);
...
class Item_sum_udf_str : public Item_udf_sum
{
 public:
  ...
  Item_sum_udf_str(udf_func *udf_arg, List<Item> &list) : Item_udf_sum(udf_arg, list) {}
  Item_sum_udf_str(THD *thd, Item_sum_udf_str *item) : Item_udf_sum(thd, item) {}
  String *val_str(String *);
  double val_real()
  {
    int err_not_used;
...
class Item_sum_udf_decimal : public Item_udf_sum
{
 public:
  ...
  Item_sum_udf_decimal(udf_func *udf_arg, List<Item> &list) : Item_udf_sum(udf_arg, list) {}
  Item_sum_udf_decimal(THD *thd, Item_sum_udf_decimal *item) : Item_udf_sum(thd, item) {}
  String *val_str(String *);
  double val_real();
  longlong val_int();
  my_decimal *val_decimal(my_decimal *);
...
class Item_sum_udf_float : public Item_sum_num
{
 public:
  ...
  Item_sum_udf_float(THD *thd, Item_sum_udf_float *item) : Item_sum_num(thd, item) {}
  enum Sumfunctype sum_func() const { return UDF_SUM_FUNC; }
  double val_real()
  {
    DBUG_ASSERT(fixed == 1);
...
class Item_sum_udf_int : public Item_sum_num
{
 public:
  ...
  Item_sum_udf_int(THD *thd, Item_sum_udf_int *item) : Item_sum_num(thd, item) {}
  enum Sumfunctype sum_func() const { return UDF_SUM_FUNC; }
  longlong val_int()
  {
    DBUG_ASSERT(fixed == 1);
...
class Item_sum_udf_decimal : public Item_sum_num
{
 public:
  ...
  Item_sum_udf_decimal(THD *thd, Item_sum_udf_float *item) : Item_sum_num(thd, item) {}
  enum Sumfunctype sum_func() const { return UDF_SUM_FUNC; }
  double val_real()
  {
    DBUG_ASSERT(fixed == 1);
...
class Item_sum_udf_str : public Item_sum_num
{
 public:
  ...
  Item_sum_udf_str(THD *thd, Item_sum_udf_str *item) : Item_sum_num(thd, item) {}
  String *val_str(String *)
  {
    DBUG_ASSERT(fixed == 1);
    null_value = 1;
...
class Item_func_group_concat : public Item_sum
{
  ...
  */
  ...
  friend int group_concat_key_cmp_with_distinct(void *arg, byte *key1, byte *key2);
  friend int group_concat_key_cmp_with_order(void *arg, byte *key1, byte *key2);
  friend int group_concat_key_cmp_with_distinct_and_order(void *arg, byte *key1, byte *key2);
  friend int dump_leaf_key(byte *key, element_count count __attribute__((unused)),
                           Item_func_group_concat *group_concat_item);

 public:
  Item_func_group_concat(Name_resolution_context *context_arg, bool is_distinct, List<Item> *is_select,
  ...
  my_decimal *val_decimal(my_decimal *decimal_value) { return val_decimal_from_string(decimal_value); }
  String *val_str(String *str);
  Item *copy_or_same(THD *thd);
  void no_rows_in_result() {}
  void print(String *str);
