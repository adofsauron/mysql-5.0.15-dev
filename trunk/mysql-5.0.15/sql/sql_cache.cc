...
#define MUTEX_LOCK(M)                                     \
  {                                                       \
    DBUG_PRINT("lock", ("mutex lock 0x%lx", (ulong)(M))); \
    pthread_mutex_lock(M);                                \
  }
#define MUTEX_UNLOCK(M)                                     \
  {                                                         \
    DBUG_PRINT("lock", ("mutex unlock 0x%lx", (ulong)(M))); \
    pthread_mutex_unlock(M);                                \
  }
#define RW_WLOCK(M)                                                                                       \
  {                                                                                                       \
    DBUG_PRINT("lock", ("rwlock wlock 0x%lx", (ulong)(M)));                                               \
    if (!rw_wrlock(M))                                                                                    \
      DBUG_PRINT("lock", ("rwlock wlock ok")) else DBUG_PRINT("lock", ("rwlock wlock FAILED %d", errno)); \
  }
#define RW_RLOCK(M)                                                                                       \
  {                                                                                                       \
    DBUG_PRINT("lock", ("rwlock rlock 0x%lx", (ulong)(M)));                                               \
    if (!rw_rdlock(M))                                                                                    \
      DBUG_PRINT("lock", ("rwlock rlock ok")) else DBUG_PRINT("lock", ("rwlock wlock FAILED %d", errno)); \
  }
#define RW_UNLOCK(M)                                                                                        \
  {                                                                                                         \
    DBUG_PRINT("lock", ("rwlock unlock 0x%lx", (ulong)(M)));                                                \
    if (!rw_unlock(M))                                                                                      \
      DBUG_PRINT("lock", ("rwlock unlock ok")) else DBUG_PRINT("lock", ("rwlock unlock FAILED %d", errno)); \
  }
#define STRUCT_LOCK(M)                                   \
  {                                                      \
    DBUG_PRINT("lock", ("%d struct lock...", __LINE__)); \
    pthread_mutex_lock(M);                               \
    DBUG_PRINT("lock", ("struct lock OK"));              \
  }
#define STRUCT_UNLOCK(M)                                   \
  {                                                        \
    DBUG_PRINT("lock", ("%d struct unlock...", __LINE__)); \
    pthread_mutex_unlock(M);                               \
    DBUG_PRINT("lock", ("struct unlock OK"));              \
  }
#define BLOCK_LOCK_WR(B)                                            \
  {                                                                 \
    DBUG_PRINT("lock", ("%d LOCK_WR 0x%lx", __LINE__, (ulong)(B))); \
    B->query()->lock_writing();                                     \
  }
#define BLOCK_LOCK_RD(B)                                            \
  {                                                                 \
    DBUG_PRINT("lock", ("%d LOCK_RD 0x%lx", __LINE__, (ulong)(B))); \
    B->query()->lock_reading();                                     \
  }
#define BLOCK_UNLOCK_WR(B)                                            \
  {                                                                   \
    DBUG_PRINT("lock", ("%d UNLOCK_WR 0x%lx", __LINE__, (ulong)(B))); \
    B->query()->unlock_writing();                                     \
  }
#define BLOCK_UNLOCK_RD(B)                                            \
  {                                                                   \
    DBUG_PRINT("lock", ("%d UNLOCK_RD 0x%lx", __LINE__, (ulong)(B))); \
    B->query()->unlock_reading();                                     \
  }
#define DUMP(C)            \
  DBUG_EXECUTE("qcache", { \
    (C)->cache_dump();     \
...
TYPELIB query_cache_type_typelib = {array_elements(query_cache_type_names) - 1, "", query_cache_type_names, NULL};
...
{
  return (Query_cache_block *)(((byte *)this) - ALIGN_SIZE(sizeof(Query_cache_block_table) * n) -
...
void Query_cache_block::init(ulong block_length)
{
  ...
  DBUG_PRINT("qcache", ("init block 0x%lx  length: %lu", (ulong)this, block_length));
  length = block_length;
  used = 0;
...
void Query_cache_block::destroy()
{
  ...
  DBUG_PRINT("qcache", ("destroy block 0x%lx, type %d", (ulong)this, type));
  type = INCOMPLETE;
  DBUG_VOID_RETURN;
}
...
inline gptr Query_cache_block::data(void) { return (gptr)(((byte *)this) + headers_len()); }

inline Query_cache_query *Query_cache_block::query()
{
#ifndef DBUG_OFF
  if (type != QUERY) query_cache.wreck(__LINE__, "incorrect block type");
#endif
...
{
#ifndef DBUG_OFF
  if (type != TABLE) query_cache.wreck(__LINE__, "incorrect block type");
#endif
...
{
#ifndef DBUG_OFF
  if (type != RESULT && type != RES_CONT && type != RES_BEG && type != RES_INCOMPLETE)
...
{
  return ((Query_cache_block_table *)(((byte *)this) + ALIGN_SIZE(sizeof(Query_cache_block)) +
...
extern "C"
{
  byte *query_cache_table_get_key(const byte *record, uint *length, my_bool not_used __attribute__((unused)))
  {
    ...
    *length = (table_block->used - table_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_table)));
    return (((byte *)table_block->data()) + ALIGN_SIZE(sizeof(Query_cache_table)));
  }
}
...
void Query_cache_query::init_n_lock()
{
  ...
  lock_writing();
  DBUG_PRINT("qcache",
             ("inited & locked query for block 0x%lx", ((byte *)this) - ALIGN_SIZE(sizeof(Query_cache_block))));
  DBUG_VOID_RETURN;
}
...
void Query_cache_query::unlock_n_destroy()
{
  ...
  DBUG_PRINT("qcache",
             ("destroyed & unlocked query for block 0x%lx", ((byte *)this) - ALIGN_SIZE(sizeof(Query_cache_block))));
  /*
...
extern "C"
{
  byte *query_cache_query_get_key(const byte *record, uint *length, my_bool not_used)
  {
    Query_cache_block *query_block = (Query_cache_block *)record;
    *length = (query_block->used - query_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_query)));
    return (((byte *)query_block->data()) + ALIGN_SIZE(sizeof(Query_cache_query)));
  }
}
...
void query_cache_insert(NET *net, const char *packet, ulong length)
{
  ...
  */
  ...
  if (query_block)
  {
    ...
    */
    if (!query_cache.append_result_data(&result, length, (gptr)packet, query_block))
    {
...
void query_cache_end_of_result(THD *thd)
{
  ...
  {
#ifdef EMBEDDED_LIBRARY
    query_cache_insert(&thd->net, (char *)thd, emb_count_querycache_size(thd));
#endif
    STRUCT_LOCK(&query_cache.structure_guard_mutex);
    /*
      It is very unlikely that following condition is TRUE (it is possible
    ...
    */
    ...
    Query_cache_block *query_block = ((Query_cache_block *)thd->net.query_cache_query);
    if (query_block)
    {
      DUMP(&query_cache);
      BLOCK_LOCK_WR(query_block);
      ...
      if (header->result() == 0)
      {
        DBUG_PRINT("error", ("end of data whith no result. query '%s'", header->query()));
...
Query_cache::Query_cache(ulong query_cache_limit_arg, ulong min_allocation_unit_arg, ulong min_result_data_size_arg,
                         uint def_query_hash_size_arg, uint def_table_hash_size_arg)
    : query_cache_size(0),
      ...
      initialized(0)
{
  ulong min_needed = (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_block_table)) +
...
ulong Query_cache::resize(ulong query_cache_size_arg)
{
  ...
  DBUG_PRINT("qcache", ("from %lu to %lu", query_cache_size, query_cache_size_arg));
...
{
  if (size < min_allocation_unit) size = min_allocation_unit;
...
void Query_cache::store_query(THD *thd, TABLE_LIST *tables_used)
{
  ...
  if (query_cache_size == 0 || thd->locked_tables) DBUG_VOID_RETURN;
  uint8 tables_type = 0;

  if ((local_tables = is_cacheable(thd, thd->query_length, thd->query, thd->lex, tables_used, &tables_type)))
  {
    NET *net = &thd->net;
    ...
    flags.client_protocol_41 = test(thd->client_capabilities & CLIENT_PROTOCOL_41);
    flags.more_results_exists = test(thd->server_status & SERVER_MORE_RESULTS_EXISTS);
    flags.pkt_nr = net->pkt_nr;
    flags.character_set_client_num = thd->variables.character_set_client->number;
    flags.character_set_results_num =
        (thd->variables.character_set_results ? thd->variables.character_set_results->number : UINT_MAX);
...
sql mode: 0x%lx, sort len: %lu, conncat len: %lu",
                          (int)flags.client_long_flag, (int)flags.client_protocol_41, (int)flags.more_results_exists,
                          flags.pkt_nr, flags.character_set_client_num, flags.character_set_results_num,
                          flags.collation_connection_num, flags.limit, (ulong)flags.time_zone, flags.sql_mode,
                          flags.max_sort_length, flags.group_concat_max_len));
    STRUCT_LOCK(&structure_guard_mutex);
    ...
    if (thd->db_length)
    {
      ...
      DBUG_PRINT("qcache", ("database : %s length %u", thd->db, thd->db_length));
    }
    else
    {
      DBUG_PRINT("qcache", ("No active database"));
    }
    tot_length = thd->query_length + thd->db_length + 1 + QUERY_CACHE_FLAGS_SIZE;
    /*
      We should only copy structure (don't use it location directly)
      because of alignment issue
    */
    memcpy((void *)(thd->query + (tot_length - QUERY_CACHE_FLAGS_SIZE)), &flags, QUERY_CACHE_FLAGS_SIZE);

    /* Check if another thread is processing the same query? */
    Query_cache_block *competitor = (Query_cache_block *)hash_search(&queries, (byte *)thd->query, tot_length);
    DBUG_PRINT("qcache", ("competitor 0x%lx", (ulong)competitor));
    if (competitor == 0)
    {
      /* Query is not in cache and no one is working with it; Store it */
      Query_cache_block *query_block;
      query_block = write_block_data(tot_length, (gptr)thd->query, ALIGN_SIZE(sizeof(Query_cache_query)),
                                     Query_cache_block::QUERY, local_tables, 1);
      if (query_block != 0)
      {
        DBUG_PRINT("qcache", ("query block 0x%lx allocated, %lu", (ulong)query_block, query_block->used));
...
int Query_cache::send_result_to_client(THD *thd, char *sql, uint query_length)
{
  ...
  if (query_cache_size == 0 || thd->locked_tables || thd->variables.query_cache_type == 0) goto err;
  ...
  */
  if ((my_toupper(system_charset_info, sql[0]) != 'S' || my_toupper(system_charset_info, sql[1]) != 'E' ||
...
CS client: %u, CS result: %u, CS conn: %u, limit: %lu, TZ: 0x%lx, \
sql mode: 0x%lx, sort len: %lu, conncat len: %lu",
              (int)flags.client_long_flag, (int)flags.client_protocol_41, (int)flags.more_results_exists, flags.pkt_nr,
              flags.character_set_client_num, flags.character_set_results_num, flags.collation_connection_num,
              flags.limit, (ulong)flags.time_zone, flags.sql_mode, flags.max_sort_length, flags.group_concat_max_len));
  memcpy((void *)(sql + (tot_length - QUERY_CACHE_FLAGS_SIZE)), &flags, QUERY_CACHE_FLAGS_SIZE);
  query_block = (Query_cache_block *)hash_search(&queries, (byte *)sql, tot_length);
  /* Quick abort on unlocked data */
  if (query_block == 0 || query_block->query()->result() == 0 ||
  ...
  if ((thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) && (query->tables_type() & HA_CACHE_TBL_TRANSACT))
  {
    DBUG_PRINT("qcache", ("we are in transaction and have transaction tables in query"));
    BLOCK_UNLOCK_RD(query_block);
    goto err_unlock;
  }
  ...
  for (; block_table != block_table_end; block_table++)
  {
    ...
    */
    ...
    {
      if (tmptable->s->key_length - TMP_TABLE_KEY_EXTRA == table->key_length() &&
          !memcmp(tmptable->s->table_cache_key, table->data(), table->key_length()))
      {
        DBUG_PRINT("qcache", ("Temporary table detected: '%s.%s'", table_list.db, table_list.alias));
        STRUCT_UNLOCK(&structure_guard_mutex);
        /*
    ...
    {
      DBUG_PRINT("qcache", ("probably no SELECT access to %s.%s =>  return to normal processing", table_list.db,
                            table_list.alias));
      STRUCT_UNLOCK(&structure_guard_mutex);
    ...
    {
      DBUG_PRINT("qcache", ("Need to check column privileges for %s.%s", table_list.db, table_list.alias));
      BLOCK_UNLOCK_RD(query_block);
      thd->lex->safe_to_cache_query = 0;  // Don't try to cache this
    ...
    if (table->callback() && !(*table->callback())(thd, table->db(), table->key_length(), &engine_data))
    {
      DBUG_PRINT("qcache", ("Handler does not allow caching for %s.%s", table_list.db, table_list.alias));
      BLOCK_UNLOCK_RD(query_block);
      if (engine_data != table->engine_data())
      {
        DBUG_PRINT("qcache", ("Handler require invalidation queries of %s.%s %lld-%lld", table_list.db,
                              table_list.alias, engine_data, table->engine_data()));
      ...
      DBUG_PRINT("qcache", ("handler allow caching %s,%s", table_list.db, table_list.alias));
  }
...
*/
  ...
  do
  {
    DBUG_PRINT("qcache", ("Results  (len %lu, used %lu, headers %lu)", result_block->length, result_block->used,
                          result_block->headers_len() + ALIGN_SIZE(sizeof(Query_cache_result))));

    Query_cache_result *result = result_block->result();
    if (net_real_write(&thd->net, result->data(),
                       result_block->used - result_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_result))))
...
#else
  {
    Querycache_stream qs(result_block, result_block->headers_len() + ALIGN_SIZE(sizeof(Query_cache_result)));
...
void Query_cache::invalidate(THD *thd, TABLE_LIST *tables_used, my_bool using_transactions)
{
  ...
  if (query_cache_size > 0)
  {
    ...
    if (query_cache_size > 0)
    {
      ...
      using_transactions = using_transactions && (thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN));
      for (; tables_used; tables_used = tables_used->next_local)
      {
        DBUG_ASSERT(!using_transactions || tables_used->table != 0);
        if (tables_used->derived) continue;
  ...
  if (query_cache_size > 0 && tables_used)
  {
    ...
    if (query_cache_size > 0)
    {
      ...
      for (; tables_used; tables_used = tables_used->next)
      {
        ...
        DBUG_PRINT("qcache", (" db %s, table %s", tables_used->key, tables_used->key + strlen(tables_used->key) + 1));
      }
    }
    STRUCT_UNLOCK(&structure_guard_mutex);
  }
...
void Query_cache::invalidate_locked_for_write(TABLE_LIST *tables_used)
{
  ...
  if (query_cache_size > 0 && tables_used)
  {
    ...
    if (query_cache_size > 0)
    {
      ...
      for (; tables_used; tables_used = tables_used->next_local)
      {
        if (tables_used->lock_type & (TL_WRITE_LOW_PRIORITY | TL_WRITE) && tables_used->table)
...
void Query_cache::invalidate(THD *thd, TABLE *table, my_bool using_transactions)
{
...
void Query_cache::invalidate(THD *thd, const char *key, uint32 key_length, my_bool using_transactions)
{
  ...
  if (query_cache_size > 0)
  {
    ...
    {
      STRUCT_LOCK(&structure_guard_mutex);
      if (query_cache_size > 0) invalidate_table((byte *)key, key_length);
      STRUCT_UNLOCK(&structure_guard_mutex);
    }
  }
  DBUG_VOID_RETURN;
  ...
  if (query_cache_size > 0)
  {
    ...
    restart_search:
      if (tables_blocks)
      {
        ...
        {
          next = curr->next;
          if (strcmp(db, (char *)(curr->table()->db())) == 0) invalidate_table(curr);
          /*
          ...
          */
          if (next->type == Query_cache_block::FREE) goto restart_search;
          curr = next;
...
void Query_cache::invalidate_by_MyISAM_filename(const char *filename)
{
  ...
  if (query_cache_size > 0)
  {
    ...
    if (query_cache_size > 0)  // Safety if cache removed
    {
      ...
      if ((table_block = (Query_cache_block *)hash_search(&tables, (byte *)key, key_length)))
...
ulong Query_cache::init_cache()
{
  ...
  approx_additional_data_size = (sizeof(Query_cache) + sizeof(gptr) * (def_query_hash_size + def_table_hash_size));
  if (query_cache_size < approx_additional_data_size) goto err;
  ...
  */
  ...
  mem_bin_steps++;
  additional_data_size = ((mem_bin_num + 1) * ALIGN_SIZE(sizeof(Query_cache_memory_bin)) +
                          (mem_bin_steps * ALIGN_SIZE(sizeof(Query_cache_memory_bin_step))));

  if (query_cache_size < additional_data_size) goto err;
  query_cache_size -= additional_data_size;

  if (!(cache = (byte *)my_malloc_lock(query_cache_size + additional_data_size, MYF(0)))) goto err;

  DBUG_PRINT("qcache", ("cache length %lu, min unit %lu, %u bins", query_cache_size, min_allocation_unit, mem_bin_num));
  ...
  */
  ...
  VOID(hash_init(&queries, &my_charset_bin, def_query_hash_size, 0, 0, query_cache_query_get_key, 0, 0));
  ...
  */
  VOID(hash_init(&tables, &my_charset_bin, def_table_hash_size, 0, 0, query_cache_table_get_key, 0, 0));
#else
  /*
    ...
    comparison.
  */
  VOID(hash_init(&tables, lower_case_table_names ? &my_charset_bin : files_charset_info, def_table_hash_size, 0, 0,
                 query_cache_table_get_key, 0, 0));
#endif

  queries_in_cache = 0;
  queries_blocks = 0;
  DBUG_RETURN(query_cache_size + additional_data_size + approx_additional_data_size);
    ...
    */
    ...
    if (queries_blocks != 0)
    {
      ...
      do
      {
        ...
        if (header->result() != 0 && header->result()->type == Query_cache_block::RESULT &&
...
void Query_cache::free_query(Query_cache_block *query_block)
{
  ...
  DBUG_PRINT("qcache", ("free query 0x%lx %lu bytes result", (ulong)query_block, query_block->query()->length()));
...
Query_cache_block *Query_cache::write_block_data(ulong data_len, gptr data, ulong header_len,
                                                 Query_cache_block::block_type type, TABLE_COUNTER_TYPE ntab,
                                                 my_bool under_guard)
{
  ulong all_headers_len =
      (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(ntab * sizeof(Query_cache_block_table)) + header_len);
  ulong len = data_len + all_headers_len;
  ulong align_len = ALIGN_SIZE(len);
  DBUG_ENTER("Query_cache::write_block_data");
  DBUG_PRINT("qcache", ("data: %ld, header: %ld, all header: %ld", data_len, header_len, all_headers_len));
  Query_cache_block *block = allocate_block(max(align_len, min_allocation_unit), 1, 0, under_guard);
  if (block != 0)
  {
    block->type = type;
    block->n_tables = ntab;
    ...
    memcpy((void *)(((byte *)block) + all_headers_len), (void *)data, data_len);
  }
  DBUG_RETURN(block);
}
...
my_bool Query_cache::append_result_data(Query_cache_block **current_block, ulong data_len, gptr data,
                                        Query_cache_block *query_block)
{
  DBUG_ENTER("Query_cache::append_result_data");
  DBUG_PRINT("qcache", ("append %lu bytes to 0x%lx query", data_len, query_block));

  if (query_block->query()->add(data_len) > query_cache_limit)
  {
    DBUG_PRINT("qcache", ("size limit reached %lu > %lu", query_block->query()->length(), query_cache_limit));
    DBUG_RETURN(0);
  }
  if (*current_block == 0)
  {
    ...
    */
    DBUG_RETURN(write_result_data(current_block, data_len, data, query_block, Query_cache_block::RES_BEG));
  }
  Query_cache_block *last_block = (*current_block)->prev;

  DBUG_PRINT("qcache", ("lastblock 0x%lx len %lu used %lu", (ulong)last_block, last_block->length, last_block->used));
  my_bool success = 1;
  ...
  */
  ...
  {
    DBUG_PRINT("qcache", ("allocate new block for %lu bytes", data_len - last_block_free_space));
    ...
    */
    success =
        write_result_data(&new_block, data_len - last_block_free_space, (gptr)(((byte *)data) + last_block_free_space),
                          query_block, Query_cache_block::RES_CONT);
    /*
       new_block may be != 0 even !success (if write_result_data
       allocate a small block but failed to allocate continue)
    */
    if (new_block != 0) double_linked_list_join(last_block, new_block);
  }
  else
  {
  ...
  if (success && last_block_free_space > 0)
  {
    ...
    DBUG_PRINT("qcache",
               ("use free space %lub at block 0x%lx to copy %lub", last_block_free_space, (ulong)last_block, to_copy));
    memcpy((void *)(((byte *)last_block) + last_block->used), (void *)data, to_copy);
    last_block->used += to_copy;
  }
  DBUG_RETURN(success);
}

my_bool Query_cache::write_result_data(Query_cache_block **result_block, ulong data_len, gptr data,
                                       Query_cache_block *query_block, Query_cache_block::block_type type)
{
  ...
  */
  ...
  my_bool success = allocate_data_chain(result_block, data_len, query_block, type == Query_cache_block::RES_BEG);
  if (success)
  {
    // It is success (nobody can prevent us write data)
    STRUCT_UNLOCK(&structure_guard_mutex);
    uint headers_len = (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_result)));
#ifndef EMBEDDED_LIBRARY
    Query_cache_block *block = *result_block;
    byte *rest = (byte *)data;
    ...
    do
    {
      ...
      DBUG_PRINT("qcache", ("write %lu byte in block 0x%lx", length, (ulong)block));
...
{
  if (queries_in_cache < QUERY_CACHE_MIN_ESTIMATED_QUERIES_NUMBER) return min_result_data_size;
...
*/
my_bool Query_cache::allocate_data_chain(Query_cache_block **result_block, ulong data_len,
                                         Query_cache_block *query_block, my_bool first_block_arg)
{
  ulong all_headers_len = (ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_result)));
  ulong min_size = (first_block_arg ? get_min_first_result_data_size() : get_min_append_result_data_size());
  Query_cache_block *prev_block = NULL;
  Query_cache_block *new_block;
  DBUG_ENTER("Query_cache::allocate_data_chain");
  DBUG_PRINT("qcache", ("data_len %lu, all_headers_len %lu", data_len, all_headers_len));
  ...
  do
  {
    ...
    if (!(new_block = allocate_block(max(min_size, align_len), min_result_data_size == 0,
                                     all_headers_len + min_result_data_size, 1)))
    {
    ...
    DBUG_PRINT("qcache", ("Block len %lu used %lu", new_block->length, new_block->used));

    if (prev_block)
      double_linked_list_join(prev_block, new_block);
    else
      *result_block = new_block;
    if (new_block->length >= len) break;
...
void Query_cache::invalidate_table(TABLE_LIST *table_list)
{
  ...
  else
  {
    ...
    uint key_length;
    Query_cache_block *table_block;
    key_length = (uint)(strmov(strmov(key, table_list->db) + 1, table_list->table_name) - key) + 1;
...
{
  Query_cache_block *table_block;
  if ((table_block = ((Query_cache_block *)hash_search(&tables, key, key_length)))) invalidate_table(table_block);
}
...
TABLE_COUNTER_TYPE
Query_cache::register_tables_from_list(TABLE_LIST *tables_used, TABLE_COUNTER_TYPE counter,
                                       Query_cache_block_table *block_table)
{
  ...
  for (n = counter; tables_used; tables_used = tables_used->next_global, n++, block_table++)
  {
    ...
    if (tables_used->view)
    {
      char key[MAX_DBKEY_LENGTH];
      uint key_length;
      DBUG_PRINT("qcache", ("view %s, db %s", tables_used->view_name.str, tables_used->view_db.str));
      key_length = (uint)(strmov(strmov(key, tables_used->view_db.str) + 1, tables_used->view_name.str) - key) + 1;
      /*
        There are not callback function for for VIEWs
      */
      if (!insert_table(key_length, key, block_table, tables_used->view_db.length + 1, HA_CACHE_TBL_NONTRANSACT, 0, 0))
        DBUG_RETURN(0);
      {
        TABLE_COUNTER_TYPE inc = register_tables_from_list(tables_used->ancestor, n + 1, block_table + 1);
        if (!inc) DBUG_RETURN(0);
        n += inc;
        block_table += inc;
      }
    }
    ...
    {
      DBUG_PRINT("qcache",
                 ("table %s, db %s, openinfo at 0x%lx, keylen %u, key at 0x%lx", tables_used->table->s->table_name,
                  tables_used->table->s->table_cache_key, (ulong)tables_used->table, tables_used->table->s->key_length,
                  (ulong)tables_used->table->s->table_cache_key));
      if (!insert_table(tables_used->table->s->key_length, tables_used->table->s->table_cache_key, block_table,
                        tables_used->db_length, tables_used->table->file->table_cache_type(),
                        tables_used->callback_func, tables_used->engine_data))
        DBUG_RETURN(0);
      ...
      if (tables_used->table->s->db_type == DB_TYPE_MRG_MYISAM)
      {
        ...
        for (MYRG_TABLE *table = file->open_tables; table != file->end_table; table++)
        {
          char key[MAX_DBKEY_LENGTH];
          uint32 db_length;
          uint key_length = filename_2_table_key(key, table->table->filename, &db_length);
          (++block_table)->n = ++n;
          /*
            There are not callback function for for MyISAM, and engine data
          */
          if (!insert_table(key_length, key, block_table, db_length, tables_used->table->file->table_cache_type(), 0,
                            0))
            DBUG_RETURN(0);
        }
      }
...
my_bool Query_cache::register_all_tables(Query_cache_block *block, TABLE_LIST *tables_used,
                                         TABLE_COUNTER_TYPE tables_arg)
{
  TABLE_COUNTER_TYPE n;
  DBUG_PRINT("qcache", ("register tables block 0x%lx, n %d, header %x", (ulong)block, (int)tables_arg,
...
my_bool Query_cache::insert_table(uint key_len, char *key, Query_cache_block_table *node, uint32 db_length,
                                  uint8 cache_type, qc_engine_callback callback, ulonglong engine_data)
{
  DBUG_ENTER("Query_cache::insert_table");
  DBUG_PRINT("qcache", ("insert table node 0x%lx, len %d", (ulong)node, key_len));

  Query_cache_block *table_block = ((Query_cache_block *)hash_search(&tables, (byte *)key, key_len));

  if (table_block && table_block->table()->engine_data() != engine_data)
  {
    DBUG_PRINT("qcache", ("Handler require invalidation queries of %s.%s %lld-%lld", table_block->table()->db(),
                          table_block->table()->table(), engine_data, table_block->table()->engine_data()));
    /*
  ...
  if (table_block == 0)
  {
    DBUG_PRINT("qcache", ("new table block from 0x%lx (%u)", (ulong)key, (int)key_len));
    table_block =
        write_block_data(key_len, (gptr)key, ALIGN_SIZE(sizeof(Query_cache_table)), Query_cache_block::TABLE, 1, 1);
    if (table_block == 0)
    {
...
Query_cache_block *Query_cache::allocate_block(ulong len, my_bool not_less, ulong min, my_bool under_guard)
{
  DBUG_ENTER("Query_cache::allocate_block");
  DBUG_PRINT("qcache", ("len %lu, not less %d, min %lu, uder_guard %d", len, not_less, min, under_guard));

  if (len >= min(query_cache_size, query_cache_limit))
  {
    DBUG_PRINT("qcache", ("Query cache hase only %lu memory and limit %lu", query_cache_size, query_cache_limit));
  ...
  {
    if (block->length >= ALIGN_SIZE(len) + min_allocation_unit) split_block(block, ALIGN_SIZE(len));
  }
...
Query_cache_block *Query_cache::get_free_block(ulong len, my_bool not_less, ulong min)
{
  Query_cache_block *block = 0, *first = 0;
  DBUG_ENTER("Query_cache::get_free_block");
  DBUG_PRINT("qcache", ("length %lu, not_less %d, min %lu", len, (int)not_less, min));
  ...
  if (bins[start].number != 0)
  {
    ...
    if (list->prev->length >= len)  // check block with max size
    {
      first = list;
      uint n = 0;
      while (n < QUERY_CACHE_MEM_BIN_TRY && first->length < len)  // we don't need irst->next != list
      {
        first = first->next;
        n++;
      }
      ...
      else  // we don't need if (first->next != list)
      {
        n = 0;
        block = list->prev;
        while (n < QUERY_CACHE_MEM_BIN_TRY && block->length > len)
        {
          block = block->prev;
          n++;
        }
        if (block->length < len) block = block->next;
      }
    }
    else
      first = list->prev;
  ...
  if (block == 0 && start > 0)
  {
    ...
    if (bins[i].number > 0) block = bins[i].free_blocks;
  }
  ...
  if (block == 0 && !not_less)
  {
    ...
    else
    {
      ...
      for (i = start + 1; bins[i].number == 0; i++)
        ;
      if (i < mem_bin_num && bins[i].free_blocks->prev->length >= min) block = bins[i].free_blocks->prev;
    }
  }
  if (block != 0) exclude_from_free_memory_list(block);
...
void Query_cache::free_memory_block(Query_cache_block *block)
{
  ...
  DBUG_PRINT("qcache", ("first_block 0x%lx, block 0x%lx, pnext 0x%lx pprev 0x%lx", (ulong)first_block, (ulong)block,
                        (ulong)block->pnext, (ulong)block->pprev));

  if (block->pnext != first_block && block->pnext->is_free()) block = join_free_blocks(block, block->pnext);
  if (block != first_block && block->pprev->is_free()) block = join_free_blocks(block->pprev, block->pprev);
...
void Query_cache::split_block(Query_cache_block *block, ulong len)
{
  ...
  DBUG_PRINT("qcache", ("split 0x%lx (%lu) new 0x%lx", (ulong)block, len, (ulong)new_block));
  DBUG_VOID_RETURN;
}

Query_cache_block *Query_cache::join_free_blocks(Query_cache_block *first_block_arg, Query_cache_block *block_in_list)
{
  Query_cache_block *second_block;
  DBUG_ENTER("Query_cache::join_free_blocks");
  DBUG_PRINT("qcache", ("join first 0x%lx, pnext 0x%lx, in list 0x%lx", (ulong)first_block_arg,
                        (ulong)first_block_arg->pnext, (ulong)block_in_list));
...
my_bool Query_cache::append_next_free_block(Query_cache_block *block, ulong add_size)
{
  Query_cache_block *next_block = block->pnext;
  DBUG_ENTER("Query_cache::append_next_free_block");
  DBUG_PRINT("enter", ("block 0x%lx, add_size %lu", (ulong)block, add_size));
...
void Query_cache::exclude_from_free_memory_list(Query_cache_block *free_block)
{
  ...
  Query_cache_memory_bin *bin = *((Query_cache_memory_bin **)free_block->data());
  double_linked_list_exclude(free_block, &bin->free_blocks);
  bin->number--;
  free_memory -= free_block->length;
  free_memory_blocks--;
  DBUG_PRINT("qcache", ("exclude block 0x%lx, bin 0x%lx", (ulong)free_block, (ulong)bin));
  DBUG_VOID_RETURN;
}
...
void Query_cache::insert_into_free_memory_list(Query_cache_block *free_block)
{
  ...
  *bin_ptr = bins + idx;
  (*bin_ptr)->number++;
  DBUG_PRINT("qcache", ("insert block 0x%lx, bin[%d] 0x%lx", (ulong)free_block, idx, (ulong)*bin_ptr));
  DBUG_VOID_RETURN;
}
...
uint Query_cache::find_bin(ulong size)
{
  ...
  uint bin = steps[left].idx - (uint)((size - steps[left].size) / steps[left].increment);
#ifndef DBUG_OFF
  bins_dump();
#endif
  DBUG_PRINT("qcache", ("bin %u step %u, size %lu step size %lu", bin, left, size, steps[left].size));
  DBUG_RETURN(bin);
}
...
void Query_cache::insert_into_free_memory_sorted_list(Query_cache_block *new_block, Query_cache_block **list)
{
...
void Query_cache::double_linked_list_simple_include(Query_cache_block *point, Query_cache_block **list_pointer)
{
...
void Query_cache::double_linked_list_exclude(Query_cache_block *point, Query_cache_block **list_pointer)
{
  DBUG_ENTER("Query_cache::double_linked_list_exclude");
  DBUG_PRINT("qcache", ("excluding block 0x%lx, list 0x%lx", (ulong)point, (ulong)list_pointer));
  if (point->next == point)
...
void Query_cache::double_linked_list_join(Query_cache_block *head_tail, Query_cache_block *tail_head)
{
  Query_cache_block *head_head = head_tail->next, *tail_tail = tail_head->prev;
...
static TABLE_COUNTER_TYPE process_and_count_tables(TABLE_LIST *tables_used, uint8 *tables_type)
{
      ...
      *tables_type |= HA_CACHE_TBL_NONTRANSACT;
      {
        ...
        if (!(subcount = process_and_count_tables(tables_used->ancestor, tables_type))) DBUG_RETURN(0);
        table_count += subcount;
      }
    }
    else
    {
      DBUG_PRINT("qcache", ("table %s, db %s, type %u", tables_used->table->s->table_name,
                            tables_used->table->s->table_cache_key, tables_used->table->s->db_type));
      if (tables_used->derived)
      {
        table_count--;
        DBUG_PRINT("qcache", ("derived table skipped"));
      ...
      */
      if (tables_used->table->s->tmp_table != NO_TMP_TABLE || (*tables_type & HA_CACHE_TBL_NOCACHE) ||
          (tables_used->db_length == 5 &&
           my_strnncoll(table_alias_charset, (uchar *)tables_used->table->s->table_cache_key, 6, (uchar *)"mysql", 6) ==
               0))
      {
        DBUG_PRINT("qcache", ("select not cacheable: temporary, system or \
...
TABLE_COUNTER_TYPE Query_cache::is_cacheable(THD *thd, uint32 query_len, char *query, LEX *lex, TABLE_LIST *tables_used,
                                             uint8 *tables_type)
{
  ...
  if (lex->sql_command == SQLCOM_SELECT &&
      (thd->variables.query_cache_type == 1 ||
       (thd->variables.query_cache_type == 2 && (lex->select_lex.options & OPTION_TO_QUERY_CACHE))) &&
      lex->safe_to_cache_query)
  {
    DBUG_PRINT("qcache", ("options %lx %lx, type %u", OPTION_TO_QUERY_CACHE, lex->select_lex.options,
                          (int)thd->variables.query_cache_type));

    if (!(table_count = process_and_count_tables(tables_used, tables_type))) DBUG_RETURN(0);

    if ((thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) && ((*tables_type) & HA_CACHE_TBL_TRANSACT))
    {
  ...
  DBUG_PRINT("qcache", ("not interesting query: %d or not cacheable, options %lx %lx, type %u", (int)lex->sql_command,
                        OPTION_TO_QUERY_CACHE, lex->select_lex.options, (int)thd->variables.query_cache_type));
  DBUG_RETURN(0);
}
...
*/
my_bool Query_cache::ask_handler_allowance(THD *thd, TABLE_LIST *tables_used)
{
  ...
  for (; tables_used; tables_used = tables_used->next_global)
  {
    ...
    if (!handler->register_query_cache_table(thd, table->s->table_cache_key, table->s->key_length,
                                             &tables_used->callback_func, &tables_used->engine_data))
    {
      DBUG_PRINT("qcache", ("Handler does not allow caching for %s.%s", tables_used->db, tables_used->alias));
      thd->lex->safe_to_cache_query = 0;  // Don't try to cache this
      DBUG_RETURN(1);
    }
  }
...
my_bool Query_cache::move_by_type(byte **border, Query_cache_block **before, ulong *gap, Query_cache_block *block)
{
  ...
  switch (block->type)
  {
    ...
    case Query_cache_block::TABLE:
    {
      ...
      Query_cache_block_table *tprev = list_root->prev, *tnext = list_root->next;
      Query_cache_block *prev = block->prev, *next = block->next, *pprev = block->pprev, *pnext = block->pnext,
      ...
      if (tables_blocks == block) tables_blocks = new_block;
      ...
      DBUG_PRINT("qcache", ("list_root: 0x%lx tnext 0x%lx tprev 0x%lx "
                            "tprev->next 0x%lx tnext->prev 0x%lx",
                            (ulong)list_root, (ulong)tnext, (ulong)tprev, (ulong)tprev->next, (ulong)tnext->prev));
      /*
      ...
      *before = new_block;
      ...
      DBUG_PRINT("qcache", ("moved %lu bytes to 0x%lx, new gap at 0x%lx", len, (ulong)new_block, (ulong)*border));
      break;
    }
    case Query_cache_block::QUERY:
    {
      ...
      memmove((char *)new_block->data(), data, len - new_block->headers_len());
      relink(block, new_block, next, prev, pnext, pprev);
      if (queries_blocks == block) queries_blocks = new_block;
      Query_cache_block_table *beg_of_table_table = block->table(0), *end_of_table_table = block->table(n_tables);
      ...
      for (TABLE_COUNTER_TYPE j = 0; j < n_tables; j++)
      {
        ...
        if ((beg_of_table_table <= block_table->next) && (block_table->next < end_of_table_table))
          ((Query_cache_block_table *)(beg_of_new_table_table +
                                       (((byte *)block_table->next) - ((byte *)beg_of_table_table))))
              ->prev = block_table;
        else
          block_table->next->prev = block_table;

        // use aligment from begining of table if 'prev' is in same block
        if ((beg_of_table_table <= block_table->prev) && (block_table->prev < end_of_table_table))
          ((Query_cache_block_table *)(beg_of_new_table_table +
                                       (((byte *)block_table->prev) - ((byte *)beg_of_table_table))))
              ->next = block_table;
        else
    ...
    {
      DBUG_PRINT("qcache", ("block 0x%lx RES* (%d)", (ulong)block, (int)block->type));
      if (*border == 0) break;
      Query_cache_block *query_block = block->result()->parent(), *next = block->next, *prev = block->prev;
      Query_cache_block::block_type type = block->type;
      BLOCK_LOCK_WR(query_block);
      ulong len = block->length, used = block->used;
      Query_cache_block *pprev = block->pprev, *pnext = block->pnext, *new_block = (Query_cache_block *)*border;
      char *data = (char *)block->data();
      block->destroy();
      new_block->init(len);
      new_block->type = type;
      ...
      Query_cache_query *query = query_block->query();
      if (query->result() == block) query->result(new_block);
      *border += len;
      *before = new_block;
      ...
      if (query->result()->type == Query_cache_block::RESULT && new_block->length > new_block->used &&
          *gap + free_space > min_allocation_unit && new_block->length - free_space > min_allocation_unit)
      {
        *border -= free_space;
        *gap += free_space;
        DBUG_PRINT("qcache", ("rest of result free space added to gap (%lu)", *gap));
    ...
    default:
      DBUG_PRINT("error", ("unexpected block type %d, block 0x%lx", (int)block->type, (ulong)block));
      ok = 0;
  }
  DBUG_RETURN(ok);
}

void Query_cache::relink(Query_cache_block *oblock, Query_cache_block *nblock, Query_cache_block *next,
                         Query_cache_block *prev, Query_cache_block *pnext, Query_cache_block *pprev)
{
...
my_bool Query_cache::join_results(ulong join_limit)
{
  ...
  if (queries_blocks != 0)
  {
    ...
    {
      Query_cache_query *header = block->query();
      if (header->result() != 0 && header->result()->type == Query_cache_block::RESULT && header->length() > join_limit)
      {
        Query_cache_block *new_result_block =
            get_free_block(ALIGN_SIZE(header->length()) + ALIGN_SIZE(sizeof(Query_cache_block)) +
        ...
        if (new_result_block != 0)
        {
          ...
          ulong new_len =
              (header->length() + ALIGN_SIZE(sizeof(Query_cache_block)) + ALIGN_SIZE(sizeof(Query_cache_result)));
          if (new_result_block->length > ALIGN_SIZE(new_len) + min_allocation_unit)
          ...
          DBUG_PRINT("qcache",
                     ("new block %lu/%lu (%lu)", new_result_block->length, new_result_block->used, header->length()));
          ...
          do
          {
            ulong len = (result_block->used - result_block->headers_len() - ALIGN_SIZE(sizeof(Query_cache_result)));
            DBUG_PRINT("loop", ("add block %lu/%lu (%lu)", result_block->length, result_block->used, len));
...
uint Query_cache::filename_2_table_key(char *key, const char *path, uint32 *db_length)
{
  ...
  *db_length = (filename - dbname) - 1;
  ...
  DBUG_RETURN((uint)(strmov(strmake(key, dbname, *db_length) + 1, filename) - key) + 1);
}
...
void Query_cache::wreck(uint line, const char *message)
{
  ...
  query_cache_size = 0;
  if (*message) DBUG_PRINT("error", (" %s", message));
  DBUG_PRINT("warning", ("=================================="));
...
void Query_cache::bins_dump()
{
  ...
  DBUG_PRINT("qcache", ("mem_bin_num=%u, mem_bin_steps=%u", mem_bin_num, mem_bin_steps));
  DBUG_PRINT("qcache", ("-------------------------"));
  DBUG_PRINT("qcache", ("      size idx       step"));
  DBUG_PRINT("qcache", ("-------------------------"));
  for (i = 0; i < mem_bin_steps; i++)
  {
    DBUG_PRINT("qcache", ("%10lu %3d %10lu", steps[i].size, steps[i].idx, steps[i].increment));
  }
  DBUG_PRINT("qcache", ("-------------------------"));
  ...
  {
    DBUG_PRINT("qcache", ("%10lu %3d 0x%lx", bins[i].size, bins[i].number, (ulong) & (bins[i])));
    if (bins[i].free_blocks)
    {
      Query_cache_block *block = bins[i].free_blocks;
      do
      {
        DBUG_PRINT("qcache", ("\\-- %lu 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", block->length, (ulong)block, (ulong)block->next,
                              (ulong)block->prev, (ulong)block->pnext, (ulong)block->pprev));
        block = block->next;
...
void Query_cache::cache_dump()
{
  ...
  do
  {
    DBUG_PRINT("qcache", ("%10lu %10lu %1d %2d 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", i->length, i->used, (int)i->type,
                          i->n_tables, (ulong)i, (ulong)i->next, (ulong)i->prev, (ulong)i->pnext, (ulong)i->pprev));
    i = i->pnext;
  } while (i != first_block);
  DBUG_PRINT("qcache", ("-------------------------------------"));
}
...
void Query_cache::queries_dump()
{
  ...
  if (queries_blocks != 0)
  {
    ...
    do
    {
      ...
      DBUG_PRINT("qcache",
                 ("F:%u C:%u L:%lu T:'%s' (%u) '%s' '%s'", flags.client_long_flag, flags.character_set_client_num,
                  (ulong)flags.limit, flags.time_zone->get_name(), len, str, strend(str) + 1));
      DBUG_PRINT("qcache", ("-b- 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", (ulong)block, (ulong)block->next, (ulong)block->prev,
                            (ulong)block->pnext, (ulong)block->pprev));
      ...
      if (header->result())
      {
        ...
        {
          DBUG_PRINT("qcache",
                     ("-r- %u %lu/%lu 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", (uint)result_block->type, result_block->length,
                      result_block->used, (ulong)result_block, (ulong)result_block->next, (ulong)result_block->prev,
                      (ulong)result_block->pnext, (ulong)result_block->pprev));
          result_block = result_block->next;
...
my_bool Query_cache::check_integrity(bool not_locked)
{
  ...
  do
  {
    DBUG_PRINT("qcache", ("block 0x%lx, type %u...", (ulong)block, (uint)block->type));
    // Check allignment
    if ((((long)block) % (long)ALIGN_SIZE(1)) != (((long)first_block) % (long)ALIGN_SIZE(1)))
    {
      DBUG_PRINT("error", ("block 0x%lx do not aligned by %d", (ulong)block, ALIGN_SIZE(1)));
      result = 1;
    }
    ...
    if (block->pnext == first_block)  // Is it last block?
    {
      ...
      {
        DBUG_PRINT("error",
                   ("block 0x%lx, type %u, ended at 0x%lx, but cache ended at 0x%lx", (ulong)block, (uint)block->type,
                    (ulong)(((byte *)block) + block->length), (ulong)(((byte *)first_block) + query_cache_size)));
        result = 1;
      }
    }
    else if (((byte *)block) + block->length != ((byte *)block->pnext))
    {
      DBUG_PRINT("error", ("block 0x%lx, type %u, ended at 0x%lx, but next "
                           "block begining at 0x%lx",
                           (ulong)block, (uint)block->type, (ulong)(((byte *)block) + block->length),
    ...
    switch (block->type)
    {
      case Query_cache_block::FREE:
      {
        Query_cache_memory_bin *bin = *((Query_cache_memory_bin **)block->data());
        // is it correct pointer?
        if (((byte *)bin) < ((byte *)bins) || ((byte *)bin) >= ((byte *)first_block))
        {
          DBUG_PRINT("error", ("free block 0x%lx have bin pointer 0x%lx beyaond of bins "
                               "array bounds [0x%lx,0x%lx]",
                               (ulong)block, (ulong)bin, (ulong)bins, (ulong)first_block));
          result = 1;
        }
        else
        {
          int idx = (((byte *)bin) - ((byte *)bins)) / sizeof(Query_cache_memory_bin);
          if (in_list(bins[idx].free_blocks, block, "free memory")) result = 1;
        }
        break;
      }
      case Query_cache_block::TABLE:
        if (in_list(tables_blocks, block, "tables")) result = 1;
        if (in_table_list(block->table(0), block->table(0), "table list root")) result = 1;
        break;
      case Query_cache_block::QUERY:
      {
        if (in_list(queries_blocks, block, "query")) result = 1;
        for (TABLE_COUNTER_TYPE j = 0; j < block->n_tables; j++)
        {
          ...
          Query_cache_block_table *block_table_root =
              (Query_cache_block_table *)(((byte *)block_table->parent) - ALIGN_SIZE(sizeof(Query_cache_block_table)));

          if (in_table_list(block_table, block_table_root, "table list")) result = 1;
        }
        break;
      }
      case Query_cache_block::RES_INCOMPLETE:
      ...
      case Query_cache_block::RESULT:
      {
        ...
        {
          DBUG_PRINT("error", ("result block 0x%lx have query block pointer 0x%lx beyaond "
                               "of block pool bounds [0x%lx,0x%lx]",
                               (ulong)block, (ulong)query_block, (ulong)first_block,
        ...
        {
          BLOCK_LOCK_RD(query_block);
          if (in_list(queries_blocks, query_block, "query from results")) result = 1;
          if (in_list(query_block->query()->result(), block, "results")) result = 1;
          BLOCK_UNLOCK_RD(query_block);
        }
        break;
      }
      default:
        DBUG_PRINT("error", ("block 0x%lx have incorrect type %u", block, block->type));
        result = 1;
    }
  ...
  {
    DBUG_PRINT("error",
               ("used memory (%lu) + free memory (%lu) !=  query_cache_size (%lu)", used, free, query_cache_size));
    result = 1;
  }

  if (free != free_memory)
  {
    DBUG_PRINT("error", ("free memory (%lu) != free_memory (%lu)", free, free_memory));
    result = 1;
  }
  ...
  if ((block = queries_blocks))
  {
    do
    {
      ...
      if (((gptr)block) != val)
      {
        DBUG_PRINT("error", ("block 0x%lx found in queries hash like 0x%lx", (ulong)block, (ulong)val));
      }
      if (in_blocks(block)) result = 1;
      ...
      if (results)
      {
        ...
        {
          DBUG_PRINT("qcache", ("block 0x%lx, type %u...", (ulong)block, (uint)block->type));
          if (in_blocks(result_block)) result = 1;
  ...
  if ((block = tables_blocks))
  {
    do
    {
      ...
      if (((gptr)block) != val)
      {
        DBUG_PRINT("error", ("block 0x%lx found in tables hash like 0x%lx", (ulong)block, (ulong)val));
      }
  ...
  for (i = 0; i < mem_bin_num; i++)
  {
    if ((block = bins[i].free_blocks))
    {
      ...
      {
        DBUG_PRINT("error", ("bin[%d].number is %d, but bin have %d blocks", bins[i].number, count));
        result = 1;
      }
    }
  }
  DBUG_ASSERT(result == 0);
  if (!not_locked) STRUCT_UNLOCK(&structure_guard_mutex);
  DBUG_RETURN(result);
}
...
my_bool Query_cache::in_blocks(Query_cache_block *point)
{
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("block 0x%lx in physical list is incorrect linked, prev "
                           "block 0x%lx refered as next to 0x%lx (check from 0x%lx)",
  ...
  {
    DBUG_PRINT("error", ("block 0x%lx (0x%lx<-->0x%lx) not owned by pysical list", (ulong)block, (ulong)block->pprev,
                         (ulong)block->pnext));
    return 1;
  }
...
err1:
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("block 0x%lx in physicel list is incorrect linked, next "
                           "block 0x%lx refered as prev to 0x%lx (check from 0x%lx)",
...
my_bool Query_cache::in_list(Query_cache_block *root, Query_cache_block *point, const char *name)
{
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error",
                 ("block 0x%lx in list '%s' 0x%lx is incorrect linked, prev "
                  "block 0x%lx refered as next to 0x%lx (check from 0x%lx)",
  ...
  if (block != root)
  {
    DBUG_PRINT("error", ("block 0x%lx (0x%lx<-->0x%lx) not owned by list '%s' 0x%lx", (ulong)block, (ulong)block->prev,
                         (ulong)block->next, name, (ulong)root));
    return 1;
  }
err1:
  // forward
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error",
                 ("block 0x%lx in list '%s' 0x%lx is incorrect linked, next "
                  "block 0x%lx refered as prev to 0x%lx (check from 0x%lx)",
...
void dump_node(Query_cache_block_table *node, const char *call, const char *descr)
{
  DBUG_PRINT("qcache", ("%s: %s: node: 0x%lx", call, descr, (ulong)node));
  DBUG_PRINT("qcache", ("%s: %s: node block: 0x%lx", call, descr, (ulong)node->block()));
  DBUG_PRINT("qcache", ("%s: %s: next: 0x%lx", call, descr, (ulong)node->next));
  DBUG_PRINT("qcache", ("%s: %s: prev: 0x%lx", call, descr, (ulong)node->prev));
}

my_bool Query_cache::in_table_list(Query_cache_block_table *root, Query_cache_block_table *point, const char *name)
{
  my_bool result = 0;
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("table 0x%lx(0x%lx) in list '%s' 0x%lx(0x%lx) is incorrect "
                           "linked, prev table 0x%lx(0x%lx) refered as next to "
                           "0x%lx(0x%lx) (check from 0x%lx(0x%lx))",
                           (ulong)table, (ulong)table->block(), name, (ulong)root, (ulong)root->block(),
                           (ulong)table->prev, (ulong)table->prev->block(), (ulong)table->prev->next,
                           (ulong)table->prev->next->block(), (ulong)point, (ulong)point->block()));
      // back trace
      for (; table != point; table = table->next)
        DBUG_PRINT("error", ("back trace 0x%lx(0x%lx)", (ulong)table, (ulong)table->block()));
      result = 1;
      goto err1;
    }
    table = table->prev;
  ...
  if (table != root)
  {
    DBUG_PRINT("error", ("table 0x%lx(0x%lx) (0x%lx(0x%lx)<-->0x%lx(0x%lx)) "
                         "not owned by list '%s' 0x%lx(0x%lx)",
                         (ulong)table, (ulong)table->block(), (ulong)table->prev, (ulong)table->prev->block(),
                         (ulong)table->next, (ulong)table->next->block(), name, (ulong)root, (ulong)root->block()));
    return 1;
  }
err1:
  // forward
  ...
  do
  {
    ...
    {
      DBUG_PRINT("error", ("table 0x%lx(0x%lx) in list '%s' 0x%lx(0x%lx) is incorrect "
                           "linked, next table 0x%lx(0x%lx) refered as prev to "
                           "0x%lx(0x%lx) (check from 0x%lx(0x%lx))",
                           (ulong)table, (ulong)table->block(), name, (ulong)root, (ulong)root->block(),
                           (ulong)table->next, (ulong)table->next->block(), (ulong)table->next->prev,
                           (ulong)table->next->prev->block(), (ulong)point, (ulong)point->block()));
      // back trace
