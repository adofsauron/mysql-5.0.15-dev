...
static void sync_cell_event_set(
    /*================*/
...
static void sync_cell_event_reset(
    /*==================*/
    sync_cell_t *cell) /* in: array cell */
{
    ...
    sync_cell_t *cell) /* in: sync cell */
{
...
#endif /* UNIV_SYNC_DEBUG */
            "waiters flag %lu\n",
            mutex, mutex->cfile_name, (ulong)mutex->cline, (ulong)mutex->lock_word,
#ifdef UNIV_SYNC_DEBUG
  ...
  else if (type == RW_LOCK_EX || type == RW_LOCK_SHARED)
  {
    ...
    fprintf(file, " RW-latch at %p created in file %s line %lu\n", rwlock, rwlock->cfile_name, (ulong)rwlock->cline);
    if (rwlock->writer != RW_LOCK_NOT_LOCKED)
    {
      fprintf(file, "a writer (thread id %lu) has reserved it in mode %s", (ulong)os_thread_pf(rwlock->writer_thread),
              rwlock->writer == RW_LOCK_EX ? " exclusive\n" : " wait exclusive\n");
    }
    ...
    ulint depth)        /* in: recursion depth */
{
  ...
  if (cell->request_type == SYNC_MUTEX)
  {
    ...
    if (mutex_get_lock_word(mutex) != 0)
    {
      ...
      if (ret)
      {
        fprintf(stderr, "Mutex %p owned by thread %lu file %s line %lu\n", mutex, (ulong)os_thread_pf(mutex->thread_id),
                mutex->file_name, (ulong)mutex->line);
...
was exceeded */
{
  ...
  for (i = 0; i < sync_primary_wait_array->n_cells; i++)
  {
    ...
    if (cell->wait_object != NULL && difftime(time(NULL), cell->reservation_time) > 240)
    {
    ...
    if (cell->wait_object != NULL && difftime(time(NULL), cell->reservation_time) > fatal_timeout)
    {
      fatal = TRUE;
    }
  }
  ...
  if (noticed)
  {
    ...
    fprintf(stderr, "InnoDB: ###### Diagnostic info printed to the standard error stream\n");
  }
