...
extern void innobase_invalidate_query_cache(
    /*============================*/
...
static void row_ins_alloc_sys_fields(
    /*=====================*/
    ...
    mtr_t *mtr)        /* in: mtr */
{
  ...
  else
  {
    ...
    err = btr_cur_pessimistic_update(BTR_KEEP_SYS_FLAG, cursor, &dummy_big_rec, update, 0, thr, mtr);
  }
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  *big_rec = NULL;
  ...
  update = row_upd_build_difference_binary(cursor->index, entry, ext_vec, n_ext_vec, rec, thr_get_trx(thr), heap);
  if (mode == BTR_MODIFY_LEAF)
  {
...
static ulint row_ins_cascade_n_ancestors(
    /*========================*/
  ...
  for (i = 0; i < foreign->n_fields; i++)
  {
    ...
    for (j = 0; j < parent_update->n_fields; j++)
    {
      ...
      if (parent_ufield->field_no == parent_field_no)
      {
        ...
        if (ufield->new_val.len == UNIV_SQL_NULL && (type->prtype & DATA_NOT_NULL))
        {
        ...
        if (ufield->new_val.len != UNIV_SQL_NULL &&
            dtype_get_at_most_n_mbchars(type, dtype_get_len(type), ufield->new_val.len, ufield->new_val.data) <
                ufield->new_val.len)
        {
          return (ULINT_UNDEFINED);
        }
        ...
        if (min_size && ufield->new_val.len != UNIV_SQL_NULL && ufield->new_val.len < min_size)
        {
          char *pad_start;
          const char *pad_end;
          ufield->new_val.data = mem_heap_alloc(heap, min_size);
          pad_start = ((char *)ufield->new_val.data) + ufield->new_val.len;
          pad_end = ((char *)ufield->new_val.data) + min_size;
          ufield->new_val.len = min_size;
          ut_memcpy(ufield->new_val.data, parent_ufield->new_val.data, parent_ufield->new_val.len);

          switch (UNIV_EXPECT(dtype_get_mbminlen(type), 1))
          {
            default:
              ut_error;
            case 1:
    ...
    dict_foreign_t *foreign) /* in: foreign key constraint */
{
  ...
  if (tf)
  {
    ...
    dict_print_info_on_foreign_key_in_create_format(tf, trx, foreign, FALSE);
...
static void row_ins_invalidate_query_cache(
    /*===========================*/
    que_thr_t *thr,   /* in: query thread whose run_node
                      is an update node */
                             ...
                             page */
{
  ...
  else
  {
    ...
    if (!page_rec_is_user_rec(clust_rec) ||
        btr_pcur_get_low_match(cascade->pcur) < dict_index_get_n_unique(clust_index))
    {
      fputs(
  ...
  if (err == DB_SUCCESS)
  {
    ...
    err = lock_clust_rec_read_check_and_lock_alt(0, clust_rec, clust_index, LOCK_X, LOCK_REC_NOT_GAP, thr);
  }
    ...
    que_thr_t *thr)       /* in: query thread */
{
  ...
  if (index->type & DICT_CLUSTERED)
  {
    err = lock_clust_rec_read_check_and_lock(0, rec, index, offsets, LOCK_S, type, thr);
  }
  else
  {
    err = lock_sec_rec_read_check_and_lock(0, rec, index, offsets, LOCK_S, type, thr);
  }
    ...
    que_thr_t *thr)       /* in: query thread */
{
  ...
  if (index->type & DICT_CLUSTERED)
  {
    err = lock_clust_rec_read_check_and_lock(0, rec, index, offsets, LOCK_X, type, thr);
  }
  else
  {
    err = lock_sec_rec_read_check_and_lock(0, rec, index, offsets, LOCK_X, type, thr);
  }
...
run_again:
  ...
  for (;;)
  {
    ...
    {
      err = row_ins_set_shared_rec_lock(LOCK_ORDINARY, rec, check_index, offsets, thr);
      if (err != DB_SUCCESS)
      {
        break;
      }
    ...
    if (cmp == 0)
    {
      ...
      else
      {
        ...
        err = row_ins_set_shared_rec_lock(LOCK_REC_NOT_GAP, rec, check_index, offsets, thr);
    ...
    if (cmp < 0)
    {
      err = row_ins_set_shared_rec_lock(LOCK_GAP, rec, check_index, offsets, thr);
      if (err != DB_SUCCESS)
      {
        break;
      }
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  while (foreign)
  {
    if (foreign->foreign_index == index)
    {
      ...
      {
        dict_table_get(foreign->referenced_table_name, trx);
      }
    ...
    const ulint *offsets) /* in: rec_get_offsets(rec, index) */
{
  ...
  cmp_dtuple_rec_with_match(entry, rec, offsets, &matched_fields, &matched_bytes);
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
  ...
  for (;;)
  {
    ...
    if (innobase_query_is_update())
    {
      ...
      err = row_ins_set_exclusive_rec_lock(LOCK_ORDINARY, rec, index, offsets, thr);
    }
    else
    {
      err = row_ins_set_shared_rec_lock(LOCK_ORDINARY, rec, index, offsets, thr);
    }
...
static ulint row_ins_duplicate_error_in_clust(
    /*=============================*/
  ...
  if (cursor->low_match >= n_unique)
  {
    ...
    if (!page_rec_is_infimum(rec))
    {
      ...
      if (innobase_query_is_update())
      {
        ...
        err = row_ins_set_exclusive_rec_lock(LOCK_REC_NOT_GAP, rec, cursor->index, offsets, thr);
      }
      else
      {
        err = row_ins_set_shared_rec_lock(LOCK_REC_NOT_GAP, rec, cursor->index, offsets, thr);
      }
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
...
#ifdef UNIV_DEBUG
  {
    ...
    {
      ut_a(rec_get_n_fields(first_rec, index) == dtuple_get_n_fields(entry));
    }
  }
#endif
  ...
  else
  {
    ...
    if (err == DB_SUCCESS)
    {
      if (ext_vec)
      {
        rec_set_field_extern_bits(insert_rec, index, ext_vec, n_ext_vec, &mtr);
      }
    }
  }
...
function_exit:
  ...
  if (big_rec)
  {
    ...
    err = btr_store_big_rec_extern_fields(index, rec, offsets, big_rec, &mtr);
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  err = row_ins_index_entry_low(BTR_MODIFY_TREE, index, entry, ext_vec, n_ext_vec, thr);
  return (err);
}
    ...
    dtuple_t *row)       /* in: row */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    {
      cur_type = dict_col_get_type(dict_field_get_col(ind_field));

      field->len =
          dtype_get_at_most_n_mbchars(cur_type, ind_field->prefix_len, dfield_get_len(row_field), row_field->data);
    }
    else
    {
...
UNIV_INLINE
void row_ins_alloc_row_id_step(
    /*======================*/
    ins_node_t *node) /* in: row insert node */
{
  dulint row_id;
...
UNIV_INLINE
void row_ins_get_row_from_values(
    /*========================*/
    ins_node_t *node) /* in: row insert node */
{
