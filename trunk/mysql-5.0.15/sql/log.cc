...
int check_binlog_magic(IO_CACHE *log, const char **errmsg) {
  ...
  if (memcmp(magic, BINLOG_MAGIC, sizeof(magic))) {
    *errmsg = "Binlog has bad magic number;  It's not a binary log file that "
...
File open_binlog(IO_CACHE *log, const char *log_file_name,
  ...
  if ((file = my_open(log_file_name, O_RDONLY | O_BINARY | O_SHARE,
                      MYF(MY_WME))) < 0) {
    sql_print_error("Failed to open log (file '%s', errno %d)", log_file_name,
                    my_errno);
...
static int find_uniq_filename(char *name) {
  ...
  *end = '.';
  ...
  if (!(dir_info = my_dir(buff, MYF(MY_DONT_SORT)))) { // This shouldn't happen
...
MYSQL_LOG::MYSQL_LOG()
    : bytes_written(0), last_time(0), query_start(0), name(0), file_id(1),
      open_count(1), log_type(LOG_CLOSED), write_error(0), inited(0),
...
void MYSQL_LOG::init(enum_log_type log_type_arg,
                     enum cache_type io_cache_type_arg, bool no_auto_events_arg,
                     ulong max_size_arg) {
...
const char *MYSQL_LOG::generate_name(const char *log_name, const char *suffix,
...
bool MYSQL_LOG::open_index_file(const char *index_file_name_arg,
  ...
  */
  ...
  fn_format(index_file_name, index_file_name_arg, mysql_data_home, ".index",
            opt);
  if ((index_file_nr = my_open(index_file_name, O_RDWR | O_CREAT | O_BINARY,
                               MYF(MY_WME))) < 0 ||
      my_sync(index_file_nr, MYF(MY_WME)) ||
      init_io_cache(&index_file, index_file_nr, IO_SIZE, WRITE_CACHE,
                    my_seek(index_file_nr, 0L, MY_SEEK_END, MYF(0)), 0,
...
bool MYSQL_LOG::open(const char *log_name, enum_log_type log_type_arg,
                     ...
                     bool no_auto_events_arg, ulong max_size_arg,
  ...
  db[0] = 0;
  open_count++;
  if ((file = my_open(log_file_name, open_flags, MYF(MY_WME | ME_WAITTANG))) <
          0 ||
  ...
  case LOG_NEW: {
    ...
    len = my_snprintf(
        buff, sizeof(buff), "# %s, Version: %s at %02d%02d%02d %2d:%02d:%02d\n",
        my_progname, server_version, tm_tmp.tm_year % 100, tm_tmp.tm_mon + 1,
  ...
  case LOG_BIN: {
    ...
    reinit_io_cache(&index_file, WRITE_CACHE, my_b_filelength(&index_file), 0,
                    0);
    ...
    if (write_file_name_to_index_file) {
      ...
      */
      if (my_b_write(&index_file, (byte *)log_file_name,
          ...
          flush_io_cache(&index_file) || my_sync(index_file.file, MYF(MY_WME)))
        goto err;
    }
    break;
  }
...
bool MYSQL_LOG::reset_logs(THD *thd) {
  ...
  */
  ...
  if (!open_index_file(index_file_name, 0))
    open(save_name, save_log_type, 0, io_cache_type, no_auto_events, max_size,
         0);
...
int MYSQL_LOG::purge_first_log(struct st_relay_log_info *rli, bool included) {
  ...
  rli->relay_log.purge_logs(rli->group_relay_log_name, included, 0, 0,
                            &rli->log_space_total);
  ...
  */
  if ((included && (error = find_log_pos(&rli->linfo, NullS, 0))) ||
    ...
    sql_print_error("next log error: %d  offset: %s  log: %s included: %d",
                    error, llstr(rli->linfo.index_file_offset, buff),
...
int MYSQL_LOG::purge_logs(const char *to_log, bool included, bool need_mutex,
...
void MYSQL_LOG::new_file(bool need_lock) {
  ...
  if (log_type == LOG_BIN) {
    if (!no_auto_events) {
      ...
      */
      ...
      Rotate_log_event r(thd, new_name + dirname_length(new_name), 0,
                         LOG_EVENT_OFFSET, 0);
      r.write(&log_file);
  ...
  */
  ...
  open(old_name, save_log_type, new_name_ptr, io_cache_type, no_auto_events,
       max_size, 1);
  ...
  if (is_open() && (what_to_log & (1L << (uint)command))) {
    ...
    if (is_open()) {
      ...
      if (skr != last_time) {
        ...
        sprintf(buff, "%02d%02d%02d %2d:%02d:%02d\t", start->tm_year % 100,
                start->tm_mon + 1, start->tm_mday, start->tm_hour,
        ...
        error = errno;
      length = my_sprintf(
          buff, (buff, "%7ld %-11.11s", id, command_name[(uint)command]));
    ...
    */
    if (opt_using_transactions && thd) {
      ...
      if (event_info->get_cache_stmt()) {
        if (!trans_log) {
          thd->ha_data[binlog_hton.slot] = trans_log =
              (IO_CACHE *)my_malloc(sizeof(IO_CACHE), MYF(MY_ZEROFILL));
          if (!trans_log ||
              open_cached_file(trans_log, mysql_tmpdir, LOG_PREFIX,
          ...
          trans_log->end_of_file = max_binlog_cache_size;
          trans_register_ha(thd, thd->options &
...
bool MYSQL_LOG::write(THD *thd, const char *query, uint query_length,
  ...
  VOID(pthread_mutex_lock(&LOCK_log));
  if (is_open()) { // Safety agains reopen
    int tmp_errno = 0;
    char buff[80], *end;
    end = buff;
    ...
    if (!(specialflag & SPECIAL_SHORT_LOG_FORMAT) || query_start_arg) {
      ...
      if (my_b_printf(&log_file, "# User@Host: %s[%s] @ %s [%s]\n",
                      ...
                      sctx->user ? sctx->user : "",
                      sctx->host ? sctx->host : "",
                      sctx->ip ? sctx->ip : "") == (uint) - 1)
        tmp_errno = errno;
    }
    if (query_start_arg) {
      /* For slow query log */
      if (my_b_printf(&log_file, "# Query_time: %lu  Lock_time: %lu  "
    ...
    if (thd->db && strcmp(thd->db, db)) { // Database changed
...
void MYSQL_LOG::close(uint exiting) { // One can't set log_type here!
...
static bool test_if_number(register const char *str, long *res,
...
void print_buffer_to_file(enum loglevel level, const char *buffer) {
  ...
  fprintf(stderr, "%02d%02d%02d %2d:%02d:%02d [%s] %s\n", start->tm_year % 100,
          start->tm_mon + 1, start->tm_mday, start->tm_hour, start->tm_min,
          start->tm_sec,
          (level == ERROR_LEVEL ? "ERROR" : level == WARNING_LEVEL ? "Warning"
                                                                   : "Note"),
          buffer);
...
#ifdef __NT__
void print_buffer_to_nt_eventlog(enum loglevel level, char *buff, uint length,
                                 int buffLen) {
  HANDLE event;
  char *buffptr;
  LPCSTR *buffmsgptr;
  ...
  if ((event = RegisterEventSource(NULL, "MySQL"))) {
    ...
    case INFORMATION_LEVEL:
      ReportEvent(event, EVENTLOG_INFORMATION_TYPE, 0, MSG_DEFAULT, NULL, 1, 0,
                  buffmsgptr, NULL);
      break;
    }
...
int TC_LOG_MMAP::open(const char *opt_name) {
  ...
  if (!(pages = (PAGE *)my_malloc(npages * sizeof(PAGE),
...
int TC_LOG_MMAP::recover() {
  ...
  */
  ...
  if (hash_init(&xids, &my_charset_bin, tc_log_page_size / 3, 0, sizeof(my_xid),
                0, 0, MYF(0)))
    goto err1;
...
int TC_LOG_BINLOG::recover(IO_CACHE *log, Format_description_log_event *fdle) {
  ...
  if (!fdle->is_valid() ||
      hash_init(&xids, &my_charset_bin, TC_LOG_PAGE_SIZE / 3, 0, sizeof(my_xid),
                0, 0, MYF(0)))
    goto err1;
  ...
  while ((ev = Log_event::read_log_event(log, 0, fdle)) && ev->is_valid()) {
    if (ev->get_type_code() == XID_EVENT) {
      ...
      byte *x =
          (byte *)memdup_root(&mem_root, (char *)&xev->xid, sizeof(xev->xid));
      if (!x)
        goto err2;
