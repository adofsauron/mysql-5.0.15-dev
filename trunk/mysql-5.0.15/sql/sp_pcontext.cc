...
sp_pcontext::sp_pcontext(sp_pcontext *prev)
    : Sql_alloc(), m_psubsize(0), m_csubsize(0), m_hsubsize(0), m_handlers(0), m_parent(prev)
{
...
*/
sp_pvar_t *sp_pcontext::find_pvar(LEX_STRING *name, my_bool scoped)
{
  ...
  while (i--)
  {
    ...
    if (my_strnncoll(system_charset_info, (const uchar *)name->str, name->length, (const uchar *)p->name.str,
                     p->name.length) == 0)
    {
      return p;
    }
...
void sp_pcontext::push_pvar(LEX_STRING *name, enum enum_field_types type, sp_param_mode_t mode)
{
...
sp_label_t *sp_pcontext::push_label(char *name, uint ip)
{
...
void sp_pcontext::push_cond(LEX_STRING *name, sp_cond_type_t *val)
{
 ...
 */
sp_cond_type_t *sp_pcontext::find_cond(LEX_STRING *name, my_bool scoped)
{
 ...
 */
bool sp_pcontext::find_handler(sp_cond_type_t *cond)
{
 ...
 * See comment for find_pvar() above
 */
my_bool sp_pcontext::find_cursor(LEX_STRING *name, uint *poff, my_bool scoped)
{
  ...
  while (i--)
  {
    ...
    if (my_strnncoll(system_charset_info, (const uchar *)name->str, name->length, (const uchar *)n.str, n.length) == 0)
    {
      *poff = m_coffset + i;
      return TRUE;
    }
