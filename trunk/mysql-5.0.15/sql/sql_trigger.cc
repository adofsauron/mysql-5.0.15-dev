...
static File_option triggers_file_parameters[] = {
    {{(char *)"triggers", 8}, offsetof(class Table_triggers_list, definitions_list), FILE_OPTIONS_STRLIST},
    {{(char *)"sql_modes", 13}, offsetof(class Table_triggers_list, definition_modes_list), FILE_OPTIONS_ULLLIST},
...
static File_option trigname_file_parameters[] = {
    {{(char *)"trigger_table", 15}, offsetof(struct st_trigname, trigger_table), FILE_OPTIONS_ESTRING},
    {{0, 0}, 0, FILE_OPTIONS_STRING}};

const LEX_STRING trg_action_time_type_names[] = {{(char *)STRING_WITH_LEN("BEFORE")},
                                                 {(char *)STRING_WITH_LEN("AFTER")}};

const LEX_STRING trg_event_type_names[] = {
    {(char *)STRING_WITH_LEN("INSERT")}, {(char *)STRING_WITH_LEN("UPDATE")}, {(char *)STRING_WITH_LEN("DELETE")}};
...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create)
{
  ...
  */
  if (!trust_routine_creators && mysql_bin_log.is_open() && !(thd->security_ctx->master_access & SUPER_ACL))
  {
  ...
  result = (create ? table->triggers->create_trigger(thd, tables) : table->triggers->drop_trigger(thd, tables));
...
bool Table_triggers_list::create_trigger(THD *thd, TABLE_LIST *tables)
{
  ...
  char dir_buff[FN_REFLEN], file_buff[FN_REFLEN], trigname_buff[FN_REFLEN], trigname_path[FN_REFLEN];
  ...
  if (my_strcasecmp(table_alias_charset, table->s->db, lex->spname->m_db.str ? lex->spname->m_db.str : thd->db))
  {
  ...
  */
  ...
  for (trg_field = (Item_trigger_field *)(lex->trg_table_fields.first); trg_field;
  ...
  */
  ...
  dir.str = dir_buff;
  file.length = strxnmov(file_buff, FN_REFLEN, tables->table_name, triggers_file_ext, NullS) - file_buff;
  file.str = file_buff;
  trigname_file.length =
      strxnmov(trigname_buff, FN_REFLEN, lex->spname->m_name.str, trigname_file_ext, NullS) - trigname_buff;
  ...
  if (sql_create_definition_file(&dir, &trigname_file, &trigname_file_type, (gptr)&trigname, trigname_file_parameters,
                                 0))
    return 1;
  ...
  /*
    ...
    QQ: Hmm... probably we should not care about setting up active thread
        mem_root too.
  */
  if (!(trg_def = (LEX_STRING *)alloc_root(&table->mem_root, sizeof(LEX_STRING))) ||
      definitions_list.push_back(trg_def, &table->mem_root) ||
      !(trg_sql_mode = (ulonglong *)alloc_root(&table->mem_root, sizeof(ulonglong))) ||
...
{
  strxnmov(path, FN_REFLEN, mysql_data_home, "/", db, "/", table_name, triggers_file_ext, NullS);
...
{
  strxnmov(path, FN_REFLEN, mysql_data_home, "/", db, "/", trigger_name, trigname_file_ext, NullS);
  unpack_filename(path, path);
  return my_delete(path, MYF(MY_WME));
}
...
bool Table_triggers_list::drop_trigger(THD *thd, TABLE_LIST *tables)
{
  ...
  while ((name = it_name++))
  {
    ...
    if (my_strcasecmp(table_alias_charset, lex->spname->m_name.str, name->str) == 0)
    {
      /*
...
Table_triggers_list::~Table_triggers_list()
{
  for (int i = 0; i < (int)TRG_EVENT_MAX; i++)
    for (int j = 0; j < (int)TRG_ACTION_MAX; j++) delete bodies[i][j];

  if (record1_field)
    for (Field **fld_ptr = record1_field; *fld_ptr; fld_ptr++) delete *fld_ptr;
}
...
bool Table_triggers_list::check_n_load(THD *thd, const char *db, const char *table_name, TABLE *table, bool names_only)
{
  ...
  strxnmov(path_buff, FN_REFLEN, mysql_data_home, "/", db, "/", table_name, triggers_file_ext, NullS);
  ...
  if ((parser = sql_parse_prepare(&path, &table->mem_root, 1)))
  {
    if (is_equal(&triggers_file_type, parser->type()))
    {
      ...
      */
      ...
      if (triggers->definition_modes_list.is_empty() && !triggers->definitions_list.is_empty())
      {
        /*
        ...
        */
        if (!(trg_sql_mode = (ulonglong *)alloc_root(&table->mem_root, sizeof(ulonglong))))
        {
          DBUG_RETURN(1);  // EOM
        }
        *trg_sql_mode = global_system_variables.sql_mode;
        ...
        {
          if (triggers->definition_modes_list.push_back(trg_sql_mode, &table->mem_root))
          {
            DBUG_RETURN(1);  // EOM
          }
        }
      ...
      DBUG_ASSERT(triggers->definition_modes_list.elements == triggers->definitions_list.elements);
      ...
      */
      ...
      while ((trg_create_str = it++))
      {
        ...
        */
        for (Item_trigger_field *trg_field = (Item_trigger_field *)(lex.trg_table_fields.first); trg_field;
             trg_field = trg_field->next_trg_field)
          trg_field->setup_field(thd, table);
    ...
    */
    my_error(ER_WRONG_OBJECT, MYF(0), table_name, triggers_file_ext + 1, "TRIGGER");
    DBUG_RETURN(1);
  }
...
bool Table_triggers_list::get_trigger_info(THD *thd, trg_event_type event, trg_action_time_type time_type,
                                           LEX_STRING *trigger_name, LEX_STRING *trigger_stmt, ulong *sql_mode)
{
  sp_head *body;
...
static TABLE_LIST *add_table_for_trigger(THD *thd, sp_name *trig)
{
  ...
  strxnmov(path_buff, FN_REFLEN, mysql_data_home, "/", db, "/", trig->m_name.str, trigname_file_ext, NullS);
  path.length = unpack_filename(path_buff, path_buff);
  path.str = path_buff;
  ...
  {
    my_error(ER_WRONG_OBJECT, MYF(0), trig->m_name.str, trigname_file_ext + 1, "TRIGGERNAME");
    DBUG_RETURN(0);
  }
  ...
  DBUG_RETURN(sp_add_to_query_tables(thd, lex, db, trigname.trigger_table.str, TL_WRITE));
}
...
bool Table_triggers_list::process_triggers(THD *thd, trg_event_type event, trg_action_time_type time_type,
