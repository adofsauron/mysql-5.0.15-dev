...
#ifdef DBUG_OFF
    {"debug", '#', "This is a non-debug version. Catch this and exit", 0, 0, 0, GET_DISABLED, OPT_ARG, 0, 0, 0, 0, 0,
     0},
#else
    {"debug", '#', "Output debug log", (gptr *)&default_dbug_option, (gptr *)&default_dbug_option, 0, GET_STR, OPT_ARG,
     0, 0, 0, 0, 0, 0},
#endif
    {"help", '?', "Display help and exit", 0, 0, 0, GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
...
void insert_into_hash(hash_lex_struct *root, const char *name, int len_from_begin, int index, int function)
{
  ...
  if (root->first_char == -1)
  {
    ...
    const char *name2 = (index2 < 0 ? sql_functions[-index2 - 1] : symbols[index2]).name + len_from_begin;
  ...
  insert_into_hash(root->char_tails + (*name) - root->first_char, name + 1, len_from_begin + 1, index, function);
}
...
void insert_symbols()
{
  ...
  {
    hash_lex_struct *root = get_hash_struct_by_len(&root_by_len, cur->length, &max_len);
...
void calc_length()
{
  ...
  for (cur = symbols; cur < end; cur++) cur->length = (uchar)strlen(cur->name);
  end = sql_functions + array_elements(sql_functions);
  for (cur = sql_functions; cur < end; cur++) cur->length = (uchar)strlen(cur->name);
}
...
void add_struct_to_map(hash_lex_struct *st)
{
  ...
  hash_map[size_hash_map - 4] = (char)(st->first_char == -1 ? 0 : st->first_char);
  hash_map[size_hash_map - 3] = (char)(st->first_char == -1 || st->first_char == 0 ? 0 : st->last_char);
  if (st->first_char == -1)
  {
...
extern "C" my_bool get_one_option(int optid, const struct my_option *opt __attribute__((unused)),
