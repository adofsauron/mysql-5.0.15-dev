...
handlerton ndbcluster_hton = {
  "ndbcluster",                                     SHOW_OPTION_YES,
  "Clustered, fault-tolerant, memory-based tables", DB_TYPE_NDBCLUSTER,
  ndbcluster_init,                                  0,    /* slot */
  0,                                                      /* savepoint size */
  ndbcluster_close_connection,                      NULL, /* savepoint_set */
  NULL, /* savepoint_rollback */
  NULL, /* savepoint_release */
  ndbcluster_commit,                                ndbcluster_rollback,
  NULL, /* prepare */
  NULL, /* recover */
  NULL, /* commit_by_xid */
...
static int packfrm(const void *data, uint len, const void **pack_data,
                   uint *pack_len);
...
struct show_var_st ndb_status_variables[] = {
  ...
  { "number_of_storage_nodes", (char *)&ndb_number_of_storage_nodes,
    SHOW_LONG },
...
static int ndb_to_mysql_error(const NdbError *err) {
  ...
  if (err_map[i].show_warning) {
    ...
    push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR,
                        ER_GET_ERRMSG, ER(ER_GET_ERRMSG), err->code,
                        err->message, "NDB");
  }
...
inline int execute_no_commit(ha_ndbcluster *h, NdbTransaction *trans) {
  ...
  if (m_batch_execute)
    return 0;
#endif
  return trans->execute(NdbTransaction::NoCommit, NdbTransaction::AbortOnError,
                        h->m_force_send);
}
...
inline int execute_commit(ha_ndbcluster *h, NdbTransaction *trans) {
  ...
  if (m_batch_execute)
    return 0;
#endif
  return trans->execute(NdbTransaction::Commit, NdbTransaction::AbortOnError,
                        h->m_force_send);
}
...
inline int execute_no_commit_ie(ha_ndbcluster *h, NdbTransaction *trans) {
  ...
  return trans->execute(NdbTransaction::NoCommit,
                        NdbTransaction::AO_IgnoreError, h->m_force_send);
}
...
bool ha_ndbcluster::set_hidden_key(NdbOperation *ndb_op, uint fieldnr,
...
int ha_ndbcluster::set_ndb_key(NdbOperation *ndb_op, Field *field, uint fieldnr,
                               const byte *field_ptr) {
  uint32 pack_len = field->pack_length();
  DBUG_ENTER("set_ndb_key");
  DBUG_PRINT("enter", ("%d: %s, ndb_type: %u, len=%d", fieldnr,
                       field->field_name, field->type(), pack_len));
...
int ha_ndbcluster::set_ndb_value(NdbOperation *ndb_op, Field *field,
  ...
  DBUG_PRINT("enter", ("%d: %s, type: %u, len=%d, is_null=%s", fieldnr,
                       field->field_name, field->type(), pack_len,
                       field->is_null() ? "Y" : "N"));
  ...
  DBUG_ASSERT(ndb_supported_type(field->type()));
  {
    ...
    if (!(field->flags & BLOB_FLAG)) {
      if (field->type() != MYSQL_TYPE_BIT) {
        ...
        // Common implementation for most field types
        DBUG_RETURN(ndb_op->setValue(fieldnr, (char *)field_ptr, pack_len) !=
                    0);
      } else // if (field->type() == MYSQL_TYPE_BIT)
      {
        ...
        if (pack_len < 5) {
          DBUG_RETURN(
              ndb_op->setValue(fieldnr, ((char *)&bits) + 4, pack_len) != 0);
        }
#endif
    ...
    for (uint i = 0; i < table->s->fields; i++) {
      ...
      if (value.ptr != NULL && (field->flags & BLOB_FLAG)) {
        ...
        if (loop == 1) {
          ...
          DBUG_PRINT("value",
                     ("read blob ptr=%x len=%u", (UintPtr)buf, (uint)blob_len));
...
int ha_ndbcluster::set_primary_key_from_record(NdbOperation *op,
  ...
  for (; key_part != end; key_part++) {
    Field *field = key_part->field;
    if (set_ndb_key(op, field, key_part->fieldnr - 1,
...
int ha_ndbcluster::set_index_key(NdbOperation *op, const KEY *key_info,
                                 const byte *key_ptr) {
  ...
  for (i = 0; key_part != end; key_part++, i++) {
    ...
    char buf[256];
    shrink_varchar(field, ptr, buf);
    if (set_ndb_key(op, field, m_index[active_index].unique_index_attrid_map[i],
                    ptr))
...
inline int ha_ndbcluster::define_read_attrs(byte *buf, NdbOperation *op) {
  ...
  for (i = 0; i < table->s->fields; i++) {
    Field *field = table->field[i];
    if ((thd->query_id == field->query_id) || ((field->flags & PRI_KEY_FLAG)) ||
...
int ha_ndbcluster::unique_index_read(const byte *key, uint key_len, byte *buf) {
  ...
  if (!(op = trans->getNdbIndexOperation(
            (NDBINDEX *)m_index[active_index].unique_index,
...
int ha_ndbcluster::set_bounds(NdbIndexScanOperation *op,
                              const key_range *keys[2], uint range_no) {
  ...
  for (j = 0; j <= 1; j++) {
    ...
    if (key != NULL) {
      ...
      DBUG_PRINT("info",
                 ("key %d length=%d flag=%d", j, key->length, key->flag));
  ...
  for (i = 0; i < key_parts; i++) {
    ...
    for (j = 0; j <= 1; j++) {
      ...
      if (tot_len < key_tot_len[j]) {
        ...
        p.bound_ptr = (const char *)p.part_null ? 0 : key_part->null_bit
                                                          ? p.part_ptr + 1
                                                          : p.part_ptr;
    ...
    for (j = 0; j <= 1; j++) {
      ...
      if (p.key != NULL) {
        DBUG_PRINT("info", ("key %d:%d offset=%d length=%d last=%d bound=%d", j,
                            i, tot_len, part_len, p.part_last, p.bound_type));
...
int ha_ndbcluster::ordered_index_scan(const key_range *start_key,
  ...
  DBUG_PRINT("enter", ("index: %u, sorted: %d, descending: %d", active_index,
                       sorted, descending));
  ...
  if (m_active_cursor == 0) {
    ...
    NdbOperation::LockMode lm =
        (NdbOperation::LockMode)get_ndb_lock_type(m_lock.type);
    if (!(op = trans->getNdbIndexScanOperation(
              (NDBINDEX *)m_index[active_index].index,
              (const NDBTAB *)m_table)) ||
...
int ha_ndbcluster::write_row(byte *record) {
  ...
  */
  ...
  if ((m_rows_to_insert == (ha_rows)1) ||
      ((m_rows_inserted % m_bulk_insert_rows) == 0) || m_primary_key_update ||
      set_blob_value) {
    // Send rows to NDB
  ...
  if ((has_auto_increment) && (m_skip_auto_increment)) {
    ...
    DBUG_PRINT("info", ("Trying to set next auto increment value to %lu",
                        (ulong)next_val));
...
int ha_ndbcluster::update_row(const byte *old_data, byte *new_data) {
  ...
  for (i = 0; i < table->s->fields; i++) {
    ...
    if (((thd->query_id == field->query_id) || m_retrieve_all_fields) &&
        (!(field->flags & PRI_KEY_FLAG)) && set_ndb_value(op, field, i))
      ERR_RETURN(op->getNdbError());
  }
...
void ha_ndbcluster::unpack_record(byte *buf) {
      ...
      if (!((*field)->flags & BLOB_FLAG)) {
        if ((*value).rec->isNULL())
          ...
          if (pack_len < 5) {
            ...
            ((Field_bit *)*field)
                ->store((longlong)(*value).rec->u_32_value(), FALSE);
          } else {
            DBUG_PRINT("info",
                       ("bit field H'%.8X%.8X", *(Uint32 *)(*value).rec->aRef(),
                        *((Uint32 *)(*value).rec->aRef() + 1)));
            ((Field_bit *)*field)
                ->store((longlong)(*value).rec->u_64_value(), TRUE);
          }
        }
      } else {
        NdbBlob *ndb_blob = (*value).blob;
  ...
  if (table->s->primary_key == MAX_KEY) {
    ...
    DBUG_ASSERT(rec);
    DBUG_PRINT("hidden", ("%d: %s \"%llu\"", hidden_no, hidden_col->getName(),
                          rec->u_64_value()));
  }
// print_results();
#endif
 ...
 */
static int check_null_in_key(const KEY *key_info, const byte *key,
                             uint key_len) {
...
int ha_ndbcluster::index_read(byte *buf, const byte *key, uint key_len,
...
int ha_ndbcluster::index_read_idx(byte *buf, uint index_no, const byte *key,
                                  uint key_len,
                                  enum ha_rkey_function find_flag) {
...
inline int ha_ndbcluster::read_range_first_to_buf(const key_range *start_key,
...
int ha_ndbcluster::read_range_first(const key_range *start_key,
                                    const key_range *end_key, bool eq_r,
                                    bool sorted) {
...
void ha_ndbcluster::position(const byte *record) {
  ...
  if (table->s->primary_key != MAX_KEY) {
    ...
    DBUG_ASSERT(hidden_col->getPrimaryKey() && hidden_col->getAutoIncrement() &&
                rec != NULL && ref_length == NDB_HIDDEN_PRIMARY_KEY_LENGTH);
#endif
  }
...
int ha_ndbcluster::extra(enum ha_extra_function operation) {
  ...
  case HA_EXTRA_KEYREAD_CHANGE_POS: /* Keyread, but change pos */
    DBUG_PRINT("info",
               ("HA_EXTRA_KEYREAD_CHANGE_POS")); /* xxxxchk -r must be used */
    break;
...
int ha_ndbcluster::extra_opt(enum ha_extra_function operation,
                             ulong cache_size) {
...
int ha_ndbcluster::external_lock(THD *thd, int lock_type) {
  ...
  DBUG_PRINT("enter", ("thd: %x, thd_ndb: %x, thd_ndb->lock_count: %d", thd,
                       thd_ndb, thd_ndb->lock_count));
  ...
  if (lock_type != F_UNLCK) {
    ...
    m_ops_pending = 0;
    {
      ...
      // Running might...
      if ((trans &&
           tab->getObjectStatus() != NdbDictionary::Object::Retrieved) ||
          tab->getObjectStatus() == NdbDictionary::Object::Invalid) {
        invalidate_dictionary_cache(FALSE);
        if (!(tab = dict->getTable(m_tabname, &tab_info)))
          ERR_RETURN(dict->getNdbError());
        DBUG_PRINT("info",
                   ("Table schema version: %d", tab->getObjectVersion()));
      }
...
int ndbcluster_commit(THD *thd, bool all) {
  ...
  while ((share = it++)) {
    ...
    DBUG_PRINT("info",
               ("Invalidate commit_count for %s, share->commit_count: %d ",
...
static int create_ndb_column(NDBCOL &col, Field *field, HA_CREATE_INFO *info) {
  ...
  if (field->flags & AUTO_INCREMENT_FLAG) {
    ...
    ulonglong value =
        info->auto_increment_value ? info->auto_increment_value : (ulonglong)1;
...
static void ndb_set_fragmentation(NDBTAB &tab, TABLE *form, uint pk_length) {
  if (form->s->max_rows ==
      (ha_rows)0) /* default setting, don't set fragmentation */
    return;
  /**
  ...
  }
  {
    ...
    if (no_fragments > 2 * no_nodes) {
      ...
      if (no_fragments > 4 * no_nodes)
        push_warning(
            current_thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_UNKNOWN_ERROR,
...
int ha_ndbcluster::create(const char *name, TABLE *form, HA_CREATE_INFO *info) {
  ...
  for (i = 0; i < form->s->fields; i++) {
    ...
    DBUG_PRINT("info",
               ("name: %s, type: %u, pack_length: %d", field->field_name,
                field->real_type(), field->pack_length()));
...
ha_ndbcluster::ha_ndbcluster(TABLE *table_arg)
    : handler(&ndbcluster_hton, table_arg), m_active_trans(NULL),
      m_active_cursor(NULL), m_table(NULL), m_table_version(-1),
      m_table_info(NULL),
      m_table_flags(HA_REC_NOT_IN_SEQ | HA_NULL_IN_KEY | HA_AUTO_PART_KEY |
                    HA_NO_PREFIX_CHAR_KEYS | HA_NEED_READ_RANGE_BUFFER |
                    HA_CAN_BIT_FIELD),
      m_share(0), m_use_write(FALSE), m_ignore_dup_key(FALSE),
      m_primary_key_update(FALSE), m_retrieve_all_fields(FALSE),
      m_retrieve_primary_key(FALSE), m_rows_to_insert((ha_rows)1),
      m_rows_inserted((ha_rows)0), m_bulk_insert_rows((ha_rows)1024),
      m_rows_changed((ha_rows)0), m_bulk_insert_not_flushed(FALSE),
      m_ops_pending(0), m_skip_auto_increment(TRUE), m_blobs_pending(0),
      m_blobs_buffer(0), m_blobs_buffer_size(0), m_dupkey((uint) - 1),
      m_ha_not_exact_count(FALSE), m_force_send(TRUE),
      m_autoincrement_prefetch((ha_rows)32), m_transaction_on(TRUE),
      m_cond_stack(NULL), m_multi_cursor(NULL) {
  int i;
...
int ha_ndbcluster::open(const char *name, int mode, uint test_if_locked) {
  ...
  KEY *key;
  DBUG_ENTER("open");
  DBUG_PRINT("enter", ("name: %s mode: %d test_if_locked: %d", name, mode,
                       test_if_locked));
...
int ndbcluster_find_files(THD *thd, const char *db, const char *path,
  ...
  { // extra bracket to avoid gcc 2.95.3 warning
    ...
    while ((file_name = it++)) {
      ...
      (void)strxnmov(name, FN_REFLEN, mysql_data_home, "/", db, "/", file_name,
                     ha_ndb_ext, NullS);
...
bool ndbcluster_init() {
  ...
  if ((g_ndb_cluster_connection =
    ...
    DBUG_PRINT("error",
               ("Ndb_cluster_connection(%s)", opt_ndbcluster_connectstring));
    goto ndbcluster_init_error;
  }
  ...
  g_ndb->getDictionary()->set_local_table_data_size(
      sizeof(Ndb_local_table_statistics));
  if (g_ndb->init() != 0) {
...
uint ndb_get_commitcount(THD *thd, char *dbname, char *tabname,
  ...
  if (!(share = (NDB_SHARE *)hash_search(&ndbcluster_open_tables, (byte *)name,
                                         strlen(name)))) {
...
static my_bool ndbcluster_cache_retrieval_allowed(THD *thd, char *full_name,
                                                  uint full_name_len,
  ...
  DBUG_PRINT("enter", ("dbname: %s, tabname: %s, is_autocommit: %d", dbname,
                       tabname, is_autocommit));
  ...
  DBUG_PRINT("info", ("*engine_data: %llu, commit_count: %llu", *engine_data,
                      commit_count));
  if (commit_count == 0) {
    *engine_data = 0; /* invalidate */
...
my_bool ha_ndbcluster::register_query_cache_table(
    THD *thd, char *full_name, uint full_name_len,
    qc_engine_callback *engine_callback, ulonglong *engine_data) {
  DBUG_ENTER("ha_ndbcluster::register_query_cache_table");

  bool is_autocommit = !(thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN));

  DBUG_PRINT("enter", ("dbname: %s, tabname: %s, is_autocommit: %d", m_dbname,
                       m_tabname, is_autocommit));
...
static int ndb_get_table_statistics(Ndb *ndb, const char *table,
  ...
  do {
    ...
    check = pTrans->execute(NdbTransaction::NoCommit,
                            NdbTransaction::AbortOnError, TRUE);
    if (check == -1)
      break;
...
int ha_ndbcluster::write_ndb_file() {
  ...
  (void)strxnmov(path, FN_REFLEN, mysql_data_home, "/", m_dbname, "/",
                 m_tabname, ha_ndb_ext, NullS);

  if ((file = my_create(path, CREATE_MODE, O_RDWR | O_TRUNC, MYF(MY_WME))) >=
      0) {
    // It's an empty file
    error = 0;
    my_close(file, MYF(0));
...
int ha_ndbcluster::read_multi_range_first(KEY_MULTI_RANGE **found_range_p,
  ...
  for (;
       multi_range_curr < multi_range_end && curr + reclength <= end_of_buffer;
       multi_range_curr++) {
    switch (index_type) {
    case PRIMARY_KEY_ORDERED_INDEX:
      ...
      if (scanOp == 0) {
        if (m_multi_cursor) {
          ...
          end_of_buffer -= reclength;
                        ...
                        m_active_trans->getNdbIndexScanOperation(idx, tab)) &&
                   !scanOp->readTuples(lm, 0, parallelism, sorted, FALSE,
                                       TRUE) &&
...
ha_ndbcluster::read_multi_range_next(KEY_MULTI_RANGE **multi_range_found_p) {
   ...
   */
  DBUG_RETURN(read_multi_range_first(multi_range_found_p, multi_range_curr,
    ...
    const char *comment) /* in:  table comment defined by user */
{
  ...
  my_snprintf(str, fmt_len_plus_extra, fmt, comment, length > 0 ? " " : "",
              tab->getReplicaCount());
  return str;
}
...
pthread_handler_t ndb_util_thread_func(void *arg __attribute__((unused))) {
  ...
  for (;;) {
    ...
    pthread_cond_timedwait(&COND_ndb_util_thread, &LOCK_ndb_util_thread,
                           &abstime);
    pthread_mutex_unlock(&LOCK_ndb_util_thread);

    DBUG_PRINT("ndb_util_thread",
               ("Started, ndb_cache_check_time: %d", ndb_cache_check_time));
    ...
    for (uint i = 0; i < ndbcluster_open_tables.records; i++) {
      ...
      DBUG_PRINT("ndb_util_thread", ("Found open table[%d]: %s, use_count: %d",
    ...
    while ((share = it++)) {
      ...
      DBUG_PRINT("ndb_util_thread",
                 ("Fetching commit count for: %s", share->table_name));
...
void ndb_serialize_cond(const Item *item, void *arg) {
  ...
  if (context->supported) {
    ...
    if (rewrite_context && (func_item = rewrite_context->func_item) &&
      ...
      if (!item) {
            ...
            if (context->expecting(Item::FIELD_ITEM) &&
                (context->expecting_field_result(field->result_type()) ||
                 ...
                 ((type == MYSQL_TYPE_TIME || type == MYSQL_TYPE_DATE ||
                   type == MYSQL_TYPE_YEAR || type == MYSQL_TYPE_DATETIME)
              ...
              if (context->expect_mask) {
                ...
                if (type == MYSQL_TYPE_TIME || type == MYSQL_TYPE_DATE ||
                    type == MYSQL_TYPE_YEAR || type == MYSQL_TYPE_DATETIME) {
        ...
        case Item::FUNC_ITEM: {
          ...
          case Item_func::EQ_FUNC: {
            DBUG_PRINT("info", ("EQ_FUNC"));
            curr_cond->ndb_item =
                new Ndb_item(func_item->functype(), func_item);
            context->expect(Item::STRING_ITEM);
            context->expect(Item::INT_ITEM);
            context->expect(Item::REAL_ITEM);
            context->expect(Item::DECIMAL_ITEM);
        ...
        case Item::STRING_ITEM:
          ...
          if (context->expecting(Item::STRING_ITEM)) {
            ...
            DBUG_PRINT("info",
                       ("value \"%s\"", string_item->val_str(&str)->ptr()));
#endif
            NDB_ITEM_QUALIFICATION q;
            q.value_type = Item::STRING_ITEM;
            curr_cond->ndb_item = new Ndb_item(NDB_VALUE, q, item);
        ...
        case Item::COND_ITEM: {
          ...
          if (context->expecting(Item::COND_ITEM)) {
            ...
            case Item_func::COND_AND_FUNC:
              DBUG_PRINT("info", ("COND_AND_FUNC"));
              curr_cond->ndb_item =
                  new Ndb_item(cond_item->functype(), cond_item);
              break;
...
bool ha_ndbcluster::serialize_cond(const COND *cond, Ndb_cond_stack *ndb_cond) {
  ...
  DBUG_PRINT("info", ("The pushed condition is %ssupported",
                      (context.supported) ? "" : "not "));

  DBUG_RETURN(context.supported);
}

int ha_ndbcluster::build_scan_filter_predicate(Ndb_cond *&cond,
  ...
  case NDB_FUNCTION: {
    ...
    case NDB_EQ_FUNC: {
      ...
      if (filter->cmp(NdbScanFilter::COND_EQ, field->get_field_no(),
    ...
    case NDB_NE_FUNC: {
      ...
      if (filter->cmp(NdbScanFilter::COND_NE, field->get_field_no(),
                      field->get_val(), field->pack_length()) == -1)
        DBUG_RETURN(1);
      cond = cond->next->next->next;
      DBUG_RETURN(0);
    ...
    case NDB_LT_FUNC: {
      ...
      if (a == field) {
        ...
        if (filter->cmp(NdbScanFilter::COND_LT, field->get_field_no(),
                        field->get_val(), field->pack_length()) == -1)
          DBUG_RETURN(1);
      } else {
        DBUG_PRINT("info", ("Generating GT filter"));
        if (filter->cmp(NdbScanFilter::COND_GT, field->get_field_no(),
                        field->get_val(), field->pack_length()) == -1)
          DBUG_RETURN(1);
      }
      cond = cond->next->next->next;
    ...
    case NDB_LE_FUNC: {
      ...
      if (a == field) {
        ...
        if (filter->cmp(NdbScanFilter::COND_LE, field->get_field_no(),
                        field->get_val(), field->pack_length()) == -1)
          DBUG_RETURN(1);
      } else {
        DBUG_PRINT("info", ("Generating GE filter"));
        if (filter->cmp(NdbScanFilter::COND_GE, field->get_field_no(),
                        field->get_val(), field->pack_length()) == -1)
          DBUG_RETURN(1);
      }
      cond = cond->next->next->next;
    ...
    case NDB_LIKE_FUNC: {
      ...
      DBUG_PRINT("info", ("Generating LIKE filter: like(%d,%s,%d)",
                          ...
                          value->pack_length()));
      if (filter->cmp(NdbScanFilter::COND_LIKE, field->get_field_no(),
                      value->get_val(), value->pack_length()) == -1)
        DBUG_RETURN(1);
      cond = cond->next->next->next;
      DBUG_RETURN(0);
    }
    case NDB_NOTLIKE_FUNC: {
      ...
      DBUG_PRINT("info", ("Generating NOTLIKE filter: notlike(%d,%s,%d)",
                          field->get_field_no(), value->get_val(),
                          value->pack_length()));
      if (filter->cmp(NdbScanFilter::COND_NOT_LIKE, field->get_field_no(),
                      value->get_val(), value->pack_length()) == -1)
        DBUG_RETURN(1);
      cond = cond->next->next->next;
      DBUG_RETURN(0);
...
int ha_ndbcluster::build_scan_filter_group(Ndb_cond *&cond,
                                           NdbScanFilter *filter) {
  uint level = 0;
  bool negated = FALSE;
  ...
  do {
    ...
    case NDB_FUNCTION: {
      ...
      case NDB_COND_OR_FUNC: {
        level++;
        DBUG_PRINT("info",
                   ("Generating %s group %u", (negated) ? "NOR" : "OR", level));
    ...
    default: { DBUG_PRINT("info", ("Illegal scan filter")); }
    }
...
int ha_ndbcluster::generate_scan_filter(Ndb_cond_stack *ndb_cond_stack,
                                        NdbScanOperation *op) {
