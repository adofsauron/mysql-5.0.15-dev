...
handlerton archive_hton = { "ARCHIVE",                SHOW_OPTION_YES,
                            "Archive storage engine", DB_TYPE_ARCHIVE_DB,
                            archive_db_init,          0, /* slot */
                            0,                           /* savepoint size. */
...
int ha_archive::write_data_header(gzFile file_to_write) {
  ...
  if (gzwrite(file_to_write, &data_buffer, DATA_BUFFER_SIZE) !=
      DATA_BUFFER_SIZE)
    goto error;
  DBUG_PRINT("ha_archive::write_data_header",
             ("Check %u", (uint)data_buffer[0]));
  DBUG_PRINT("ha_archive::write_data_header",
             ("Version %u", (uint)data_buffer[1]));
...
int ha_archive::read_meta_file(File meta_file, ha_rows *rows) {
  ...
  *rows = (ha_rows)uint8korr(meta_buffer + 2);
  ...
  DBUG_PRINT("ha_archive::read_meta_file",
             ("Version %d", (uint)meta_buffer[1]));
...
int ha_archive::write_meta_file(File meta_file, ha_rows rows, bool dirty) {
  ...
  *(meta_buffer + 18) = (uchar)dirty;
  DBUG_PRINT("ha_archive::write_meta_file",
             ("Check %d", (uint)ARCHIVE_CHECK_HEADER));
  DBUG_PRINT("ha_archive::write_meta_file",
             ("Version %d", (uint)ARCHIVE_VERSION));
...
int ha_archive::real_write_row(byte *buf, gzFile writer) {
  ...
  DBUG_PRINT("ha_archive::real_write_row",
             ("Wrote %d bytes expected %d", written, table->s->reclength));
...
int ha_archive::get_row(gzFile file_to_read, byte *buf) {
  ...
  DBUG_PRINT("ha_archive::get_row",
             ("Read %d bytes expected %d", read, table->s->reclength));
  ...
  if (lock_type != TL_IGNORE && lock.type == TL_UNLOCK) {
    ...
    */
    ...
    if ((lock_type >= TL_WRITE_CONCURRENT_INSERT && lock_type <= TL_WRITE) &&
        !thd->in_lock_tables && !thd->tablespace_op)
