...
#define FSP_FREE_LIMIT                      \
  12 /* Minimum page number for which the   \
     ...
     file */
#define FSP_LOWEST_NO_WRITE                \
  16 /* The lowest page offset for which   \
...
ulint xdes_calc_descriptor_index(
    ...
    ulint offset) /* in: page offset */
{
  return (ut_2pow_remainder(offset, XDES_DESCRIBED_PER_PAGE) / FSP_EXTENT_SIZE);
}
    ...
    mtr_t *mtr)              /* in: mtr handle */
{
  ...
  {
    descr_page = buf_page_get(space, descr_page_no, RW_X_LATCH, mtr);
#ifdef UNIV_SYNC_DEBUG
    ...
    xdes_t *descr) /* in: extent descriptor */
{
  ...
  return (buf_frame_get_page_no(descr) +
          ((descr - buf_frame_align(descr) - XDES_ARR_OFFSET) / XDES_SIZE) * FSP_EXTENT_SIZE);
}
...
void fsp_header_write_space_id(
    /*======================*/
    ...
    mtr_t *mtr)  /* in: mini-transaction handle */
{
  ...
  if (space == 0)
  {
    ...
    btr_create(DICT_CLUSTERED | DICT_UNIVERSAL | DICT_IBUF, space, ut_dulint_add(DICT_IBUF_ID_MIN, space), FALSE, mtr);
  }
  else
  {
    ...
    page_t *page) /* in: first page of a tablespace */
{
  ...
  if (id != fsp_id)
  {
    fprintf(stderr, "InnoDB: Error: space id in fsp header %lu, but in the page header %lu\n", (ulong)fsp_id,
            (ulong)id);
    return (ULINT_UNDEFINED);
  }
    ...
    mtr_t *mtr)             /* in: mtr */
{
  ...
  *actual_increase = 0;
  ...
  if (space == 0 && srv_last_file_size_max != 0)
  {
    if (srv_last_file_size_max < srv_data_file_sizes[srv_n_data_files - 1])
    {
      fprintf(stderr, "InnoDB: Error: Last data file size is %lu, max size allowed %lu\n",
              (ulong)srv_data_file_sizes[srv_n_data_files - 1], (ulong)srv_last_file_size_max);
    }
  ...
  mlog_write_ulint(header + FSP_SIZE, ut_calc_align_down(actual_size, (1024 * 1024) / UNIV_PAGE_SIZE), MLOG_4BYTES,
                   mtr);
  new_size = mtr_read_ulint(header + FSP_SIZE, MLOG_4BYTES, mtr);
    ...
    mtr_t *mtr)                 /* in: mini-transaction handle */
{
  ...
  page = buf_page_get(buf_frame_get_space_id(space_header), page_no, RW_X_LATCH, mtr);
#ifdef UNIV_SYNC_DEBUG
  buf_page_dbg_add_level(page, SYNC_FSP_PAGE);
#endif /* UNIV_SYNC_DEBUG */
...
UNIV_INLINE
ulint fseg_get_nth_frag_page_no(
    /*======================*/
    ...
    mtr_t *mtr)              /* in: mtr handle */
{
           ...
           (used >= FSEG_FRAG_LIMIT))
  {
    ...
    /*-------------------------------------------------------------*/
           ...
           (NULL != (ret_descr = fseg_alloc_free_extent(seg_inode, space, mtr))))
  {
    ...
    /*-------------------------------------------------------------*/
           ...
           (0 == ut_dulint_cmp(mtr_read_dulint(descr + XDES_ID, mtr), seg_id)) && (!xdes_is_full(descr, mtr)))
  {
  ...
  if (space != 0)
  {
    ...
    if (space_size <= ret_page)
    {
      ...
      success = fsp_try_extend_data_file_with_pages(space, ret_page, space_header, mtr);
      if (!success)
      {
        /* No disk space left */
        return (FIL_NULL);
...
pages. */
static ibool fsp_reserve_free_pages(
    /*===================*/
...
ulint fsp_get_available_space_in_free_extents(
    /*====================================*/
  ...
  */
  if (0 != ut_dulint_cmp(descr_id, seg_id))
  {
    ...
    fprintf(stderr,
            ...
            (ulong)space, (ulong)page, (ulong)ut_dulint_get_high(descr_id), (ulong)ut_dulint_get_low(descr_id),
            (ulong)ut_dulint_get_high(seg_id), (ulong)ut_dulint_get_low(seg_id));
    goto crash;
  }
    ...
    mtr_t *mtr)              /* in: mtr handle */
{
  ...
  ut_a(0 == ut_dulint_cmp(mtr_read_dulint(descr + XDES_ID, mtr), mtr_read_dulint(seg_inode + FSEG_ID, mtr)));
...
/***********************************************************************
Validates a segment. */
static ibool fseg_validate_low(
    /*==============*/
    /* out: TRUE if ok */
    fseg_inode_t *inode, /* in: segment inode */
    mtr_t *mtr2)         /* in: mtr */
{
  ...
  while (!fil_addr_is_null(node_addr))
  {
    ...
    ut_a(xdes_get_state(descr, &mtr) == XDES_FSEG);
    ut_a(0 == ut_dulint_cmp(mtr_read_dulint(descr + XDES_ID, &mtr), seg_id));
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  fprintf(stderr,
          ...
          (ulong)seg_id_high, (ulong)seg_id_low, (ulong)space, (ulong)page_no, (ulong)reserved, (ulong)used,
          (ulong)n_full, (ulong)n_frag, (ulong)n_free, (ulong)n_not_full, (ulong)n_used);
}
    ...
    ulint space) /* in: space id */
{
  ...
  ut_a(n_used + n_full_frag_pages == n_used2 +
                                         2 * ((free_limit + XDES_DESCRIBED_PER_PAGE - 1) / XDES_DESCRIBED_PER_PAGE) +
    ...
    ulint space) /* in: space id */
{
  ...
  fprintf(stderr,
          ...
          (long)space, (ulong)size, (ulong)free_limit, (ulong)n_free, (ulong)n_free_frag, (ulong)frag_n_used,
          (ulong)n_full_frag, (ulong)seg_id_high, (ulong)seg_id_low);
