...
static inline void write_to_binlog(THD *thd, char *query, uint q_len, char *db,
                                   uint db_len) {
...
bool my_dbopt_init(void) {
  ...
  if (!dboptions_init) {
    dboptions_init = 1;
    error = hash_init(&dboptions, lower_case_table_names ? &my_charset_bin
                                                         : system_charset_info,
                      32, 0, 0, (hash_get_key)dboptions_get_key, free_dbopt, 0);
  }
  return error;
}
...
void my_dbopt_cleanup(void) {
  ...
  hash_free(&dboptions);
  hash_init(&dboptions,
            lower_case_table_names ? &my_charset_bin : system_charset_info, 32,
            0, 0, (hash_get_key)dboptions_get_key, free_dbopt, 0);
...
static my_bool put_dbopt(const char *dbname, HA_CREATE_INFO *create) {
  ...
  if (!(opt = (my_dbopt_t *)hash_search(&dboptions, (byte *)dbname, length))) {
    ...
    char *tmp_name;
    if (!my_multi_malloc(MYF(MY_WME | MY_ZEROFILL), &opt, (uint)sizeof(*opt),
...
static bool write_db_opt(THD *thd, const char *path, HA_CREATE_INFO *create) {
  ...
  if ((file = my_create(path, CREATE_MODE, O_RDWR | O_TRUNC, MYF(MY_WME))) >=
    ...
    ulong length;
    length = (ulong)(
        strxnmov(buf, sizeof(buf), "default-character-set=",
...
bool mysql_create_db(THD *thd, char *db, HA_CREATE_INFO *create_info,
  ...
  if (my_stat(path, &stat_info, MYF(0))) {
    ...
    push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_DB_CREATE_EXISTS,
                        ER(ER_DB_CREATE_EXISTS), db);
    error = 0;
    if (!silent)
      send_ok(thd);
    goto exit;
  ...
  if (!silent) {
    ...
    if (!thd->query) // Only in replication
    {
      query = path;
      query_length =
          (uint)(strxmov(path, "create database `", db, "`", NullS) - path);
    } else {
      query = thd->query;
...
bool mysql_rm_db(THD *thd, char *db, bool if_exists, bool silent) {
  ...
  if (!(dirp = my_dir(path, MYF(MY_DONT_SORT)))) {
    if (!if_exists) {
      ...
      push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_DB_DROP_EXISTS,
                          ER(ER_DB_DROP_EXISTS), db);
  } else {
  ...
  if (!silent && deleted >= 0) {
    ...
    if (!thd->query) {
      ...
      query = path;
      query_length =
          (uint)(strxmov(path, "drop database `", db, "`", NullS) - path);
    } else {
      query = thd->query;
      query_length = thd->query_length;
    }
...
static long mysql_rm_known_files(THD *thd, MY_DIR *dirp, const char *db,
  ...
  for (uint idx = 0; idx < (uint)dirp->number_off_files && !thd->killed;
       idx++) {
    ...
    if ((my_isdigit(system_charset_info, file->name[0]) ||
      ...
      if ((new_dirp = my_dir(newpath, MYF(MY_DONT_SORT)))) {
        ...
        if (!(copy_of_path = thd->memdup(newpath, length + 1)) ||
            !(dir = new (thd->mem_root)
              String(copy_of_path, length, &my_charset_bin)) ||
    ...
    if (db && !my_strcasecmp(files_charset_info, extension, reg_ext)) {
      ...
      *extension = 0; // Remove extension
      TABLE_LIST *table_list = (TABLE_LIST *)thd->calloc(
          sizeof(*table_list) + strlen(db) + strlen(file->name) + 2);
      if (!table_list)
        goto err;
      table_list->db = (char *)(table_list + 1);
      strmov(table_list->table_name = strmov(table_list->db, db) + 1,
             file->name);
