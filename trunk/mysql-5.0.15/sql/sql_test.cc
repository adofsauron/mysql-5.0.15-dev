...
void TEST_join(JOIN *join) {
  ...
  for (i = 0; i < join->tables; i++) {
    ...
    fprintf(DBUG_FILE,
            ...
            tab->keys.print(key_map_buff), tab->ref.key_parts, tab->ref.key,
            tab->ref.key_length);
    if (tab->select) {
      char buf[MAX_KEY / 8 + 1];
      if (tab->use_quick == 2)
        fprintf(DBUG_FILE, "                  quick select checked for each "
                           "record (keys: %s)\n",
...
static void display_table_locks(void) {
  ...
  qsort((gptr)dynamic_element(&saved_table_locks, 0, TABLE_LOCK_INFO *),
        saved_table_locks.elements, sizeof(TABLE_LOCK_INFO),
        (qsort_cmp)dl_compare);
  ...
  for (i = 0; i < saved_table_locks.elements; i++) {
    TABLE_LOCK_INFO *dl_ptr =
        dynamic_element(&saved_table_locks, i, TABLE_LOCK_INFO *);
    printf("%-8ld%-28.28s%-22s%s\n", dl_ptr->thread_id, dl_ptr->table_name,
           dl_ptr->lock_text, lock_descriptions[(int)dl_ptr->type]);
  }
  puts("\n\n");
end:
...
r_requests:     %10s\n\
reads:          %10s\n\n",
           name, (ulong)key_cache->param_buff_size, key_cache->param_block_size,
...
delete      %10lu\n\
update:     %10lu\n",
         tmp.ha_read_key_count, tmp.ha_read_next_count, tmp.ha_read_rnd_count,
         tmp.ha_read_first_count, tmp.ha_write_count, tmp.ha_delete_count,
         tmp.ha_update_count);
...
Open files:    %10lu\n\
Open streams:  %10lu\n",
         tmp.opened_tables, (ulong)cached_tables(), (ulong)my_file_opened,
...
Estimated memory (with thread stack):    %ld\n",
         (int)info.arena, (int)info.ordblks, (int)info.smblks, (int)info.hblks,
         (int)info.hblkhd, (int)info.usmblks, (int)info.fsmblks,
