...
static void rw_lock_debug_free(
    /*===============*/
    ...
    rw_lock_t *lock)
{
  ...
  ut_a((rw_lock_get_reader_count(lock) == 0) || (rw_lock_get_writer(lock) != RW_LOCK_EX));
  ut_a((rw_lock_get_writer(lock) == RW_LOCK_EX) || (rw_lock_get_writer(lock) == RW_LOCK_WAIT_EX) ||
       (rw_lock_get_writer(lock) == RW_LOCK_NOT_LOCKED));
  ut_a((rw_lock_get_waiters(lock) == 0) || (rw_lock_get_waiters(lock) == 1));
...
lock_loop:
  ...
  {
    fprintf(stderr, "Thread %lu spin wait rw-s-lock at %p cfile %s cline %lu rnds %lu\n",
            (ulong)os_thread_pf(os_thread_get_curr_id()), lock, lock->cfile_name, (ulong)lock->cline, (ulong)i);
  }
  ...
  else
  {
    ...
    if (srv_print_latch_waits)
    {
      fprintf(stderr, "Thread %lu OS wait rw-s-lock at %p cfile %s cline %lu\n", os_thread_pf(os_thread_get_curr_id()),
...
lock_loop:
  ...
  else if (state == RW_LOCK_WAIT_EX)
  {
    ...
    while (rw_lock_get_reader_count(lock) != 0 && i < SYNC_SPIN_ROUNDS)
    {
      if (srv_spin_wait_delay)
      {
        ut_delay(ut_rnd_interval(0, srv_spin_wait_delay));
  ...
  if (srv_print_latch_waits)
  {
    fprintf(stderr, "Thread %lu spin wait rw-x-lock at %p cfile %s cline %lu rnds %lu\n",
            os_thread_pf(os_thread_get_curr_id()), lock, lock->cfile_name, (ulong)lock->cline, (ulong)i);
  }
  ...
  if (srv_print_latch_waits)
  {
    fprintf(stderr, "Thread %lu OS wait for rw-x-lock at %p cfile %s cline %lu\n",
            os_thread_pf(os_thread_get_curr_id()), lock, lock->cfile_name, (ulong)lock->cline);
  }
    ...
    rw_lock_debug_t *info) /* in: debug struct */
{
  ...
  fprintf(stderr, "Locked: thread %ld file %s line %ld  ", (ulong)os_thread_pf(info->thread_id), info->file_name,
          (ulong)info->line);
