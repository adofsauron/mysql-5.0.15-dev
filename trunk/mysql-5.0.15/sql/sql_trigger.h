...
class Table_triggers_list : public Sql_alloc
{
  ...
  */
  ...
  Table_triggers_list(TABLE *table_arg) : record1_field(0), table(table_arg) { bzero((char *)bodies, sizeof(bodies)); }
  ~Table_triggers_list();
  ...
  bool get_trigger_info(THD *thd, trg_event_type event, trg_action_time_type time_type, LEX_STRING *trigger_name,
  ...
  {
    return (bodies[TRG_EVENT_DELETE][TRG_ACTION_BEFORE] || bodies[TRG_EVENT_DELETE][TRG_ACTION_AFTER]);
  }

  bool has_before_update_triggers() { return test(bodies[TRG_EVENT_UPDATE][TRG_ACTION_BEFORE]); }

  void set_table(TABLE *new_table);

  friend class Item_trigger_field;
  friend void sp_cache_routines_and_add_tables_for_triggers(THD *thd, LEX *lex, Table_triggers_list *triggers);
