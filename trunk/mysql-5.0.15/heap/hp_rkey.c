...
int heap_rkey(HP_INFO *info, byte *record, int inx, const byte *key, uint key_len, enum ha_rkey_function find_flag)
{
  ...
  if (keyinfo->algorithm == HA_KEY_ALG_BTREE)
  {
    ...
    custom_arg.key_length = info->lastkey_len = hp_rb_pack_key(keyinfo, (uchar *)info->lastkey, (uchar *)key, key_len);
    ...
    if (!(pos = tree_search_key(&keyinfo->rb_tree, info->lastkey, info->parents, &info->last_pos, find_flag,
                                &custom_arg)))
    {
      info->update = 0;
...
gptr heap_find(HP_INFO *info, int inx, const byte *key) { return hp_search(info, info->s->keydef + inx, key, 0); }
