...
void Item_subselect::init(st_select_lex *select_lex, select_subselect *result) {
  ...
  if (unit->item) {
    ...
    */
    parsing_place =
        (outer_select->in_sum_expr ? NO_MATTER : outer_select->parsing_place);
...
Item_singlerow_subselect::select_transformer(JOIN *join) {
  ...
  if (!select_lex->master_unit()->first_select()->next_select() &&
      !select_lex->table_list.elements && select_lex->item_list.elements == 1 &&
    ...
    if (thd->lex->describe) {
      ...
      push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_SELECT_REDUCED,
                   warn_buff);
    }
...
Item_allany_subselect::Item_allany_subselect(Item *left_exp, Comp_creator *fn,
                                             st_select_lex *select_lex,
                                             bool all_arg)
...
Item_in_subselect::single_value_transformer(JOIN *join, Comp_creator *func) {
  ...
  */
  if ((abort_on_null || (upper_item && upper_item->top_level())) &&
    ...
    Item *subs;
    if (!select_lex->group_list.elements && !select_lex->having &&
        !select_lex->with_sum_func && !(select_lex->next_select())) {
      ...
      */
      ...
      subs = item =
          new Item_maxmin_subselect(thd, this, select_lex, func->l_op());
      if (upper_item)
  ...
  if (!substitution) {
    ...
    */
    expr = new Item_direct_ref(
        &select_lex->context, (Item **)optimizer->get_cache(),
  ...
  */
  if (join->having || select_lex->with_sum_func ||
    ...
    bool tmp;
    Item *item = func->create(
        expr, new Item_ref_null_helper(&select_lex->context, this,
    ...
    if (select_lex->table_list.elements) {
      ...
      if (!abort_on_null && orig_item->maybe_null) {
        ...
        /*
          ...
          argument (reference) to fix_fields()
        */
        select_lex->having = join->having =
            (join->having ? new Item_cond_and(having, join->having) : having);
        select_lex->having_fix_field = 1;
        /*
      ...
      */
      ...
      if (select_lex->master_unit()->first_select()->next_select()) {
        ...
        */
        item =
            func->create(expr, new Item_null_helper(&select_lex->context, this,
                                                    item, (char *)"<no matter>",
                                                    (char *)"<result>"));
#ifdef CORRECT_BUT_TOO_SLOW_TO_BE_USABLE
        if (!abort_on_null && left_expr->maybe_null)
    ...
    */
    ...
    for (uint i = 0; i < cols_num; i++) {
      ...
      Item *item_isnull = new Item_func_isnull(new Item_direct_ref(
          &select_lex->context, select_lex->ref_pointer_array + i,
          (char *)"<no matter>", (char *)"<list ref>"));
      having_item =
          and_items(having_item, new Item_cond_or(item_eq, item_isnull));
      item_having_part2 = and_items(
          item_having_part2,
          new Item_is_not_null_test(
              this, new Item_direct_ref(
                        &select_lex->context, select_lex->ref_pointer_array + i,
                        (char *)"<no matter>", (char *)"<list ref>")));
    ...
    for (uint i = 0; i < cols_num; i++) {
      ...
      if (!abort_on_null) {
        having_item = and_items(
            having_item,
            new Item_is_not_null_test(
                this, new Item_direct_ref(&select_lex->context,
                                          select_lex->ref_pointer_array + i,
                                          (char *)"<no matter>",
                                          (char *)"<list ref>")));
        item_isnull = new Item_func_isnull(new Item_direct_ref(
            &select_lex->context, select_lex->ref_pointer_array + i,
            (char *)"<no matter>", (char *)"<list ref>"));
...
subselect_single_select_engine::subselect_single_select_engine(
    st_select_lex *select, select_subselect *result, Item_subselect *item)
    : subselect_engine(item, result), prepared(0), optimized(0), executed(0),
...
int subselect_single_select_engine::prepare() {
  ...
  if (join->prepare(&select_lex->ref_pointer_array,
                    ...
                    select_lex->with_wild, select_lex->where,
...
static Item_result set_row(List<Item> &item_list, Item *item, Item_cache **row,
                           bool *maybe_null) {
...
int subselect_uniquesubquery_engine::exec() {
  ...
  if (error && error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
    ...
    error = 0;
    table->null_row = 0;
    ((Item_in_subselect *)item)->value =
        (!table->status && (!cond || cond->val_int()) ? 1 : 0);
  }
  ...
  if (error && error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
    ...
    for (;;) {
      ...
      if (!table->status) {
        ...
        error = table->file->index_next_same(
...
table_map subselect_single_select_engine::upper_select_const_tables() {
  return calc_const_tables(
      (TABLE_LIST *)select_lex->outer_select()->leaf_tables);
}
