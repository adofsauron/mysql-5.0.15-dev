...
String *Item_func_md5::val_str(String *str)
{
  ...
  if (sptr)
  {
    ...
    sprintf((char *)str->ptr(), "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", digest[0],
            digest[1], digest[2], digest[3], digest[4], digest[5], digest[6], digest[7], digest[8], digest[9],
...
void Item_func_md5::fix_length_and_dec() { max_length = 32; }
...
%02x%02x%02x%02x%02x%02x%02x%02x",
              ...
              digest[9], digest[10], digest[11], digest[12], digest[13], digest[14], digest[15], digest[16], digest[17],
              digest[18], digest[19]);
...
String *Item_func_aes_encrypt::val_str(String *str)
{
  ...
  if (sptr && key)  // we need both arguments to be not NULL
  {
    ...
    if (!str_value.alloc(aes_length))  // Ensure that memory is free
    {
      ...
      if (my_aes_encrypt(sptr->ptr(), sptr->length(), (char *)str_value.ptr(), key->ptr(), key->length()) == aes_length)
      {
...
void Item_func_aes_encrypt::fix_length_and_dec() { max_length = my_aes_get_size(args[0]->max_length); }
...
String *Item_func_aes_decrypt::val_str(String *str)
{
  ...
  if (sptr && key)                         // Need to have both arguments not NULL
  {
    ...
    if (!str_value.alloc(sptr->length()))  // Ensure that memory is free
    {
      ...
      int length;
      length = my_aes_decrypt(sptr->ptr(), sptr->length(), (char *)str_value.ptr(), key->ptr(), key->length());
...
String *Item_func_des_encrypt::val_str(String *str)
{
  ...
  */
  ...
  // Real encryption
  bzero((char *)&ivec, sizeof(ivec));
  DES_ede3_cbc_encrypt((const uchar *)(res->ptr()), (uchar *)(tmp_value.ptr() + 1), res_length, &keyschedule.ks1,
                       &keyschedule.ks2, &keyschedule.ks3, &ivec, TRUE);
  return &tmp_value;

error:
  push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, code, ER(code), "des_encrypt");
#else
  push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_FEATURE_DISABLED, ER(ER_FEATURE_DISABLED),
...
error:
  push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, code, ER(code), "des_decrypt");
wrong_key:
#else
  push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_FEATURE_DISABLED, ER(ER_FEATURE_DISABLED),
...
String *Item_func_concat_ws::val_str(String *str)
{
  ...
  String tmp_sep_str(tmp_str_buff, sizeof(tmp_str_buff), default_charset_info), *sep_str, *res, *res2, *use_as_buff;
  uint i;
  ...
  for (i++; i < arg_count; i++)
  {
    ...
    if (res->alloced_length() >= res->length() + sep_str->length() + res2->length())
    {                         // Use old buffer
      res->append(*sep_str);  // res->length() > 0 always
      res->append(*res2);
    }
    else if (str->alloced_length() >= res->length() + sep_str->length() + res2->length())
    {
    ...
    {  // Two big const strings
      if (tmp_value.alloc(max_length) || tmp_value.copy(*res) || tmp_value.append(*sep_str) || tmp_value.append(*res2))
        goto null;
      res = &tmp_value;
      use_as_buff = str;
    }
...
void Item_func_concat_ws::fix_length_and_dec()
{
  ...
  */
  ...
  for (uint i = 1; i < arg_count; i++) max_result_length += args[i]->max_length;
...
String *Item_func_insert::val_str(String *str)
{
  ...
  if (args[0]->null_value || args[1]->null_value || args[2]->null_value || args[3]->null_value)
...
String *Item_str_conv::val_str(String *str)
{
  ...
  if (multiply == 1)
  {
    uint len;
    res = copy_if_not_alloced(str, res, res->length());
    len = converter(collation.collation, (char *)res->ptr(), res->length(), (char *)res->ptr(), res->length());
  ...
  else
  {
    ...
    len = converter(collation.collation, (char *)res->ptr(), res->length(), (char *)tmp_value.ptr(), len);
...
String *Item_func_substr::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || args[1]->null_value || (arg_count == 3 && args[2]->null_value))))
    return 0; /* purecov: inspected */
...
Item *Item_func_sysconst::safe_charset_converter(CHARSET_INFO *tocs)
{
  ...
  if (conv_errors || !(conv = new Item_static_string_func(fully_qualified_func_name(), cstr.ptr(), cstr.length(),
...
String *Item_func_user::val_str(String *str)
{
  ...
  res_length = cs->cset->snprintf(cs, (char *)str->ptr(), res_length, "%s@%s", user, host);
...
static char soundex_toupper(char ch) { return (ch >= 'a' && ch <= 'z') ? ch - 'a' + 'A' : ch; }
...
String *Item_func_soundex::val_str(String *str)
{
  ...
  *to++ = soundex_toupper(*from);  // Copy first letter
  ...
  for (end = (char *)tmp_value.ptr() + 4; to < end; to++) *to = '0';
  *to = 0;  // end string
...
Item_func_format::Item_func_format(Item *org, int dec) : Item_str_func(org) { decimals = (uint)set_zone(dec, 0, 30); }
...
String *Item_func_format::val_str(String *str)
{
  ...
  if (args[0]->result_type() == DECIMAL_RESULT || args[0]->result_type() == INT_RESULT)
  {
  ...
  if (str_length >= dec + 4)
  {
    ...
    for (pos = (char *)str->ptr() + length - 1; pos != tmp; pos--) pos[0] = pos[-diff];
    while (diff)
    {
      *pos = *(pos - diff);
      pos--;
...
void Item_func_make_set::split_sum_func(THD *thd, Item **ref_pointer_array, List<Item> &fields)
{
...
String *Item_func_char::val_str(String *str)
{
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    ...
    if (!args[i]->null_value)
    {
      ...
      {
      b2:
        str->append((char)(num >> 16));
        goto b1;
      }
      else if (num & 0xFF00L)
      {
      b1:
        str->append((char)(num >> 8));
      }
      str->append((char)num);
    }
  }
  ...
  uint wlen = cs->cset->well_formed_len(cs, str->ptr(), str->ptr() + str->length(), str->length(), &well_formed_error);
  if (wlen < str->length())
  {
    THD *thd = current_thd;
    char hexbuf[7];
    ...
    if (thd->variables.sql_mode & (MODE_STRICT_TRANS_TABLES | MODE_STRICT_ALL_TABLES))
    {
    ...
    push_warning_printf(thd, level, ER_INVALID_CHARACTER_STRING, ER(ER_INVALID_CHARACTER_STRING), cs->csname, hexbuf);
  }
  return str;
}

inline String *alloc_buffer(String *res, String *str, String *tmp_value, ulong length)
{
...
void Item_func_repeat::fix_length_and_dec()
{
  ...
  if (args[1]->const_item())
  {
    ulonglong max_result_length = ((ulonglong)args[0]->max_length * args[1]->val_int());
    if (max_result_length >= MAX_BLOB_WIDTH)
    {
      max_result_length = MAX_BLOB_WIDTH;
      maybe_null = 1;
...
void Item_func_rpad::fix_length_and_dec()
{
  ...
  if (args[1]->const_item())
  {
    ulonglong length = ((ulonglong)args[1]->val_int() * collation.collation->mbmaxlen);
...
String *Item_func_conv::val_str(String *str)
{
  ...
  if (args[0]->null_value || args[1]->null_value || args[2]->null_value || abs(to_base) > 36 || abs(to_base) < 2 ||
...
void Item_func_set_collation::fix_length_and_dec()
{
  ...
  if (colname == binary_keyword)
    set_collation = get_charset_by_csname(args[0]->collation.collation->csname, MY_CS_BINSORT, MYF(0));
  else
  {
  ...
  if (!set_collation || !my_charset_same(args[0]->collation.collation, set_collation))
  {
    my_error(ER_COLLATION_CHARSET_MISMATCH, MYF(0), colname, args[0]->collation.collation->csname);
    return;
  }
...
void Item_func_set_collation::print(String *str)
{
  ...
  DBUG_ASSERT(args[1]->basic_const_item() && args[1]->type() == Item::STRING_ITEM);
...
String *Item_func_charset::val_str(String *str)
{
  ...
  null_value = 0;
  str->copy(cs->csname, strlen(cs->csname), &my_charset_latin1, collation.collation, &dummy_errors);
  return str;
}
...
String *Item_func_collation::val_str(String *str)
{
  ...
  null_value = 0;
  str->copy(cs->name, strlen(cs->name), &my_charset_latin1, collation.collation, &dummy_errors);
  return str;
}
...
String *Item_func_hex::val_str(String *str)
{
  ...
  if (args[0]->result_type() != STRING_RESULT)
  {
    ...
    if (args[0]->result_type() == REAL_RESULT || args[0]->result_type() == DECIMAL_RESULT)
    {
      double val = args[0]->val_real();
      if ((val <= (double)LONGLONG_MIN) || (val >= (double)(ulonglong)ULONGLONG_MAX))
        dec = ~(longlong)0;
      else
...
String *Item_func_quote::val_str(String *str)
{
  ...
  */
  ...
  static uchar escmask[32] = {0x01, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
String *Item_func_compress::val_str(String *str)
{
  ...
  */
  ...
  if (((uint32)(new_size + 5) <= res->length()) || buffer.realloc((uint32)new_size + 4 + 1))
  {
    null_value = 1;
    return 0;
  }
...
String *Item_func_uncompress::val_str(String *str)
{
  ...
  {
    push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_TOO_BIG_FOR_UNCOMPRESS,
...
String *Item_func_uuid::val_str(String *str)
{
  ...
  if (!uuid_time) /* first UUID() call. initializing data */
  {
    ...
    randominit(&uuid_rand, tmp + (ulong)start_time, tmp + thd->status_var.bytes_sent);
    set_clock_seq_str();
  }
