...
handlerton myisammrg_hton = {"MRG_MYISAM",
                             SHOW_OPTION_YES,
...
ha_myisammrg::ha_myisammrg(TABLE *table_arg) : handler(&myisammrg_hton, table_arg), file(0) {}

static const char *ha_myisammrg_exts[] = {".MRG", NullS};

const char **ha_myisammrg::bas_ext() const { return ha_myisammrg_exts; }
...
int ha_myisammrg::open(const char *name, int mode, uint test_if_locked)
{
  ...
  if (!(file = myrg_open(fn_format(name_buff, name, "", "", 2 | 4), mode, test_if_locked)))
  {
  ...
  {
    DBUG_PRINT("error", ("reclength: %d  mean_rec_length: %d", table->s->reclength, mean_rec_length));
    goto err;
  }
...
int ha_myisammrg::close(void) { return myrg_close(file); }
...
int ha_myisammrg::index_read(byte *buf, const byte *key, uint key_len, enum ha_rkey_function find_flag)
{
...
int ha_myisammrg::index_read_idx(byte *buf, uint index, const byte *key, uint key_len, enum ha_rkey_function find_flag)
{
  statistic_increment(table->in_use->status_var.ha_read_key_count, &LOCK_status);
...
{
  statistic_increment(table->in_use->status_var.ha_read_key_count, &LOCK_status);
  int error = myrg_rkey(file, buf, active_index, key, key_len, HA_READ_PREFIX_LAST);
  table->status = error ? STATUS_NOT_FOUND : 0;
  return error;
}
...
int ha_myisammrg::index_next_same(byte *buf, const byte *key __attribute__((unused)),
...
int ha_myisammrg::rnd_init(bool scan) { return myrg_extra(file, HA_EXTRA_RESET, 0); }
...
ha_rows ha_myisammrg::records_in_range(uint inx, key_range *min_key, key_range *max_key)
{
...
int ha_myisammrg::external_lock(THD *thd, int lock_type) { return myrg_lock_database(file, lock_type); }

uint ha_myisammrg::lock_count(void) const { return file->tables; }

THR_LOCK_DATA **ha_myisammrg::store_lock(THD *thd, THR_LOCK_DATA **to, enum thr_lock_type lock_type)
{
...
static void split_file_name(const char *file_name, LEX_STRING *db, LEX_STRING *name)
{
...
int ha_myisammrg::create(const char *name, register TABLE *form, HA_CREATE_INFO *create_info)
{
  char buff[FN_REFLEN];
  ...
  if (!(table_names = (const char **)thd->alloc((create_info->merge_list.elements + 1) * sizeof(char *))))
  ...
  for (pos = table_names; tables; tables = tables->next_local)
  {
    ...
    if (!tbl)
    {
      ...
      */
      uint length = my_snprintf(buff, FN_REFLEN, "%s/%s/%s", mysql_data_home, tables->db, tables->table_name);
      /*
      ...
      */
      ...
      else if (!(table_name = thd->strmake(buff, length)))
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
    else
  ...
  *pos = 0;
  DBUG_RETURN(myrg_create(fn_format(buff, name, "", "", 2 + 4 + 16), table_names, create_info->merge_insert_method,
                          (my_bool)0));
}
...
void ha_myisammrg::append_create_info(String *packet)
{
  ...
  for (first = open_table = file->open_tables; open_table != file->end_table; open_table++)
  {
    LEX_STRING db, name;
    split_file_name(open_table->table->filename, &db, &name);
    if (open_table != first)
