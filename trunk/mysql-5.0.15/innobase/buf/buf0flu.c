    ...
    buf_block_t *block) /* in: block which is modified */
{
  ...
  ut_ad(
      (UT_LIST_GET_FIRST(buf_pool->flush_list) == NULL) ||
    ...
    buf_block_t *block) /* in: block which is modified */
{
  ...
  while (b && (ut_dulint_cmp(b->oldest_modification, block->oldest_modification) > 0))
  {
    prev_b = b;
                        ...
                        BUF_BLOCK_FILE_PAGE and in the LRU list */
{
  ...
  if (block->state != BUF_BLOCK_FILE_PAGE)
  {
    ...
    fprintf(stderr, "  InnoDB: Error: buffer block state %lu in the LRU list!\n", (ulong)block->state);
    ut_print_buf(stderr, (byte *)block, sizeof(buf_block_t));

    return (FALSE);
  }

  if ((ut_dulint_cmp(block->oldest_modification, ut_dulint_zero) > 0) || (block->buf_fix_count != 0) ||
      (block->io_fix != 0))
  {
    return (FALSE);
  }
...
/*===========================*/
{
  ...
  for (i = 0; i < trx_doublewrite->first_free; i++)
  {
    ...
    if (mach_read_from_4(block->frame + FIL_PAGE_LSN + 4) !=
        mach_read_from_4(block->frame + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4))
    {
      ut_print_timestamp(stderr);
      fprintf(stderr,
    ...
    if (block->check_index_page_at_flush && !page_simple_validate(block->frame))
    {
  ...
  fil_io(OS_FILE_WRITE, TRUE, TRX_SYS_SPACE, trx_doublewrite->block1, 0, len, (void *)trx_doublewrite->write_buf, NULL);
  ...
  for (len2 = 0; len2 + UNIV_PAGE_SIZE <= len; len2 += UNIV_PAGE_SIZE)
  {
    if (mach_read_from_4(write_buf + len2 + FIL_PAGE_LSN + 4) !=
        mach_read_from_4(write_buf + len2 + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4))
    {
      ut_print_timestamp(stderr);
      fprintf(stderr,
              "  InnoDB: ERROR: The page to be written seems corrupt!\n"
  ...
  for (i = 0; i < trx_doublewrite->first_free; i++)
  {
        ...
        mach_read_from_4(block->frame + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4))
    {
      ...
      fprintf(stderr,
              ...
              (ulong)block->buf_fix_count, (ulong)block->io_fix, (ulong)block->state);
    }
    ut_a(block->state == BUF_BLOCK_FILE_PAGE);
    ...
    ulint page_no)     /* in: page number */
{
  ...
  mach_write_to_4(page + FIL_PAGE_SPACE_OR_CHKSUM,
                  srv_use_checksums ? buf_calc_page_new_checksum(page) : BUF_NO_CHECKSUM_MAGIC);
...
static void buf_flush_write_block_low(
    /*======================*/
    buf_block_t *block) /* in: buffer block to write */
{
...
of writes! */
static ulint buf_flush_try_page(
    /*===============*/
                      ...
                      BUF_FLUSH_SINGLE_PAGE */
{
  ...
  if (flush_type == BUF_FLUSH_LIST && block && buf_flush_ready_for_flush(block, flush_type))
  {
    ...
    if (buf_debug_prints)
    {
      fprintf(stderr, "Flushing page space %lu, page no %lu \n", (ulong)block->space, (ulong)block->offset);
    }
#endif /* UNIV_DEBUG */
  ...
  else if (flush_type == BUF_FLUSH_LRU && block && buf_flush_ready_for_flush(block, flush_type))
  {
    /* VERY IMPORTANT:
  ...
  else if (flush_type == BUF_FLUSH_SINGLE_PAGE && block && buf_flush_ready_for_flush(block, flush_type))
  {
    block->io_fix = BUF_IO_WRITE;
    ...
    if (buf_debug_prints)
    {
      fprintf(stderr, "Flushing single page space %lu, page no %lu \n", (ulong)block->space, (ulong)block->offset);
    }
#endif /* UNIV_DEBUG */
    ...
    ulint flush_type) /* in: BUF_FLUSH_LRU or BUF_FLUSH_LIST */
{
  ...
  for (i = low; i < high; i++)
  {
    ...
    if (block && buf_flush_ready_for_flush(block, flush_type) && (i == offset || block->buf_fix_count == 0))
    {
                      ...
                      exceed min_n), otherwise ignored */
{
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      if (!block || (ut_dulint_cmp(block->oldest_modification, lsn_limit) >= 0))
      {
...
/*=======================*/
{
  ...
  if (n_to_flush > 0)
  {
    n_flushed = buf_flush_batch(BUF_FLUSH_LRU, n_to_flush, ut_dulint_zero);
