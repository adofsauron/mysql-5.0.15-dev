...
static const dec1 powers10[DIG_PER_DEC1 + 1] = {1,      10,      100,      1000,      10000,
                                                100000, 1000000, 10000000, 100000000, 1000000000};
static const int dig2bytes[DIG_PER_DEC1 + 1] = {0, 1, 1, 2, 2, 3, 3, 4, 4, 4};
static const dec1 frac_max[DIG_PER_DEC1 - 1] = {900000000, 990000000, 999000000, 999900000,
                                                999990000, 999999000, 999999900, 999999990};

#ifdef HAVE_purify
#define sanity(d) DBUG_ASSERT((d)->len > 0)
#else
#define sanity(d) DBUG_ASSERT((d)->len > 0 && ((d)->buf[0] | (d)->buf[(d)->len - 1] | 1))
#endif
  ...
  *to_len = len;
  ...
  if (frac)
  {
    ...
    *s1++ = '.';
    ...
    for (; fill; fill--) *s1++ = filler;
  }

  fill = intg_len - intg;
  if (intg == 0)
    fill--; /* symbol 0 before digital point */
  for (; fill; fill--) *s++ = filler;
  if (intg)
  {
    s += intg;
...
static void digits_bounds(decimal_t *from, int *start_result, int *end_result)
{
  ...
  *start_result = start; /* index of first decimal digit (from 0) */
  ...
  {
    stop = (int)(((buf_end - from->buf) * DIG_PER_DEC1 + (i = ((from->frac - 1) % DIG_PER_DEC1 + 1))));
    i = DIG_PER_DEC1 - i + 1;
  }
  else
  {
...
void do_mini_left_shift(decimal_t *dec, int shift, int beg, int last)
{
  ...
  for (; from < end; from++)
    *from = ((*from % powers10[c_shift]) * powers10[shift] + (*(from + 1)) / powers10[c_shift]);
...
void do_mini_right_shift(decimal_t *dec, int shift, int beg, int last)
{
  ...
  for (; from > end; from--) *from = (*from / powers10[shift] + (*(from - 1) % powers10[shift]) * powers10[c_shift]);
...
int decimal_shift(decimal_t *dec, int shift)
{
  ...
  if ((new_len = ROUND_UP(digits_int) + (new_frac_len = ROUND_UP(digits_frac))) > dec->len)
  {
...
int internal_str2dec(const char *from, decimal_t *to, char **end, my_bool fixed)
{
  ...
  {
    endp = s + 1;
    while (endp < end_of_string && my_isdigit(&my_charset_latin1, *endp)) endp++;
    frac = (int)(endp - s - 1);
  }
  else
  {
...
int decimal2double(decimal_t *from, double *to)
{
  ...
  for (intg = from->intg; intg > 0; intg -= DIG_PER_DEC1) x = x * DIG_BASE + *buf++;
  for (frac = from->frac; frac > 0; frac -= DIG_PER_DEC1, t *= DIG_BASE) x += *buf++ / t;
  *to = from->sign ? -x : x;
  return E_DEC_OK;
}
...
int decimal2bin(decimal_t *from, char *to, int precision, int frac)
{
  ...
  int error = E_DEC_OK, intg = precision - frac, isize1, intg1, intg1x, from_intg, intg0 = intg / DIG_PER_DEC1,
      frac0 = frac / DIG_PER_DEC1, intg0x = intg - intg0 * DIG_PER_DEC1, frac0x = frac - frac0 * DIG_PER_DEC1,
      frac1 = from->frac / DIG_PER_DEC1, frac1x = from->frac - frac1 * DIG_PER_DEC1,
      isize0 = intg0 * sizeof(dec1) + dig2bytes[intg0x], fsize0 = frac0 * sizeof(dec1) + dig2bytes[frac0x],
...
{
  int error = E_DEC_OK, intg = precision - scale, intg0 = intg / DIG_PER_DEC1, frac0 = scale / DIG_PER_DEC1,
      intg0x = intg - intg0 * DIG_PER_DEC1, frac0x = scale - frac0 * DIG_PER_DEC1, intg1 = intg0 + (intg0x > 0),
...
{
  int frac0 = scale > 0 ? ROUND_UP(scale) : scale / DIG_PER_DEC1, frac1 = ROUND_UP(from->frac), round_digit,
      intg0 = ROUND_UP(from->intg), error = E_DEC_OK, len = to->len,
      intg1 = ROUND_UP(from->intg + (((intg0 + frac0) > 0) && (from->buf[0] == DIG_MAX)));
...
int decimal_result_size(decimal_t *from1, decimal_t *from2, char op, int param)
{
  ...
  {
    case '-':
      return ROUND_UP(max(from1->intg, from2->intg)) + ROUND_UP(max(from1->frac, from2->frac));
    case '+':
      return ROUND_UP(max(from1->intg, from2->intg) + 1) + ROUND_UP(max(from1->frac, from2->frac));
    case '*':
      return ROUND_UP(from1->intg + from2->intg) + ROUND_UP(from1->frac) + ROUND_UP(from2->frac);
    case '/':
...
{
  int intg1 = ROUND_UP(from1->intg), intg2 = ROUND_UP(from2->intg), frac1 = ROUND_UP(from1->frac),
      frac2 = ROUND_UP(from2->frac), frac0 = max(frac1, frac2), intg0 = max(intg1, intg2), error;
  dec1 *buf1, *buf2, *buf0, *stop, *stop2, x, carry;

  sanity(to);

  /* is there a need for extra word because of carry ? */
  x = intg1 > intg2 ? from1->buf[0] : intg2 > intg1 ? from2->buf[0] : from1->buf[0] + from2->buf[0];
  ...
  buf1 = intg1 > intg2 ? ((stop = from1->buf) + intg1 - intg2) : ((stop = from2->buf) + intg2 - intg1);
  while (buf1 > stop)
  {
...
{
  dec1 *buf1 = from->buf, *end = buf1 + ROUND_UP(from->intg) + ROUND_UP(from->frac);
  while (buf1 < end)
    if (*buf1++)
      return 0;
  return 1;
...
{
  int intg1 = ROUND_UP(from1->intg), intg2 = ROUND_UP(from2->intg), frac1 = ROUND_UP(from1->frac),
      frac2 = ROUND_UP(from2->frac), intg0 = ROUND_UP(from1->intg + from2->intg), frac0 = frac1 + frac2, error, i, j,
      d_to_move;
  dec1 *buf1 = from1->buf + intg1, *buf2 = from2->buf + intg2, *buf0, *start2, *stop2, *stop1, *start0, carry;
  ...
  for (buf1 += frac1 - 1; buf1 >= stop1; buf1--, start0--)
  {
    ...
    for (; carry; buf0--) ADD(*buf0, *buf0, 0, carry);
  }
...
static int do_div_mod(decimal_t *from1, decimal_t *from2, decimal_t *to, decimal_t *mod, int scale_incr)
{
  int frac1 = ROUND_UP(from1->frac) * DIG_PER_DEC1, prec1 = from1->intg + frac1,
      frac2 = ROUND_UP(from2->frac) * DIG_PER_DEC1, prec2 = from2->intg + frac2, error, i, intg0, frac0, len1, len2,
      dintg, div = (!mod);
  dec1 *buf0, *buf1 = from1->buf, *buf2 = from2->buf, *tmp1, *start2, *stop2, *stop1, *stop0, norm2, carry, *start1,
              dcarry;
...
int decimal_mod(decimal_t *from1, decimal_t *from2, decimal_t *to) { return do_div_mod(from1, from2, 0, to, 0); }
...
void dump_decimal(decimal_t *d)
{
  ...
  for (i = 0; i < ROUND_UP(d->frac) + ROUND_UP(d->intg) - 1; i++) printf("%09d, ", d->buf[i]);
  printf("%09d} */ ", d->buf[i]);
}
...
void test_s2d(const char *s, const char *orig, int ex)
{
  ...
  printf("len=%2d %-30s => res=%d    ", a.len, s1, (res = string2decimal(s, &a, &end)));
...
const char *round_mode[] = {"TRUNCATE", "HALF_EVEN", "HALF_UP", "CEILING", "FLOOR"};
...
void test_pr(const char *s1, int prec, int dec, char filler, const char *orig, int ex)
{
  char s[100], *end;
  char s2[100];
  int slen = sizeof(s2);
