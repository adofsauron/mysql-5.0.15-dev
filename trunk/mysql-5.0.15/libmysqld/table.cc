...
static void frm_error(int error, TABLE *form, const char *name, int errortype, int errarg);
...
int openfrm(THD *thd, const char *name, const char *alias, uint db_stat, uint prgflag, uint ha_open_flags,
            TABLE *outparam)
{
  reg1 uint i;
  reg2 uchar *strpos;
  ...
  if ((file = my_open(fn_format(index_file, name, "", reg_ext, MY_UNPACK_FILENAME), O_RDONLY | O_SHARE, MYF(0))) < 0)
    goto err;
  ...
  */
  ...
  if (!(keyinfo = (KEY *)alloc_root(&outparam->mem_root, n_length + uint2korr(disk_buff + 4))))
    goto err; /* purecov: inspected */
  ...
  {
    *root_ptr = old_root crypted = get_crypt_for_frm();
    *root_ptr = &outparam->mem_root;
    outparam->crypted = 1;
  }
#endif
  ...
  record_offset =
      (ulong)(uint2korr(head + 6) + ((uint2korr(head + 14) == 0xffff ? uint4korr(head + 47) : uint2korr(head + 14))));
  ...
  if ((n_length = uint2korr(head + 55)))
  {
    ...
    if (!(share->connect_string.str = strmake_root(&outparam->mem_root, next_chunk + 2, share->connect_string.length)))
    {
      my_free(buff, MYF(0));
      goto err;
    }
  ...
  */
  ...
  DBUG_PRINT("info", ("i_count: %d  i_parts: %d  index: %d  n_length: %d  int_length: %d  com_length: %d",
                      interval_count, interval_parts, share->keys, n_length, int_length, com_length));

  if (!(field_ptr = (Field **)alloc_root(
            &outparam->mem_root, (uint)((share->fields + 1) * sizeof(Field *) + interval_count * sizeof(TYPELIB) +
                                        (share->fields + interval_parts + keys + 3) * sizeof(my_string) +
  ...
  fix_type_pointers(&int_array, share->intervals, interval_count, &names);
  ...
  else
  {
    ...
    outparam->null_flags = null_pos = (uchar *)(record + 1 + share->reclength - share->null_bytes);
    null_bit_pos = 0;
  }
  ...
  if (use_hash)
    use_hash =
        !hash_init(&share->name_hash, system_charset_info, share->fields, 0, 0, (hash_get_key)get_field_name, 0, 0);
  ...
  for (i = 0; i < share->fields; i++, strpos += field_pack_length, field_ptr++)
  {
    ...
    else
    {
      ...
      if (f_is_binary(pack_flag))
      {
        ...
        */
        ...
        {
          // 3.23 or 4.0 string
          if (!(charset = get_charset_by_csname(share->table_charset->csname, MY_CS_BINSORT, MYF(0))))
            charset = &my_charset_bin;
        }
        else
          charset = &my_charset_bin;
    ...
    if (field_type == FIELD_TYPE_NEWDECIMAL && !share->mysql_version)
    {
      ...
      */
      ...
      field_length = my_decimal_precision_to_length(field_length, decimals, f_is_dec(pack_flag) == 0);
      sql_print_error("Found incompatible DECIMAL field '%s' in %s; Please do \"ALTER TABLE '%s' FORCE\" to fix it!",
                      share->fieldnames.type_names[i], name, share->table_name);
      push_warning_printf(
          thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_CRASHED_ON_USAGE,
          "Found incompatible DECIMAL field '%s' in %s; Please do \"ALTER TABLE '%s' FORCE\" to fix it!",
          share->fieldnames.type_names[i], name, share->table_name);
      share->crashed = 1;  // Marker for CHECK TABLE
    }
#endif

    *field_ptr = reg_field = make_field(record + recpos, (uint32)field_length, null_pos, null_bit_pos, pack_flag,
                                        field_type, charset, geom_type, (Field::utype)MTYP_TYPENR(unireg_type),
                                        (interval_nr ? share->intervals + interval_nr - 1 : (TYPELIB *)0),
                                        share->fieldnames.type_names[i], outparam);
  ...
  *field_ptr = 0;  // End marker
  ...
  if (key_parts)
  {
    uint primary_key = (uint)(find_type((char *)primary_key_name, &share->keynames, 3) - 1);
    ...
    for (uint key = 0; key < share->keys; key++, keyinfo++)
    {
      ...
      if (primary_key >= MAX_KEY && (keyinfo->flags & HA_NOSAME))
      {
        ...
        */
        ...
        for (i = 0; i < keyinfo->key_parts; i++)
        {
          ...
          if (!fieldnr || outparam->field[fieldnr - 1]->null_ptr ||
              outparam->field[fieldnr - 1]->key_length() != key_part[i].length)
          {
      ...
      for (i = 0; i < keyinfo->key_parts; key_part++, i++)
      {
        ...
        {  // Should always be true !
          ...
          if (field->null_ptr)
          {
            key_part->null_offset = (uint)((byte *)field->null_ptr - outparam->record[0]);
          ...
          if (field->type() == FIELD_TYPE_BLOB || field->real_type() == MYSQL_TYPE_VARCHAR)
          {
          ...
          if (field->key_length() != key_part->length)
          {
            ...
            if (field->type() == FIELD_TYPE_NEWDECIMAL)
            {
              ...
              */
              ...
              key_part->store_length -= (uint16)(key_part->length - field->key_length());
              key_part->length = (uint16)field->key_length();
              sql_print_error("Found wrong key definition in %s; Please do \"ALTER TABLE '%s' FORCE \" to fix it!",
                              name, share->table_name);
              push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_CRASHED_ON_USAGE,
            ...
            {  // Create a new field
              field = key_part->field = field->new_field(&outparam->mem_root, outparam);
      ...
      set_if_bigger(share->max_key_length, keyinfo->key_length + keyinfo->key_parts);
      share->total_key_length += keyinfo->key_length;
      /*
        MERGE tables do not have unique indexes. But every key could be
        an unique index on the underlying MyISAM table. (Bug #10400)
      */
      if ((keyinfo->flags & HA_NOSAME) || (ha_option & HA_ANY_INDEX_MAY_BE_UNIQUE))
  ...
  if (new_field_pack_flag <= 1)
  {
    ...
    bfill(share->default_values + (outparam->null_flags - (uchar *)record), null_length, 255);
  }

  if ((reg_field = outparam->found_next_number_field))
  {
    if ((int)(share->next_number_index = (uint)find_ref_key(outparam, reg_field, &share->next_number_key_offset)) < 0)
    {
  ...
  if (share->blob_fields)
  {
    ...
    if (!(share->blob_field = save =
              (uint *)alloc_root(&outparam->mem_root, (uint)(share->blob_fields * sizeof(uint)))))
      goto err;
  ...
  */
  ...
  if (db_stat)
  {
    ...
    if ((ha_err = (outparam->file->ha_open(
             index_file, (db_stat & HA_READ_ONLY ? O_RDONLY : O_RDWR),
             (db_stat & HA_OPEN_TEMPORARY                                                 ? HA_OPEN_TMP_TABLE
              : ((db_stat & HA_WAIT_IF_LOCKED) || (specialflag & SPECIAL_WAIT_IF_LOCKED)) ? HA_OPEN_WAIT_IF_LOCKED
              : (db_stat & (HA_ABORT_IF_LOCKED | HA_GET_INFO))                            ? HA_OPEN_ABORT_IF_LOCKED
                                                                                          : HA_OPEN_IGNORE_IF_LOCKED) |
                 ha_open_flags))))
    {
      /* Set a flag if the table is crashed and it can be auto. repaired */
      share->crashed = ((ha_err == HA_ERR_CRASHED_ON_USAGE) && outparam->file->auto_repair() &&
...
int closefrm(register TABLE *table)
{
  ...
  if (table->field)
  {
    for (Field **ptr = table->field; *ptr; ptr++) delete *ptr;
    table->field = 0;
  }
  delete table->file;
...
int read_string(File file, gptr *to, uint length)
{
  ...
  x_free((gptr)*to);
  if (!(*to = (gptr)my_malloc(length + 1, MYF(MY_WME))) || my_read(file, (byte *)*to, length, MYF(MY_NABP)))
  {
...
ulong make_new_entry(File file, uchar *fileinfo, TYPELIB *formnames, const char *newname)
{
  ...
  if (my_write(file, (byte *)buff, (uint)length + 1, MYF(MY_NABP + MY_WME)) ||
      (names && my_write(file, (byte *)(*formnames->type_names + n_length - 1), names * 4, MYF(MY_NABP + MY_WME))) ||
...
static void frm_error(int error, TABLE *form, const char *name, myf errortype, int errarg)
{
  int err_no;
  char buff[FN_REFLEN];
  ...
  switch (error)
  {
    case 1:
      if (my_errno == ENOENT)
      {
        ...
        my_error(ER_FILE_NOT_FOUND, errortype, fn_format(buff, name, form_dev, reg_ext, 0), my_errno);
      break;
    case 2:
    {
      datext = form->file ? *form->file->bas_ext() : "";
      datext = datext == NullS ? "" : datext;
      err_no = (my_errno == ENOENT) ? ER_FILE_NOT_FOUND : (my_errno == EAGAIN) ? ER_FILE_USED : ER_CANT_OPEN_FILE;
      my_error(err_no, errortype, fn_format(buff, real_name, form_dev, datext, 2), my_errno);
      break;
    }
    case 5:
    {
      ...
      my_printf_error(ER_UNKNOWN_COLLATION, "Unknown collation '%s' in table '%-.64s' definition", MYF(0), csname,
                      real_name);
      break;
    }
    case 6:
    ...
    case 4:
      my_error(ER_NOT_FORM_FILE, errortype, fn_format(buff, name, form_dev, reg_ext, 0));
      break;
  }
  DBUG_VOID_RETURN;
} /* frm_error */
...
void append_unescaped(String *res, const char *pos, uint length)
{
  ...
  for (; pos != end; pos++)
  {
    ...
    uint mblen;
    if (use_mb(default_charset_info) && (mblen = my_ismbchar(default_charset_info, pos, end)))
    {
...
File create_frm(THD *thd, my_string name, const char *db, const char *table, uint reclength, uchar *fileinfo,
                HA_CREATE_INFO *create_info, uint keys)
{
  ...
  */
  ...
  if ((file = my_create(name, CREATE_MODE, create_flags, MYF(0))) >= 0)
  {
    ...
    length = next_io_size((ulong)(IO_SIZE + key_length + reclength + create_info->extra_size));
    ...
    fileinfo[38] = (create_info->default_table_charset ? create_info->default_table_charset->number : 0);
...
bool check_db_name(char *name)
{
  ...
  while (*name)
  {
    ...
    {
      int len = my_ismbchar(system_charset_info, name, name + system_charset_info->mbmaxlen);
      if (len)
      {
        name += len;
        continue;
...
db_type get_table_type(THD *thd, const char *name)
{
  ...
  if (error || head[0] != (uchar)254 || head[1] != 1 ||
      (head[2] != FRM_VER && head[2] != FRM_VER + 1 && (head[2] < FRM_VER + 3 || head[2] > FRM_VER + 4)))
    DBUG_RETURN(DB_TYPE_UNKNOWN);
...
void st_table_list::calc_md5(char *buffer)
{
  ...
  sprintf((char *)buffer, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", digest[0], digest[1],
...
bool st_table_list::setup_ancestor(THD *thd)
{
  ...
  if (!field_translation)
  {
    ...
    {
      if (tbl->ancestor && tbl->setup_ancestor(thd))
      {
        DBUG_RETURN(TRUE);
      }
    }
    ...
    if (!(transl = (Field_translator *)(thd->stmt_arena->alloc(select->item_list.elements * sizeof(Field_translator)))))
    {
      DBUG_RETURN(TRUE);
    }
    ...
    if (view->select_lex.ftfunc_list->elements)
    {
      ...
      List_iterator_fast<Item_func_match> li(*(view->select_lex.ftfunc_list));
      while ((ifm = li++)) current_select->ftfunc_list->push_front(ifm);
    }
  }
  DBUG_RETURN(FALSE);
}
...
bool st_table_list::prep_where(THD *thd, Item **conds, bool no_where_clause)
{
...
bool st_table_list::prep_check_option(THD *thd, uint8 check_opt_type)
{
  ...
  for (TABLE_LIST *tbl = ancestor; tbl; tbl = tbl->next_local)
  {
    ...
    if (tbl->view &&
        tbl->prep_check_option(thd, ((check_opt_type == VIEW_CHECK_CASCADED) ? VIEW_CHECK_CASCADED : VIEW_CHECK_NONE)))
    {
      DBUG_RETURN(TRUE);
    }
  }
  ...
  if (check_option)
  {
    ...
    if (!check_option->fixed && check_option->fix_fields(thd, &check_option) || check_option->check_cols(1))
    {
      DBUG_RETURN(TRUE);
    }
...
int st_table_list::view_check_option(THD *thd, bool ignore_failure)
{
  if (check_option && check_option->val_int() == 0)
  {
    ...
    if (ignore_failure)
    {
      push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_VIEW_CHECK_FAILED, ER(ER_VIEW_CHECK_FAILED),
...
bool st_table_list::check_single_table(st_table_list **table, table_map map, st_table_list *view)
{
  for (TABLE_LIST *tbl = ancestor; tbl; tbl = tbl->next_local)
  {
    if (tbl->table)
...
bool st_table_list::set_insert_values(MEM_ROOT *mem_root)
{
  if (table)
  {
    if (!table->insert_values && !(table->insert_values = (byte *)alloc_root(mem_root, table->s->rec_buff_length)))
      return TRUE;
  }
  else
  {
...
{
  return (view || is_natural_join || is_join_columns_complete || !nested_join);
}
...
Natural_join_column::Natural_join_column(Field_translator *field_param, TABLE_LIST *tab)
{
...
Natural_join_column::Natural_join_column(Field *field_param, TABLE_LIST *tab)
{
...
const char *Natural_join_column::table_name() { return table_ref->alias; }
...
bool Natural_join_column::check_grants(THD *thd, const char *name, uint length)
{
  GRANT_INFO *grant;
  const char *db_name;
...
Item *create_view_field(THD *thd, TABLE_LIST *view, Item **field_ref, const char *name)
{
  ...
  Item *item = new Item_direct_view_ref(&view->view->select_lex.context, field_ref, view->alias, name);
  DBUG_RETURN(item);
}
...
void Field_iterator_natural_join::set(TABLE_LIST *table_ref)
{
  ...
  */
  column_ref_it = new List_iterator_fast<Natural_join_column>(*(table_ref->join_columns));
...
void Field_iterator_table_ref::set_field_iterator()
{
  ...
  */
        ...
        table_ref->join_columns &&
            ...
            ((table_ref->field_translation &&
              table_ref->join_columns->elements ==
                  (ulong)(table_ref->field_translation_end - table_ref->field_translation)) ||
             /* This is stored table or a tmptable view. */
             (!table_ref->field_translation && table_ref->join_columns->elements == table_ref->table->s->fields)));
    field_it = &natural_join_it;
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_natural_join", table_ref->alias));
  }
  /* This is a merge view, so use field_translation. */
  else if (table_ref->field_translation)
  {
    DBUG_ASSERT(table_ref->view && table_ref->effective_algorithm == VIEW_ALGORITHM_MERGE);
    field_it = &view_field_it;
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_view", table_ref->alias));
  }
  ...
  else
  {
    ...
    DBUG_PRINT("info", ("field_it for '%s' is Field_iterator_table", table_ref->alias));
  }
...
const char *Field_iterator_table_ref::table_name()
{
  ...
  DBUG_ASSERT(!strcmp(table_ref->table_name, table_ref->table->s->table_name));
...
Natural_join_column *Field_iterator_table_ref::get_or_create_column_ref(THD *thd, bool *is_created)
{
  ...
  *is_created = TRUE;
  ...
  DBUG_ASSERT(!nj_col->table_field || nj_col->table_ref->table == nj_col->table_field->table);
  return nj_col;
}
