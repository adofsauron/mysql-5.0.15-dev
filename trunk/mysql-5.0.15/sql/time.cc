...
void get_date_from_daynr(long daynr, uint *ret_year, uint *ret_month, uint *ret_day)
{
  ...
  else
  {
    ...
    *ret_month = 1;
    for (month_pos = days_in_month; day_of_year > (uint)*month_pos; day_of_year -= *(month_pos++), (*ret_month)++)
      ;
    *ret_year = year;
...
timestamp_type str_to_datetime_with_warn(const char *str, uint length, TIME *l_time, uint flags)
{
  ...
  ts_type = str_to_datetime(str, length, l_time,
                            (flags | (thd->variables.sql_mode & (MODE_INVALID_DATES | MODE_NO_ZERO_DATE))), &was_cut);
...
bool parse_date_time_format(timestamp_type format_type, const char *format, uint format_length,
                            DATE_TIME_FORMAT *date_time_format)
{
  ...
  */
  dt_pos[0] = dt_pos[1] = dt_pos[2] = dt_pos[3] = dt_pos[4] = dt_pos[5] = dt_pos[6] = dt_pos[7] = 255;
  ...
  /*
    ...
    it's needed.
  */
  if ((format_type == MYSQL_TIMESTAMP_DATETIME && !test_all_bits(part_map, (1 | 2 | 4 | 8 | 16 | 32))) ||
      (format_type == MYSQL_TIMESTAMP_DATE && part_map != (1 | 2 | 4)) ||
      (format_type == MYSQL_TIMESTAMP_TIME && !test_all_bits(part_map, 8 | 16 | 32)) ||
  ...
  if (dt_pos[6] != 255)  // If fractional seconds
  {
    ...
    separator_map = ((separator_map & ((ulong)(1 << pos) - 1)) | ((separator_map & ~((ulong)(1 << pos) - 1)) >> 1));
    if (part_map & 64)
    {
  ...
  */
  ...
  separator_map = ((separator_map & ((ulong)(1 << offset) - 1)) | ((separator_map & ~((ulong)(1 << offset) - 1)) >> 1));
    ...
    case MYSQL_TIMESTAMP_TIME:
      ...
      */
      if (format_length == 6 && !need_p &&
          !my_strnncoll(&my_charset_bin, (const uchar *)format, 6, (const uchar *)format_str, 6))
        return 0;
      ...
      */
      if ((format_length == 12 && !need_p &&
           !my_strnncoll(&my_charset_bin, (const uchar *)format, 12,
                         (const uchar *)known_date_time_formats[INTERNAL_FORMAT].datetime_format, 12)) ||
...
DATE_TIME_FORMAT
*date_time_format_make(timestamp_type format_type, const char *format_str, uint format_length)
{
  DATE_TIME_FORMAT tmp;

  if (format_length && format_length < 255 && !parse_date_time_format(format_type, format_str, format_length, &tmp))
  {
...
DATE_TIME_FORMAT *date_time_format_copy(THD *thd, DATE_TIME_FORMAT *format)
{
  ...
  if (new_format)
  {
    ...
    memcpy((char *)new_format->positions, (char *)format->positions, sizeof(format->positions));
    new_format->time_separator = format->time_separator;
    /* We make the string null terminated for easy printf in SHOW VARIABLES */
    memcpy((char *)new_format->format.str, format->format.str, format->format.length);
...
KNOWN_DATE_TIME_FORMAT known_date_time_formats[6] = {
    {"USA", "%m.%d.%Y", "%Y-%m-%d %H.%i.%s", "%h:%i:%s %p"}, {"JIS", "%Y-%m-%d", "%Y-%m-%d %H:%i:%s", "%H:%i:%s"},
    {"ISO", "%Y-%m-%d", "%Y-%m-%d %H:%i:%s", "%H:%i:%s"},    {"EUR", "%d.%m.%Y", "%Y-%m-%d %H.%i.%s", "%H.%i.%s"},
    {"INTERNAL", "%Y%m%d", "%Y%m%d%H%i%s", "%H%i%s"},        {0, 0, 0, 0}};
...
void make_time(const DATE_TIME_FORMAT *format __attribute__((unused)), const TIME *l_time, String *str)
{
...
void make_date(const DATE_TIME_FORMAT *format __attribute__((unused)), const TIME *l_time, String *str)
{
...
void make_datetime(const DATE_TIME_FORMAT *format __attribute__((unused)), const TIME *l_time, String *str)
{
                                  ...
                                  const char *field_name)
{
  ...
  if (field_name)
    cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff), ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), type_str, str.c_ptr(),
                       field_name, (ulong)thd->row_count);
  else
    cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff), ER(ER_TRUNCATED_WRONG_VALUE), type_str, str.c_ptr());
  push_warning(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE, warn_buff);
}
