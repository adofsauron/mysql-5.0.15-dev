...
int find_ref_key(TABLE *table, Field *field, uint *key_length)
{
  ...
  for (i = 0, key_info = table->key_info; i < (int)table->s->keys; i++, key_info++)
  {
...
void key_copy(byte *to_key, byte *from_record, KEY *key_info, uint key_length)
{
  ...
  for (key_part = key_info->key_part; (int)key_length > 0; key_part++)
  {
    ...
    if (key_part->type == HA_KEYTYPE_BIT)
    {
      ...
      if (field->bit_len)
      {
        uchar bits = get_rec_bits((uchar *)from_record + key_part->null_offset + (key_part->null_bit == 128),
  ...
  for (key_part = key_info->key_part; (int)key_length > 0; key_part++)
  {
    ...
    if (key_part->type == HA_KEYTYPE_BIT)
    {
      ...
      if (field->bit_len)
      {
        ...
        set_rec_bits(bits, to_record + key_part->null_offset + (key_part->null_bit == 128), field->bit_ofs,
                     field->bit_len);
      }
    }
    if (key_part->key_part_flag & HA_BLOB_PART)
    {
      ...
      key_length -= HA_KEY_BLOB_LENGTH;
      ((Field_blob *)key_part->field)->set_ptr((ulong)blob_length, (char *)from_key);
...
bool key_cmp_if_same(TABLE *table, const byte *key, uint idx, uint key_length)
{
  ...
  for (key_part = table->key_info[idx].key_part; key < key_end; key_part++, key += store_length)
  {
    uint length;
    ...
    if (!(key_part->key_type & (FIELDFLAG_NUMBER + FIELDFLAG_BINARY + FIELDFLAG_PACK)))
    {
...
int key_cmp(KEY_PART_INFO *key_part, const byte *key, uint key_length)
{
  ...
  for (const byte *end = key + key_length; key < end; key += store_length, key_part++)
  {
    int cmp;
    store_length = key_part->store_length;
    if (key_part->null_bit)
