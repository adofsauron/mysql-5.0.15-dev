...
bool binlog_init() { return !opt_bin_log; }
...
static int binlog_end_trans(THD *thd, IO_CACHE *trans_log, Log_event *end_ev)
{
  ...
  if (end_ev) error = mysql_bin_log.write(thd, trans_log, end_ev);
...
static int binlog_commit(THD *thd, bool all)
{
  ...
  DBUG_ASSERT(mysql_bin_log.is_open() && (all || !(thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))));
...
File open_binlog(IO_CACHE *log, const char *log_file_name, const char **errmsg)
{
  ...
  if ((file = my_open(log_file_name, O_RDONLY | O_BINARY | O_SHARE, MYF(MY_WME))) < 0)
  {
    sql_print_error("Failed to open log (file '%s', errno %d)", log_file_name, my_errno);
    *errmsg = "Could not open log file";
    goto err;
  }
  if (init_io_cache(log, file, IO_SIZE * 2, READ_CACHE, 0, 0, MYF(MY_WME | MY_DONT_CHECK_FILESIZE)))
  {
...
void setup_windows_event_source()
{
  ...
  dwError =
      RegCreateKey(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\MySQL", &hRegKey);

  /* Name of the PE module that contains the message resource */
  GetModuleFileName(NULL, szPath, MAX_PATH);

  /* Register EventMessageFile */
  dwError = RegSetValueEx(hRegKey, "EventMessageFile", 0, REG_EXPAND_SZ, (PBYTE)szPath, strlen(szPath) + 1);

  /* Register supported event types */
  dwTypes = (EVENTLOG_ERROR_TYPE | EVENTLOG_WARNING_TYPE | EVENTLOG_INFORMATION_TYPE);
  dwError = RegSetValueEx(hRegKey, "TypesSupported", 0, REG_DWORD, (LPBYTE)&dwTypes, sizeof dwTypes);
...
static int find_uniq_filename(char *name)
{
  ...
  *end = '.';
  ...
  {
    if (bcmp(file_info->name, start, length) == 0 && test_if_number(file_info->name + length, &number, 0))
    {
...
void MYSQL_LOG::init(enum_log_type log_type_arg, enum cache_type io_cache_type_arg, bool no_auto_events_arg,
                     ulong max_size_arg)
{
...
const char *MYSQL_LOG::generate_name(const char *log_name, const char *suffix, bool strip_ext, char *buff)
{
...
bool MYSQL_LOG::open_index_file(const char *index_file_name_arg, const char *log_name)
{
  ...
  */
  ...
  if ((index_file_nr = my_open(index_file_name, O_RDWR | O_CREAT | O_BINARY, MYF(MY_WME))) < 0 ||
      my_sync(index_file_nr, MYF(MY_WME)) ||
      init_io_cache(&index_file, index_file_nr, IO_SIZE, WRITE_CACHE, my_seek(index_file_nr, 0L, MY_SEEK_END, MYF(0)),
...
bool MYSQL_LOG::open(const char *log_name, enum_log_type log_type_arg, const char *new_name,
                     enum cache_type io_cache_type_arg, bool no_auto_events_arg, ulong max_size_arg,
                     bool null_created_arg)
{
  ...
  db[0] = 0;
  open_count++;
  if ((file = my_open(log_file_name, open_flags, MYF(MY_WME | ME_WAITTANG))) < 0 ||
      init_io_cache(&log_file, file, IO_SIZE, io_cache_type, my_tell(file, MYF(MY_WME)), 0,
                    MYF(MY_WME | MY_NABP | ((log_type == LOG_BIN) ? MY_WAIT_IF_FULL : 0))))
    goto err;
  ...
  switch (log_type)
  {
    case LOG_NORMAL:
    {
      ...
      if (my_b_write(&log_file, (byte *)buff, (uint)(end - buff)) || flush_io_cache(&log_file)) goto err;
      break;
    }
    case LOG_NEW:
    {
      ...
      len = my_snprintf(buff, sizeof(buff), "# %s, Version: %s at %02d%02d%02d %2d:%02d:%02d\n", my_progname,
                        server_version, tm_tmp.tm_year % 100, tm_tmp.tm_mon + 1, tm_tmp.tm_mday, tm_tmp.tm_hour,
                        tm_tmp.tm_min, tm_tmp.tm_sec);
      if (my_b_write(&log_file, (byte *)buff, len) || flush_io_cache(&log_file)) goto err;
      break;
    }
    case LOG_BIN:
    {
      ...
      if (!my_b_filelength(&log_file))
      {
        ...
        */
        if (my_b_safe_write(&log_file, (byte *)BINLOG_MAGIC, BIN_LOG_HEADER_SIZE)) goto err;
        bytes_written += BIN_LOG_HEADER_SIZE;
        write_file_name_to_index_file = 1;
      }

      DBUG_ASSERT(my_b_inited(&index_file) != 0);
      reinit_io_cache(&index_file, WRITE_CACHE, my_b_filelength(&index_file), 0, 0);
      if (need_start_event && !no_auto_events)
      {
        ...
        */
        if (io_cache_type == WRITE_CACHE) s.flags |= LOG_EVENT_BINLOG_IN_USE_F;
        if (!s.is_valid()) goto err;
        if (null_created_arg) s.created = 0;
        if (s.write(&log_file)) goto err;
        bytes_written += s.data_written;
      }
      if (description_event_for_queue && description_event_for_queue->binlog_version >= 4)
      {
        /*
        ...
        */
        ...
        if (description_event_for_queue->write(&log_file)) goto err;
        bytes_written += description_event_for_queue->data_written;
      }
      if (flush_io_cache(&log_file) || my_sync(log_file.file, MYF(MY_WME))) goto err;
      ...
      if (write_file_name_to_index_file)
      {
        ...
        */
        if (my_b_write(&index_file, (byte *)log_file_name, strlen(log_file_name)) ||
            my_b_write(&index_file, (byte *)"\n", 1) || flush_io_cache(&index_file) ||
...
static bool copy_up_file_and_fill(IO_CACHE *index_file, my_off_t offset)
{
  ...
  for (;; offset += bytes_read)
  {
    ...
    if ((bytes_read = (int)my_read(file, io_buf, sizeof(io_buf), MYF(MY_WME))) < 0) goto err;
    if (!bytes_read) break;  // end of file
    (void)my_seek(file, offset - init_offset, MY_SEEK_SET, MYF(0));
    if (my_write(file, (byte *)io_buf, bytes_read, MYF(MY_WME | MY_NABP))) goto err;
  }
  /* The following will either truncate the file or fill the end with \n' */
  if (my_chsize(file, offset - init_offset, '\n', MYF(MY_WME)) || my_sync(file, MYF(MY_WME))) goto err;
...
int MYSQL_LOG::find_log_pos(LOG_INFO *linfo, const char *log_name, bool need_lock)
{
  int error = 0;
  ...
  */
  if (need_lock) pthread_mutex_lock(&LOCK_index);
  ...
  if (need_lock) pthread_mutex_unlock(&LOCK_index);
  DBUG_RETURN(error);
}
...
int MYSQL_LOG::find_next_log(LOG_INFO *linfo, bool need_lock)
{
  ...
  /* As the file is flushed, we can't get an error here */
  (void)reinit_io_cache(&index_file, READ_CACHE, linfo->index_file_offset, 0, 0);
...
bool MYSQL_LOG::reset_logs(THD *thd)
{
  ...
  */
  ...
  for (;;)
  {
    ...
    if (find_next_log(&linfo, 0)) break;
  }

  /* Start logging with a new file */
  close(LOG_CLOSE_INDEX);
  my_delete(index_file_name, MYF(MY_WME));  // Reset (open will update)
  if (!thd->slave_thread) need_start_event = 1;
  if (!open_index_file(index_file_name, 0))
    open(save_name, save_log_type, 0, io_cache_type, no_auto_events, max_size, 0);
...
int MYSQL_LOG::purge_first_log(struct st_relay_log_info *rli, bool included)
{
  ...
  rli->relay_log.purge_logs(rli->group_relay_log_name, included, 0, 0, &rli->log_space_total);
  ...
  */
  if ((included && (error = find_log_pos(&rli->linfo, NullS, 0))) ||
      (!included &&
       ((error = find_log_pos(&rli->linfo, rli->event_relay_log_name, 0)) || (error = find_next_log(&rli->linfo, 0)))))
  {
    char buff[22];
    sql_print_error("next log error: %d  offset: %s  log: %s included: %d", error,
                    llstr(rli->linfo.index_file_offset, buff), rli->group_relay_log_name, included);
    goto err;
  }
...
{
  if (copy_up_file_and_fill(&index_file, log_info->index_file_start_offset)) return LOG_INFO_IO;

  // now update offsets in index file for running threads
  if (need_update_threads) adjust_linfo_offsets(log_info->index_file_start_offset);
  return 0;
}
...
int MYSQL_LOG::purge_logs(const char *to_log, bool included, bool need_mutex, bool need_update_threads,
                          ulonglong *decrease_log_space)
{
  ...
  if (need_mutex) pthread_mutex_lock(&LOCK_index);
  if ((error = find_log_pos(&log_info, to_log, 0 /*no mutex*/))) goto err;
  ...
  */
  ...
  while ((strcmp(to_log, log_info.log_file_name) || (exit_loop = included)) && !log_in_use(log_info.log_file_name))
  {
    ulong file_size = 0;
    if (decrease_log_space)  // stat the file we want to delete
    {
      ...
      */
      if (my_stat(log_info.log_file_name, &s, MYF(0)))
        ...
        sql_print_information("Failed to execute my_stat on file '%s'", log_info.log_file_name);
    }
    /*
    ...
    */
    ...
    if (!my_delete(log_info.log_file_name, MYF(0)) && decrease_log_space) *decrease_log_space -= file_size;
    if (find_next_log(&log_info, 0) || exit_loop) break;
  }
...
err:
  if (need_mutex) pthread_mutex_unlock(&LOCK_index);
  DBUG_RETURN(error);
}
...
int MYSQL_LOG::purge_logs_before_date(time_t purge_time)
{
  ...
  */
  ...
  while (strcmp(log_file_name, log_info.log_file_name) && !log_in_use(log_info.log_file_name))
  {
    /* It's not fatal even if we can't delete a log file */
    if (!my_stat(log_info.log_file_name, &stat_area, MYF(0)) || stat_area.st_mtime >= purge_time) break;
    my_delete(log_info.log_file_name, MYF(0));
    if (find_next_log(&log_info, 0)) break;
  }
...
bool MYSQL_LOG::is_active(const char *log_file_name_arg) { return !strcmp(log_file_name, log_file_name_arg); }
...
void MYSQL_LOG::new_file(bool need_lock)
{
  ...
  */
  if (prepared_xids)
  {
    ...
    while (prepared_xids) pthread_cond_wait(&COND_prep_xids, &LOCK_prep_xids);
  ...
  */
  if (generate_new_name(new_name, name)) goto end;
  ...
  if (log_type == LOG_BIN)
  {
    if (!no_auto_events)
    {
      ...
      */
      ...
      Rotate_log_event r(thd, new_name + dirname_length(new_name), 0, LOG_EVENT_OFFSET, 0);
      r.write(&log_file);
  ...
  */
  ...
  open(old_name, save_log_type, new_name_ptr, io_cache_type, no_auto_events, max_size, 1);
...
bool MYSQL_LOG::append(Log_event *ev)
{
  ...
  */
  ...
  if ((uint)my_b_append_tell(&log_file) > max_size) new_file(0);
...
bool MYSQL_LOG::write(THD *thd, enum enum_server_command command, const char *format, ...)
{
  if (is_open() && (what_to_log & (1L << (uint)command)))
  {
    ...
    if (is_open())
    {
      ...
      {  // Normal thread
        ...
        id = thd->thread_id;
        if (thd->user_time || !(skr = thd->query_start())) skr = time(NULL);  // Connected
      }
      else
      ...
      if (skr != last_time)
      {
        ...
        sprintf(buff, "%02d%02d%02d %2d:%02d:%02d\t", start->tm_year % 100, start->tm_mon + 1, start->tm_mday,
                start->tm_hour, start->tm_min, start->tm_sec);
        if (my_b_write(&log_file, (byte *)buff, 16)) error = errno;
      }
      else if (my_b_write(&log_file, (byte *)"\t\t", 2) < 0)
        error = errno;
      length = my_sprintf(buff, (buff, "%7ld %-11.11s", id, command_name[(uint)command]));
      if (my_b_write(&log_file, (byte *)buff, length)) error = errno;
      if (format)
      {
        if (my_b_write(&log_file, (byte *)" ", 1) || my_b_vprintf(&log_file, format, args) == (uint)-1) error = errno;
      }
      if (my_b_write(&log_file, (byte *)"\n", 1) || flush_io_cache(&log_file)) error = errno;
...
bool MYSQL_LOG::flush_and_sync()
{
  ...
  safe_mutex_assert_owner(&LOCK_log);
  if (flush_io_cache(&log_file)) return 1;
...
bool MYSQL_LOG::write(Log_event *event_info)
{
  ...
  */
  if (likely(is_open()))
  {
    ...
    */
    if ((thd && !(thd->options & OPTION_BIN_LOG)) || (!db_ok(local_db, binlog_do_db, binlog_ignore_db)))
    {
      VOID(pthread_mutex_unlock(&LOCK_log));
    ...
    */
    if (opt_using_transactions && thd)
    {
      ...
      if (event_info->get_cache_stmt())
      {
        if (!trans_log)
        {
          thd->ha_data[binlog_hton.slot] = trans_log = (IO_CACHE *)my_malloc(sizeof(IO_CACHE), MYF(MY_ZEROFILL));
          if (!trans_log || open_cached_file(trans_log, mysql_tmpdir, LOG_PREFIX, binlog_cache_size, MYF(MY_WME)))
          {
          ...
          trans_log->end_of_file = max_binlog_cache_size;
          trans_register_ha(thd, thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN), &binlog_hton);
        }
    ...
    */
    ...
    if (thd)
    {
      ...
      {
        Intvar_log_event e(thd, (uchar)LAST_INSERT_ID_EVENT, thd->current_insert_id);
        if (e.write(file)) goto err;
      }
      ...
      if (thd->user_var_events.elements)
      {
        for (uint i = 0; i < thd->user_var_events.elements; i++)
        {
          ...
          User_var_log_event e(thd, user_var_event->user_var_event->name.str,
                               user_var_event->user_var_event->name.length, user_var_event->value,
                               user_var_event->length, user_var_event->type, user_var_event->charset_number);
          if (e.write(file)) goto err;
        }
      }
    }
    ...
    if (event_info->write(file)) goto err;

    if (file == &log_file)  // we are writing to the real log (disk)
    {
      if (flush_and_sync()) goto err;
      signal_update();
...
{
  if (!(flags & RP_LOCK_LOG_IS_ALREADY_LOCKED)) pthread_mutex_lock(&LOCK_log);
  if ((flags & RP_FORCE_ROTATE) || (my_b_tell(&log_file) >= (my_off_t)max_size))
  {
    new_file(0);
#ifdef HAVE_REPLICATION
    if (expire_logs_days)
    {
      ...
      if (purge_time >= 0) purge_logs_before_date(purge_time);
    }
#endif
  }
  if (!(flags & RP_LOCK_LOG_IS_ALREADY_LOCKED)) pthread_mutex_unlock(&LOCK_log);
}
  ...
  if (likely(is_open()))  // Should always be true
  {
    ...
    */
    if (thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
    {
      ...
      */
      if (qinfo.write(&log_file)) goto err;
    }
    /* Read from the file used to cache the queries .*/
    if (reinit_io_cache(cache, READ_CACHE, 0, 0, 0)) goto err;
    length = my_b_bytes_in_cache(cache);
    DBUG_EXECUTE_IF("half_binlogged_transaction", length -= 100;);
    do
    {
      /* Write data to the binary log file */
      if (my_b_write(&log_file, cache->read_pos, length)) goto err;
      cache->read_pos = cache->read_end;  // Mark buffer used up
      DBUG_EXECUTE_IF("half_binlogged_transaction", goto DBUG_skip_commit;);
    } while ((length = my_b_fill(cache)));

    if (commit_event->write(&log_file)) goto err;
#ifndef DBUG_OFF
  DBUG_skip_commit:
#endif
    if (flush_and_sync()) goto err;
...
bool MYSQL_LOG::write(THD *thd, const char *query, uint query_length, time_t query_start_arg)
{
  ...
  {  // Safety agains reopen
    ...
    if (!(specialflag & SPECIAL_SHORT_LOG_FORMAT) || query_start_arg)
    {
      ...
      if (current_time != last_time)
      {
        ...
        sprintf(buff, "# Time: %02d%02d%02d %2d:%02d:%02d\n", start->tm_year % 100, start->tm_mon + 1, start->tm_mday,
                start->tm_hour, start->tm_min, start->tm_sec);
        if (my_b_write(&log_file, (byte *)buff, 24)) tmp_errno = errno;
      }
      if (my_b_printf(&log_file, "# User@Host: %s[%s] @ %s [%s]\n", sctx->priv_user ? sctx->priv_user : "",
                      sctx->user ? sctx->user : "", sctx->host ? sctx->host : "", sctx->ip ? sctx->ip : "") == (uint)-1)
        tmp_errno = errno;
    }
    if (query_start_arg)
    {
    ...
    if (end != buff)
    {
      *end++ = ';';
      *end = '\n';
      if (my_b_write(&log_file, (byte *)"SET ", 4) || my_b_write(&log_file, (byte *)buff + 1, (uint)(end - buff)))
        tmp_errno = errno;
    }
    if (!query)
    {
      end = strxmov(buff, "# administrator command: ", command_name[thd->command], NullS);
      query_length = (ulong)(end - buff);
      query = buff;
    }
    if (my_b_write(&log_file, (byte *)query, query_length) || my_b_write(&log_file, (byte *)";\n", 2) ||
...
void MYSQL_LOG::wait_for_update(THD *thd, bool is_slave)
{
  ...
  old_msg = thd->enter_cond(&update_cond, &LOCK_log,
                            is_slave ? "Has read all relay log; waiting for the slave I/O "
                                       "thread to update it"
  ...
  if (opt_error_log)
  {
    ...
    */
    ...
    if (freopen(err_temp, "a+", stdout))
    {
      ...
      if (freopen(log_error_file, "a+", stdout)) freopen(log_error_file, "a+", stderr);
      int fd, bytes;
      char buf[IO_SIZE];
...
#ifdef __NT__
void print_buffer_to_nt_eventlog(enum loglevel level, char *buff, uint length, int buffLen)
{
  HANDLE event;
  char *buffptr;
  LPCSTR *buffmsgptr;
  ...
  if (buffptr != buff) delete[] buffptr;
...
int TC_LOG_MMAP::open(const char *opt_name)
{
  ...
  if ((fd = my_open(logname, O_RDWR, MYF(0))) < 0)
  {
    ...
    if ((fd = my_create(logname, O_RDWR, 0, MYF(MY_WME))) < 0) goto err;
    inited = 1;
    file_length = opt_tc_log_size;
    if (my_chsize(fd, file_length, 0, MYF(MY_WME))) goto err;
  }
  else
  {
    inited = 1;
    ...
    if (file_length == MY_FILEPOS_ERROR || file_length % tc_log_page_size) goto err;
  }
  ...
  if (!(pages = (PAGE *)my_malloc(npages * sizeof(PAGE), MYF(MY_WME | MY_ZEROFILL)))) goto err;
  inited = 3;
...
void TC_LOG_MMAP::get_active_from_pool()
{
  ...
  if (syncing) pthread_mutex_unlock(&LOCK_pool);
}
...
int TC_LOG_MMAP::log(THD *thd, my_xid xid)
{
  ...
  */
  while (unlikely(active && active->free == 0)) pthread_cond_wait(&COND_active, &LOCK_active);

  /* no active page ? take one from the pool */
  if (active == 0) get_active_from_pool();
  ...
  *p->ptr++ = xid;
  ...
  {  // somebody's syncing. let's wait
    ...
    */
    while (p->state == DIRTY && syncing) pthread_cond_wait(&p->cond, &LOCK_sync);
    p->waiters--;
    err = p->state == ERROR;
    if (p->state != DIRTY)  // page was synced
    {
      if (p->waiters == 0) pthread_cond_signal(&COND_pool);  // in case somebody's waiting
...
int TC_LOG_MMAP::recover()
{
  ...
  */
  ...
  if (hash_init(&xids, &my_charset_bin, tc_log_page_size / 3, 0, sizeof(my_xid), 0, 0, MYF(0))) goto err1;

  for (; p < end_p; p++)
  {
    for (my_xid *x = p->start; x < p->end; x++)
      if (*x && my_hash_insert(&xids, (byte *)x)) goto err2;  // OOM
  }

  if (ha_recover(&xids)) goto err2;
...
{
  if (!tc_heuristic_recover) return 0;

  sql_print_information("Heuristic crash recovery mode");
  if (ha_recover(0)) sql_print_error("Heuristic crash recovery failed");
...
int TC_LOG_BINLOG::open(const char *opt_name)
{
...

  {
    ...
    if ((ev = Log_event::read_log_event(&log, 0, &fdle)) && ev->get_type_code() == FORMAT_DESCRIPTION_EVENT &&
...
{
  pthread_mutex_lock(&LOCK_prep_xids);
  if (--prepared_xids == 0) pthread_cond_signal(&COND_prep_xids);
  pthread_mutex_unlock(&LOCK_prep_xids);
...
int TC_LOG_BINLOG::recover(IO_CACHE *log, Format_description_log_event *fdle)
{
  ...
  if (!fdle->is_valid() || hash_init(&xids, &my_charset_bin, TC_LOG_PAGE_SIZE / 3, 0, sizeof(my_xid), 0, 0, MYF(0)))
    goto err1;
  ...
  while ((ev = Log_event::read_log_event(log, 0, fdle)) && ev->is_valid())
  {
    if (ev->get_type_code() == XID_EVENT)
    {
      ...
      byte *x = (byte *)memdup_root(&mem_root, (char *)&xev->xid, sizeof(xev->xid));
      if (!x) goto err2;
