...
void init_read_record(READ_RECORD *info, THD *thd, TABLE *table,
  ...
  if (tempfile && my_b_inited(tempfile)) // Test if ref-records was used
  {
    ...
    info->read_record =
        (table->sort.addon_field ? rr_unpack_from_tempfile : rr_from_tempfile);
    ...
    */
    if (!table->sort.addon_field && !(specialflag & SPECIAL_SAFE_MODE) &&
        thd->variables.read_rnd_buff_size &&
        !(table->file->table_flags() & HA_FAST_KEY_READ) &&
        (table->db_stat & HA_READ_ONLY ||
         table->reginfo.lock_type <= TL_READ_NO_INSERT) &&
        (ulonglong)table->s->reclength *
                (table->file->records + table->file->deleted) >
    ...
    info->read_record =
        (table->sort.addon_field ? rr_unpack_from_buffer : rr_from_pointers);
  } else {
...
void end_read_record(READ_RECORD *info) { /* free cache if used */
  if (info->cache) {
...
static int init_rr_cache(THD *thd, READ_RECORD *info) {
  ...
  if (info->cache_records <= 2 ||
      !(info->cache = (byte *)my_malloc_lock(
  ...
  for (;;) {
    ...
    for (i = 0; i < length; i++) {
      ...
      if ((error = (int16)info->file->rnd_pos(record_pos, info->ref_pos))) {
        ...
        DBUG_PRINT("error",
                   ("Got error: %d:%d when reading row", my_errno, error));
      } else
