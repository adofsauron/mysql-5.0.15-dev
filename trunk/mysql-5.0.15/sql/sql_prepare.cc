...
static Prepared_statement *find_prepared_statement(THD *thd, ulong id, const char *where)
{
  /*
  ...
  */
  ...
  {
    char llbuf[22];
    my_error(ER_UNKNOWN_STMT_HANDLER, MYF(0), sizeof(llbuf), llstr(id, llbuf), where);
    return 0;
  }
...
static bool send_prep_stmt(Prepared_statement *stmt, uint columns)
{
  ...
  */
  DBUG_RETURN(my_net_write(net, buff, sizeof(buff)) ||
              (stmt->param_count &&
               stmt->thd->protocol_simple.send_fields((List<Item> *)&stmt->lex->param_list, Protocol::SEND_EOF)));
}
#else
static bool send_prep_stmt(Prepared_statement *stmt, uint columns __attribute__((unused)))
{
  THD *thd = stmt->thd;
...
static void set_param_short(Item_param *param, uchar **pos, ulong len)
{
  ...
  shortget(value, *pos);
#endif
  param->set_int(param->unsigned_flag ? (longlong)((uint16)value) : (longlong)value, 6);
  *pos += 2;
}
...
static void set_param_int32(Item_param *param, uchar **pos, ulong len)
{
  ...
  longget(value, *pos);
#endif
  param->set_int(param->unsigned_flag ? (longlong)((uint32)value) : (longlong)value, 11);
  *pos += 4;
}
...
static void set_param_time(Item_param *param, uchar **pos, ulong len)
{
  ...
  param->set_time(&tm, MYSQL_TIMESTAMP_TIME, MAX_TIME_WIDTH * MY_CHARSET_BIN_MB_MAXLEN);
  *pos += length;
}
...
static void set_param_datetime(Item_param *param, uchar **pos, ulong len)
{
  ...
  param->set_time(&tm, MYSQL_TIMESTAMP_DATETIME, MAX_DATETIME_WIDTH * MY_CHARSET_BIN_MB_MAXLEN);
  *pos += length;
}
...
static void set_param_date(Item_param *param, uchar **pos, ulong len)
{
  ...
  param->set_time(&tm, MYSQL_TIMESTAMP_DATE, MAX_DATE_WIDTH * MY_CHARSET_BIN_MB_MAXLEN);
  *pos += length;
}
...
void set_param_datetime(Item_param *param, uchar **pos, ulong len)
{
  ...
  param->set_time(to, MYSQL_TIMESTAMP_DATETIME, MAX_DATETIME_WIDTH * MY_CHARSET_BIN_MB_MAXLEN);
}
...
void set_param_date(Item_param *param, uchar **pos, ulong len)
{
  ...
  param->set_time(to, MYSQL_TIMESTAMP_DATE, MAX_DATE_WIDTH * MY_CHARSET_BIN_MB_MAXLEN);
}
#endif /*!EMBEDDED_LIBRARY*/
...
static void setup_one_conversion_function(THD *thd, Item_param *param, uchar param_type)
{
  switch (param_type)
  {
    ...
    case MYSQL_TYPE_BLOB:
      ...
      param->value.cs_info.character_set_client = thd->variables.character_set_client;
...
static bool insert_params_withlog(Prepared_statement *stmt, uchar *null_array, uchar *read_pos, uchar *data_end,
                                  String *query)
{
  THD *thd = stmt->thd;
...
static bool insert_params(Prepared_statement *stmt, uchar *null_array, uchar *read_pos, uchar *data_end,
...
static bool setup_conversion_functions(Prepared_statement *stmt, uchar **data, uchar *data_end)
{
  /* skip null bits */
...
static bool insert_params_from_vars(Prepared_statement *stmt, List<LEX_STRING> &varnames,
...
static bool insert_params_from_vars_with_log(Prepared_statement *stmt, List<LEX_STRING> &varnames, String *query)
{
  Item_param **begin = stmt->param_array;
  Item_param **end = begin + stmt->param_count;
  user_var_entry *entry;
  ...
  for (Item_param **it = begin; it < end; ++it)
  {
    ...
    if (entry)
    {
      ...
      *ptr++ = '@';
      *ptr++ = '\'';
      ptr += escape_string_for_mysql(&my_charset_utf8_general_ci, ptr, 0, entry->name.str, entry->name.length);
      *ptr++ = '\'';
...
static bool mysql_test_insert(Prepared_statement *stmt, TABLE_LIST *table_list, List<Item> &fields,
                              List<List_item> &values_list, List<Item> &update_fields, List<Item> &update_values,
                              enum_duplicates duplic)
{
  ...
  */
  ...
  if ((values = its++))
  {
    ...
    if (mysql_prepare_insert(thd, table_list, table_list->table, fields, values, update_fields, update_values, duplic,
                             &unused_conds, FALSE))
      goto error;
    ...
    if (table_list->lock_type == TL_WRITE_DELAYED && !(table_list->table->file->table_flags() & HA_CAN_INSERT_DELAYED))
    {
      my_error(ER_ILLEGAL_HA, MYF(0), (table_list->view ? table_list->view_name.str : table_list->table_name));
      goto error;
    }
    while ((values = its++))
    {
...
static int mysql_test_update(Prepared_statement *stmt, TABLE_LIST *table_list)
{
  int res;
  THD *thd = stmt->thd;
  uint table_count = 0;
  ...
  */
  ...
  if (mysql_prepare_update(thd, table_list, &select->where, select->order_list.elements,
...
static bool mysql_test_delete(Prepared_statement *stmt, TABLE_LIST *table_list)
{
  THD *thd = stmt->thd;
  LEX *lex = stmt->lex;
...
static int mysql_test_select(Prepared_statement *stmt, TABLE_LIST *tables, bool text_protocol)
{
  THD *thd = stmt->thd;
  LEX *lex = stmt->lex;
  ...
  if (!lex->describe && !text_protocol)
  {
    ...
    */
    if (send_prep_stmt(stmt, lex->result->field_count(fields)) ||
        lex->result->send_fields(fields, Protocol::SEND_EOF) || thd->protocol->flush())
      goto error;
    DBUG_RETURN(2);
  }
  DBUG_RETURN(0);
...
static bool mysql_test_do_fields(Prepared_statement *stmt, TABLE_LIST *tables, List<Item> *values)
{
  THD *thd = stmt->thd;
...
static bool select_like_stmt_test(Prepared_statement *stmt, bool (*specific_prepare)(THD *thd),
...
static bool select_like_stmt_test_with_open_n_lock(Prepared_statement *stmt, TABLE_LIST *tables,
                                                   bool (*specific_prepare)(THD *thd), ulong setup_tables_done_option)
{
...
static bool mysql_test_multidelete(Prepared_statement *stmt, TABLE_LIST *tables)
{
  ...
  if (multi_delete_precheck(stmt->thd, tables) ||
      select_like_stmt_test_with_open_n_lock(stmt, tables, &mysql_multi_delete_prepare, OPTION_SETUP_TABLES_DONE))
    goto error;
  if (!tables->table)
  {
    my_error(ER_VIEW_DELETE_MERGE_VIEW, MYF(0), tables->view_db.str, tables->view_name.str);
    goto error;
  }
  return FALSE;
error:
...
static bool mysql_insert_select_prepare_tester(THD *thd)
{
  ...
  first_select->table_list.first = (byte *)(first = ((TABLE_LIST *)first_select->table_list.first)->next_local);
...
static bool mysql_test_insert_select(Prepared_statement *stmt, TABLE_LIST *tables)
{
  int res;
  LEX *lex = stmt->lex;
  ...
  res = select_like_stmt_test_with_open_n_lock(stmt, tables, &mysql_insert_select_prepare_tester,
...
static bool check_prepared_statement(Prepared_statement *stmt, bool text_protocol)
{
  THD *thd = stmt->thd;
  LEX *lex = stmt->lex;
  SELECT_LEX *select_lex = &lex->select_lex;
  ...
  DBUG_PRINT("enter", ("command: %d, param_count: %ld", sql_command, stmt->param_count));

  lex->first_lists_tables_same();
  tables = lex->query_tables;

  /* set context for commands which do not use setup_tables */
  lex->select_lex.context.resolve_in_table_list_only(select_lex->get_table_list());

  switch (sql_command)
  {
    case SQLCOM_REPLACE:
    case SQLCOM_INSERT:
      res = mysql_test_insert(stmt, tables, lex->field_list, lex->many_values, select_lex->item_list, lex->value_list,
                              lex->duplicates);
      break;
...
static const char *get_dynamic_sql_string(LEX *lex, uint *query_len)
{
  ...
  if (lex->prepared_stmt_code_is_varref)
  {
    ...
    */
    if ((entry = (user_var_entry *)hash_search(&thd->user_vars, (byte *)lex->prepared_stmt_code.str,
    ...
    {
      uint dummy_errors;
      len = copy_and_convert(query_str, len, to_cs, var_value->ptr(), var_value->length(), var_value->charset(),
                             &dummy_errors);
    }
    else
...
static void init_stmt_after_parse(LEX *lex)
{
  ...
  */
  for (; sl; sl = sl->next_select_in_list()) sl->uncacheable &= ~UNCACHEABLE_PREPARE;
}
...
void reinit_stmt_before_use(THD *thd, LEX *lex)
{
  ...
  for (; sl; sl = sl->next_select_in_list())
  {
    if (!sl->first_execution)
    {
      ...
      */
      ...
      ORDER *order;
      /* Fix GROUP list */
      for (order = (ORDER *)sl->group_list.first; order; order = order->next) order->item = &order->item_ptr;
      /* Fix ORDER list */
  ...
  */
  for (TABLE_LIST *tables = lex->query_tables; tables; tables = tables->next_global)
  {
    /*
...
void mysql_stmt_execute(THD *thd, char *packet, uint packet_length)
{
  ...
  if (stmt->param_count)
  {
    ...
    if (setup_conversion_functions(stmt, (uchar **)&packet, packet_end) ||
        stmt->set_params(stmt, null_array, (uchar *)packet, packet_end, &expanded_query))
...
void mysql_sql_stmt_execute(THD *thd)
{
  ...
  if (!(stmt = (Prepared_statement *)thd->stmt_map.find_by_name(name)))
  {
    my_error(ER_UNKNOWN_STMT_HANDLER, MYF(0), name->length, name->str, "EXECUTE");
    DBUG_VOID_RETURN;
  }
...
void mysql_sql_stmt_close(THD *thd)
{
  ...
  if (!(stmt = (Prepared_statement *)thd->stmt_map.find_by_name(name)))
  {
    my_error(ER_UNKNOWN_STMT_HANDLER, MYF(0), name->length, name->str, "DEALLOCATE PREPARE");
    return;
  }
...
void mysql_stmt_get_longdata(THD *thd, char *packet, ulong packet_length)
{
  ...
  if (param_number >= stmt->param_count)
  {
    ...
    sprintf(stmt->last_error, ER(ER_WRONG_ARGUMENTS), "mysql_stmt_send_long_data");
    DBUG_VOID_RETURN;
  }
#endif
...
Select_fetch_protocol_prep::Select_fetch_protocol_prep(THD *thd) : protocol(thd) {}
...
bool Select_fetch_protocol_prep::send_data(List<Item> &fields)
{
  Protocol *save_protocol = thd->protocol;
  bool rc;
...
Prepared_statement::Prepared_statement(THD *thd_arg, Protocol *protocol_arg)
    : Statement(INITIALIZED, ++thd_arg->statement_id_counter, thd_arg->variables.query_alloc_block_size,
...
void Prepared_statement::setup_set_params()
{
  ...
  if (mysql_bin_log.is_open() && is_update_query(lex->sql_command) || mysql_log.is_open() || mysql_slow_log.is_open())
  {
...
Query_arena::Type Prepared_statement::type() const { return PREPARED_STATEMENT; }
...
bool Prepared_statement::prepare(const char *packet, uint packet_len)
{
  ...
  */
  ...
  error = yyparse((void *)thd) || thd->is_fatal_error || thd->net.report_error || init_param_array(this);
  /*
...
bool Prepared_statement::execute(String *expanded_query, bool open_cursor)
{
  ...
  */
  ...
  DBUG_ASSERT(thd->free_list == NULL);
  thd->set_n_backup_statement(this, &stmt_backup);
  if (expanded_query->length() && alloc_query(thd, (char *)expanded_query->ptr(), expanded_query->length() + 1))
  {
  ...
  */
  ...
  error = (open_cursor ? mysql_open_cursor(thd, (uint)ALWAYS_MATERIALIZED_CURSOR, &result, &cursor)
