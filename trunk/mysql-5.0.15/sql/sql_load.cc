...
public:
  ...
  READ_INFO(File file, uint tot_length, CHARSET_INFO *cs, String &field_term,
            String &line_start, String &line_term, String &enclosed, int escape,
...
static bool write_execute_load_query_log_event(THD *thd, bool duplicates,
                                               bool ignore,
...
bool mysql_load(THD *thd, sql_exchange *ex, TABLE_LIST *table_list,
                List<Item> &fields_vars, List<Item> &set_fields,
                List<Item> &set_values, enum enum_duplicates handle_duplicates,
                bool ignore, bool read_file_from_client) {
  char name[FN_REFLEN];
  File file;
  TABLE *table;
  int error;
  ...
  */
  ...
  if (setup_tables(thd, &thd->lex->select_lex.context,
                   &thd->lex->select_lex.top_join_list, table_list,
                   &unused_conds, &thd->lex->select_lex.leaf_tables, FALSE))
    DBUG_RETURN(-1);
  ...
  */
  ...
  if (!(error = test(read_info.error))) {
    ...
    if (!field_term->length() && !enclosed->length())
      error = read_fixed_length(thd, info, table_list, fields_vars, set_fields,
                                set_values, read_info, skip_lines, ignore);
    else
      error =
          read_sep_field(thd, info, table_list, fields_vars, set_fields,
                         set_values, read_info, *enclosed, skip_lines, ignore);
...
static bool write_execute_load_query_log_event(THD *thd, bool duplicates,
  ...
  Execute_load_query_log_event e(
      thd, thd->query, thd->query_length,
...
int READ_INFO::read_field() {
  ...
  for (;;) {
    ...
    */
    if (!(new_buffer = (byte *)my_realloc(
              (char *)buffer, buff_length + 1 + IO_SIZE, MYF(MY_WME))))
      return (error = 1);
...
#ifdef USE_MB
    if (my_mbcharlen(read_charset, chr) > 1) {
      for (int i = 1; chr != my_b_EOF && i < my_mbcharlen(read_charset, chr);
           i++)
        chr = GET;
      if (chr == escape_char)
        continue;
