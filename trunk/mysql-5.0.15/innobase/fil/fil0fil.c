...
ib_longlong fil_space_get_version(
    /*==================*/
...
ibuf_data_t *fil_space_get_ibuf_data(
    /*====================*/
    ...
    fil_space_t *space)   /* in: space */
{
  ...
  if (node->size == 0)
  {
    ...
    size_bytes = (((ib_longlong)size_high) << 32) + (ib_longlong)size_low;
    ...
    if (size_bytes < FIL_IBD_FILE_INITIAL_SIZE * UNIV_PAGE_SIZE)
    {
      fprintf(stderr,
              ...
              node->name, (ulong)size_high, (ulong)size_low, (ulong)(4 * UNIV_PAGE_SIZE));
    ...
    {
      node->size = (ulint)((size_bytes / (1024 * 1024)) * ((1024 * 1024) / UNIV_PAGE_SIZE));
    }
    else
    {
      node->size = (ulint)(size_bytes / UNIV_PAGE_SIZE);
                      ...
                      cannot close a file */
{
  ...
  if (print_info)
  {
    fprintf(stderr, "InnoDB: fil_sys open file LRU len %lu\n", (ulong)UT_LIST_GET_LEN(system->LRU));
  }
  ...
  while (node != NULL)
  {
    ...
    if (print_info && node->modification_counter != node->flush_counter)
    {
      fputs("InnoDB: cannot close file ", stderr);
      ut_print_filename(stderr, node->name);
      fprintf(stderr, ", because mod_count %ld != fl_count %ld\n", (long)node->modification_counter,
...
static void fil_mutex_enter_and_prepare_for_io(
    /*===============================*/
...
retry:
  ...
  if (space != NULL && space->stop_ios)
  {
    ...
    if (count2 > 20000)
    {
      ...
      fprintf(stderr, " has i/o ops stopped for a long time %lu\n", (ulong)count2);
    }
...
try_again:
  ...
  HASH_SEARCH(name_hash, system->name_hash, ut_fold_string(name), space, 0 == strcmp(name, space->name));
  if (space != NULL)
  {
    ut_print_timestamp(stderr);
    fprintf(stderr,
  ...
  if (space != NULL)
  {
    fprintf(stderr, "InnoDB: Error: trying to add tablespace %lu of name ", (ulong)id);
    ut_print_filename(stderr, name);
    fprintf(stderr,
            "\n"
  ...
  HASH_INSERT(fil_space_t, name_hash, system->name_hash, ut_fold_string(name), space);
    ...
    ulint id) /* in: space id */
{
  ...
  HASH_SEARCH(name_hash, system->name_hash, ut_fold_string(space->name), namespace,
              0 == strcmp(space->name, namespace->name));
  ut_a(namespace);
  ut_a(space == namespace);

  HASH_DELETE(fil_space_t, name_hash, system->name_hash, ut_fold_string(space->name), space);
...
tablespace memory cache. */
static fil_space_t *fil_get_space_for_id_low(
    /*=====================*/
...
ibool fil_check_adress_in_tablespace(
    /*===========================*/
...
static fil_system_t *fil_system_create(
    /*==============*/
...
void fil_set_max_space_id_if_bigger(
    /*===========================*/
    ulint max_id) /* in: maximum known id */
{
  ...
  {
    fprintf(stderr, "InnoDB: Fatal error: max tablespace id is too high, %lu\n", (ulong)max_id);
    ut_a(0);
  }
...
void fil_ibuf_init_at_db_start(void)
/*===========================*/
{
  fil_space_t *space;
...
ibool fil_inc_pending_ibuf_merges(
    /*========================*/
    ...
    ulint id) /* in: space id */
{
  ...
  if (space == NULL)
  {
    fprintf(stderr, "InnoDB: Error: trying to do ibuf merge to a dropped tablespace %lu\n", (ulong)id);
  }
...
void fil_decr_pending_ibuf_merges(
    /*========================*/
    ulint id) /* in: space id */
{
  fil_system_t *system = fil_system;
  ...
  if (space == NULL)
  {
    fprintf(stderr, "InnoDB: Error: decrementing ibuf merge of a dropped tablespace %lu\n", (ulong)id);
  }
    ...
    mtr_t *mtr)           /* in: mini-transaction handle */
{
  ...
  log_ptr = mlog_write_initial_log_record_for_file_op(type, space_id, 0, log_ptr, mtr);
  ...
  */
  ...
  else if (type == MLOG_FILE_RENAME)
  {
    ...
    if (fil_tablespace_exists_in_mem(space_id))
    {
      ...
      if (fil_get_space_id_for_table(new_name) == ULINT_UNDEFINED)
      {
        ...
        ut_a(fil_rename_tablespace(NULL, space_id, new_name));
      }
    }
  }
  else
  {
    ...
    else
    {
      ...
      ut_a(DB_SUCCESS == fil_create_new_single_table_tablespace(&space_id, name, FALSE, FIL_IBD_FILE_INITIAL_SIZE));
    }
  }
...
stop_ibuf_merges:
  ...
  if (space != NULL)
  {
    ...
    else
    {
      ...
      {
        ut_print_timestamp(stderr);
        fputs("  InnoDB: Warning: trying to delete tablespace ", stderr);
        ut_print_filename(stderr, space->name);
        fprintf(stderr,
                ",\n"
  ...
  if (space->n_pending_flushes > 0 || node->n_pending > 0)
  {
    if (count > 1000)
    {
      ...
      fprintf(stderr,
              ...
              "InnoDB: Loop %lu.\n",
              (ulong)space->n_pending_flushes, (ulong)node->n_pending, (ulong)count);
    }
    mutex_exit(&(system->mutex));
    os_thread_sleep(20000);
    ...
    const char *path)   /* in: new name */
{
  ...
  HASH_SEARCH(name_hash, system->name_hash, ut_fold_string(old_name), space2, 0 == strcmp(old_name, space2->name));
  if (space != space2)
  {
  ...
  HASH_SEARCH(name_hash, system->name_hash, ut_fold_string(path), space2, 0 == strcmp(path, space2->name));
  if (space2 != NULL)
  {
  ...
  HASH_INSERT(fil_space_t, name_hash, system->name_hash, ut_fold_string(path), space);
  return (TRUE);
}
...
retry:
  ...
  if (success)
  {
    ...
    fil_op_write_log(MLOG_FILE_RENAME, id, old_name, new_name, &mtr);
    mtr_commit(&mtr);
  }
#endif
  return (success);
                           ...
                           must be >= FIL_IBD_FILE_INITIAL_SIZE */
{
  ...
  if (!ret)
  {
    fputs("InnoDB: Error: could not write the first page to tablespace ", stderr);
    ut_print_filename(stderr, path);
    putc('\n', stderr);
    goto error_exit;
  }
                        ...
                        first page is too high */
{
  ...
  fprintf(stderr,
          ...
          (ulong)space_id, (ulong)ut_dulint_get_high(flush_lsn), (ulong)ut_dulint_get_low(flush_lsn),
          (ulong)ut_dulint_get_high(current_lsn), (ulong)ut_dulint_get_low(current_lsn));
  ut_print_filename(stderr, filepath);
  fputs(".\n", stderr);
  ...
  for (offset = 0; offset < file_size; offset += UNIV_PAGE_SIZE)
  {
    ...
    if (ut_dulint_cmp(mach_read_from_8(page + FIL_PAGE_LSN), current_lsn) > 0)
    {
      /* We have to reset the lsn */
      space_id = mach_read_from_4(page + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID);
      page_no = mach_read_from_4(page + FIL_PAGE_OFFSET);

      buf_flush_init_for_writing(page, current_lsn, space_id, page_no);
      success =
          os_file_write(filepath, file, page, (ulint)(offset & 0xFFFFFFFFUL), (ulint)(offset >> 32), UNIV_PAGE_SIZE);
      if (!success)
      {
        goto func_exit;
      }
...
static char *fil_make_ibbackup_old_name(
    /*=======================*/
    /* out, own: file name */
                          ...
                          including the .ibd extension */
{
  ...
  fil_space_t *space;
#endif
  filepath = mem_alloc(strlen(dbname) + strlen(filename) + strlen(fil_path_to_mysql_datadir) + 3);

  sprintf(filepath, "%s/%s/%s", fil_path_to_mysql_datadir, dbname, filename);
  ...
  if (size < FIL_IBD_FILE_INITIAL_SIZE * UNIV_PAGE_SIZE)
  {
    fprintf(stderr,
            "InnoDB: Error: the size of single-table tablespace file %s\n"
            "InnoDB: is only %lu %lu, should be at least %lu!",
            filepath, (ulong)size_high, (ulong)size_low, (ulong)(4 * UNIV_PAGE_SIZE));
    os_file_close(file);
    mem_free(filepath);
...
ulint fil_load_single_table_tablespaces(void)
/*===================================*/
/* out: DB_SUCCESS or error number */
{
  int ret;
...
"     Looking at file %s\n", fileinfo.name); */
        ...
        if (strlen(fileinfo.name) > 4 && 0 == strcmp(fileinfo.name + strlen(fileinfo.name) - 4, ".ibd"))
        {
          /* The name ends in .ibd; try opening
          the file */
          fil_load_single_table_tablespace(dbinfo.name, fileinfo.name);
        }
      next_file_item:
        ret = fil_file_readdir_next_file(&err, dbpath, dbdir, &fileinfo);
      }
                         ...
                         parameter is ignored */
{
  ...
  if (version != ((ib_longlong)-1) && space->tablespace_version != version)
  {
    mutex_exit(&(system->mutex));
...
memory */
{
  ...
  HASH_SEARCH(name_hash, system->name_hash, ut_fold_string(path), namespace, 0 == strcmp(namespace->name, path));
                           ...
                           than this already, the function does nothing */
{
  ...
  buf_size = ut_min(64, size_after_extend - start_page_no) * UNIV_PAGE_SIZE;
  ...
  {
    ulint n_pages = ut_min(buf_size / UNIV_PAGE_SIZE, size_after_extend - start_page_no);

    offset_high = (start_page_no - file_start_page_no) / (4096 * ((1024 * 1024) / UNIV_PAGE_SIZE));
    offset_low = ((start_page_no - file_start_page_no) % (4096 * ((1024 * 1024) / UNIV_PAGE_SIZE))) * UNIV_PAGE_SIZE;
#ifdef UNIV_HOTBACKUP
    success = os_file_write(node->name, node->handle, buf, offset_low, offset_high, UNIV_PAGE_SIZE * n_pages);
#else
    success = os_aio(OS_FILE_WRITE, OS_AIO_SYNC, node->name, node->handle, buf, offset_low, offset_high,
                     UNIV_PAGE_SIZE * n_pages, NULL, NULL);
#endif
    if (success)
    {
      node->size += n_pages;
    ...
    else
    {
      ...
      n_pages = ((ulint)(os_file_get_size_as_iblonglong(node->handle) / UNIV_PAGE_SIZE)) - node->size;
...
/*======================================*/
{
  ...
  while (space)
  {
    ...
    mutex_exit(&(system->mutex)); /* no need to protect with a
                                  ...
                                  threaded operation */
    error = fil_read(TRUE, space->id, 0, 0, UNIV_PAGE_SIZE, buf, NULL);
    ut_a(error == DB_SUCCESS);

    size_in_header = fsp_get_size_low(buf);

    success = fil_extend_space_to_desired_size(&actual_size, space->id, size_in_header);
    if (!success)
    {
      fprintf(stderr,
...
ulint fil_space_get_n_reserved_extents(
    /*=============================*/
    ulint id) /* in: space id */
{
  fil_system_t *system = fil_system;
    ...
    fil_space_t *space)   /* in: space */
{
  ...
  {
    ut_print_timestamp(stderr);
    fprintf(stderr, "  InnoDB: Warning: open files %lu exceeds the limit %lu\n", (ulong)system->n_open,
                        ...
                        aio used, else ignored */
{
...
#ifdef UNIV_SYNC_DEBUG
  ut_ad(!ibuf_inside() || is_log || (type == OS_FILE_WRITE) || ibuf_page(space_id, block_offset));
#endif
#endif
  if (sync)
  {
  ...
  for (;;)
  {
    if (node == NULL)
    {
      fprintf(stderr,
              ...
              (ulong)block_offset, (ulong)space_id, space->name, (ulong)byte_offset, (ulong)len, (ulong)type);
  ...
  single-table tablespace */
  if (space->purpose == FIL_TABLESPACE && space->id != 0 && node->size <= block_offset)
  {
    fprintf(stderr,
            "InnoDB: Error: trying to access page number %lu in space %lu,\n"
            "InnoDB: space name %s,\n"
  ...
  offset_low = ((block_offset << UNIV_PAGE_SIZE_SHIFT) & 0xFFFFFFFFUL) + byte_offset;
  ...
  {
    ret = os_file_write(node->name, node->handle, buf, offset_low, offset_high, len);
  }
#else
...
ulint fil_page_get_prev(byte *page) { return (mach_read_from_4(page + FIL_PAGE_PREV)); }

ulint fil_page_get_next(byte *page) { return (mach_read_from_4(page + FIL_PAGE_NEXT)); }
