...
static HASH_INFO *hp_find_free_hash(HP_SHARE *info, HP_BLOCK *block, ulong records);
...
int hp_rb_write_key(HP_INFO *info, HP_KEYDEF *keyinfo, const byte *record, byte *recpos)
{
  ...
  if (!tree_insert(&keyinfo->rb_tree, (void *)info->recbuf, custom_arg.key_length, &custom_arg))
  {
...
static byte *next_free_record_pos(HP_SHARE *info)
{
  ...
  DBUG_PRINT("exit", ("Used new position: %lx",
                      (byte *)info->block.level_info[0].last_blocks + block_pos * info->block.recbuffer));
  DBUG_RETURN((byte *)info->block.level_info[0].last_blocks + block_pos * info->block.recbuffer);
}
...
int hp_write_key(HP_INFO *info, HP_KEYDEF *keyinfo, const byte *record, byte *recpos)
{
  ...
  */
  ...
  pos = hp_find_hash(&keyinfo->block, hp_mask(hp_rec_hashnr(keyinfo, record), share->blength, share->records + 1));
  if (pos == empty)
  {
  ...
  else
  {
    ...
    gpos = hp_find_hash(&keyinfo->block,
                        hp_mask(hp_rec_hashnr(keyinfo, pos->ptr_to_rec), share->blength, share->records + 1));
    if (pos == gpos)
    {
      pos->ptr_to_rec = recpos;
      pos->next_key = empty;
    ...
    if ((keyinfo->flag & HA_NOSAME) && pos == gpos &&
        (!(keyinfo->flag & HA_NULL_PART_KEY) || !hp_if_null_in_key(keyinfo, record)))
    {
      pos = empty;
      do
      {
...
static HASH_INFO *hp_find_free_hash(HP_SHARE *info, HP_BLOCK *block, ulong records)
{
  ...
  return ((HASH_INFO *)((byte *)block->level_info[0].last_blocks + block_pos * block->recbuffer));
}
