...
Log_event::Log_event()
    : temp_buf(0), exec_time(0), flags(0), cache_stmt(0), thd(0) {
  server_id = ::server_id;
  when = time(NULL);
  log_pos = 0;
}
...
void Log_event::init_show_field_list(List<Item> *field_list) {
  ...
  field_list->push_back(
      new Item_return_int("End_log_pos", 11, MYSQL_TYPE_LONGLONG));
...
#ifndef MYSQL_CLIENT
Log_event *
Log_event::read_log_event(IO_CACHE *file, pthread_mutex_t *log_lock,
                          const Format_description_log_event *description_event)
#else
Log_event *
Log_event::read_log_event(IO_CACHE *file,
                          const Format_description_log_event *description_event)
#endif
{
  DBUG_ENTER(
  ...
  */
  uint header_size =
      min(description_event->common_header_len, LOG_EVENT_MINIMAL_HEADER_LEN);
...
Log_event *Log_event::read_log_event(
  ...
  default:
    DBUG_PRINT("error",
               ("Unknown evernt code: %d", (int)buf[EVENT_TYPE_OFFSET]));
    ev = NULL;
    break;
  }
...
void Log_event::print_timestamp(FILE *file, time_t *ts) {
  ...
  fprintf(file, "%02d%02d%02d %2d:%02d:%02d", res->tm_year % 100,
...
static void write_str_with_code_and_len(char **dst, const char *src, int len,
                                        uint code) {
  DBUG_ASSERT(src);
  *((*dst)++) = code;
  *((*dst)++) = (uchar)len;
...
bool Query_log_event::write(IO_CACHE *file) {
  uchar buf
      [QUERY_HEADER_LEN + 1 + 4 + // code of flags2 and flags2
  ...
  */
  ...
  {
    write_str_with_code_and_len((char **)(&start), catalog, catalog_len,
                                Q_CATALOG_NZ_CODE);
    /*
...
Query_log_event::Query_log_event(THD *thd_arg, const char *query_arg,
    ...
    : Log_event(thd_arg,
                ...
                using_trans),
      data_buf(0), query(query_arg), catalog(thd_arg->catalog), db(thd_arg->db),
      ...
      slave_proxy_id(thd_arg->variables.pseudo_thread_id), flags2_inited(1),
      sql_mode_inited(1), charset_inited(1),
  ...
  */
  ...
  if (thd_arg->time_zone_used) {
    ...
    */
    ...
    time_zone_len = 0;
  DBUG_PRINT("info",
             ("Query_log_event has flags2=%lu sql_mode=%lu", flags2, sql_mode));
}
#endif /* MYSQL_CLIENT */

/* 2 utility functions for the next method */

static void get_str_len_and_pointer(const char **dst, const char **src,
                                    uint *len) {
  if ((*len = **src))
...
Query_log_event::Query_log_event(
    ...
    : Log_event(buf, description_event), data_buf(0), query(NullS), db(NullS),
      catalog_len(0), status_vars_len(0), flags2_inited(0), sql_mode_inited(0),
      charset_inited(0), auto_increment_increment(1), auto_increment_offset(1),
      time_zone_len(0) {
  ulong data_len;
  uint32 tmp;
 ...
 code: %u), skipping the rest of them",
  ...
  if (!(start = data_buf =
            (char *)my_malloc(catalog_len + 1 + time_zone_len + 1 + data_len +
                                  1 + QUERY_CACHE_FLAGS_SIZE + db_len + 1,
                              MYF(MY_WME))))
#else
  if (!(start = data_buf = (char *)my_malloc(
            catalog_len + 1 + time_zone_len + 1 + data_len + 1, MYF(MY_WME))))
#endif
    DBUG_VOID_RETURN;
...
void Query_log_event::print_query_header(FILE *file, bool short_form,
  ...
  */
  ...
  if (last_event_info->auto_increment_increment != auto_increment_increment ||
    ...
    fprintf(file, "SET @@session.auto_increment_increment=%lu, "
  ...
  if (likely(charset_inited)) {
    if (unlikely(!last_event_info->charset_inited)) /* first Query event */
    {
      ...
      last_event_info->charset[0] =
          ~charset[0]; // force a difference to force write
    }
    if (unlikely(bcmp(last_event_info->charset, charset, 6))) {
      fprintf(file, "SET "
                    ...
                    ";\n",
              uint2korr(charset), uint2korr(charset + 2),
              uint2korr(charset + 4));
      memcpy(last_event_info->charset, charset, 6);
    }
  }
  if (time_zone_len) {
    if (bcmp(last_event_info->time_zone_str, time_zone_str,
             time_zone_len + 1)) {
  ...
  if (db_ok(thd->db, replicate_do_db, replicate_ignore_db)) {
    ...
    if (ignored_error_code((expected_error = error_code)) ||
      ...
      if (flags2_inited)
        ...
        */
        thd->options =
            flags2 | (thd->options & ~(ulong)OPTIONS_WRITTEN_TO_BIN_LOG);
      /*
...
START SLAVE; . Query: '%s'",
   ...
   */
    DBUG_PRINT("info", ("expected_error: %d  last_errno: %d", expected_error,
                        thd->net.last_errno));
...
Start_log_event_v3::Start_log_event_v3()
    : Log_event(), binlog_version(BINLOG_VERSION), artificial_event(0) {
  created = when;
...
#ifdef MYSQL_CLIENT
void Start_log_event_v3::print(FILE *file, bool short_form,
                               LAST_EVENT_INFO *last_event_info) {
  if (!short_form) {
    print_header(file);
...
Start_log_event_v3::Start_log_event_v3(
    const char *buf, const Format_description_log_event *description_event)
...
Format_description_log_event::Format_description_log_event(
  ...
  case 4: /* MySQL 5.0 */
    ...
    */
    if (post_header_len) {
      ...
      post_header_len[FORMAT_DESCRIPTION_EVENT - 1] =
  ...
  case 3: /* 4.0.x x>=2 */
    ...
    */
    ...
    common_header_len =
        binlog_ver == 1 ? OLD_HEADER_LEN : LOG_EVENT_MINIMAL_HEADER_LEN;
    /*
    ...
    */
    ...
    post_header_len =
        (uint8 *)my_malloc(number_of_event_types * sizeof(uint8), MYF(0));
    if (post_header_len) {
      post_header_len[START_EVENT_V3 - 1] = START_V3_HEADER_LEN;
      post_header_len[QUERY_EVENT - 1] = QUERY_HEADER_MINIMAL_LEN;
      post_header_len[STOP_EVENT - 1] = 0;
      post_header_len[ROTATE_EVENT - 1] =
          (binlog_ver == 1) ? 0 : ROTATE_HEADER_LEN;
...
Format_description_log_event::Format_description_log_event(
  ...
  post_header_len = (uint8 *)my_memdup(
      (byte *)buf + ST_COMMON_HEADER_LEN_OFFSET + 1,
...
bool Format_description_log_event::write(IO_CACHE *file) {
  ...
  */
  ...
  memcpy((char *)buff + ST_COMMON_HEADER_LEN_OFFSET + 1,
         (byte *)post_header_len, LOG_EVENT_TYPES);
  return (write_header(file, sizeof(buff)) ||
...
void Load_log_event::print_query(bool need_db, char *buf, char **end,
...
Load_log_event::Load_log_event(THD *thd_arg, sql_exchange *ex,
                               ...
                               bool using_trans)
    : Log_event(thd_arg,
                !thd_arg->tmp_table_used ? 0 : LOG_EVENT_THREAD_SPECIFIC_F,
                using_trans),
      thread_id(thd_arg->thread_id),
      slave_proxy_id(thd_arg->variables.pseudo_thread_id), num_fields(0),
      fields(0), field_lens(0), field_block_len(0),
      table_name(table_name_arg ? table_name_arg : ""), db(db_arg),
      fname(ex->file_name), local_fname(FALSE) {
  time_t end_time;
  time(&end_time);
...
Load_log_event::Load_log_event(
    ...
    : Log_event(buf, description_event), num_fields(0), fields(0),
      field_lens(0), field_block_len(0), table_name(0), db(0), fname(0),
      local_fname(FALSE) {
  ...
  if (event_len)
    copy_log_event(buf, event_len,
                   ((buf[EVENT_TYPE_OFFSET] == LOAD_EVENT)
                        ? LOAD_HEADER_LEN + description_event->common_header_len
...
int Load_log_event::copy_log_event(
    const char *buf, ulong event_len, int body_offset,
    const Format_description_log_event *description_event) {
  ...
  */
  if (!(field_lens =
            (uchar *)sql_ex.init((char *)buf + body_offset, buf_end,
                                 buf[EVENT_TYPE_OFFSET] != LOAD_EVENT)))
    DBUG_RETURN(1);
...
#ifndef MYSQL_CLIENT
void Load_log_event::set_fields(const char *affected_db, List<Item> &field_list,
                                Name_resolution_context *context) {
  uint i;
  const char *field = fields;
  for (i = 0; i < num_fields; i++) {
    field_list.push_back(
        new Item_field(context, affected_db, table_name, field));
    field += field_lens[i] + 1;
  }
}
#endif /* !MYSQL_CLIENT */
      ...
      */
      ...
      if (thd->cuted_fields) {
        ...
        sql_print_warning("Slave: load data infile on table '%s' at "
                          ...
                          (char *)table_name, llstr(log_pos, llbuff),
                          RPL_LOG_NAME, (ulong)thd->cuted_fields,
...
#ifndef MYSQL_CLIENT
Rotate_log_event::Rotate_log_event(THD *thd_arg, const char *new_log_ident_arg,
                                   uint ident_len_arg, ulonglong pos_arg,
                                   uint flags_arg)
    : Log_event(), new_log_ident(new_log_ident_arg), pos(pos_arg),
      ident_len(ident_len_arg ? ident_len_arg
                              : (uint)strlen(new_log_ident_arg)),
      flags(flags_arg) {
#ifndef DBUG_OFF
  char buff[22];
...
int Rotate_log_event::exec_event(struct st_relay_log_info *rli) {
  ...
  */
  if (!(thd->options & OPTION_BEGIN)) {
    ...
    DBUG_PRINT("info",
               ("group_master_log_name: '%s' group_master_log_pos:\
%lu",
...
#ifdef MYSQL_CLIENT
void Rand_log_event::print(FILE *file, bool short_form,
                           LAST_EVENT_INFO *last_event_info) {
...
#ifdef MYSQL_CLIENT
void Xid_log_event::print(FILE *file, bool short_form,
                          LAST_EVENT_INFO *last_event_info) {
  if (!short_form) {
    char buf[64];
...
void User_var_log_event::pack_info(Protocol *protocol) {
  ...
  if (is_null) {
    ...
    case REAL_RESULT:
      ...
      event_len +=
          my_sprintf(buf + val_offset, (buf + val_offset, "%.14g", real_val));
      break;
    case INT_RESULT:
      buf = my_malloc(val_offset + 22, MYF(MY_WME));
      event_len =
          longlong10_to_str(uint8korr(val), buf + val_offset, -10) - buf;
      break;
    case DECIMAL_RESULT: {
    ...
    case STRING_RESULT:
      ...
      buf = my_malloc(event_len + val_len * 2 + 1 + 2 * MY_CS_NAME_SIZE + 15,
                      MYF(MY_WME));
      CHARSET_INFO *cs;
...
#ifdef MYSQL_CLIENT
void User_var_log_event::print(FILE *file, bool short_form,
                               LAST_EVENT_INFO *last_event_info) {
  if (!short_form) {
    print_header(file);
  ...
  if (is_null) {
    ...
    case STRING_RESULT: {
      ...
      */
      ...
      if (!(hex_str =
                (char *)my_alloca(2 * val_len + 1 + 2))) // 2 hex digits / byte
...
#ifdef HAVE_REPLICATION
#ifdef MYSQL_CLIENT
void Unknown_log_event::print(FILE *file, bool short_form,
                              LAST_EVENT_INFO *last_event_info) {
  if (short_form)
    return;
  print_header(file);
...
Slave_log_event::Slave_log_event(THD *thd_arg, struct st_relay_log_info *rli)
  ...
  if ((mem_pool = (char *)my_malloc(get_data_size() + 1, MYF(MY_WME)))) {
    ...
    DBUG_PRINT("info",
               ("master_log: %s  pos: %d", master_log, (ulong)master_pos));
  } else
...
#ifdef MYSQL_CLIENT
void Slave_log_event::print(FILE *file, bool short_form,
                            LAST_EVENT_INFO *last_event_info) {
  char llbuff[22];
  if (short_form)
    return;
...
Create_file_log_event::Create_file_log_event(
    THD *thd_arg, sql_exchange *ex, const char *db_arg,
    ...
    */
    block_offset =
        (description_event->common_header_len +
         Load_log_event::get_data_size() + create_file_header_len + 1);
    if (len < block_offset)
      return;
...
void Create_file_log_event::print(FILE *file, bool short_form,
  ...
  if (enable_local) {
    Load_log_event::print(file, short_form, last_event_info,
...
int Create_file_log_event::exec_event(struct st_relay_log_info *rli) {
  ...
  if ((fd = my_create(fname_buf, CREATE_MODE,
                      O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW, MYF(MY_WME))) <
          0 ||
  ...
  if (write_base(&file)) {
    ...
    slave_print_error(
        rli, my_errno,
        "Error in Create_file event: could not write to file '%s'", fname_buf);
    goto err;
  }
  end_io_cache(&file);
  my_close(fd, MYF(0));
...
void Append_block_log_event::pack_info(Protocol *protocol) {
  char buf[256];
  uint length;
  length = (uint)my_sprintf(
      buf, (buf, ";file_id=%u;block_len=%u", file_id, block_len));
...
int Append_block_log_event::exec_event(struct st_relay_log_info *rli) {
  ...
  if (my_write(fd, (byte *)block, block_len, MYF(MY_WME + MY_NABP))) {
    slave_print_error(rli, my_errno, "Error in %s event: write to '%s' failed",
                      get_type_str(), fname);
    goto err;
  }
  error = 0;
...
Execute_load_log_event::Execute_load_log_event(
    ...
    : Log_event(buf, description_event), file_id(0) {
  uint8 common_header_len = description_event->common_header_len;
  uint8 exec_load_header_len =
      description_event->post_header_len[EXEC_LOAD_EVENT - 1];
...
int Execute_load_log_event::exec_event(struct st_relay_log_info *rli) {
  ...
  memcpy(p, ".info", 6);
  if ((fd = my_open(fname, O_RDONLY | O_BINARY | O_NOFOLLOW, MYF(MY_WME))) <
          0 ||
  ...
  if (!(lev = (Load_log_event *)Log_event::read_log_event(
            &file, (pthread_mutex_t *)0,
            rli->relay_log.description_event_for_exec)) ||
      lev->get_type_code() != NEW_LOAD_EVENT) {
    slave_print_error(
        rli, 0, "Error in Exec_load event: file '%s' appears corrupted", fname);
    goto err;
  }
...
Begin_load_query_log_event::Begin_load_query_log_event(
    const char *buf, uint len, const Format_description_log_event *desc_event)
...
Execute_load_query_log_event::write_post_header_for_derived(IO_CACHE *file) {
  ...
  *(buf + 4 + 4 + 4) = (char)dup_handling;
  return my_b_safe_write(file, (byte *)buf,
...
char *sql_ex_info::init(char *buf, char *buf_end, bool use_new_format) {
  ...
  if (use_new_format) {
    ...
    */
    ...
    field_term_len = enclosed_len = line_term_len = line_start_len =
        escaped_len = 1;
