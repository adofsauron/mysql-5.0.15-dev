...
my_bool mysql_make_view(File_parser *parser, TABLE_LIST *table)
{
  ...
  */
  ...
  /*
    ...
    be used here
  */
  if (parser->parse((gptr)table, thd->mem_root, view_parameters, required_view_parameters))
    goto err;
        ...
        !old_lex->can_not_use_merged())
    {
      ...
      */
...

      {
        ...
        if (!(nested_join = table->nested_join = (NESTED_JOIN *)thd->calloc(sizeof(NESTED_JOIN))))
          goto err;
  ...
  */
  ...
  for (; key_info != key_info_end; key_info++)
  {
    if ((key_info->flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME)
    {
      ...
      for (;;)
      {
        ...
        {
          Item_field *field;
          if ((field = k->item->filed_for_view_update()) && field->field == key_part->field)
            break;
        }
        if (k == end_of_trans)
  ...
  /* check all fields presence */
  {
    ...
    for (field_ptr = table->field; *field_ptr; field_ptr++)
    {
      ...
      {
        Item_field *field;
        if ((field = fld->item->filed_for_view_update()) && field->field == *field_ptr)
          break;
      }
...
bool mysql_rename_view(THD *thd, const char *new_name, TABLE_LIST *view)
{
  ...
  if ((parser = sql_parse_prepare(&pathstr, thd->mem_root, 1)) && is_equal(&view_type, parser->type()))
  {
    ...
    */
    ...
    if (parser->parse((gptr)&view_def, thd->mem_root, view_parameters, array_elements(view_parameters) - 1))
      goto err;

    /* rename view and it's backups */
    if (rename_in_schema_file(view->db, view->table_name, new_name, view_def.revision - 1, num_view_backups))
      goto err;
