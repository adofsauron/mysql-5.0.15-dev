    ...
    byte *read_buf) /* in: a database page */
{
  ...
  {
    if (ut_dulint_cmp(current_lsn, mach_read_from_8(read_buf + FIL_PAGE_LSN)) < 0)
    {
      ...
      fprintf(stderr,
              ...
              (ulong)ut_dulint_get_high(mach_read_from_8(read_buf + FIL_PAGE_LSN)),
  ...
  if (srv_use_checksums)
  {
    ...
    if (old_checksum_field != mach_read_from_4(read_buf + FIL_PAGE_LSN) && old_checksum_field != old_checksum &&
    ...
    if (checksum_field != 0 && checksum_field != checksum && checksum_field != BUF_NO_CHECKSUM_MAGIC)
    {
      return (TRUE);
    }
  }
    ...
    byte *read_buf) /* in: a database page */
{
  ...
  fprintf(stderr,
          ...
          (ulong)checksum, (ulong)old_checksum, (ulong)mach_read_from_4(read_buf + FIL_PAGE_SPACE_OR_CHKSUM),
                     ...
                     as max_size */
{
  ...
  if (srv_use_awe)
  {
    ...
    buf_pool->frame_mem = os_awe_allocate_virtual_mem_window(UNIV_PAGE_SIZE * (n_frames + 1));
    ...
    if (!os_awe_allocate_physical_mem(&(buf_pool->awe_info), curr_size / ((1024 * 1024) / UNIV_PAGE_SIZE)))
    {
      return (NULL);
    }
    /*----------------------------------------*/
  ...
  if (srv_use_awe)
  {
    ...
    os_awe_map_physical_mem_to_window(buf_pool->frame_zero, n_frames * (UNIV_PAGE_SIZE / OS_AWE_X86_PAGE_SIZE),
                                      buf_pool->awe_info);
    /*----------------------------------------*/
  }
  ...
  for (i = 0; i < max_size; i++)
  {
    ...
    {
      /*----------------------------------------*/
      block->awe_info = buf_pool->awe_info + i * (UNIV_PAGE_SIZE / OS_AWE_X86_PAGE_SIZE);
      /*----------------------------------------*/
    }
  }
  ...
  for (i = 0; i < curr_size; i++)
  {
    ...
    if (block->frame)
    {
      ...
      if (srv_use_awe)
      {
        ...
        UT_LIST_ADD_LAST(awe_LRU_free_mapped, buf_pool->awe_LRU_free_mapped, block);
      }
    }
                          ...
                          awe_LRU_free_mapped list */
{
  ...
  while (bck)
  {
    if (bck->state == BUF_BLOCK_FILE_PAGE && (bck->buf_fix_count != 0 || bck->io_fix != 0))
    {
    ...
    else
    {
      ...
      os_awe_map_physical_mem_to_window(bck->frame, UNIV_PAGE_SIZE / OS_AWE_X86_PAGE_SIZE, block->awe_info);

      block->frame = bck->frame;

      *(buf_pool->blocks_of_frames + (((ulint)(block->frame - buf_pool->frame_zero)) >> UNIV_PAGE_SIZE_SHIFT)) = block;

      bck->frame = NULL;
      UT_LIST_REMOVE(awe_LRU_free_mapped, buf_pool->awe_LRU_free_mapped, bck);

      if (add_to_mapped_list)
      {
        UT_LIST_ADD_FIRST(awe_LRU_free_mapped, buf_pool->awe_LRU_free_mapped, block);
      }
...
UNIV_INLINE
buf_block_t *buf_block_alloc(void)
/*=================*/
    ...
    mtr_t *mtr)          /* in: mini-transaction */
{
  ...
  if (UNIV_UNLIKELY(block->state != BUF_BLOCK_FILE_PAGE) || UNIV_UNLIKELY(block->frame != guess))
  {
  exit_func:
    mutex_exit(&(buf_pool->mutex));
...
#ifdef UNIV_IBUF_DEBUG
  ut_a((mode == BUF_KEEP_OLD) || (ibuf_count_get(block->space, block->offset) == 0));
#endif
  buf_pool->n_page_gets++;
    ...
    buf_block_t *block) /* in: block to init */
{
  ...
  {
    fprintf(stderr, "InnoDB: Error: page %lu %lu already found from the hash table\n", (ulong)space, (ulong)offset);
#ifdef UNIV_DEBUG
    buf_print();
    buf_LRU_print();
    buf_validate();
    ...
    ulint offset)                   /* in: page number */
{
  ...
  *err = DB_SUCCESS;
  ...
  if (*err == DB_TABLESPACE_DELETED || NULL != buf_page_hash_get(space, offset))
  {
    ...
    buf_block_t *block) /* in: pointer to the block in question */
{
  ...
  if (io_type == BUF_IO_READ)
  {
    ...
    if (read_page_no != 0 && !trx_doublewrite_page_inside(read_page_no) && read_page_no != block->offset)
    {
    ...
    {
      ibuf_merge_or_delete_for_page(block->frame, block->space, block->offset, TRUE);
    }
  }
...
#ifdef UNIV_IBUF_DEBUG
      ut_a((block->io_fix == BUF_IO_READ) || ibuf_count_get(block->space, block->offset) == 0);
#endif
...
/*===========*/
{
  ...
  fprintf(stderr,
          ...
          (ulong)UT_LIST_GET_LEN(buf_pool->flush_list), (ulong)buf_pool->n_pend_reads,
          (ulong)buf_pool->n_flush[BUF_FLUSH_LRU], (ulong)buf_pool->n_flush[BUF_FLUSH_LIST],
          (ulong)buf_pool->n_flush[BUF_FLUSH_SINGLE_PAGE], (ulong)buf_pool->n_pages_read, buf_pool->n_pages_created,
  ...
  for (i = 0; i < n_found; i++)
  {
    ...
    fprintf(stderr, "Block count for index %lu in buffer is about %lu", (ulong)ut_dulint_get_low(index_ids[i]),
            (ulong)counts[i]);
...
ulint buf_get_latched_pages_number(void)
{
  ...
  for (i = 0; i < buf_pool->curr_size; i++)
  {
    ...
    if (((block->buf_fix_count != 0) || (block->io_fix != 0)) && block->magic_n == BUF_BLOCK_MAGIC_N)
      fixed_pages_number++;
  }
...
/*=======================*/
{
  return (buf_pool->n_pend_reads + buf_pool->n_flush[BUF_FLUSH_LRU] + buf_pool->n_flush[BUF_FLUSH_LIST] +
    ...
    FILE *file) /* in/out: buffer where to print */
{
  ...
  if (srv_use_awe)
  {
    fprintf(stderr, "AWE: Buffer pool memory frames                        %lu\n", (ulong)buf_pool->n_frames);

    fprintf(stderr, "AWE: Database pages and free buffers mapped in frames %lu\n",
            (ulong)UT_LIST_GET_LEN(buf_pool->awe_LRU_free_mapped));
  }
  fprintf(file,
          "Buffer pool size   %lu\n"
          ...
          (ulong)size, (ulong)UT_LIST_GET_LEN(buf_pool->free), (ulong)UT_LIST_GET_LEN(buf_pool->LRU),
          (ulong)UT_LIST_GET_LEN(buf_pool->flush_list), (ulong)buf_pool->n_pend_reads,
  ...
  fprintf(file,
          ...
          (ulong)buf_pool->n_pages_read, (ulong)buf_pool->n_pages_created, (ulong)buf_pool->n_pages_written,
  ...
  if (srv_use_awe)
  {
    fprintf(file, "AWE: %.2f page remaps/s\n",
            (buf_pool->n_pages_awe_remapped - buf_pool->n_pages_awe_remapped_old) / time_elapsed);
  }
...
/*===============*/
{
  ...
  for (i = 0; i < buf_pool->curr_size; i++)
  {
    ...
    if (block->state == BUF_BLOCK_FILE_PAGE)
    {
      ...
      {
        fprintf(stderr, "Page %lu %lu still fixed or dirty\n", (ulong)block->space, (ulong)block->offset);
        ut_error;
      }
    }
  }
