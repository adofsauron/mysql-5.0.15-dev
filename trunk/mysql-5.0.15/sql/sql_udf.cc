...
void udf_init() {
  ...
  THD *new_thd = new THD;
  if (!new_thd || hash_init(&udf_hash, system_charset_info, 32, 0, 0,
                            get_hash_key, NULL, 0)) {
  ...
  if (simple_open_n_lock_tables(new_thd, &tables)) {
    ...
    sql_print_error("Can't open the mysql.func table. Please run the "
  ...
  while (!(error = read_record_info.read_record(&read_record_info))) {
    ...
    */
    if (strchr(dl_name, '/') || IF_WIN(strchr(dl_name, '\\'), 0) ||
...
int mysql_drop_function(THD *thd, const LEX_STRING *udf_name) {
  ...
  */
  ...
  if (!table->file->index_read_idx(
           table->record[0], 0, (byte *)table->field[0]->ptr,
