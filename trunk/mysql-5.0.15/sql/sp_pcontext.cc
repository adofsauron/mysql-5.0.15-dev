...
sp_pcontext::sp_pcontext(sp_pcontext *prev)
    : Sql_alloc(), m_psubsize(0), m_csubsize(0), m_hsubsize(0), m_handlers(0),
      m_parent(prev) {
...
sp_pvar_t *sp_pcontext::find_pvar(LEX_STRING *name, my_bool scoped) {
  ...
  while (i--) {
    ...
    if (my_strnncoll(system_charset_info, (const uchar *)name->str,
                     name->length, (const uchar *)p->name.str,
                     p->name.length) == 0) {
      return p;
    }
  }
...
void sp_pcontext::push_pvar(LEX_STRING *name, enum enum_field_types type,
...
my_bool sp_pcontext::find_cursor(LEX_STRING *name, uint *poff, my_bool scoped) {
  ...
  while (i--) {
    ...
    if (my_strnncoll(system_charset_info, (const uchar *)name->str,
                     name->length, (const uchar *)n.str, n.length) == 0) {
      *poff = m_coffset + i;
      return TRUE;
    }
  }
