...
ha_heap::ha_heap(TABLE *table_arg) : handler(&heap_hton, table_arg), file(0), records_changed(0), key_stats_ok(0) {}

static const char *ha_heap_exts[] = {NullS};

const char **ha_heap::bas_ext() const { return ha_heap_exts; }
...
int ha_heap::close(void) { return heap_close(file); }
...
void ha_heap::update_key_stats()
{
  for (uint i = 0; i < table->s->keys; i++)
  {
    ...
    if (key->algorithm != HA_KEY_ALG_BTREE)
    {
      ...
      key->rec_per_key[key->key_parts - 1] = hash_buckets ? file->s->records / hash_buckets : 0;
    }
  }
  records_changed = 0;
...
int ha_heap::delete_row(const byte *buf)
{
  ...
  if (!res && table->s->tmp_table == NO_TMP_TABLE && ++records_changed * HEAP_STATS_UPDATE_THRESHOLD > file->s->records)
    key_stats_ok = FALSE;
  return res;
}

int ha_heap::index_read(byte *buf, const byte *key, uint key_len, enum ha_rkey_function find_flag)
{
...
int ha_heap::index_read_last(byte *buf, const byte *key, uint key_len)
{
  DBUG_ASSERT(inited == INDEX);
  statistic_increment(table->in_use->status_var.ha_read_key_count, &LOCK_status);
  int error = heap_rkey(file, buf, active_index, key, key_len, HA_READ_PREFIX_LAST);
  table->status = error ? STATUS_NOT_FOUND : 0;
  return error;
}

int ha_heap::index_read_idx(byte *buf, uint index, const byte *key, uint key_len, enum ha_rkey_function find_flag)
{
  statistic_increment(table->in_use->status_var.ha_read_key_count, &LOCK_status);
...
int ha_heap::indexes_are_disabled(void) { return heap_indexes_are_disabled(file); }
...
{
  char buff[FN_REFLEN];
  int error = heap_delete_table(fn_format(buff, name, "", "", MY_REPLACE_EXT | MY_UNPACK_FILENAME));
  return error == ENOENT ? 0 : error;
}

int ha_heap::rename_table(const char *from, const char *to) { return heap_rename(from, to); }
...
int ha_heap::create(const char *name, TABLE *table_arg, HA_CREATE_INFO *create_info)
{
  ...
  for (key = parts = 0; key < keys; key++) parts += table_arg->key_info[key].key_parts;

  if (!(keydef = (HP_KEYDEF *)my_malloc(keys * sizeof(HP_KEYDEF) + parts * sizeof(HA_KEYSEG), MYF(MY_WME))))
    return my_errno;
  ...
  for (key = 0; key < keys; key++)
  {
    ...
    for (; key_part != key_part_end; key_part++, seg++)
    {
      ...
      {
        if ((seg->type = field->key_type()) != (int)HA_KEYTYPE_TEXT && seg->type != HA_KEYTYPE_VARTEXT1 &&
            seg->type != HA_KEYTYPE_VARTEXT2 && seg->type != HA_KEYTYPE_VARBINARY1 &&
      ...
      if (field->flags & AUTO_INCREMENT_FLAG && table_arg->found_next_number_field && key == share->next_number_index)
      {
        /*
  ...
  max_rows = (ha_rows)(table->in_use->variables.max_heap_table_size / mem_per_row);
  ...
  hp_create_info.auto_increment = (create_info->auto_increment_value ? create_info->auto_increment_value - 1 : 0);
  hp_create_info.max_table_size = current_thd->variables.max_heap_table_size;
  hp_create_info.with_auto_increment = found_real_auto_increment;
  max_rows = (ha_rows)(hp_create_info.max_table_size / mem_per_row);
  error =
      heap_create(fn_format(buff, name, "", "", MY_REPLACE_EXT | MY_UNPACK_FILENAME), keys, keydef, share->reclength,
