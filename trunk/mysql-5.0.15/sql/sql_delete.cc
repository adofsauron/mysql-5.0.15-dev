  ...
  */
  ...
  if (order && order->elements) {
    ...
    if (select_lex->setup_ref_array(thd, order->elements) ||
        setup_order(thd, select_lex->ref_pointer_array, &tables, fields,
                    all_fields, (ORDER *)order->first) ||
        !(sortorder = make_unireg_sortorder((ORDER *)order->first, &length)) ||
        (table->sort.found_records =
             filesort(thd, table, sortorder, length, select, HA_POS_ERROR,
...
bool mysql_prepare_delete(THD *thd, TABLE_LIST *table_list, Item **conds) {
  ...
  if (setup_tables(thd, &thd->lex->select_lex.context,
                   &thd->lex->select_lex.top_join_list, table_list, conds,
...
bool mysql_multi_delete_prepare(THD *thd) {
  ...
  */
  if (setup_tables(thd, &thd->lex->select_lex.context,
                   &thd->lex->select_lex.top_join_list, lex->query_tables,
  ...
  */
  ...
  for (target_tbl = (TABLE_LIST *)aux_tables; target_tbl;
    ...
    if (!target_tbl->correspondent_table->updatable ||
      ...
      my_error(ER_NON_UPDATABLE_TABLE, MYF(0), target_tbl->table_name,
               "DELETE");
      DBUG_RETURN(TRUE);
    }
    /*
    ...
    */
    {
      ...
      if ((duplicate = unique_table(target_tbl->correspondent_table,
                                    lex->query_tables))) {
        update_non_unique_table_error(target_tbl->correspondent_table, "DELETE",
                                      duplicate);
        DBUG_RETURN(TRUE);
      }
    }
...
multi_delete::multi_delete(TABLE_LIST *dt, uint num_of_tables_arg)
    : delete_tables(dt), deleted(0), found(0), num_of_tables(num_of_tables_arg),
...
bool multi_delete::initialize_tables(JOIN *join) {
  ...
  for (; walk; walk = walk->next_local) {
    ...
    *tempfiles_ptr++ = new Unique(refpos_order_cmp, (void *)table->file,
...
multi_delete::~multi_delete() {
  for (table_being_deleted = delete_tables; table_being_deleted;
...
bool multi_delete::send_data(List<Item> &values) {
  ...
  for (del_table = delete_tables; del_table;
...
int multi_delete::do_deletes() {
  ...
  table_being_deleted =
      (delete_while_scanning ? delete_tables->next_local : delete_tables);
...
bool mysql_truncate(THD *thd, TABLE_LIST *table_list, bool dont_send_ok) {
  ...
  if (!dont_send_ok && (table_ptr = find_temporary_table(
...
trunc_by_del:
  ...
  error = mysql_delete(thd, table_list, (COND *)0, (SQL_LIST *)0, HA_POS_ERROR,
                       LL(0), TRUE);
