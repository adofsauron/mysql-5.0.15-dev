...
const LEX_STRING trg_event_type_names[] = {
  {(char *)STRING_WITH_LEN("INSERT") }, {(char *)STRING_WITH_LEN("UPDATE") },
...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create) {
  ...
  */
  ...
  result = (create ? table->triggers->create_trigger(thd, tables)
...
bool Table_triggers_list::create_trigger(THD *thd, TABLE_LIST *tables) {
  ...
  if (my_strcasecmp(table_alias_charset, table->s->db,
                    lex->spname->m_db.str ? lex->spname->m_db.str : thd->db)) {
  ...
  */
  ...
  trigname_file.length =
      strxnmov(trigname_buff, FN_REFLEN, lex->spname->m_name.str,
  ...
  */
  if (!(trg_def =
            (LEX_STRING *)alloc_root(&table->mem_root, sizeof(LEX_STRING))) ||
      definitions_list.push_back(trg_def, &table->mem_root) ||
      !(trg_sql_mode =
            (ulonglong *)alloc_root(&table->mem_root, sizeof(ulonglong))) ||
  ...
  *trg_sql_mode = thd->variables.sql_mode;
  ...
  if (!sql_create_definition_file(&dir, &file, &triggers_file_type, (gptr) this,
...
bool Table_triggers_list::prepare_record1_accessors(TABLE *table) {
  ...
  if (!(record1_field = (Field **)alloc_root(
            &table->mem_root, (table->s->fields + 1) * sizeof(Field *))))
    return 1;
  ...
  */
  ...
  if ((parser = sql_parse_prepare(&path, &table->mem_root, 1))) {
    if (is_equal(&triggers_file_type, parser->type())) {
      ...
      */
      triggers->sroutines_key.length =
          1 + strlen(db) + 1 + strlen(table_name) + 1;
      ...
      */
      ...
      while ((trg_create_str = it++)) {
        ...
        triggers->bodies[lex.trg_chistics.event][lex.trg_chistics.action_time] =
            lex.sphead;
    ...
    */
    my_error(ER_WRONG_OBJECT, MYF(0), table_name, triggers_file_ext + 1,
             "TRIGGER");
    DBUG_RETURN(1);
  }
...
static TABLE_LIST *add_table_for_trigger(THD *thd, sp_name *trig) {
  ...
  DBUG_RETURN(sp_add_to_query_tables(thd, lex, db, trigname.trigger_table.str,
                                     TL_WRITE));
}
