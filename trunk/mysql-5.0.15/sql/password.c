...
void randominit(struct rand_struct *rand_st, ulong seed1, ulong seed2) { /* For mysql 3.21.# */
#ifdef HAVE_purify
...
void hash_password(ulong *result, const char *password, uint password_len) {
  ...
  for (; password < password_end; password++) {
    if (*password == ' ' || *password == '\t') continue; /* skip space in password */
...
void scramble_323(char *to, const char *message, const char *password) {
  ...
  if (password && password[0]) {
    ...
    for (; message < message_end; message++) *to++ = (char)(floor(my_rnd(&rand_st) * 31) + 64);
    extra = (char)(floor(my_rnd(&rand_st) * 31));
    while (to_start != to) *(to_start++) ^= extra;
  }
  *to = 0;
}
...
my_bool check_scramble_323(const char *scrambled, const char *message, ulong *hash_pass) {
  ...
  for (pos = scrambled; *pos && to < buff + sizeof(buff); pos++) *to++ = (char)(floor(my_rnd(&rand_st) * 31) + 64);
  if (pos - scrambled != SCRAMBLE_LENGTH_323) return 1;
  extra = (char)(floor(my_rnd(&rand_st) * 31));
  to = buff;
  while (*scrambled) {
    if (*scrambled++ != (char)(*to++ ^ extra)) return 1; /* Wrong password */
  }
  return 0;
}
...
void get_salt_from_password_323(ulong *res, const char *password) {
  ...
  if (password) {
    while (*password) {
      ulong val = 0;
      uint i;
      for (i = 0; i < 8; i++) val = (val << 4) + char_val(*password++);
      *res++ = val;
    }
  }
}
...
void make_password_from_salt_323(char *to, const ulong *salt) { sprintf(to, "%08lx%08lx", salt[0], salt[1]); }
...
void create_random_string(char *to, uint length, struct rand_struct *rand_st) {
  ...
  for (; to < end; to++) *to = (char)(my_rnd(rand_st) * 94 + 33);
  *to = '\0';
}
