...
static bool find_key_for_maxmin(bool max_fl, TABLE_REF *ref, Field *field, COND *cond, uint *range_fl,
    ...
    if (item->type() == Item::SUM_FUNC_ITEM)
    {
      ...
      switch (item_sum->sum_func())
      {
        case Item_sum::COUNT_FUNC:
          ...
          */
          if (!conds && !((Item_sum_count *)item)->args[0]->maybe_null && !outer_tables && is_exact_count)
          {
            longlong count = 1;
            TABLE_LIST *table;
            for (table = tables; table; table = table->next_leaf)
            {
              if (outer_tables || (table->table->file->table_flags() & HA_NOT_EXACT_COUNT) || table->schema_table)
              {
          ...
          if (expr->type() == Item::FIELD_ITEM)
          {
            ...
            */
            if ((outer_tables & table->map) ||
                !find_key_for_maxmin(0, &ref, item_field->field, conds, &range_fl, &prefix_len))
            {
              const_result = 0;
              break;
            }
            ...
            if (!ref.key_length)
              ...
              error = table->file->index_read(table->record[0], key_buff, ref.key_length,
                                              range_fl & NEAR_MIN ? HA_READ_AFTER_KEY : HA_READ_KEY_OR_NEXT);
            if (!error && reckey_in_range(0, &ref, item_field->field, conds, range_fl, prefix_len))
        ...
        case Item_sum::MAX_FUNC:
        {
          ...
          */
          ...
          if (expr->type() == Item::FIELD_ITEM)
          {
            ...
            */
            if ((outer_tables & table->map) ||
                !find_key_for_maxmin(1, &ref, item_field->field, conds, &range_fl, &prefix_len))
            {
              const_result = 0;
              break;
            }
            ...
            if (!ref.key_length)
              ...
              error = table->file->index_read(table->record[0], key_buff, ref.key_length,
                                              range_fl & NEAR_MAX ? HA_READ_BEFORE_KEY : HA_READ_PREFIX_LAST_OR_PREV);
            if (!error && reckey_in_range(1, &ref, item_field->field, conds, range_fl, prefix_len))
              error = HA_ERR_KEY_NOT_FOUND;
            if (table->key_read)
            {
              table->key_read = 0;
                          ...
                          key_part_map *key_part_used, uint *range_fl, uint *prefix_len)
{
  ...
  if (org_key_part_used != *key_part_used ||
      (is_field_part && (between || eq_type || max_fl == less_fl) && !cond->val_int()))
  {
    /*
...
static bool find_key_for_maxmin(bool max_fl, TABLE_REF *ref, Field *field, COND *cond, uint *range_fl, uint *prefix_len)
{
  ...
  for (keyinfo = table->key_info, keyinfo_end = keyinfo + table->s->keys; keyinfo != keyinfo_end; keyinfo++, idx++)
  {
         ...
         part++, jdx++, key_part_to_use = (key_part_to_use << 1) | 1)
    {
      ...
      if (field->eq(part->field))
      {
        ...
        *range_fl = NO_MIN_RANGE | NO_MAX_RANGE;
        if (matching_cond(max_fl, ref, keyinfo, part, cond, &key_part_used, range_fl, prefix_len) &&
