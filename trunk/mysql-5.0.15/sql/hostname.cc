...
bool hostname_cache_init() {
  ...
  if (!(hostname_cache =
            new hash_filo(HOST_CACHE_SIZE, offset, sizeof(struct in_addr), NULL,
                          (hash_free_key)free, &my_charset_latin1)))
    return 1;
...
static void add_hostname(struct in_addr *in, const char *name) {
  if (!(specialflag & SPECIAL_NO_HOST_CACHE)) {
    ...
    host_entry *entry;
    if (!(entry =
              (host_entry *)hostname_cache->search((gptr) & in->s_addr, 0))) {
...
inline void add_wrong_ip(struct in_addr *in) { add_hostname(in, NullS); }
...
#endif
  if (!(hp = gethostbyaddr_r((char *)in, sizeof(*in), AF_INET, &tmp_hostent,
                             buff, sizeof(buff), &tmp_errno))) {
    DBUG_PRINT("error", ("gethostbyaddr_r returned %d", tmp_errno));
    return 0;
  }
  if (!(check = my_gethostbyname_r(hp->h_name, &tmp_hostent2, buff2,
                                   sizeof(buff2), &tmp_errno))) {
