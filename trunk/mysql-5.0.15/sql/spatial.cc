...
Geometry::Class_info::Class_info(const char *name, int type_id,
    ...
    : m_name(name, strlen(name)), m_type_id(type_id),
...
linestring_class("LINESTRING", Geometry::wkb_linestring, create_linestring);
static Geometry::Class_info polygon_class("POLYGON", Geometry::wkb_polygon,
                                          create_polygon);
static Geometry::Class_info
multipoint_class("MULTIPOINT", Geometry::wkb_multipoint, create_multipoint);
...
Geometry::Class_info *Geometry::find_class(const char *name, uint32 len) {
  for (Class_info **cur_rt = ci_collection; cur_rt < ci_collection_end;
    ...
    if (*cur_rt && ((*cur_rt)->m_name.length == len) &&
        (my_strnncoll(&my_charset_latin1, (const uchar *)(*cur_rt)->m_name.str,
                      len, (const uchar *)name, len) == 0))
      return *cur_rt;
  }
  return 0;
}

Geometry *Geometry::create_from_wkb(Geometry_buffer *buffer, const char *data,
                                    uint32 data_len) {
  uint32 geom_type;
  Geometry *result;
...
Geometry *Geometry::create_from_wkt(Geometry_buffer *buffer,
  ...
  if (trs->check_next_symbol('(') || result->init_from_wkt(trs, wkt) ||
...
bool Gis_polygon::init_from_wkt(Gis_read_stream *trs, String *wkb) {
  ...
  for (;;) {
    ...
    if (trs->check_next_symbol('(') || ls.init_from_wkt(trs, wkb) ||
        trs->check_next_symbol(')'))
      return 1;
...
uint32 Gis_multi_line_string::get_data_size() const {
  ...
  while (n_line_strings--) {
    ...
    data += (WKB_HEADER_SIZE + 4 +
             uint4korr(data + WKB_HEADER_SIZE) * POINT_DATA_SIZE);
  }
  return (uint32)(data - m_data);
}
...
bool Gis_multi_polygon::init_from_wkt(Gis_read_stream *trs, String *wkb) {
  ...
  for (;;) {
    ...
    if (trs->check_next_symbol('(') || p.init_from_wkt(trs, wkb) ||
        trs->check_next_symbol(')'))
      return 1;
    n_polygons++;
    if (trs->skip_char(',')) // Didn't find ','
