...
void safe_mutex_global_init(void) { pthread_mutex_init(&THR_LOCK_mutex, MY_MUTEX_INIT_FAST); }

int safe_mutex_init(safe_mutex_t *mp, const pthread_mutexattr_t *attr __attribute__((unused)), const char *file,
                    uint line)
{
...
int safe_mutex_lock(safe_mutex_t *mp, const char *file, uint line)
{
  ...
  if (!mp->file)
  {
    fprintf(stderr, "safe_mutex: Trying to lock unitialized mutex at %s, line %d\n", file, line);
    fflush(stderr);
    abort();
  }
  ...
  {
    fprintf(stderr,
            "safe_mutex: Trying to lock mutex at %s, line %d, when the mutex was already locked at %s, line %d in "
            "thread %s\n",
...
int safe_cond_wait(pthread_cond_t *cond, safe_mutex_t *mp, const char *file, uint line)
{
  ...
  {
    fprintf(stderr,
            "safe_mutex: Trying to cond_wait on a mutex at %s, line %d  that was locked by another thread at: %s, "
            "line: %d\n",
            file, line, mp->file, mp->line);
    fflush(stderr);
    abort();
  ...
  if (mp->count++)
  {
    fprintf(stderr, "safe_mutex:  Count was %d in thread 0x%lx when locking mutex at %s, line %d\n", mp->count - 1,
            my_thread_id(), file, line);
    fflush(stderr);
    abort();
  }
  mp->thread = pthread_self();
...
int safe_cond_timedwait(pthread_cond_t *cond, safe_mutex_t *mp, struct timespec *abstime, const char *file, uint line)
{
  ...
  if (mp->count++)
  {
    fprintf(stderr, "safe_mutex:  Count was %d in thread 0x%lx when locking mutex at %s, line %d (error: %d (%d))\n",
...
int safe_mutex_destroy(safe_mutex_t *mp, const char *file, uint line)
{
  ...
  if (!mp->file)
  {
    fprintf(stderr, "safe_mutex: Trying to destroy unitialized mutex at %s, line %d\n", file, line);
    fflush(stderr);
    abort();
  }
  if (mp->count != 0)
