...
int ha_heap::index_read_last(byte *buf, const byte *key, uint key_len) {
  ...
  statistic_increment(table->in_use->status_var.ha_read_key_count,
                      &LOCK_status);
  int error =
      heap_rkey(file, buf, active_index, key, key_len, HA_READ_PREFIX_LAST);
  table->status = error ? STATUS_NOT_FOUND : 0;
  return error;
}
...
THR_LOCK_DATA **ha_heap::store_lock(THD *thd, THR_LOCK_DATA **to,
...
int ha_heap::delete_table(const char *name) {
  char buff[FN_REFLEN];
  int error = heap_delete_table(
      fn_format(buff, name, "", "", MY_REPLACE_EXT | MY_UNPACK_FILENAME));
...
int ha_heap::create(const char *name, TABLE *table_arg,
  ...
  max_rows =
      (ha_rows)(table->in_use->variables.max_heap_table_size / mem_per_row);
  ...
  hp_create_info.auto_increment =
      (create_info->auto_increment_value ? create_info->auto_increment_value - 1
                                         : 0);
  hp_create_info.max_table_size = current_thd->variables.max_heap_table_size;
  hp_create_info.with_auto_increment = found_real_auto_increment;
  max_rows = (ha_rows)(hp_create_info.max_table_size / mem_per_row);
  error = heap_create(
      fn_format(buff, name, "", "", MY_REPLACE_EXT | MY_UNPACK_FILENAME), keys,
      keydef, share->reclength,
      (ulong)((share->max_rows < max_rows && share->max_rows) ? share->max_rows
                                                              : max_rows),
