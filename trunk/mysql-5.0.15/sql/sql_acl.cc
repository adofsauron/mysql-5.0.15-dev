...
static uint grant_version =
    0; /* Version of priv tables. incremented by acl_load */
...
static void set_user_salt(ACL_USER *acl_user, const char *password,
                          uint password_len) {
...
my_bool acl_init(bool dont_read_acl_tables) {
  ...
  acl_cache =
      new hash_filo(ACL_CACHE_SIZE, 0, 0, (hash_get_key)acl_entry_get_key,
...
static my_bool acl_load(THD *thd, TABLE_LIST *tables) {
  ...
  while (!(read_record_info.read_record(&read_record_info))) {
    ...
    if (table->s->fields == 8) { // Without grant
  ...
  while (!(read_record_info.read_record(&read_record_info))) {
    ...
    } else // password is correct
    {
      ...
      */
      ...
      user.hostname_length =
          (user.host.hostname ? (uint)strlen(user.host.hostname) : 0);
...
my_bool acl_reload(THD *thd) {
  ...
  if (thd->locked_tables) { // Can't have locked tables here
  ...
  */
  ...
  if ((return_val = acl_load(thd, tables))) { // Error. Revert to old list
  ...
  for (uint i = 0; i < acl_users.elements; i++) {
    ...
    if (!acl_user->user && !user[0] ||
      ...
      if (!acl_user->host.hostname && !host[0] ||
          acl_user->host.hostname && !my_strcasecmp(system_charset_info, host,
        ...
        if (ssl_type != SSL_TYPE_NOT_SPECIFIED) {
          ...
          acl_user->ssl_cipher =
              (ssl_cipher ? strdup_root(&mem, ssl_cipher) : 0);
          acl_user->x509_issuer =
              (x509_issuer ? strdup_root(&mem, x509_issuer) : 0);
          acl_user->x509_subject =
              (x509_subject ? strdup_root(&mem, x509_subject) : 0);
        }
        if (password)
  ...
  for (uint i = 0; i < acl_dbs.elements; i++) {
    ...
    if (!acl_db->user && !user[0] ||
      ...
      if (!acl_db->host.hostname && !host[0] ||
          acl_db->host.hostname && !my_strcasecmp(system_charset_info, host,
...
ulong acl_get(const char *host, const char *ip, const char *user,
  ...
  if (!db_is_pattern &&
      (entry = (acl_entry *)acl_cache->search(key, key_length))) {
...
static void init_check_host(void) {
  ...
  VOID(hash_init(&acl_check_hosts, system_charset_info, acl_users.elements, 0,
                 0, (hash_get_key)check_get_key, 0, 0));
  if (!allow_all_hosts) {
    for (uint i = 0; i < acl_users.elements; i++) {
      ACL_USER *acl_user = dynamic_element(&acl_users, i, ACL_USER *);
      if (strchr(acl_user->host.hostname, wild_many) ||
          strchr(acl_user->host.hostname, wild_one) ||
          acl_user->host.ip_mask) { // Has wildcard
        uint j;
        for (j = 0; j < acl_wild_hosts.elements;
             j++) { // Check if host already exists
          acl_host_and_ip *acl =
              dynamic_element(&acl_wild_hosts, j, acl_host_and_ip *);
          if (!my_strcasecmp(system_charset_info, acl_user->host.hostname,
                             acl->hostname))
        ...
        if (my_hash_insert(&acl_check_hosts,
                           (byte *)acl_user)) { // End of memory
...
bool acl_check_host(const char *host, const char *ip) {
  ...
  for (uint i = 0; i < acl_wild_hosts.elements; i++) {
    acl_host_and_ip *acl =
        dynamic_element(&acl_wild_hosts, i, acl_host_and_ip *);
...
bool check_change_password(THD *thd, const char *host, const char *user,
  ...
  if (!thd->slave_thread && (strcmp(thd->security_ctx->user, user) ||
...
static bool compare_hostname(const acl_host_and_ip *host, const char *hostname,
  ...
  return (!host->hostname ||
          (hostname &&
           !wild_case_compare(system_charset_info, hostname, host->hostname)) ||
...
static bool update_user_table(THD *thd, TABLE *table, const char *host,
  ...
  if (table->file->index_read_idx(table->record[0], 0, (byte *)user_key,
                                  table->key_info->key_length,
                                  HA_READ_KEY_EXACT)) {
...
static bool test_if_create_new_users(THD *thd) {
  ...
  bool create_new_users =
      ...
      (!opt_safe_user_create && test(sctx->master_access & CREATE_USER_ACL));
  if (!create_new_users) {
    TABLE_LIST tl;
    ulong db_access;
...
static int replace_user_table(THD *thd, TABLE *table, const LEX_USER &combo,
  ...
  table->field[0]
      ->store(combo.host.str, combo.host.length, system_charset_info);
  table->field[1]
      ->store(combo.user.str, combo.user.length, system_charset_info);
  key_copy(user_key, table->record[0], table->key_info,
           table->key_info->key_length);

  table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
  if (table->file->index_read_idx(table->record[0], 0, user_key,
                                  table->key_info->key_length,
                                  HA_READ_KEY_EXACT)) {
    /* what == 'N' means revoke */
    if (what == 'N') {
    ...
    */
      ...
      my_error(ER_CANT_CREATE_USER_WITH_GRANT, MYF(0), thd->security_ctx->user,
  ...
  if (old_row_exists) {
    ...
    */
    ...
    if (cmp_record(table, record[1]) &&
        (error = table->file->update_row(
             table->record[1], table->record[0]))) { // This should never happen
      table->file->print_error(error, MYF(0));       /* purecov: deadcode */
  ...
  if (!error) {
    ...
    if (old_row_exists)
      acl_update_user(combo.user.str, combo.host.str, combo.password.str,
                      password_len, lex->ssl_type, lex->ssl_cipher,
                      lex->x509_issuer, lex->x509_subject, &lex->mqh, rights);
    else
      acl_insert_user(combo.user.str, combo.host.str, password, password_len,
                      lex->ssl_type, lex->ssl_cipher, lex->x509_issuer,
                      lex->x509_subject, &lex->mqh, rights);
  }
  DBUG_RETURN(error);
}
...
static int replace_db_table(TABLE *table, const char *db, const LEX_USER &combo,
                            ulong rights, bool revoke_grant) {
  uint i;
  ...
  for (i = 3, priv = 1; i < table->s->fields; i++, priv <<= 1) {
    if (priv & store_rights) // do it if priv is chosen
      table->field[i]
          ->store(&what, 1, &my_charset_latin1); // set requested privileges
  }
...
GRANT_TABLE::GRANT_TABLE(const char *h, const char *d, const char *u,
  ...
  (void)hash_init(&hash_columns, system_charset_info, 0, 0, 0,
                  (hash_get_key)get_key_column, 0, 0);
}
...
GRANT_NAME::GRANT_NAME(TABLE *form) {
  ...
  key_length =
      ((uint)strlen(db) + (uint)strlen(user) + (uint)strlen(tname) + 3);
  hash_key = (char *)alloc_root(&memex, key_length);
  strmov(strmov(strmov(hash_key, user) + 1, db) + 1, tname);
...
static GRANT_NAME *name_hash_search(HASH *name_hash, const char *host,
  ...
  len = (uint)(strmov(strmov(strmov(helping, user) + 1, db) + 1, tname) -
               helping) +
        1;
  for (grant_name = (GRANT_NAME *)hash_search(name_hash, (byte *)helping, len);
       grant_name;
...
inline GRANT_NAME *routine_hash_search(const char *host, const char *ip,
                                       ...
                                       bool exact) {
  return (GRANT_TABLE *)name_hash_search(proc ? &proc_priv_hash
                                              : &func_priv_hash,
...
inline GRANT_TABLE *table_hash_search(const char *host, const char *ip,
  ...
  return (GRANT_TABLE *)name_hash_search(&column_priv_hash, host, ip, db, user,
                                         tname, exact);
}
...
static int replace_column_table(GRANT_TABLE *g_t, TABLE *table,
                                const LEX_USER &combo,
                                List<LEX_COLUMN> &columns, const char *db,
                                const char *table_name, ulong rights,
                                bool revoke_grant) {
  ...
  table->field[2]
      ->store(combo.user.str, combo.user.length, system_charset_info);
  table->field[3]
      ->store(table_name, (uint)strlen(table_name), system_charset_info);
  ...
  */
  ...
  if (revoke_grant) {
    ...
    table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
    if (table->file->index_read(table->record[0], user_key, key_prefix_length,
                                HA_READ_KEY_EXACT))
      goto end;
    ...
    do {
      ...
      if (privileges & rights) // is in this record the priv to be revoked ??
      {
        ...
        privileges &= ~rights;
        table->field[6]
            ->store((longlong)get_rights_for_column(privileges), TRUE);
        table->field[4]->val_str(&column_name);
        grant_column =
            column_hash_search(g_t, column_name.ptr(), column_name.length());
        if (privileges) {
          int tmp_error;
          if ((tmp_error = table->file->update_row(
                   table->record[1],
                   table->record[0]))) {                 /* purecov: deadcode */
          ...
          int tmp_error;
          if ((tmp_error = table->file->delete_row(
                   table->record[1]))) {                 /* purecov: deadcode */
            table->file->print_error(tmp_error, MYF(0)); /* purecov: deadcode */
            result = -1;                                 /* purecov: deadcode */
            goto end;                                    /* purecov: deadcode */
          }
...
static int replace_table_table(THD *thd, GRANT_TABLE *grant_table, TABLE *table,
                               const LEX_USER &combo, const char *db,
                               const char *table_name, ulong rights,
                               ulong col_rights, bool revoke_grant) {
  ...
  strxmov(grantor, thd->security_ctx->user, "@", thd->security_ctx->host_or_ip,
          NullS);
...
static int replace_routine_table(THD *thd, GRANT_NAME *grant_name, TABLE *table,
                                 const LEX_USER &combo, const char *db,
  ...
  */
  ...
  table->field[3]
      ->store(routine_name, (uint)strlen(routine_name), &my_charset_latin1);
  table->field[4]->store(
      (longlong)(is_proc ? TYPE_ENUM_PROCEDURE : TYPE_ENUM_FUNCTION), TRUE);
  store_record(table, record[1]); // store at pos 1
  ...
  if (rights) {
    ...
    hash_delete(is_proc ? &proc_priv_hash : &func_priv_hash,
                (byte *)grant_name);
  }
  DBUG_RETURN(0);
...
bool mysql_table_grant(THD *thd, TABLE_LIST *table_list,
    ...
    if (columns.elements) {
      ...
      while ((column = column_iter++)) {
        ...
        Field *f = find_field_in_table_ref(
            thd, table_list, column->column.ptr(), column->column.ptr(), NULL,
            NULL, column->column.length(), 0, 1, 1, 0, &unused_field_idx, FALSE,
            &dummy);
        if (f == (Field *)0) {
  ...
  */
  ...
  if (simple_open_n_lock_tables(thd, tables)) { // Should never happen
  ...
  while ((Str = str_list++)) {
    ...
    error = replace_user_table(
        thd, tables[0].table, *Str, 0, revoke_grant, create_new_users,
    ...
    db_name =
        (table_list->view_db.length ? table_list->view_db.str : table_list->db);
    table_name = (table_list->view_name.length ? table_list->view_name.str
                                               : table_list->table_name);
    ...
    if (!grant_table) {
      ...
      grant_table = new GRANT_TABLE(Str->host.str, db_name, Str->user.str,
    ...
    if (revoke_grant) {
      ...
      for (uint idx = 0; idx < grant_table->hash_columns.records; idx++) {
        grant_column =
            (GRANT_COLUMN *)hash_element(&grant_table->hash_columns, idx);
    ...
    if (replace_table_table(thd, grant_table, tables[1].table, *Str, db_name,
  ...
  */
  ...
  while ((Str = str_list++)) {
    ...
    if (!grant_name) {
      ...
      grant_name = new GRANT_NAME(Str->host.str, db_name, Str->user.str,
                                  table_name, rights);
      if (!grant_name) {
        result = TRUE;
        continue;
      }
      my_hash_insert(is_proc ? &proc_priv_hash : &func_priv_hash,
                     (byte *)grant_name);
    }

    if (replace_routine_table(thd, grant_name, tables[1].table, *Str, db_name,
                              table_name, is_proc, rights, revoke_grant)) {
      result = TRUE;
      continue;
    }
  }
...
bool mysql_grant(THD *thd, const char *db, List<LEX_USER> &list, ulong rights,
  ...
  */
  ...
  if (simple_open_n_lock_tables(thd, tables)) { // This should never happen
    close_thread_tables(thd);                   /* purecov: deadcode */
    DBUG_RETURN(TRUE);                          /* purecov: deadcode */
  }
  ...
  while ((Str = str_list++)) {
    ...
    if (replace_user_table(
            thd, tables[0].table, *Str, (!db ? rights : 0), revoke_grant,
            create_new_users,
            test(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER)))
      result = -1;
    else if (db) {
...
static my_bool grant_load(TABLE_LIST *tables) {
  ...
  bool check_no_resolve = specialflag & SPECIAL_NO_RESOLVE;
  MEM_ROOT **save_mem_root_ptr =
      my_pthread_getspecific_ptr(MEM_ROOT **, THR_MALLOC);
  DBUG_ENTER("grant_load");

  grant_option = FALSE;
  (void)hash_init(&column_priv_hash, system_charset_info, 0, 0, 0,
                  (hash_get_key)get_grant_table,
                  (hash_free_key)free_grant_table, 0);
  (void)hash_init(&proc_priv_hash, system_charset_info, 0, 0, 0,
                  (hash_get_key)get_grant_table, 0, 0);
  (void)hash_init(&func_priv_hash, system_charset_info, 0, 0, 0,
                  (hash_get_key)get_grant_table, 0, 0);
...
my_bool grant_reload(THD *thd) {
  ...
  */
  ...
  if ((return_val = grant_load(tables))) { // Error. Revert to old hash
    DBUG_PRINT("error", ("Reverting to old privileges"));
...
bool check_grant(THD *thd, ulong want_access, TABLE_LIST *tables,
  ...
  for (table = tables; table && number--; table = table->next_global) {
    ...
    if (!(~table->grant.privilege & want_access) || table->derived ||
        table->schema_table || table->belong_to_view) {
      /*
    ...
    if (!(grant_table =
              table_hash_search(sctx->host, sctx->ip, table->db,
                                sctx->priv_user, table->table_name, 0))) {
    ...
    table->grant.want_privilege =
        ((want_access & COL_ACLS) & ~table->grant.privilege);
...
err:
  ...
  if (!no_errors) // Not a silent skip of table
  {
    ...
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user,
             sctx->host_or_ip, table ? table->table_name : "unknown");
  }
  DBUG_RETURN(1);
}

bool check_grant_column(THD *thd, GRANT_INFO *grant, const char *db_name,
                        const char *table_name, const char *name, uint length,
                        uint show_tables) {
  Security_context *sctx = thd->security_ctx;
  GRANT_TABLE *grant_table;
  GRANT_COLUMN *grant_column;
  ...
  if (grant->version != grant_version) {
    grant->grant_table =
        table_hash_search(sctx->host, sctx->ip, db_name, sctx->priv_user,
...
bool check_grant_all_columns(THD *thd, ulong want_access, GRANT_INFO *grant,
  ...
  for (; !fields->end_of_fields(); fields->next()) {
    ...
    grant_column =
        column_hash_search(grant_table, field_name, (uint)strlen(field_name));
    if (!grant_column || (~grant_column->rights & want_access))
      goto err;
  }
  rw_unlock(&LOCK_grant);
...
err2:
  ...
  get_privilege_desc(command, sizeof(command), want_access);
  my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user,
           sctx->host_or_ip, fields->name(), table_name);
  return 1;
}
...
bool check_grant_db(THD *thd, const char *db) {
  ...
  for (uint idx = 0; idx < column_priv_hash.records; idx++) {
    GRANT_TABLE *grant_table =
        (GRANT_TABLE *)hash_element(&column_priv_hash, idx);
...
static uint command_lengths[] = {
  6, 6,  6, 6,  6,  4, 6,  8,  7,  4, 5,  10, 5,
  5, 14, 5, 23, 11, 7, 17, 18, 11, 9, 14, 13, 11
};

static int show_routine_grants(THD *thd, LEX_USER *lex_user, HASH *hash,
                               const char *type, int typelen, char *buff,
                               int buffsize);
...
bool mysql_show_grants(THD *thd, LEX_USER *lex_user) {
  ...
  for (index = 0; index < column_priv_hash.records; index++) {
    const char *user;
    GRANT_TABLE *grant_table =
        (GRANT_TABLE *)hash_element(&column_priv_hash, index);
    ...
    if (!strcmp(lex_user->user.str, user) &&
      ...
      if ((table_access | grant_table->cols) != 0) {
        ...
        if (test_all_bits(table_access, (TABLE_ACLS & ~GRANT_ACL)))
          ...
          for (counter = 0, j = SELECT_ACL; j <= TABLE_ACLS;
               counter++, j <<= 1) {
            if (test_access & j) {
              if (found)
                global.append(", ", 2);
              ...
              if (grant_table->cols) {
                ...
                for (uint col_index = 0;
                     ...
                     col_index++) {
                  GRANT_COLUMN *grant_column = (GRANT_COLUMN *)hash_element(
                      &grant_table->hash_columns, col_index);
  ...
  if (show_routine_grants(thd, lex_user, &proc_priv_hash, "PROCEDURE", 9, buff,
                          sizeof(buff))) {
    error = -1;
    goto end;
  }

  if (show_routine_grants(thd, lex_user, &func_priv_hash, "FUNCTION", 8, buff,
                          sizeof(buff))) {
    error = -1;
    goto end;
  }
    ...
    if (!strcmp(lex_user->user.str, user) &&
      ...
      if (proc_access != 0) {
        ...
        if (!test_access)
          ...
          for (counter = 0, j = SELECT_ACL; j <= PROC_ACLS;
               counter++, j <<= 1) {
            if (test_access & j) {
              if (found)
                global.append(", ", 2);
...
int open_grant_tables(THD *thd, TABLE_LIST *tables) {
  ...
  tables->lock_type = (tables + 1)->lock_type = (tables + 2)->lock_type =
      (tables + 3)->lock_type = (tables + 4)->lock_type = TL_WRITE;
  tables->db = (tables + 1)->db = (tables + 2)->db = (tables + 3)->db =
      (tables + 4)->db = (char *)"mysql";
...
static int handle_grant_table(TABLE_LIST *tables, uint table_no, bool drop,
  ...
  if (!table_no) // mysql.user table
  {
    ...
    */
    ...
    if ((error = table->file->index_read_idx(table->record[0], 0, user_key,
                                             key_prefix_length,
                                             HA_READ_KEY_EXACT))) {
...
#ifdef EXTRA_DEBUG
        DBUG_PRINT("loop", ("scan fields: '%s'@'%s' '%s' '%s' '%s'", user, host,
...
static int handle_grant_struct(uint struct_no, bool drop, LEX_USER *user_from,
                               LEX_USER *user_to) {
  int result = 0;
  uint idx;
  uint elements;
...
static int handle_grant_data(TABLE_LIST *tables, bool drop, LEX_USER *user_from,
                             LEX_USER *user_to) {
  int result = 0;
  int found;
...
bool mysql_revoke_all(THD *thd, List<LEX_USER> &list) {
  ...
  while ((lex_user = user_list++)) {
    ...
    if (replace_user_table(thd, tables[0].table, *lex_user, ~(ulong)0, 1, 0,
                           0)) {
      result = -1;
      continue;
    }
    ...
    do {
      for (counter = 0, revoked = 0; counter < column_priv_hash.records;) {
        const char *user, *host;
        GRANT_TABLE *grant_table =
            (GRANT_TABLE *)hash_element(&column_priv_hash, counter);
        if (!(user = grant_table->user))
          user = "";
    ...
    for (is_proc = 0; is_proc < 2; is_proc++)
      do {
        ...
        for (counter = 0, revoked = 0; counter < hash->records;) {
          ...
          if (!strcmp(lex_user->user.str, user) &&
              !my_strcasecmp(system_charset_info, lex_user->host.str, host)) {
            if (!replace_routine_table(thd, grant_proc, tables[4].table,
                                       *lex_user, grant_proc->db,
                                       grant_proc->tname, is_proc, ~(ulong)0,
                                       1)) {
              revoked = 1;
              continue;
            }
    ...
    for (counter = 0, revoked = 0; counter < hash->records;) {
      ...
      if (!my_strcasecmp(system_charset_info, grant_proc->db, sp_db) &&
        ...
        if (!replace_routine_table(thd, grant_proc, tables[4].table, lex_user,
                                   grant_proc->db, grant_proc->tname, is_proc,
                                   ~(ulong)0, 1)) {
          revoked = 1;
          continue;
        }
...
bool sp_grant_privileges(THD *thd, const char *sp_db, const char *sp_name,
  ...
  if (!find_acl_user(combo->host.str = (char *)sctx->host_or_ip,
...
int fill_schema_user_privileges(THD *thd, TABLE_LIST *tables, COND *cond) {
  ...
  for (counter = 0; counter < acl_users.elements; counter++) {
    ...
    if (!(want_access & ~GRANT_ACL))
      update_schema_privilege(table, buff, 0, 0, 0, 0, "USAGE", 5,
                              is_grantable);
    else {
      uint priv_id;
      ulong j, test_access = want_access & ~GRANT_ACL;
...
int fill_schema_schema_privileges(THD *thd, TABLE_LIST *tables, COND *cond) {
  ...
  for (counter = 0; counter < acl_dbs.elements; counter++) {
    ...
    if (want_access) {
      ...
      strxmov(buff, "'", user, "'@'", host, "'", NullS);
      if (!(want_access & ~GRANT_ACL))
        update_schema_privilege(table, buff, acl_db->db, 0, 0, 0, "USAGE", 5,
                                is_grantable);
      else {
        int cnt;
        ulong j, test_access = want_access & ~GRANT_ACL;
        ...
        for (cnt = 0, j = SELECT_ACL; j <= TABLE_ACLS; cnt++, j <<= 1) {
          if (test_access & j)
            update_schema_privilege(
                table, buff, grant_table->db, grant_table->tname, 0, 0,
    ...
    if (table_access != 0) {
      ...
      if (!test_access)
        ...
        for (cnt = 0, j = SELECT_ACL; j <= TABLE_ACLS; cnt++, j <<= 1) {
          if (test_access & j) {
            for (uint col_index = 0;
                 col_index < grant_table->hash_columns.records; col_index++) {
              GRANT_COLUMN *grant_column = (GRANT_COLUMN *)hash_element(
                  &grant_table->hash_columns, col_index);
              if ((grant_column->rights & j) && (table_access & j))
                update_schema_privilege(
                    table, buff, grant_table->db, grant_table->tname,
                    grant_column->column, grant_column->key_length,
                    command_array[cnt], command_lengths[cnt], is_grantable);
            }
          }
        }
...
void fill_effective_table_privileges(THD *thd, GRANT_INFO *grant,
  ...
  if (grant->version != grant_version) {
    rw_rdlock(&LOCK_grant);
    grant->grant_table =
        table_hash_search(sctx->host, sctx->ip, db, sctx->priv_user, table,
                          0);       /* purecov: inspected */
    grant->version = grant_version; /* purecov: inspected */
    rw_unlock(&LOCK_grant);
  }
