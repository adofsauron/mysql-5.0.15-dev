...
my_decimal *Hybrid_type_traits::val_decimal(Hybrid_type *val,
                                            my_decimal *to) const {
  double2my_decimal(E_DEC_FATAL_ERROR, val->real, val->dec_buf);
  return val->dec_buf;
}

String *Hybrid_type_traits::val_str(Hybrid_type *val, String *to,
...
void Hybrid_type_traits_decimal::fix_length_and_dec(Item *item,
                                                    Item *arg) const {
  item->decimals = arg->decimals;
  item->max_length =
      min(arg->max_length + DECIMAL_LONGLONG_DIGITS, DECIMAL_MAX_STR_LENGTH);
}
...
void Hybrid_type_traits_decimal::add(Hybrid_type *val, Field *f) const {
  my_decimal_add(E_DEC_FATAL_ERROR, &val->dec_buf[val->used_dec_buf_no ^ 1],
...
void Hybrid_type_traits_decimal::div(Hybrid_type *val, ulonglong u) const {
  ...
  my_decimal_div(E_DEC_FATAL_ERROR, &val->dec_buf[val->used_dec_buf_no ^ 1],
                 &val->dec_buf[val->used_dec_buf_no], &val->dec_buf[2], 4);
  val->used_dec_buf_no ^= 1;
}

longlong Hybrid_type_traits_decimal::val_int(Hybrid_type *val,
...
String *Hybrid_type_traits_decimal::val_str(Hybrid_type *val, String *to,
                                            uint8 decimals) const {
...
void Hybrid_type_traits_integer::fix_length_and_dec(Item *item,
                                                    Item *arg) const {
  item->decimals = 0;
  item->max_length = 21;
...
Item::Item(THD *thd, Item *item)
    : rsize(0), str_value(item->str_value), name(item->name),
      orig_name(item->orig_name), max_length(item->max_length),
      marker(item->marker), decimals(item->decimals),
      maybe_null(item->maybe_null), null_value(item->null_value),
      unsigned_flag(item->unsigned_flag), with_sum_func(item->with_sum_func),
      fixed(item->fixed), collation(item->collation) {
...
void Item::cleanup() {
  ...
  DBUG_PRINT("info", ("Item: 0x%lx, Type: %d, name %s, original name %s", this,
...
Item_ident::Item_ident(Name_resolution_context *context_arg,
    ...
    : orig_db_name(db_name_arg), orig_table_name(table_name_arg),
      ...
      cached_field_index(NO_CACHED_FIELD_INDEX), cached_table(0),
      depended_from(0) {
...
Item_ident::Item_ident(THD *thd, Item_ident *item)
    : Item(thd, item), orig_db_name(item->orig_db_name),
      orig_table_name(item->orig_table_name),
      orig_field_name(item->orig_field_name), context(item->context),
      db_name(item->db_name), table_name(item->table_name),
      field_name(item->field_name), alias_name_used(item->alias_name_used),
      cached_field_index(item->cached_field_index),
      cached_table(item->cached_table), depended_from(item->depended_from) {}
...
void Item::set_name(const char *str, uint length, CHARSET_INFO *cs) {
  ...
  if (!my_charset_same(cs, system_charset_info)) {
    uint32 res_length;
    name =
        sql_strmake_with_convert(str, name_length = length, cs, MAX_ALIAS_NAME,
...
bool Item::get_date(TIME *ltime, uint fuzzydate) {
  ...
  if (!(res = val_str(&tmp)) ||
      str_to_datetime_with_warn(res->ptr(), res->length(), ltime, fuzzydate) <=
...
Item **Item_splocal::this_item_addr(THD *thd, Item **addr) {
  return thd->spcont->get_item_addr(m_offset);
}

Item *Item_splocal::this_const_item() const {
...
bool agg_item_collations(DTCollation &c, const char *fname, Item **av,
...
bool agg_item_collations_for_comparison(DTCollation &c, const char *fname,
  ...
  return (
      agg_item_collations(c, fname, av, count, flags | MY_COLL_DISALLOW_NONE));
}
  ...
  */
  ...
  for (arg = args, last = args + nargs; arg < last; arg++) {
    ...
    if (!String::needs_conversion(0, coll.collation,
                                  (*arg)->collation.collation, &dummy_offset))
      continue;
...
Item_field::Item_field(Field *f)
    : Item_ident(0, NullS, *f->table_name, f->field_name), item_equal(0),
      no_const_subst(0), have_privileges(0), any_privileges(0) {
  set_field(f);
  /*
...
Item_field::Item_field(THD *thd, Name_resolution_context *context_arg, Field *f)
    : Item_ident(context_arg, f->table->s->db, *f->table_name, f->field_name),
      item_equal(0), no_const_subst(0), have_privileges(0), any_privileges(0) {
  /*
...
Item_field::Item_field(Name_resolution_context *context_arg, const char *db_arg,
    ...
    : Item_ident(context_arg, db_arg, table_name_arg, field_name_arg), field(0),
      result_field(0), item_equal(0), no_const_subst(0), have_privileges(0),
      any_privileges(0) {
...
Item_field::Item_field(THD *thd, Item_field *item)
    : Item_ident(thd, item), field(item->field),
      result_field(item->result_field), item_equal(item->item_equal),
...
Item_decimal::Item_decimal(const char *bin, int precision, int scale) {
  ...
  unsigned_flag = !decimal_value.sign();
  max_length =
      my_decimal_precision_to_length(precision, decimals, unsigned_flag);
}
...
double Item_string::val_real() {
  ...
  tmp =
      my_strntod(cs, (char *)str_value.ptr(), str_value.length(), &end, &error);
...
Item_param::Item_param(unsigned pos_in_query_arg)
    : state(NO_VALUE), item_result_type(STRING_RESULT),
      /* Don't pretend to be a literal unless value for this item is set. */
      item_type(PARAM_ITEM), param_type(MYSQL_TYPE_VARCHAR),
      pos_in_query(pos_in_query_arg), set_param_func(default_set_param_func) {
  name = (char *)"?";
  /*
...
longlong Item_param::val_int() {
  ...
  case LONG_DATA_VALUE: {
    int dummy_err;
    return my_strntoll(str_value.charset(), str_value.ptr(), str_value.length(),
                       10, (char **)0, &dummy_err);
  }
  case TIME_VALUE:
...
const String *Item_param::query_val_str(String *str) const {
  ...
  case LONG_DATA_VALUE: {
    ...
    buf = str->c_ptr_quick();
    ptr = buf;
    if (value.cs_info.character_set_client
...
static void mark_as_dependent(THD *thd, SELECT_LEX *last, SELECT_LEX *current,
  ...
  if (thd->lex->describe & DESCRIBE_EXTENDED) {
    ...
    sprintf(warn_buff, ER(ER_WARN_FIELD_RESOLVED), db_name,
            (db_name[0] ? "." : ""), table_name, (table_name[0] ? "." : ""),
            resolved_item->field_name, current->select_number,
            last->select_number);
    push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_WARN_FIELD_RESOLVED,
                 warn_buff);
  }
}
...
bool Item_field::fix_fields(THD *thd, Item **reference) {
    ...
    */
    if ((from_field = find_field_in_tables(
      ...
      if (thd->lex->current_select->is_item_list_lookup) {
        ...
        bool not_used;
        Item **res =
            find_item_in_list(this, thd->lex->current_select->item_list,
                              &counter, REPORT_EXCEPT_NOT_FOUND, &not_used);
        ...
        */
        if ((place != IN_HAVING ||
            ...
            (from_field = find_field_in_tables(
                 ...
                 IGNORE_EXCEPT_NON_UNIQUE, outer_context->check_privileges,
                 TRUE)) != not_found_field) {
          if (from_field) {
      ...
      if (ref == not_found_item && from_field == not_found_field) {
        if (upward_lookup) {
          ...
          find_field_in_tables(
              thd, this, context->first_name_resolution_table,
              context->last_name_resolution_table, reference, REPORT_ALL_ERRORS,
              !any_privileges && context->check_privileges, TRUE);
        }
        goto error;
  ...
  if (any_privileges) {
    ...
    if (!(have_privileges = (get_column_grant(thd, &field->table->grant, db,
                                              tab, field_name) &
                             VIEW_ANY_ACL))) {
...
void Item::init_make_field(Send_field *tmp_field,
  ...
  tmp_field->flags = (maybe_null ? 0 : NOT_NULL_FLAG) |
                     (my_binary_compare(collation.collation) ? BINARY_FLAG : 0);
...
Field *Item::tmp_table_field_from_field_type(TABLE *table) {
  ...
  */
  ...
  case MYSQL_TYPE_TINY:
    return new Field_tiny((char *)0, max_length, null_ptr, 0, Field::NONE, name,
                          table, 0, unsigned_flag);
  case MYSQL_TYPE_SHORT:
    return new Field_short((char *)0, max_length, null_ptr, 0, Field::NONE,
                           name, table, 0, unsigned_flag);
  case MYSQL_TYPE_LONG:
    return new Field_long((char *)0, max_length, null_ptr, 0, Field::NONE, name,
                          table, 0, unsigned_flag);
  ...
  case MYSQL_TYPE_NULL:
    return new Field_null((char *)0, max_length, Field::NONE, name, table,
                          &my_charset_bin);
  ...
  case MYSQL_TYPE_YEAR:
    return new Field_year((char *)0, max_length, null_ptr, 0, Field::NONE, name,
                          table);
  case MYSQL_TYPE_BIT:
...
int Item_hex_string::save_in_field(Field *field, bool no_conversions) {
  ...
  if (field->result_type() == STRING_RESULT) {
    error =
        field->store(str_value.ptr(), str_value.length(), collation.collation);
  } else {
    longlong nr = val_int();
...
bool Item_ref::fix_fields(THD *thd, Item **reference) {
  ...
  */
  if (((*ref)->with_sum_func && name &&
       (depended_from || !(current_sel->linkage != GLOBAL_OPTIONS_TYPE &&
...
bool Item_direct_view_ref::eq(const Item *item, bool binary_cmp) const {
  ...
  return (!it->name ||
          !my_strcasecmp(system_charset_info, it->name, field_name)) &&
         ref && (*ref)->real_item()->eq(it, binary_cmp);
}
  ...
  if (!arg) {
    if (field_arg->flags & NO_DEFAULT_VALUE_FLAG) {
      if (context->error_processor == &view_error_processor) {
        ...
        push_warning_printf(
            field_arg->table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN,
            ER_NO_DEFAULT_FOR_VIEW_FIELD, ER(ER_NO_DEFAULT_FOR_VIEW_FIELD),
...
void Item_trigger_field::setup_field(THD *thd, TABLE *table) {
  ...
  */
  (void)find_field_in_table(thd, table, field_name, (uint)strlen(field_name), 0,
                            0, &field_idx);
...
void resolve_const_item(THD *thd, Item **ref, Item *comp_item) {
  ...
  case REAL_RESULT: { // It must REAL_RESULT
    ...
    bool null_value = item->null_value;
    new_item =
        (null_value ? (Item *)new Item_null(name)
                    : (Item *)new Item_float(name, result, decimals, length));
    break;
  }
  case DECIMAL_RESULT: {
    my_decimal decimal_value;
...
bool Item_type_holder::join_types(THD *thd, Item *item) {
  ...
  DBUG_PRINT("info:", ("was type %d len %d, dec %d name %s", fld_type,
                       max_length, decimals, (name ? name : "<NULL>")));
  DBUG_PRINT("info:", ("in type %d len %d, dec %d", get_real_type(item),
  ...
  if (Field::result_merge_type(fld_type) == DECIMAL_RESULT) {
    ...
    int precision =
        min(max(prev_decimal_int_part, item->decimal_int_part()) + decimals,
            DECIMAL_MAX_PRECISION);
    unsigned_flag &= item->unsigned_flag;
    max_length =
        my_decimal_precision_to_length(precision, decimals, unsigned_flag);
  ...
  case STRING_RESULT: {
    ...
    if (collation.aggregate(item->collation)) {
      my_error(ER_CANT_AGGREGATE_2COLLATIONS, MYF(0), old_cs, old_derivation,
  ...
  case REAL_RESULT: {
    if (decimals != NOT_FIXED_DEC) {
      ...
      max_length =
          min(max(delta1, delta2) + decimals,
  ...
  DBUG_PRINT("info", ("become type: %d  len: %u  dec: %u", (int)fld_type,
...
Field *Item_type_holder::make_field_by_type(TABLE *table) {
  ...
  */
  ...
  case MYSQL_TYPE_ENUM:
    ...
    return new Field_enum((char *)0, max_length, null_ptr, 0, Field::NONE, name,
                          table, get_enum_pack_length(enum_set_typelib->count),
                          enum_set_typelib, collation.collation);
  case MYSQL_TYPE_SET:
    DBUG_ASSERT(enum_set_typelib);
    return new Field_set((char *)0, max_length, null_ptr, 0, Field::NONE, name,
