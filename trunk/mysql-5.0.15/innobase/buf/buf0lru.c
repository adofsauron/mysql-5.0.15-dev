...
static void buf_LRU_block_remove_hashed_page(
    /*=============================*/
...
void buf_LRU_invalidate_tablespace(
    /*==========================*/
...
scan_again:
  ...
  while (block != NULL)
  {
    ...
    if (block->space == id && (block->buf_fix_count > 0 || block->io_fix != 0))
    {
    ...
    if (block->space == id)
    {
      ...
      if (buf_debug_prints)
      {
        printf("Dropping space %lu page %lu\n", (ulong)block->space, (ulong)block->offset);
      }
#endif
      if (block->is_hashed)
      {
      ...
      if (0 != ut_dulint_cmp(block->oldest_modification, ut_dulint_zero))
      {
                        ...
                        of the LRU list */
{
  ...
  while (block != NULL)
  {
    ...
    if (!freed && n_iterations <= 10 && distance > 100 + (n_iterations * buf_pool->curr_size) / 10)
    {
...
left */
{
  ...
  if (!recv_recovery_on && UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU) < buf_pool->max_size / 7)
  {
    ret = TRUE;
  }
...
loop:
  ...
  if (!recv_recovery_on && UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU) < buf_pool->max_size / 10)
  {
    ...
    ut_error;
           ...
           UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU) < buf_pool->max_size / 5)
  {
  ...
  if (UT_LIST_GET_LEN(buf_pool->free) > 0)
  {
    ...
    if (srv_use_awe)
    {
      if (block->frame)
      {
        ...
        UT_LIST_REMOVE(awe_LRU_free_mapped, buf_pool->awe_LRU_free_mapped, block);
      }
      else
      {
...
UNIV_INLINE
void buf_LRU_add_block_to_end_low(
    /*=========================*/
    buf_block_t *block) /* in: control block */
{
    ...
    buf_block_t *block) /* in: block, must not contain a file page */
{
  ...
  ut_a((block->state == BUF_BLOCK_MEMORY) || (block->state == BUF_BLOCK_READY_FOR_USE));
                        ...
                        may or may not be a hash index to the page */
{
  ...
  {
    fprintf(stderr, "InnoDB: Error: page %lu %lu not found from the hash table\n", (ulong)block->space,
            (ulong)block->offset);
    if (buf_page_hash_get(block->space, block->offset))
    {
      fprintf(stderr, "InnoDB: From hash table we find block %p of %lu %lu which is not %p\n",
              buf_page_hash_get(block->space, block->offset),
              (ulong)buf_page_hash_get(block->space, block->offset)->space,
              (ulong)buf_page_hash_get(block->space, block->offset)->offset, block);
    }
...
/*===============*/
{
  ...
  while (block != NULL)
  {
    ...
    {
      fprintf(stderr, "buffix count %lu ", (ulong)block->buf_fix_count);
    }
    ...
    fprintf(stderr, "LRU pos %lu type %lu index id %lu ", (ulong)block->LRU_position, (ulong)fil_page_get_type(frame),
