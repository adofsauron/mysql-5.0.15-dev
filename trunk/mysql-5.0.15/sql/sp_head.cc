...
static Item *sp_prepare_func_item(THD *thd, Item **it_addr)
{
  Item *it = *it_addr;
...
Item *sp_eval_func_item(THD *thd, Item **it_addr, enum enum_field_types type, Item *reuse, bool use_callers_arena)
{
  ...
  if (reuse)
  {
    ...
    p_free_list = use_callers_arena ? &thd->spcont->callers_arena->free_list : &thd->free_list;
  ...
  switch (sp_map_result_type(type))
  {
    case INT_RESULT:
    {
      ...
      CREATE_ON_CALLERS_ARENA(it = new (reuse, &rsize) Item_int(i), use_callers_arena, &backup_arena);
      break;
    }
    case REAL_RESULT:
    {
      ...
      */
      ...
      CREATE_ON_CALLERS_ARENA(it = new (reuse, &rsize) Item_float(d), use_callers_arena, &backup_arena);
    ...
    case STRING_RESULT:
    {
      ...
      DBUG_PRINT("info", ("STRING_RESULT: %.*s", s->length(), s->c_ptr_quick()));
      /*
...
return_null_item:
  CREATE_ON_CALLERS_ARENA(it = new (reuse, &rsize) Item_null(), use_callers_arena, &backup_arena);
end:
  it->rsize = rsize;
...
sp_name *sp_name_current_db_new(THD *thd, LEX_STRING name)
{
  sp_name *qname;
...
void *sp_head::operator new(size_t size)
{
...
void sp_head::operator delete(void *ptr, size_t size)
{
  ...
  DBUG_PRINT("info", ("mem_root 0x%lx moved to 0x%lx", (ulong)&sp->mem_root, (ulong)&own_root));
  free_root(&own_root, MYF(0));

  DBUG_VOID_RETURN;
}

sp_head::sp_head() : Query_arena(&main_mem_root, INITIALIZED_FOR_SP), m_flags(0), m_returns_cs(NULL)
{
...
void sp_head::init(LEX *lex)
{
  ...
  */
  ...
  m_qname.str = m_db.str = m_name.str = m_params.str = m_body.str = m_defstr.str = 0;
  m_qname.length = m_db.length = m_name.length = m_params.length = m_body.length = m_defstr.length = 0;
  m_returns_cs = NULL;
  DBUG_VOID_RETURN;
}

void sp_head::init_strings(THD *thd, LEX *lex, sp_name *name)
{
  ...
  if (m_param_begin && m_param_end)
  {
    ...
    m_params.str = strmake_root(root, (char *)m_param_begin, m_params.length);
  }
...
TYPELIB *sp_head::create_typelib(List<String> *src)
{
  TYPELIB *result = NULL;
  ...
  if (src->elements)
  {
    ...
    if (!(result->type_names =
              (const char **)alloc_root(mem_root, (sizeof(char *) + sizeof(int)) * (result->count + 1))))
      return 0;
    ...
    for (uint i = 0; i < result->count; i++)
    {
      ...
      if (String::needs_conversion(tmp->length(), tmp->charset(), cs, &dummy))
      {
        ...
        length = conv.length();
        result->type_names[i] = (char *)strmake_root(mem_root, conv.ptr(), length);
      }
      else
      {
        length = tmp->length();
...
int sp_head::create(THD *thd)
{
  ...
  DBUG_PRINT("info", ("type: %d name: %s params: %s body: %s", m_type, m_name.str, m_params.str, m_body.str));
...
Field *sp_head::make_field(uint max_length, const char *name, TABLE *dummy)
{
  Field *field;
  DBUG_ENTER("sp_head::make_field");

  field = ::make_field((char *)0, !m_returns_len ? max_length : m_returns_len, (uchar *)"", 0, m_returns_pack,
                       m_returns, m_returns_cs, m_geom_returns, Field::NONE, m_returns_typelib,
...
static bool subst_spvars(THD *thd, sp_instr *instr, LEX_STRING *query_str)
{
  ...
  if (thd->prelocked_mode == NON_PRELOCKED && mysql_bin_log.is_open())
  {
    ...
    */
    ...
    cur = query_str->str;
    prev_pos = res = 0;
    for (Item_splocal **splocal = sp_vars_uses.front(); splocal < sp_vars_uses.back(); splocal++)
    {
      Item *val;
...
int sp_head::execute(THD *thd)
{
  ...
  Query_arena execute_arena(&execute_mem_root, INITIALIZED_FOR_SP), backup_arena;
...
int sp_head::execute_function(THD *thd, Item **argp, uint argcount, Item **resp)
{
  Item **param_values;
  ...
  if (argcount != params)
  {
    ...
    */
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "FUNCTION", m_qname.str, params, argcount);
    goto end;
  }
  ...
  */
  for (; i < csize; i++) nctx->push_item(NULL);
  ...
  if (need_binlog_call && thd->binlog_evt_union.unioned_events)
  {
    ...
    Query_log_event qinfo(thd, bufstr.ptr(), bufstr.length(), thd->binlog_evt_union.unioned_events_trans, FALSE);
    if (mysql_bin_log.write(&qinfo) && thd->binlog_evt_union.unioned_events_trans)
    {
...
int sp_head::execute_procedure(THD *thd, List<Item> *args)
{
  ...
  {
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "PROCEDURE", m_qname.str, params, args->elements);
    DBUG_RETURN(-1);
  }
...
void sp_head::push_backpatch(sp_instr *i, sp_label_t *lab)
{
...
void sp_head::backpatch(sp_label_t *lab)
{
  bp_t *bp;
  ...
  {
    if (bp->lab == lab ||
        (bp->lab->type == SP_LAB_REF && my_strcasecmp(system_charset_info, bp->lab->name, lab->name) == 0))
    {
...
int sp_head::check_backpatch(THD *thd)
{
  bp_t *bp;
  List_iterator_fast<bp_t> li(m_backpatch);
...
void sp_head::set_info(char *definer, uint definerlen, longlong created, longlong modified, st_sp_chistics *chistics,
                       ulong sql_mode)
{
  ...
  m_created = created;
  m_modified = modified;
  m_chistics = (st_sp_chistics *)memdup_root(mem_root, (char *)chistics, sizeof(*chistics));
  if (m_chistics->comment.length == 0)
    m_chistics->comment.str = 0;
  else
    m_chistics->comment.str = strmake_root(mem_root, m_chistics->comment.str, m_chistics->comment.length);
  m_sql_mode = sql_mode;
}
...
void sp_head::reset_thd_mem_root(THD *thd)
{
  ...
  DBUG_PRINT("info", ("mem_root 0x%lx moved to thd mem root 0x%lx", (ulong)&mem_root, (ulong)&thd->mem_root));
...
void sp_head::restore_thd_mem_root(THD *thd)
{
  ...
  DBUG_PRINT("info", ("mem_root 0x%lx returned from thd mem root 0x%lx", (ulong)&mem_root, (ulong)&thd->mem_root));
...
bool check_show_routine_access(THD *thd, sp_head *sp, bool *full_access)
{
  ...
  *full_access = (!check_table_access(thd, SELECT_ACL, &tables, 1) ||
                  (!strcmp(sp->m_definer_user.str, thd->security_ctx->priv_user) &&
                   !strcmp(sp->m_definer_host.str, thd->security_ctx->priv_host)));
  if (!*full_access)
...
int sp_head::show_create_procedure(THD *thd)
{
  ...
  field_list.push_back(new Item_empty_string("Create Procedure", max(buffer.length(), 1024)));
...
int sp_head::show_create_function(THD *thd)
{
  ...
  field_list.push_back(new Item_empty_string("sql_mode", sql_mode_len));
  field_list.push_back(new Item_empty_string("Create Function", max(buffer.length(), 1024)));
  if (protocol->send_fields(&field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
  {
    res = 1;
    goto done;
...
void sp_head::opt_mark(uint ip)
{
  ...
  while ((i = get_instr(ip)) && !i->marked) ip = i->opt_mark(this);
}
...
int sp_lex_keeper::reset_lex_and_exec_core(THD *thd, uint *nextp, bool open_tables, sp_instr *instr)
{
  int res = 0;
...
int sp_instr_stmt::execute(THD *thd, uint *nextp)
{
  char *query;
  uint32 query_length;
  int res;
  ...
  if (!(res = alloc_query(thd, m_query.str, m_query.length + 1)) && !(res = subst_spvars(thd, this, &m_query)))
  {
    /*
...
int sp_instr_set::execute(THD *thd, uint *nextp)
{
...
int sp_instr_set::exec_core(THD *thd, uint *nextp)
{
...
int sp_instr_set_trigger_field::execute(THD *thd, uint *nextp)
{
  DBUG_ENTER("sp_instr_set_trigger_field::execute");
  DBUG_RETURN(m_lex_keeper.reset_lex_and_exec_core(thd, nextp, TRUE, this));
}

int sp_instr_set_trigger_field::exec_core(THD *thd, uint *nextp)
{
  int res = 0;
  Item *it = sp_prepare_func_item(thd, &value);
  if (!it || !trigger_field->fixed && trigger_field->fix_fields(thd, 0) ||
...
int sp_instr_jump::execute(THD *thd, uint *nextp)
{
...
void sp_instr_jump::opt_move(uint dst, List<sp_instr> *bp)
{
  if (m_dest > m_ip)
...
int sp_instr_jump_if::execute(THD *thd, uint *nextp)
{
...
int sp_instr_jump_if::exec_core(THD *thd, uint *nextp)
{
  Item *it;
  int res;
...
void sp_instr_jump_if::print(String *str)
{
  str->reserve(12);
  str->append("jump_if ");
  str->qs_append(m_dest);
...
uint sp_instr_jump_if::opt_mark(sp_head *sp)
{
  sp_instr *i;
...
int sp_instr_jump_if_not::execute(THD *thd, uint *nextp)
{
...
int sp_instr_jump_if_not::exec_core(THD *thd, uint *nextp)
{
  Item *it;
  int res;
...
uint sp_instr_jump_if_not::opt_mark(sp_head *sp)
{
  sp_instr *i;
...
int sp_instr_freturn::execute(THD *thd, uint *nextp)
{
  DBUG_ENTER("sp_instr_freturn::execute");
  DBUG_RETURN(m_lex_keeper.reset_lex_and_exec_core(thd, nextp, TRUE, this));
}

int sp_instr_freturn::exec_core(THD *thd, uint *nextp)
{
  Item *it;
  int res;
...
int sp_instr_hpush_jump::execute(THD *thd, uint *nextp)
{
  DBUG_ENTER("sp_instr_hpush_jump::execute");
  List_iterator_fast<sp_cond_type_t> li(m_cond);
  sp_cond_type_t *p;

  while ((p = li++)) thd->spcont->push_handler(p, m_ip + 1, m_type, m_frame);
...
int sp_instr_hpop::execute(THD *thd, uint *nextp)
{
...
void sp_instr_hpop::backpatch(uint dest, sp_pcontext *dst_ctx) { m_count = m_ctx->diff_handlers(dst_ctx); }

/*
  sp_instr_hreturn class functions
*/

int sp_instr_hreturn::execute(THD *thd, uint *nextp)
{
...
int sp_instr_cpush::execute(THD *thd, uint *nextp)
{
  Query_arena backup_arena;
...
void sp_instr_cpush::print(String *str) { str->append("cpush"); }

/*
  sp_instr_cpop class functions
*/

int sp_instr_cpop::execute(THD *thd, uint *nextp)
{
...
void sp_instr_cpop::backpatch(uint dest, sp_pcontext *dst_ctx) { m_count = m_ctx->diff_cursors(dst_ctx); }

/*
  sp_instr_copen class functions
*/

int sp_instr_copen::execute(THD *thd, uint *nextp)
{
  /*
...
int sp_instr_copen::exec_core(THD *thd, uint *nextp)
{
  sp_cursor *c = thd->spcont->get_cursor(m_cursor);
  int res = c->open(thd);
  *nextp = m_ip + 1;
...
int sp_instr_cclose::execute(THD *thd, uint *nextp)
{
  sp_cursor *c = thd->spcont->get_cursor(m_cursor);
  int res;
...
int sp_instr_cfetch::execute(THD *thd, uint *nextp)
{
  sp_cursor *c = thd->spcont->get_cursor(m_cursor);
  int res;
  Query_arena backup_arena;
...
int sp_instr_error::execute(THD *thd, uint *nextp)
{
...
bool sp_change_security_context(THD *thd, sp_head *sp, Security_context **backup)
{
  *backup = 0;
  if (sp->m_chistics->suid != SP_IS_NOT_SUID &&
      (strcmp(sp->m_definer_user.str, thd->security_ctx->priv_user) ||
       my_strcasecmp(system_charset_info, sp->m_definer_host.str, thd->security_ctx->priv_host)))
  {
    if (acl_getroot_no_password(&sp->m_security_ctx, sp->m_definer_user.str, sp->m_definer_host.str,
                                sp->m_definer_host.str, sp->m_db.str))
    {
      ...
      */
      my_error(ER_NO_SUCH_USER, MYF(0), sp->m_definer_user.str, sp->m_definer_host.str);
      return TRUE;
#else
      return FALSE;
#endif
...
byte *sp_table_key(const byte *ptr, uint *plen, my_bool first)
{
...
bool sp_head::merge_table_list(THD *thd, TABLE_LIST *table, LEX *lex_for_tmp_check)
{
  SP_TABLE *tab;
...
bool sp_head::add_used_tables_to_table_list(THD *thd, TABLE_LIST ***query_tables_last_ptr)
{
  uint i;
...
TABLE_LIST *sp_add_to_query_tables(THD *thd, LEX *lex, const char *db, const char *name, thr_lock_type locktype)
{
  TABLE_LIST *table;
