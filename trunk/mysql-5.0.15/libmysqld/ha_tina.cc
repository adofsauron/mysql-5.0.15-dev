...
*/
int sort_set(tina_set *a, tina_set *b) { return (a->begin > b->begin ? 1 : (a->begin < b->begin ? -1 : 0)); }

static byte *tina_get_key(TINA_SHARE *share, uint *length, my_bool not_used __attribute__((unused)))
{
...
int get_mmap(TINA_SHARE *share, int write)
{
  ...
  if (share->file_stat.st_size)
  {
    if (write)
      share->mapped_file =
          (byte *)mmap(NULL, share->file_stat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, share->data_file, 0);
    else
      share->mapped_file = (byte *)mmap(NULL, share->file_stat.st_size, PROT_READ, MAP_PRIVATE, share->data_file, 0);
...
static TINA_SHARE *get_share(const char *table_name, TABLE *table)
{
  ...
  if (!tina_init)
  {
    ...
    if (!tina_init)
    {
      ...
      (void)hash_init(&tina_open_tables, system_charset_info, 32, 0, 0, (hash_get_key)tina_get_key, 0, 0);
    }
    pthread_mutex_unlock(&LOCK_mysql_create_db);
  }
  pthread_mutex_lock(&tina_mutex);
  length = (uint)strlen(table_name);
  if (!(share = (TINA_SHARE *)hash_search(&tina_open_tables, (byte *)table_name, length)))
  {
...
int ha_tina::find_current_row(byte *buf)
{
  ...
  for (Field **field = table->field; *field; field++)
  {
    ...
    for (; mapped_ptr != end_ptr; mapped_ptr++)
    {
      ...
      if (*mapped_ptr == '"' && (((mapped_ptr[1] == ',') && (mapped_ptr[2] == '"')) || (mapped_ptr == end_ptr - 1)))
      {
...
/*
  ...
  with a key.
*/
int ha_tina::index_read(byte *buf, const byte *key, uint key_len __attribute__((unused)),
...
*/
int ha_tina::index_read_idx(byte *buf, uint index, const byte *key, uint key_len __attribute__((unused)),
                            enum ha_rkey_function find_flag __attribute__((unused)))
{
  ...
  if ((chain_ptr - chain) > 0)
  {
    ...
    */
    ...
    for (ptr = chain; ptr < chain_ptr; ptr++)
    {
      ...
      if (ptr + 1 == chain_ptr)
        memmove(share->mapped_file + ptr->begin, share->mapped_file + ptr->end, length - (size_t)ptr->end);
      else
...
/*
  ...
  is an internal lock.
*/
THR_LOCK_DATA **ha_tina::store_lock(THD *thd, THR_LOCK_DATA **to, enum thr_lock_type lock_type)
{
