...
{
  if (*copy->null_row || copy->from_null_ptr && (*copy->from_null_ptr & copy->from_bit))
  {
    bzero(copy->to_ptr, copy->from_length);
    copy->to_null_ptr[0] = 1;  // Always bit 1
  }
...
static void do_conv_blob(Copy_field *copy)
{
  ...
  ((Field_blob *)copy->to_field)->store(copy->tmp.ptr(), copy->tmp.length(), copy->tmp.charset());
}
...
static void do_field_int(Copy_field *copy)
{
  ...
  copy->to_field->store(value, test(copy->from_field->flags & UNSIGNED_FLAG));
}
...
static void do_cut_string(Copy_field *copy)
{
  ...
  if (cs->cset->scan(cs, copy->from_ptr + copy->to_length, copy->from_ptr + copy->from_length, MY_SEQ_SPACES) <
      copy->from_length - copy->to_length)
  {
    copy->to_field->set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
  }
}
...
{  // Shorter string field
  ...
  if (well_formed_error ||
      cs->cset->scan(cs, copy->from_ptr + copy_length, from_end, MY_SEQ_SPACES) < (copy->from_length - copy_length))
  {
    copy->to_field->set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
  }
...
static void do_expand_string(Copy_field *copy)
{
  CHARSET_INFO *cs = copy->from_field->charset();
  memcpy(copy->to_ptr, copy->from_ptr, copy->from_length);
  cs->cset->fill(cs, copy->to_ptr + copy->from_length, copy->to_length - copy->from_length, ' ');
}
...
static void do_varstring2(Copy_field *copy)
{
  ...
  memcpy(copy->to_ptr + HA_KEY_BLOB_LENGTH, copy->from_ptr + HA_KEY_BLOB_LENGTH, length);
}
...
void (*Copy_field::get_copy_func(Field *to, Field *from))(Copy_field *)
{
  ...
  else
  {
    ...
    if (from->result_type() == STRING_RESULT)
    {
      ...
      */
      if (to->real_type() != from->real_type() || !compatible_db_low_byte_first ||
          ((to->table->in_use->variables.sql_mode & (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_INVALID_DATES)) &&
      ...
      else if (to->real_type() == MYSQL_TYPE_VARCHAR)
      {
        ...
        return (from->charset()->mbmaxlen == 1 ? do_cut_string : do_cut_string_complex);
      else if (to_length > from_length)
...
void field_conv(Field *to, Field *from)
{
  ...
  {
    if (to->pack_length() == from->pack_length() && !(to->flags & UNSIGNED_FLAG && !(from->flags & UNSIGNED_FLAG)) &&
        to->real_type() != FIELD_TYPE_ENUM && to->real_type() != FIELD_TYPE_SET && to->real_type() != FIELD_TYPE_BIT &&
        (to->real_type() != FIELD_TYPE_NEWDECIMAL ||
         (to->field_length == from->field_length && (((Field_num *)to)->dec == ((Field_num *)from)->dec))) &&
        from->charset() == to->charset() && to->table->s->db_low_byte_first == from->table->s->db_low_byte_first &&
        (!(to->table->in_use->variables.sql_mode & (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_INVALID_DATES)) ||
         to->type() != FIELD_TYPE_DATE && to->type() != FIELD_TYPE_DATETIME) &&
        (from->real_type() != MYSQL_TYPE_VARCHAR ||
         ((Field_varstring *)from)->length_bytes == ((Field_varstring *)to)->length_bytes))
    {  // Identical fields
#ifdef HAVE_purify
  ...
  {  // Be sure the value is stored
    ...
    if (!blob->value.is_alloced() && from->real_type() != MYSQL_TYPE_STRING && from->real_type() != MYSQL_TYPE_VARCHAR)
      blob->value.copy();
  ...
  if ((from->result_type() == STRING_RESULT &&
       (to->result_type() == STRING_RESULT ||
        (from->real_type() != FIELD_TYPE_ENUM && from->real_type() != FIELD_TYPE_SET))) ||
