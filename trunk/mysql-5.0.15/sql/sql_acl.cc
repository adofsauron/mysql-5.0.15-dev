...
static bool update_user_table(THD *thd, TABLE *table, const char *host, const char *user, const char *new_password,
...
my_bool acl_init(bool dont_read_acl_tables)
{
  ...
  acl_cache =
      new hash_filo(ACL_CACHE_SIZE, 0, 0, (hash_get_key)acl_entry_get_key, (hash_free_key)free, system_charset_info);
...
static my_bool acl_load(THD *thd, TABLE_LIST *tables)
{
  ...
  qsort((gptr)dynamic_element(&acl_hosts, 0, ACL_HOST *), acl_hosts.elements, sizeof(ACL_HOST), (qsort_cmp)acl_compare);
  ...
  DBUG_PRINT("info", ("user table fields: %d, password length: %d", table->s->fields, table->field[2]->field_length));
      ...
      switch (password_len)
      {
        case 45: /* 4.1: to be removed */
          sql_print_warning(
              ...
              user.user ? user.user : "", user.host.hostname ? user.host.hostname : "");
          break;
        default:
          sql_print_warning(
    ...
    else  // password is correct
    {
      ...
      */
      ...
      if (table->s->fields >= 31)
      {
        ...
        if (user.user_resource.questions || user.user_resource.updates || user.user_resource.conn_per_hour)
          mqh_used = 1;
      ...
      if (!user.host.hostname || user.host.hostname[0] == wild_many && !user.host.hostname[1])
        allow_all_hosts = 1;  // Anyone can connect
    }
  }
  qsort((gptr)dynamic_element(&acl_users, 0, ACL_USER *), acl_users.elements, sizeof(ACL_USER), (qsort_cmp)acl_compare);
  end_read_record(&read_record_info);
  ...
  qsort((gptr)dynamic_element(&acl_dbs, 0, ACL_DB *), acl_dbs.elements, sizeof(ACL_DB), (qsort_cmp)acl_compare);
  end_read_record(&read_record_info);
  freeze_size(&acl_dbs);
  init_check_host();
...
my_bool acl_reload(THD *thd)
{
  ...
  */
  ...
  {
    sql_print_error("Fatal error: Can't open and lock privilege tables: %s", thd->net.last_error);
    goto end;
  }
...
static ulong get_access(TABLE *form, uint fieldnr, uint *next_field)
{
  ...
  for (pos = form->field + fieldnr, bit = 1;
       *pos && (*pos)->real_type() == FIELD_TYPE_ENUM && ((Field_enum *)(*pos))->typelib->count == 2;
       pos++, fieldnr++, bit <<= 1)
  {
    (*pos)->val_str(&res);
  ...
  for (uint i = 0; i < acl_users.elements; i++)
  {
    ...
    if (!acl_user_tmp->user || !strcmp(sctx->user, acl_user_tmp->user))
    {
      if (compare_hostname(&acl_user_tmp->host, sctx->host, sctx->ip))
      {
        ...
        else if (passwd_len == SCRAMBLE_LENGTH && acl_user_tmp->salt_len == SCRAMBLE_LENGTH_323)
          res = -1;
        else if (passwd_len == SCRAMBLE_LENGTH_323 && acl_user_tmp->salt_len == SCRAMBLE_LENGTH)
          res = 2;
    ...
    */
    switch (acl_user->ssl_type)
    {
      ...
      case SSL_TYPE_ANY:  // Any kind of SSL is ok
        if (vio_type(vio) == VIO_TYPE_SSL) user_access = acl_user->access;
        break;
      case SSL_TYPE_X509: /* Client should have any valid certificate. */
        ...
        */
        if (vio_type(vio) == VIO_TYPE_SSL && SSL_get_verify_result(ssl) == X509_V_OK && SSL_get_peer_certificate(ssl))
          user_access = acl_user->access;
        break;
      case SSL_TYPE_SPECIFIED: /* Client should have specified attrib */
        ...
        */
        X509 *cert;
        if (vio_type(vio) != VIO_TYPE_SSL || SSL_get_verify_result(ssl) != X509_V_OK) break;
        if (acl_user->ssl_cipher)
        {
          ...
          else
          {
            if (global_system_variables.log_warnings)
              sql_print_information("X509 ciphers mismatch: should be '%s' but is '%s'", acl_user->ssl_cipher,
                                    SSL_get_cipher(ssl));
            break;
          }
        }
        ...
        if (acl_user->x509_subject)
        {
          ...
          {
            if (global_system_variables.log_warnings)
              sql_print_information("X509 subject mismatch: '%s' vs '%s'", acl_user->x509_subject, ptr);
          }
          else
            user_access = acl_user->access;
          free(ptr);
...
bool acl_getroot_no_password(Security_context *sctx, char *user, char *host, char *ip, char *db)
{
  ...
  */
  for (i = 0; i < acl_users.elements; i++)
  {
    ...
    if ((!acl_user->user && (!user || !user[0])) || (acl_user->user && strcmp(user, acl_user->user) == 0))
    {
...
static void acl_update_user(const char *user, const char *host, const char *password, uint password_len,
                            enum SSL_type ssl_type, const char *ssl_cipher, const char *x509_issuer,
                            const char *x509_subject, USER_RESOURCES *mqh, ulong privileges)
{
  for (uint i = 0; i < acl_users.elements; i++)
  {
    ...
    if (!acl_user->user && !user[0] || acl_user->user && !strcmp(user, acl_user->user))
    {
      if (!acl_user->host.hostname && !host[0] ||
          acl_user->host.hostname && !my_strcasecmp(system_charset_info, host, acl_user->host.hostname))
      {
...
static void acl_insert_user(const char *user, const char *host, const char *password, uint password_len,
                            enum SSL_type ssl_type, const char *ssl_cipher, const char *x509_issuer,
                            const char *x509_subject, USER_RESOURCES *mqh, ulong privileges)
{
  ACL_USER acl_user;
  ...
  if (!acl_user.host.hostname || acl_user.host.hostname[0] == wild_many && !acl_user.host.hostname[1])
  ...
  for (uint i = 0; i < acl_dbs.elements; i++)
  {
    ...
    if (!acl_db->user && !user[0] || acl_db->user && !strcmp(user, acl_db->user))
    {
      if (!acl_db->host.hostname && !host[0] ||
          acl_db->host.hostname && !my_strcasecmp(system_charset_info, host, acl_db->host.hostname))
      {
    ...
    if (!acl_db->user || !strcmp(user, acl_db->user))
    {
      if (compare_hostname(&acl_db->host, host, ip))
      {
        if (!acl_db->db || !wild_compare(db, acl_db->db, db_is_pattern))
        {
          ...
          if (acl_db->host.hostname) goto exit;  // Fully specified. Take it
...
exit:
  ...
  if (!db_is_pattern && (entry = (acl_entry *)malloc(sizeof(acl_entry) + key_length)))
  {
...
static void init_check_host(void)
{
  ...
  if (!allow_all_hosts)
  {
    ...
    {
      ACL_USER *acl_user = dynamic_element(&acl_users, i, ACL_USER *);
      if (strchr(acl_user->host.hostname, wild_many) || strchr(acl_user->host.hostname, wild_one) ||
      ...
      {  // Has wildcard
        ...
        {  // Check if host already exists
          ...
          if (!my_strcasecmp(system_charset_info, acl_user->host.hostname, acl->hostname)) break;  // already stored
        }
        if (j == acl_wild_hosts.elements)  // If new
          (void)push_dynamic(&acl_wild_hosts, (char *)&acl_user->host);
      }
      else if (!hash_search(&acl_check_hosts, (byte *)&acl_user->host, (uint)strlen(acl_user->host.hostname)))
      {
...
bool check_change_password(THD *thd, const char *host, const char *user, char *new_password, uint new_password_len)
{
  ...
  if (!thd->slave_thread &&
      (strcmp(thd->security_ctx->user, user) || my_strcasecmp(system_charset_info, host, thd->security_ctx->priv_host)))
  {
  ...
  */
  if (thd->slave_thread && table_rules_on)
  {
    ...
    */
    ...
    if (!tables_ok(thd, &tables)) DBUG_RETURN(0);
  }
#endif

  if (!(table = open_ltable(thd, &tables, TL_WRITE))) DBUG_RETURN(1);
  ...
  if (update_user_table(thd, table, acl_user->host.hostname ? acl_user->host.hostname : "",
                        acl_user->user ? acl_user->user : "", new_password, new_password_len))
  {
  ...
  {
    query_length = my_sprintf(
        buff, (buff, "SET PASSWORD FOR \"%-.120s\"@\"%-.120s\"=\"%-.120s\"", acl_user->user ? acl_user->user : "",
               acl_user->host.hostname ? acl_user->host.hostname : "", new_password));
    thd->clear_error();
...
static ACL_USER *find_acl_user(const char *host, const char *user, my_bool exact)
{
  ...
  {
    ACL_USER *acl_user = dynamic_element(&acl_users, i, ACL_USER *);
    DBUG_PRINT("info", ("strcmp('%s','%s'), compare_hostname('%s','%s'),", user, acl_user->user ? acl_user->user : "",
                        host, acl_user->host.hostname ? acl_user->host.hostname : ""));
    if (!acl_user->user && !user[0] || acl_user->user && !strcmp(user, acl_user->user))
    {
      if (exact ? !my_strcasecmp(&my_charset_latin1, host, acl_user->host.hostname)
...
static void update_hostname(acl_host_and_ip *host, const char *hostname)
{
  ...
  if (!hostname ||
      (!(hostname = calc_ip(hostname, &host->ip, '/')) || !(hostname = calc_ip(hostname + 1, &host->ip_mask, '\0'))))
  {
...
static bool compare_hostname(const acl_host_and_ip *host, const char *hostname, const char *ip)
{
  ...
  return (!host->hostname || (hostname && !wild_case_compare(system_charset_info, hostname, host->hostname)) ||
...
bool hostname_requires_resolving(const char *hostname)
{
  ...
  if ((namelen == lhlen) && !my_strnncoll(system_charset_info, (const uchar *)hostname, namelen,
                              ...
                              uint new_password_len)
{
  ...
  key_copy((byte *)user_key, table->record[0], table->key_info, table->key_info->key_length);

  table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
  if (table->file->index_read_idx(table->record[0], 0, (byte *)user_key, table->key_info->key_length,
                                  HA_READ_KEY_EXACT))
  {
    my_message(ER_PASSWORD_NO_MATCH, ER(ER_PASSWORD_NO_MATCH), MYF(0)); /* purecov: deadcode */
...
static bool test_if_create_new_users(THD *thd)
{
  ...
  bool create_new_users =
      test(sctx->master_access & INSERT_ACL) || (!opt_safe_user_create && test(sctx->master_access & CREATE_USER_ACL));
  if (!create_new_users)
  {
    TABLE_LIST tl;
    ulong db_access;
...
static int replace_user_table(THD *thd, TABLE *table, const LEX_USER &combo, ulong rights, bool revoke_grant,
                              bool can_create_user, bool no_auto_create)
{
  ...
  table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
  if (table->file->index_read_idx(table->record[0], 0, user_key, table->key_info->key_length, HA_READ_KEY_EXACT))
  {
    /* what == 'N' means revoke */
    if (what == 'N')
    {
    ...
    */
    ...
    {
      my_error(ER_CANT_CREATE_USER_WITH_GRANT, MYF(0), thd->security_ctx->user, thd->security_ctx->host_or_ip);
      goto end;
    }
    old_row_exists = 0;
  ...
  for (tmp_field = table->field + 3, priv = SELECT_ACL;
       *tmp_field && (*tmp_field)->real_type() == FIELD_TYPE_ENUM && ((Field_enum *)(*tmp_field))->typelib->count == 2;
       tmp_field++, priv <<= 1)
  {
  ...
  if (table->s->fields >= 31) /* From 4.0.0 we have more fields */
  {
    ...
    switch (lex->ssl_type)
    {
      ...
      case SSL_TYPE_SPECIFIED:
        ...
        table->field[next_field + 3]->store("", 0, &my_charset_latin1);
        if (lex->ssl_cipher)
          table->field[next_field + 1]->store(lex->ssl_cipher, strlen(lex->ssl_cipher), system_charset_info);
        if (lex->x509_issuer)
          table->field[next_field + 2]->store(lex->x509_issuer, strlen(lex->x509_issuer), system_charset_info);
        if (lex->x509_subject)
          table->field[next_field + 3]->store(lex->x509_subject, strlen(lex->x509_subject), system_charset_info);
        break;
    ...
    if (table->s->fields >= 36 && (mqh.specified_limits & USER_RESOURCES::USER_CONNECTIONS))
  ...
  if (old_row_exists)
  {
    ...
    */
    table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
    if (cmp_record(table, record[1]) && (error = table->file->update_row(table->record[1], table->record[0])))
  ...
  {                                                                                   // This should never happen
    if (error && error != HA_ERR_FOUND_DUPP_KEY && error != HA_ERR_FOUND_DUPP_UNIQUE) /* purecov: inspected */
    {
      table->file->print_error(error, MYF(0)); /* purecov: deadcode */
      error = -1;                              /* purecov: deadcode */
      goto end;                                /* purecov: deadcode */
  ...
  if (!error)
  {
    ...
    if (old_row_exists)
      acl_update_user(combo.user.str, combo.host.str, combo.password.str, password_len, lex->ssl_type, lex->ssl_cipher,
                      lex->x509_issuer, lex->x509_subject, &lex->mqh, rights);
    else
      acl_insert_user(combo.user.str, combo.host.str, password, password_len, lex->ssl_type, lex->ssl_cipher,
                      lex->x509_issuer, lex->x509_subject, &lex->mqh, rights);
  }
  DBUG_RETURN(error);
}
...
static int replace_db_table(TABLE *table, const char *db, const LEX_USER &combo, ulong rights, bool revoke_grant)
{
  ...
  if (old_row_exists)
  {
    ...
    {
      if ((error = table->file->delete_row(table->record[1]))) goto table_error; /* purecov: deadcode */
    }
  }
...
class GRANT_COLUMN : public Sql_alloc
{
 public:
  ...
  ulong rights;
  uint key_length;
  GRANT_COLUMN(String &c, ulong y) : rights(y) { column = memdup_root(&memex, c.ptr(), key_length = c.length()); }
};
...
GRANT_NAME::GRANT_NAME(const char *h, const char *d, const char *u, const char *t, ulong p) : privs(p)
{
  /* Host given by user */
...
GRANT_TABLE::GRANT_TABLE(const char *h, const char *d, const char *u, const char *t, ulong p, ulong c)
    : GRANT_NAME(h, d, u, t, p), cols(c)
{
  (void)hash_init(&hash_columns, system_charset_info, 0, 0, 0, (hash_get_key)get_key_column, 0, 0);
}
...
GRANT_TABLE::GRANT_TABLE(TABLE *form, TABLE *col_privs) : GRANT_NAME(form)
{
  ...
  if (cols)
  {
    ...
    col_privs->field[0]->store(host.hostname, host.hostname ? (uint)strlen(host.hostname) : 0, system_charset_info);
    col_privs->field[1]->store(db, (uint)strlen(db), system_charset_info);
    col_privs->field[2]->store(user, (uint)strlen(user), system_charset_info);
    col_privs->field[3]->store(tname, (uint)strlen(tname), system_charset_info);

    key_prefix_len =
        (key_part[0].store_length + key_part[1].store_length + key_part[2].store_length + key_part[3].store_length);
    key_copy(key, col_privs->record[0], col_privs->key_info, key_prefix_len);
    col_privs->field[4]->store("", 0, &my_charset_latin1);

    col_privs->file->ha_index_init(0);
    if (col_privs->file->index_read(col_privs->record[0], (byte *)key, key_prefix_len, HA_READ_KEY_EXACT))
    {
    ...
    do
    {
      ...
      if (!(mem_check = new GRANT_COLUMN(*res, fix_rights_for_column(priv))))
      {
    ...
    } while (!col_privs->file->index_next(col_privs->record[0]) && !key_cmp_if_same(col_privs, key, 0, key_prefix_len));
    col_privs->file->ha_index_end();
  }
}

GRANT_TABLE::~GRANT_TABLE() { hash_free(&hash_columns); }
...
void free_grant_table(GRANT_TABLE *grant_table) { hash_free(&grant_table->hash_columns); }

/* Search after a matching grant. Prefer exact grants before not exact ones */

static GRANT_NAME *name_hash_search(HASH *name_hash, const char *host, const char *ip, const char *db, const char *user,
                                    const char *tname, bool exact)
{
  ...
  for (grant_name = (GRANT_NAME *)hash_search(name_hash, (byte *)helping, len); grant_name;
       grant_name = (GRANT_NAME *)hash_next(name_hash, (byte *)helping, len))
  {
    if (exact)
    {
      if (compare_hostname(&grant_name->host, host, ip)) return grant_name;
    }
    else
    {
      if (compare_hostname(&grant_name->host, host, ip) && (!found || found->sort < grant_name->sort))
...
inline GRANT_NAME *routine_hash_search(const char *host, const char *ip, const char *db, const char *user,
                                       const char *tname, bool proc, bool exact)
{
  return (GRANT_TABLE *)name_hash_search(proc ? &proc_priv_hash : &func_priv_hash, host, ip, db, user, tname, exact);
}

inline GRANT_TABLE *table_hash_search(const char *host, const char *ip, const char *db, const char *user,
                                      const char *tname, bool exact)
{
  return (GRANT_TABLE *)name_hash_search(&column_priv_hash, host, ip, db, user, tname, exact);
}
...
static int replace_column_table(GRANT_TABLE *g_t, TABLE *table, const LEX_USER &combo, List<LEX_COLUMN> &columns,
                                const char *db, const char *table_name, ulong rights, bool revoke_grant)
{
  ...
  while ((column = iter++))
  {
    ...
    if (table->file->index_read(table->record[0], user_key, table->key_info->key_length, HA_READ_KEY_EXACT))
    {
      if (revoke_grant)
      {
        my_error(ER_NONEXISTING_TABLE_GRANT, MYF(0), combo.user.str, combo.host.str,
  ...
  */
  ...
  if (revoke_grant)
  {
    ...
    table->file->extra(HA_EXTRA_RETRIEVE_ALL_COLS);
    if (table->file->index_read(table->record[0], user_key, key_prefix_length, HA_READ_KEY_EXACT)) goto end;
    ...
    do
    {
      ...
      if (privileges & rights)  // is in this record the priv to be revoked ??
      {
        ...
        {
          int tmp_error;
          if ((tmp_error = table->file->update_row(table->record[1], table->record[0])))
          {                                              /* purecov: deadcode */
        ...
        else
        {
          ...
          if (grant_column) hash_delete(&g_t->hash_columns, (byte *)grant_column);
        }
      }
...
static int replace_table_table(THD *thd, GRANT_TABLE *grant_table, TABLE *table, const LEX_USER &combo, const char *db,
                               const char *table_name, ulong rights, ulong col_rights, bool revoke_grant)
{
  ...
  strxmov(grantor, thd->security_ctx->user, "@", thd->security_ctx->host_or_ip, NullS);
  ...
  */
  ...
  if (table->file->index_read_idx(table->record[0], 0, user_key, table->key_info->key_length, HA_READ_KEY_EXACT))
  {
    ...
    */
    if (revoke_grant)
    {                                                                                           // no row, no revoke
      my_error(ER_NONEXISTING_TABLE_GRANT, MYF(0), combo.user.str, combo.host.str, table_name); /* purecov: deadcode */
      DBUG_RETURN(-1);                                                                          /* purecov: deadcode */
    }
    old_row_exists = 0;
  ...
  else
  {
    ...
    if (error && error != HA_ERR_FOUND_DUPP_KEY) goto table_error; /* purecov: deadcode */
  }
...
static int replace_routine_table(THD *thd, GRANT_NAME *grant_name, TABLE *table, const LEX_USER &combo, const char *db,
                                 const char *routine_name, bool is_proc, ulong rights, bool revoke_grant)
{
  char grantor[HOSTNAME_LENGTH + USERNAME_LENGTH + 2];
  int old_row_exists = 1;
  int error = 0;
  ...
  */
  ...
  if (table->file->index_read_idx(table->record[0], 0, (byte *)table->field[0]->ptr, 0, HA_READ_KEY_EXACT))
  {
    /*
      The following should never happen as we first check the in memory
      grant tables for the user.  There is however always a small change that
  ...
  if (old_row_exists)
  {
    ...
    {
      if ((error = table->file->update_row(table->record[1], table->record[0]))) goto table_error;
    }
    else if ((error = table->file->delete_row(table->record[1])))
      goto table_error;
  }
  ...
  {
    error = table->file->write_row(table->record[0]);
    if (error && error != HA_ERR_FOUND_DUPP_KEY) goto table_error;
  }
...
bool mysql_table_grant(THD *thd, TABLE_LIST *table_list, List<LEX_USER> &user_list, List<LEX_COLUMN> &columns,
                       ulong rights, bool revoke_grant)
{
  ...
  if (!initialized)
  {
    my_error(ER_OPTION_PREVENTS_STATEMENT, MYF(0), "--skip-grant-tables"); /* purecov: inspected */
  ...
  if (!revoke_grant)
  {
    if (columns.elements)
    {
      ...
      while ((column = column_iter++))
      {
        ...
        TABLE_LIST *dummy;
        Field *f = find_field_in_table_ref(thd, table_list, column->column.ptr(), column->column.ptr(), NULL, NULL,
                                           column->column.length(), 0, 1, 1, 0, &unused_field_idx, FALSE, &dummy);
        if (f == (Field *)0)
        {
    ...
    else
    {
      ...
      {
        char buf[FN_REFLEN];
        sprintf(buf, "%s/%s/%s.frm", mysql_data_home, table_list->db, table_list->table_name);
      ...
      if (table_list->grant.want_privilege)
      {
        ...
        my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, thd->security_ctx->priv_user,
  ...
  tables[1].next_local = tables[1].next_global =
      ((column_priv || (revoke_grant && ((rights & COL_ACLS) || columns.elements))) ? tables + 2 : 0);
  ...
  */
  if (thd->slave_thread && table_rules_on)
  {
    ...
    */
    ...
    if (!tables_ok(thd, tables)) DBUG_RETURN(FALSE);
  }
#endif
  ...
  while ((Str = str_list++))
  {
    ...
    if (Str->host.length > HOSTNAME_LENGTH || Str->user.length > USERNAME_LENGTH)
    {
      my_message(ER_GRANT_WRONG_HOST_OR_USER, ER(ER_GRANT_WRONG_HOST_OR_USER), MYF(0));
      result = TRUE;
      continue;
    }
    /* Create user if needed */
    pthread_mutex_lock(&acl_cache->lock);
    error = replace_user_table(thd, tables[0].table, *Str, 0, revoke_grant, create_new_users,
                               test(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER));
    ...
    db_name = (table_list->view_db.length ? table_list->view_db.str : table_list->db);
    table_name = (table_list->view_name.length ? table_list->view_name.str : table_list->table_name);
    ...
    if (replace_table_table(thd, grant_table, tables[1].table, *Str, db_name, table_name, rights, column_priv,
                            revoke_grant))
    {
    ...
    {
      if ((replace_column_table(grant_table, tables[2].table, *Str, columns, db_name, table_name, rights,
                                revoke_grant)))
      {
        result = TRUE;
      }
...
bool mysql_routine_grant(THD *thd, TABLE_LIST *table_list, bool is_proc, List<LEX_USER> &user_list, ulong rights,
                         bool revoke_grant, bool no_error)
{
  ...
  if (!initialized)
  {
    if (!no_error) my_error(ER_OPTION_PREVENTS_STATEMENT, MYF(0), "--skip-grant-tables");
    DBUG_RETURN(TRUE);
  }
  if (rights & ~PROC_ACLS)
  {
    if (!no_error) my_message(ER_ILLEGAL_GRANT_FOR_TABLE, ER(ER_ILLEGAL_GRANT_FOR_TABLE), MYF(0));
    DBUG_RETURN(TRUE);
  }

  if (!revoke_grant)
  {
    if (sp_exists_routine(thd, table_list, is_proc, no_error) < 0) DBUG_RETURN(TRUE);
  }
  ...
  */
  ...
  while ((Str = str_list++))
  {
    ...
    if (Str->host.length > HOSTNAME_LENGTH || Str->user.length > USERNAME_LENGTH)
    {
      if (!no_error) my_message(ER_GRANT_WRONG_HOST_OR_USER, ER(ER_GRANT_WRONG_HOST_OR_USER), MYF(0));
      result = TRUE;
      continue;
    }
    /* Create user if needed */
    ...
    if (!grant_name)
    {
      if (revoke_grant)
      {
        if (!no_error) my_error(ER_NONEXISTING_PROC_GRANT, MYF(0), Str->user.str, Str->host.str, table_name);
        result = TRUE;
        continue;
      }
      grant_name = new GRANT_NAME(Str->host.str, db_name, Str->user.str, table_name, rights);
      if (!grant_name)
      {
        result = TRUE;
        continue;
    ...
    if (replace_routine_table(thd, grant_name, tables[1].table, *Str, db_name, table_name, is_proc, rights,
                              revoke_grant))
    {
      result = TRUE;
      continue;
  ...
  thd->mem_root = old_root;
  rw_unlock(&LOCK_grant);
  if (!result && !no_error) send_ok(thd);
  /* Tables are automatically closed */
  DBUG_RETURN(result);
}
...
bool mysql_grant(THD *thd, const char *db, List<LEX_USER> &list, ulong rights, bool revoke_grant)
{
  ...
  if (!initialized)
  {
    my_error(ER_OPTION_PREVENTS_STATEMENT, MYF(0), "--skip-grant-tables"); /* purecov: tested */
  ...
  */
  ...
  while ((Str = str_list++))
  {
    ...
    if (replace_user_table(thd, tables[0].table, *Str, (!db ? rights : 0), revoke_grant, create_new_users,
                           test(thd->variables.sql_mode & MODE_NO_AUTO_CREATE_USER)))
      result = -1;
    else if (db)
    {
...
static my_bool grant_load(TABLE_LIST *tables)
{
  ...
  grant_option = FALSE;
  (void)hash_init(&column_priv_hash, system_charset_info, 0, 0, 0, (hash_get_key)get_grant_table,
                  (hash_free_key)free_grant_table, 0);
  (void)hash_init(&proc_priv_hash, system_charset_info, 0, 0, 0, (hash_get_key)get_grant_table, 0, 0);
  (void)hash_init(&func_priv_hash, system_charset_info, 0, 0, 0, (hash_get_key)get_grant_table, 0, 0);
      ...
      if (check_no_resolve)
      {
        if (hostname_requires_resolving(mem_check->host.hostname))
        {
          sql_print_warning(
              ...
              "ignored in --skip-name-resolve mode.",
              mem_check->tname, mem_check->user, mem_check->host, mem_check->host);
          continue;
        }
      }
      ...
      if (check_no_resolve)
      {
        if (hostname_requires_resolving(mem_check->host.hostname))
        {
          sql_print_warning(
              ...
              "ignored in --skip-name-resolve mode.",
              mem_check->tname, mem_check->user, mem_check->host);
          continue;
        }
      }
...
bool check_grant(THD *thd, ulong want_access, TABLE_LIST *tables, uint show_table, uint number, bool no_errors)
{
  ...
  if (!want_access) DBUG_RETURN(0);  // ok

  rw_rdlock(&LOCK_grant);
  for (table = tables; table && number--; table = table->next_global)
  {
    GRANT_TABLE *grant_table;
    if (!(~table->grant.privilege & want_access) || table->derived || table->schema_table || table->belong_to_view)
    {
      /*
    ...
    if (!(grant_table = table_hash_search(sctx->host, sctx->ip, table->db, sctx->priv_user, table->table_name, 0)))
    {
      want_access &= ~table->grant.privilege;
      goto err;  // No grants
    }
    if (show_table) continue;  // We have some priv on this
...
err:
  ...
  if (!no_errors)  // Not a silent skip of table
  {
    ...
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user, sctx->host_or_ip,
             table ? table->table_name : "unknown");
  }
  DBUG_RETURN(1);
}

bool check_grant_column(THD *thd, GRANT_INFO *grant, const char *db_name, const char *table_name, const char *name,
                        uint length, uint show_tables)
{
  ...
  if (!want_access) DBUG_RETURN(0);  // Already checked
  ...
  {
    grant->grant_table =
        table_hash_search(sctx->host, sctx->ip, db_name, sctx->priv_user, table_name, 0); /* purecov: inspected */
...
bool check_grant_all_columns(THD *thd, ulong want_access, GRANT_INFO *grant, const char *db_name,
                             const char *table_name, Field_iterator *fields)
{
  Security_context *sctx = thd->security_ctx;
  GRANT_TABLE *grant_table;
  GRANT_COLUMN *grant_column;
  ...
  if (!want_access) return 0;  // Already checked
  if (!grant_option) goto err2;
  ...
  if (!(grant_table = grant->grant_table)) goto err; /* purecov: inspected */
...
err2:
  char command[128];
  get_privilege_desc(command, sizeof(command), want_access);
  my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), command, sctx->priv_user, sctx->host_or_ip, fields->name(),
           table_name);
  return 1;
}
...
bool check_grant_db(THD *thd, const char *db)
{
  ...
  for (uint idx = 0; idx < column_priv_hash.records; idx++)
  {
    ...
    if (len < grant_table->key_length && !memcmp(grant_table->hash_key, helping, len) &&
...
bool check_grant_routine(THD *thd, ulong want_access, TABLE_LIST *procs, bool is_proc, bool no_errors)
{
  ...
  for (table = procs; table; table = table->next_global)
  {
    ...
    if ((grant_proc = routine_hash_search(host, sctx->ip, table->db, user, table->table_name, is_proc, 0)))
...
bool check_routine_level_acl(THD *thd, const char *db, const char *name, bool is_proc)
{
  ...
  if (grant_option)
  {
    ...
    Security_context *sctx = thd->security_ctx;
    rw_rdlock(&LOCK_grant);
    if ((grant_proc = routine_hash_search(sctx->priv_host, sctx->ip, db, sctx->priv_user, name, is_proc, 0)))
...
ulong get_table_grant(THD *thd, TABLE_LIST *table)
{
  ...
  grant_table = NULL;
#else
  grant_table = table_hash_search(sctx->host, sctx->ip, db, sctx->priv_user, table->table_name, 0);
#endif
  table->grant.grant_table = grant_table;  // Remember for column test
  table->grant.version = grant_version;
...
static uint command_lengths[] = {6, 6,  6, 6,  6,  4, 6,  8,  7,  4, 5,  10, 5,
                                 5, 14, 5, 23, 11, 7, 17, 18, 11, 9, 14, 13, 11};

static int show_routine_grants(THD *thd, LEX_USER *lex_user, HASH *hash, const char *type, int typelen, char *buff,
                               int buffsize);
...
bool mysql_show_grants(THD *thd, LEX_USER *lex_user)
{
  ...
  if (lex_user->host.length > HOSTNAME_LENGTH || lex_user->user.length > USERNAME_LENGTH)
  {
    my_message(ER_GRANT_WRONG_HOST_OR_USER, ER(ER_GRANT_WRONG_HOST_OR_USER), MYF(0));
    DBUG_RETURN(TRUE);
  }
  ...
  for (counter = 0; counter < acl_users.elements; counter++)
  {
    ...
    if (!strcmp(lex_user->user.str, user) && !my_strcasecmp(system_charset_info, lex_user->host.str, host)) break;
  }
  ...
  /* Add first global access grants */
  {
    ...
    if ((want_access & GRANT_ACL) || (acl_user->user_resource.questions || acl_user->user_resource.updates ||
                                      acl_user->user_resource.conn_per_hour || acl_user->user_resource.user_conn))
    {
      global.append(" WITH", 5);
      if (want_access & GRANT_ACL) global.append(" GRANT OPTION", 13);
  ...
  for (counter = 0; counter < acl_dbs.elements; counter++)
  {
    ...
    if (!strcmp(lex_user->user.str, user) && !my_strcasecmp(system_charset_info, lex_user->host.str, host))
    {
      ...
      if (want_access)
      {
        ...
        db.append("'@'", 3);
        db.append(lex_user->host.str, lex_user->host.length, system_charset_info);
        db.append('\'');
        if (want_access & GRANT_ACL) db.append(" WITH GRANT OPTION", 18);
        protocol->prepare_for_resend();
  ...
  for (index = 0; index < column_priv_hash.records; index++)
  {
    ...
    if (!strcmp(lex_user->user.str, user) &&
        !my_strcasecmp(system_charset_info, lex_user->host.str, grant_table->host.hostname))
    {
      ...
      if ((table_access | grant_table->cols) != 0)
      {
        ...
        else
        {
          ...
          for (counter = 0, j = SELECT_ACL; j <= TABLE_ACLS; counter++, j <<= 1)
          {
            if (test_access & j)
            {
              ...
              {
                uint found_col = 0;
                for (uint col_index = 0; col_index < grant_table->hash_columns.records; col_index++)
                {
        ...
        global.append(lex_user->host.str, lex_user->host.length, system_charset_info);
        global.append('\'');
        if (table_access & GRANT_ACL) global.append(" WITH GRANT OPTION", 18);
        protocol->prepare_for_resend();
        protocol->store(global.ptr(), global.length(), global.charset());
        if (protocol->write())
        {
  ...
  if (show_routine_grants(thd, lex_user, &proc_priv_hash, "PROCEDURE", 9, buff, sizeof(buff)))
  {
    error = -1;
    goto end;
  }

  if (show_routine_grants(thd, lex_user, &func_priv_hash, "FUNCTION", 8, buff, sizeof(buff)))
  {
    error = -1;
    goto end;
  }
                               ...
                               int buffsize)
{
  ...
  for (index = 0; index < hash->records; index++)
  {
    ...
    if (!strcmp(lex_user->user.str, user) &&
        !my_strcasecmp(system_charset_info, lex_user->host.str, grant_proc->host.hostname))
    {
      ...
      if (proc_access != 0)
      {
        ...
        global.append(lex_user->host.str, lex_user->host.length, system_charset_info);
        global.append('\'');
        if (proc_access & GRANT_ACL) global.append(" WITH GRANT OPTION", 18);
        protocol->prepare_for_resend();
        protocol->store(global.ptr(), global.length(), global.charset());
        if (protocol->write())
        {
...
int open_grant_tables(THD *thd, TABLE_LIST *tables)
{
  ...
  tables->lock_type = (tables + 1)->lock_type = (tables + 2)->lock_type = (tables + 3)->lock_type =
  ...
  */
  if (thd->slave_thread && table_rules_on)
  {
    ...
    */
    ...
    if (!tables_ok(thd, tables)) DBUG_RETURN(1);
...
static int modify_grant_table(TABLE *table, Field *host_field, Field *user_field, LEX_USER *user_to)
{
  ...
  if (user_to)
  {
    ...
    if ((error = table->file->update_row(table->record[1], table->record[0]))) table->file->print_error(error, MYF(0));
  }
  else
  {
    /* delete */
    if ((error = table->file->delete_row(table->record[0]))) table->file->print_error(error, MYF(0));
  }
...
static int handle_grant_table(TABLE_LIST *tables, uint table_no, bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  ...
  if (!table_no)  // mysql.user table
  {
    ...
    */
    DBUG_PRINT("info", ("read table: '%s'  search: '%s'@'%s'", table->s->table_name, user_str, host_str));
    ...
    if ((error = table->file->index_read_idx(table->record[0], 0, user_key, key_prefix_length, HA_READ_KEY_EXACT)))
    {
  ...
  else
  {
    ...
    */
    ...
    {
#ifdef EXTRA_DEBUG
      DBUG_PRINT("info", ("scan table: '%s'  search: '%s'@'%s'", table->s->table_name, user_str, host_str));
#endif
      while ((error = table->file->rnd_next(table->record[0])) != HA_ERR_END_OF_FILE)
      {
        if (error)
        {
        ...
        if (!(host = get_field(&mem, host_field))) host = "";
        if (!(user = get_field(&mem, user_field))) user = "";

#ifdef EXTRA_DEBUG
        DBUG_PRINT("loop",
                   ("scan fields: '%s'@'%s' '%s' '%s' '%s'", user, host, get_field(&mem, table->field[1]) /*db*/,
                    get_field(&mem, table->field[3]) /*table*/, get_field(&mem, table->field[4]) /*column*/));
#endif
        if (strcmp(user_str, user) || my_strcasecmp(system_charset_info, host_str, host)) continue;

        /* If requested, delete or update the record. */
        result = ((drop || user_to) && modify_grant_table(table, host_field, user_field, user_to)) ? -1
                 : result                                                                          ? result
                          : 1; /* Error or keep result or found. */
...
#ifdef EXTRA_DEBUG
  DBUG_PRINT("loop", ("scan struct: %u  search    user: '%s'  host: '%s'", struct_no, user_from->user.str,
                      user_from->host.str));
#endif
    ...
    */
    switch (struct_no)
    {
      ...
      case 2:
        ...
        if (!(host = grant_name->host.hostname)) host = "%";
        break;
    ...
    if (!host) host = "";
#ifdef EXTRA_DEBUG
    DBUG_PRINT("loop", ("scan struct: %u  index: %u  user: '%s'  host: '%s'", struct_no, idx, user, host));
#endif
    if (strcmp(user_from->user.str, user) || my_strcasecmp(system_charset_info, user_from->host.str, host)) continue;
    ...
    else if (user_to)
    {
      switch (struct_no)
      {
        ...
        case 3:
          ...
          update_hostname(&grant_name->host, strdup_root(&mem, user_to->host.str));
          break;
      }
    }
    else
...
static int handle_grant_data(TABLE_LIST *tables, bool drop, LEX_USER *user_from, LEX_USER *user_to)
{
  ...
  {
    /* Handle db array. */
    if (((handle_grant_struct(1, drop, user_from, user_to) && !result) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (!drop && !user_to) goto end;
  ...
  {
    /* Handle procs array. */
    if (((handle_grant_struct(3, drop, user_from, user_to) && !result) || found) && !result)
    {
      result = 1; /* At least one record/element found. */
      /* If search is requested, we do not need to search further. */
      if (!drop && !user_to) goto end;
  ...
  else
  {
    ...
    {
      /* Handle columns hash. */
      if (((handle_grant_struct(2, drop, user_from, user_to) && !result) || found) && !result)
        result = 1; /* At least one record/element found. */
    }
  }
end:
  ...
  while ((lex_user = user_list++))
  {
    ...
    do
    {
      for (counter = 0, revoked = 0; counter < column_priv_hash.records;)
      {
        ...
        if (!strcmp(lex_user->user.str, user) && !my_strcasecmp(system_charset_info, lex_user->host.str, host))
        {
          if (replace_table_table(thd, grant_table, tables[2].table, *lex_user, grant_table->db, grant_table->tname,
                                  ~(ulong)0, 0, 1))
          {
            result = -1;
          }
          else
          {
            ...
            if (!replace_column_table(grant_table, tables[3].table, *lex_user, columns, grant_table->db,
    ...
    for (is_proc = 0; is_proc < 2; is_proc++) do
      {
        ...
        for (counter = 0, revoked = 0; counter < hash->records;)
        {
          ...
          if (!strcmp(lex_user->user.str, user) && !my_strcasecmp(system_charset_info, lex_user->host.str, host))
          {
            if (!replace_routine_table(thd, grant_proc, tables[4].table, *lex_user, grant_proc->db, grant_proc->tname,
                                       is_proc, ~(ulong)0, 1))
            {
              revoked = 1;
              continue;
...
bool sp_grant_privileges(THD *thd, const char *sp_db, const char *sp_name, bool is_proc)
{
  ...
  if (!find_acl_user(combo->host.str = (char *)sctx->host_or_ip, combo->user.str, FALSE) &&
      !find_acl_user(combo->host.str = (char *)sctx->host, combo->user.str, FALSE) &&
      !find_acl_user(combo->host.str = (char *)sctx->ip, combo->user.str, FALSE) &&
  ...
  result = mysql_routine_grant(thd, tables, is_proc, user_list, DEFAULT_CREATE_PROC_ACLS, 0, 1);
  DBUG_RETURN(result);
}
...
void update_schema_privilege(TABLE *table, char *buff, const char *db, const char *t_name, const char *column,
                             uint col_length, const char *priv, uint priv_length, const char *is_grantable)
{
  int i = 2;
...
int fill_schema_user_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (counter = 0; counter < acl_users.elements; counter++)
  {
    ...
    if (no_global_access &&
        (strcmp(thd->security_ctx->priv_user, user) || my_strcasecmp(system_charset_info, curr_host, host)))
      continue;
    ...
    else
    {
      ...
      {
        if (test_access & j)
          update_schema_privilege(table, buff, 0, 0, 0, 0, command_array[priv_id], command_lengths[priv_id],
                                  is_grantable);
      }
    }
  }
...
int fill_schema_schema_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (counter = 0; counter < acl_dbs.elements; counter++)
  {
    ...
    if (want_access)
    {
      ...
      strxmov(buff, "'", user, "'@'", host, "'", NullS);
      if (!(want_access & ~GRANT_ACL))
        update_schema_privilege(table, buff, acl_db->db, 0, 0, 0, "USAGE", 5, is_grantable);
      else
      {
        int cnt;
        ulong j, test_access = want_access & ~GRANT_ACL;
        for (cnt = 0, j = SELECT_ACL; j <= DB_ACLS; cnt++, j <<= 1)
          if (test_access & j)
            update_schema_privilege(table, buff, acl_db->db, 0, 0, 0, command_array[cnt], command_lengths[cnt],
                                    is_grantable);
      }
    }
  }
...
int fill_schema_table_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (index = 0; index < column_priv_hash.records; index++)
  {
    ...
    if (no_global_access && (strcmp(thd->security_ctx->priv_user, user) ||
                             my_strcasecmp(system_charset_info, curr_host, grant_table->host.hostname)))
      continue;
    ...
    if (table_access)
    {
      ...
      */
      ...
      if (!test_access)
        update_schema_privilege(table, buff, grant_table->db, grant_table->tname, 0, 0, "USAGE", 5, is_grantable);
      else
      {
        ulong j;
        int cnt;
        ...
        {
          if (test_access & j)
            update_schema_privilege(table, buff, grant_table->db, grant_table->tname, 0, 0, command_array[cnt],
          ...
          if (test_access & j)
          {
            for (uint col_index = 0; col_index < grant_table->hash_columns.records; col_index++)
            {
              ...
              if ((grant_column->rights & j) && (table_access & j))
                update_schema_privilege(table, buff, grant_table->db, grant_table->tname, grant_column->column,
                                        grant_column->key_length, command_array[cnt], command_lengths[cnt],
                                        is_grantable);
            }
          }
        }
...
void fill_effective_table_privileges(THD *thd, GRANT_INFO *grant, const char *db, const char *table)
{
  ...
  if (!sctx->priv_user) return;  // it is slave

  /* db privileges */
  grant->privilege |= acl_get(sctx->host, sctx->ip, sctx->priv_user, db, 0);

  if (!grant_option) return;
  ...
  if (grant->version != grant_version)
  {
    rw_rdlock(&LOCK_grant);
    grant->grant_table =
        table_hash_search(sctx->host, sctx->ip, db, sctx->priv_user, table, 0); /* purecov: inspected */
    grant->version = grant_version;                                             /* purecov: inspected */
    rw_unlock(&LOCK_grant);
  }
...
bool check_routine_level_acl(THD *thd, const char *db, const char *name, bool is_proc) { return FALSE; }
