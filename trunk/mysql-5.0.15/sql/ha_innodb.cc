...
long innobase_mirrored_log_groups, innobase_log_files_in_group,
    ...
    innobase_buffer_pool_awe_mem_mb, innobase_buffer_pool_size,
    innobase_additional_mem_pool_size, innobase_file_io_threads,
    innobase_lock_wait_timeout, innobase_force_recovery, innobase_open_files;
                           ...
                           use the default max length */
{
  ...
  fprintf(f, "MySQL thread id %lu, query id %lu", thd->thread_id,
          (ulong)thd->query_id);
  if (sctx->host) {
    putc(' ', f);
    fputs(sctx->host, f);
    ...
    /* out: temporary file descriptor, or < 0 on error */
{
  ...
  if (fd >= 0) {
    ...
    if (fd2 < 0) {
      ...
      my_errno = errno;
      my_error(EE_OUT_OF_FILERESOURCES, MYF(ME_BELL + ME_WAITTANG), filename,
               my_errno);
    }
...
ha_innobase::ha_innobase(TABLE *table_arg)
    : handler(&innobase_hton, table_arg),
      int_table_flags(HA_REC_NOT_IN_SEQ | HA_NULL_IN_KEY | HA_CAN_INDEX_BLOBS |
                      ...
                      HA_PRIMARY_KEY_IN_READ_INDEX | HA_TABLE_SCAN_ON_INDEX),
      last_dup_key((uint) - 1), start_of_scan(0), num_write_row(0) {}
...
/* Argument TRUE below means we are using transactions */
  ...
  query_cache.invalidate((THD *)(trx->mysql_thd), (const char *)full_name,
                         (uint32)full_name_len, TRUE);
#endif
}
    ...
    ulint namelen)    /* in: length of name */
{
  ...
  return (
      get_quote_char_for_identifier((THD *)trx->mysql_thd, name, (int)namelen));
}
    ...
    /* out: &innobase_hton, or NULL on error */
{
  ...
  srv_data_home =
      (innobase_data_home_dir ? innobase_data_home_dir : default_path);
  ...
  internal_innobase_data_file_path =
      my_strdup(innobase_data_file_path, MYF(MY_FAE));

  ret = (bool)srv_parse_data_file_paths_and_sizes(
      internal_innobase_data_file_path, &srv_data_file_names,
      &srv_data_file_sizes, &srv_data_file_is_raw_partition, &srv_n_data_files,
      &srv_auto_extend_last_data_file, &srv_last_file_size_max);
  if (ret == FALSE) {
  ...
  ut_a(0 ==
       strcmp((char *)my_charset_latin1.name, (char *)"latin1_swedish_ci"));
    ...
    trx_t *trx) /* in: transaction handle */
{
  ...
  if (thd && thd->slave_thread) {
    ...
    trx->mysql_master_log_pos =
        ((ib_longlong)active_mi->rli.future_group_master_log_pos);
  }
    ...
    THD *thd) /* in: user thread */
{
...
#ifdef HAVE_REPLICATION
  if (thd->variables.sync_replication && trx->repl_wait_binlog_name &&
  ...
  try_again:
    ...
    if (cmp > 0 || (cmp == 0 &&
                    innobase_repl_pos >= (my_off_t)trx->repl_wait_binlog_pos)) {
    ...
    if (innobase_repl_wait_file_name_inited != 0) {
      ...
      if (cmp < 0 || (cmp == 0 && (my_off_t)trx->repl_wait_binlog_pos <=
    ...
    if (ret != 0) {
      ...
      sql_print_error("MySQL synchronous replication was "
                      ...
                      innobase_repl_file_name, (ulong)innobase_repl_pos,
    ...
    uint test_if_locked) /* in: not used */
{
  ...
  if (!(mysql_byte *)my_multi_malloc(MYF(MY_WME), &upd_buff,
                                     upd_and_key_val_buff_len, &key_val_buff,
                      ...
                      ROW_MYSQL_REC_FIELDS */
{
  ...
  if (!prebuilt->mysql_template) {
    prebuilt->mysql_template = (mysql_row_templ_t *)mem_alloc_noninline(
  ...
  include_field:
    ...
    if (field->null_ptr) {
      templ->mysql_null_byte_offset =
          (ulint)((char *)field->null_ptr - (char *)table->record[0]);
    ...
    templ->charset =
        dtype_get_charset_coll_noninline(index->table->cols[i].type.prtype);
    ...
    THD *thd)                 /* in: user thread */
{
  ...
  for (i = 0; i < n_fields; i++) {
    ...
    if (o_len != n_len ||
        (o_len != UNIV_SQL_NULL && 0 != memcmp(o_ptr, n_ptr, o_len))) {
    ...
    mysql_byte *new_row)       /* in: new row in MySQL format */
{
  ...
  calc_row_difference(uvect, (mysql_byte *)old_row, new_row, table, upd_buff,
                      (ulint)upd_and_key_val_buff_len, prebuilt, user_thd);
    ...
    enum ha_rkey_function find_flag) /* in: search flags from my_base.h */
{
  ...
  if (key_ptr) {
    ...
    row_sel_convert_mysql_key_to_innobase(
        prebuilt->search_tuple, (byte *)key_val_buff,
        (ulint)upd_and_key_val_buff_len, index, (byte *)key_ptr, (ulint)key_len,
        prebuilt->trx);
  } else {
                ...
                InnoDB */
{
  ...
  if (keynr != MAX_KEY && table->s->keys > 0) {
    ...
    prebuilt->index =
        dict_table_get_index_noninline(prebuilt->table, key->name);
  } else {
    ...
    uint key_num)           /* in: index number */
{
  ...
  index = dict_mem_index_create((char *)table_name, key->name, 0, ind_type,
                                n_fields);
  ...
  for (i = 0; i < n_fields; i++) {
    ...
    for (j = 0; j < form->s->fields; j++) {
      ...
      if (0 ==
          innobase_strcasecmp(field->field_name, key_part->field->field_name)) {
    ...
    if (DATA_BLOB == col_type || (key_part->length < field->pack_length() &&
      ...
      if (col_type == DATA_INT || col_type == DATA_FLOAT ||
          col_type == DATA_DOUBLE || col_type == DATA_DECIMAL) {
    ...
    dict_mem_index_add_field(index, (char *)key_part->field->field_name, 0,
                             prefix_len);
  }
    ...
    const char *table_name) /* in: table name */
{
  ...
  index = dict_mem_index_create((char *)table_name, (char *)"GEN_CLUST_INDEX",
                                0, DICT_CLUSTERED, 0);
                                 ...
                                 create statement string */
{
  ...
  primary_key_no =
      (table->s->primary_key != MAX_KEY ? (int)table->s->primary_key : -1);
  ...
  if (current_thd->query != NULL) {
    ...
    if (thd->convert_string(&q, system_charset_info, current_thd->query,
    ...
    error = row_table_add_foreign_constraints(
        trx, q.str, norm_name, create_info->options & HA_LEX_CREATE_TMP_TABLE);
                           ...
                           also be 0 */
{
  ...
  mysql_byte *key_val_buff2 = (mysql_byte *)my_malloc(
      table->s->reclength + table->s->max_key_length + 100, MYF(MY_FAE));
  ...
  row_sel_convert_mysql_key_to_innobase(
      range_start, (byte *)key_val_buff, (ulint)upd_and_key_val_buff_len, index,
      (byte *)(min_key ? min_key->key : (const mysql_byte *)0),
      (ulint)(min_key ? min_key->length : 0), prebuilt->trx);

  row_sel_convert_mysql_key_to_innobase(
      range_end, (byte *)key_val_buff2, buff2_len, index,
  ...
  n_rows =
      btr_estimate_n_rows_in_range(index, range_start, mode1, range_end, mode2);
    ...
    uint flag) /* in: what information MySQL requests */
{
  ...
  if (flag & HA_STATUS_VARIABLE) {
    ...
    deleted = 0;
    data_file_length =
        ((ulonglong)ib_table->stat_clustered_index_size) * UNIV_PAGE_SIZE;
    index_file_length =
        ((ulonglong)ib_table->stat_sum_of_other_index_sizes) * UNIV_PAGE_SIZE;
    delete_length = 0;
    check_time = 0;
      ...
      for (j = 0; j < table->key_info[i].key_parts; j++) {
        ...
        if (j + 1 > index->n_uniq) {
          ...
          sql_print_error("Index %s of %s has "
                          ...
                          "section 15.1 at "
                          "http://www.innodb.com/ibman.html",
                          index->name, ib_table->name,
                          (unsigned long)index->n_uniq, j + 1);
          break;
        }
        ...
        table->key_info[i].rec_per_key[j] =
            rec_per_key >= ~(ulong)0 ? ~(ulong)0 : rec_per_key;
      }
    ...
    const char *comment) /* in: table comment defined by user */
{
  ...
  if (FILE *file = os_file_create_tmpfile()) {
    ...
    fprintf(
        file, "InnoDB free: %lu kB",
        (ulong)fsp_get_available_space_in_free_extents(prebuilt->table->space));

    dict_print_info_on_foreign_keys(FALSE, file, prebuilt->trx,
                                    prebuilt->table);
    flen = ftell(file);
    if (flen < 0) {
      flen = 0;
    ...
    MUST be freed with ::free_foreign_key_create_info */
{
  ...
  if (FILE *file = os_file_create_tmpfile()) {
    ...
    str = my_strdup("/* Error: cannot display foreign key constraints */",
                    MYF(0));
  }
...
int ha_innobase::get_foreign_key_list(THD *thd,
  ...
  while (foreign != NULL) {
    ...
    while (tmp_buff[i] != '/')
      i++;
    tmp_buff += i + 1;
    f_key_info.forein_id =
        make_lex_string(thd, 0, tmp_buff, (uint)strlen(tmp_buff), 1);
    ...
    f_key_info.constraint_method =
        make_lex_string(thd, f_key_info.constraint_method, tmp_buff, length, 1);

    FOREIGN_KEY_INFO *pf_key_info = ((FOREIGN_KEY_INFO *)thd->memdup(
        (gptr) & f_key_info, sizeof(FOREIGN_KEY_INFO)));
...
int ha_innobase::start_stmt(
  ...
  if (!prebuilt->mysql_has_locked) {
    ...
    if (trx->isolation_level != TRX_ISO_SERIALIZABLE &&
        thd->lex->sql_command == SQLCOM_SELECT && lock_type == TL_READ) {
    ...
    int lock_type) /* in: lock type */
{
  ...
  if (lock_type != F_UNLCK) {
    ...
    if (prebuilt->select_lock_type != LOCK_NONE) {
      ...
      if (thd->in_lock_tables && thd->lex->sql_command != SQLCOM_CALL &&
...
static INNOBASE_SHARE *get_share(const char *table_name) {
  ...
  if (!(share = (INNOBASE_SHARE *)hash_search(
            &innobase_open_tables, (mysql_byte *)table_name, length))) {
                                  ...
                                  TL_IGNORE */
{
  ...
  if (lock_type != TL_IGNORE && lock.type == TL_UNLOCK) {
    ...
    if ((lock_type >= TL_WRITE_CONCURRENT_INSERT && lock_type <= TL_WRITE) &&
        (!thd->in_lock_tables || thd->lex->sql_command == SQLCOM_CALL) &&
        !thd->tablespace_op && thd->lex->sql_command != SQLCOM_TRUNCATE &&
    ...
    longlong *ret) /* out: auto-inc value */
{
  ...
  if (error) {
    ...
    auto_inc = (longlong)table->found_next_number_field->val_int_offset(
                            ...
                            MySQL key value format */
{
  ...
  for (; key_part != key_part_end; ++key_part) {
    ...
    if (mysql_type == FIELD_TYPE_TINY_BLOB ||
        mysql_type == FIELD_TYPE_MEDIUM_BLOB || mysql_type == FIELD_TYPE_BLOB ||
        mysql_type == FIELD_TYPE_LONG_BLOB) {
      ...
      ref1 += 2;
      ref2 += 2;
      result = ((Field_blob *)field)
                   ->cmp((const char *)ref1, len1, (const char *)ref2, len2);
    } else {
