...
static uchar *pack_screens(List<create_field> &create_fields, uint *info_length,
                           uint *screens, bool small_file);
static uint pack_keys(uchar *keybuff, uint key_count, KEY *key_info,
                      ulong data_offset);
static bool pack_header(uchar *forminfo, enum db_type table_type,
                        List<create_field> &create_fields, uint info_length,
                        uint screens, uint table_options, ulong data_offset,
                        handler *file);
...
static bool make_empty_rec(THD *thd, int file, enum db_type table_type,
                           uint table_options,
                           List<create_field> &create_fields, uint reclength,
                           ulong data_offset);
...
bool mysql_create_frm(THD *thd, my_string file_name, const char *db,
  ...
  if (pack_header(forminfo, create_info->db_type, create_fields, info_length,
    ...
    if (!(screen_buff = pack_screens(create_fields, &info_length, &screens, 1)))
      DBUG_RETURN(1);
    if (pack_header(forminfo, create_info->db_type, create_fields, info_length,
                    screens, create_info->table_options, data_offset,
                    db_file)) {
      my_free((gptr)screen_buff, MYF(0));
      DBUG_RETURN(1);
    }
  ...
  create_info->extra_size =
      (2 + str_db_type.length + 2 + create_info->connect_string.length);
  ...
  strmake((char *)forminfo + 47,
          create_info->comment ? create_info->comment : "", 60);
  ...
  if (my_pwrite(file, (byte *)fileinfo, 64, 0L, MYF_RW) ||
    ...
    goto err;
  VOID(my_seek(file, (ulong)uint2korr(fileinfo + 6) + (ulong)key_buff_length,
               MY_SEEK_SET, MYF(0)));
  if (make_empty_rec(thd, file, create_info->db_type,
                     create_info->table_options, create_fields, reclength,
                     data_offset))
    goto err;
  ...
  if (my_write(file, (const byte *)buff, sizeof(buff), MYF(MY_NABP)) ||
      my_write(file, (const byte *)str_db_type.str, str_db_type.length,
               MYF(MY_NABP)))
    goto err;
...
int rea_create_table(THD *thd, my_string file_name, const char *db,
  ...
  if (mysql_create_frm(thd, file_name, db, table, create_info, create_fields,
                       keys, key_info, NULL))
    DBUG_RETURN(1);
...
static uint pack_keys(uchar *keybuff, uint key_count, KEY *keyinfo,
  ...
  for (key = keyinfo, end = keyinfo + key_count; key != end; key++) {
    ...
    DBUG_PRINT("loop", ("flags: %d  key_parts: %d at 0x%lx", key->flags,
                        key->key_parts, key->key_part));
    for (key_part = key->key_part, key_part_end = key_part + key->key_parts;
         key_part != key_part_end; key_part++) {
      uint offset;
      DBUG_PRINT("loop",
                 ("field: %d  startpos: %lu  length: %ld", key_part->fieldnr,
                  key_part->offset + data_offset, key_part->length));
...
static bool pack_header(uchar *forminfo, enum db_type table_type,
  ...
  while ((field = it++)) {
    ...
    if (MTYP_TYPENR(field->unireg_check) == Field::NOEMPTY ||
      ...
      field->unireg_check =
          (Field::utype)((uint)field->unireg_check | MTYP_NOEMPTY_BIT);
      no_empty++;
    }
    /*
    ...
    */
    if (field->sql_type == FIELD_TYPE_TIMESTAMP &&
        MTYP_TYPENR(field->unireg_check) != Field::NONE && !time_stamp_pos)
  ...
  if (info_length + (ulong)create_fields.elements * FCOMP + 288 + n_length +
  ...
  length = (info_length + create_fields.elements * FCOMP + 288 + n_length +
            int_length + com_length);
...
static bool make_empty_rec(THD *thd, File file, enum db_type table_type,
  ...
  if (!handler || !(buff = (uchar *)my_malloc((uint)reclength,
  ...
  while ((field = it++)) {
    ...
    */
    Field *regfield = make_field(
        (char *)buff + field->offset + data_offset, field->length,
        null_pos + null_count / 8, null_count & 7, field->pack_flag,
        field->sql_type, field->charset, field->geom_type, field->unireg_check,
        field->interval, field->field_name, &table);
    if (!regfield)
    ...
    if (field->def && (regfield->real_type() != FIELD_TYPE_YEAR ||
                       field->def->val_int() != 0)) {
