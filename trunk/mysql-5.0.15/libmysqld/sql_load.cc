...
class READ_INFO
{
 ...
 public:
  ...
  READ_INFO(File file, uint tot_length, CHARSET_INFO *cs, String &field_term, String &line_start, String &line_term,
...
static int read_fixed_length(THD *thd, COPY_INFO &info, TABLE_LIST *table_list, List<Item> &fields_vars,
                             List<Item> &set_fields, List<Item> &set_values, READ_INFO &read_info, ulong skip_lines,
                             bool ignore_check_option_errors);
static int read_sep_field(THD *thd, COPY_INFO &info, TABLE_LIST *table_list, List<Item> &fields_vars,
                          List<Item> &set_fields, List<Item> &set_values, READ_INFO &read_info, String &enclosed,
...
bool mysql_load(THD *thd, sql_exchange *ex, TABLE_LIST *table_list, List<Item> &fields_vars, List<Item> &set_fields,
                List<Item> &set_values, enum enum_duplicates handle_duplicates, bool ignore, bool read_file_from_client)
{
  char name[FN_REFLEN];
  File file;
  TABLE *table;
  ...
  */
  ...
  {
    Field **field;
    for (field = table->field; *field; field++) fields_vars.push_back(new Item_field(*field));
  ...
  {  // Part field list
    ...
    if (setup_fields(thd, 0, fields_vars, 1, 0, 0) || setup_fields(thd, 0, set_fields, 1, 0, 0) ||
  ...
  {
    my_message(ER_BLOBS_AND_NO_TERMINATED, ER(ER_BLOBS_AND_NO_TERMINATED), MYF(0));
    DBUG_RETURN(TRUE);
  }
...
#endif
            ((stat_info.st_mode & S_IFREG) == S_IFREG || (stat_info.st_mode & S_IFIFO) == S_IFIFO)))
      {
  ...
  READ_INFO read_info(file, tot_length, thd->variables.collation_database, *field_term, *ex->line_start, *ex->line_term,
  ...
  if (!(error = test(read_info.error)))
  {
    ...
    thd->abort_on_warning =
        (!ignore && (thd->variables.sql_mode & (MODE_STRICT_TRANS_TABLES | MODE_STRICT_ALL_TABLES)));

    if (!field_term->length() && !enclosed->length())
      error =
          read_fixed_length(thd, info, table_list, fields_vars, set_fields, set_values, read_info, skip_lines, ignore);
    else
  ...
  */
  ...
  sprintf(name, ER(ER_LOAD_INFO), (ulong)info.records, (ulong)info.deleted, (ulong)(info.records - info.copied),
...
static bool write_execute_load_query_log_event(THD *thd, bool duplicates, bool ignore, bool transactional_table)
{
  Execute_load_query_log_event e(
      thd, thd->query, thd->query_length, (char *)thd->lex->fname_start - (char *)thd->query,
      (char *)thd->lex->fname_end - (char *)thd->query,
      (duplicates == DUP_REPLACE) ? LOAD_DUP_REPLACE : (ignore ? LOAD_DUP_IGNORE : LOAD_DUP_ERROR), transactional_table,
      FALSE);
                             ...
                             bool ignore_check_option_errors)
{
  ...
  while (!read_info.read_fixed_length())
  {
    ...
    */
    while ((sql_field = (Item_field *)it++))
    {
      ...
      */
      ...
      if (pos == read_info.row_end)
      {
        ...
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_TOO_FEW_RECORDS, ER(ER_WARN_TOO_FEW_RECORDS),
                            thd->row_count);
      }
      else
      {
    ...
    if (thd->killed || fill_record_n_invoke_before_triggers(thd, set_fields, set_values, ignore_check_option_errors,
                          ...
                          ulong skip_lines, bool ignore_check_option_errors)
{
  ...
  for (;; it.rewind())
  {
    ...
    while ((item = it++))
    {
      ...
      if (!read_info.enclosed && (enclosed_length && length == 4 && !memcmp(pos, "NULL", 4)) ||
          (length == 1 && read_info.found_null))
      {
        if (item->type() == Item::FIELD_ITEM)
        {
          ...
          ((Item_user_var_as_out_param *)item)->set_null_value(read_info.read_charset);
        continue;
      }
...
READ_INFO::READ_INFO(File file_par, uint tot_length, CHARSET_INFO *cs, String &field_term, String &line_start,
                     String &line_term, String &enclosed_par, int escape, bool get_it_from_net, bool is_fifo)
    : file(file_par), escape_char(escape)
{
  ...
  if (field_term_length == line_term_length && !memcmp(field_term_ptr, line_term_ptr, field_term_length))
  {
    line_term_length = 0;
    line_term_ptr = (char *)"";
  }
  enclosed_char = (enclosed_length = enclosed_par.length()) ? (uchar)enclosed_par[0] : INT_MAX;
  ...
  else
  {
    ...
    if (init_io_cache(&cache, (get_it_from_net) ? -1 : file, 0,
                      (get_it_from_net) ? READ_NET : (is_fifo ? READ_FIFO : READ_CACHE), 0L, 1, MYF(MY_WME)))
    {
...
int READ_INFO::read_field()
{
  ...
  for (;;)
  {
    while (to < end_of_buff)
    {
      chr = GET;
#ifdef USE_MB
      if ((my_mbcharlen(read_charset, chr) > 1) && to + my_mbcharlen(read_charset, chr) <= end_of_buff)
      {
        uchar *p = (uchar *)to;
        *to++ = chr;
      ...
      if (chr == found_enclosed_char)
      {
        ...
        if (chr == my_b_EOF || chr == line_term_char && terminator(line_term_ptr, line_term_length))
        {  // Maybe unexpected linefeed
          enclosed = 1;
          found_end_of_line = 1;
          row_start = buffer + 1;
        ...
        if (chr == field_term_char && terminator(field_term_ptr, field_term_length))
        {
          enclosed = 1;
          row_start = buffer + 1;
          row_end = to;
