...
Geometry::Class_info *Geometry::ci_collection[Geometry::wkb_end + 1] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL};

static Geometry::Class_info **ci_collection_end = Geometry::ci_collection + Geometry::wkb_end + 1;

Geometry::Class_info::Class_info(const char *name, int type_id, void (*create_func)(void *))
    : m_name(name, strlen(name)), m_type_id(type_id), m_create_func(create_func)
{
  ci_collection[type_id] = this;
}

static void create_point(void *buffer) { new (buffer) Gis_point; }

static void create_linestring(void *buffer) { new (buffer) Gis_line_string; }

static void create_polygon(void *buffer) { new (buffer) Gis_polygon; }

static void create_multipoint(void *buffer) { new (buffer) Gis_multi_point; }

static void create_multipolygon(void *buffer) { new (buffer) Gis_multi_polygon; }

static void create_multilinestring(void *buffer) { new (buffer) Gis_multi_line_string; }

static void create_geometrycollection(void *buffer) { new (buffer) Gis_geometry_collection; }
...
static Geometry::Class_info multilinestring_class("MULTILINESTRING", Geometry::wkb_multilinestring,
...
static Geometry::Class_info geometrycollection_class("GEOMETRYCOLLECTION", Geometry::wkb_geometrycollection,
...
{
  for (Class_info **cur_rt = ci_collection; cur_rt < ci_collection_end; cur_rt++)
  {
    if (*cur_rt && ((*cur_rt)->m_name.length == len) &&
        (my_strnncoll(&my_charset_latin1, (const uchar *)(*cur_rt)->m_name.str, len, (const uchar *)name, len) == 0))
      return *cur_rt;
  }
  return 0;
}

Geometry *Geometry::create_from_wkb(Geometry_buffer *buffer, const char *data, uint32 data_len)
{
  uint32 geom_type;
  Geometry *result;

  if (data_len < 1 + 4) return NULL;
  data++;
  /*
  ...
  */
  ...
  data += 4;
  if (!(result = create_by_typeid(buffer, (int)geom_type))) return NULL;
  result->m_data = data;
  result->m_data_end = data + data_len;
  return result;
}

Geometry *Geometry::create_from_wkt(Geometry_buffer *buffer, Gis_read_stream *trs, String *wkt, bool init_stream)
{
  LEX_STRING name;
  Class_info *ci;
  ...
  if (!(ci = find_class(name.str, name.length)) || wkt->reserve(1 + 4, 512)) return NULL;
  ...
  if (trs->check_next_symbol('(') || result->init_from_wkt(trs, wkt) || trs->check_next_symbol(')')) return NULL;
  if (init_stream)
  {
...
bool Geometry::envelope(String *result) const
{
  ...
  if (get_mbr(&mbr, &end) || result->reserve(1 + 4 * 3 + SIZEOF_STORED_DOUBLE * 10)) return 1;
...
{
  if (no_data(data, SIZEOF_STORED_DOUBLE * 2) || result->reserve(1 + 4 + SIZEOF_STORED_DOUBLE * 2)) return 1;
  result->q_append((char)wkb_ndr);
...
{
  if (result->reserve(1 + 4 + SIZEOF_STORED_DOUBLE * 2)) return 1;
...
const char *Geometry::append_points(String *txt, uint32 n_points, const char *data, uint32 offset) const
{
  while (n_points--)
  {
    double x, y;
...
const char *Geometry::get_mbr_for_points(MBR *mbr, const char *data, uint offset) const
{
  uint32 points;
  /* read number of points */
  if (no_data(data, 4)) return 0;
  points = uint4korr(data);
  data += 4;

  if (no_data(data, (SIZEOF_STORED_DOUBLE * 2 + offset) * points)) return 0;
...
uint32 Gis_point::get_data_size() const { return POINT_DATA_SIZE; }

bool Gis_point::init_from_wkt(Gis_read_stream *trs, String *wkb)
{
  double x, y;
  if (trs->get_next_number(&x) || trs->get_next_number(&y) || wkb->reserve(SIZEOF_STORED_DOUBLE * 2)) return 1;
  wkb->q_append(x);
  wkb->q_append(y);
  return 0;
}
...
{
  double x, y;
  if (get_xy(&x, &y)) return 1;
  if (txt->reserve(MAX_DIGITS_IN_DOUBLE * 2 + 1)) return 1;
  txt->qs_append(x);
  txt->qs_append(' ');
  txt->qs_append(y);
...
{
  if (no_data(m_data, 4)) return GET_SIZE_ERROR;
...
bool Gis_line_string::init_from_wkt(Gis_read_stream *trs, String *wkb)
{
  ...
  if (wkb->reserve(4, 512)) return 1;
  wkb->length(wkb->length() + 4);  // Reserve space for points

  for (;;)
  {
    if (p.init_from_wkt(trs, wkb)) return 1;
    n_points++;
...
int Gis_line_string::length(double *len) const
{
  ...
  *len = 0;  // In case of errors
  ...
  n_points = uint4korr(data);
  data += 4;
  if (n_points < 1 || no_data(data, SIZEOF_STORED_DOUBLE * 2 * n_points)) return 1;
...
int Gis_line_string::is_closed(int *closed) const
{
  ...
  data += 4;
  if (no_data(data, SIZEOF_STORED_DOUBLE * 2 * n_points)) return 1;
...
uint32 Gis_polygon::get_data_size() const
{
  ...
  if (no_data(data, 4)) return GET_SIZE_ERROR;
...
bool Gis_polygon::init_from_wkt(Gis_read_stream *trs, String *wkb)
{
  ...
  for (;;)
  {
    ...
    if (trs->check_next_symbol('(') || ls.init_from_wkt(trs, wkb) || trs->check_next_symbol(')')) return 1;
...
bool Gis_polygon::get_mbr(MBR *mbr, const char **end) const
{
  ...
  while (n_linear_rings--)
  {
    if (!(data = get_mbr_for_points(mbr, data, 0))) return 1;
  }
  *end = data;
  return 0;
}
...
int Gis_polygon::exterior_ring(String *result) const
{
  ...
  if (no_data(data, length) || result->reserve(1 + 4 + 4 + length)) return 1;
...
int Gis_polygon::interior_ring_n(uint32 num, String *result) const
{
  ...
  if (num >= n_linear_rings || num < 1) return 1;
  ...
  data += 4;
  if (no_data(data, points_size) || result->reserve(1 + 4 + 4 + points_size)) return 1;
...
bool Gis_multi_point::get_data_as_wkt(String *txt, const char **end) const
{
  ...
  n_points = uint4korr(m_data);
  if (no_data(m_data + 4, n_points * (SIZEOF_STORED_DOUBLE * 2 + WKB_HEADER_SIZE)) ||
      txt->reserve(((MAX_DIGITS_IN_DOUBLE + 1) * 2 + 1) * n_points))
    return 1;
...
uint32 Gis_multi_line_string::get_data_size() const
{
  ...
  {
    if (no_data(data, WKB_HEADER_SIZE + 4)) return GET_SIZE_ERROR;
...
bool Gis_multi_line_string::get_data_as_wkt(String *txt, const char **end) const
{
  uint32 n_line_strings;
  const char *data = m_data;
  ...
  {
    uint32 n_points;
    if (no_data(data, (WKB_HEADER_SIZE + 4))) return 1;
...
int Gis_multi_line_string::geometry_n(uint32 num, String *result) const
{
  ...
  for (;;)
  {
    ...
    if (no_data(data, length)) return 1;
    if (!--num) break;
    data += length;
  }
...
int Gis_multi_line_string::is_closed(int *closed) const
{
  ...
  if (no_data(data, 4 + WKB_HEADER_SIZE)) return 1;
  n_line_strings = uint4korr(data);
...
uint32 Gis_multi_polygon::get_data_size() const
{
  ...
  {
    uint32 n_linear_rings;
    if (no_data(data, 4 + WKB_HEADER_SIZE)) return GET_SIZE_ERROR;
...
bool Gis_multi_polygon::init_from_wkt(Gis_read_stream *trs, String *wkb)
{
  ...
  for (;;)
  {
    ...
    if (trs->check_next_symbol('(') || p.init_from_wkt(trs, wkb) || trs->check_next_symbol(')')) return 1;
    n_polygons++;
    if (trs->skip_char(','))  // Didn't find ','
      break;
  }
...
bool Gis_multi_polygon::get_data_as_wkt(String *txt, const char **end) const
{
  ...
  {
    uint32 n_linear_rings;
    if (no_data(data, 4 + WKB_HEADER_SIZE) || txt->reserve(1, 512)) return 1;
    n_linear_rings = uint4korr(data + WKB_HEADER_SIZE);
    data += 4 + WKB_HEADER_SIZE;
    txt->q_append('(');
...
int Gis_multi_polygon::geometry_n(uint32 num, String *result) const
{
  ...
  if (no_data(data, 0))  // We must check last segment
    return 1;
  return result->append(start_of_polygon, (uint32)(data - start_of_polygon), (uint32)0);
}
...
int Gis_multi_polygon::area(double *ar, const char **end_of_data) const
{
  ...
  while (n_polygons--)
  {
    ...
    if (p.area(&p_area, &data)) return 1;
    result += p_area;
  }
  *ar = result;
  *end_of_data = data;
...
int Gis_multi_polygon::centroid(String *result) const
{
  ...
  while (n_polygons--)
  {
    data += WKB_HEADER_SIZE;
    p.init_from_wkb(data, (uint32)(m_data_end - data));
    if (p.area(&cur_area, &data) || p.centroid_xy(&cur_cx, &cur_cy)) return 1;
...
uint32 Gis_geometry_collection::get_data_size() const
{
  ...
  while (n_objects--)
  {
    ...
    if (no_data(data, WKB_HEADER_SIZE)) return GET_SIZE_ERROR;
    wkb_type = uint4korr(data + 1);
    data += WKB_HEADER_SIZE;

    if (!(geom = create_by_typeid(&buffer, wkb_type))) return GET_SIZE_ERROR;
    geom->init_from_wkb(data, (uint)(m_data_end - data));
    if ((object_size = geom->get_data_size()) == GET_SIZE_ERROR) return GET_SIZE_ERROR;
    data += object_size;
  }
  return (uint32)(data - m_data);
}
...
bool Gis_geometry_collection::get_data_as_wkt(String *txt, const char **end) const
{
  uint32 n_objects;
  Geometry_buffer buffer;
  Geometry *geom;
  ...
  while (n_objects--)
  {
    ...
    if (no_data(data, WKB_HEADER_SIZE)) return 1;
    wkb_type = uint4korr(data + 1);
    data += WKB_HEADER_SIZE;

    if (!(geom = create_by_typeid(&buffer, wkb_type))) return 1;
    geom->init_from_wkb(data, (uint)(m_data_end - data));
    if (geom->as_wkt(txt, &data)) return 1;
    if (txt->append(",", 1, 512)) return 1;
  }
  txt->length(txt->length() - 1);
  *end = data;
  return 0;
...
bool Gis_geometry_collection::get_mbr(MBR *mbr, const char **end) const
{
  ...
  while (n_objects--)
  {
    ...
    if (geom->get_mbr(mbr, &data)) return 1;
  }
  *end = data;
  return 0;
}
...
int Gis_geometry_collection::geometry_n(uint32 num, String *result) const
{
  ...
  if (result->reserve(1 + 4 + length)) return 1;
  result->q_append((char)wkb_ndr);
...
bool Gis_geometry_collection::dimension(uint32 *res_dim, const char **end) const
{
  ...
  *res_dim = 0;
  while (n_objects--)
  {
    ...
    if (!(geom = create_by_typeid(&buffer, wkb_type))) return 1;
    geom->init_from_wkb(data, (uint32)(m_data_end - data));
    if (geom->dimension(&dim, &end_data)) return 1;
