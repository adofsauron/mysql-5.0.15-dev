...
static int write_keys(SORTPARAM *param, uchar **sort_keys, uint count,
                      IO_CACHE *buffer_file, IO_CACHE *tempfile);
static void make_sortkey(SORTPARAM *param, uchar *to, byte *ref_pos);
static int merge_index(SORTPARAM *param, uchar *sort_buffer, BUFFPEK *buffpek,
                       uint maxbuffer, IO_CACHE *tempfile, IO_CACHE *outfile);
...
ha_rows filesort(THD *thd, TABLE *table, SORT_FIELD *sortorder, uint s_length,
  ...
  */
  ...
  if (!(table->file->table_flags() & HA_FAST_KEY_READ) &&
    ...
    */
    param.addon_field = get_addon_fields(thd, table->field, param.sort_length,
                                         &param.addon_length);
  }
  ...
  if (param.addon_field) {
    ...
    if (!(table_sort.addon_buf =
              (byte *)my_malloc(param.addon_length, MYF(MY_WME))))
      goto err;
  } else {
  ...
  while (memavl >= min_sort_memory) {
    ...
    if ((sort_keys =
             (uchar **)make_char_array(param.keys, param.rec_length, MYF(0))))
      break;
    old_memavl = memavl;
  ...
  if (maxbuffer == 0) // The whole set is in memory
  {
    ...
    */
    param.keys =
        ((param.keys * (param.rec_length + sizeof(char *))) / param.rec_length -
         1);
...
static ha_rows find_all_keys(SORTPARAM *param, SQL_SELECT *select,
                             uchar **sort_keys, IO_CACHE *buffpek_pointers,
                             IO_CACHE *tempfile, IO_CACHE *indexfile) {
  ...
  DBUG_PRINT("info", ("using: %s", (select ? select->quick ? "ranges" : "where"
                                                           : "every row")));
  ...
  record = 0;
  flag =
      ((!indexfile && file->table_flags() & HA_REC_NOT_IN_SEQ) || quick_select);
  if (indexfile || flag)
    ref_pos = &file->ref[0];
  next_pos = ref_pos;
  ...
  for (;;) {
    ...
    {
      if (indexfile) {
        if (my_b_read(indexfile, (byte *)ref_pos,
                      ref_length)) /* purecov: deadcode */
        {
  ...
  if (error != HA_ERR_END_OF_FILE) {
    file->print_error(error,
                      MYF(ME_ERROR | ME_WAITTANG)); /* purecov: inspected */
    DBUG_RETURN(HA_POS_ERROR);                      /* purecov: inspected */
  }
  if (indexpos && idx &&
...
static void make_sortkey(register SORTPARAM *param, register uchar *to,
                         byte *ref_pos) {
  reg3 Field *field;
  reg1 SORT_FIELD *sort_field;
  reg5 uint length;

  for (sort_field = param->local_sortorder; sort_field != param->end;
       sort_field++) {
    bool maybe_null = 0;
    if ((field = sort_field->field)) { // Field
      if (field->maybe_null()) {
        if (field->is_null()) {
      ...
      case STRING_RESULT: {
        ...
        if (sort_field->need_strxnfrm) {
          ...
          my_strnxfrm(cs, (uchar *)to, length, (const uchar *)res->ptr(),
                      length);
    ...
    if (sort_field->reverse) { /* Revers key */
      if (maybe_null)
        to[-1] = ~to[-1];
...
int merge_buffers(SORTPARAM *param, IO_CACHE *from_file, IO_CACHE *to_file,
  ...
  for (buffpek = Fb; buffpek <= Tb; buffpek++) {
    ...
    if (error == -1)
      goto err; /* purecov: inspected */
    buffpek->max_keys =
        buffpek->mem_count; // If less data in buffers than expected
  ...
  */
  ...
  do {
    if ((ha_rows)buffpek->mem_count >
        max_rows) { /* Don't write too many records */
    ...
    if (flag == 0) {
      ...
      for (end = strpos + buffpek->mem_count * rec_length; strpos != end;
           strpos += rec_length) {
    ...
    if (sortorder->field) {
      ...
      case DECIMAL_RESULT:
        sortorder->length = my_decimal_get_binary_size(
            sortorder->item->max_length - (sortorder->item->decimals ? 1 : 0),
...
static SORT_ADDON_FIELD *get_addon_fields(THD *thd, Field **ptabfield,
  ...
  */
  ...
  if (length + sortlength > thd->variables.max_length_for_sort_data ||
      !(addonf = (SORT_ADDON_FIELD *)my_malloc(
            sizeof(SORT_ADDON_FIELD) * (fields + 1), MYF(MY_WME))))
    return 0;
