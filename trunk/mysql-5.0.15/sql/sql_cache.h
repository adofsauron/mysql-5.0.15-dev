...
struct Query_cache_query
{
  ...
  inline gptr query() { return (gptr)(((byte *)this) + ALIGN_SIZE(sizeof(Query_cache_query))); }
  void lock_writing();
  void lock_reading();
...
struct Query_cache_table
{
  ...
  inline gptr data() { return (gptr)(((byte *)this) + ALIGN_SIZE(sizeof(Query_cache_table))); }
};
...
struct Query_cache_result
{
  ...
  inline gptr data() { return (gptr)(((byte *)this) + ALIGN_SIZE(sizeof(Query_cache_result))); }
...
extern "C"
{
  ...
  byte *query_cache_table_get_key(const byte *record, uint *length, my_bool not_used);
}
...
class Query_cache
{
 public:
  ...
  /* statistics */
  ulong free_memory, queries_in_cache, hits, inserts, refused, free_memory_blocks, total_blocks, lowmem_prunes;
  ...
  */
  ...
  my_bool allocate_data_chain(Query_cache_block **result_block, ulong data_len, Query_cache_block *query_block,
                              my_bool first_block);
  ...
  TABLE_COUNTER_TYPE
  register_tables_from_list(TABLE_LIST *tables_used, TABLE_COUNTER_TYPE counter, Query_cache_block_table *block_table);
  my_bool register_all_tables(Query_cache_block *block, TABLE_LIST *tables_used, TABLE_COUNTER_TYPE tables);
  my_bool insert_table(uint key_len, char *key, Query_cache_block_table *node, uint32 db_length, uint8 cache_type,
  ...
  my_bool move_by_type(byte **border, Query_cache_block **before, ulong *gap, Query_cache_block *i);
  ...
  void pack_cache();
  void relink(Query_cache_block *oblock, Query_cache_block *nblock, Query_cache_block *next, Query_cache_block *prev,
  ...
  */
  ...
  void make_disabled();
  void free_cache();
  Query_cache_block *write_block_data(ulong data_len, gptr data, ulong header_len, Query_cache_block::block_type type,
                                      TABLE_COUNTER_TYPE ntab = 0, my_bool under_guard = 0);
  my_bool append_result_data(Query_cache_block **result, ulong data_len, gptr data, Query_cache_block *parent);
  my_bool write_result_data(Query_cache_block **result, ulong data_len, gptr data, Query_cache_block *parent,
                            Query_cache_block::block_type type = Query_cache_block::RESULT);
  inline ulong get_min_first_result_data_size();
  inline ulong get_min_append_result_data_size();
  Query_cache_block *allocate_block(ulong len, my_bool not_less, ulong min, my_bool under_guard = 0);
  /*
    If query is cacheable return number tables in query
    (query without tables not cached)
  */
  TABLE_COUNTER_TYPE is_cacheable(THD *thd, uint32 query_len, char *query, LEX *lex, TABLE_LIST *tables_used,
                                  uint8 *tables_type);

  static my_bool ask_handler_allowance(THD *thd, TABLE_LIST *tables_used);

 public:
  Query_cache(ulong query_cache_limit = ULONG_MAX, ulong min_allocation_unit = QUERY_CACHE_MIN_ALLOCATION_UNIT,
