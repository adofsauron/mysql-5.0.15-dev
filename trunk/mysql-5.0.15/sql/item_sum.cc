...
Field *Item_sum::create_tmp_field(bool group, TABLE *table,
  ...
  case STRING_RESULT:
    if (max_length > 255 && convert_blob_length)
      return new Field_varstring(convert_blob_length, maybe_null, name, table,
                                 collation.collation);
...
void Item_sum_variance::fix_length_and_dec() {
  ...
  case DECIMAL_RESULT: {
    ...
    */
    ...
    f_precision1 =
        min(args[0]->decimal_precision() * 2 + DECIMAL_LONGLONG_DIGITS,
            DECIMAL_MAX_PRECISION);
...
bool Item_sum_variance::add() {
  if (hybrid_type == DECIMAL_RESULT) {
    ...
    if (!args[0]->null_value) {
      ...
      my_decimal_add(E_DEC_FATAL_ERROR, dec_sqr + next_dec, dec_sqr + cur_dec,
                     &sqr_buf);
      my_decimal_add(E_DEC_FATAL_ERROR, dec_sum + next_dec, dec_sum + cur_dec,
                     dec);
      cur_dec = next_dec;
    }
  } else {
...
my_decimal *Item_sum_variance::val_decimal(my_decimal *dec_buf) {
  ...
  my_decimal_mul(E_DEC_FATAL_ERROR, &sum_sqr_buf, dec_sum + cur_dec,
                 dec_sum + cur_dec);
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &sum_sqr_buf, &count_buf,
                 prec_increment);
  my_decimal_sub(E_DEC_FATAL_ERROR, &sum_sqr_buf, dec_sqr + cur_dec, dec_buf);
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &sum_sqr_buf, &count1_buf,
                 prec_increment);
  return dec_buf;
}
...
void Item_sum_variance::reset_field() {
  ...
  if (hybrid_type == DECIMAL_RESULT) {
    ...
    my_decimal2binary(E_DEC_FATAL_ERROR, arg2_dec, res + dec_bin_size0,
                      f_precision1, f_scale1);
...
void Item_sum_variance::update_field() {
  ...
  if (hybrid_type == DECIMAL_RESULT) {
    ...
    if (!args[0]->null_value) {
      binary2my_decimal(E_DEC_FATAL_ERROR, res, dec_sum + 1, f_precision0,
                        f_scale0);
      binary2my_decimal(E_DEC_FATAL_ERROR, res + dec_bin_size0, dec_sqr + 1,
                        f_precision1, f_scale1);
      ...
      field_count++;
      my_decimal2binary(E_DEC_FATAL_ERROR, dec_sum, res, f_precision0,
                        f_scale0);
      my_decimal2binary(E_DEC_FATAL_ERROR, dec_sqr, res + dec_bin_size0,
                        f_precision1, f_scale1);
      res += dec_bin_size0 + dec_bin_size1;
...
bool Item_sum_min::add() {
  ...
  case INT_RESULT: {
    ...
    if (!args[0]->null_value &&
        (null_value || (unsigned_flag && (ulonglong)nr < (ulonglong)sum_int) ||
...
my_decimal *Item_variance_field::val_decimal(my_decimal *dec_buf) {
  ...
  binary2my_decimal(E_DEC_FATAL_ERROR, field->ptr, &dec_sum, f_precision0,
                    f_scale0);
  binary2my_decimal(E_DEC_FATAL_ERROR, field->ptr + dec_bin_size0, &dec_sqr,
                    f_precision1, f_scale1);
  ...
  my_decimal_div(E_DEC_FATAL_ERROR, dec_buf, &dec_sum, &dec1_count,
                 prec_increment);
  return dec_buf;
}
...
bool Item_sum_count_distinct::setup(THD *thd) {
  ...
  */
  ...
  DBUG_ASSERT(table == 0);
  if (!(table = create_tmp_table(thd, tmp_table_param, list, (ORDER *)0, 1, 0,
...
int group_concat_key_cmp_with_distinct(void *arg, byte *key1, byte *key2) {
  ...
  for (field_item = grp_item->args,
      end = field_item + grp_item->arg_count_field;
       field_item < end; field_item++) {
    /*
...
int group_concat_key_cmp_with_order(void *arg, byte *key1, byte *key2) {
  ...
  for (order_item = grp_item->order,
      end = order_item + grp_item->arg_count_order;
...
Item_func_group_concat::Item_func_group_concat(
    Name_resolution_context *context_arg, bool distinct_arg,
    List<Item> *select_list, SQL_LIST *order_list, String *separator_arg)
    : tmp_table_param(0), warning(0), separator(separator_arg), tree(0),
      table(0), order(0), context(context_arg),
      arg_count_order(order_list ? order_list->elements : 0),
      arg_count_field(select_list->elements), count_cut_values(0),
      distinct(distinct_arg), warning_for_row(FALSE), original(0) {
  Item *item_select;
  Item **arg_ptr;
  ...
  */
  ...
  if (arg_count_order) {
    ...
    for (ORDER *order_item = (ORDER *)order_list->first; order_item != NULL;
...
Item_func_group_concat::Item_func_group_concat(THD *thd,
                                               Item_func_group_concat *item)
    : Item_sum(thd, item), tmp_table_param(item->tmp_table_param),
      warning(item->warning), separator(item->separator), tree(item->tree),
      table(item->table), order(item->order), context(item->context),
      arg_count_order(item->arg_count_order),
      arg_count_field(item->arg_count_field),
      count_cut_values(item->count_cut_values), distinct(item->distinct),
      warning_for_row(item->warning_for_row), always_null(item->always_null),
      original(item) {
  quick_group = item->quick_group;
}
...
bool Item_func_group_concat::add() {
  ...
  */
  if (result.length() <= max_length && !warning_for_row &&
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref) {
  ...
  */
  ...
  if (agg_item_charsets(collation, func_name(), args, arg_count,
                        MY_COLL_ALLOW_CONV))
    return 1;
...
bool Item_func_group_concat::setup(THD *thd) {
  ...
  */
  if (!(table = create_tmp_table(thd, tmp_table_param, all_fields, (ORDER *)0,
                                 0, TRUE, (select_lex->options | thd->options),
                                 HA_POS_ERROR, (char *)"")))
    DBUG_RETURN(TRUE);
  ...
  if (distinct || arg_count_order) {
    ...
    */
    init_tree(tree, min(thd->variables.max_heap_table_size,
              ...
              0, tree_key_length, compare_key, 0, NULL, (void *)this);
  }
