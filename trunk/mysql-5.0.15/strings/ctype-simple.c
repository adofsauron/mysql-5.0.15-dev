...
int my_strnxfrm_simple(CHARSET_INFO *cs, uchar *dest, uint len, const uchar *src, uint srclen)
{
  ...
  {
    const uchar *end;
    for (end = src + len; src < end;) *dest++ = map[*src++];
  }
  else
  {
    const uchar *end;
    for (end = dest + len; dest < end; dest++) *dest = (char)map[(uchar)*dest];
  }
  if (dstlen > len)
...
uint my_caseup_8bit(CHARSET_INFO *cs, char *src, uint srclen, char *dst __attribute__((unused)),
                    uint dstlen __attribute__((unused)))
{
  uint srclen0 = srclen;
  register uchar *map = cs->to_upper;
  DBUG_ASSERT(src == dst && srclen == dstlen);
  for (; srclen > 0; srclen--, src++) *src = (char)map[(uchar)*src];
  return srclen0;
}

uint my_casedn_8bit(CHARSET_INFO *cs, char *src, uint srclen, char *dst __attribute__((unused)),
                    uint dstlen __attribute__((unused)))
{
  uint srclen0 = srclen;
  register uchar *map = cs->to_lower;
...
int my_mb_wc_8bit(CHARSET_INFO *cs, my_wc_t *wc, const unsigned char *str,
...
int my_snprintf_8bit(CHARSET_INFO *cs __attribute__((unused)), char *to, uint n __attribute__((unused)),
                     const char *fmt, ...)
{
  va_list args;
  int result;
...
longlong my_strntoll_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *nptr, uint l, int base, char **endptr,
                          int *err)
{
  int negative;
...
my_bool my_like_range_simple(CHARSET_INFO *cs, const char *ptr, uint ptr_length, pbool escape, pbool w_one,
                             pbool w_many, uint res_length, char *min_str, char *max_str, uint *min_length,
                             uint *max_length)
{
...
void my_fill_8bit(CHARSET_INFO *cs __attribute__((unused)), char *s, uint l, int fill) { bfill(s, l, fill); }

uint my_numchars_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *b, const char *e) { return (uint)(e - b); }

uint my_numcells_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *b, const char *e) { return (uint)(e - b); }

uint my_charpos_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *b __attribute__((unused)),
                     const char *e __attribute__((unused)), uint pos)
{
  return pos;
}

uint my_well_formed_len_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *start, const char *end, uint nchars,
                             int *error)
{
...
uint my_lengthsp_8bit(CHARSET_INFO *cs __attribute__((unused)), const char *ptr, uint length)
{
  ...
  while (end > ptr && end[-1] == ' ') end--;
  return (uint)(end - ptr);
}

uint my_instr_simple(CHARSET_INFO *cs, const char *b, uint b_length, const char *s, uint s_length, my_match_t *match,
                     uint nmatch)
{
...
static my_bool create_fromuni(CHARSET_INFO *cs, void *(*alloc)(uint))
{
  ...
  */
  ...
  for (i = 0; i < n; i++) cs->tab_from_uni[i] = idx[i].uidx;
...
my_bool my_propagate_simple(CHARSET_INFO *cs __attribute__((unused)), const uchar *str __attribute__((unused)),
                            uint length __attribute__((unused)))
{
  return 1;
}

my_bool my_propagate_complex(CHARSET_INFO *cs __attribute__((unused)), const uchar *str __attribute__((unused)),
                             uint length __attribute__((unused)))
{
  return 0;
}
...
MY_COLLATION_HANDLER my_collation_8bit_simple_ci_handler = {
    ...
    my_strnncoll_simple, my_strnncollsp_simple, my_strnxfrm_simple, my_strnxfrmlen_simple, my_like_range_simple,
    my_wildcmp_8bit,     my_strcasecmp_8bit,    my_instr_simple,    my_hash_sort_simple,   my_propagate_simple};
