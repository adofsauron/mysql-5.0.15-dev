...
class hash_filo
{
 ...
 public:
  ...
  hash_filo(uint size_arg, uint key_offset_arg, uint key_length_arg, hash_get_key get_key_arg,
            hash_free_key free_element_arg, CHARSET_INFO *hash_charset_arg)
      : size(size_arg),
        key_offset(key_offset_arg),
  ...
  {
    hash_filo_element *entry = (hash_filo_element *)hash_search(&cache, (byte *)key, length);
    if (entry)
    {  // Found; link it first
      if (entry != first_link)
      {  // Relink used-chain
