...
#ifdef HAVE_purify
static int inno_bcmp(register const char *s1, register const char *s2, register uint len)
{
  while (len-- != 0 && *s1++ == *s2++)
    ;
  return len + 1;
  ...
  if (ret == FALSE)
  {
    ...
    if (size != srv_calc_low32(srv_log_file_size) || size_high != srv_calc_high32(srv_log_file_size))
    {
      fprintf(stderr,
              ...
              name, (ulong)size_high, (ulong)size, (ulong)srv_calc_high32(srv_log_file_size),
    ...
    ulint *sum_of_new_sizes) /* out: sum of sizes of the new files added */
{
  ...
  *create_new_db = FALSE;
  ...
  for (i = 0; i < srv_n_data_files; i++)
  {
    ...
    ut_a(strlen(srv_data_home) + strlen(srv_data_file_names[i]) < (sizeof name) - 1);
    ...
    if (ret == FALSE)
    {
      ...
      if (i == srv_n_data_files - 1 && srv_auto_extend_last_data_file)
      {
            ...
            (srv_last_file_size_max > 0 && srv_last_file_size_max < rounded_size_pages))
        {
          fprintf(stderr,
                  ...
                  name, (ulong)rounded_size_pages, (ulong)srv_data_file_sizes[i], (ulong)srv_last_file_size_max);
      ...
      if (rounded_size_pages != srv_data_file_sizes[i])
      {
        fprintf(stderr,
                ...
                name, (ulong)rounded_size_pages, (ulong)srv_data_file_sizes[i]);
    ...
    else
    {
      ...
      ret = os_file_set_size(name, files[i], srv_calc_low32(srv_data_file_sizes[i]),
    ...
    {
      fil_node_create(name, srv_data_file_sizes[i], 0, FALSE);
    }
  }
...
/* out: DB_SUCCESS or error code */
{
  ...
  fprintf(stderr, "InnoDB: !!!!!!!!!!!!!! UNIV_SIMULATE_AWE switched on !!!!!!!!!!!!!!!!!\n");
#endif
  if (srv_sizeof_trx_t_in_ha_innodb_cc != (ulint)sizeof(trx_t))
  {
    fprintf(stderr,
            ...
            (ulong)srv_sizeof_trx_t_in_ha_innodb_cc, (ulong)sizeof(trx_t));
    return (DB_ERROR);
  }
...
#ifdef __WIN__
  if (os_get_os_version() == OS_WIN95 || os_get_os_version() == OS_WIN31 || os_get_os_version() == OS_WINNT)
  {
  ...
  else if (0 == ut_strcmp(srv_file_flush_method_str, "async_unbuffered"))
  {
    srv_win_file_flush_method = SRV_WIN_IO_UNBUFFERED;
#endif
  }
  ...
  {
    srv_monitor_file_name = mem_alloc(strlen(fil_path_to_mysql_datadir) + 20 + sizeof "/innodb_status.");
    sprintf(srv_monitor_file_name, "%s/innodb_status.%lu", fil_path_to_mysql_datadir, os_proc_get_number());
  ...
  if (!os_aio_use_native_aio)
  {
    ...
    os_aio_init(8 * SRV_N_PENDING_IOS_PER_THREAD * srv_n_file_io_threads, srv_n_file_io_threads,
                SRV_MAX_N_PENDING_SYNC_IOS);
  }
  else
  {
    os_aio_init(SRV_N_PENDING_IOS_PER_THREAD * srv_n_file_io_threads, srv_n_file_io_threads,
                SRV_MAX_N_PENDING_SYNC_IOS);
  }
  ...
  if (srv_use_awe)
  {
    fprintf(stderr, "InnoDB: Using AWE: Memory window is %lu MB and AWE memory is %lu MB\n",
    ...
    ret = buf_pool_init(srv_pool_size, srv_pool_size, srv_awe_window_size);
  }
  else
  {
    ret = buf_pool_init(srv_pool_size, srv_pool_size, srv_pool_size);
  }

  if (ret == NULL)
  {
    fprintf(stderr, "InnoDB: Fatal error: cannot allocate the memory for the buffer pool\n");
  ...
  err = open_or_create_data_files(&create_new_db,
                                  ...
                                  &min_arch_log_no, &max_arch_log_no,
#endif /* UNIV_LOG_ARCHIVE */
                                  &min_flushed_lsn, &max_flushed_lsn, &sum_of_new_sizes);
  if (err != DB_SUCCESS)
  {
    fprintf(stderr,
  ...
  {
    fprintf(stderr, "InnoDB: Starting archive recovery from a backup...\n");
  ...
  if (srv_force_recovery > 0)
  {
    fprintf(stderr, "InnoDB: !!! innodb_force_recovery is set to %lu !!!\n", (ulong)srv_force_recovery);
  }
  ...
  if (trx_doublewrite_must_reset_space_ids)
  {
    ...
    fprintf(stderr, "InnoDB: Full purge and insert buffer merge completed.\n");
...
/* out: DB_SUCCESS or error code */
{
  ...
  if (i == 1000)
  {
    fprintf(stderr, "InnoDB: Warning: %lu threads created by InnoDB had not exited at shutdown!\n",
  ...
  if (os_thread_count != 0 || os_event_count != 0 || os_mutex_count != 0 || os_fast_mutex_count != 0)
  {
    fprintf(stderr,
            ...
            (ulong)os_thread_count, (ulong)os_event_count, (ulong)os_mutex_count, (ulong)os_fast_mutex_count);
  }
  ...
  if (srv_print_verbose_log)
  {
    ...
    fprintf(stderr, "  InnoDB: Shutdown completed; log sequence number %lu %lu\n",
            (ulong)ut_dulint_get_high(srv_shutdown_lsn), (ulong)ut_dulint_get_low(srv_shutdown_lsn));
  }
