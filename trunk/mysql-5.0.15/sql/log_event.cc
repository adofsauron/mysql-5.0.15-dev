...
{
  return ((err_code == ER_SLAVE_IGNORED_TABLE) || (use_slave_mask && bitmap_is_set(&slave_error_mask, err_code)));
}
#endif
...
#if defined(HAVE_REPLICATION) && !defined(MYSQL_CLIENT)
static inline char *slave_load_file_stem(char *buf, uint file_id, int event_server_id)
{
...
static bool write_str(IO_CACHE *file, char *str, uint length)
{
  byte tmp[1];
  tmp[0] = (byte)length;
  return (my_b_safe_write(file, tmp, sizeof(tmp)) || my_b_safe_write(file, (byte *)str, length));
}
...
static void print_set_option(FILE *file, uint32 bits_changed, uint32 option, uint32 flags, const char *name,
                             bool *need_comma)
{
...
Log_event::Log_event() : temp_buf(0), exec_time(0), flags(0), cache_stmt(0), thd(0)
{
  server_id = ::server_id;
  when = time(NULL);
  log_pos = 0;
...
Log_event::Log_event(const char *buf, const Format_description_log_event *description_event)
    : temp_buf(0), cache_stmt(0)
{
  ...
  */
  if (description_event->binlog_version == 3 && buf[EVENT_TYPE_OFFSET] < FORMAT_DESCRIPTION_EVENT && log_pos)
  {
    /*
...
void Log_event::pack_info(Protocol *protocol) { protocol->store("", &my_charset_bin); }
...
void Log_event::init_show_field_list(List<Item> *field_list)
{
  ...
  field_list->push_back(new Item_return_int("Pos", 11, MYSQL_TYPE_LONGLONG));
  field_list->push_back(new Item_empty_string("Event_type", 20));
  field_list->push_back(new Item_return_int("Server_id", 10, MYSQL_TYPE_LONG));
  field_list->push_back(new Item_return_int("End_log_pos", 11, MYSQL_TYPE_LONGLONG));
...
int Log_event::read_log_event(IO_CACHE *file, String *packet, pthread_mutex_t *log_lock)
{
  ulong data_len;
  int result = 0;
  ...
  if (data_len < LOG_EVENT_MINIMAL_HEADER_LEN || data_len > current_thd->variables.max_allowed_packet)
  {
    DBUG_PRINT("error", ("data_len: %ld", data_len));
    result = ((data_len < LOG_EVENT_MINIMAL_HEADER_LEN) ? LOG_READ_BOGUS : LOG_READ_TOO_LARGE);
    goto end;
  }
...
#endif
{
  ...
  */
  uint header_size = min(description_event->common_header_len, LOG_EVENT_MINIMAL_HEADER_LEN);
                                     ...
                                     const Format_description_log_event *description_event)
{
  ...
  if (event_len < EVENT_LEN_OFFSET || (uint)event_len != uint4korr(buf + EVENT_LEN_OFFSET))
  {
...
void Log_event::print_header(FILE *file)
{
  ...
  fprintf(file, " server id %d  end_log_pos %s ", server_id, llstr(log_pos, llbuff));
}
...
void Log_event::print_timestamp(FILE *file, time_t *ts)
{
  ...
  fprintf(file, "%02d%02d%02d %2d:%02d:%02d", res->tm_year % 100, res->tm_mon + 1, res->tm_mday, res->tm_hour,
...
{
  uchar buf[QUERY_HEADER_LEN + 1 + 4 +         // code of flags2 and flags2
  ...
  */
  ...
  {
    write_str_with_code_and_len((char **)(&start), catalog, catalog_len, Q_CATALOG_NZ_CODE);
    /*
  ...
  */
  ...
  return (write_header(file, event_length) || my_b_safe_write(file, (byte *)buf, QUERY_HEADER_LEN) ||
          write_post_header_for_derived(file) ||
          my_b_safe_write(file, (byte *)start_of_status, (uint)(start - start_of_status)) ||
          my_b_safe_write(file, (db) ? (byte *)db : (byte *)"", db_len + 1) ||
          my_b_safe_write(file, (byte *)query, q_len))
             ? 1
             : 0;
}
...
*/
Query_log_event::Query_log_event(THD *thd_arg, const char *query_arg, ulong query_length, bool using_trans,
                                 bool suppress_use)
    : Log_event(
          thd_arg,
          ((thd_arg->tmp_table_used ? LOG_EVENT_THREAD_SPECIFIC_F : 0) | (suppress_use ? LOG_EVENT_SUPPRESS_USE_F : 0)),
          using_trans),
      data_buf(0),
      query(query_arg),
      ...
      error_code((thd_arg->killed != THD::NOT_KILLED)
                     ? ((thd_arg->system_thread & SYSTEM_THREAD_DELAYED_INSERT) ? 0 : thd->killed_errno())
...
Query_log_event::Query_log_event(const char *buf, uint event_len, const Format_description_log_event *description_event,
      ...
      time_zone_len(0)
{
  ...
  DBUG_PRINT("info", ("event_len=%ld, common_header_len=%d, post_header_len=%d", event_len, common_header_len,
                      post_header_len));
  ...
  */
  ...
  if (tmp)
  {
    ...
    DBUG_PRINT("info", ("Query_log_event has status_vars_len: %u", (uint)status_vars_len));
    tmp -= 2;
  }
  /*
  ...
  for (const uchar *pos = (const uchar *)start; pos < (const uchar *)end;)
  {
    switch (*pos++)
    {
      ...
      case Q_SQL_MODE_CODE:
      {
        ...
        DBUG_PRINT("info", ("In Query_log_event, read sql_mode: %s", llstr(sql_mode, buff)));
        pos += 8;
        break;
      }
 ...
 code: %u), skipping the rest of them",
  ...
  if (!(start = data_buf = (char *)my_malloc(
            catalog_len + 1 + time_zone_len + 1 + data_len + 1 + QUERY_CACHE_FLAGS_SIZE + db_len + 1, MYF(MY_WME))))
#else
  if (!(start = data_buf = (char *)my_malloc(catalog_len + 1 + time_zone_len + 1 + data_len + 1, MYF(MY_WME))))
#endif
    DBUG_VOID_RETURN;
...
#ifdef MYSQL_CLIENT
void Query_log_event::print_query_header(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info)
{
  ...
  {
    print_header(file);
    fprintf(file, "\t%s\tthread_id=%lu\texec_time=%lu\terror_code=%d\n", get_type_str(), (ulong)thread_id,
            (ulong)exec_time, error_code);
  }
  ...
  */
  if (likely(flags2_inited)) /* likely as this will mainly read 5.0 logs */
  {
    ...
    if (likely(last_event_info->flags2_inited)) /* All bits which have changed */
    ...
    if (unlikely(tmp)) /* some bits have changed */
    {
      bool need_comma = 0;
      fprintf(file, "SET ");
      print_set_option(file, tmp, OPTION_NO_FOREIGN_KEY_CHECKS, ~flags2, "@@session.foreign_key_checks", &need_comma);
      print_set_option(file, tmp, OPTION_AUTO_IS_NULL, flags2, "@@session.sql_auto_is_null", &need_comma);
      print_set_option(file, tmp, OPTION_RELAXED_UNIQUE_CHECKS, ~flags2, "@@session.unique_checks", &need_comma);
      fprintf(file, ";\n");
  ...
  if (likely(charset_inited))
  {
    ...
    if (unlikely(bcmp(last_event_info->charset, charset, 6)))
    {
      fprintf(file,
              ...
              ";\n",
              uint2korr(charset), uint2korr(charset + 2), uint2korr(charset + 4));
...
void Query_log_event::print(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info)
{
...
#if defined(HAVE_REPLICATION) && !defined(MYSQL_CLIENT)
int Query_log_event::exec_event(struct st_relay_log_info *rli) { return exec_event(rli, query, q_len); }
...
int Query_log_event::exec_event(struct st_relay_log_info *rli, const char *query_arg, uint32 q_len_arg)
{
  ...
  */
  if (db_ok(thd->db, replicate_do_db, replicate_ignore_db))
  {
    ...
    if (ignored_error_code((expected_error = error_code)) || !check_expected_error(thd, rli, expected_error))
    {
      if (flags2_inited)
        /*
      ...
      */
      if (sql_mode_inited)
        thd->variables.sql_mode =
            (ulong)((thd->variables.sql_mode & MODE_NO_DIR_IN_CREATE) | (sql_mode & ~(ulong)MODE_NO_DIR_IN_CREATE));
      if (charset_inited)
      {
        if (rli->cached_charset_compare(charset))
        {
          /* Verify that we support the charsets found in the event. */
          if (!(thd->variables.character_set_client = get_charset(uint2korr(charset), MYF(MY_WME))) ||
              !(thd->variables.collation_connection = get_charset(uint2korr(charset + 2), MYF(MY_WME))) ||
              !(thd->variables.collation_server = get_charset(uint2korr(charset + 4), MYF(MY_WME))))
          {
            /*
      ...
      if (time_zone_len)
      {
        ...
        if (!(thd->variables.time_zone = my_tz_find_with_opening_tz_tables(thd, &tmp)))
        {
...
START SLAVE; . Query: '%s'",
   ...
   */
    DBUG_PRINT("info", ("expected_error: %d  last_errno: %d", expected_error, thd->net.last_errno));
    if ((expected_error != (actual_error = thd->net.last_errno)) && expected_error &&
        !ignored_error_code(actual_error) && !ignored_error_code(expected_error))
    {
...
Default database: '%s'. Query: '%s'",
                        ER_SAFE(expected_error), expected_error, actual_error ? thd->net.last_error : "no error",
    ...
    */
    ...
    {
      slave_print_error(rli, actual_error, "Error '%s' on query. Default database: '%s'. Query: '%s'",
                        (actual_error ? thd->net.last_error : "unexpected success or fatal error"),
  ...
  */
  return (thd->query_error ? thd->query_error
                           : (thd->one_shot_set ? (rli->inc_event_relay_log_pos(), 0) : Log_event::exec_event(rli)));
}
#endif
...
Start_log_event_v3::Start_log_event_v3(const char *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
...
bool Start_log_event_v3::write(IO_CACHE *file)
{
  ...
  return (write_header(file, sizeof(buff)) || my_b_safe_write(file, (byte *)buff, sizeof(buff)));
}
#endif
...
Format_description_log_event::Format_description_log_event(uint8 binlog_ver, const char *server_ver)
    : Start_log_event_v3()
{
  created = when;
  ...
  switch (binlog_ver)
  {
    case 4: /* MySQL 5.0 */
      ...
      post_header_len = (uint8 *)my_malloc(number_of_event_types * sizeof(uint8), MYF(MY_ZEROFILL));
      /*
    ...
    case 3: /* 4.0.x x>=2 */
      ...
      */
      ...
      common_header_len = binlog_ver == 1 ? OLD_HEADER_LEN : LOG_EVENT_MINIMAL_HEADER_LEN;
      /*
      ...
      */
      ...
      post_header_len = (uint8 *)my_malloc(number_of_event_types * sizeof(uint8), MYF(0));
      if (post_header_len)
      {
        post_header_len[START_EVENT_V3 - 1] = START_V3_HEADER_LEN;
...
Format_description_log_event::Format_description_log_event(const char *buf, uint event_len,
                                                           const Format_description_log_event *description_event)
    : Start_log_event_v3(buf, description_event)
{
  ...
  post_header_len = (uint8 *)my_memdup((byte *)buf + ST_COMMON_HEADER_LEN_OFFSET + 1,
                                       number_of_event_types * sizeof(*post_header_len), MYF(0));
  DBUG_VOID_RETURN;
}
...
bool Format_description_log_event::write(IO_CACHE *file)
{
  ...
  */
  ...
  memcpy((char *)buff + ST_COMMON_HEADER_LEN_OFFSET + 1, (byte *)post_header_len, LOG_EVENT_TYPES);
  return (write_header(file, sizeof(buff)) || my_b_safe_write(file, buff, sizeof(buff)));
}
#endif
...
void Load_log_event::print_query(bool need_db, char *buf, char **end, char **fn_start, char **fn_end)
{
  char *pos = buf;
...
bool Load_log_event::write_data_body(IO_CACHE *file)
{
  ...
  return (my_b_safe_write(file, (byte *)table_name, table_name_len + 1) ||
          my_b_safe_write(file, (byte *)db, db_len + 1) || my_b_safe_write(file, (byte *)fname, fname_len));
}
...
Load_log_event::Load_log_event(THD *thd_arg, sql_exchange *ex, const char *db_arg, const char *table_name_arg,
                               List<Item> &fields_arg, enum enum_duplicates handle_dup, bool ignore, bool using_trans)
...
Load_log_event::Load_log_event(const char *buf, uint event_len, const Format_description_log_event *description_event)
    : Log_event(buf, description_event),
      num_fields(0),
      fields(0),
      field_lens(0),
  ...
  */
  if (event_len)
    copy_log_event(buf, event_len,
                   ((buf[EVENT_TYPE_OFFSET] == LOAD_EVENT) ? LOAD_HEADER_LEN + description_event->common_header_len
...
int Load_log_event::copy_log_event(const char *buf, ulong event_len, int body_offset,
                                   const Format_description_log_event *description_event)
{
  ...
  */
  if (!(field_lens = (uchar *)sql_ex.init((char *)buf + body_offset, buf_end, buf[EVENT_TYPE_OFFSET] != LOAD_EVENT)))
    DBUG_RETURN(1);

  data_len = event_len - body_offset;
  if (num_fields > data_len)  // simple sanity check against corruption
    DBUG_RETURN(1);
  for (uint i = 0; i < num_fields; i++) field_block_len += (uint)field_lens[i] + 1;
...
void Load_log_event::print(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info, bool commented)
{
  ...
  if (db)
  {
    /*
      ...
      update the last_db.
    */
    if ((different_db = memcmp(last_event_info->db, db, db_len + 1)) && !commented)
...
#ifndef MYSQL_CLIENT
void Load_log_event::set_fields(const char *affected_db, List<Item> &field_list, Name_resolution_context *context)
{
  uint i;
  const char *field = fields;
  for (i = 0; i < num_fields; i++)
  {
    field_list.push_back(new Item_field(context, affected_db, table_name, field));
    field += field_lens[i] + 1;
  }
}
#endif /* !MYSQL_CLIENT */
...
int Load_log_event::exec_event(NET *net, struct st_relay_log_info *rli, bool use_rli_only_for_errors)
{
  thd->db_length = db_len;
  thd->db = (char *)rewrite_db(db, &thd->db_length);
    ...
    */
    ...
    else
    {
      ...
      */
      ...
      print_query(FALSE, load_data_query, &end, (char **)&thd->lex->fname_start, (char **)&thd->lex->fname_end);
      *end = 0;
      ...
      */
      if (mysql_load(thd, &ex, &tables, field_list, set_fields, set_fields, handle_dup, ignore, net != 0))
        thd->query_error = 1;
      if (thd->cuted_fields)
      {
...
#ifndef MYSQL_CLIENT
Rotate_log_event::Rotate_log_event(THD *thd_arg, const char *new_log_ident_arg, uint ident_len_arg, ulonglong pos_arg,
                                   uint flags_arg)
    : Log_event(),
    ...
    : Log_event(buf, description_event), new_log_ident(0), flags(DUP_NAME)
{
  ...
  new_log_ident = my_strdup_with_length((byte *)buf + ident_offset, (uint)ident_len, MYF(MY_WME));
  DBUG_VOID_RETURN;
}
...
bool Rotate_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, ROTATE_HEADER_LEN + ident_len) || my_b_safe_write(file, (byte *)buf, ROTATE_HEADER_LEN) ||
...
%lu",
    ...
    */
    ...
    thd->variables.auto_increment_increment = thd->variables.auto_increment_offset = 1;
  }
...
Intvar_log_event::Intvar_log_event(const char *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  buf += description_event->common_header_len;
...
bool Intvar_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, sizeof(buf)) || my_b_safe_write(file, buf, sizeof(buf)));
}
#endif
...
Rand_log_event::Rand_log_event(const char *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  buf += description_event->common_header_len;
...
Xid_log_event::Xid_log_event(const char *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  buf += description_event->common_header_len;
  ...
  else
  {
    switch (type)
    {
      case REAL_RESULT:
        ...
        event_len += my_sprintf(buf + val_offset, (buf + val_offset, "%.14g", real_val));
        break;
      case INT_RESULT:
...
User_var_log_event::User_var_log_event(const char *buf, const Format_description_log_event *description_event)
    : Log_event(buf, description_event)
{
  buf += description_event->common_header_len;
  ...
  else
  {
    ...
    val_len = uint4korr(buf + UV_VAL_IS_NULL + UV_VAL_TYPE_SIZE + UV_CHARSET_NUMBER_SIZE);
    val = (char *)(buf + UV_VAL_IS_NULL + UV_VAL_TYPE_SIZE + UV_CHARSET_NUMBER_SIZE + UV_VAL_LEN_SIZE);
  }
}
...
bool User_var_log_event::write(IO_CACHE *file)
{
  ...
  char buf1[UV_VAL_IS_NULL + UV_VAL_TYPE_SIZE + UV_CHARSET_NUMBER_SIZE + UV_VAL_LEN_SIZE];
  ...
  return (write_header(file, event_length) || my_b_safe_write(file, (byte *)buf, sizeof(buf)) ||
          my_b_safe_write(file, (byte *)name, name_len) || my_b_safe_write(file, (byte *)buf1, buf1_length) ||
...
#ifndef MYSQL_CLIENT
Slave_log_event::Slave_log_event(THD *thd_arg, struct st_relay_log_info *rli)
    : Log_event(thd_arg, 0, 0), mem_pool(0), master_host(0)
{
  ...
  if ((mem_pool = (char *)my_malloc(get_data_size() + 1, MYF(MY_WME))))
  {
    ...
    DBUG_PRINT("info", ("master_log: %s  pos: %d", master_log, (ulong)master_pos));
  }
  else
...
Slave_log_event::~Slave_log_event() { my_free(mem_pool, MYF(MY_ALLOW_ZERO_PTR)); }
...
int Slave_log_event::get_data_size() { return master_host_len + master_log_len + 1 + SL_MASTER_HOST_OFFSET; }
...
bool Slave_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, event_length) || my_b_safe_write(file, (byte *)mem_pool, event_length));
}
#endif
...
#ifndef MYSQL_CLIENT
Create_file_log_event::Create_file_log_event(THD *thd_arg, sql_exchange *ex, const char *db_arg,
                                             const char *table_name_arg, List<Item> &fields_arg,
                                             enum enum_duplicates handle_dup, bool ignore, char *block_arg,
                                             uint block_len_arg, bool using_trans)
    : Load_log_event(thd_arg, ex, db_arg, table_name_arg, fields_arg, handle_dup, ignore, using_trans),
      fake_base(0),
      block(block_arg),
      event_buf(0),
      ...
      copy_log_event(event_buf, len,
                     ((buf[EVENT_TYPE_OFFSET] == LOAD_EVENT)
                          ...
                          : (fake_base ? (header_len + load_header_len)
                                       : (header_len + load_header_len) + create_file_header_len)),
                     description_event))
    DBUG_VOID_RETURN;
  if (description_event->binlog_version != 1)
  {
    file_id = uint4korr(buf + header_len + load_header_len + CF_FILE_ID_OFFSET);
    /*
...
#ifdef MYSQL_CLIENT
void Create_file_log_event::print(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info, bool enable_local)
{
  if (short_form)
  {
...
void Create_file_log_event::print(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info)
{
  print(file, short_form, last_event_info, 0);
}
#endif /* MYSQL_CLIENT */
...
int Create_file_log_event::exec_event(struct st_relay_log_info *rli)
{
  ...
  if ((fd = my_create(fname_buf, CREATE_MODE, O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW, MYF(MY_WME))) < 0 ||
      init_io_cache(&file, fd, IO_SIZE, WRITE_CACHE, (my_off_t)0, 0, MYF(MY_WME | MY_NABP)))
  {
...
#ifndef MYSQL_CLIENT
Append_block_log_event::Append_block_log_event(THD *thd_arg, const char *db_arg, char *block_arg, uint block_len_arg,
                                               bool using_trans)
...
bool Append_block_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, APPEND_BLOCK_HEADER_LEN + block_len) ||
          my_b_safe_write(file, buf, APPEND_BLOCK_HEADER_LEN) || my_b_safe_write(file, (byte *)block, block_len));
}
#endif
...
void Append_block_log_event::pack_info(Protocol *protocol)
{
  char buf[256];
  uint length;
  length = (uint)my_sprintf(buf, (buf, ";file_id=%u;block_len=%u", file_id, block_len));
...
int Append_block_log_event::exec_event(struct st_relay_log_info *rli)
{
  ...
  if (get_create_or_append())
  {
    ...
    if ((fd = my_create(fname, CREATE_MODE, O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW, MYF(MY_WME))) < 0)
    {
      slave_print_error(rli, my_errno, "Error in %s event: could not create file '%s'", get_type_str(), fname);
      goto err;
    }
  }
  else if ((fd = my_open(fname, O_WRONLY | O_APPEND | O_BINARY | O_NOFOLLOW, MYF(MY_WME))) < 0)
  {
    slave_print_error(rli, my_errno, "Error in %s event: could not open file '%s'", get_type_str(), fname);
    goto err;
  }
  if (my_write(fd, (byte *)block, block_len, MYF(MY_WME + MY_NABP)))
  {
    slave_print_error(rli, my_errno, "Error in %s event: write to '%s' failed", get_type_str(), fname);
    goto err;
  }
  error = 0;
...
#ifndef MYSQL_CLIENT
Delete_file_log_event::Delete_file_log_event(THD *thd_arg, const char *db_arg, bool using_trans)
...
#ifndef MYSQL_CLIENT
Execute_load_log_event::Execute_load_log_event(THD *thd_arg, const char *db_arg, bool using_trans)
    : Log_event(thd_arg, 0, using_trans), file_id(thd_arg->file_id), db(db_arg)
{
}
#endif
...
int Execute_load_log_event::exec_event(struct st_relay_log_info *rli)
{
  ...
  if ((fd = my_open(fname, O_RDONLY | O_BINARY | O_NOFOLLOW, MYF(MY_WME))) < 0 ||
      init_io_cache(&file, fd, IO_SIZE, READ_CACHE, (my_off_t)0, 0, MYF(MY_WME | MY_NABP)))
  {
    slave_print_error(rli, my_errno, "Error in Exec_load event: could not open file '%s'", fname);
    goto err;
  }
  if (!(lev = (Load_log_event *)Log_event::read_log_event(&file, (pthread_mutex_t *)0,
  ...
  */
  ...
  if (lev->exec_event(0, rli, 1))
  {
    ...
    */
    ...
    if (tmp)
    {
      slave_print_error(rli, rli->last_slave_errno, /* ok to re-use error code */
...
#ifndef MYSQL_CLIENT
Begin_load_query_log_event::Begin_load_query_log_event(THD *thd_arg, const char *db_arg, char *block_arg,
                                                       uint block_len_arg, bool using_trans)
    : Append_block_log_event(thd_arg, db_arg, block_arg, block_len_arg, using_trans)
{
  file_id = thd_arg->file_id = mysql_bin_log.next_file_id();
}
#endif

Begin_load_query_log_event::Begin_load_query_log_event(const char *buf, uint len,
...
#ifndef MYSQL_CLIENT
Execute_load_query_log_event::Execute_load_query_log_event(THD *thd_arg, const char *query_arg, ulong query_length_arg,
                                                           uint fn_pos_start_arg, uint fn_pos_end_arg,
                                                           enum_load_dup_handling dup_handling_arg, bool using_trans,
                                                           bool suppress_use)
    : Query_log_event(thd_arg, query_arg, query_length_arg, using_trans, suppress_use),
      file_id(thd_arg->file_id),
...
Execute_load_query_log_event::Execute_load_query_log_event(const char *buf, uint event_len,
                                                           const Format_description_log_event *desc_event)
...
ulong Execute_load_query_log_event::get_post_header_size_for_derived() { return EXECUTE_LOAD_QUERY_EXTRA_HEADER_LEN; }

#ifndef MYSQL_CLIENT
bool Execute_load_query_log_event::write_post_header_for_derived(IO_CACHE *file)
{
...
#ifdef MYSQL_CLIENT
void Execute_load_query_log_event::print(FILE *file, bool short_form, LAST_EVENT_INFO *last_event_info)
{
  print(file, short_form, last_event_info, 0);
}
...
int Execute_load_query_log_event::exec_event(struct st_relay_log_info *rli)
{
  char *p;
  char *buf;
  char *fname;
  ...
  if (!(buf = my_malloc(q_len + 1 - (fn_pos_end - fn_pos_start) + (FN_REFLEN + 10) + 10 + 8 + 5, MYF(MY_WME))))
  {
...
bool sql_ex_info::write_data(IO_CACHE *file)
{
  if (new_format())
  {
    return (write_str(file, field_term, (uint)field_term_len) || write_str(file, enclosed, (uint)enclosed_len) ||
            write_str(file, line_term, (uint)line_term_len) || write_str(file, line_start, (uint)line_start_len) ||
            write_str(file, escaped, (uint)escaped_len) || my_b_safe_write(file, (byte *)&opt_flags, 1));
  }
  else
  {
    old_sql_ex old_ex;
...
char *sql_ex_info::init(char *buf, char *buf_end, bool use_new_format)
{
  ...
  if (use_new_format)
  {
    ...
    */
    if (read_str(&buf, buf_end, &field_term, &field_term_len) || read_str(&buf, buf_end, &enclosed, &enclosed_len) ||
        read_str(&buf, buf_end, &line_term, &line_term_len) || read_str(&buf, buf_end, &line_start, &line_start_len) ||
