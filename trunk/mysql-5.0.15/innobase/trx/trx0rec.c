    ...
    mtr_t *mtr)           /* in: mtr */
{
  ...
  field = rec_get_nth_field(rec, offsets, dict_index_get_sys_col_pos(index, DATA_TRX_ID), &len);
  ut_ad(len == DATA_TRX_ID_LEN);
  trx_id = trx_read_trx_id(field);
  field = rec_get_nth_field(rec, offsets, dict_index_get_sys_col_pos(index, DATA_ROLL_PTR), &len);
    ...
    upd_t **upd)         /* out, own: update vector */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    {
      fprintf(stderr, "InnoDB: Error: trying to access update undo rec field %lu in ", (ulong)field_no);
                           ...
                           flag was specified */
{
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
  ...
  for (;;)
  {
    undo_page = buf_page_get_gen(undo->space, page_no, RW_X_LATCH, undo->guess_page, BUF_GET, __FILE__, __LINE__, &mtr);
                        ...
                        history data has been deleted */
{
  ...
  *old_vers = NULL;
  ...
  ptr = trx_undo_update_rec_get_update(ptr, index, type, trx_id, roll_ptr, info_bits, NULL, heap, &update);
  ...
  if (ptr == NULL)
  {
    ...
    fprintf(stderr,
            ...
            index->table_name, index->name, (ulong)dict_index_get_n_unique(index), undo_rec, (ulong)type,
            (ulong)cmpl_info, (ulong)ut_dulint_get_high(table_id), (ulong)ut_dulint_get_low(table_id),
            (ulong)ut_dulint_get_high(index->table->id), (ulong)ut_dulint_get_low(index->table->id));
    ...
    fprintf(stderr,
            ...
            (ulong)ut_dulint_get_high(old_roll_ptr), (ulong)ut_dulint_get_low(old_roll_ptr),
            (ulong)ut_dulint_get_high(roll_ptr), (ulong)ut_dulint_get_low(roll_ptr));
  ...
  if (row_upd_changes_field_size_or_external(index, offsets, update))
  {
    ...
    ext_vect = mem_alloc(sizeof(ulint) * rec_offs_n_fields(offsets));
    ...
    *old_vers = rec_convert_dtuple_to_rec(buf, index, entry);
    ...
    rec_set_field_extern_bits(*old_vers, index, ext_vect, n_ext_vect, NULL);
    mem_free(ext_vect);
  }
  else
  {
