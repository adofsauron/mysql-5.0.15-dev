...
int decimal_operation_results(int result)
{
  switch (result)
  {
    ...
    case E_DEC_TRUNCATED:
      push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, ER(WARN_DATA_TRUNCATED), "",
                          (long)-1);
      break;
    case E_DEC_OVERFLOW:
      push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_TRUNCATED_WRONG_VALUE,
                          ER(ER_TRUNCATED_WRONG_VALUE), "DECIMAL", "");
      break;
    case E_DEC_DIV_ZERO:
      push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_DIVISION_BY_ZERO, ER(ER_DIVISION_BY_ZERO));
      break;
    case E_DEC_BAD_NUM:
      push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                          ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "decimal", "", "", (long)-1);
      break;
    case E_DEC_OOM:
...
%09d,%09d,%09d,%09d,%09d,%09d,%09d,%09d\n",
          dec->sign(), dec->intg, dec->frac, dec->buf[0], dec->buf[1], dec->buf[2], dec->buf[3], dec->buf[4],
          dec->buf[5], dec->buf[6], dec->buf[7]);
}
