...
TYPELIB rpl_role_typelib = {array_elements(rpl_role_type) - 1, "", rpl_role_type, NULL};

const char *rpl_status_type[] = {"AUTH_MASTER",   "ACTIVE_SLAVE",     "IDLE_SLAVE", "LOST_SOLDIER",
                                 "TROOP_SOLDIER", "RECOVERY_CAPTAIN", "NULL",       NullS};
TYPELIB rpl_status_typelib = {array_elements(rpl_status_type) - 1, "", rpl_status_type, NULL};

static Slave_log_event *find_slave_event(IO_CACHE *log, const char *log_file_name, char *errmsg);
...
{
  return cmp_master_pos(sev->master_log, sev->master_pos, mi->log_file_name, mi->pos);
}
...
err:
  ...
  my_message(ER_UNKNOWN_ERROR, "Wrong parameters to function register_slave", MYF(0));
err2:
  return 1;
}

extern "C" uint32 *slave_list_key(SLAVE_INFO *si, uint *len, my_bool not_used __attribute__((unused)))
{
  *len = 4;
  return &si->server_id;
}

extern "C" void slave_info_free(void *s) { my_free((gptr)s, MYF(MY_WME)); }

void init_slave_list()
{
  hash_init(&slave_list, system_charset_info, SLAVE_LIST_CHUNK, 0, 0, (hash_get_key)slave_list_key,
            (hash_free_key)slave_info_free, 0);
...
int translate_master(THD *thd, LEX_MASTER_INFO *mi, char *errmsg)
{
  ...
  for (;;)
  {
    ...
    else if (cmp_res > 0)
    {
      if (!last_pos)
      {
        strmov(errmsg, "Slave event in first log points past the target position");
        goto err;
      }
      end_io_cache(&log);
      (void)my_close(file, MYF(MY_WME));
      if (init_io_cache(&log, (file = last_file), IO_SIZE, READ_CACHE, 0, 0, MYF(MY_WME)))
      {
        errmsg[0] = 0;
        goto err;
      }
...
static Slave_log_event *find_slave_event(IO_CACHE *log, const char *log_file_name, char *errmsg)
{
  ...
  for (i = 0; i < 2; i++)
  {
    if (!(ev = Log_event::read_log_event(log, (pthread_mutex_t *)0, 0)))
    {
      my_snprintf(errmsg, SLAVE_ERRMSG_SIZE, "Error reading event in log '%s'", (char *)log_file_name);
      return 0;
    }
  ...
  {
    my_snprintf(errmsg, SLAVE_ERRMSG_SIZE, "Could not find slave event in log '%s'", (char *)log_file_name);
    return 0;
  }
...
int update_slave_list(MYSQL *mysql, MASTER_INFO *mi)
{
  ...
  if (mysql_real_query(mysql, "SHOW SLAVE HOSTS", 16) || !(res = mysql_store_result(mysql)))
  {
...
HOSTS";
  ...
  while ((row = mysql_fetch_row(res)))
  {
    ...
    if ((old_si = (SLAVE_INFO *)hash_search(&slave_list, (byte *)&server_id, 4)))
      si = old_si;
    else
    {
      if (!(si = (SLAVE_INFO *)my_malloc(sizeof(SLAVE_INFO), MYF(MY_WME))))
...
pthread_handler_t handle_failsafe_rpl(void *arg)
{
  ...
  pthread_mutex_lock(&LOCK_rpl_status);
  msg = thd->enter_cond(&COND_rpl_status, &LOCK_rpl_status, "Waiting for request");
...
static inline void cleanup_mysql_results(MYSQL_RES *db_res, MYSQL_RES **cur, MYSQL_RES **start)
{
  for (; cur >= start; --cur)
  {
    if (*cur)
...
static int fetch_db_tables(THD *thd, MYSQL *mysql, const char *db, MYSQL_RES *table_res, MASTER_INFO *mi)
{
  MYSQL_ROW row;
  for (row = mysql_fetch_row(table_res); row; row = mysql_fetch_row(table_res))
  {
    TABLE_LIST table;
...
bool load_master_data(THD *thd)
{
  ...
  */
  ...
  if (restart_thread_mask && (error = terminate_slave_threads(active_mi, restart_thread_mask, 1 /*skip lock*/)))
  {
  ...
  // now that we are connected, get all database and tables in each
  {
    ...
    if (mysql_real_query(&mysql, "SHOW DATABASES", 14) || !(db_res = mysql_store_result(&mysql)))
    {
    ...
    */
    if (mysql_real_query(&mysql, "FLUSH TABLES WITH READ LOCK", 27) ||
        mysql_real_query(&mysql, "SHOW MASTER STATUS", 18) || !(master_status_res = mysql_store_result(&mysql)))
    {
      my_error(error = ER_QUERY_ON_MASTER, MYF(0), mysql_error(&mysql));
      goto err;
    }
    ...
    */
    ...
    for (cur_table_res = table_res; cur_table_res < table_res_end; cur_table_res++)
    {
      ...
      */
      ...
      if (!db_ok(db, replicate_do_db, replicate_ignore_db) || !db_ok_with_wild_table(db) || !strcmp(db, "mysql"))
      {
        *cur_table_res = 0;
        continue;
      }
      ...
      if (mysql_select_db(&mysql, db) || mysql_real_query(&mysql, "SHOW TABLES", 11) ||
      ...
      */
      if (row && row[0] && row[1])
      {
        ...
        */
        ...
        if (init_master_info(active_mi, master_info_file, relay_log_info_file, 0, (SLAVE_IO | SLAVE_SQL)))
  ...
  if (purge_relay_logs(&active_mi->rli, thd, 0 /* not only reset, but also reinit */, &errmsg))
  {
  ...
  */
  ...
  {
    error = start_slave_threads(0 /* mutex not needed */, 1 /* wait for start */, active_mi, master_info_file,
                                relay_log_info_file, restart_thread_mask);
  }
