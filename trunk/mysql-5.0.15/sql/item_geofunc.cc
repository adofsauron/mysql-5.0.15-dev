...
String *Item_func_as_wkt::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || !(geom = Geometry::create_from_wkb(&buffer, swkb->ptr() + SRID_SIZE,
...
void Item_func_as_wkt::fix_length_and_dec() { max_length = MAX_BLOB_WIDTH; }
...
String *Item_func_as_wkb::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value ||
                     !(Geometry::create_from_wkb(&buffer, swkb->ptr() + SRID_SIZE, swkb->length() - SRID_SIZE)))))
    return 0;

  str->copy(swkb->ptr() + SRID_SIZE, swkb->length() - SRID_SIZE, &my_charset_bin);
  return str;
}
...
String *Item_func_geometry_type::val_str(String *str)
{
  ...
  str->copy(geom->get_class_info()->m_name.str, geom->get_class_info()->m_name.length, default_charset());
  return str;
}
...
String *Item_func_envelope::val_str(String *str)
{
  ...
  if ((null_value = args[0]->null_value ||
                    !(geom = Geometry::create_from_wkb(&buffer, swkb->ptr() + SRID_SIZE, swkb->length() - SRID_SIZE))))
    return 0;
...
String *Item_func_point::val_str(String *str)
{
  ...
  if ((null_value = (args[0]->null_value || args[1]->null_value || str->realloc(1 + 4 + SIZEOF_STORED_DOUBLE * 2))))
    return 0;
...
longlong Item_func_spatial_rel::val_int()
{
  ...
  if ((null_value = (args[0]->null_value || args[1]->null_value ||
                     !(g1 = Geometry::create_from_wkb(&buffer1, res1->ptr() + SRID_SIZE, res1->length() - SRID_SIZE)) ||
                     !(g2 = Geometry::create_from_wkb(&buffer2, res2->ptr() + SRID_SIZE, res2->length() - SRID_SIZE)) ||
                     g1->get_mbr(&mbr1, &dummy) || g2->get_mbr(&mbr2, &dummy))))
    return 0;
...
longlong Item_func_isclosed::val_int()
{
  ...
  null_value = (!swkb || args[0]->null_value ||
                !(geom = Geometry::create_from_wkb(&buffer, swkb->ptr() + SRID_SIZE, swkb->length() - SRID_SIZE)) ||
...
longlong Item_func_numinteriorring::val_int()
{
  ...
  null_value =
      (!swkb || !(geom = Geometry::create_from_wkb(&buffer, swkb->ptr() + SRID_SIZE, swkb->length() - SRID_SIZE)) ||
