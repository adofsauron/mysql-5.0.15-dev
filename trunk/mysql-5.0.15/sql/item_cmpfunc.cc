...
int Arg_comparator::set_compare_func(Item_bool_func2 *item, Item_result type) {
  owner = item;
  func =
      comparator_matrix[type][test(owner->functype() == Item_func::EQUAL_FUNC)];
  switch (type) {
  case ROW_RESULT: {
    uint n = (*a)->cols();
    if (n != (*b)->cols()) {
...
void Item_func_interval::fix_length_and_dec() {
  ...
  if (row->cols() > 8) {
    ...
    if (consts && (intervals = (interval_range *)sql_alloc(
                       sizeof(interval_range) * (row->cols() - 1)))) {
...
uint Item_func_ifnull::decimal_precision() const {
  int max_int_part =
      max(args[0]->decimal_int_part(), args[1]->decimal_int_part());
...
uint Item_func_if::decimal_precision() const {
  int precision =
      (max(args[1]->decimal_int_part(), args[2]->decimal_int_part()) +
       decimals);
...
bool Item_func_case::fix_fields(THD *thd, Item **ref) {
  ...
  */
  char buff[MAX_FIELD_WIDTH * 2 + sizeof(String) * 2 + sizeof(String *) * 2 +
  ...
  if ((comparators = (cmp_item **)sql_alloc(sizeof(cmp_item *) * n))) {
    ...
    for (uint i = 0; i < n; i++) {
      if (!(comparators[i] = tmpl->comparators[i]->make_same()))
        break; // new failed
      comparators[i]
          ->store_value_by_template(tmpl->comparators[i], item->el(i));
      item->null_value |= item->el(i)->null_value;
    }
  }
}
...
table_map Item_cond::used_tables() const { // This caches used_tables
    ...
    */
    if (args[1]->const_item() && !use_strnxfrm(collation.collation) &&
      ...
      */
      ...
      if (len > MIN_TURBOBM_PATTERN_LEN + 2 && *first == wild_many &&
          *last == wild_many) {
      ...
      if (canDoTurboBM) {
        ...
        int *suff = (int *)thd->alloc(
            (int)(sizeof(int) * ((pattern_len + 1) * 2 + alphabet_size)));
...
bool Item_func_regex::fix_fields(THD *thd, Item **ref) {
  ...
  if ((!args[0]->fixed && args[0]->fix_fields(thd, args)) ||
      args[0]->check_cols(1) ||
      (!args[1]->fixed && args[1]->fix_fields(thd, args + 1)) ||
      args[1]->check_cols(1))
    return TRUE; /* purecov: inspected */
  ...
  if (!regex_compiled && args[1]->const_item()) {
    ...
    if (args[1]->null_value) { // Will always return NULL
      maybe_null = 1;
      return FALSE;
    }
    int error;
...
void Item_equal::fix_length_and_dec() {
  ...
  eval_item =
      cmp_item::get_comparator(item->result_type(), item->collation.collation);
