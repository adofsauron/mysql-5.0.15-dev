...
ha_archive::ha_archive(TABLE *table_arg) : handler(&archive_hton, table_arg), delayed_insert(0), bulk_insert(0)
{
...
int ha_archive::read_data_header(gzFile file_to_read)
{
  ...
  if ((data_buffer[0] != (uchar)ARCHIVE_CHECK_HEADER) && (data_buffer[1] != (uchar)ARCHIVE_VERSION))
    DBUG_RETURN(HA_ERR_CRASHED_ON_USAGE);
...
int ha_archive::read_meta_file(File meta_file, ha_rows *rows)
{
  ...
  *rows = (ha_rows)uint8korr(meta_buffer + 2);
  ...
  if ((meta_buffer[0] != (uchar)ARCHIVE_CHECK_HEADER) || ((bool)meta_buffer[18] == TRUE))
    DBUG_RETURN(HA_ERR_CRASHED_ON_USAGE);
...
ARCHIVE_SHARE *ha_archive::get_share(const char *table_name, TABLE *table)
{
  ...
  if (!(share = (ARCHIVE_SHARE *)hash_search(&archive_open_tables, (byte *)table_name, length)))
  {
...
*/
static const char *ha_archive_exts[] = {ARZ, ARN, ARM, NullS};

const char **ha_archive::bas_ext() const { return ha_archive_exts; }
...
int ha_archive::create(const char *name, TABLE *table_arg, HA_CREATE_INFO *create_info)
{
  ...
  if ((create_file = my_create(fn_format(name_buff, name, "", ARM, MY_REPLACE_EXT | MY_UNPACK_FILENAME), 0,
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    while ((read = gzread(archive, block, IO_SIZE))) gzwrite(writer, block, read);
  }
...
*/
THR_LOCK_DATA **ha_archive::store_lock(THD *thd, THR_LOCK_DATA **to, enum thr_lock_type lock_type)
{
