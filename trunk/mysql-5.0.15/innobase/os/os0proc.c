...
prints error info to stderr if no success */
{
  ...
  if (Result != TRUE)
  {
    fprintf(stderr, "InnoDB: AWE: Cannot open process token, error %lu\n", (ulint)GetLastError());
    return (FALSE);
  }
  ...
  if (Result != TRUE)
  {
    fprintf(stderr, "InnoDB: AWE: Cannot get local privilege value for %s, error %lu.\n", SE_LOCK_MEMORY_NAME,
            (ulint)GetLastError());
  ...
  if (Result != TRUE)
  {
    fprintf(stderr, "InnoDB: AWE: Cannot adjust process token privileges, error %u.\n", GetLastError());
    return (FALSE);
  }
...
#else
#ifdef __WIN__
  fprintf(stderr, "InnoDB: AWE: Error: to use AWE you must use a ...-nt MySQL executable.\n");
#endif
  return (FALSE);
#endif
}
...
ibool os_awe_allocate_physical_mem(
    /*=========================*/
    /* out: TRUE if success */
...
{
#ifdef UNIV_SIMULATE_AWE
  os_awe_simulate_page_info = ut_malloc(sizeof(os_awe_t) * n_megabytes * ((1024 * 1024) / OS_AWE_X86_PAGE_SIZE));

  os_awe_simulate_mem = ut_align(ut_malloc(4096 + 1024 * 1024 * n_megabytes), 4096);
  ...
  *page_info = (os_awe_t *)HeapAlloc(GetProcessHeap(), 0, PFNArraySize);
  ...
  if (*page_info == NULL)
  {
    fprintf(stderr, "InnoDB: AWE: Failed to allocate page info array from process heap, error %lu\n",
            (ulint)GetLastError());
  ...
  bResult = AllocateUserPhysicalPages(GetCurrentProcess(), &NumberOfPages, *page_info);
  if (bResult != TRUE)
  {
    fprintf(stderr, "InnoDB: AWE: Cannot allocate physical pages, error %lu.\n", (ulint)GetLastError());
  ...
  fprintf(stderr, "InnoDB: Using Address Windowing Extensions (AWE); allocated %lu MB\n", n_megabytes);
...
byte *os_awe_allocate_virtual_mem_window(
    /*===============================*/
                ...
                be < 2 GB */
{
  ...
  ptr = VirtualAlloc(NULL, (SIZE_T)size, MEM_RESERVE | MEM_PHYSICAL, PAGE_READWRITE);
  if (ptr == NULL)
  {
    fprintf(stderr, "InnoDB: AWE: Cannot allocate %lu bytes of virtual memory, error %lu\n", size,
            (ulint)GetLastError());
...
ibool os_awe_map_physical_mem_to_window(
    /*==============================*/
                         ...
                         array */
{
  ...
  ut_a(page_info < os_awe_simulate_page_info + (os_awe_simulate_mem_size / 4096));

  /* First look if some other 'physical pages' are mapped at ptr,
  and copy them back to where they were if yes */

  map = os_awe_simulate_map + ((ulint)(ptr - os_awe_simulate_window)) / 4096;
  page = ptr;
  ...
  phys_page = os_awe_simulate_mem + (ulint)(page_info - os_awe_simulate_page_info) * 4096;
  ...
  {
    fprintf(stderr, "InnoDB: AWE: Error: trying to map to address %lx but AWE window start %lx\n", (ulint)ptr,
            (ulint)os_awe_window);
    ut_a(0);
  }
  ...
  {
    fprintf(stderr, "InnoDB: AWE: Error: trying to map to address %lx but AWE window end %lx\n", (ulint)ptr,
            (ulint)os_awe_window + os_awe_window_size);
    ut_a(0);
  }

  if (!(page_info >= os_awe_page_info))
  {
    fprintf(stderr, "InnoDB: AWE: Error: trying to map page info at %lx but array start %lx\n", (ulint)page_info,
            (ulint)os_awe_page_info);
    ut_a(0);
  }

  if (!(page_info <= os_awe_page_info + (os_awe_n_pages - 4)))
  {
    fprintf(stderr, "InnoDB: AWE: Error: trying to map page info at %lx but array end %lx\n", (ulint)page_info,
    ...
    ulint n) /* in: number of bytes */
{
  ...
  ptr = VirtualAlloc(NULL, n, MEM_COMMIT, PAGE_READWRITE | PAGE_NOCACHE);
  ut_a(ptr);
                          ...
                          cannot be allocated */
{
  ...
  {
    fprintf(stderr,
            "InnoDB: HugeTLB: Warning: Failed to allocate %lu bytes. "
            "errno %d\n",
            n, errno);
  }
  else
  {
    ...
    {
      fprintf(stderr,
              "InnoDB: HugeTLB: Warning: Failed to attach shared memory "
              "segment, errno %d\n",
              errno);
    }
    /*
...
void os_process_set_priority_boost(
    /*==========================*/
