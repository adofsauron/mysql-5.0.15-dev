...
static int copy_data_between_tables(TABLE *from, TABLE *to,
                                    ...
                                    bool ignore, uint order_num, ORDER *order,
...
int mysql_rm_table_part2_with_lock(THD *thd, TABLE_LIST *tables, bool if_exists,
...
int mysql_rm_table_part2(THD *thd, TABLE_LIST *tables, bool if_exists,
  ...
  for (table = tables; table; table = table->next_local) {
    ...
    if (drop_temporary ||
        (access(path, F_OK) && ha_create_table_from_engine(thd, db, alias)) ||
...
static int sort_keys(KEY *a, KEY *b) {
  ...
  */
  return ((a->usable_key_parts < b->usable_key_parts)
              ? -1
              : (a->usable_key_parts > b->usable_key_parts) ? 1 : 0);
}
...
void check_duplicates_in_interval(const char *set_or_name, const char *name,
                                  TYPELIB *typelib, CHARSET_INFO *cs) {
  TYPELIB tmp = *typelib;
  ...
  for (; tmp.count > 1; cur_value++, cur_length++) {
    ...
    if (find_type2(&tmp, (const char *)*cur_value, *cur_length, cs)) {
      push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_NOTE,
                          ...
                          ER(ER_DUPLICATED_VALUE_IN_TYPE), name, *cur_value,
                          set_or_name);
    }
  }
}
...
void calculate_interval_lengths(CHARSET_INFO *cs, TYPELIB *interval,
  ...
  *max_length = *tot_length = 0;
  for (pos = interval->type_names, len = interval->type_lengths; *pos;
       pos++, len++) {
...
int prepare_create_field(create_field *sql_field, uint *blob_columns,
  ...
  */
  ...
  case FIELD_TYPE_LONG_BLOB:
    sql_field->pack_flag =
        FIELDFLAG_BLOB | pack_length_to_packflag(sql_field->pack_length -
  ...
  case FIELD_TYPE_GEOMETRY:
    ...
    sql_field->pack_flag =
        FIELDFLAG_GEOM | pack_length_to_packflag(sql_field->pack_length -
                                                 portable_sizeof_char_ptr);
    if (sql_field->charset->state & MY_CS_BINSORT)
      sql_field->pack_flag |= FIELDFLAG_BINARY;
    sql_field->length = 8; // Unireg field length
  ...
  case FIELD_TYPE_ENUM:
    sql_field->pack_flag =
        pack_length_to_packflag(sql_field->pack_length) | FIELDFLAG_INTERVAL;
    if (sql_field->charset->state & MY_CS_BINSORT)
      sql_field->pack_flag |= FIELDFLAG_BINARY;
  ...
  case FIELD_TYPE_SET:
    sql_field->pack_flag =
        pack_length_to_packflag(sql_field->pack_length) | FIELDFLAG_BITFIELD;
    if (sql_field->charset->state & MY_CS_BINSORT)
      sql_field->pack_flag |= FIELDFLAG_BINARY;
  ...
  case FIELD_TYPE_NEWDECIMAL:
    sql_field->pack_flag =
        (FIELDFLAG_NUMBER |
         (sql_field->flags & UNSIGNED_FLAG ? 0 : FIELDFLAG_DECIMAL) |
...
static int mysql_prepare_table(THD *thd, HA_CREATE_INFO *create_info,
                               ...
                               bool tmp_table, uint *db_options, handler *file,
      ...
      if (!interval) {
        ...
        for (uint i = 0; (tmp = it++); i++) {
          ...
          if (String::needs_conversion(tmp->length(), tmp->charset(), cs,
            ...
            interval->type_names[i] =
                strmake_root(thd->mem_root, conv.ptr(), conv.length());
      ...
      */
      ...
      if (sql_field->sql_type == FIELD_TYPE_SET) {
        ...
        if (sql_field->def) {
          ...
          (void)find_set(interval, def->ptr(), def->length(), cs, &not_used,
                         &not_used2, &not_found);
          if (not_found) {
            my_error(ER_INVALID_DEFAULT, MYF(0), sql_field->field_name);
            DBUG_RETURN(-1);
  ...
  while ((sql_field = it++)) {
    ...
    if (prepare_create_field(sql_field, &blob_columns, &timestamps,
                             &timestamps_with_niladic, file->table_flags()))
      DBUG_RETURN(-1);
  ...
  *key_count = 0;
  ...
  while ((key = key_iterator++)) {
    ...
    if (key->type != Key::FOREIGN_KEY) {
      while ((key2 = key_iterator2++) != key) {
        ...
        */
        if ((key2->type != Key::FOREIGN_KEY && key2->name != ignore_key &&
             !foreign_key_prefix(key, key2))) {
          /* TODO: issue warning message */
          /* mark that the generated key should be ignored */
          if (!key2->generated ||
              (key->generated &&
               key->columns.elements < key2->columns.elements))
            key->name = ignore_key;
          else {
  ...
  for (; (key = key_iterator++); key_number++) {
    ...
    case Key::SPATIAL:
      ...
      break;
#else
      my_error(ER_FEATURE_DISABLED, MYF(0), sym_group_geom.name,
    ...
    */
    ...
    if (key_info->flags & HA_SPATIAL) {
      ...
      DBUG_RETURN(-1);
#else
      my_error(ER_FEATURE_DISABLED, MYF(0), sym_group_rtree.name,
               sym_group_rtree.needed_define);
      DBUG_RETURN(-1);
#endif
    }
    ...
    for (uint column_nr = 0; (column = cols++); column_nr++) {
      ...
      field = 0;
      while ((sql_field = it++) &&
             my_strcasecmp(system_charset_info, column->field_name,
      ...
      while ((dup_column = cols2++) != column) {
        if (!my_strcasecmp(system_charset_info, column->field_name,
                           dup_column->field_name)) {
      ...
      if (key->type == Key::FULLTEXT) {
        ...
        */
        ...
        if (!(sql_field->flags & NOT_NULL_FLAG)) {
          ...
          if (key->type == Key::SPATIAL) {
            my_message(ER_SPATIAL_CANT_HAVE_NULL, ER(ER_SPATIAL_CANT_HAVE_NULL),
                       MYF(0));
            DBUG_RETURN(-1);
          }
        }
          ...
          if ((length = column->length) > max_key_length ||
            ...
            if (key->type == Key::MULTIPLE) {
              ...
              my_snprintf(warn_buff, sizeof(warn_buff), ER(ER_TOO_LONG_KEY),
                          length);
              push_warning(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_TOO_LONG_KEY,
                           warn_buff);
            } else {
...
void sp_prepare_create_field(THD *thd, create_field *sql_field) {
  if (sql_field->sql_type == FIELD_TYPE_SET ||
    ...
    if (sql_field->sql_type == FIELD_TYPE_SET) {
      calculate_interval_lengths(sql_field->charset, sql_field->interval,
                                 &dummy, &field_length);
...
bool mysql_create_table(THD *thd, const char *db, const char *table_name,
                        HA_CREATE_INFO *create_info, List<create_field> &fields,
  ...
  */
  ...
  if (mysql_prepare_table(thd, create_info, &fields, &keys, internal_tmp_table,
                          &db_options, file, &key_info_buffer, &key_count,
                          select_field_count))
    DBUG_RETURN(TRUE);

  /* Check if table exists */
  if (create_info->options & HA_LEX_CREATE_TMP_TABLE) {
    my_snprintf(path, sizeof(path), "%s%s%lx_%lx_%x%s", mysql_tmpdir,
                tmp_file_prefix, current_pid, thd->thread_id, thd->tmp_table++,
                reg_ext);
...
warn:
  error = FALSE;
  push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_TABLE_EXISTS_ERROR,
                      ER(ER_TABLE_EXISTS_ERROR), alias);
  create_info->table_existed = 1; // Mark that table existed
  goto end;
}
...
static char *make_unique_key_name(const char *field_name, KEY *start,
                                  KEY *end) {
...
TABLE *create_table_from_items(THD *thd, HA_CREATE_INFO *create_info,
  ...
  tmp_table.s->db_low_byte_first =
      test(create_info->db_type == DB_TYPE_MYISAM ||
  ...
  while ((item = it++)) {
    ...
    if (item->type() == Item::FUNC_ITEM)
      ...
      field = create_tmp_field(thd, &tmp_table, item, item->type(), (Item ***)0,
                               &tmp_field, 0, 0, 0, 0);
    if (!field ||
...
bool mysql_rename_table(enum db_type base, const char *old_db,
  ...
  handler *file =
      (base == DB_TYPE_UNKNOWN ? 0 : get_new_handler((TABLE *)0, base));
  int error = 0;
...
static void wait_while_table_is_used(THD *thd, TABLE *table,
  ...
  remove_table_from_cache(thd, table->s->db, table->s->table_name,
                          RTFC_WAIT_OTHER_THREAD_FLAG);
  DBUG_VOID_RETURN;
}
...
static int prepare_for_restore(THD *thd, TABLE_LIST *table,
  ...
  {
    DBUG_RETURN(send_check_errmsg(
        thd, table, "restore", "table exists, will not overwrite on restore"));
  } else {
    ...
    my_snprintf(dst_path, sizeof(dst_path), "%s%s/%s", mysql_real_data_home, db,
                table_name);

    if (lock_and_wait_for_table_name(thd, table))
      DBUG_RETURN(-1);

    if (my_copy(src_path, fn_format(dst_path, dst_path, "", reg_ext, 4),
                MYF(MY_WME))) {
      pthread_mutex_lock(&LOCK_open);
      unlock_table_name(thd, table);
      pthread_mutex_unlock(&LOCK_open);
      DBUG_RETURN(
          send_check_errmsg(thd, table, "restore", "Failed copying .frm file"));
    }
...
static int prepare_for_repair(THD *thd, TABLE_LIST *table_list,
  ...
  */
  ...
  my_snprintf(tmp, sizeof(tmp), "%s-%lx_%lx", from, current_pid,
              thd->thread_id);
...
*/
static bool
mysql_admin_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt,
                  const char *operator_name, thr_lock_type lock_type,
                  bool open_for_modify, bool no_warnings_for_error,
    ...
    */
    ...
    if (lock_type == TL_WRITE && table->table->s->version) {
      ...
      remove_table_from_cache(
          thd, table->table->s->db, table->table->s->table_name,
  ...
  send_result_message:
    ...
    case HA_ADMIN_NOT_IMPLEMENTED: {
      ...
      uint length = my_snprintf(buf, ERRMSGSIZE, ER(ER_CHECK_NOT_IMPLEMENTED),
                                operator_name);
    ...
    case HA_ADMIN_REJECT:
      protocol->store("status", 6, system_charset_info);
      protocol->store("Operation need committed state", 30,
                      system_charset_info);
...
bool mysql_backup_table(THD *thd, TABLE_LIST *table_list) {
  ...
  DBUG_RETURN(mysql_admin_table(thd, table_list, 0, "backup", TL_READ, 0, 0, 0,
                                0, &handler::backup, 0));
}

bool mysql_restore_table(THD *thd, TABLE_LIST *table_list) {
  DBUG_ENTER("mysql_restore_table");
  DBUG_RETURN(mysql_admin_table(thd, table_list, 0, "restore", TL_WRITE, 1, 1,
                                0, &prepare_for_restore, &handler::restore, 0));
}

bool mysql_repair_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt) {
  DBUG_ENTER("mysql_repair_table");
  DBUG_RETURN(mysql_admin_table(thd, tables, check_opt, "repair", TL_WRITE, 1,
                                test(check_opt->sql_flags & TT_USEFRM),
                                HA_OPEN_FOR_REPAIR, &prepare_for_repair,
                                &handler::repair, 0));
}

bool mysql_optimize_table(THD *thd, TABLE_LIST *tables,
                          HA_CHECK_OPT *check_opt) {
  DBUG_ENTER("mysql_optimize_table");
  DBUG_RETURN(mysql_admin_table(thd, tables, check_opt, "optimize", TL_WRITE, 1,
                                0, 0, 0, &handler::optimize, 0));
}
...
bool mysql_assign_to_keycache(THD *thd, TABLE_LIST *tables,
  ...
  DBUG_RETURN(mysql_admin_table(thd, tables, &check_opt, "assign_to_keycache",
                                TL_READ_NO_INSERT, 0, 0, 0, 0,
...
bool mysql_preload_keys(THD *thd, TABLE_LIST *tables) {
  ...
  DBUG_RETURN(mysql_admin_table(thd, tables, 0, "preload_keys", TL_READ, 0, 0,
                                0, 0, &handler::preload_keys, 0));
}
...
bool mysql_create_like_table(THD *thd, TABLE_LIST *table,
  ...
  */
  ...
  if ((tmp_table = find_temporary_table(thd, src_db, src_table)))
    ...
    strxmov(src_path, mysql_data_home, "/", src_db, "/", src_table, reg_ext,
            NullS);
  ...
  */
  if (create_info->options & HA_LEX_CREATE_TMP_TABLE) {
    if (find_temporary_table(thd, db, table_name))
      goto table_exists;
    my_snprintf(dst_path, sizeof(dst_path), "%s%s%lx_%lx_%x%s", mysql_tmpdir,
                tmp_file_prefix, current_pid, thd->thread_id, thd->tmp_table++,
                reg_ext);
    if (lower_case_table_names)
    ...
    strxmov(dst_path, mysql_data_home, "/", db, "/", table_name, reg_ext,
            NullS);
...
table_exists:
  if (create_info->options & HA_LEX_CREATE_IF_NOT_EXISTS) {
    char warn_buff[MYSQL_ERRMSG_SIZE];
    my_snprintf(warn_buff, sizeof(warn_buff), ER(ER_TABLE_EXISTS_ERROR),
                table_name);
    push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_TABLE_EXISTS_ERROR,
                 warn_buff);
    res = FALSE;
  } else
    my_error(ER_TABLE_EXISTS_ERROR, MYF(0), table_name);
...
bool mysql_analyze_table(THD *thd, TABLE_LIST *tables,
  ...
  DBUG_RETURN(mysql_admin_table(thd, tables, check_opt, "analyze", lock_type, 1,
                                0, 0, 0, &handler::analyze, 0));
}
...
bool mysql_check_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt) {
  ...
  DBUG_RETURN(mysql_admin_table(thd, tables, check_opt, "check", lock_type, 0,
                                HA_OPEN_FOR_REPAIR, 0, 0, &handler::check,
                                &view_checksum));
}

/* table_list should contain just one table */
static int
mysql_discard_or_import_tablespace(THD *thd, TABLE_LIST *table_list,
...
int mysql_create_indexes(THD *thd, TABLE_LIST *table_list, List<Key> &keys) {
  ...
  */
  ...
  db_options = 0;
  if (mysql_prepare_table(thd, &create_info, &fields, &keys, /*tmp_table*/ 0,
                          &db_options, table->file, &key_info_buffer, key_count,
  ...
  */
  ...
  if ((idx < key_count) || !key_count) {
    ...
    if (real_alter_table(thd, table_list->db, table_list->table_name,
                         &create_info, table_list, table, fields, keys, drop,
                         alter, 0, (ORDER *)0, ALTER_ADD_INDEX, DUP_ERROR))
...
int mysql_drop_indexes(THD *thd, TABLE_LIST *table_list,
  ...
  */
  ...
  if ((drop_key) || (drop.elements <= 0)) {
    if (real_alter_table(thd, table_list->db, table_list->table_name,
                         &create_info, table_list, table, fields, keys, drop,
                         alter, 0, (ORDER *)0, ALTER_DROP_INDEX, DUP_ERROR))
      /*don't need to free((gptr) key_numbers);*/
      DBUG_RETURN(-1);
  } else {
    db_options = 0;
    if (table->file->drop_index(table, key_numbers, key_count) ||
        mysql_prepare_table(thd, &create_info, &fields, &keys, /*tmp_table*/ 0,
...
bool mysql_alter_table(THD *thd, char *new_db, char *new_name,
    ...
    if (new_db == db &&
      ...
      */
      ...
      if (table->s->tmp_table) {
        ...
        if (!access(
                 fn_format(new_name_buff, new_name_buff, dir_buff, reg_ext, 0),
                 F_OK)) {
  ...
  */
  ...
  for (uint i = 0; i < table->s->keys; i++, key_info++) {
    ...
    for (uint j = 0; j < key_info->key_parts; j++, key_part++) {
      ...
      key_part_length /= key_part->field->charset()->mbmaxlen;
      key_parts.push_back(
          new key_part_spec(cfield->field_name, key_part_length));
    }
    if (key_parts.elements)
      key_list.push_back(
          new Key(key_info->flags & HA_SPATIAL
                      ? Key::SPATIAL
                      : (key_info->flags & HA_NOSAME
                             ? (!my_strcasecmp(system_charset_info, key_name,
                                    ...
                                    ? Key::PRIMARY
                                    : Key::UNIQUE)
                             : (key_info->flags & HA_FULLTEXT ? Key::FULLTEXT
                                                              : Key::MULTIPLE)),
                  key_name, key_info->algorithm,
                  test(key_info->flags & HA_GENERATED_KEY), key_parts));
  }
  {
    Key *key;
  ...
  */
  ...
  {
    tmp_disable_binlog(thd);
    error = mysql_create_table(thd, new_db, tmp_name, create_info, create_list,
                               key_list, 1, 0);
    reenable_binlog(thd);
    if (error)
      DBUG_RETURN(error);
  ...
  if (need_copy_table) {
    if (table->s->tmp_table) {
      ...
      char path[FN_REFLEN];
      my_snprintf(path, sizeof(path), "%s/%s/%s", mysql_data_home, new_db,
                  tmp_name);
  ...
  if (new_table && !new_table->s->is_view) {
    ...
    error = copy_data_between_tables(table, new_table, create_list,
                                     handle_duplicates, ignore, order_num,
  ...
  if (table->s->tmp_table) {
    ...
    if (rename_temporary_table(thd, new_table, new_db,
                               new_name)) { // Fatal error
      close_temporary_table(thd, new_db, tmp_name);
  ...
  */
  ...
  error = 0;
  if (!need_copy_table)
    new_db_type = old_db_type =
        DB_TYPE_UNKNOWN; // this type cannot happen in regular ALTER
  if (mysql_rename_table(old_db_type, db, table_name, db, old_name)) {
    error = 1;
    VOID(quick_rm_table(new_db_type, new_db, tmp_name));
  } else if (mysql_rename_table(new_db_type, new_db, tmp_name, new_db,
                                new_alias)) { // Try to get everything back
    error = 1;
  ...
  if (thd->lock || new_name != table_name) // True if WIN32
  {
    ...
    */
    ...
    if (close_data_tables(thd, db, table_name) ||
        reopen_tables(thd, 1, 0)) { // This shouldn't happen
      if (table)
...
static int copy_data_between_tables(TABLE *from, TABLE *to,
  ...
  */
  ...
  thd->abort_on_warning =
      !ignore && test(thd->variables.sql_mode &
                      (MODE_STRICT_TRANS_TABLES | MODE_STRICT_ALL_TABLES));
  ...
  if (order) {
    from->sort.io_cache =
        (IO_CACHE *)my_malloc(sizeof(IO_CACHE), MYF(MY_FAE | MY_ZEROFILL));
    ...
    if (thd->lex->select_lex.setup_ref_array(thd, order_num) ||
        ...
        (from->sort.found_records =
             filesort(thd, from, sortorder, length, (SQL_SELECT *)0,
                      HA_POS_ERROR, &examined_rows)) == HA_POS_ERROR)
      goto err;
  };
...
bool mysql_recreate_table(THD *thd, TABLE_LIST *table_list, bool do_send_ok) {
  ...
  DBUG_RETURN(mysql_alter_table(thd, NullS, NullS, &create_info, table_list,
                                lex->create_list, lex->key_list, 0, (ORDER *)0,
          ...
          for (;;) {
            ...
            for (uint i = 0; i < t->s->fields; i++) {
              ...
              if (f->type() == FIELD_TYPE_BLOB) {
                ...
                row_crc =
                    my_checksum(row_crc, (byte *)f->ptr, f->pack_length());
            }
...
static bool check_engine(THD *thd, const char *table_name,
  ...
  if ((*new_engine = ha_checktype(thd, req_engine, no_substitution, 1)) ==
      DB_TYPE_UNKNOWN)
    return TRUE;
