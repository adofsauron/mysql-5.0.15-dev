        ...
        */
        if (!conds && !((Item_sum_count *)item)->args[0]->maybe_null &&
          ...
          for (table = tables; table; table = table->next_leaf) {
            if (outer_tables ||
                (table->table->file->table_flags() & HA_NOT_EXACT_COUNT) ||
                table->schema_table) {
        ...
        if (expr->type() == Item::FIELD_ITEM) {
          ...
          */
          if ((outer_tables & table->map) ||
              !find_key_for_maxmin(0, &ref, item_field->field, conds, &range_fl,
                                   &prefix_len)) {
            const_result = 0;
            break;
          }
          ...
          if (!ref.key_length)
            ...
            error = table->file->index_read(
                table->record[0], key_buff, ref.key_length,
                range_fl & NEAR_MIN ? HA_READ_AFTER_KEY : HA_READ_KEY_OR_NEXT);
          if (!error && reckey_in_range(0, &ref, item_field->field, conds,
                                        range_fl, prefix_len))
      ...
      case Item_sum::MAX_FUNC: {
        ...
        */
        ...
        if (expr->type() == Item::FIELD_ITEM) {
          ...
          */
          if ((outer_tables & table->map) ||
              !find_key_for_maxmin(1, &ref, item_field->field, conds, &range_fl,
                                   &prefix_len)) {
            const_result = 0;
            break;
          }
          ...
          if (!error && reckey_in_range(1, &ref, item_field->field, conds,
                                        range_fl, prefix_len))
            error = HA_ERR_KEY_NOT_FOUND;
          if (table->key_read) {
            table->key_read = 0;
...
static bool matching_cond(bool max_fl, TABLE_REF *ref, KEY *keyinfo,
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    Item *item;
    while ((item = li++)) {
      if (!matching_cond(max_fl, ref, keyinfo, field_part, item, key_part_used,
                         range_fl, prefix_len))
        return 0;
    }
    return 1;
    ...
    for (part = keyinfo->key_part, part_end = part + keyinfo->key_parts;
      ...
      if (field->eq(part->field)) {
        ...
        *range_fl = NO_MIN_RANGE | NO_MAX_RANGE;
        if (matching_cond(max_fl, ref, keyinfo, part, cond, &key_part_used,
                          range_fl, prefix_len) &&
