    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  return (trx_undo_get_next_rec_from_next_page(buf_frame_align(rec), page_no, offset, RW_S_LATCH, mtr));
}
    ...
    mtr_t *mtr)    /* in: mtr */
{
  ...
  return (trx_undo_get_next_rec_from_next_page(undo_page, page_no, offset, mode, mtr));
}
...
static page_t *trx_undo_seg_create(
    /*================*/
                      ...
                      the rollback segment mutex */
{
  ...
  page_no = fseg_alloc_free_page_general(header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_FSEG_HEADER, undo->top_page_no + 1,
                                         FSP_UP, TRUE, mtr);
  ...
  flst_add_last(header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_PAGE_LIST, new_page + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_NODE,
                mtr);
  undo->size++;
  rseg->curr_size++;
                       ...
                       the rollback segment mutex */
{
  ...
  flst_remove(header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_PAGE_LIST, undo_page + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_NODE,
              mtr);
...
static trx_undo_t *trx_undo_create(
    /*============*/
    ...
    mtr_t *mtr)       /* in: mtr */
{
  ...
  mlog_write_ulint(hdr_page + undo->hdr_offset + TRX_UNDO_DICT_TRANS, trx->dict_operation, MLOG_1BYTE, mtr);

  mlog_write_dulint(hdr_page + undo->hdr_offset + TRX_UNDO_TABLE_ID, trx->table_id, mtr);
    ...
    ulint type) /* in: TRX_UNDO_INSERT or TRX_UNDO_UPDATE */
{
  ...
  if (undo == NULL)
  {
    undo = trx_undo_create(trx, rseg, type, trx->id, &trx->xid, &mtr);
    if (undo == NULL)
    {
      /* Did not succeed */
