    ...
    page_dir_slot_t *slot) /* in: slot */
{
  ...
  n_owned = rec_get_n_owned(page_dir_slot_get_rec(slot), page_is_comp(page));
    ...
    ulint comp)         /* in: nonzero=compact page format */
{
  ...
  dtype_set(dfield_get_type(field), DATA_VARCHAR, DATA_ENGLISH | DATA_NOT_NULL, 8, 0);
  ...
  ut_a(infimum_rec == page + (comp ? PAGE_NEW_INFIMUM : PAGE_OLD_INFIMUM));
  ...
  dtype_set(dfield_get_type(field), DATA_VARCHAR, DATA_ENGLISH | DATA_NOT_NULL, comp ? 8 : 9, 0);

  supremum_rec = rec_convert_dtuple_to_rec(heap_top, index, tuple);

  ut_a(supremum_rec == page + (comp ? PAGE_NEW_SUPREMUM : PAGE_OLD_SUPREMUM));
  ...
  memset(heap_top, 0, UNIV_PAGE_SIZE - PAGE_EMPTY_DIR_START - (heap_top - page));
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
  ...
  for (;;)
  {
    ...
    if (UNIV_UNLIKELY(!page_cur_rec_insert(&cur2, cur1_rec, index, offsets, mtr)))
    {
      ...
      fprintf(stderr, "InnoDB: rec offset %lu, cur1 offset %lu, cur2 offset %lu\n", (ulong)(rec - page),
              (ulong)(page_cur_get_rec(&cur1) - page), (ulong)(page_cur_get_rec(&cur2) - new_page));
      ut_error;
    }
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  {
    page_copy_rec_list_end_to_created_page(new_page, page, rec, index, mtr);
  }
  else
  {
    page_copy_rec_list_end_no_locks(new_page, page, rec, index, mtr);
  }
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
  ...
  while (page_cur_get_rec(&cur1) != rec)
  {
    ...
    rec_t *cur1_rec = page_cur_get_rec(&cur1);
    offsets = rec_get_offsets(cur1_rec, index, offsets, ULINT_UNDEFINED, &heap);
    ins_rec = page_cur_rec_insert(&cur2, cur1_rec, index, offsets, mtr);
    ut_a(ins_rec);
...
{
  byte *log_ptr;
  ut_ad(type == MLOG_LIST_END_DELETE || type == MLOG_LIST_START_DELETE || type == MLOG_COMP_LIST_END_DELETE ||
    ...
    mtr_t *mtr)          /* in: mtr or NULL */
{
  ...
  {
    page_delete_rec_list_end(page, page + offset, index, ULINT_UNDEFINED, ULINT_UNDEFINED, mtr);
  }
  else
  {
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  page_delete_rec_list_write_log(rec, index, comp ? MLOG_COMP_LIST_END_DELETE : MLOG_LIST_END_DELETE, mtr);
    ...
    mtr_t *mtr)          /* in: mtr */
{
  ...
  page_delete_rec_list_end(page, split_rec, index, new_n_recs - old_n_recs, new_data_size - old_data_size, mtr);
}
    ...
    const ulint *offsets) /* in: record descriptor */
{
  ...
  fprintf(stderr, "            n_owned: %lu; heap_no: %lu; next rec: %lu\n", (ulong)rec_get_n_owned(rec, comp),
          (ulong)rec_get_heap_no(rec, comp), (ulong)rec_get_next_offs(rec, comp));
    ...
    ulint pr_n)   /* in: print n first and n last entries */
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if ((i < pr_n) || (i >= n - pr_n))
    {
      fprintf(stderr, "Contents of slot: %lu: n_owned: %lu, rec offs: %lu\n", (ulong)i,
              (ulong)page_dir_slot_get_n_owned(slot), (ulong)(page_dir_slot_get_rec(slot) - page));
    }
  }
  fprintf(stderr,
          "Total of %lu records\n"
    ...
    page_t *page)
{
  fprintf(stderr,
          ...
          (ulong)page_header_get_field(page, PAGE_N_DIR_SLOTS), (ulong)page_header_get_field(page, PAGE_HEAP_TOP),
    ...
    const ulint *offsets) /* in: array returned by rec_get_offsets() */
{
  ...
  {
    fprintf(stderr, "InnoDB: Heap no of rec %lu too big %lu %lu\n", (ulong)(rec - page), (ulong)heap_no,
    ...
    page_t *page) /* in: index page */
{
  ...
  if (page_dir_slot_get_rec(page_dir_get_nth_slot(page, 0)) != page_get_infimum_rec(page))
  {
    fprintf(stderr, "InnoDB: Page directory corruption: supremum not pointed to\n");
    buf_page_print(page);
  }

  if (page_dir_slot_get_rec(page_dir_get_nth_slot(page, n_slots - 1)) != page_get_supremum_rec(page))
  {
    fprintf(stderr, "InnoDB: Page directory corruption: supremum not pointed to\n");
    buf_page_print(page);
  }
    ...
    page_t *page) /* in: index page */
{
  ...
  {
    fprintf(stderr, "InnoDB: Nonsensical number %lu of page dir slots\n", (ulong)n_slots);
  ...
  for (;;)
  {
    ...
    if (rec > rec_heap_top)
    {
      fprintf(stderr, "InnoDB: Record %lu is above rec heap top %lu\n", (ulong)(rec - page),
    ...
    if (rec_get_n_owned(rec, comp) != 0)
    {
      ...
      if (rec_get_n_owned(rec, comp) != own_count)
      {
        fprintf(stderr, "InnoDB: Wrong owned count %lu, %lu, rec %lu\n", (ulong)rec_get_n_owned(rec, comp),
                (ulong)own_count, (ulong)(rec - page));
      ...
      {
        fprintf(stderr, "InnoDB: Dir slot does not point to right rec %lu\n", (ulong)(rec - page));
    ...
    if (rec_get_next_offs(rec, comp) < FIL_PAGE_DATA || rec_get_next_offs(rec, comp) >= UNIV_PAGE_SIZE)
    {
      fprintf(stderr, "InnoDB: Next record offset nonsensical %lu for rec %lu\n", (ulong)rec_get_next_offs(rec, comp),
              (ulong)(rec - page));
  ...
  while (rec != NULL)
  {
    if (rec < page + FIL_PAGE_DATA || rec >= page + UNIV_PAGE_SIZE)
    {
      fprintf(stderr, "InnoDB: Free list record has a nonsensical offset %lu\n", (ulong)(rec - page));
    ...
    if (rec > rec_heap_top)
    {
      fprintf(stderr, "InnoDB: Free list record %lu is above rec heap top %lu\n", (ulong)(rec - page),
              (ulong)(rec_heap_top - page));
                         ...
                         the page record type definition */
{
  ...
  {
    fputs("InnoDB: Record heap and dir overlap on a page ", stderr);
    dict_index_name_print(stderr, NULL, index);
    fprintf(stderr, ", %p, %p\n", page_header_get_ptr(page, PAGE_HEAP_TOP), page_dir_get_nth_slot(page, n_slots - 1));
  ...
  for (;;)
  {
    ...
    if (comp && page_rec_is_user_rec(rec) && rec_get_node_ptr_flag(rec) != (ibool)(btr_page_get_level_low(page) != 0))
    {
    ...
    {
      if (!(1 == cmp_rec_rec(rec, old_rec, offsets, old_offsets, index)))
      {
        fprintf(stderr, "InnoDB: Records in wrong order on page %lu", (ulong)buf_frame_get_page_no(page));
        dict_index_name_print(stderr, NULL, index);
    ...
    if (rec_get_n_owned(rec, comp) != 0)
    {
      ...
      if (rec_get_n_owned(rec, comp) != own_count)
      {
        fprintf(stderr, "InnoDB: Wrong owned count %lu, %lu\n", (ulong)rec_get_n_owned(rec, comp), (ulong)own_count);
        goto func_exit;
      }

      if (page_dir_slot_get_rec(slot) != rec)
      {
        fputs("InnoDB: Dir slot does not point to right rec\n", stderr);
        goto func_exit;
      }
    ...
    if (rec_get_next_offs(rec, comp) < FIL_PAGE_DATA || rec_get_next_offs(rec, comp) >= UNIV_PAGE_SIZE)
    {
      fprintf(stderr, "InnoDB: Next record offset wrong %lu\n", (ulong)rec_get_next_offs(rec, comp));
      goto func_exit;
    }
  ...
  while (rec != NULL)
  {
    ...
    for (i = 0; i < rec_offs_size(offsets); i++)
    {
      if (buf[offs + i] != 0)
      {
        fputs("InnoDB: Record overlaps another in free list\n", stderr);
        goto func_exit;
      }
...
func_exit:
  ...
  {
  func_exit2:
    fprintf(stderr, "InnoDB: Apparent corruption in page %lu in ", (ulong)buf_frame_get_page_no(page));
    dict_index_name_print(stderr, NULL, index);
    putc('\n', stderr);
    buf_page_print(page);
  }
...
rec_t *page_find_rec_with_heap_no(
    /*=======================*/
