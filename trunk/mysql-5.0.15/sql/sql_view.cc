...
TYPELIB updatable_views_with_limit_typelib = {array_elements(updatable_views_with_limit_names) - 1, "",
                                              updatable_views_with_limit_names, 0};
...
{
  char *name = (target->orig_name ? target->orig_name : target->name);
  ...
  for (attempt = 0;; attempt++)
  {
    ...
    {
      check = itc++;
      if (check != target && my_strcasecmp(system_charset_info, buff, check->name) == 0)
      {
        ok = FALSE;
        break;
      }
...
bool check_duplicate_names(List<Item> &item_list, bool gen_unique_view_name)
{
  ...
  while ((item = it++))
  {
    ...
    if (item->real_item()->type() == Item::FIELD_ITEM) item->is_autogenerated_name = FALSE;
    itc.rewind();
...
bool mysql_create_view(THD *thd, enum_view_create_mode mode)
{
  ...
  if (lex->proc_list.first || lex->result)
  {
    my_error(ER_VIEW_SELECT_CLAUSE, MYF(0), (lex->result ? "INTO" : "PROCEDURE"));
    res = TRUE;
    goto err;
  }
  if (lex->derived_tables || lex->variables_used || lex->param_list.elements)
  {
    int err = (lex->derived_tables ? ER_VIEW_SELECT_DERIVED : ER_VIEW_SELECT_VARIABLE);
    my_message(err, ER(err), MYF(0));
    res = TRUE;
    goto err;
  }

  if (mode != VIEW_CREATE_NEW) sp_cache_invalidate();
  ...
  */
  if (strcmp(lex->create_view_definer->user.str, thd->security_ctx->priv_user) != 0 ||
      my_strcasecmp(system_charset_info, lex->create_view_definer->host.str, thd->security_ctx->priv_host) != 0)
  {
    if (!(thd->security_ctx->master_access & SUPER_ACL))
    {
      my_error(ER_VIEW_OTHER_USER, MYF(0), lex->create_view_definer->user.str, lex->create_view_definer->host.str);
      res = TRUE;
      goto err;
    }
    else
    {
      if (!is_acl_user(lex->create_view_definer->host.str, lex->create_view_definer->user.str))
      {
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_NO_SUCH_USER, ER(ER_NO_SUCH_USER),
                            lex->create_view_definer->user.str, lex->create_view_definer->host.str);
      }
    }
  }
  /*
  ...
  */
  if ((check_access(thd, CREATE_VIEW_ACL, view->db, &view->grant.privilege, 0, 0, is_schema_db(view->db)) ||
       grant_option && check_grant(thd, CREATE_VIEW_ACL, view, 0, 1, 0)) ||
      (mode != VIEW_CREATE_NEW &&
       (check_access(thd, DROP_ACL, view->db, &view->grant.privilege, 0, 0, is_schema_db(view->db)) ||
  ...
  for (sl = select_lex; sl; sl = sl->next_select())
  {
    for (tbl = sl->get_table_list(); tbl; tbl = tbl->next_local)
    {
      ...
      */
      ...
      {
        my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), "ANY", thd->security_ctx->priv_user, thd->security_ctx->priv_host,
                 tbl->table_name);
        res = TRUE;
        goto err;
      }
      ...
      */
      ...
      /*
        ...
        not opened yet.
      */
      fill_effective_table_privileges(thd, &tbl->grant, tbl->db, tbl->table_name);
    }
  }
  ...
  if (&lex->select_lex != lex->all_selects_list)
  {
    ...
    for (tbl = tables; tbl; tbl = tbl->next_global)
    {
      ...
      {
        if (check_access(thd, SELECT_ACL, tbl->db, &tbl->grant.privilege, 0, 0, test(tbl->schema_table)) ||
  ...
  */
  for (sl = select_lex; sl; sl = sl->next_select())
  {
    ...
    {
      Item_field *field;
      if ((field = item->filed_for_view_update())) field->any_privileges = 1;
    }
  }
#endif
  ...
  */
  for (tbl = lex->query_tables; tbl; tbl = tbl->next_global)
  {
    ...
    if (tbl->table->s->tmp_table != NO_TMP_TABLE && !tbl->view && !tbl->schema_table)
    {
    ...
    if (tbl->view && strcmp(tbl->view_db.str, view->db) == 0 && strcmp(tbl->view_name.str, view->table_name) == 0)
    {
  ...
  */
  for (sl = select_lex; sl; sl = sl->next_select())
  {
    ...
    List_iterator_fast<Item> it(sl->item_list);
    Item *item;
    fill_effective_table_privileges(thd, &view->grant, db, view->table_name);
    while ((item = it++))
    {
      Item_field *fld;
      uint priv = (get_column_grant(thd, &view->grant, db, view->table_name, item->name) & VIEW_ANY_ACL);
      if ((fld = item->filed_for_view_update()))
      {
        ...
        */
        if (!fld->field->table->s->tmp_table && (~fld->have_privileges & priv))
        {
          ...
          my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), "create view", thd->security_ctx->priv_user,
                   thd->security_ctx->priv_host, item->name, view->table_name);
          res = TRUE;
          goto err;
        }
      }
  ...
  if (view->revision != 1) query_cache_invalidate3(thd, view, 0);
...
static File_option view_parameters[] = {
    {{(char *)STRING_WITH_LEN("query")}, offsetof(TABLE_LIST, query), FILE_OPTIONS_STRING},
    {{(char *)STRING_WITH_LEN("md5")}, offsetof(TABLE_LIST, md5), FILE_OPTIONS_STRING},
    {{(char *)STRING_WITH_LEN("updatable")}, offsetof(TABLE_LIST, updatable_view), FILE_OPTIONS_ULONGLONG},
    {{(char *)STRING_WITH_LEN("algorithm")}, offsetof(TABLE_LIST, algorithm), FILE_OPTIONS_ULONGLONG},
    {{(char *)STRING_WITH_LEN("definer_user")}, offsetof(TABLE_LIST, definer.user), FILE_OPTIONS_STRING},
    {{(char *)STRING_WITH_LEN("definer_host")}, offsetof(TABLE_LIST, definer.host), FILE_OPTIONS_STRING},
    {{(char *)STRING_WITH_LEN("suid")}, offsetof(TABLE_LIST, view_suid), FILE_OPTIONS_ULONGLONG},
    {{(char *)STRING_WITH_LEN("with_check_option")}, offsetof(TABLE_LIST, with_check), FILE_OPTIONS_ULONGLONG},
    {{(char *)STRING_WITH_LEN("revision")}, offsetof(TABLE_LIST, revision), FILE_OPTIONS_REV},
    {{(char *)STRING_WITH_LEN("timestamp")}, offsetof(TABLE_LIST, timestamp), FILE_OPTIONS_TIMESTAMP},
    {{(char *)STRING_WITH_LEN("create-version")}, offsetof(TABLE_LIST, file_version), FILE_OPTIONS_ULONGLONG},
    {{(char *)STRING_WITH_LEN("source")}, offsetof(TABLE_LIST, source), FILE_OPTIONS_ESTRING},
...
static int mysql_register_view(THD *thd, TABLE_LIST *view, enum_view_create_mode mode)
{
  ...
  /* print file name */
  (void)my_snprintf(dir_buff, FN_REFLEN, "%s/%s/", mysql_data_home, view->db);
  unpack_filename(dir_buff, dir_buff);
  dir.str = dir_buff;
  dir.length = strlen(dir_buff);

  file.str = file_buff;
  file.length = (strxnmov(file_buff, FN_REFLEN, view->table_name, reg_ext, NullS) - file_buff);
  /* init timestamp */
  ...
  /* check old .frm */
  {
    ...
    if (!access(path.str, F_OK))
    {
      ...
      if (!(parser = sql_parse_prepare(&path, thd->mem_root, 0))) DBUG_RETURN(1);

      if (!parser->ok() || !is_equal(&view_type, parser->type()))
      {
        my_error(ER_WRONG_OBJECT, MYF(0), (view->db ? view->db : thd->db), view->table_name, "VIEW");
        DBUG_RETURN(-1);
      }
      ...
      */
      if (parser->parse((gptr)view, thd->mem_root, view_parameters + revision_number_position, 1))
      {
  ...
  view->source.length = (thd->query_length - (thd->lex->create_view_select_start - thd->query));
  view->file_version = 1;
  view->calc_md5(md5);
  view->md5.str = md5;
  view->md5.length = 32;
  can_be_merged = lex->can_be_merged();
  if (lex->create_view_algorithm == VIEW_ALGORITHM_MERGE && !lex->can_be_merged())
  {
    push_warning(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_VIEW_MERGE, ER(ER_WARN_VIEW_MERGE));
  ...
  if ((view->updatable_view = (can_be_merged && view->algorithm != VIEW_ALGORITHM_TMPTABLE)))
  {
    /* TODO: change here when we will support UNIONs */
    for (TABLE_LIST *tbl = (TABLE_LIST *)lex->select_lex.table_list.first; tbl; tbl = tbl->next_local)
    {
...
loop_out:
  /*
    ...
    UNION
  */
  if (view->updatable_view && !lex->select_lex.next_select() &&
      !((TABLE_LIST *)lex->select_lex.table_list.first)->next_local &&
      find_table_in_global_list(lex->query_tables->next_global, lex->query_tables->db, lex->query_tables->table_name))
  {
    view->updatable_view = 0;
  }
  ...
  if (sql_create_definition_file(&dir, &file, view_file_type, (gptr)view, view_parameters, num_view_backups))
  {
...
my_bool mysql_make_view(File_parser *parser, TABLE_LIST *table)
{
  ...
  if (table->view)
  {
    DBUG_PRINT("info", ("VIEW %s.%s is already processed on previous PS/SP execution", table->view_db.str,
                        table->view_name.str));
    DBUG_RETURN(0);
  }
  ...
  */
  ...
  /* init timestamp */
  if (!table->timestamp.str) table->timestamp.str = table->timestamp_buffer;
  ...
  /*
    ...
    be used here
  */
  if (parser->parse((gptr)table, thd->mem_root, view_parameters, required_view_parameters)) goto err;
  ...
  */
  ...
  {
    DBUG_ASSERT(!table->definer.host.str && !table->definer.user.length && !table->definer.host.length);
    push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_WARN, ER_VIEW_FRM_NO_USER, ER(ER_VIEW_FRM_NO_USER), table->db,
                        table->table_name);
    if (default_view_definer(thd->security_ctx, &table->definer)) goto err;
  }
  ...
  */
  ...
  view_select->select_number = ++thd->select_number;
  {
    ...
    */
    thd->variables.sql_mode &=
        ~(MODE_PIPES_AS_CONCAT | MODE_ANSI_QUOTES | MODE_IGNORE_SPACE | MODE_NO_BACKSLASH_ESCAPES);
  ...
  if (!res && !thd->is_fatal_error)
  {
    ...
    */
    if (!table->prelocking_placeholder && (old_lex->sql_command == SQLCOM_SELECT && old_lex->describe))
    {
      if (check_table_access(thd, SELECT_ACL, view_tables, 1) && check_table_access(thd, SHOW_VIEW_ACL, table, 1))
      {
        my_message(ER_VIEW_NO_EXPLAIN, ER(ER_VIEW_NO_EXPLAIN), MYF(0));
        goto err;
      }
    }
    else if (!table->prelocking_placeholder && old_lex->sql_command == SQLCOM_SHOW_CREATE)
    {
      if (check_table_access(thd, SHOW_VIEW_ACL, table, 0)) goto err;
    }
    ...
    /*
      ...
      last view table
    */
    for (tbl = view_tables; tbl; tbl = (view_tables_tail = tbl)->next_global)
    {
    ...
    */
    if (!lex->select_lex.next_select())
      for (tbl = lex->select_lex.get_table_list(); tbl; tbl = tbl->next_local) tbl->lock_type = table->lock_type;
    ...
    /*
      ...
      further processing.
    */
    if (table->prelocking_placeholder) goto ok2;

    old_lex->derived_tables |= DERIVED_VIEW;

    /* move SQL_NO_CACHE & Co to whole query */
    old_lex->safe_to_cache_query = (old_lex->safe_to_cache_query && lex->safe_to_cache_query);
    /* move SQL_CACHE to whole query */
    if (view_select->options & OPTION_TO_QUERY_CACHE) old_lex->select_lex.options |= OPTION_TO_QUERY_CACHE;
    ...
    */
    if (table->algorithm != VIEW_ALGORITHM_TMPTABLE && lex->can_be_merged() &&
        (table->select_lex->master_unit() != &old_lex->unit || old_lex->can_use_merged()) &&
        !old_lex->can_not_use_merged())
    {
      ...
      table->effective_with_check = old_lex->get_effective_with_check(table);

      /* prepare view context */
      lex->select_lex.context.resolve_in_table_list_only(table->ancestor = view_tables);
      lex->select_lex.context.outer_context = 0;
      lex->select_lex.context.select_lex = table->select_lex;
      lex->select_lex.select_n_having_items += table->select_lex->select_n_having_items;
      ...
      /*
        ...
        with unions).
      */
      for (tbl = lex->select_lex.get_table_list(); tbl; tbl = tbl->next_local) tbl->select_lex = table->select_lex;

      {
        if (view_tables->next_local)
        {
          table->multitable_view = TRUE;
          if (table->belong_to_view) table->belong_to_view->multitable_view = TRUE;
        }
        /* make nested join structure for view tables */
        NESTED_JOIN *nested_join;
        if (!(nested_join = table->nested_join = (NESTED_JOIN *)thd->calloc(sizeof(NESTED_JOIN)))) goto err;
      ...
      */
      ...
      for (SELECT_LEX_UNIT *unit = lex->select_lex.first_inner_unit(); unit; unit = next_unit)
      {
...
ok:
  ...
  lex->all_selects_list->link_prev = (st_select_lex_node **)&old_lex->all_selects_list;
...
bool mysql_drop_view(THD *thd, TABLE_LIST *views, enum_drop_mode drop_mode)
{
  ...
  {
    strxnmov(path, FN_REFLEN, mysql_data_home, "/", view->db, "/", view->table_name, reg_ext, NullS);
    ...
    if (access(path, F_OK) || (type = (mysql_frm_type(path) != FRMTYPE_VIEW)))
    {
      ...
      {
        push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_BAD_TABLE_ERROR, ER(ER_BAD_TABLE_ERROR), name);
        VOID(pthread_mutex_unlock(&LOCK_open));
        continue;
      }
      if (type)
    ...
    if (my_delete(path, MYF(MY_WME))) goto err;
...
frm_type_enum mysql_frm_type(char *path)
{
  ...
  if (length == (int)MY_FILE_ERROR) DBUG_RETURN(FRMTYPE_ERROR);
  if (length < (int)sizeof(header) || !strncmp(header, "TYPE=VIEW\n", sizeof(header))) DBUG_RETURN(FRMTYPE_VIEW);
  ...
  */
  ...
  for (; key_info != key_info_end; key_info++)
  {
    if ((key_info->flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME)
    {
      ...
      for (;;)
      {
        ...
        {
          Item_field *field;
          if ((field = k->item->filed_for_view_update()) && field->field == key_part->field) break;
        }
  ...
  /* check all fields presence */
  {
    ...
    for (field_ptr = table->field; *field_ptr; field_ptr++)
    {
      ...
      {
        Item_field *field;
        if ((field = fld->item->filed_for_view_update()) && field->field == *field_ptr) break;
      }
      if (fld == end_of_trans)  // If field didn't exists
      {
        ...
        */
        if (thd->variables.updatable_views_with_limit)
        {
          ...
          push_warning(thd, MYSQL_ERROR::WARN_LEVEL_NOTE, ER_WARN_VIEW_WITHOUT_KEY, ER(ER_WARN_VIEW_WITHOUT_KEY));
          DBUG_RETURN(FALSE);
        }
        /* prohibit update */
        DBUG_RETURN(TRUE);
...
{
  char md5[MD5_BUFF_LENGTH];
  if (!view->view || view->md5.length != 32) return HA_ADMIN_NOT_IMPLEMENTED;
  view->calc_md5(md5);
  return (strncmp(md5, view->md5.str, 32) ? HA_ADMIN_WRONG_CHECKSUM : HA_ADMIN_OK);
}
...
bool mysql_rename_view(THD *thd, const char *new_name, TABLE_LIST *view)
{
  ...
  strxnmov(view_path, FN_REFLEN, mysql_data_home, "/", view->db, "/", view->table_name, reg_ext, NullS);
  (void)unpack_filename(view_path, view_path);

  pathstr.str = (char *)view_path;
  pathstr.length = strlen(view_path);

  if ((parser = sql_parse_prepare(&pathstr, thd->mem_root, 1)) && is_equal(&view_type, parser->type()))
  {
    TABLE_LIST view_def;
    char dir_buff[FN_REFLEN], file_buff[FN_REFLEN];
    ...
    */
    ...
    if (parser->parse((gptr)&view_def, thd->mem_root, view_parameters, array_elements(view_parameters) - 1)) goto err;

    /* rename view and it's backups */
    if (rename_in_schema_file(view->db, view->table_name, new_name, view_def.revision - 1, num_view_backups)) goto err;
    ...
    file.str = file_buff;
    file.length = (strxnmov(file_buff, FN_REFLEN, new_name, reg_ext, NullS) - file_buff);

    if (sql_create_definition_file(&pathstr, &file, view_file_type, (gptr)&view_def, view_parameters, num_view_backups))
    {
      /* restore renamed view in case of error */
      rename_in_schema_file(view->db, new_name, view->table_name, view_def.revision - 1, num_view_backups);
      goto err;
    }
  }
  else
