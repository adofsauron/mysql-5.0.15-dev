...
#define RW_WLOCK(M)                                                            \
  {                                                                            \
    ...
    if (!rw_wrlock(M))                                                         \
      DBUG_PRINT("lock", ("rwlock wlock ok")) else DBUG_PRINT(                 \
          "lock", ("rwlock wlock FAILED %d", errno));                          \
  }
#define RW_RLOCK(M)                                                            \
  {                                                                            \
    DBUG_PRINT("lock", ("rwlock rlock 0x%lx", (ulong)(M)));                    \
    if (!rw_rdlock(M))                                                         \
      DBUG_PRINT("lock", ("rwlock rlock ok")) else DBUG_PRINT(                 \
          "lock", ("rwlock wlock FAILED %d", errno));                          \
  }
#define RW_UNLOCK(M)                                                           \
  {                                                                            \
    ...
    if (!rw_unlock(M))                                                         \
      DBUG_PRINT("lock", ("rwlock unlock ok")) else DBUG_PRINT(                \
          "lock", ("rwlock unlock FAILED %d", errno));                         \
  }
...
void Query_cache_block::init(ulong block_length) {
  ...
  DBUG_PRINT("qcache",
             ("init block 0x%lx  length: %lu", (ulong) this, block_length));
  length = block_length;
  used = 0;
...
inline Query_cache_block_table *Query_cache_block::table(TABLE_COUNTER_TYPE n) {
  return ((Query_cache_block_table *)(((byte *)this) +
                                      ALIGN_SIZE(sizeof(Query_cache_block)) +
...
void query_cache_insert(NET *net, const char *packet, ulong length) {
  ...
  */
  ...
  Query_cache_block *query_block =
      ((Query_cache_block *)net->query_cache_query);
  if (query_block) {
...
void query_cache_end_of_result(THD *thd) {
  ...
  if (thd->net.query_cache_query != 0) // Quick check on unlocked structure
  {
    ...
    */
    ...
    Query_cache_block *query_block =
        ((Query_cache_block *)thd->net.query_cache_query);
    if (query_block) {
      DUMP(&query_cache);
      BLOCK_LOCK_WR(query_block);
      Query_cache_query *header = query_block->query();
...
Query_cache::Query_cache(ulong query_cache_limit_arg,
    ...
    : query_cache_size(0), query_cache_limit(query_cache_limit_arg),
      queries_in_cache(0), hits(0), inserts(0), refused(0), total_blocks(0),
      lowmem_prunes(0),
      min_allocation_unit(ALIGN_SIZE(min_allocation_unit_arg)),
      min_result_data_size(ALIGN_SIZE(min_result_data_size_arg)),
      def_query_hash_size(ALIGN_SIZE(def_query_hash_size_arg)),
      def_table_hash_size(ALIGN_SIZE(def_table_hash_size_arg)), initialized(0) {
...
ulong Query_cache::resize(ulong query_cache_size_arg) {
  ...
  DBUG_PRINT("qcache",
             ("from %lu to %lu", query_cache_size, query_cache_size_arg));
...
void Query_cache::store_query(THD *thd, TABLE_LIST *tables_used) {
  ...
  if ((local_tables = is_cacheable(thd, thd->query_length, thd->query, thd->lex,
                                   tables_used, &tables_type))) {
    NET *net = &thd->net;
    ...
    flags.client_protocol_41 =
        test(thd->client_capabilities & CLIENT_PROTOCOL_41);
    flags.more_results_exists =
        test(thd->server_status & SERVER_MORE_RESULTS_EXISTS);
...
sql mode: 0x%lx, sort len: %lu, conncat len: %lu",
                (int)flags.client_long_flag, (int)flags.client_protocol_41,
                (int)flags.more_results_exists, flags.pkt_nr,
                flags.character_set_client_num, flags.character_set_results_num,
                flags.collation_connection_num, flags.limit,
                (ulong)flags.time_zone, flags.sql_mode, flags.max_sort_length,
    ...
    if (thd->db_length) {
      ...
      DBUG_PRINT("qcache",
                 ("database : %s length %u", thd->db, thd->db_length));
    } else {
      DBUG_PRINT("qcache", ("No active database"));
    }
    tot_length =
        thd->query_length + thd->db_length + 1 + QUERY_CACHE_FLAGS_SIZE;
    /*
      We should only copy structure (don't use it location directly)
      because of alignment issue
    */
    memcpy((void *)(thd->query + (tot_length - QUERY_CACHE_FLAGS_SIZE)), &flags,
           QUERY_CACHE_FLAGS_SIZE);

    /* Check if another thread is processing the same query? */
    Query_cache_block *competitor = (Query_cache_block *)hash_search(
        &queries, (byte *)thd->query, tot_length);
...
sql mode: 0x%lx, sort len: %lu, conncat len: %lu",
              ...
              (ulong)flags.time_zone, flags.sql_mode, flags.max_sort_length,
              flags.group_concat_max_len));
  memcpy((void *)(sql + (tot_length - QUERY_CACHE_FLAGS_SIZE)), &flags,
         QUERY_CACHE_FLAGS_SIZE);
  query_block =
      (Query_cache_block *)hash_search(&queries, (byte *)sql, tot_length);
  ...
  for (; block_table != block_table_end; block_table++) {
    ...
    */
    ...
    if (table->callback() &&
        !(*table->callback())(thd, table->db(), table->key_length(),
                              &engine_data)) {
      ...
      if (engine_data != table->engine_data()) {
        DBUG_PRINT("qcache",
                   ("Handler require invalidation queries of %s.%s %lld-%lld",
                    table_list.db, table_list.alias, engine_data,
                    table->engine_data()));
      ...
      DBUG_PRINT("qcache", ("handler allow caching %s,%s", table_list.db,
                            table_list.alias));
  }
...
*/
  ...
  do {
    ...
    if (net_real_write(&thd->net, result->data(),
                       result_block->used - result_block->headers_len() -
                           ALIGN_SIZE(sizeof(Query_cache_result))))
...
#else
  {
    Querycache_stream qs(result_block,
                         result_block->headers_len() +
                             ALIGN_SIZE(sizeof(Query_cache_result)));
    ...
    if (query_cache_size > 0) {
      ...
      for (; tables_used; tables_used = tables_used->next) {
        ...
        DBUG_PRINT("qcache", (" db %s, table %s", tables_used->key,
                              tables_used->key + strlen(tables_used->key) + 1));
      }
    }
    STRUCT_UNLOCK(&structure_guard_mutex);
  }
...
void Query_cache::invalidate_by_MyISAM_filename(const char *filename) {
  ...
  if (query_cache_size > 0) {
    ...
    if (query_cache_size > 0) // Safety if cache removed
    {
      ...
      if ((table_block = (Query_cache_block *)hash_search(&tables, (byte *)key,
                                                          key_length)))
...
ulong Query_cache::init_cache() {
  ...
  approx_additional_data_size =
      (sizeof(Query_cache) +
       sizeof(gptr) * (def_query_hash_size + def_table_hash_size));
  ...
  */
  ...
  additional_data_size =
      ((mem_bin_num + 1) * ALIGN_SIZE(sizeof(Query_cache_memory_bin)) +
  ...
  if (!(cache = (byte *)my_malloc_lock(query_cache_size + additional_data_size,
                                       MYF(0))))
    goto err;
  ...
  bins = ((Query_cache_memory_bin *)(cache +
                                     mem_bin_steps *
                                         ALIGN_SIZE(sizeof(
  ...
  */
  VOID(hash_init(&tables,
                 lower_case_table_names ? &my_charset_bin : files_charset_info,
                 def_table_hash_size, 0, 0, query_cache_table_get_key, 0, 0));
#endif

  queries_in_cache = 0;
  queries_blocks = 0;
  DBUG_RETURN(query_cache_size + additional_data_size +
              approx_additional_data_size);
...
void Query_cache::free_cache() {
  ...
  if (query_cache_size > 0) {
    ...
    if (free_memory != query_cache_size)
      DBUG_PRINT("qcache", ("free memory %lu (should be %lu)", free_memory,
                            query_cache_size));
#endif
...
Query_cache_block *
Query_cache::write_block_data(ulong data_len, gptr data, ulong header_len,
                              Query_cache_block::block_type type,
                              TABLE_COUNTER_TYPE ntab, my_bool under_guard) {
  ulong all_headers_len =
      (ALIGN_SIZE(sizeof(Query_cache_block)) +
       ALIGN_SIZE(ntab * sizeof(Query_cache_block_table)) + header_len);
  ulong len = data_len + all_headers_len;
  ulong align_len = ALIGN_SIZE(len);
  DBUG_ENTER("Query_cache::write_block_data");
  DBUG_PRINT("qcache", ("data: %ld, header: %ld, all header: %ld", data_len,
                        header_len, all_headers_len));
  Query_cache_block *block =
      allocate_block(max(align_len, min_allocation_unit), 1, 0, under_guard);
  if (block != 0) {
    block->type = type;
    block->n_tables = ntab;
    block->used = len;
...
my_bool Query_cache::append_result_data(Query_cache_block **current_block,
                                        ulong data_len, gptr data,
                                        Query_cache_block *query_block) {
  DBUG_ENTER("Query_cache::append_result_data");
  DBUG_PRINT("qcache",
             ("append %lu bytes to 0x%lx query", data_len, query_block));

  if (query_block->query()->add(data_len) > query_cache_limit) {
    DBUG_PRINT("qcache", ("size limit reached %lu > %lu",
                          query_block->query()->length(), query_cache_limit));
    DBUG_RETURN(0);
  }
  if (*current_block == 0) {
  ...
  DBUG_PRINT("qcache", ("lastblock 0x%lx len %lu used %lu", (ulong)last_block,
                        last_block->length, last_block->used));
  my_bool success = 1;
  ...
  */
  ...
  if (last_block_free_space < data_len &&
      append_next_free_block(last_block, max(tail, append_min)))
  ...
  */
  ...
  if (success) {
    ...
    do {
      ...
      DBUG_PRINT("qcache",
                 ("write %lu byte in block 0x%lx", length, (ulong)block));
...
my_bool Query_cache::allocate_data_chain(Query_cache_block **result_block,
  ...
  ulong all_headers_len = (ALIGN_SIZE(sizeof(Query_cache_block)) +
                           ALIGN_SIZE(sizeof(Query_cache_result)));
  ulong min_size = (first_block_arg ? get_min_first_result_data_size()
  ...
  DBUG_PRINT("qcache",
             ("data_len %lu, all_headers_len %lu", data_len, all_headers_len));

  do {
    ulong len = data_len + all_headers_len;
    ulong align_len = ALIGN_SIZE(len);

    if (!(new_block = allocate_block(
              max(min_size, align_len), min_result_data_size == 0,
    ...
    DBUG_PRINT("qcache",
               ("Block len %lu used %lu", new_block->length, new_block->used));
...
void Query_cache::invalidate_table(TABLE_LIST *table_list) {
  if (table_list->table != 0)
    ...
    Query_cache_block *table_block;
    key_length =
        (uint)(strmov(strmov(key, table_list->db) + 1, table_list->table_name) -
               key) +
        1;
...
Query_cache::register_tables_from_list(TABLE_LIST *tables_used,
  ...
  for (n = counter; tables_used;
    ...
    if (tables_used->view) {
      char key[MAX_DBKEY_LENGTH];
      uint key_length;
      DBUG_PRINT("qcache", ("view %s, db %s", tables_used->view_name.str,
      ...
      */
      if (!insert_table(key_length, key, block_table,
        ...
        DBUG_RETURN(0);
      {
        TABLE_COUNTER_TYPE inc = register_tables_from_list(
            tables_used->ancestor, n + 1, block_table + 1);
        if (!inc)
          DBUG_RETURN(0);
        n += inc;
        block_table += inc;
      ...
      DBUG_PRINT("qcache",
                 ("table %s, db %s, openinfo at 0x%lx, keylen %u, key at 0x%lx",
                  ...
                  (ulong)tables_used->table, tables_used->table->s->key_length,
      ...
      if (tables_used->table->s->db_type == DB_TYPE_MRG_MYISAM) {
        ...
        for (MYRG_TABLE *table = file->open_tables; table != file->end_table;
             table++) {
          char key[MAX_DBKEY_LENGTH];
          uint32 db_length;
          uint key_length =
              filename_2_table_key(key, table->table->filename, &db_length);
          (++block_table)->n = ++n;
          /*
            There are not callback function for for MyISAM, and engine data
          */
          if (!insert_table(key_length, key, block_table, db_length,
                            tables_used->table->file->table_cache_type(), 0, 0))
            DBUG_RETURN(0);
        }
      }
    }
...
my_bool Query_cache::register_all_tables(Query_cache_block *block,
  ...
  TABLE_COUNTER_TYPE n;
  DBUG_PRINT("qcache",
             ("register tables block 0x%lx, n %d, header %x", (ulong)block,
              (int)tables_arg, (int)ALIGN_SIZE(sizeof(Query_cache_block))));
  ...
  if (n) {
    ...
    for (Query_cache_block_table *tmp = block->table(0); tmp != block_table;
         tmp++)
      unlink_table(tmp);
  }
  return (n);
...
my_bool Query_cache::insert_table(uint key_len, char *key,
  ...
  DBUG_PRINT("qcache",
             ("insert table node 0x%lx, len %d", (ulong)node, key_len));

  Query_cache_block *table_block =
      ((Query_cache_block *)hash_search(&tables, (byte *)key, key_len));

  if (table_block && table_block->table()->engine_data() != engine_data) {
    DBUG_PRINT("qcache",
               ("Handler require invalidation queries of %s.%s %lld-%lld",
                table_block->table()->db(), table_block->table()->table(),
                engine_data, table_block->table()->engine_data()));
    /*
  ...
  if (table_block == 0) {
    DBUG_PRINT("qcache",
               ("new table block from 0x%lx (%u)", (ulong)key, (int)key_len));
    table_block = write_block_data(key_len, (gptr)key,
                                   ALIGN_SIZE(sizeof(Query_cache_table)),
                                   Query_cache_block::TABLE, 1, 1);
    if (table_block == 0) {
...
Query_cache_block *Query_cache::allocate_block(ulong len, my_bool not_less,
                                               ulong min, my_bool under_guard) {
  DBUG_ENTER("Query_cache::allocate_block");
  DBUG_PRINT("qcache", ("len %lu, not less %d, min %lu, uder_guard %d", len,
                        not_less, min, under_guard));
...
Query_cache_block *Query_cache::get_free_block(ulong len, my_bool not_less,
                                               ulong min) {
  Query_cache_block *block = 0, *first = 0;
  DBUG_ENTER("Query_cache::get_free_block");
  DBUG_PRINT("qcache",
             ("length %lu, not_less %d, min %lu", len, (int)not_less, min));
...
void Query_cache::split_block(Query_cache_block *block, ulong len) {
  ...
  DBUG_PRINT("qcache", ("split 0x%lx (%lu) new 0x%lx", (ulong)block, len,
                        (ulong)new_block));
  DBUG_VOID_RETURN;
}
...
Query_cache::join_free_blocks(Query_cache_block *first_block_arg,
  ...
  DBUG_PRINT("qcache", ("join first 0x%lx, pnext 0x%lx, in list 0x%lx",
...
void Query_cache::exclude_from_free_memory_list(Query_cache_block *free_block) {
  ...
  Query_cache_memory_bin *bin =
      *((Query_cache_memory_bin **)free_block->data());
  double_linked_list_exclude(free_block, &bin->free_blocks);
  bin->number--;
  free_memory -= free_block->length;
  free_memory_blocks--;
  DBUG_PRINT("qcache",
             ("exclude block 0x%lx, bin 0x%lx", (ulong)free_block, (ulong)bin));
  DBUG_VOID_RETURN;
}
...
void Query_cache::insert_into_free_memory_list(Query_cache_block *free_block) {
  ...
  *bin_ptr = bins + idx;
  (*bin_ptr)->number++;
  DBUG_PRINT("qcache", ("insert block 0x%lx, bin[%d] 0x%lx", (ulong)free_block,
                        idx, (ulong) * bin_ptr));
  DBUG_VOID_RETURN;
}
...
uint Query_cache::find_bin(ulong size) {
  ...
  bins_dump();
#endif
  DBUG_PRINT("qcache", ("bin %u step %u, size %lu step size %lu", bin, left,
                        size, steps[left].size));
  DBUG_RETURN(bin);
}
...
void
Query_cache::insert_into_free_memory_sorted_list(Query_cache_block *new_block,
...
void Query_cache::double_linked_list_simple_include(
    Query_cache_block *point, Query_cache_block **list_pointer) {
...
void Query_cache::double_linked_list_exclude(Query_cache_block *point,
                                             Query_cache_block **list_pointer) {
  DBUG_ENTER("Query_cache::double_linked_list_exclude");
  DBUG_PRINT("qcache", ("excluding block 0x%lx, list 0x%lx", (ulong)point,
                        (ulong)list_pointer));
  if (point->next == point)
      ...
      *tables_type |= HA_CACHE_TBL_NONTRANSACT;
      {
        ...
        if (!(subcount =
                  process_and_count_tables(tables_used->ancestor, tables_type)))
          DBUG_RETURN(0);
        table_count += subcount;
      }
    } else {
      DBUG_PRINT("qcache",
                 ("table %s, db %s, type %u", tables_used->table->s->table_name,
                  tables_used->table->s->table_cache_key,
...
TABLE_COUNTER_TYPE Query_cache::is_cacheable(THD *thd, uint32 query_len,
  ...
  if (lex->sql_command == SQLCOM_SELECT &&
    ...
    DBUG_PRINT("qcache",
               ("options %lx %lx, type %u", OPTION_TO_QUERY_CACHE,
                lex->select_lex.options, (int)thd->variables.query_cache_type));
  ...
  DBUG_PRINT(
      ...
      ("not interesting query: %d or not cacheable, options %lx %lx, type %u",
       (int)lex->sql_command, OPTION_TO_QUERY_CACHE, lex->select_lex.options,
...
my_bool Query_cache::ask_handler_allowance(THD *thd, TABLE_LIST *tables_used) {
  ...
  for (; tables_used; tables_used = tables_used->next_global) {
    ...
    if (!handler->register_query_cache_table(
             thd, table->s->table_cache_key, table->s->key_length,
             &tables_used->callback_func, &tables_used->engine_data)) {
      DBUG_PRINT("qcache", ("Handler does not allow caching for %s.%s",
...
my_bool Query_cache::move_by_type(byte **border, Query_cache_block **before,
                                  ulong *gap, Query_cache_block *block) {
  ...
  case Query_cache_block::TABLE: {
    ...
    Query_cache_block *prev = block->prev, *next = block->next,
    ...
    *before = new_block;
    ...
    DBUG_PRINT("qcache", ("moved %lu bytes to 0x%lx, new gap at 0x%lx", len,
                          (ulong)new_block, (ulong) * border));
    break;
  }
  case Query_cache_block::QUERY: {
    ...
    Query_cache_block *first_result_block =
        ((Query_cache_query *)block->data())->result();
    byte *key;
    uint key_length;
  ...
  case Query_cache_block::RESULT: {
    DBUG_PRINT("qcache",
               ("block 0x%lx RES* (%d)", (ulong)block, (int)block->type));
    if (*border == 0)
      break;
    ...
    BLOCK_LOCK_WR(query_block);
    ulong len = block->length, used = block->used;
    Query_cache_block *pprev = block->pprev, *pnext = block->pnext,
                      *new_block = (Query_cache_block *)*border;
    char *data = (char *)block->data();
    block->destroy();
    new_block->init(len);
...
void Query_cache::relink(Query_cache_block *oblock, Query_cache_block *nblock,
    ...
    do {
      ...
      if (header->result() != 0 &&
        ...
        if (new_result_block != 0) {
          ...
          ulong new_len =
              (header->length() + ALIGN_SIZE(sizeof(Query_cache_block)) +
               ALIGN_SIZE(sizeof(Query_cache_result)));
          ...
          DBUG_PRINT("qcache",
                     ("new block %lu/%lu (%lu)", new_result_block->length,
                      new_result_block->used, header->length()));
          ...
          do {
            ulong len = (result_block->used - result_block->headers_len() -
                         ALIGN_SIZE(sizeof(Query_cache_result)));
            DBUG_PRINT("loop", ("add block %lu/%lu (%lu)", result_block->length,
                                result_block->used, len));
            memcpy((char *)write_to, (char *)result_block->result()->data(),
                   len);
            write_to += len;
...
uint Query_cache::filename_2_table_key(char *key, const char *path,
  ...
  *db_length = (filename - dbname) - 1;
  ...
  DBUG_RETURN(
      (uint)(strmov(strmake(key, dbname, *db_length) + 1, filename) - key) + 1);
}
...
void Query_cache::bins_dump() {
  ...
  DBUG_PRINT("qcache",
             ("mem_bin_num=%u, mem_bin_steps=%u", mem_bin_num, mem_bin_steps));
  DBUG_PRINT("qcache", ("-------------------------"));
  ...
  for (i = 0; i < mem_bin_num; i++) {
    ...
    if (bins[i].free_blocks) {
      ...
      do {
        DBUG_PRINT("qcache",
                   ("\\-- %lu 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", block->length,
                    (ulong)block, (ulong)block->next, (ulong)block->prev,
...
void Query_cache::cache_dump() {
  ...
  do {
    DBUG_PRINT("qcache",
               ("%10lu %10lu %1d %2d 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", i->length,
                i->used, (int)i->type, i->n_tables, (ulong)i, (ulong)i->next,
                (ulong)i->prev, (ulong)i->pnext, (ulong)i->pprev));
    i = i->pnext;
  } while (i != first_block);
  DBUG_PRINT("qcache", ("-------------------------------------"));
}
...
void Query_cache::queries_dump() {
  ...
  if (queries_blocks != 0) {
    ...
    do {
      ...
      DBUG_PRINT("qcache",
                 ("F:%u C:%u L:%lu T:'%s' (%u) '%s' '%s'",
                  flags.client_long_flag, flags.character_set_client_num,
                  (ulong)flags.limit, flags.time_zone->get_name(), len, str,
                  strend(str) + 1));
        ...
        do {
          DBUG_PRINT("qcache",
                     ("-r- %u %lu/%lu 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx",
                      (uint)result_block->type, result_block->length,
                      result_block->used, (ulong)result_block,
                      (ulong)result_block->next, (ulong)result_block->prev,
...
my_bool Query_cache::check_integrity(bool not_locked) {
  ...
  do {
    DBUG_PRINT("qcache",
               ("block 0x%lx, type %u...", (ulong)block, (uint)block->type));
    // Check allignment
    if ((((long)block) % (long)ALIGN_SIZE(1)) !=
        (((long)first_block) % (long)ALIGN_SIZE(1))) {
      DBUG_PRINT("error", ("block 0x%lx do not aligned by %d", (ulong)block,
                           ALIGN_SIZE(1)));
      result = 1;
    }
    ...
    case Query_cache_block::FREE: {
      ...
      if (((byte *)bin) < ((byte *)bins) ||
        ...
        DBUG_PRINT("error",
                   ("free block 0x%lx have bin pointer 0x%lx beyaond of bins "
                    "array bounds [0x%lx,0x%lx]",
                    (ulong)block, (ulong)bin, (ulong)bins, (ulong)first_block));
        result = 1;
      } else {
        int idx =
            (((byte *)bin) - ((byte *)bins)) / sizeof(Query_cache_memory_bin);
    ...
    case Query_cache_block::QUERY: {
      ...
      for (TABLE_COUNTER_TYPE j = 0; j < block->n_tables; j++) {
        ...
        Query_cache_block_table *block_table_root =
            (Query_cache_block_table *)(((byte *)block_table->parent) -
                                        ALIGN_SIZE(
    ...
    case Query_cache_block::RESULT: {
      ...
      if (((byte *)query_block) < ((byte *)first_block) ||
        ...
        DBUG_PRINT("error",
                   ("result block 0x%lx have query block pointer 0x%lx beyaond "
    ...
    default:
      DBUG_PRINT("error",
                 ("block 0x%lx have incorrect type %u", block, block->type));
      result = 1;
    }
  ...
  if (free != free_memory) {
    DBUG_PRINT("error",
               ("free memory (%lu) != free_memory (%lu)", free, free_memory));
    result = 1;
  }
  ...
  if ((block = queries_blocks)) {
    do {
      ...
      if (results) {
        ...
        do {
          DBUG_PRINT("qcache", ("block 0x%lx, type %u...", (ulong)block,
                                (uint)block->type));
...
my_bool Query_cache::in_blocks(Query_cache_block *point) {
  ...
  do {
    if (block->pprev->pnext != block) {
      DBUG_PRINT("error",
                 ("block 0x%lx in physical list is incorrect linked, prev "
                  "block 0x%lx refered as next to 0x%lx (check from 0x%lx)",
                  (ulong)block, (ulong)block->pprev, (ulong)block->pprev->pnext,
                  (ulong)point));
      // back trace
...
err1:
  ...
  do {
    if (block->pnext->pprev != block) {
      DBUG_PRINT("error",
                 ("block 0x%lx in physicel list is incorrect linked, next "
                  "block 0x%lx refered as prev to 0x%lx (check from 0x%lx)",
                  (ulong)block, (ulong)block->pnext, (ulong)block->pnext->pprev,
                  (ulong)point));
      // back trace
    ...
    if (block->prev->next != block) {
      DBUG_PRINT("error",
                 ("block 0x%lx in list '%s' 0x%lx is incorrect linked, prev "
                  ...
                  (ulong)block->prev->next, (ulong)point));
      // back trace
  ...
  if (block != root) {
    DBUG_PRINT("error",
               ("block 0x%lx (0x%lx<-->0x%lx) not owned by list '%s' 0x%lx",
                (ulong)block, (ulong)block->prev, (ulong)block->next, name,
                (ulong)root));
    return 1;
  }
err1:
    ...
    if (block->next->prev != block) {
      DBUG_PRINT("error",
                 ("block 0x%lx in list '%s' 0x%lx is incorrect linked, next "
                  ...
                  (ulong)block->next->prev, (ulong)point));
      // back trace
...
void dump_node(Query_cache_block_table *node, const char *call,
               const char *descr) {
  DBUG_PRINT("qcache", ("%s: %s: node: 0x%lx", call, descr, (ulong)node));
  DBUG_PRINT("qcache",
             ("%s: %s: node block: 0x%lx", call, descr, (ulong)node->block()));
...
my_bool Query_cache::in_table_list(Query_cache_block_table *root,
  ...
  do {
    ...
    if (table->prev->next != table) {
      DBUG_PRINT("error",
                 ("table 0x%lx(0x%lx) in list '%s' 0x%lx(0x%lx) is incorrect "
                  "linked, prev table 0x%lx(0x%lx) refered as next to "
                  "0x%lx(0x%lx) (check from 0x%lx(0x%lx))",
                  (ulong)table, (ulong)table->block(), name, (ulong)root,
                  (ulong)root->block(), (ulong)table->prev,
                  (ulong)table->prev->block(), (ulong)table->prev->next,
                  (ulong)table->prev->next->block(), (ulong)point,
                  (ulong)point->block()));
      // back trace
      for (; table != point; table = table->next)
        DBUG_PRINT("error", ("back trace 0x%lx(0x%lx)", (ulong)table,
                             (ulong)table->block()));
      result = 1;
      goto err1;
    }
  ...
  if (table != root) {
    DBUG_PRINT("error", ("table 0x%lx(0x%lx) (0x%lx(0x%lx)<-->0x%lx(0x%lx)) "
                         ...
                         (ulong)table->next, (ulong)table->next->block(), name,
    ...
    if (table->next->prev != table) {
      DBUG_PRINT("error",
                 ("table 0x%lx(0x%lx) in list '%s' 0x%lx(0x%lx) is incorrect "
                  ...
                  "0x%lx(0x%lx) (check from 0x%lx(0x%lx))",
                  (ulong)table, (ulong)table->block(), name, (ulong)root,
                  (ulong)root->block(), (ulong)table->next,
                  (ulong)table->next->block(), (ulong)table->next->prev,
                  (ulong)table->next->prev->block(), (ulong)point,
                  (ulong)point->block()));
      // back trace
