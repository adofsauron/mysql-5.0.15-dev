...
String *Item_func_aes_decrypt::val_str(String *str) {
  ...
  if (sptr && key) // Need to have both arguments not NULL
  {
    ...
    if (!str_value.alloc(sptr->length())) // Ensure that memory is free
    {
      ...
      int length;
      length =
          my_aes_decrypt(sptr->ptr(), sptr->length(), (char *)str_value.ptr(),
                         key->ptr(), key->length());
  ...
  for (i = 1; i < arg_count; i++) {
    if (res->length() == 0) {
      ...
      if (res->alloced_length() >=
          res->length() + res2->length()) { // Use old buffer
        res->append(*res2);
        ...
        if (res->append(*res2)) // Must be a blob
          goto null;
      } else if (res2 == &tmp_value) { // This can happend only 1 time
        ...
        */
        ...
        res = &tmp_value;
        use_as_buff = str; // Put next arg here
      } else {             // Two big const strings
        if (tmp_value.alloc(max_length) || tmp_value.copy(*res) ||
...
String *Item_func_des_encrypt::val_str(String *str) {
  ...
  if (arg_count == 1) {
    ...
    EVP_BytesToKey(EVP_des_ede3_cbc(), EVP_md5(), NULL, (uchar *)keystr->ptr(),
  ...
  */
  ...
  DES_ede3_cbc_encrypt(
      (const uchar *)(res->ptr()), (uchar *)(tmp_value.ptr() + 1), res_length,
      &keyschedule.ks1, &keyschedule.ks2, &keyschedule.ks3, &ivec, TRUE);
  return &tmp_value;

error:
  push_warning_printf(current_thd, MYSQL_ERROR::WARN_LEVEL_ERROR, code,
                      ER(code), "des_encrypt");
#else
...
String *Item_func_des_decrypt::val_str(String *str) {
  ...
  DES_ede3_cbc_encrypt((const uchar *)res->ptr() + 1,
                       (uchar *)(tmp_value.ptr()), length - 1, &keyschedule.ks1,
...
String *Item_func_concat_ws::val_str(String *str) {
  ...
  for (i++; i < arg_count; i++) {
    ...
    if (res->alloced_length() >=
        res->length() + sep_str->length() + res2->length()) { // Use old buffer
      res->append(*sep_str); // res->length() > 0 always
      res->append(*res2);
    } else if (str->alloced_length() >=
               res->length() + sep_str->length() + res2->length()) {
      /* We have room in str;  We can't get any errors here */
      if (str == res2) { // This is quote uncommon!
      ...
      */
      ...
      if (tmp_value.alloc(max_length) || tmp_value.copy(*res) ||
          tmp_value.append(*sep_str) || tmp_value.append(*res2))
        goto null;
      res = &tmp_value;
      use_as_buff = str;
    }
...
#ifdef USE_MB
  binary_cmp =
      ((res->charset()->state & MY_CS_BINSORT) || !use_mb(res->charset()));
#endif
...
void Item_func_replace::fix_length_and_dec() {
  ...
  if (diff > 0 && args[1]->max_length) { // Calculate of maxreplaces
...
void Item_func_insert::fix_length_and_dec() {
  ...
  args[3] = cargs[1];
  max_result_length =
      ((ulonglong)args[0]->max_length + (ulonglong)args[3]->max_length);
  if (max_result_length >= MAX_BLOB_WIDTH) {
    max_result_length = MAX_BLOB_WIDTH;
    maybe_null = 1;
  }
...
#endif /* USE_MB */
  {
    if (count > 0) { // start counting from the beginning
...
String *Item_func_encrypt::val_str(String *str) {
  ...
  if (arg_count == 1) { // generate random salt
    time_t timestamp = current_thd->query_start();
    salt[0] = bin_to_ascii((ulong)timestamp & 0x3f);
    salt[1] = bin_to_ascii(((ulong)timestamp >> 5) & 0x3f);
    salt[2] = 0;
    salt_ptr = salt;
  } else { // obtain salt from the first two bytes
...
Item *Item_func_sysconst::safe_charset_converter(CHARSET_INFO *tocs) {
  ...
  if (conv_errors ||
      !(conv = new Item_static_string_func(
            fully_qualified_func_name(), cstr.ptr(), cstr.length(),
...
String *Item_func_char::val_str(String *str) {
  ...
  uint wlen =
      cs->cset->well_formed_len(cs, str->ptr(), str->ptr() + str->length(),
...
void Item_func_repeat::fix_length_and_dec() {
  ...
  if (args[1]->const_item()) {
    ulonglong max_result_length =
        ((ulonglong)args[0]->max_length * args[1]->val_int());
    if (max_result_length >= MAX_BLOB_WIDTH) {
      max_result_length = MAX_BLOB_WIDTH;
      maybe_null = 1;
    }
...
void Item_func_rpad::fix_length_and_dec() {
  ...
  if (args[1]->const_item()) {
    ulonglong length =
        ((ulonglong)args[1]->val_int() * collation.collation->mbmaxlen);
...
String *Item_func_rpad::val_str(String *str) {
  ...
  if (!res || args[1]->null_value || !rpad || count < 0)
    goto err;
  null_value = 0;
  if (count <= (int32)(res_char_length =
                           res->numchars())) { // String to pad is big enough
...
String *Item_func_conv::val_str(String *str) {
  ...
  if (args[0]->null_value || args[1]->null_value || args[2]->null_value ||
      abs(to_base) > 36 || abs(to_base) < 2 || abs(from_base) > 36 ||
      abs(from_base) < 2 || !(res->length())) {
    null_value = 1;
    return 0;
  }
  null_value = 0;
...
String *Item_func_uncompress::val_str(String *str) {
  ...
  if ((err = uncompress((Byte *)buffer.ptr(), &new_size,
                        ((const Bytef *)res->ptr()) + 4, res->length())) ==
      Z_OK) {
