...
static MYSQL_LOCK *get_lock_data(THD *thd, TABLE **table, uint count, bool unlock, TABLE **write_locked);
...
static int thr_lock_errno_to_mysql[] = {0, 1, ER_LOCK_WAIT_TIMEOUT, ER_LOCK_DEADLOCK};

MYSQL_LOCK *mysql_lock_tables(THD *thd, TABLE **tables, uint count, uint flags, bool *need_reopen)
{
  MYSQL_LOCK *sql_lock;
  ...
  *need_reopen = FALSE;
  ...
  for (;;)
  {
    ...
    thd->locked = 1;
    rc = thr_lock_errno_to_mysql[(int)thr_multi_lock(sql_lock->locks, sql_lock->lock_count, thd->lock_id)];
...
static int lock_external(THD *thd, TABLE **tables, uint count)
{
  ...
  for (i = 1; i <= count; i++, tables++)
  {
    ...
    if ((*tables)->db_stat & HA_READ_ONLY ||
        ((*tables)->reginfo.lock_type >= TL_READ && (*tables)->reginfo.lock_type <= TL_READ_NO_INSERT))
      lock_type = F_RDLCK;
...
void mysql_lock_remove(THD *thd, MYSQL_LOCK *locked, TABLE *table)
{
  ...
  if (locked)
  {
    ...
    for (i = 0; i < locked->table_count; i++)
    {
      ...
      {
        locked->table_count--;
        bmove((char *)(locked->table + i), (char *)(locked->table + i + 1),
...
void mysql_lock_abort(THD *thd, TABLE *table)
{
  ...
  {
    for (uint i = 0; i < locked->lock_count; i++) thr_abort_locks(locked->locks[i]->lock);
...
MYSQL_LOCK *mysql_lock_merge(MYSQL_LOCK *a, MYSQL_LOCK *b)
{
  ...
  if (!(sql_lock =
            (MYSQL_LOCK *)my_malloc(sizeof(*sql_lock) + sizeof(THR_LOCK_DATA *) * (a->lock_count + b->lock_count) +
  ...
  memcpy(sql_lock->locks + a->lock_count, b->locks, b->lock_count * sizeof(*b->locks));
  memcpy(sql_lock->table, a->table, a->table_count * sizeof(*a->table));
  memcpy(sql_lock->table + a->table_count, b->table, b->table_count * sizeof(*b->table));
...
static MYSQL_LOCK *get_lock_data(THD *thd, TABLE **table_ptr, uint count, bool get_old_locks, TABLE **write_lock_used)
{
  ...
  *write_lock_used = 0;
  for (i = tables = lock_count = 0; i < count; i++)
  {
    ...
    */
    if (table_ptr[i]->s->system_table && table_ptr[i]->reginfo.lock_type >= TL_WRITE_ALLOW_WRITE && count != 1)
    {
      my_error(ER_WRONG_LOCK_OF_SYSTEM_TABLE, MYF(0), table_ptr[i]->s->db, table_ptr[i]->s->table_name);
      return 0;
    }
  }

  if (!(sql_lock = (MYSQL_LOCK *)my_malloc(
            sizeof(*sql_lock) + sizeof(THR_LOCK_DATA *) * tables + sizeof(table_ptr) * lock_count, MYF(0))))
    return 0;
  ...
  for (i = 0; i < count; i++)
  {
    ...
    *to++ = table;
    ...
    locks = table->file->store_lock(thd, locks, get_old_locks ? TL_IGNORE : lock_type);
    if (locks)
      for (; org_locks != locks; org_locks++) (*org_locks)->debug_print_param = (void *)table;
  }
  return sql_lock;
}
...
int lock_table_name(THD *thd, TABLE_LIST *table_list)
{
  ...
  key_length = (uint)(strmov(strmov(key, db) + 1, table_list->table_name) - key) + 1;
  ...
  */
  ...
  DBUG_RETURN(test(remove_table_from_cache(thd, db, table_list->table_name, RTFC_NO_FLAG)));
}
...
{
  for (TABLE_LIST *table = table_list; table != last_table; table = table->next_local) unlock_table_name(thd, table);
...
bool lock_global_read_lock(THD *thd)
{
  ...
  if (!thd->global_read_lock)
  {
    ...
    const char *old_message = thd->enter_cond(&COND_refresh, &LOCK_global_read_lock, "Waiting to get readlock");
    DBUG_PRINT("info",
               ("waiting_for: %d  protect_against: %d", waiting_for_read_lock, protect_against_global_read_lock));

    waiting_for_read_lock++;
    while (protect_against_global_read_lock && !thd->killed) pthread_cond_wait(&COND_refresh, &LOCK_global_read_lock);
    waiting_for_read_lock--;
    if (thd->killed)
    {
...
#define must_wait (global_read_lock && (is_not_commit || global_read_lock_blocks_commit))
...
bool make_global_read_lock_block_commit(THD *thd)
{
  ...
  */
  ...
  DBUG_EXECUTE_IF("make_global_read_lock_block_commit_loop", protect_against_global_read_lock++;);
  old_message = thd->enter_cond(&COND_refresh, &LOCK_global_read_lock, "Waiting for all running commits to finish");
  while (protect_against_global_read_lock && !thd->killed) pthread_cond_wait(&COND_refresh, &LOCK_global_read_lock);
  DBUG_EXECUTE_IF("make_global_read_lock_block_commit_loop", protect_against_global_read_lock--;);
