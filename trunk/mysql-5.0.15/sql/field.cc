...
{
  return (field_type < FIELDTYPE_TEAR_FROM ? field_type
...
enum_field_types Field::field_type_merge(enum_field_types a, enum_field_types b)
{
...
{
  DBUG_ASSERT(field_type < FIELDTYPE_TEAR_FROM || field_type > FIELDTYPE_TEAR_TO);
...
*/
Field_num::Field_num(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
                     const char *field_name_arg, struct st_table *table_arg, uint8 dec_arg, bool zero_arg,
                     bool unsigned_arg)
    : Field(ptr_arg, len_arg, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg, table_arg),
      dec(dec_arg),
      zerofill(zero_arg),
...
void Field_num::prepend_zeros(String *value)
{
  ...
  {
    bmove_upp((char *)value->ptr() + field_length, value->ptr() + value->length(), value->length());
...
bool Field::check_int(const char *str, int length, const char *int_end, CHARSET_INFO *cs)
{
  const char *end;
  if (str == int_end)
  {
    ...
    push_warning_printf(table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "integer", tmp.c_ptr(), field_name,
                        (ulong)table->in_use->row_count);
    return 1;  // Empty string
  }
  end = str + length;
  if ((str = int_end) == end) return 0;  // OK; All digits was used
...
static bool test_if_real(const char *str, int length, CHARSET_INFO *cs)
{
  ...
  if (*str == '+' || *str == '-')
  {
    length--;
    str++;
    if (!length || !(my_isdigit(cs, *str) || *str == '.')) return 0;
  }
  ...
  if (*str == 'E' || *str == 'e')
  {
    if (length < 3 || (str[1] != '+' && str[1] != '-') || !my_isdigit(cs, str[2])) return 0;
    length -= 3;
    str += 3;
    while (length && my_isdigit(cs, *str))
    {
...
String *Field::val_int_as_str(String *val_buffer, my_bool unsigned_val)
{
  ...
  if (val_buffer->alloc(length)) return 0;
  length = (uint)(*cs->cset->longlong10_to_str)(cs, (char *)val_buffer->ptr(), length, unsigned_val ? 10 : -10, value);
...
Field::Field(char *ptr_arg, uint32 length_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
...
uint Field::offset() { return (uint)(ptr - (char *)table->record[0]); }
...
{
  if (unsigned_flag) res.append(" unsigned");
  if (zerofill) res.append(" zerofill");
}
...
*/
longlong Field::convert_decimal2longlong(const my_decimal *val, bool unsigned_flag, int *err)
{
  longlong i;
  if (unsigned_flag)
  {
    ...
    else if (warn_if_overflow(my_decimal2int(E_DEC_ERROR & ~E_DEC_OVERFLOW & ~E_DEC_TRUNCATED, val, TRUE, &i)))
    {
      i = ~(longlong)0;
      *err = 1;
    }
  }
  else if (warn_if_overflow(my_decimal2int(E_DEC_ERROR & ~E_DEC_OVERFLOW & ~E_DEC_TRUNCATED, val, FALSE, &i)))
  {
...
Field_str::Field_str(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, utype unireg_check_arg,
                     const char *field_name_arg, struct st_table *table_arg, CHARSET_INFO *charset)
    : Field(ptr_arg, len_arg, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg, table_arg)
{
  field_charset = charset;
  if (charset->state & MY_CS_BINSORT) flags |= BINARY_FLAG;
}
...
int Field_str::store_decimal(const my_decimal *d)
{
  ...
  int err = warn_if_overflow(my_decimal2double(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, d, &val));
...
uint Field::fill_cache_field(CACHE_FIELD *copy)
{
  ...
  else if (!zero_pack() && (type() == MYSQL_TYPE_STRING && copy->length >= 4 && copy->length < 256))
  {
...
bool Field::get_time(TIME *ltime)
{
  char buff[40];
  String tmp(buff, sizeof(buff), &my_charset_bin), *res;
  if (!(res = val_str(&tmp)) || str_to_time_with_warn(res->ptr(), res->length(), ltime)) return 1;
  return 0;
}
...
bool Field::optimize_range(uint idx, uint part) { return test(table->file->index_flags(idx, part, 1) & HA_READ_RANGE); }

Field *Field::new_field(MEM_ROOT *root, struct st_table *new_table)
{
  Field *tmp;
  if (!(tmp = (Field *)memdup_root(root, (char *)this, size_of()))) return 0;

  if (tmp->table->maybe_null) tmp->flags &= ~NOT_NULL_FLAG;
  tmp->table = new_table;
...
Field *Field::new_key_field(MEM_ROOT *root, struct st_table *new_table, char *new_ptr, uchar *new_null_ptr,
                            uint new_null_bit)
{
  Field *tmp;
...
bool Field::quote_data(String *unquoted_string)
{
  ...
  if (!needs_quotes()) DBUG_RETURN(0);

  // this is the same call that mysql_real_escape_string() calls
  if (escape_string_for_mysql(&my_charset_bin, (char *)escaped_string, sizeof(escaped_string), unquoted_string->ptr(),
                              unquoted_string->length()) == (ulong)~0)
    DBUG_RETURN(1);

  // reset string, then re-append with quotes and escaped values
  unquoted_string->length(0);
  if (unquoted_string->append('\'') || unquoted_string->append((char *)escaped_string) || unquoted_string->append('\''))
    DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
void Field_null::sql_type(String &res) const { res.set_ascii("null", 4); }
...
void Field_decimal::reset(void) { Field_decimal::store("0", 1, &my_charset_bin); }
...
*/
  ...
  else  // (expo_sign_char=='+')
  {
    ...
    */
    if (int_digits_from == int_digits_end)
    {
      ...
      */
      for (; int_digits_tail_from != frac_digits_from && *int_digits_tail_from == '0'; int_digits_tail_from++)
        ;
    ...
    tmp_uint = (uint)(tmp_dec + (int_digits_end - int_digits_from) + (uint)(frac_digits_from - int_digits_tail_from) +
  ...
  */
  ...
  else
  {
    ...
    if (pos == tmp_right_pos - 1) *pos-- = '0';  // no 0 has ever been written, so write one
    left_wall = to - 1;
  ...
  if (expo_sign_char == '-')
  {
    ...
    while (int_digits_end != frac_digits_head_end)
    {
      ...
      if (pos == right_wall)
      {
        ...
        {
          if (!is_cuted_fields_incr) set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
          return 0;
        }
        continue;
      }
...
double Field_decimal::val_real(void)
{
  int not_used;
  char *end_not_used;
  return my_strntod(&my_charset_bin, ptr, field_length, &end_not_used, &not_used);
}
...
longlong Field_decimal::val_int(void)
{
  int not_used;
  if (unsigned_flag)
    return my_strntoull(&my_charset_bin, ptr, field_length, 10, NULL, &not_used);
  else
    return my_strntoll(&my_charset_bin, ptr, field_length, 10, NULL, &not_used);
}

String *Field_decimal::val_str(String *val_buffer __attribute__((unused)), String *val_ptr)
{
  char *str;
  for (str = ptr; *str == ' '; str++)
    ;
...
int Field_decimal::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  for (end = a_ptr + field_length;
       a_ptr != end && (*a_ptr == *b_ptr || ((my_isspace(&my_charset_bin, *a_ptr) || *a_ptr == '+' || *a_ptr == '0') &&
                                             (my_isspace(&my_charset_bin, *b_ptr) || *b_ptr == '+' || *b_ptr == '0')));
       a_ptr++, b_ptr++)
  {
  ...
  while (a_ptr != end)
  {
    if (*a_ptr++ != *b_ptr++) return swap ^ (a_ptr[-1] < b_ptr[-1] ? -1 : 1);  // compare digits
  }
  return 0;
}
...
{
  char *str, *end;
  for (str = ptr, end = ptr + length; str != end && ((my_isspace(&my_charset_bin, *str) || *str == '+' || *str == '0'));
       str++)
    *to++ = ' ';
  if (str == end) return; /* purecov: inspected */
...
void Field_decimal::sql_type(String &res) const
{
  ...
  if (dec) tmp--;
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "decimal(%d,%d)", tmp, dec));
...
Field_new_decimal::Field_new_decimal(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
                                     enum utype unireg_check_arg, const char *field_name_arg,
                                     struct st_table *table_arg, uint8 dec_arg, bool zero_arg, bool unsigned_arg)
                ...
                zero_arg, unsigned_arg)
{
  ...
  DBUG_ASSERT((precision <= DECIMAL_MAX_PRECISION) && (dec <= DECIMAL_MAX_SCALE));
  bin_size = my_decimal_get_binary_size(precision, dec);
}

Field_new_decimal::Field_new_decimal(uint32 len_arg, bool maybe_null, const char *name, struct st_table *t_arg,
                                     uint8 dec_arg, bool unsigned_arg)
    : Field_num((char *)0, len_arg, maybe_null ? (uchar *)"" : 0, 0, NONE, name, t_arg, dec_arg, 0, unsigned_arg)
{
  precision = my_decimal_length_to_precision(len_arg, dec_arg, unsigned_arg);
  DBUG_ASSERT((precision <= DECIMAL_MAX_PRECISION) && (dec <= DECIMAL_MAX_SCALE));
  bin_size = my_decimal_get_binary_size(precision, dec);
...
void Field_new_decimal::reset(void) { store_value(&decimal_zero); }
...
void Field_new_decimal::set_value_on_overflow(my_decimal *decimal_value, bool sign)
{
...
bool Field_new_decimal::store_value(const my_decimal *decimal_value)
{
  ...
  {
    char dbug_buff[DECIMAL_MAX_STR_LENGTH + 1];
    DBUG_PRINT("info", ("saving with precision %d, scale: %d, value %s", (int)precision, (int)dec,
                        dbug_decimal_as_string(dbug_buff, decimal_value)));
  }
#endif

  if (warn_if_overflow(my_decimal2binary(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, decimal_value, ptr, precision, dec)))
  {
    my_decimal buff;
...
int Field_new_decimal::store(const char *from, uint length, CHARSET_INFO *charset)
{
  int err;
  my_decimal decimal_value;
  DBUG_ENTER("Field_new_decimal::store(char*)");

  if ((err = str2my_decimal(E_DEC_FATAL_ERROR & ~(E_DEC_OVERFLOW | E_DEC_BAD_NUM), from, length, charset,
                            &decimal_value)) &&
      table->in_use->abort_on_warning)
  {
    push_warning_printf(table->in_use, MYSQL_ERROR::WARN_LEVEL_ERROR, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "decimal", from, field_name,
                        (ulong)table->in_use->row_count);
    DBUG_RETURN(err);
  }
...
int Field_new_decimal::store(double nr)
{
  ...
  err = double2my_decimal(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, nr, &decimal_value);
  /*
  ...
  */
  if (!err)
  {
    ...
    if (nr2 != nr) err = E_DEC_TRUNCATED;
  }
  if (err)
  {
    if (check_overflow(err)) set_value_on_overflow(&decimal_value, decimal_value.sign());
...
int Field_new_decimal::store(longlong nr, bool unsigned_val)
{
  ...
  if ((err = int2my_decimal(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, nr, unsigned_val, &decimal_value)))
  {
    if (check_overflow(err)) set_value_on_overflow(&decimal_value, decimal_value.sign());
    /* Only issue a warning if store_value doesn't issue an warning */
    table->in_use->got_warning = 0;
...
int Field_new_decimal::store_decimal(const my_decimal *decimal_value) { return store_value(decimal_value); }
...
longlong Field_new_decimal::val_int(void)
{
  longlong i;
  my_decimal decimal_value;
  my_decimal2int(E_DEC_FATAL_ERROR, val_decimal(&decimal_value), unsigned_flag, &i);
  return i;
}
...
String *Field_new_decimal::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  my_decimal decimal_value;
  uint fixed_precision = zerofill ? precision : 0;
  my_decimal2string(E_DEC_FATAL_ERROR, val_decimal(&decimal_value), fixed_precision, dec, '0', val_buffer);
  return val_buffer;
}

int Field_new_decimal::cmp(const char *a, const char *b) { return memcmp(a, b, bin_size); }

void Field_new_decimal::sort_string(char *buff, uint length __attribute__((unused))) { memcpy(buff, ptr, bin_size); }

void Field_new_decimal::sql_type(String &str) const
{
  CHARSET_INFO *cs = str.charset();
  str.length(cs->cset->snprintf(cs, (char *)str.ptr(), str.alloced_length(), "decimal(%d,%d)", precision, (int)dec));
...
{
  int tmp = unsigned_flag ? (int)((uchar *)ptr)[0] : (int)((signed char *)ptr)[0];
  return (double)tmp;
}
...
String *Field_tiny::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  CHARSET_INFO *cs = &my_charset_bin;
  uint length;
  ...
  if (unsigned_flag)
    length = (uint)cs->cset->long10_to_str(cs, to, mlength, 10, (long)*((uchar *)ptr));
  else
    length = (uint)cs->cset->long10_to_str(cs, to, mlength, -10, (long)*((signed char *)ptr));
...
bool Field_tiny::send_binary(Protocol *protocol) { return protocol->store_tiny((longlong)(int8)ptr[0]); }
...
int Field_tiny::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  if (unsigned_flag) return ((uchar)a < (uchar)b) ? -1 : ((uchar)a > (uchar)b) ? 1 : 0;
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "tinyint(%d)", (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
String *Field_short::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  CHARSET_INFO *cs = &my_charset_bin;
  uint length;
  ...
  if (unsigned_flag)
    length = (uint)cs->cset->long10_to_str(cs, to, mlength, 10, (long)(uint16)j);
  else
...
bool Field_short::send_binary(Protocol *protocol) { return protocol->store_short(Field_short::val_int()); }
...
int Field_short::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  if (unsigned_flag)
    return ((unsigned short)a < (unsigned short)b) ? -1 : ((unsigned short)a > (unsigned short)b) ? 1 : 0;
  return (a < b) ? -1 : (a > b) ? 1 : 0;
}
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "smallint(%d)", (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
int Field_medium::store(longlong nr, bool unsigned_val)
{
  ...
  {
    if (nr < 0 && unsigned_val) nr = (ulonglong)(long)(1L << 24);  // Generate overflow
...
String *Field_medium::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  CHARSET_INFO *cs = &my_charset_bin;
  uint length;
...
bool Field_medium::send_binary(Protocol *protocol) { return protocol->store_long(Field_medium::val_int()); }
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "mediumint(%d)", (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
static bool test_if_minus(CHARSET_INFO *cs, const char *s, const char *e)
{
  my_wc_t wc;
...
int Field_long::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  else if (from + len != end && table->in_use->count_cuted_fields && check_int(from, len, end, cs))
    error = 2;
...
int Field_long::store(double nr)
{
  ...
  if (error) set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, 1);
...
int Field_long::store(longlong nr, bool unsigned_val)
{
  ...
  {
    if (nr < 0 && unsigned_val) nr = ((longlong)INT_MAX32) + 1;  // Generate overflow
...
String *Field_long::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  CHARSET_INFO *cs = &my_charset_bin;
  uint length;
...
bool Field_long::send_binary(Protocol *protocol) { return protocol->store_long(Field_long::val_int()); }
...
int Field_long::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  if (unsigned_flag) return ((uint32)a < (uint32)b) ? -1 : ((uint32)a > (uint32)b) ? 1 : 0;
  return (a < b) ? -1 : (a > b) ? 1 : 0;
}
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "int(%d)", (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
String *Field_longlong::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  CHARSET_INFO *cs = &my_charset_bin;
  uint length;
  ...
  length = (uint)(cs->cset->longlong10_to_str)(cs, to, mlength, unsigned_flag ? 10 : -10, j);
  val_buffer->length(length);
  if (zerofill) prepend_zeros(val_buffer);
  return val_buffer;
}
...
int Field_longlong::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  if (unsigned_flag) return ((ulonglong)a < (ulonglong)b) ? -1 : ((ulonglong)a > (ulonglong)b) ? 1 : 0;
  return (a < b) ? -1 : (a > b) ? 1 : 0;
}
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "bigint(%d)", (int)field_length));
  add_zerofill_and_unsigned(res);
}
...
int Field_float::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  {
    set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, (error ? ER_WARN_DATA_OUT_OF_RANGE : WARN_DATA_TRUNCATED), 1);
    error = error ? 1 : 2;
  }
  ...
  else
  {
    ...
    else
    {
      ...
      /*
        ...
        is close to FLT_MAX
      */
      if (fabs(nr) < FLT_MAX / 10.0e+32) nr = floor(nr * log_10[dec] + 0.5) / log_10[dec];
    }
    if (nr < -max_value)
    {
      j = (float)-max_value;
...
String *Field_float::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  float nr;
#ifdef WORDS_BIGENDIAN
  if (table->s->db_low_byte_first)
  ...
  else
  {
    ...
    { /* val_buffer is < 0 */
      *to++ = '0';
      if (!tmp_dec) goto end;
      *to++ = '.';
      if (-decpt > tmp_dec) decpt = -(int)tmp_dec;
...
void Field_float::sql_type(String &res) const
{
  ...
  {
    CHARSET_INFO *cs = res.charset();
    res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "float(%d,%d)", (int)field_length, dec));
  }
  add_zerofill_and_unsigned(res);
}
...
int Field_double::store(double nr)
{
  ...
  else
  {
    ...
    else
    {
      uint tmp = min(field_length, array_elements(log_10) - 1);
      max_value = (log_10[tmp] - 1) / log_10[dec];
      if (fabs(nr) < DBL_MAX / 10.0e+32) nr = floor(nr * log_10[dec] + 0.5) / log_10[dec];
    }
    if (nr < -max_value)
    {
      nr = -max_value;
...
String *Field_double::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  double nr;
#ifdef WORDS_BIGENDIAN
  if (table->s->db_low_byte_first)
...
void Field_double::sql_type(String &res) const
{
  ...
  {
    res.length(
        cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "double(%d,%d)", (int)field_length, dec));
  }
  add_zerofill_and_unsigned(res);
}
...
Field_timestamp::Field_timestamp(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
                                 enum utype unireg_check_arg, const char *field_name_arg, struct st_table *table_arg,
                                 CHARSET_INFO *cs)
    : Field_str(ptr_arg, 19, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg, table_arg, cs)
{
  /* For 4.0 MYD and 4.0 InnoDB compatibility */
  flags |= ZEROFILL_FLAG | UNSIGNED_FLAG;
  if (table && !table->timestamp_field && unireg_check != NONE)
  {
...
int Field_timestamp::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  have_smth_to_conv = (str_to_datetime(from, len, &l_time,
                                       (table->in_use->variables.sql_mode & MODE_NO_ZERO_DATE) | MODE_NO_ZERO_IN_DATE,
                                       &error) > MYSQL_TIMESTAMP_ERROR);

  if (error || !have_smth_to_conv)
  {
    error = 1;
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, from, len, MYSQL_TIMESTAMP_DATETIME, 1);
  }
  ...
  if (have_smth_to_conv && l_time.month)
  {
    ...
    {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, from, len, MYSQL_TIMESTAMP_DATETIME,
                           !error);
      error = 1;
    }
    else if (in_dst_time_gap)
    {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_INVALID_TIMESTAMP, from, len, MYSQL_TIMESTAMP_DATETIME,
                           !error);
      error = 1;
    }
  }
...
int Field_timestamp::store(double nr)
{
  ...
  {
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_DATETIME);
...
int Field_timestamp::store(longlong nr, bool unsigned_val)
{
  ...
  /* We don't want to store invalid or fuzzy datetime values in TIMESTAMP */
  longlong tmp =
      number_to_datetime(nr, &l_time, (thd->variables.sql_mode & MODE_NO_ZERO_DATE) | MODE_NO_ZERO_IN_DATE, &error);
  if (tmp == LL(-1))
  {
    error = 2;
  }
  ...
  if (!error && tmp)
  {
    ...
    {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_DATETIME, 1);
      error = 1;
    }
    if (in_dst_time_gap)
    {
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_INVALID_TIMESTAMP, nr, MYSQL_TIMESTAMP_DATETIME, 1);
      error = 1;
    }
  }
  else if (error)
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, nr, MYSQL_TIMESTAMP_DATETIME, 1);
...
longlong Field_timestamp::val_int(void)
{
  ...
  return time_tmp.year * LL(10000000000) + time_tmp.month * LL(100000000) + time_tmp.day * 1000000L +
         time_tmp.hour * 10000L + time_tmp.minute * 100 + time_tmp.second;
}
...
bool Field_timestamp::get_date(TIME *ltime, uint fuzzydate)
{
  ...
  if (temp == 0L)
  { /* Zero time is "000000" */
    if (fuzzydate & TIME_NO_ZERO_DATE) return 1;
...
bool Field_timestamp::get_time(TIME *ltime) { return Field_timestamp::get_date(ltime, 0); }
...
void Field_timestamp::sql_type(String &res) const { res.set_ascii("timestamp", 9); }
...
int Field_time::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  if (str_to_time(from, len, &ltime, &error))
  {
    tmp = 0L;
    error = 2;
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, from, len, MYSQL_TIMESTAMP_TIME, 1);
  }
  else
  {
    if (error)
    ...
    if (ltime.month) ltime.day = 0;
    tmp = (ltime.day * 24L + ltime.hour) * 10000L + (ltime.minute * 100 + ltime.second);
    if (tmp > 8385959)
    {
      tmp = 8385959;
      set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, from, len, MYSQL_TIMESTAMP_TIME,
                           !error);
      error = 1;
    }
    if (error > 1) error = 2;
...
{
  long tmp = ((ltime->month ? 0 : ltime->day * 24L) + ltime->hour) * 10000L + (ltime->minute * 100 + ltime->second);
  if (ltime->neg) tmp = -tmp;
...
int Field_time::store(double nr)
{
  ...
  {
    tmp = 8385959L;
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_TIME);
    error = 1;
  }
...
int Field_time::store(longlong nr, bool unsigned_val)
{
  ...
  {
    tmp = -8385959L;
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_TIME, 1);
    error = 1;
  }
...
longlong Field_time::val_int(void) { return (longlong)sint3korr(ptr); }
...
String *Field_time::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  TIME ltime;
  val_buffer->alloc(19);
...
bool Field_time::get_date(TIME *ltime, uint fuzzydate)
{
  ...
  {
    push_warning_printf(table->in_use, MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE,
                        ER(ER_WARN_DATA_OUT_OF_RANGE), field_name, table->in_use->row_count);
    return 1;
  }
...
void Field_time::sql_type(String &res) const { res.set_ascii("time", 4); }
...
int Field_year::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  if (table->in_use->count_cuted_fields && check_int(from, len, end, cs)) error = 1;
...
String *Field_year::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  val_buffer->alloc(5);
  val_buffer->length(field_length);
  char *to = (char *)val_buffer->ptr();
...
{
  CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "year(%d)", (int)field_length));
}
...
int Field_date::store(const char *from, uint len, CHARSET_INFO *cs)
{
  ...
  if (str_to_datetime(from, len, &l_time,
                      TIME_FUZZY_DATE | (table->in_use->variables.sql_mode &
  ...
  if (error)
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, from, len, MYSQL_TIMESTAMP_DATE, 1);
...
int Field_date::store(double nr)
{
  longlong tmp;
  int error = 0;
  if (nr >= 19000000000000.0 && nr <= 99991231235959.0) nr = floor(nr / 1000000.0);  // Timestamp to date
  if (nr < 0.0 || nr > 99991231.0)
  {
    tmp = LL(0);
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, nr, MYSQL_TIMESTAMP_DATE);
    error = 1;
  }
  else
...
int Field_date::store(longlong nr, bool unsigned_val)
{
  ...
  nr = number_to_datetime(nr, &not_used,
                          (TIME_FUZZY_DATE | (table->in_use->variables.sql_mode &
                                              (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_INVALID_DATES))),
                          &error);
  ...
  if (error)
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, error == 2 ? ER_WARN_DATA_OUT_OF_RANGE : WARN_DATA_TRUNCATED,
                         initial_nr, MYSQL_TIMESTAMP_DATETIME, 1);
...
String *Field_date::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  TIME ltime;
...
void Field_date::sql_type(String &res) const { res.set_ascii("date", 4); }
...
int Field_newdate::store(double nr)
{
  if (nr < 0.0 || nr > 99991231235959.0)
  {
    ...
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, nr, MYSQL_TIMESTAMP_DATE);
    return 1;
  }
...
int Field_newdate::store(longlong nr, bool unsigned_val)
{
  ...
  if (error)
    set_datetime_warning(MYSQL_ERROR::WARN_LEVEL_WARN, error == 2 ? ER_WARN_DATA_OUT_OF_RANGE : WARN_DATA_TRUNCATED, nr,
...
double Field_newdate::val_real(void) { return (double)Field_newdate::val_int(); }
...
String *Field_newdate::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  val_buffer->alloc(field_length);
  val_buffer->length(field_length);
...
bool Field_newdate::get_time(TIME *ltime) { return Field_newdate::get_date(ltime, 0); }
...
void Field_newdate::sql_type(String &res) const { res.set_ascii("date", 4); }
...
double Field_datetime::val_real(void) { return (double)Field_datetime::val_int(); }
...
String *Field_datetime::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  val_buffer->alloc(field_length);
  val_buffer->length(field_length);
  ulonglong tmp;
...
bool Field_datetime::get_time(TIME *ltime) { return Field_datetime::get_date(ltime, 0); }
...
int Field_datetime::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  return ((ulonglong)a < (ulonglong)b) ? -1 : ((ulonglong)a > (ulonglong)b) ? 1 : 0;
}
...
void Field_datetime::sql_type(String &res) const { res.set_ascii("datetime", 8); }
...
int Field_string::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  */
  copy_length = field_charset->cset->well_formed_len(field_charset, from, from + length,
                                                     field_length / field_charset->mbmaxlen, &well_formed_error);
  memcpy(ptr, from, copy_length);
  if (copy_length < field_length)  // Append spaces if shorter
    field_charset->cset->fill(field_charset, ptr + copy_length, field_length - copy_length, field_charset->pad_char);
...
int Field_str::store(double nr)
{
  ...
  */
  ...
  length = (uint)my_sprintf(
      buff, (buff, "%-.*g", (use_scientific_notation ? max(0, (int)char_length - neg - 5) : char_length), nr));
  /*
...
int Field_string::store(longlong nr, bool unsigned_val)
{
  ...
  l = (cs->cset->longlong10_to_str)(cs, buff, sizeof(buff), unsigned_val ? 10 : -10, nr);
...
String *Field_string::val_str(String *val_buffer __attribute__((unused)), String *val_ptr)
{
...
{
  str2my_decimal(E_DEC_FATAL_ERROR, ptr, field_length, charset(), decimal_value);
  return decimal_value;
}
...
int Field_string::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  if (field_charset->strxfrm_multiply > 1)
  {
    ...
    */
    return field_charset->coll->strnncollsp(field_charset, (const uchar *)a_ptr, field_length, (const uchar *)b_ptr,
                                            field_length, 0);
  }
  ...
  return my_strnncoll(field_charset, (const uchar *)a_ptr, a_len, (const uchar *)b_ptr, b_len);
}

void Field_string::sort_string(char *to, uint length)
{
  uint tmp = my_strnxfrm(field_charset, (unsigned char *)to, length, (unsigned char *)ptr, field_length);
...
void Field_string::sql_type(String &res) const
{
  ...
  length = cs->cset->snprintf(
      ...
      ((type() == MYSQL_TYPE_VAR_STRING && !thd->variables.new_mode) ? (has_charset() ? "varchar" : "varbinary")
                                                                     : (has_charset() ? "char" : "binary")),
      (int)field_length / charset()->mbmaxlen);
  res.length(length);
  if ((thd->variables.sql_mode & (MODE_MYSQL323 | MODE_MYSQL40)) && has_charset() && (charset()->state & MY_CS_BINSORT))
    res.append(" binary");
}
...
char *Field_string::pack(char *to, const char *from, uint max_length)
{
  ...
  if (length > char_length) char_length = my_charpos(field_charset, from, from + length, char_length);
  set_if_smaller(length, char_length);
  while (length && from[length - 1] == ' ') length--;
  *to++ = (char)(uchar)length;
  if (field_length > 255) *to++ = (char)(uchar)(length >> 8);
...
int Field_string::pack_cmp(const char *a, const char *b, uint length, my_bool insert_or_update)
{
  ...
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)a, a_length, (const uchar *)b, b_length,
                                          insert_or_update);
}
...
int Field_string::pack_cmp(const char *key, uint length, my_bool insert_or_update)
{
  ...
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)ptr, row_length, (const uchar *)key, key_length,
                                          insert_or_update);
}

uint Field_string::packed_col_length(const char *data_ptr, uint length)
{
  if (length > 255) return uint2korr(data_ptr) + 2;
  return (uint)((uchar)*data_ptr) + 1;
}

uint Field_string::max_packed_col_length(uint max_length) { return (max_length > 255 ? 2 : 1) + max_length; }

Field *Field_string::new_field(MEM_ROOT *root, struct st_table *new_table)
{
  if (type() != MYSQL_TYPE_VAR_STRING || table == new_table) return Field::new_field(root, new_table);
  ...
  */
  return new Field_varstring(field_length, maybe_null(), field_name, new_table, charset());
}
...
int Field_varstring::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  if (String::needs_conversion(length, cs, field_charset, &not_used))
  {
    ...
    from = tmpstr.ptr();
    length = tmpstr.length();
    if (conv_errors) error_code = WARN_DATA_TRUNCATED;
  }
  /*
    Make sure we don't break a multibyte sequence
    as well as don't copy a malformed data.
  */
  ...
  if ((copy_length < length) && table->in_use->count_cuted_fields && !error_code)
  {
    const char *end = from + length;
    from += copy_length;
    from += field_charset->cset->scan(field_charset, from, end, MY_SEQ_SPACES);
    /* If we lost only spaces then produce a NOTE, not a WARNING */
    if (from == end) level = MYSQL_ERROR::WARN_LEVEL_NOTE;
    error_code = WARN_DATA_TRUNCATED;
  }
  if (error_code)
  {
    if (level == MYSQL_ERROR::WARN_LEVEL_WARN && table->in_use->abort_on_warning) error_code = ER_DATA_TOO_LONG;
...
int Field_varstring::store(longlong nr, bool unsigned_val)
{
  ...
  uint length;
  length =
      (uint)(field_charset->cset->longlong10_to_str)(field_charset, buff, sizeof(buff), (unsigned_val ? 10 : -10), nr);
...
longlong Field_varstring::val_int(void)
{
  ...
  char *end_not_used;
  uint length = length_bytes == 1 ? (uint)(uchar)*ptr : uint2korr(ptr);
  return my_strntoll(field_charset, ptr + length_bytes, length, 10, &end_not_used, &not_used);
}

String *Field_varstring::val_str(String *val_buffer __attribute__((unused)), String *val_ptr)
{
  uint length = length_bytes == 1 ? (uint)(uchar)*ptr : uint2korr(ptr);
...
{
  uint length = length_bytes == 1 ? (uint)(uchar)*ptr : uint2korr(ptr);
  str2my_decimal(E_DEC_FATAL_ERROR, ptr + length_bytes, length, charset(), decimal_value);
  return decimal_value;
}
...
int Field_varstring::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  diff = field_charset->coll->strnncollsp(field_charset, (const uchar *)a_ptr + length_bytes, a_length,
                                          (const uchar *)b_ptr + length_bytes, b_length, 0);
  return diff;
}
...
int Field_varstring::key_cmp(const byte *key_ptr, uint max_key_length)
{
  ...
  char_length = my_charpos(field_charset, ptr + length_bytes, ptr + length_bytes + length, char_length);
  set_if_smaller(length, char_length);
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)ptr + length_bytes, length,
                                          (const uchar *)key_ptr + HA_KEY_BLOB_LENGTH, uint2korr(key_ptr), 0);
}
...
{
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)a + HA_KEY_BLOB_LENGTH, uint2korr(a),
                                          (const uchar *)b + HA_KEY_BLOB_LENGTH, uint2korr(b), 0);
}
...
void Field_varstring::sort_string(char *to, uint length)
{
  ...
  tot_length = my_strnxfrm(field_charset, (uchar *)to, length, (uchar *)ptr + length_bytes, tot_length);
...
void Field_varstring::sql_type(String &res) const
{
  ...
  length = cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "%s(%d)",
                              (has_charset() ? "varchar" : "varbinary"), (int)field_length / charset()->mbmaxlen);
  res.length(length);
  if ((thd->variables.sql_mode & (MODE_MYSQL323 | MODE_MYSQL40)) && has_charset() && (charset()->state & MY_CS_BINSORT))
    res.append(" binary");
}
...
char *Field_varstring::pack(char *to, const char *from, uint max_length)
{
  ...
  *to++ = (char)(length & 255);
  if (max_length > 255) *to++ = (char)(length >> 8);
  if (length) memcpy(to, from + length_bytes, length);
  return to + length;
}
...
char *Field_varstring::pack_key(char *to, const char *key, uint max_length)
{
  ...
  *to++ = (char)(length & 255);
  if (max_length > 255) *to++ = (char)(length >> 8);
  if (length) memcpy(to, key, length);
  return to + length;
}
...
const char *Field_varstring::unpack_key(char *to, const char *key, uint max_length)
{
  /* get length of the blob key */
  uint32 length = *((uchar *)key++);
  if (max_length > 255) length += (*((uchar *)key++)) << 8;
...
char *Field_varstring::pack_key_from_key_image(char *to, const char *from, uint max_length)
{
  ...
  *to++ = (char)(length & 255);
  if (max_length > 255) *to++ = (char)(length >> 8);
  if (length) memcpy(to, from + HA_KEY_BLOB_LENGTH, length);
  return to + length;
}
...
int Field_varstring::pack_cmp(const char *a, const char *b, uint key_length, my_bool insert_or_update)
{
  uint a_length, b_length;
  if (key_length > 255)
  {
...
int Field_varstring::pack_cmp(const char *b, uint key_length, my_bool insert_or_update)
{
  char *a = ptr + length_bytes;
  uint a_length = length_bytes == 1 ? (uint)(uchar)*ptr : uint2korr(ptr);
  uint b_length;
  uint char_length = ((field_charset->mbmaxlen > 1) ? key_length / field_charset->mbmaxlen : key_length);
...
uint Field_varstring::max_packed_col_length(uint max_length) { return (max_length > 255 ? 2 : 1) + max_length; }
...
void Field_varstring::set_key_image(char *buff, uint length)
{
  ...
  (void)Field_varstring::store(buff + HA_KEY_BLOB_LENGTH, length, field_charset);
}

int Field_varstring::cmp_binary(const char *a_ptr, const char *b_ptr, uint32 max_length)
{
...
Field *Field_varstring::new_key_field(MEM_ROOT *root, struct st_table *new_table, char *new_ptr, uchar *new_null_ptr,
                                      uint new_null_bit)
{
  Field_varstring *res;
  if ((res = (Field_varstring *)Field::new_key_field(root, new_table, new_ptr, new_null_ptr, new_null_bit)))
  {
...
Field_blob::Field_blob(char *ptr_arg, uchar *null_ptr_arg, uchar null_bit_arg, enum utype unireg_check_arg,
                       const char *field_name_arg, struct st_table *table_arg, uint blob_pack_length, CHARSET_INFO *cs)
    : Field_longstr(ptr_arg, BLOB_PACK_LENGTH_TO_MAX_LENGH(blob_pack_length), null_ptr_arg, null_bit_arg,
...
int Field_blob::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  else
  {
    ...
    /* Convert character set if necessary */
    if ((was_conversion = String::needs_conversion(length, cs, field_charset, &not_used)))
    {
      uint conv_errors;
    ...
    /*
      ...
      the 'min()' call below.
    */
    copy_length = field_charset->cset->well_formed_len(field_charset, from, from + min(length, copy_length),
                                                       copy_length, &well_formed_error);
    if (copy_length < length) error = 2;
...
String *Field_blob::val_str(String *val_buffer __attribute__((unused)), String *val_ptr)
{
  char *blob;
  memcpy_fixed(&blob, ptr + packlength, sizeof(char *));
  if (!blob)
...
my_decimal *Field_blob::val_decimal(my_decimal *decimal_value)
{
  ...
  if (!blob) blob = "";
  str2my_decimal(E_DEC_FATAL_ERROR, blob, get_length(ptr), charset(), decimal_value);
  return decimal_value;
}

int Field_blob::cmp(const char *a, uint32 a_length, const char *b, uint32 b_length)
{
  return field_charset->coll->strnncollsp(field_charset, (const uchar *)a, a_length, (const uchar *)b, b_length, 0);
}
...
int Field_blob::cmp(const char *a_ptr, const char *b_ptr)
{
  ...
  return Field_blob::cmp(blob1, get_length(a_ptr), blob2, get_length(b_ptr));
}

int Field_blob::cmp_binary(const char *a_ptr, const char *b_ptr, uint32 max_length)
{
  char *a, *b;
  uint diff;
  uint32 a_length, b_length;
...
void Field_blob::get_key_image(char *buff, uint length, imagetype type)
{
  ...
  if (type == itMBR)
  {
    ...
    get_ptr(&blob);
    gobj = Geometry::create_from_wkb(&buffer, blob + SRID_SIZE, blob_length - SRID_SIZE);
  ...
  get_ptr(&blob);
  uint char_length = length / field_charset->mbmaxlen;
  char_length = my_charpos(field_charset, blob, blob + blob_length, char_length);
...
int Field_blob::key_cmp(const byte *key_ptr, uint max_key_length)
{
  ...
  set_if_smaller(blob_length, char_length);
  return Field_blob::cmp(blob1, blob_length, (char *)key_ptr + HA_KEY_BLOB_LENGTH, uint2korr(key_ptr));
}

int Field_blob::key_cmp(const byte *a, const byte *b)
{
  return Field_blob::cmp((char *)a + HA_KEY_BLOB_LENGTH, uint2korr(a), (char *)b + HA_KEY_BLOB_LENGTH, uint2korr(b));
}
...
void Field_blob::sort_string(char *to, uint length)
{
  ...
  else
  {
    ...
    blob_length = my_strnxfrm(field_charset, (uchar *)to, length, (uchar *)blob, blob_length);
...
int Field_blob::pack_cmp(const char *a, const char *b, uint key_length, my_bool insert_or_update)
{
  uint a_length, b_length;
  if (key_length > 255)
  {
...
int Field_blob::pack_cmp(const char *b, uint key_length, my_bool insert_or_update)
{
  char *a;
  memcpy_fixed(&a, ptr + packlength, sizeof(char *));
  if (!a) return key_length > 0 ? -1 : 0;
...
const char *Field_blob::unpack_key(char *to, const char *from, uint max_length)
{
  ...
  if (max_length > 255) length += (*((uchar *)from++)) << 8;
...
char *Field_blob::pack_key_from_key_image(char *to, const char *from, uint max_length)
{
  uint length = uint2korr(from);
  if (length > max_length) length = max_length;
  *to++ = (char)(length & 255);
...
uint Field_blob::max_packed_col_length(uint max_length) { return (max_length > 255 ? 2 : 1) + max_length; }
...
int Field_geom::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  {
    if (from == Geometry::bad_geometry_data.ptr()) goto err;
    // Check given WKB
    uint32 wkb_type;
    if (length < SRID_SIZE + WKB_HEADER_SIZE + SIZEOF_STORED_DOUBLE * 2) goto err;
    wkb_type = uint4korr(from + WKB_HEADER_SIZE);
    if (wkb_type < (uint32)Geometry::wkb_point || wkb_type > (uint32)Geometry::wkb_end) goto err;
...
int Field_enum::store(double nr) { return Field_enum::store((longlong)nr, FALSE); }
...
String *Field_enum::val_str(String *val_buffer __attribute__((unused)), String *val_ptr)
{
...
int Field_set::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  ulonglong tmp = find_set(typelib, from, length, field_charset, &not_used, &not_used2, &got_warning);
  if (!tmp && length && length < 22)
  {
    /* This is for reading numbers with LOAD DATA INFILE */
    char *end;
    tmp = my_strntoull(cs, from, length, 10, &end, &err);
    if (err || end != from + length || tmp > (ulonglong)(((longlong)1 << typelib->count) - (longlong)1))
    {
      tmp = 0;
      set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, WARN_DATA_TRUNCATED, 1);
    }
...
{
  int error = 0;
  if ((ulonglong)nr > (ulonglong)(((longlong)1 << typelib->count) - (longlong)1))
  {
...
String *Field_set::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
  ...
  while (tmp && bitnr < (uint)typelib->count)
  {
    if (tmp & 1)
    {
      if (val_buffer->length()) val_buffer->append(&field_separator, 1, &my_charset_latin1);
      String str(typelib->type_names[bitnr], typelib->type_lengths[bitnr], field_charset);
...
bool Field_enum::eq_def(Field *field)
{
  ...
  if (typelib->count < from_lib->count) return 0;
  for (uint i = 0; i < from_lib->count; i++)
    if (my_strnncoll(field_charset, (const uchar *)typelib->type_names[i], (uint)strlen(typelib->type_names[i]),
                     (const uchar *)from_lib->type_names[i], (uint)strlen(from_lib->type_names[i])))
      return 0;
  return 1;
}
...
bool Field_num::eq_def(Field *field)
{
  ...
  if (unsigned_flag != from_num->unsigned_flag || zerofill && !from_num->zerofill && !zero_pack() ||
      dec != from_num->dec)
    return 0;
  return 1;
}
...
Field_bit::Field_bit(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg, uchar *bit_ptr_arg,
                     uchar bit_ofs_arg, enum utype unireg_check_arg, const char *field_name_arg,
                     struct st_table *table_arg)
    : Field(ptr_arg, len_arg >> 3, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg, table_arg),
      bit_ptr(bit_ptr_arg),
      bit_ofs(bit_ofs_arg),
      bit_len(len_arg & 7)
{
...
Field *Field_bit::new_key_field(MEM_ROOT *root, struct st_table *new_table, char *new_ptr, uchar *new_null_ptr,
                                uint new_null_bit)
{
  Field_bit *res;
  if ((res = (Field_bit *)Field::new_key_field(root, new_table, new_ptr, new_null_ptr, new_null_bit)))
  {
...
int Field_bit::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  for (; !*from && length; from++, length--)
    ;  // skip left 0's
  delta = field_length - length;

  if (delta < -1 || (delta == -1 && (uchar)*from > ((1 << bit_len) - 1)) || (!bit_len && delta < 0))
  {
...
int Field_bit::store(double nr) { return store((longlong)nr, FALSE); }
...
longlong Field_bit::val_int(void)
{
  ...
  switch (field_length)
  {
    case 0:
      return bits;
    case 1:
      return bits | (ulonglong)(uchar)ptr[0];
    case 2:
    ...
    default:
      return mi_uint8korr(ptr + field_length - sizeof(longlong));
  }
}

String *Field_bit::val_str(String *val_buffer, String *val_ptr __attribute__((unused)))
{
...
int Field_bit::key_cmp(const byte *str, uint length)
{
  if (bit_len)
  {
    ...
    if ((flag = (int)(bits - *(uchar *)str))) return flag;
    str++;
    length--;
  }
...
void Field_bit::sql_type(String &res) const
{
  CHARSET_INFO *cs = res.charset();
  ulong length =
      cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "bit(%d)", (int)field_length * 8 + bit_len);
...
Field_bit_as_char::Field_bit_as_char(char *ptr_arg, uint32 len_arg, uchar *null_ptr_arg, uchar null_bit_arg,
                                     uchar *bit_ptr_arg, uchar bit_ofs_arg, enum utype unireg_check_arg,
                                     const char *field_name_arg, struct st_table *table_arg)
    : Field_bit(ptr_arg, len_arg, null_ptr_arg, null_bit_arg, bit_ptr_arg, bit_ofs_arg, unireg_check_arg,
                field_name_arg, table_arg),
...
int Field_bit_as_char::store(const char *from, uint length, CHARSET_INFO *cs)
{
  ...
  if (delta < 0 || (delta == 0 && bits && (uint)(uchar)*from >= (uint)(1 << bits)))
  {
    memset(ptr, 0xff, field_length);
    if (bits) *ptr &= ((1 << bits) - 1); /* set first byte */
    if (table->in_use->really_abort_on_warning())
      set_warning(MYSQL_ERROR::WARN_LEVEL_ERROR, ER_DATA_TOO_LONG, 1);
    else
      set_warning(MYSQL_ERROR::WARN_LEVEL_WARN, ER_WARN_DATA_OUT_OF_RANGE, 1);
...
{
  CHARSET_INFO *cs = res.charset();
  ulong length = cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "bit(%d)", (int)create_length);
  res.length((uint)length);
}
...
void create_field::create_length_to_internal_length(void)
{
  switch (sql_type)
  {
    ...
    case MYSQL_TYPE_NEWDECIMAL:
      key_length = pack_length =
          my_decimal_get_binary_size(my_decimal_length_to_precision(length, decimals, flags & UNSIGNED_FLAG), decimals);
      break;
    default:
...
void create_field::init_for_tmp_table(enum_field_types sql_type_arg, uint32 length_arg, uint32 decimals,
                                      bool maybe_null, bool is_unsigned)
{
  ...
  pack_flag = (FIELDFLAG_NUMBER | ((decimals & FIELDFLAG_MAX_DEC) << FIELDFLAG_DEC_SHIFT) |
               (maybe_null ? FIELDFLAG_MAYBE_NULL : 0) | (is_unsigned ? 0 : FIELDFLAG_DECIMAL));
}
...
uint pack_length_to_packflag(uint type)
{
  ...
  {
    case 1:
      return f_settype((uint)FIELD_TYPE_TINY);
    case 2:
      return f_settype((uint)FIELD_TYPE_SHORT);
    case 3:
      return f_settype((uint)FIELD_TYPE_INT24);
    case 4:
      return f_settype((uint)FIELD_TYPE_LONG);
    case 8:
      return f_settype((uint)FIELD_TYPE_LONGLONG);
  }
  return 0;  // This shouldn't happen
}

Field *make_field(char *ptr, uint32 field_length, uchar *null_pos, uchar null_bit, uint pack_flag,
                  enum_field_types field_type, CHARSET_INFO *field_charset, Field::geometry_type geom_type,
                  Field::utype unireg_check, TYPELIB *interval, const char *field_name, struct st_table *table)
{
  ...
  if (f_is_alpha(pack_flag))
  {
    if (!f_is_packed(pack_flag))
    {
      if (field_type == MYSQL_TYPE_STRING || field_type == FIELD_TYPE_DECIMAL ||  // 3.23 or 4.0 string
        ...
        return new Field_string(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table, field_charset);
      if (field_type == MYSQL_TYPE_VARCHAR)
        return new Field_varstring(ptr, field_length, HA_VARCHAR_PACKLENGTH(field_length), null_pos, null_bit,
                                   unireg_check, field_name, table, field_charset);
      return 0;  // Error
    }

    uint pack_length = calc_pack_length((enum_field_types)f_packtype(pack_flag), field_length);

#ifdef HAVE_SPATIAL
    if (f_is_geom(pack_flag))
      return new Field_geom(ptr, null_pos, null_bit, unireg_check, field_name, table, pack_length, geom_type);
#endif
    if (f_is_blob(pack_flag))
      return new Field_blob(ptr, null_pos, null_bit, unireg_check, field_name, table, pack_length, field_charset);
    if (interval)
    {
      if (f_is_enum(pack_flag))
        return new Field_enum(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table, pack_length,
                              interval, field_charset);
      else
        return new Field_set(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table, pack_length,
                             interval, field_charset);
    }
  }
  ...
  {
    case FIELD_TYPE_DECIMAL:
      return new Field_decimal(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                               f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_NEWDECIMAL:
      return new Field_new_decimal(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                                   f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_FLOAT:
      return new Field_float(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                             f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_DOUBLE:
      return new Field_double(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                              f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_TINY:
      return new Field_tiny(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                            f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_SHORT:
      return new Field_short(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                             f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_INT24:
      return new Field_medium(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                              f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_LONG:
      return new Field_long(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                            f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_LONGLONG:
      return new Field_longlong(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table,
                                f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case FIELD_TYPE_TIMESTAMP:
      return new Field_timestamp(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_YEAR:
      return new Field_year(ptr, field_length, null_pos, null_bit, unireg_check, field_name, table);
    case FIELD_TYPE_DATE:
      return new Field_date(ptr, null_pos, null_bit, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_NEWDATE:
      return new Field_newdate(ptr, null_pos, null_bit, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_TIME:
      return new Field_time(ptr, null_pos, null_bit, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_DATETIME:
      return new Field_datetime(ptr, null_pos, null_bit, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_NULL:
      return new Field_null(ptr, field_length, unireg_check, field_name, table, field_charset);
    case FIELD_TYPE_BIT:
      return f_bit_as_char(pack_flag) ? new Field_bit_as_char(ptr, field_length, null_pos, null_bit, bit_ptr,
                                                              bit_offset, unireg_check, field_name, table)
                                      : new Field_bit(ptr, field_length, null_pos, null_bit, bit_ptr, bit_offset,
...
create_field::create_field(Field *old_field, Field *orig_field)
{
  ...
  if (flags & BLOB_FLAG) pack_length = (pack_length - old_field->table->s->blob_ptr_size + portable_sizeof_char_ptr);
  ...
  switch (sql_type)
  {
    ...
    case MYSQL_TYPE_STRING:
      ...
      if (old_field->type() == MYSQL_TYPE_VAR_STRING) sql_type = MYSQL_TYPE_VARCHAR;
      /* fall through */
...
#endif
    case FIELD_TYPE_BIT:
      length = (old_field->key_type() == HA_KEYTYPE_BIT) ? ((Field_bit *)old_field)->bit_len + length * 8
       ...
       unireg_check == Field::TIMESTAMP_UN_FIELD))       /* has default val */
  {
    ...
    diff = (my_ptrdiff_t)(orig_field->table->s->default_values - orig_field->table->record[0]);
...
bool Field::set_warning(MYSQL_ERROR::enum_warning_level level, uint code, int cuted_increment)
{
  THD *thd = table->in_use;
  if (thd->count_cuted_fields)
  {
    thd->cuted_fields += cuted_increment;
    push_warning_printf(thd, level, code, ER(code), field_name, thd->row_count);
    return 0;
  }
  return 1;
}
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level, uint code, const char *str, uint str_length,
                                 timestamp_type ts_type, int cuted_increment)
{
  if (table->in_use->really_abort_on_warning() || set_warning(level, code, cuted_increment))
    make_truncated_value_warning(table->in_use, str, str_length, ts_type, field_name);
}
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level, uint code, longlong nr, timestamp_type ts_type,
                                 int cuted_increment)
{
  if (table->in_use->really_abort_on_warning() || set_warning(level, code, cuted_increment))
  {
    ...
    make_truncated_value_warning(table->in_use, str_nr, (uint)(str_end - str_nr), ts_type, field_name);
  }
}
...
void Field::set_datetime_warning(MYSQL_ERROR::enum_warning_level level, uint code, double nr, timestamp_type ts_type)
{
  if (table->in_use->really_abort_on_warning() || set_warning(level, code, 1))
  {
    /* DBL_DIG is enough to print '-[digits].E+###' */
    char str_nr[DBL_DIG + 8];
    uint str_len = my_sprintf(str_nr, (str_nr, "%g", nr));
    make_truncated_value_warning(table->in_use, str_nr, str_len, ts_type, field_name);
  }
}
