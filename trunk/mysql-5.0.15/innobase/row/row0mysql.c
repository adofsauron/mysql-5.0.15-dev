...
static ibool row_mysql_is_recovered_tmp_table(
    /*=============================*/
    ...
    const char *name)
{
  ...
  return (namelen >= sizeof S_recover_innodb_tmp_table &&
          !memcmp(name + namelen - sizeof S_recover_innodb_tmp_table, S_recover_innodb_tmp_table,
                  sizeof S_recover_innodb_tmp_table));
}

/***********************************************************************
Determine if the given name is a name reserved for MySQL system tables. */
static ibool row_mysql_is_system_table(
    /*======================*/
    ...
    const char *name)
{
  ...
  return (0 == strcmp(name + 6, "host") || 0 == strcmp(name + 6, "user") || 0 == strcmp(name + 6, "db"));
}
...
byte *row_mysql_read_true_varchar(
    /*========================*/
    /* out: pointer to the data, we skip the 1 or 2 bytes
    at the start that are used to store the len */
    ...
    ulint comp)           /* in: nonzero=compact format */
{
  ...
  else if ((type == DATA_VARCHAR || type == DATA_VARMYSQL || type == DATA_BINARY))
  {
    ...
    else
    {
      ...
      if (mbminlen == 2)
      {
        ...
        while (col_len >= 2 && ptr[col_len - 2] == 0x00 && ptr[col_len - 1] == 0x20)
        {
          col_len -= 2;
        }
      }
  ...
  else if (comp && type == DATA_MYSQL && dtype_get_mbminlen(dtype) == 1 && dtype_get_mbmaxlen(dtype) > 1)
  {
                              ...
                              pointers to this record! */
{
  ...
  for (i = 0; i < prebuilt->n_template; i++)
  {
    ...
    if (templ->mysql_null_bit_mask != 0)
    {
      ...
      if (mysql_rec[templ->mysql_null_byte_offset] & (byte)(templ->mysql_null_bit_mask))
      {
        /* It is SQL NULL */
    ...
    row_mysql_store_col_in_innobase_format(
        dfield, prebuilt->ins_upd_rec_buff + templ->mysql_col_offset, TRUE, /* MySQL row format data */
        mysql_rec + templ->mysql_col_offset, templ->mysql_col_len, prebuilt->table->comp);
  next_column:;
  }
}
...
handle_new_error:
  ...
  else if (err == DB_ROW_IS_REFERENCED || err == DB_NO_REFERENCED_ROW || err == DB_CANNOT_ADD_CONSTRAINT)
  {
    if (savept)
    {
      /* Roll back the latest, possibly incomplete
  ...
  else if (err == DB_DEADLOCK || err == DB_LOCK_TABLE_FULL)
  {
  ...
  else if (err == DB_OUT_OF_FILE_SPACE || err == DB_LOCK_WAIT_TIMEOUT)
  {
    if (savept)
    {
      /* Roll back the latest, possibly incomplete
    ...
    row_prebuilt_t *prebuilt) /* in, own: prebuilt struct */
{
  ...
  for (i = 0; i < MYSQL_FETCH_CACHE_SIZE; i++)
  {
    ...
    {
      if ((ROW_PREBUILT_FETCH_MAGIC_N != mach_read_from_4((prebuilt->fetch_cache[i]) - 4)) ||
          (ROW_PREBUILT_FETCH_MAGIC_N != mach_read_from_4((prebuilt->fetch_cache[i]) + prebuilt->mysql_row_len)))
      {
        fputs(
            "InnoDB: Error: trying to free a corrupt\n"
            "InnoDB: fetch buffer.\n",
        ...
        mem_analyze_corruption(prebuilt->fetch_cache[i]);
                              ...
                              handle */
{
  ...
  if (prebuilt->ins_node == NULL)
  {
    ...
    {
      prebuilt->ins_upd_rec_buff = mem_heap_alloc(prebuilt->heap, prebuilt->mysql_row_len);
    }
    ...
    prebuilt->ins_graph = que_node_get_parent(pars_complete_graph_for_exec(node, prebuilt->trx, prebuilt->heap));
    ...
    dict_table_t *table) /* in: table */
{
  ...
  if (counter > 2000000000 || ((ib_longlong)counter > 16 + table->stat_n_rows / 16))
  {
                              ...
                              handle */
{
  ...
  if (prebuilt->sel_graph == NULL)
  {
    ...
    prebuilt->sel_graph = que_node_get_parent(pars_complete_graph_for_exec(node, prebuilt->trx, prebuilt->heap));
                              ...
                              handle */
{
  ...
  if (prebuilt->upd_node == NULL)
  {
    ...
    prebuilt->upd_graph = que_node_get_parent(pars_complete_graph_for_exec(node, prebuilt->trx, prebuilt->heap));
...
ulint row_update_cascade_for_mysql(
    /*=========================*/
    /* out: error code or DB_SUCCESS */
    que_thr_t *thr,      /* in: query thread */
    ...
    dict_table_t *table)
{
  ...
  if (dtype_get_mtype(dict_index_get_nth_type(clust_index, 0)) == DATA_SYS)
  {
    return (TRUE);
  }
...
name is "#sql..." */
static int row_mysql_recover_tmp_table(
    /*========================*/
    /* out: error code or DB_SUCCESS */
    ...
    trx_t *trx)          /* in: transaction handle */
{
  ...
  else
  {
    ...
    ut_ad(!strcmp(old_name + namelen + 1 - sizeof S_recover_innodb_tmp_table, S_recover_innodb_tmp_table));
    old_name[namelen + 1 - sizeof S_recover_innodb_tmp_table] = 0;
    status = row_rename_table_for_mysql(old_name, table->name, trx);
    mem_free(old_name);
    return (status);
  }
}
...
int row_create_table_for_mysql(
    /*=======================*/
    /* out: error code or DB_SUCCESS */
    dict_table_t *table, /* in: table definition */
    trx_t *trx)          /* in: transaction handle */
{
  ...
  if (table_name_len == sizeof S_innodb_monitor && !memcmp(table_name, S_innodb_monitor, sizeof S_innodb_monitor))
  {
    ...
    os_event_set(srv_lock_timeout_thread_event);
           ...
           !memcmp(table_name, S_innodb_lock_monitor, sizeof S_innodb_lock_monitor))
  {
    srv_print_innodb_monitor = TRUE;
    ...
    os_event_set(srv_lock_timeout_thread_event);
           ...
           !memcmp(table_name, S_innodb_tablespace_monitor, sizeof S_innodb_tablespace_monitor))
  {
    srv_print_innodb_tablespace_monitor = TRUE;
    os_event_set(srv_lock_timeout_thread_event);
  }
  else if (table_name_len == sizeof S_innodb_table_monitor &&
           !memcmp(table_name, S_innodb_table_monitor, sizeof S_innodb_table_monitor))
  {
    srv_print_innodb_table_monitor = TRUE;
    os_event_set(srv_lock_timeout_thread_event);
  }
  else if (table_name_len == sizeof S_innodb_mem_validate &&
           !memcmp(table_name, S_innodb_mem_validate, sizeof S_innodb_mem_validate))
  {
...
int row_create_index_for_mysql(
    /*=======================*/
                                ...
                                large. */
{
  ...
  for (i = 0; i < dict_index_get_n_fields(index); i++)
  {
    for (j = 0; j < i; j++)
    {
      if (0 == ut_strcmp(dict_index_get_nth_field(index, j)->name, dict_index_get_nth_field(index, i)->name))
      {
        ut_print_timestamp(stderr);

        fputs("  InnoDB: Error: column ", stderr);
        ut_print_name(stderr, trx, dict_index_get_nth_field(index, i)->name);
...
int row_table_add_foreign_constraints(
    /*==============================*/
    /* out: error code or DB_SUCCESS */
    trx_t *trx,             /* in: transaction */
                            ...
                            any foreign keys are found. */
{
  ...
  err = dict_create_foreign_constraints(trx, sql_string, name, reject_fks);
...
in srv0srv.c. */
static int row_drop_table_for_mysql_in_background(
    /*===================================*/
    /* out: error code or DB_SUCCESS */
...
already_dropped:
  ...
  ut_print_timestamp(stderr);
  fprintf(stderr, "  InnoDB: Dropped table %s in background drop queue.\n", drop->table_name);
...
int row_discard_tablespace_for_mysql(
    /*=============================*/
    /* out: error code or DB_SUCCESS */
    const char *name, /* in: table name */
    trx_t *trx)       /* in: transaction handle */
{
  ...
  buf = mem_alloc((sizeof discard_tablespace_proc1) + (sizeof discard_tablespace_proc2) + 20 + ut_strlenq(name, '\''));

  memcpy(buf, discard_tablespace_proc1, sizeof discard_tablespace_proc1);
  sprintf(ut_strcpyq(buf + (sizeof discard_tablespace_proc1 - 1), '\'', name), discard_tablespace_proc2,
          (ulong)ut_dulint_get_high(new_id), (ulong)ut_dulint_get_low(new_id));
...
int row_import_tablespace_for_mysql(
    /*============================*/
    /* out: error code or DB_SUCCESS */
    const char *name, /* in: table name */
    trx_t *trx)       /* in: transaction handle */
{
  ...
  success = fil_open_single_table_tablespace(TRUE, table->space, table->name);
  if (success)
  {
...
int row_truncate_table_for_mysql(
    /*=========================*/
    /* out: error code or DB_SUCCESS */
    ...
    trx_t *trx)          /* in: transaction handle */
{
  ...
  sprintf(sql, renumber_tablespace_proc, (ulong)ut_dulint_get_high(table->id), (ulong)ut_dulint_get_low(table->id),
          (ulong)ut_dulint_get_high(new_id), (ulong)ut_dulint_get_low(new_id));
    ...
    ibool drop_db)    /* in: TRUE=dropping whole database */
{
  ...
  else
  {
    ...
    {
      dir_path_of_temp_table = mem_strdup(table->dir_path_of_temp_table);
      is_path = TRUE;
    ...
    {
      ut_print_timestamp(stderr);
      fputs("  InnoDB: Error: not able to remove table ", stderr);
      ut_print_name(stderr, trx, name);
    ...
    if (err == DB_SUCCESS && space_id > 0)
    {
      ...
      if (!success)
      {
        ...
        ut_print_timestamp(stderr);
        fprintf(stderr, "  InnoDB: Error: not able to delete tablespace %lu of table ", (ulong)space_id);
        ut_print_name(stderr, trx, name);
        fputs("!\n", stderr);
        err = DB_ERROR;
      }
...
int row_drop_database_for_mysql(
    /*========================*/
    /* out: error code or DB_SUCCESS */
...
loop:
  ...
  while ((table_name = dict_get_first_table_name_in_db(name)))
  {
    ...
    if (table->n_mysql_handles_opened > 0)
    {
      ...
      ut_print_timestamp(stderr);
      fputs("  InnoDB: Warning: MySQL is trying to drop database ", stderr);
      ut_print_name(stderr, trx, name);
      fputs(
          "\n"
    ...
    if (err != DB_SUCCESS)
    {
      ...
      ut_print_name(stderr, trx, name);
      fprintf(stderr, " failed with error %lu for table ", (ulint)err);
      ut_print_name(stderr, trx, table_name);
      putc('\n', stderr);
      break;
    }
...
tables in MySQL. */
static ibool row_is_mysql_tmp_table_name(
    /*========================*/
...
int row_rename_table_for_mysql(
    /*=======================*/
    /* out: error code or DB_SUCCESS */
    ...
    trx_t *trx)           /* in: transaction handle */
{
  ...
  len = (sizeof str1) + (sizeof str2) + (sizeof str3) + (sizeof str5) - 4 + ut_strlenq(new_name, '\'') +
        ut_strlenq(old_name, '\'');
  ...
  if (row_is_mysql_tmp_table_name(new_name))
  {
    ...
    err = dict_foreign_parse_drop_constraints(heap, trx, table, &n_constraints_to_drop, &constraints_to_drop);
    if (err != DB_SUCCESS)
    {
      goto funct_exit;
    }
    ...
    len += 2 * n_constraints_to_drop * (ut_strlenq(old_name, '\'') - ut_strlenq(old_name + db_name_len, '\''));

    for (i = 0; i < n_constraints_to_drop; i++)
    {
      ulint addlen =
          2 * ut_strlenq(constraints_to_drop[i], '\'') + ((sizeof str4a1) + (sizeof str4a2) + (sizeof str4a3) - 3);
  ...
  if (db_name_len)
  {
    ...
    for (i = 0; i < n_constraints_to_drop; i++)
    {
      ...
      sqlend = ut_memcpyq(sqlend, '\'', old_name, db_name_len);
      sqlend = ut_strcpyq(sqlend, '\'', constraints_to_drop[i]);
  ...
  else
  {
    ...
    success = dict_table_rename_in_cache(table, new_name, !row_is_mysql_tmp_table_name(new_name));
    if (!success)
    {
      trx->error_state = DB_SUCCESS;
      trx_general_rollback_for_mysql(trx, FALSE, NULL);
      trx->error_state = DB_SUCCESS;
      ut_print_timestamp(stderr);
      fputs(" InnoDB: Error in table rename, cannot rename ", stderr);
      ut_print_name(stderr, trx, old_name);
      fputs(" to ", stderr);
      ut_print_name(stderr, trx, new_name);
      putc('\n', stderr);
    ...
    if (row_is_mysql_tmp_table_name(old_name))
    {
      ...
      if (err != DB_SUCCESS)
      {
        ...
        ut_a(dict_table_rename_in_cache(table, old_name, FALSE));
        trx->error_state = DB_SUCCESS;
        trx_general_rollback_for_mysql(trx, FALSE, NULL);
        trx->error_state = DB_SUCCESS;
      }
    ...
    else
    {
      if (err != DB_SUCCESS)
      {
        ...
        fputs("  InnoDB: Error: in RENAME TABLE table ", stderr);
        ut_print_name(stderr, trx, new_name);
        fputs(
            "\n"
...
static ibool row_scan_and_check_index(
    /*=====================*/
    /* out: TRUE if ok */
    row_prebuilt_t *prebuilt, /* in: prebuilt struct in MySQL */
...
loop:
  ...
  *n_rows = *n_rows + 1;
  ...
  if (prev_entry != NULL)
  {
    ...
    cmp = cmp_dtuple_rec_with_match(prev_entry, rec, offsets, &matched_fields, &matched_bytes);
    contains_null = FALSE;
    ...
    {
      if (UNIV_SQL_NULL == dfield_get_len(dtuple_get_nth_field(prev_entry, i)))
      {
        contains_null = TRUE;
      }
    }
    ...
    if (cmp > 0)
    {
      fputs("InnoDB: index records in a wrong order in ", stderr);
    not_ok:
    ...
    else if ((index->type & DICT_UNIQUE) && !contains_null &&
