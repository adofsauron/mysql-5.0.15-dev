...
sp_rcontext::sp_rcontext(uint fsize, uint hmax, uint cmax)
    : m_count(0), m_fsize(fsize), m_result(NULL), m_hcount(0), m_hsp(0), m_ihsp(0), m_hfound(-1), m_ccount(0)
{
...
int sp_rcontext::set_item_eval(THD *thd, uint idx, Item **item_addr, enum_field_types type)
{
  Item *it;
  Item *reuse_it;
...
void sp_rcontext::save_variables(uint fp)
{
  while (fp < m_count)
  {
...
void sp_rcontext::restore_variables(uint fp)
{
  uint i = m_count;

  while (i-- > fp) m_frame[i] = m_saved.pop();
}
...
sp_cursor::sp_cursor(sp_lex_keeper *lex_keeper, sp_instr_cpush *i)
    : m_lex_keeper(lex_keeper), server_side_cursor(NULL), m_i(i)
{
  /*
...
int sp_cursor::fetch(THD *thd, List<struct sp_pvar> *vars)
{
  if (!server_side_cursor)
  {
    my_message(ER_SP_CURSOR_NOT_OPEN, ER(ER_SP_CURSOR_NOT_OPEN), MYF(0));
  ...
  {
    my_message(ER_SP_WRONG_NO_OF_FETCH_ARGS, ER(ER_SP_WRONG_NO_OF_FETCH_ARGS), MYF(0));
    return -1;
  }
