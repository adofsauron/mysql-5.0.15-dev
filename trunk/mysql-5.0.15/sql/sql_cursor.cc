...
int mysql_open_cursor(THD *thd, uint flags, select_result *result, Server_side_cursor **pcursor)
{
  ...
  */
  ...
  else if (result_materialize->table)
  {
    ...
    if (!(materialized_cursor = new (mem_root) Materialized_cursor(result, table)))
    {
      rc = 1;
      goto err_open;
    }
...
Sensitive_cursor::Sensitive_cursor(THD *thd, select_result *result_arg)
    : Server_side_cursor(&main_mem_root, result_arg), stmt_arena(0), join(0), close_at_commit(FALSE)
{
...
void Sensitive_cursor::post_open(THD *thd)
{
  ...
  *mem_root = *thd->mem_root;
  ...
  init_sql_alloc(thd->mem_root, thd->variables.query_alloc_block_size, thd->variables.query_prealloc_size);
...
void Sensitive_cursor::reset_thd(THD *thd)
{
...
int Sensitive_cursor::open(JOIN *join_arg)
{
  join = join_arg;
  THD *thd = join->thd;
...
void Sensitive_cursor::fetch(ulong num_rows)
{
  THD *thd = join->thd;
  JOIN_TAB *join_tab = join->join_tab + join->const_tables;
  ...
  DBUG_ASSERT(thd->derived_tables == 0 && thd->open_tables == 0 && thd->lock == 0);
  ...
  for (info = ht_info; info->read_view; info++) (info->ht->set_cursor_read_view)(info->read_view);
  ...
  for (info = ht_info; info->read_view; info++) (info->ht->set_cursor_read_view)(0);
...
Materialized_cursor::Materialized_cursor(select_result *result_arg, TABLE *table_arg)
    : Server_side_cursor(&table_arg->mem_root, result_arg), table(table_arg), fetch_limit(0), fetch_count(0)
{
...
int Materialized_cursor::open(JOIN *join __attribute__((unused)))
{
  ...
  rc = (table->fill_item_list(&item_list) || result->prepare(item_list, &fake_unit) || table->file->ha_rnd_init(TRUE));
  thd->restore_active_arena(this, &backup_arena);
  return rc;
}
...
bool Select_materialize::send_fields(List<Item> &list, uint flags)
{
  bool rc;
  DBUG_ASSERT(table == 0);
  if (create_result_table(unit->thd, unit->get_unit_column_types(), FALSE, thd->options | TMP_TABLE_ALL_COLUMNS, ""))
    return TRUE;
  /*
