...
loop:
  ...
  if (!rec_get_deleted_flag(rec, sys_tables->comp))
  {
    ...
    if (space_id != 0 && in_crash_recovery)
    {
      ...
      fil_space_for_table_exists_in_mem(space_id, name, FALSE, TRUE, TRUE);
    }
    ...
    if (space_id != 0 && !in_crash_recovery)
    {
      ...
      fil_open_single_table_tablespace(FALSE, space_id, name);
    }
    ...
    mem_heap_t *heap)    /* in: memory heap for temporary storage */
{
  ...
  for (i = 0; i < table->n_cols - DATA_N_SYS_COLS; i++)
  {
    ...
    ut_a(0 == ut_strcmp("NAME", dict_field_get_col(dict_index_get_nth_field(sys_index, 4))->name));
    ...
    if (dtype_is_non_binary_string_type(mtype, prtype) && dtype_get_charset_coll(prtype) == 0)
    {
    ...
    ut_a(0 == ut_strcmp("PREC", dict_field_get_col(dict_index_get_nth_field(sys_index, 8))->name));
...
static void dict_load_report_deleted_index(
    /*===========================*/
    ...
    ulint field)      /* in: index field, or ULINT_UNDEFINED */
{
  ...
  {
    fprintf(stderr, "InnoDB: Index field %lu is delete marked.\n", field);
  }
  else
  {
    ...
    mem_heap_t *heap)    /* in: memory heap for temporary storage */
{
  ...
  for (i = 0; i < index->n_fields; i++)
  {
    ...
    ut_a(0 == ut_strcmp("COL_NAME", dict_field_get_col(dict_index_get_nth_field(sys_index, 4))->name));

    field = rec_get_nth_field_old(rec, 4, &len);

    dict_mem_index_add_field(index, mem_heap_strdupl(heap, (char *)field, len), 0, prefix_len);
...
cache. */
static ibool dict_load_indexes(
    /*==============*/
    ...
    mem_heap_t *heap)    /* in: memory heap for temporary storage */
{
  ...
  for (;;)
  {
    ...
    {
      dict_load_report_deleted_index(table->name, ULINT_UNDEFINED);
    ...
    ut_a(0 == ut_strcmp("PAGE_NO", dict_field_get_col(dict_index_get_nth_field(sys_index, 8))->name));
                      ...
                      databasename/tablename format */
{
  ...
  if (!btr_pcur_is_on_user_rec(&pcur, &mtr) || rec_get_deleted_flag(rec, sys_tables->comp))
  {
    /* Not found */
  ...
  ut_a(0 == ut_strcmp("SPACE", dict_field_get_col(dict_index_get_nth_field(sys_index, 9))->name));
  ...
  if (space != 0)
  {
    if (fil_space_for_table_exists_in_mem(space, name, FALSE, FALSE, FALSE))
    {
    ...
    else
    {
      ...
      if (!fil_open_single_table_tablespace(TRUE, space, name))
      {
  ...
  ut_a(0 == ut_strcmp("N_COLS", dict_field_get_col(dict_index_get_nth_field(sys_index, 4))->name));

  field = rec_get_nth_field_old(rec, 4, &len);
  n_cols = mach_read_from_4(field);

  /* The high-order bit of N_COLS is the "compact format" flag. */
  table = dict_mem_table_create(name, space, n_cols & ~0x80000000UL, !!(n_cols & 0x80000000UL));

  table->ibd_file_missing = ibd_file_missing;

  ut_a(0 == ut_strcmp("ID", dict_field_get_col(dict_index_get_nth_field(sys_index, 3))->name));
...
static ulint dict_load_foreign(
    /*==============*/
    ...
    ibool check_types) /* in: TRUE=check type compatibility */
{
  ...
  if (!btr_pcur_is_on_user_rec(&pcur, &mtr) || rec_get_deleted_flag(rec, sys_foreign->comp))
  {
    /* Not found */
  ...
  foreign->n_fields = mach_read_from_4(rec_get_nth_field_old(rec, 5, &len));
...
loop:
  ...
  if (0 != cmp_data_data(dfield_get_type(dfield), dfield_get_data(dfield), dfield_get_len(dfield), field, len))
  {
    goto load_next_index;
  }
