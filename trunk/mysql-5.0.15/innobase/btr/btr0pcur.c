    ...
    mtr_t *mtr)         /* in: mtr */
{
  ...
  cursor->old_stored = BTR_PCUR_OLD_STORED;
  cursor->old_rec =
      dict_tree_copy_rec_order_prefix(tree, rec, &cursor->old_n_fields, &cursor->old_rec_buf, &cursor->buf_size);
...
ibool btr_pcur_restore_position(
    /*======================*/
    ...
    mtr_t *mtr)         /* in: mtr */
{
  ...
  if (UNIV_UNLIKELY(cursor->old_stored != BTR_PCUR_OLD_STORED) ||
      UNIV_UNLIKELY(cursor->pos_state != BTR_PCUR_WAS_POSITIONED && cursor->pos_state != BTR_PCUR_IS_POSITIONED))
  {
  ...
  if (UNIV_UNLIKELY(cursor->rel_pos == BTR_PCUR_AFTER_LAST_IN_TREE || cursor->rel_pos == BTR_PCUR_BEFORE_FIRST_IN_TREE))
  {
    /* In these cases we do not try an optimistic restoration,
    but always do a search */

    btr_cur_open_at_index_side(cursor->rel_pos == BTR_PCUR_BEFORE_FIRST_IN_TREE, btr_pcur_get_btr_cur(cursor)->index,
                               latch_mode, btr_pcur_get_btr_cur(cursor), mtr);

    cursor->block_when_stored = buf_block_align(btr_pcur_get_page(cursor));
  ...
  if (UNIV_LIKELY(latch_mode == BTR_SEARCH_LEAF) || UNIV_LIKELY(latch_mode == BTR_MODIFY_LEAF))
  {
    /* Try optimistic restoration */

    if (UNIV_LIKELY(buf_page_optimistic_get(latch_mode, cursor->block_when_stored, page, cursor->modify_clock, mtr)))
    {
      ...
      if (cursor->rel_pos == BTR_PCUR_ON)
      {
        ...
        index = dict_tree_find_index(btr_cur_get_tree(btr_pcur_get_btr_cur(cursor)), rec);
        ...
        ut_ad(cmp_rec_rec(cursor->old_rec, rec, offsets1, offsets2, index) == 0);
        mem_heap_free(heap);
#endif /* UNIV_DEBUG */
        return (TRUE);
      }
  ...
  if (cursor->rel_pos == BTR_PCUR_ON && btr_pcur_is_on_user_rec(cursor, mtr) &&
      0 == cmp_dtuple_rec(tuple, btr_pcur_get_rec(cursor),
                          rec_get_offsets(btr_pcur_get_rec(cursor), btr_pcur_get_btr_cur(cursor)->index, NULL,
