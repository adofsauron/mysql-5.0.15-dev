...
static void trx_doublewrite_init(
    /*=================*/
...
/*===============================================*/
{
  ...
  mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED, TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED_N,
                   MLOG_4BYTES, &mtr);
  mtr_commit(&mtr);
  ...
  else
  {
    ...
    for (i = 0; i < 2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2; i++)
    {
      ...
      if (i == FSP_EXTENT_SIZE / 2)
      {
        ...
        mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_REPEAT + TRX_SYS_DOUBLEWRITE_BLOCK1, page_no, MLOG_4BYTES,
                         &mtr);
      }
      else if (i == FSP_EXTENT_SIZE / 2 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE)
      {
        ...
        mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_REPEAT + TRX_SYS_DOUBLEWRITE_BLOCK2, page_no, MLOG_4BYTES,
                         &mtr);
      }
    ...
    mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC + TRX_SYS_DOUBLEWRITE_REPEAT, TRX_SYS_DOUBLEWRITE_MAGIC_N,
                     MLOG_4BYTES, &mtr);
    ...
    ibool restore_corrupt_pages)
{
  ...
  if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED) != TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED_N)
  {
    ...
    fprintf(stderr, "InnoDB: Resetting space id's in the doublewrite buffer\n");
  }
  else
  {
    trx_sys_multiple_tablespace_format = TRUE;
  ...
  fil_io(OS_FILE_READ, TRUE, TRX_SYS_SPACE, block1, 0, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE, buf, NULL);
  fil_io(OS_FILE_READ, TRUE, TRX_SYS_SPACE, block2, 0, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE,
  ...
  for (i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++)
  {
    ...
    if (trx_doublewrite_must_reset_space_ids)
    {
      ...
      fil_io(OS_FILE_WRITE, TRUE, 0, source_page_no, 0, UNIV_PAGE_SIZE, page, NULL);
    ...
    else
    {
      ...
      if (buf_page_is_corrupted(read_buf))
      {
        ...
        {
          fprintf(stderr, "InnoDB: Dump of the page:\n");
          buf_page_print(read_buf);
          fprintf(stderr, "InnoDB: Dump of corresponding page in doublewrite buffer:\n");
          buf_page_print(page);
...
/*==========================*/
{
  ...
  mlog_write_dulint(sys_header + TRX_SYS_TRX_ID_STORE, trx_sys->max_trx_id, &mtr);
  mtr_commit(&mtr);
}
    ...
    mtr_t *mtr)            /* in: mtr */
{
  ...
  {
    mlog_write_string(sys_header + field + TRX_SYS_MYSQL_LOG_NAME, (byte *)file_name, 1 + ut_strlen(file_name), mtr);
  }
  ...
  mlog_write_ulint(sys_header + field + TRX_SYS_MYSQL_LOG_OFFSET_LOW, (ulint)(offset & 0xFFFFFFFFUL), MLOG_4BYTES, mtr);
}
                ...
                i.e., page number TRX_SYS_PAGE_NO in the tablespace */
{
  ...
  if (mach_read_from_4(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_MAGIC_N_FLD) ==
...
/*===================================*/
{
  ...
  trx_sys_mysql_bin_log_pos =
      (((ib_longlong)trx_sys_mysql_bin_log_pos_high) << 32) + (ib_longlong)trx_sys_mysql_bin_log_pos_low;
  ...
  fprintf(stderr, "InnoDB: Last MySQL binlog file position %lu %lu, file name %s\n", trx_sys_mysql_bin_log_pos_high,
          trx_sys_mysql_bin_log_pos_low, trx_sys_mysql_bin_log_name);
...
/*====================================*/
{
  ...
  if (mach_read_from_4(sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_MAGIC_N_FLD) !=
      TRX_SYS_MYSQL_LOG_MAGIC_N)
  {
    mtr_commit(&mtr);
  ...
  fprintf(stderr,
          ...
          (ulong)mach_read_from_4(sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_HIGH),
          (ulong)mach_read_from_4(sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_LOW),
          sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_NAME);
  /* Copy the master log position info to global variables we can
  use in ha_innobase.cc to initialize glob_mi to right values */

  ut_memcpy(trx_sys_mysql_master_log_name, sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_NAME,
            TRX_SYS_MYSQL_LOG_NAME_LEN);

  trx_sys_mysql_master_log_pos =
      (((ib_longlong)mach_read_from_4(sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_HIGH))
       << 32) +
      (ib_longlong)mach_read_from_4(sys_header + TRX_SYS_MYSQL_MASTER_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_LOW);
  mtr_commit(&mtr);
}
    ...
    mtr_t *mtr) /* in: mtr */
{
  ...
  mlog_write_dulint(sys_header + TRX_SYS_TRX_ID_STORE, ut_dulint_create(0, 1), mtr);
...
/*==========================*/
{
  ...
  trx_sys->max_trx_id = ut_dulint_add(
      ut_dulint_align_up(mtr_read_dulint(sys_header + TRX_SYS_TRX_ID_STORE, &mtr), TRX_SYS_TRX_ID_WRITE_MARGIN),
