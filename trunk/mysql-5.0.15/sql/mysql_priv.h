...
bool mysql_select(THD *thd, Item ***rref_pointer_array, TABLE_LIST *tables,
                  uint wild_num, List<Item> &list, COND *conds, uint og_num,
...
int mysql_handle_derived(LEX *lex, int (*processor)(THD *thd, LEX *lex,
                                                    TABLE_LIST *table));
...
bool mysql_prepare_update(THD *thd, TABLE_LIST *table_list, Item **conds,
                          uint order_num, ORDER *order);
...
bool mysql_ha_read(THD *, TABLE_LIST *, enum enum_ha_read_modes, char *,
                   List<Item> *, enum ha_rkey_function, Item *, ha_rows,
                   ha_rows);
...
bool add_field_to_list(THD *thd, char *field_name, enum enum_field_types type,
                       char *length, char *decimal, uint type_modifier,
...
bool setup_fields(THD *thd, Item **ref_pointer_array, List<Item> &item,
...
TABLE_LIST *find_table_in_list(TABLE_LIST *table, uint offset_to_list,
                               const char *db_name, const char *table_name);
...
inline TABLE_LIST *find_table_in_global_list(TABLE_LIST *table,
                                             const char *db_name,
                                             const char *table_name) {
  return find_table_in_list(table, offsetof(TABLE_LIST, next_global), db_name,
                            table_name);
}
...
inline TABLE_LIST *find_table_in_local_list(TABLE_LIST *table,
                                            const char *db_name,
                                            const char *table_name) {
  return find_table_in_list(table, offsetof(TABLE_LIST, next_local), db_name,
                            table_name);
}
...
void print_plan(JOIN *join, double read_time, double record_count, uint idx,
                const char *info);
#endif
...
extern pthread_mutex_t LOCK_mysql_create_db, LOCK_Acl, LOCK_open,
    ...
    LOCK_global_system_variables, LOCK_user_conn, LOCK_bytes_sent,
    LOCK_bytes_received;
#ifdef HAVE_OPENSSL
extern pthread_mutex_t LOCK_des_key_file;
#endif
...
bool mysql_create_frm(THD *thd, my_string file_name, const char *db,
                      const char *table, HA_CREATE_INFO *create_info,
                      List<create_field> &create_field, uint key_count,
                      KEY *key_info, handler *db_type);
int rea_create_table(THD *thd, my_string file_name, const char *db,
                     const char *table, HA_CREATE_INFO *create_info,
                     List<create_field> &create_field, uint key_count,
                     KEY *key_info);
...
ha_rows filesort(THD *thd, TABLE *form, struct st_sort_field *sortorder,
                 uint s_length, SQL_SELECT *select, ha_rows max_rows,
...
inline void setup_table_map(TABLE *table, TABLE_LIST *table_list,
                            uint tablenr) {
  table->used_fields = 0;
  table->const_table = 0;
  table->null_row = 0;
