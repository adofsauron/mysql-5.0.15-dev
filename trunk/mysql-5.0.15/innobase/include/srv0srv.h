...
what these mean */
#define SRV_UNIX_FDATASYNC                         \
  1 /* This is the default; it is currently mapped \
...
#define SRV_FORCE_IGNORE_CORRUPT     \
  1 /* let the server run even if it \
    detects a corrupt page */
#define SRV_FORCE_NO_BACKGROUND     \
  2 /* prevent the main thread from \
    running: if a crash would occur \
    in purge, this prevents it */
#define SRV_FORCE_NO_TRX_UNDO        \
  3 /* do not run trx rollback after \
    recovery */
#define SRV_FORCE_NO_IBUF_MERGE         \
  4 /* prevent also ibuf operations:    \
    if they would cause a crash, better \
    not do them */
#define SRV_FORCE_NO_UNDO_LOG_SCAN     \
  5 /* do not look at undo logs when   \
...
#define SRV_WORKER                              \
  3 /* threads serving parallelized queries and \
    queries released from lock wait */
#define SRV_BUFFER                          \
  4 /* thread flushing dirty buffer blocks, \
    not currently in use */
#define SRV_RECOVERY                 \
  5 /* threads finishing a recovery, \
    not currently in use */
#define SRV_INSERT                                \
  6 /* thread flushing the insert buffer to disk, \
    not currently in use */
#define SRV_MASTER                                \
  7 /* the master thread, (whose type number must \
    be biggest) */
