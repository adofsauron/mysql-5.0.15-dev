...
const char *rpl_status_type[] = { "AUTH_MASTER",   "ACTIVE_SLAVE",
                                  "IDLE_SLAVE",    "LOST_SOLDIER",
                                  "TROOP_SOLDIER", "RECOVERY_CAPTAIN",
                                  "NULL",          NullS };
TYPELIB rpl_status_typelib = { array_elements(rpl_status_type) - 1, "",
                               rpl_status_type,                     NULL };

static Slave_log_event *
find_slave_event(IO_CACHE *log, const char *log_file_name, char *errmsg);
...
extern "C" uint32 *slave_list_key(SLAVE_INFO *si, uint *len,
...
find_slave_event(IO_CACHE *log, const char *log_file_name, char *errmsg) {
  ...
  for (i = 0; i < 2; i++) {
    if (!(ev = Log_event::read_log_event(log, (pthread_mutex_t *)0, 0))) {
      my_snprintf(errmsg, SLAVE_ERRMSG_SIZE, "Error reading event in log '%s'",
                  (char *)log_file_name);
      return 0;
    }
...
bool show_new_master(THD *thd) {
  ...
  if (translate_master(thd, lex_mi, errmsg)) {
    if (errmsg[0])
      my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), "SHOW NEW MASTER",
               errmsg);
    DBUG_RETURN(TRUE);
  } else {
    field_list.push_back(new Item_empty_string("Log_name", 20));
    field_list.push_back(
        new Item_return_int("Log_pos", 10, MYSQL_TYPE_LONGLONG));
...
HOSTS";
  ...
  while ((row = mysql_fetch_row(res))) {
    ...
    if ((old_si =
             (SLAVE_INFO *)hash_search(&slave_list, (byte *)&server_id, 4)))
      si = old_si;
    else {
      if (!(si = (SLAVE_INFO *)my_malloc(sizeof(SLAVE_INFO), MYF(MY_WME)))) {
...
pthread_handler_t handle_failsafe_rpl(void *arg) {
  ...
  pthread_mutex_lock(&LOCK_rpl_status);
  msg = thd->enter_cond(&COND_rpl_status, &LOCK_rpl_status,
                        "Waiting for request");
...
bool show_slave_hosts(THD *thd) {
  ...
  field_list.push_back(
      new Item_return_int("Rpl_recovery_rank", 7, MYSQL_TYPE_LONG));
...
int connect_to_master(THD *thd, MYSQL *mysql, MASTER_INFO *mi) {
  ...
  if (!mysql_real_connect(mysql, mi->host, mi->user, mi->password, 0, mi->port,
                          0, 0))
    DBUG_RETURN(1);
  mysql->reconnect = 1;
  DBUG_RETURN(0);
...
static inline void cleanup_mysql_results(MYSQL_RES *db_res, MYSQL_RES **cur,
                                         MYSQL_RES **start) {
  for (; cur >= start; --cur) {
    if (*cur)
...
bool load_master_data(THD *thd) {
  ...
  */
  ...
  // now that we are connected, get all database and tables in each
  {
    ...
    */
    ...
    if (!(table_res =
              (MYSQL_RES **)thd->alloc(num_dbs * sizeof(MYSQL_RES *)))) {
    ...
    for (cur_table_res = table_res; cur_table_res < table_res_end;
      ...
      */
      ...
      if (!db_ok(db, replicate_do_db, replicate_ignore_db) ||
          !db_ok_with_wild_table(db) || !strcmp(db, "mysql")) {
        *cur_table_res = 0;
        continue;
      }
      ...
      if ((error =
               fetch_db_tables(thd, &mysql, db, *cur_table_res, active_mi))) {
  ...
  if (purge_relay_logs(&active_mi->rli, thd,
                       0 /* not only reset, but also reinit */, &errmsg)) {
  ...
  */
  ...
  if (restart_thread_mask) {
    error = start_slave_threads(
        0 /* mutex not needed */, 1 /* wait for start */, active_mi,
        master_info_file, relay_log_info_file, restart_thread_mask);
  }
