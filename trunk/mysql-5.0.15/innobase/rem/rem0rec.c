                         ...
                         in: n=rec_offs_n_fields(offsets) */
{
  ...
  if (UNIV_LIKELY(index->table->comp))
  {
    ...
    do
    {
      ...
      if (!(dtype_get_prtype(dict_col_get_type(dict_field_get_col(field))) & DATA_NOT_NULL))
      {
      ...
      if (UNIV_UNLIKELY(!field->fixed_len))
      {
        ...
        dtype_t *type = dict_col_get_type(dict_field_get_col(field));
        len = *lens--;
        if (UNIV_UNLIKELY(dtype_get_len(type) > 255) || UNIV_UNLIKELY(dtype_get_mtype(type) == DATA_BLOB))
        {
          if (len & 0x80)
          {
            /* 1exxxxxxx xxxxxxxx */
    ...
    ulint line)          /* in: line number where called */
{
  ...
  if (UNIV_UNLIKELY(!offsets) || UNIV_UNLIKELY(rec_offs_get_n_alloc(offsets) < size))
  {
    if (!*heap)
    {
      *heap = mem_heap_create_func(size * sizeof(ulint), NULL, MEM_HEAP_DYNAMIC, file, line);
    }
...
ulint rec_get_converted_size_new(
    /*=======================*/
    /* out: size */
    dict_index_t *index, /* in: record descriptor */
    dtuple_t *dtuple)    /* in: data tuple */
{
  ulint size = REC_N_NEW_EXTRA_BYTES + (index->n_nullable + 7) / 8;
  dict_field_t *field;
  dtype_t *type;
  ulint i;
  ulint n_fields;
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    ut_ad(len != UNIV_SQL_NULL || !(dtype_get_prtype(type) & DATA_NOT_NULL));
    ...
    else if (len < 128 || (dtype_get_len(type) < 256 && dtype_get_mtype(type) != DATA_BLOB))
    {
      size++;
    }
    else
...
void rec_set_nth_field_extern_bit_old(
    /*=============================*/
                ...
                write to log about the change */
{
  ...
  if (mtr)
  {
    mlog_write_ulint(rec - REC_N_OLD_EXTRA_BYTES - 2 * (i + 1), info, MLOG_2BYTES, mtr);
  }
  else
  {
    rec_2_set_field_end_info(rec, i, info);
...
void rec_set_nth_field_extern_bit_new(
    /*=============================*/
    rec_t *rec,          /* in: record */
    dict_index_t *index, /* in: record descriptor */
                         ...
                         we do not write to log about the change */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    lens--;
    if (dtype_get_len(type) > 255 || dtype_get_mtype(type) == DATA_BLOB)
    {
      ulint len = lens[1];
      if (len & 0x80)
      { /* 1exxxxxx: 2-byte length */
        if (i == ith)
        {
          if (!val == !(len & 0x40))
          {
          ...
          if (mtr)
          {
            mlog_write_ulint(lens + 1, len, MLOG_1BYTE, mtr);
          }
          else
          {
            lens[1] = (byte)len;
...
void rec_set_field_extern_bits(
    /*======================*/
    rec_t *rec,          /* in: record */
    dict_index_t *index, /* in: record descriptor */
                         ...
                         to log about the change */
{
  ...
  if (UNIV_LIKELY(index->table->comp))
  {
    for (i = 0; i < n_fields; i++)
    {
      rec_set_nth_field_extern_bit_new(rec, index, vec[i], TRUE, mtr);
    }
  }
  else
  {
    for (i = 0; i < n_fields; i++)
    {
      rec_set_nth_field_extern_bit_old(rec, vec[i], TRUE, mtr);
    }
  }
}
    ...
    dtuple_t *dtuple) /* in: data tuple */
{
  ...
  rec_set_info_bits(rec, FALSE, dtuple_get_info_bits(dtuple) & REC_INFO_BITS_MASK);
    ...
    dtuple_t *dtuple)    /* in: data tuple */
{
  ...
  const ulint status = dtuple_get_info_bits(dtuple) & REC_NEW_STATUS_MASK;
  ut_ad(index->table->comp);
...
init:
  ...
  rec_set_info_bits(rec, TRUE, dtuple_get_info_bits(dtuple) & REC_INFO_BITS_MASK);
    ...
    mem_heap_t *heap)    /* in: memory heap */
{
  ...
  *offsets_ = (sizeof offsets_) / sizeof *offsets_;
  ...
  dtuple_set_info_bits(tuple, rec_get_info_bits(rec, index->table->comp));
    ...
    rec_t *rec) /* in: physical record */
{
  ...
  fprintf(file,
          ...
          (ulong)n, rec_get_1byte_offs_flag(rec) ? 1 : 2, (ulong)rec_get_info_bits(rec, FALSE));
  ...
  for (i = 0; i < n; i++)
  {
    ...
    {
      fprintf(file, " SQL NULL, size %lu ", rec_get_nth_field_size(rec, i));
    }
    putc(';', file);
  }
    ...
    const ulint *offsets) /* in: array returned by rec_get_offsets() */
{
  ...
  fprintf(file,
          ...
          (ulong)rec_offs_n_fields(offsets), (ulong)rec_get_info_bits(rec, TRUE));
    ...
    dict_index_t *index) /* in: record descriptor */
{
  ...
  else
  {
    ...
    *offsets_ = (sizeof offsets_) / sizeof *offsets_;
    ...
    rec_print_new(file, rec, rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &heap));
    if (UNIV_LIKELY_NULL(heap))
    {
      mem_heap_free(heap);
    }
