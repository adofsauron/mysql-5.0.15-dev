...
my_bool my_net_write(NET *net, const char *packet, ulong len)
{
  ...
  */
  while (len >= MAX_PACKET_LENGTH)
  {
    ...
    if (net_write_buff(net, (char *)buff, NET_HEADER_SIZE) || net_write_buff(net, packet, z_size))
      return 1;
    packet += z_size;
    len -= z_size;
  }
...
static my_bool net_write_buff(NET *net, const char *packet, ulong len)
{
  ...
  if (len > left_length)
  {
    if (net->write_pos != net->buff)
    {
      ...
      if (net_real_write(net, (char *)net->buff, (ulong)(net->write_pos - net->buff) + left_length))
        return 1;
      net->write_pos = net->buff;
      packet += left_length;
      len -= left_length;
