...
static my_bool emb_advanced_command(MYSQL *mysql, enum enum_server_command command, const char *header,
                                    ulong header_length, const char *arg, ulong arg_length, my_bool skip_check)
{
  my_bool result = 1;
  THD *thd = (THD *)mysql->thd;
  NET *net = &mysql->net;
  ...
  {
    strmov(net->last_error, ER(net->last_errno = CR_COMMANDS_OUT_OF_SYNC));
    return 1;
  }
...
static MYSQL_DATA *emb_read_rows(MYSQL *mysql, MYSQL_FIELD *mysql_fields __attribute__((unused)),
...
static MYSQL_FIELD *emb_list_fields(MYSQL *mysql) { return mysql->fields; }
...
static void emb_fetch_lengths(ulong *to, MYSQL_ROW column, unsigned int field_count)
{
  MYSQL_ROW end;
...
static int emb_stmt_execute(MYSQL_STMT *stmt)
{
  ...
  if (emb_advanced_command(stmt->mysql, COM_STMT_EXECUTE, 0, 0, header, sizeof(header), 1) ||
...
static MYSQL_RES *emb_mysql_store_result(MYSQL *mysql) { return mysql_store_result(mysql); }
...
my_bool emb_next_result(MYSQL *mysql)
{
  ...
  if (emb_advanced_command(mysql, COM_QUERY, 0, 0, thd->query_rest.ptr(), thd->query_rest.length(), 1) ||
...
int emb_read_change_user_result(MYSQL *mysql, char *buff __attribute__((unused)),
...
char **copy_arguments(int argc, char **argv)
{
  ...
  for (from = argv; from != end; from++) length += strlen(*from);

  if ((res = (char **)my_malloc(sizeof(argv) * (argc + 1) + length + argc, MYF(MY_WME))))
  {
...
extern "C"
{
  ...
  int init_embedded_server(int argc, char **argv, char **groups)
  {
    ...
    if (!(acl_error = acl_init(opt_noacl)) && !opt_noacl)
      (void)grant_init();
#endif
...
int check_embedded_connection(MYSQL *mysql)
{
  ...
  if ((result = check_user(thd, COM_CONNECT, scramble_buff, passwd_len, thd->db, true)))
    goto err;
...
static char *dup_str_aux(MEM_ROOT *root, const char *from, uint length, CHARSET_INFO *fromcs, CHARSET_INFO *tocs)
{
  ...
  if (tocs && String::needs_conversion(0, fromcs, tocs, &dummy32))
  {
    ...
    length = copy_and_convert(result, new_len, tocs, from, length, fromcs, &dummy_err);
  }
  else
  {
...
bool Protocol::send_fields(List<Item> *list, uint flags)
{
  ...
  if (!(client_field = thd->mysql->fields = (MYSQL_FIELD *)alloc_root(field_alloc, sizeof(MYSQL_FIELD) * field_count)))
    goto err;
  ...
  while ((item = it++))
  {
    ...
    client_field->db = dup_str_aux(field_alloc, server_field.db_name, strlen(server_field.db_name), cs, thd_cs);
    client_field->table =
        dup_str_aux(field_alloc, server_field.table_name, strlen(server_field.table_name), cs, thd_cs);
    client_field->name = dup_str_aux(field_alloc, server_field.col_name, strlen(server_field.col_name), cs, thd_cs);
    client_field->org_table =
        dup_str_aux(field_alloc, server_field.org_table_name, strlen(server_field.org_table_name), cs, thd_cs);
    client_field->org_name =
        dup_str_aux(field_alloc, server_field.org_col_name, strlen(server_field.org_col_name), cs, thd_cs);
    ...
    if (flags & (int)Protocol::SEND_DEFAULTS)
    {
      ...
      else
      {
        ...
        client_field->def = strmake_root(field_alloc, res->ptr(), client_field->def_length);
      }
    }
    else
      client_field->def = 0;
...
bool Protocol_prep::write()
{
  ...
  if (!data)
  {
    if (!(data = (MYSQL_DATA *)my_malloc(sizeof(MYSQL_DATA), MYF(MY_WME | MY_ZEROFILL))))
      return true;
