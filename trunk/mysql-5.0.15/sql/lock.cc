  ...
  *need_reopen = FALSE;
  ...
  for (;;) {
    ...
    rc = thr_lock_errno_to_mysql[(int)thr_multi_lock(
        sql_lock->locks, sql_lock->lock_count, thd->lock_id)];
...
static MYSQL_LOCK *get_lock_data(THD *thd, TABLE **table_ptr, uint count,
  ...
  *write_lock_used = 0;
  for (i = tables = lock_count = 0; i < count; i++) {
    ...
    */
    if (table_ptr[i]->s->system_table &&
        table_ptr[i]->reginfo.lock_type >= TL_WRITE_ALLOW_WRITE && count != 1) {
  ...
  if (!(sql_lock = (MYSQL_LOCK *)my_malloc(
            sizeof(*sql_lock) + sizeof(THR_LOCK_DATA *) * tables +
  ...
  for (i = 0; i < count; i++) {
    ...
    *to++ = table;
    ...
    locks = table->file->store_lock(thd, locks,
                                    get_old_locks ? TL_IGNORE : lock_type);
    if (locks)
...
int lock_table_name(THD *thd, TABLE_LIST *table_list) {
  ...
  key_length =
      (uint)(strmov(strmov(key, db) + 1, table_list->table_name) - key) + 1;

  /* Only insert the table if we haven't insert it already */
  for (table = (TABLE *)hash_search(&open_cache, (byte *)key, key_length);
       table; table = (TABLE *)hash_next(&open_cache, (byte *)key, key_length))
    if (table->in_use == thd)
      DBUG_RETURN(0);
  ...
  */
  ...
  DBUG_RETURN(test(
      remove_table_from_cache(thd, db, table_list->table_name, RTFC_NO_FLAG)));
}
...
bool lock_table_names(THD *thd, TABLE_LIST *table_list) {
  ...
  for (lock_table = table_list; lock_table;
       lock_table = lock_table->next_local) {
    int got_lock;
...
void unlock_table_names(THD *thd, TABLE_LIST *table_list,
  ...
  for (TABLE_LIST *table = table_list; table != last_table;
...
bool lock_global_read_lock(THD *thd) {
  ...
  if (!thd->global_read_lock) {
    ...
    const char *old_message = thd->enter_cond(
        &COND_refresh, &LOCK_global_read_lock, "Waiting to get readlock");
    DBUG_PRINT("info",
               ("waiting_for: %d  protect_against: %d", waiting_for_read_lock,
                protect_against_global_read_lock));
