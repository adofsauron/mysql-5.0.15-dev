...
static uchar *pack_screens(List<create_field> &create_fields, uint *info_length, uint *screens, bool small_file);
static uint pack_keys(uchar *keybuff, uint key_count, KEY *key_info, ulong data_offset);
static bool pack_header(uchar *forminfo, enum db_type table_type, List<create_field> &create_fields, uint info_length,
                        uint screens, uint table_options, ulong data_offset, handler *file);
static uint get_interval_id(uint *int_count, List<create_field> &create_fields, create_field *last_field);
static bool pack_fields(File file, List<create_field> &create_fields, ulong data_offset);
static bool make_empty_rec(THD *thd, int file, enum db_type table_type, uint table_options,
                           List<create_field> &create_fields, uint reclength, ulong data_offset);
...
bool mysql_create_frm(THD *thd, my_string file_name, const char *db, const char *table, HA_CREATE_INFO *create_info,
                      List<create_field> &create_fields, uint keys, KEY *key_info, handler *db_file)
{
  ...
  create_info->extra_size = (2 + str_db_type.length + 2 + create_info->connect_string.length);

  if ((file = create_frm(thd, file_name, db, table, reclength, fileinfo, create_info, keys)) < 0)
  {
    my_free((gptr)screen_buff, MYF(0));
    DBUG_RETURN(1);
  }
  ...
  fileinfo[26] = (uchar)test((create_info->max_rows == 1) && (create_info->min_rows == 1) && (keys == 0));
  int2store(fileinfo + 28, key_info_length);
  strmake((char *)forminfo + 47, create_info->comment ? create_info->comment : "", 60);
  forminfo[46] = (uchar)strlen((char *)forminfo + 47);  // Length of comment

  if (my_pwrite(file, (byte *)fileinfo, 64, 0L, MYF_RW) ||
      my_pwrite(file, (byte *)keybuff, key_info_length, (ulong)uint2korr(fileinfo + 6), MYF_RW))
    goto err;
  VOID(my_seek(file, (ulong)uint2korr(fileinfo + 6) + (ulong)key_buff_length, MY_SEEK_SET, MYF(0)));
  if (make_empty_rec(thd, file, create_info->db_type, create_info->table_options, create_fields, reclength,
                     data_offset))
    goto err;

  int2store(buff, create_info->connect_string.length);
  if (my_write(file, (const byte *)buff, sizeof(buff), MYF(MY_NABP)) ||
      my_write(file, (const byte *)create_info->connect_string.str, create_info->connect_string.length, MYF(MY_NABP)))
    goto err;

  int2store(buff, str_db_type.length);
  if (my_write(file, (const byte *)buff, sizeof(buff), MYF(MY_NABP)) ||
      my_write(file, (const byte *)str_db_type.str, str_db_type.length, MYF(MY_NABP)))
    goto err;

  VOID(my_seek(file, filepos, MY_SEEK_SET, MYF(0)));
  if (my_write(file, (byte *)forminfo, 288, MYF_RW) || my_write(file, (byte *)screen_buff, info_length, MYF_RW) ||
...
static uchar *pack_screens(List<create_field> &create_fields, uint *info_length, uint *screens, bool small_file)
{
  ...
  end_row = 22;
  cols = 80;
  fields_on_screen = end_row + 1 - start_row;
...
static uint pack_keys(uchar *keybuff, uint key_count, KEY *keyinfo, ulong data_offset)
{
  ...
  for (key = keyinfo, end = keyinfo + key_count; key != end; key++)
  {
    ...
    DBUG_PRINT("loop", ("flags: %d  key_parts: %d at 0x%lx", key->flags, key->key_parts, key->key_part));
    for (key_part = key->key_part, key_part_end = key_part + key->key_parts; key_part != key_part_end; key_part++)
    {
      uint offset;
      DBUG_PRINT("loop", ("field: %d  startpos: %lu  length: %ld", key_part->fieldnr, key_part->offset + data_offset,
                          key_part->length));
                        ...
                        uint screens, uint table_options, ulong data_offset, handler *file)
{
  ...
  no_empty = int_count = int_parts = int_length = time_stamp_pos = null_fields = com_length = 0;
  n_length = 2L;
  ...
  while ((field = it++))
  {
    ...
    if (MTYP_TYPENR(field->unireg_check) == Field::NOEMPTY || field->unireg_check & MTYP_NOEMPTY_BIT)
    {
      field->unireg_check = (Field::utype)((uint)field->unireg_check | MTYP_NOEMPTY_BIT);
      no_empty++;
    }
    /*
    ...
    */
    if (field->sql_type == FIELD_TYPE_TIMESTAMP && MTYP_TYPENR(field->unireg_check) != Field::NONE && !time_stamp_pos)
  ...
  if (info_length + (ulong)create_fields.elements * FCOMP + 288 + n_length + int_length + com_length > 65535L ||
      int_count > 255)
  {
    my_message(ER_TOO_MANY_FIELDS, ER(ER_TOO_MANY_FIELDS), MYF(0));
    DBUG_RETURN(1);
  ...
  length = (info_length + create_fields.elements * FCOMP + 288 + n_length + int_length + com_length);
...
static uint get_interval_id(uint *int_count, List<create_field> &create_fields, create_field *last_field)
{
  ...
  while ((field = it++) != last_field)
  {
    ...
    {
      const char **a, **b;
      for (a = field->interval->type_names, b = interval->type_names; *a && !strcmp(*a, *b); a++, b++)
        ;
...
static bool make_empty_rec(THD *thd, File file, enum db_type table_type, uint table_options,
                           List<create_field> &create_fields, uint reclength, ulong data_offset)
{
  int error;
  Field::utype type;
  ...
  if (!handler || !(buff = (uchar *)my_malloc((uint)reclength, MYF(MY_WME | MY_ZEROFILL))))
  {
    delete handler;
    DBUG_RETURN(1);
  }
  ...
  while ((field = it++))
  {
    ...
    */
    ...
    if (field->def && (regfield->real_type() != FIELD_TYPE_YEAR || field->def->val_int() != 0))
    {
    ...
    else if (regfield->real_type() == FIELD_TYPE_ENUM && (field->flags & NOT_NULL_FLAG))
    {
