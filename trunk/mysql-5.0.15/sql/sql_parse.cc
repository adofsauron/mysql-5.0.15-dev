...
const char *command_name[] = {
  "Sleep",          "Quit",           "Init DB",     "Query",
  "Field List",     "Create DB",      "Drop DB",     "Refresh",
  "Shutdown",       "Statistics",     "Processlist", "Connect",
  "Kill",           "Debug",          "Ping",        "Time",
  "Delayed insert", "Change user",    "Binlog Dump", "Table Dump",
  "Connect Out",    "Register Slave", "Prepare",     "Execute",
  "Long Data",      "Close stmt",     "Reset stmt",  "Set option",
  "Fetch",          "Error" // Last command number
};
...
static bool end_active_trans(THD *thd) {
  ...
  if (thd->options &
      (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN | OPTION_TABLE_LOCK)) {
...
int check_user(THD *thd, enum enum_server_command command, const char *passwd,
               uint passwd_len, const char *db, bool check_count) {
  ...
  */
  ...
  if (res == 0) {
                                                             ...
                                                             // is OK
    {
      ...
      if (check_count) {
        ...
        bool count_ok =
            thread_count <= max_connections + delayed_insert_threads ||
            (thd->main_security_ctx.master_access & SUPER_ACL);
        VOID(pthread_mutex_unlock(&LOCK_thread_count));
        if (!count_ok) { // too many connections
      ...
      mysql_log.write(thd, command, (thd->main_security_ctx.priv_user ==
                                             thd->main_security_ctx.user
                                         ? (char *)"%s@%s on %s"
                                         : (char *)"%s@%s as anonymous on %s"),
                      thd->main_security_ctx.user,
                      thd->main_security_ctx.host_or_ip, db ? db : (char *)"");
  ...
  net_printf_error(thd, ER_ACCESS_DENIED_ERROR, thd->main_security_ctx.user,
                   thd->main_security_ctx.host_or_ip,
                   passwd_len ? ER(ER_YES) : ER(ER_NO));
  mysql_log.write(
      thd, COM_CONNECT, ER(ER_ACCESS_DENIED_ERROR), thd->main_security_ctx.user,
...
void init_max_user_conn(void) {
  (void)hash_init(&hash_user_connections, system_charset_info, max_connections,
                  0, 0, (hash_get_key)get_key_conn, (hash_free_key)free_user,
                  0);
}
...
static void reset_mqh(LEX_USER *lu, bool get_them = 0) {
  ...
  if (lu) // for GRANT
  {
    ...
    for (uint idx = 0; idx < hash_user_connections.records; idx++) {
      USER_CONN *uc =
          (struct user_conn *)hash_element(&hash_user_connections, idx);
      if (get_them)
    ...
    if (!(specialflag & SPECIAL_NO_RESOLVE)) {
      ...
      if (thd->main_security_ctx.host) {
        if (thd->main_security_ctx.host != my_localhost)
          thd->main_security_ctx.host
              [min(strlen(thd->main_security_ctx.host), HOSTNAME_LENGTH)] = 0;
        thd->main_security_ctx.host_or_ip = thd->main_security_ctx.host;
      }
      if (connect_errors > max_connect_errors)
        return (ER_HOST_IS_BLOCKED);
    }
    DBUG_PRINT("info", ("Host: %s  ip: %s", (thd->main_security_ctx.host
                                                 ? thd->main_security_ctx.host
                                                 : "unknown host"),
...
#include "_cust_sql_parse.h"
  ...
  if (thd->client_capabilities & CLIENT_SSL) {
    ...
    if ((pkt_len = my_net_read(net)) == packet_error ||
      ...
      DBUG_PRINT("error",
                 ("Failed to read user information (pkt_len= %lu)", pkt_len));
      inc_host_errors(&thd->remote.sin_addr);
      return (ER_HANDSHAKE_ERROR);
    }
  }
  ...
  */
  uint passwd_len = thd->client_capabilities & CLIENT_SECURE_CONNECTION
                        ? *passwd++
                        : strlen(passwd);
  db = thd->client_capabilities & CLIENT_CONNECT_WITH_DB ? db + passwd_len + 1
                                                         : 0;

  /* Since 4.1 all database names are stored in utf8 */
  if (db) {
    db_buff[copy_and_convert(db_buff, sizeof(db_buff) - 1, system_charset_info,
                             db, strlen(db), thd->charset(), &dummy_errors)] =
        0;
    db = db_buff;
  }

  user_buff
      [copy_and_convert(user_buff, sizeof(user_buff) - 1, system_charset_info,
                        user, strlen(user), thd->charset(), &dummy_errors)] =
          '\0';
  user = user_buff;
...
/* now that we've called my_thread_init(), it is safe to call DBUG_* */
  ...
  do {
    ...
    if ((error = check_connection(thd))) { // Wrong permissions
      if (error > 0)
    ...
    if (net->error && net->vio != 0 && net->report_error) {
      if (!thd->killed && thd->variables.log_warnings > 1)
        sql_print_warning(
            ER(ER_NEW_ABORTING_CONNECTION), thd->thread_id,
...
pthread_handler_t handle_bootstrap(void *arg) {
  ...
  thd->security_ctx->priv_user = thd->security_ctx->user =
      (char *)my_strdup("boot", MYF(MY_WME));
  ...
  */
  ...
  if ((packet_length = my_net_read(net)) == packet_error) {
    DBUG_PRINT("info", ("Got error %d reading command from socket %s",
                        net->error, vio_description(net->vio)));
    ...
    DBUG_PRINT("info", ("Command on %s = %d (%s)", vio_description(net->vio),
  ...
  */
  ...
  case COM_INIT_DB: {
    ...
    statistic_increment(thd->status_var.com_stat[SQLCOM_CHANGE_DB],
                        &LOCK_status);
    thd->convert_string(&tmp, system_charset_info, packet, strlen(packet),
                        thd->charset());
...
*/
...
#else
  {
    ...
    pend = strend(packet);
    thd->convert_string(&conv_name, system_charset_info, packet,
                        (uint)(pend - packet), thd->charset());
    ...
    thd->lex->select_lex.table_list.link_in_list(
        (byte *)&table_list, (byte **)&table_list.next_local);
  ...
  case COM_STATISTICS: {
    ...
    sprintf((char *)buff, "Uptime: %lu  Threads: %d  Questions: %lu  Slow "
                          "queries: %lu  Opens: %lu  Flush tables: %lu  Open "
                          "tables: %u  Queries per second avg: %.3f",
            uptime, (int)thread_count, (ulong)thd->query_id,
  ...
  case COM_PROCESS_INFO:
    ...
    if (!thd->security_ctx->priv_user[0] &&
      ...
      break;
    mysql_log.write(thd, command, NullS);
    mysqld_list_processes(thd, thd->security_ctx->master_access & PROCESS_ACL
                                   ? NullS
...
bool alloc_query(THD *thd, const char *packet, uint packet_length) {
  ...
  if (!(thd->query = (char *)thd->memdup_w_gap((gptr)(packet), packet_length,
                                               thd->db_length + 1 +
...
bool mysql_execute_command(THD *thd) {
  ...
  */
  ...
  select_lex->context.resolve_in_table_list_only(
  ...
  */
  ...
  case SQLCOM_SELECT: {
    ...
    if (all_tables) {
      if (lex->orig_sql_command != SQLCOM_SHOW_STATUS_PROC &&
        ...
        res = check_table_access(thd, lex->exchange ? SELECT_ACL | FILE_ACL
                                                    : SELECT_ACL,
                                 all_tables, 0);
    } else
      res =
          check_access(thd, lex->exchange ? SELECT_ACL | FILE_ACL : SELECT_ACL,
                       any_db, 0, 0, 0, 0);
    if (res)
      goto error;
  ...
  case SQLCOM_SHOW_WARNS: {
    res = mysqld_show_warnings(
        thd, (ulong)((1L << (uint)MYSQL_ERROR::WARN_LEVEL_NOTE) |
  ...
  case SQLCOM_SHOW_ERRORS: {
    res = mysqld_show_warnings(
        thd, (ulong)(1L << (uint)MYSQL_ERROR::WARN_LEVEL_ERROR));
    break;
  }
...
/* This query don't work now. See comment in repl_failsafe.cc */
    ...
    if (select_lex->item_list.elements) // With select
    {
      ...
      if (!(res = open_and_lock_tables(thd, select_tables))) {
        ...
        */
        ...
        if (lex->create_info.used_fields & HA_CREATE_USED_UNION) {
          TABLE_LIST *tab;
          for (tab = (TABLE_LIST *)lex->create_info.merge_list.first; tab;
               tab = tab->next_local) {
            TABLE_LIST *duplicate;
        ...
        if ((result = new select_create(create_table, &lex->create_info,
                                        lex->create_list, lex->key_list,
      ...
      if (lex->name)
        ...
        res = mysql_create_table(thd, create_table->db,
                                 ...
                                 lex->create_list, lex->key_list, 0, 0);
      }
      if (!res)
        send_ok(thd);
    }
  ...
  case SQLCOM_ALTER_TABLE:
...
#else
    {
      ...
      if (check_access(thd, ALTER_ACL, first_table->db,
                       &first_table->grant.privilege, 0, 0,
                       test(first_table->schema_table)) ||
          check_access(thd, INSERT_ACL | CREATE_ACL, select_lex->db, &priv, 0,
                       0, is_schema_db(select_lex->db)) ||
      ...
      if (grant_option) {
        if (check_grant(thd, ALTER_ACL, all_tables, 0, UINT_MAX, 0))
          goto error;
        if (lex->name &&
            !test_all_bits(priv, INSERT_ACL | CREATE_ACL)) { // Rename of table
          TABLE_LIST tmp_table;
          bzero((char *)&tmp_table, sizeof(tmp_table));
          tmp_table.table_name = lex->name;
          tmp_table.db = select_lex->db;
          tmp_table.grant.privilege = priv;
          if (check_grant(thd, INSERT_ACL | CREATE_ACL, &tmp_table, 0, UINT_MAX,
                          0))
            goto error;
        }
      }
      ...
      if (end_active_trans(thd))
        ...
        thd->enable_slow_log = opt_log_slow_admin_statements;
        res = mysql_alter_table(
            thd, select_lex->db, lex->name, &lex->create_info, first_table,
            lex->create_list, lex->key_list, select_lex->order_list.elements,
            (ORDER *)select_lex->order_list.first, lex->duplicates, lex->ignore,
            &lex->alter_info, 1);
      }
      break;
    }
  ...
  case SQLCOM_SHOW_CREATE:
...
#else
    {
      ...
      if (check_db_used(thd, all_tables) ||
        ...
        goto error;
      if (grant_option &&
          check_grant(thd, SELECT_ACL, all_tables, 2, UINT_MAX, 0))
        goto error;
  ...
  case SQLCOM_UPDATE:
    ...
    unit->set_limit(select_lex);
    res = (result = mysql_update(
               thd, all_tables, select_lex->item_list, lex->value_list,
               select_lex->where, select_lex->order_list.elements,
               (ORDER *)select_lex->order_list.first, unit->select_limit_cnt,
  ...
  case SQLCOM_UPDATE_MULTI: {
    ...
    res = mysql_multi_update(thd, all_tables, &select_lex->item_list,
                             &lex->value_list, select_lex->where,
                             select_lex->options, lex->duplicates, lex->ignore,
                             unit, select_lex);
    break;
  }
  case SQLCOM_REPLACE:
  ...
  case SQLCOM_INSERT_SELECT: {
    ...
    if (!(res = open_and_lock_tables(thd, all_tables))) {
      ...
      if (!res && (result = new select_insert(
                       first_table, first_table->table, &lex->field_list,
                       &lex->update_list, &lex->value_list, lex->duplicates,
                       lex->ignore))) {
        /*
          Skip first table, which is the table we are inserting in.
  ...
  case SQLCOM_DELETE: {
    ...
    res = mysql_delete(thd, all_tables, select_lex->where,
                       &select_lex->order_list, unit->select_limit_cnt,
  ...
  case SQLCOM_DELETE_MULTI: {
    ...
    if (!thd->is_fatal_error &&
      ...
      res = mysql_select(
          thd, &select_lex->ref_pointer_array, select_lex->get_table_list(),
          select_lex->with_wild, select_lex->item_list, select_lex->where, 0,
          (ORDER *)NULL, (ORDER *)NULL, (Item *)NULL, (ORDER *)NULL,
          select_lex->options | thd->options | SELECT_NO_JOIN_CACHE |
  ...
  case SQLCOM_LOAD: {
    DBUG_ASSERT(first_table == all_tables && first_table != 0);
    uint privilege = (lex->duplicates == DUP_REPLACE ? INSERT_ACL | DELETE_ACL
                                                     : INSERT_ACL) |
...
*/
    ...
    if (check_access(thd, CREATE_ACL, lex->name, 0, 1, 0,
                     is_schema_db(lex->name)))
      break;
    res =
        mysql_create_db(thd, (lower_case_table_names == 2 ? alias : lex->name),
                        &lex->create_info, 0);
    break;
  }
  case SQLCOM_DROP_DB: {
...
*/
      ...
      while ((user = user_list++)) {
        ...
        if (strcmp(thd->security_ctx->user, user->user.str) ||
          ...
          if (check_acl_user(user, &counter) && user->password.str &&
            ...
            my_message(ER_PASSWORD_NOT_ALLOWED, ER(ER_PASSWORD_NOT_ALLOWED),
                       MYF(0));
            goto error;
          }
        }
    ...
    if (first_table) {
      if (lex->type == TYPE_ENUM_PROCEDURE || lex->type == TYPE_ENUM_FUNCTION) {
        ...
        res = mysql_routine_grant(
            thd, all_tables, lex->type == TYPE_ENUM_PROCEDURE, lex->users_list,
            grants, lex->sql_command == SQLCOM_REVOKE, 0);
      } else {
        if (grant_option &&
            check_grant(thd, (lex->grant | lex->grant_tot_col | GRANT_ACL),
                        all_tables, 0, UINT_MAX, 0))
          goto error;
        res = mysql_table_grant(thd, all_tables, lex->users_list, lex->columns,
  ...
  case SQLCOM_BEGIN:
    ...
    break;
  case SQLCOM_COMMIT:
    if (end_trans(thd, lex->tx_release ? COMMIT_RELEASE : lex->tx_chain
                                                              ? COMMIT_AND_CHAIN
                                                              : COMMIT))
      goto error;
    send_ok(thd);
  ...
  case SQLCOM_ROLLBACK:
    if (end_trans(thd, lex->tx_release
                           ? ROLLBACK_RELEASE
  ...
  case SQLCOM_RELEASE_SAVEPOINT: {
    ...
    for (sv = thd->transaction.savepoints; sv; sv = sv->prev) {
      if (my_strnncoll(system_charset_info, (uchar *)lex->ident.str,
                       lex->ident.length, (uchar *)sv->name, sv->length) == 0)
        break;
    }
    if (sv) {
  ...
  case SQLCOM_SAVEPOINT:
    if (!(thd->options & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) ||
      ...
      for (sv = &thd->transaction.savepoints; *sv; sv = &(*sv)->prev) {
        if (my_strnncoll(system_charset_info, (uchar *)lex->ident.str,
                         lex->ident.length, (uchar *)(*sv)->name,
                         (*sv)->length) == 0)
          break;
      }
      ...
      newsv->name = strmake_root(&thd->transaction.mem_root, lex->ident.str,
                                 lex->ident.length);
  ...
  case SQLCOM_CREATE_SPFUNCTION: {
    ...
    if (!lex->sphead->m_db.str || !lex->sphead->m_db.str[0]) {
      ...
      lex->sphead->m_db.str =
          strmake_root(thd->mem_root, thd->db, lex->sphead->m_db.length);
    }
    ...
    if (result == SP_OK) {
      ...
      if (sp_automatic_privileges && !opt_noacl &&
          check_routine_access(thd, DEFAULT_CREATE_PROC_ACLS, db, name,
    ...
    */
    if (!(sp = sp_find_procedure(thd, lex->spname, TRUE))) {
      ...
      */
...
#ifndef NO_EMBEDDED_ACCESS_CHECKS
      if (check_routine_access(thd, EXECUTE_ACL, sp->m_db.str, sp->m_name.str,
                               TRUE, 0) ||
      ...
      if (save_ctx && check_routine_access(thd, EXECUTE_ACL, sp->m_db.str,
                                           sp->m_name.str, TRUE, 0)) {
#ifndef EMBEDDED_LIBRARY
        thd->net.no_send_ok = nsok;
#endif
      ...
      */
      ...
      if (!res)
        send_ok(thd,
                (ulong)(thd->row_count_func < 0 ? 0 : thd->row_count_func));
      else
  ...
  case SQLCOM_DROP_FUNCTION: {
    ...
    if (sp) {
      ...
      if (sp_automatic_privileges && !opt_noacl &&
        ...
        push_warning(thd, MYSQL_ERROR::WARN_LEVEL_WARN,
                     ER_PROC_AUTO_REVOKE_FAIL, ER(ER_PROC_AUTO_REVOKE_FAIL));
      }
#endif
      if (lex->sql_command == SQLCOM_DROP_PROCEDURE)
  ...
  case SQLCOM_SHOW_CREATE_PROC: {
    ...
    if (sp_show_create_procedure(thd, lex->spname) !=
        SP_OK) { /* We don't distinguish between errors for now */
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), SP_COM_STRING(lex),
  ...
  case SQLCOM_SHOW_CREATE_FUNC: {
    ...
    if (sp_show_create_function(thd, lex->spname) !=
        SP_OK) { /* We don't distinguish between errors for now */
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), SP_COM_STRING(lex),
               lex->spname->m_name.str);
      goto error;
  ...
  case SQLCOM_XA_START:
    ...
    if (thd->lex->xa_opt != XA_NONE) { // JOIN is not supported yet. TODO
  ...
  case SQLCOM_XA_END:
    /* fake it */
    if (thd->lex->xa_opt !=
        XA_NONE) { // SUSPEND and FOR MIGRATE are not supported yet. TODO
      my_error(ER_XAER_INVAL, MYF(0));
      break;
    }
...
bool check_one_table_access(THD *thd, ulong privilege, TABLE_LIST *all_tables) {
  if (check_access(thd, privilege, all_tables->db, &all_tables->grant.privilege,
                   0, 0, test(all_tables->schema_table)))
    return 1;
  ...
  if ((sctx->master_access & want_access) == want_access) {
    ...
    */
    ...
    if (!(sctx->master_access & SELECT_ACL) &&
      ...
      db_access =
          acl_get(sctx->host, sctx->ip, sctx->priv_user, db, db_is_pattern);
...
bool check_table_access(THD *thd, ulong want_access, TABLE_LIST *tables,
  ...
  for (; tables; tables = tables->next_global) {
    if (tables->schema_table &&
      ...
      if (!no_errors)
        my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), thd->security_ctx->priv_user,
                 thd->security_ctx->priv_host, information_schema_name.str);
      return TRUE;
    }
    ...
    } else if (check_access(thd, want_access, tables->db,
...
bool check_routine_access(THD *thd, ulong want_access, char *db, char *name,
  ...
  */
  if ((thd->security_ctx->master_access & want_access) == want_access)
    tables->grant.privilege = want_access;
  else if (check_access(thd, want_access, db, &tables->grant.privilege, 0,
                        no_errors, 0))
    return TRUE;
...
bool check_some_access(THD *thd, ulong want_access, TABLE_LIST *table) {
  ...
  for (access = 1; access < want_access; access <<= 1) {
    if (access & want_access) {
      if (!check_access(thd, access, table->db, &table->grant.privilege, 0, 1,
                        test(table->schema_table)) &&
              !grant_option ||
...
bool check_stack_overrun(THD *thd, long margin,
  ...
  if ((stack_used = used_stack(thd->thread_stack, (char *)&stack_used)) >=
    ...
    sprintf(errbuff[0], ER(ER_STACK_OVERRUN_NEED_MORE), stack_used,
            thread_stack, margin);
...
bool my_yyoverflow(short **yyss, YYSTYPE **yyvs, ulong *yystacksize) {
  ...
  *yystacksize = set_zone((*yystacksize) * 2, MY_YACC_INIT, MY_YACC_MAX);
  if (!(lex->yacc_yyvs = (char *)my_realloc(
            (gptr)lex->yacc_yyvs, *yystacksize * sizeof(**yyvs),
            MYF(MY_ALLOW_ZERO_PTR | MY_FREE_ON_ERROR))) ||
      !(lex->yacc_yyss = (char *)my_realloc(
            (gptr)lex->yacc_yyss, *yystacksize * sizeof(**yyss),
            MYF(MY_ALLOW_ZERO_PTR | MY_FREE_ON_ERROR))))
    return 1;
  if (old_info) { // Copy old info from stack
...
void mysql_reset_thd_for_next_command(THD *thd) {
  ...
  thd->server_status &=
      ~(SERVER_MORE_RESULTS_EXISTS | SERVER_QUERY_NO_INDEX_USED |
...
bool add_field_to_list(THD *thd, char *field_name, enum_field_types type,
                       char *length, char *decimals, uint type_modifier,
  ...
  if (type_modifier & PRI_KEY_FLAG) {
    ...
    lex->key_list.push_back(
        new Key(Key::PRIMARY, NullS, HA_KEY_ALG_UNDEF, 0, lex->col_list));
    lex->col_list.empty();
  }
  if (type_modifier & (UNIQUE_FLAG | UNIQUE_KEY_FLAG)) {
    lex->col_list.push_back(new key_part_spec(field_name, 0));
    lex->key_list.push_back(
        new Key(Key::UNIQUE, NullS, HA_KEY_ALG_UNDEF, 0, lex->col_list));
    lex->col_list.empty();
  }
  ...
  if (type == FIELD_TYPE_TIMESTAMP && length) {
    ...
    */
    ...
    push_warning_printf(thd, MYSQL_ERROR::WARN_LEVEL_WARN,
                        ...
                        ER(ER_WARN_DEPRECATED_SYNTAX), buf, "TIMESTAMP");
  }

  if (!(new_field = new_create_field(thd, field_name, type, length, decimals,
                                     type_modifier, default_value,
                                     on_update_value, comment, change,
...
create_field *new_create_field(THD *thd, char *field_name,
                               enum_field_types type, char *length,
                               char *decimals, uint type_modifier,
                               Item *default_value, Item *on_update_value,
                               LEX_STRING *comment, char *change,
                               List<String> *interval_list, CHARSET_INFO *cs,
                               uint uint_geom_type) {
  ...
  new_field->unireg_check =
      (type_modifier & AUTO_INCREMENT_FLAG ? Field::NEXT_NUMBER : Field::NONE);
  ...
  */
  ...
  case FIELD_TYPE_NEWDECIMAL:
    ...
    new_field->length = my_decimal_precision_to_length(
        new_field->length, new_field->decimals, type_modifier & UNSIGNED_FLAG);
  ...
  case FIELD_TYPE_TIMESTAMP:
    if (!length)
      ...
      */
      new_field->length =
          ((new_field->length + 1) / 2) * 2;          /* purecov: inspected */
    ...
    if (default_value) {
      ...
      if (default_value->type() == Item::FUNC_ITEM &&
        ...
        */
        ...
        new_field->unireg_check =
            (on_update_value ? Field::TIMESTAMP_UN_FIELD : Field::NONE);
    } else {
      /*
      ...
      */
      new_field->unireg_check =
          (on_update_value
               ...
               : (new_field->flags & NOT_NULL_FLAG ? Field::TIMESTAMP_OLD_FIELD
                                                   : Field::NONE));
    }
    break;
  ...
  if (!(new_field->flags & BLOB_FLAG) &&
      ((new_field->length > max_field_charlength && type != FIELD_TYPE_SET &&
       ...
       (!new_field->length && type != MYSQL_TYPE_STRING &&
...
TABLE_LIST *st_select_lex::add_table_to_list(
    THD *thd, Table_ident *table, LEX_STRING *alias, ulong table_options,
  ...
  {
    if (table->sel) {
      my_message(ER_DERIVED_MUST_HAVE_ALIAS, ER(ER_DERIVED_MUST_HAVE_ALIAS),
                 MYF(0));
      DBUG_RETURN(0);
    }
  ...
  if (table_list.elements > 0) {
    ...
    */
    previous_table_ref =
        (TABLE_LIST *)(table_list.next - offsetof(TABLE_LIST, next_local));
...
void st_select_lex::set_lock_for_tables(thr_lock_type lock_type) {
  ...
  for (TABLE_LIST *tables = (TABLE_LIST *)table_list.first; tables;
       tables = tables->next_local) {
    ...
    if ((options & REFRESH_READ_LOCK) && thd) {
      ...
      */
      ...
      if (lock_global_read_lock(thd))
        return 1; // Killed
      result =
          close_cached_tables(thd, (options & REFRESH_FAST) ? 0 : 1, tables);
...
bool mysql_create_index(THD *thd, TABLE_LIST *table_list, List<Key> &keys) {
  ...
  DBUG_RETURN(mysql_alter_table(thd, table_list->db, table_list->table_name,
                                &create_info, table_list, fields, keys, 0,
  ...
  for (table = tables; table; table = table->next_local) {
    if (table->derived)
      table->grant.privilege = SELECT_ACL;
                           ...
                           0, 1, test(table->schema_table)) ||
              grant_option && check_grant(thd, UPDATE_ACL, table, 0, 1, 1)) &&
             (check_access(thd, SELECT_ACL, table->db, &table->grant.privilege,
  ...
  if (&lex->select_lex != lex->all_selects_list || lex->time_zone_tables_used) {
    ...
    for (table = tables; table; table = table->next_global) {
      if (!my_tz_check_n_skip_implicit_tables(&table,
        ...
        if (check_access(thd, SELECT_ACL, table->db, &table->grant.privilege, 0,
                         0, test(table->schema_table)) ||
            grant_option && check_grant(thd, SELECT_ACL, table, 0, 1, 0))
          DBUG_RETURN(TRUE);
      }
    }
...
bool multi_delete_set_locks_and_link_aux_tables(LEX *lex) {
  ...
  for (target_tbl = (TABLE_LIST *)lex->auxilliary_table_list.first; target_tbl;
       target_tbl = target_tbl->next_local) {
    lex->table_count++;
    /* All tables in aux_tables must be found in FROM PART */
    TABLE_LIST *walk;
    for (walk = tables; walk; walk = walk->next_local) {
      if (!my_strcasecmp(table_alias_charset, target_tbl->alias, walk->alias) &&
          !strcmp(walk->db, target_tbl->db))
        break;
    }
    if (!walk) {
      my_error(ER_UNKNOWN_TABLE, MYF(0), target_tbl->table_name,
               "MULTI DELETE");
      DBUG_RETURN(TRUE);
    }
    if (!walk->derived) {
...
bool insert_precheck(THD *thd, TABLE_LIST *tables) {
  ...
  */
  ulong privilege =
      (INSERT_ACL | (lex->duplicates == DUP_REPLACE ? DELETE_ACL : 0) |
...
bool create_table_precheck(THD *thd, TABLE_LIST *tables,
  ...
  want_priv =
      ((lex->create_info.options & HA_LEX_CREATE_TMP_TABLE) ? CREATE_TMP_ACL
                                                            : CREATE_ACL);
