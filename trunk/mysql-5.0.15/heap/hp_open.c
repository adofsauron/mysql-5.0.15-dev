...
HP_INFO *heap_open(const char *name, int mode)
{
  ...
  if (!(info = (HP_INFO *)my_malloc((uint)sizeof(HP_INFO) + 2 * share->max_key_length, MYF(MY_ZEROFILL))))
  {
    pthread_mutex_unlock(&THR_LOCK_heap);
    DBUG_RETURN(0);
  }
...
#endif
  DBUG_PRINT("exit", ("heap: 0x%lx  reclength: %d  records_in_block: %d", info, share->reclength,
