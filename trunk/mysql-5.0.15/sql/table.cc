...
int openfrm(THD *thd, const char *name, const char *alias, uint db_stat,
  ...
  if ((file =
           my_open(fn_format(index_file, name, "", reg_ext, MY_UNPACK_FILENAME),
  ...
  *root_ptr = &outparam->mem_root;
  ...
  share->table_name =
      strdup_root(&outparam->mem_root, name + dirname_length(name));
  ...
  */
  ...
  if ((n_length = uint2korr(head + 55))) {
    ...
    if (!(share->connect_string.str =
              strmake_root(&outparam->mem_root, next_chunk + 2,
  ...
  rec_buff_length =
      ALIGN_SIZE(share->reclength + 1 + outparam->file->extra_rec_buf_length());
  share->rec_buff_length = rec_buff_length;
  if (!(record =
            (char *)alloc_root(&outparam->mem_root, rec_buff_length * records)))
    goto err; /* purecov: inspected */
  share->default_values = (byte *)record;

  if (my_pread(file, (byte *)record, (uint)share->reclength, record_offset,
               MYF(MY_NABP)))
    goto err; /* purecov: inspected */
  ...
  */
  ...
  read_length = (uint)(share->fields * field_pack_length + pos +
                       (uint)(n_length + int_length + com_length));
...

  {
    ...
    for (interval = share->intervals;
      ...
      if (!(interval->type_lengths =
                (uint *)alloc_root(&outparam->mem_root, count)))
        goto err;
  ...
  if (use_hash)
    use_hash = !hash_init(&share->name_hash, system_charset_info, share->fields,
                          0, 0, (hash_get_key)get_field_name, 0, 0);

  for (i = 0; i < share->fields;
       i++, strpos += field_pack_length, field_ptr++) {
    ...
    if (field_type == FIELD_TYPE_NEWDECIMAL && !share->mysql_version) {
      ...
      */
      ...
      sql_print_error("Found incompatible DECIMAL field '%s' in %s; Please do "
    ...
    *field_ptr = reg_field = make_field(
        record + recpos, (uint32)field_length, null_pos, null_bit_pos,
        pack_flag, field_type, charset, geom_type,
  ...
  *field_ptr = 0; // End marker
  ...
  if (key_parts) {
    uint primary_key =
        (uint)(find_type((char *)primary_key_name, &share->keynames, 3) - 1);
    ...
    for (uint key = 0; key < share->keys; key++, keyinfo++) {
      ...
      for (i = 0; i < keyinfo->key_parts; key_part++, i++) {
        if (new_field_pack_flag <= 1)
          key_part->fieldnr = (uint16)find_field(
              outparam, (uint)key_part->offset, (uint)key_part->length);
#ifdef EXTRA_DEBUG
        if (key_part->fieldnr > share->fields)
          goto err; // sanity check
#endif
        if (key_part->fieldnr) { // Should always be true !
          Field *field = key_part->field =
              outparam->field[key_part->fieldnr - 1];
          if (field->null_ptr) {
            key_part->null_offset =
                (uint)((byte *)field->null_ptr - outparam->record[0]);
          ...
          if (i == 0 && key != primary_key)
            field->flags |=
                ((keyinfo->flags & HA_NOSAME) && (keyinfo->key_parts == 1))
                    ? UNIQUE_KEY_FLAG
                    : MULTIPLE_KEY_FLAG;
          if (i == 0)
          ...
          if (field->key_length() != key_part->length) {
            ...
            if (field->type() == FIELD_TYPE_NEWDECIMAL) {
              ...
              */
              ...
              sql_print_error("Found wrong key definition in %s; Please do "
                              "\"ALTER TABLE '%s' FORCE \" to fix it!",
            ...
            if (!(field->flags & BLOB_FLAG)) { // Create a new field
              field = key_part->field =
                  field->new_field(&outparam->mem_root, outparam);
      ...
      set_if_bigger(share->max_key_length,
                    keyinfo->key_length + keyinfo->key_parts);
  ...
  if ((reg_field = outparam->found_next_number_field)) {
    if ((int)(share->next_number_index = (uint)find_ref_key(
                  outparam, reg_field, &share->next_number_key_offset)) < 0) {
  ...
  if (share->blob_fields) {
    ...
    if (!(share->blob_field = save = (uint *)alloc_root(
              &outparam->mem_root, (uint)(share->blob_fields * sizeof(uint)))))
      goto err;
  ...
  */
  share->null_bytes =
      (null_pos - (uchar *)outparam->null_flags + (null_bit_pos + 7) / 8);
...
ulong make_new_entry(File file, uchar *fileinfo, TYPELIB *formnames,
  ...
  if (64 + length + n_length + (names + 1) * 4 > maxlength) { /* Expand file */
    newpos += IO_SIZE;
    int4store(fileinfo + 10, newpos);
    endpos =
        (ulong)my_seek(file, 0L, MY_SEEK_END, MYF(0)); /* Copy from file-end */
...
static void frm_error(int error, TABLE *form, const char *name, myf errortype,
  ...
  case 2: {
    ...
    err_no = (my_errno == ENOENT) ? ER_FILE_NOT_FOUND : (my_errno == EAGAIN)
                                                            ? ER_FILE_USED
                                                            : ER_CANT_OPEN_FILE;
    my_error(err_no, errortype, fn_format(buff, real_name, form_dev, datext, 2),
             my_errno);
    break;
  }
  case 5: {
...
File create_frm(THD *thd, my_string name, const char *db, const char *table,
                uint reclength, uchar *fileinfo, HA_CREATE_INFO *create_info,
                uint keys) {
  register File file;
  ulong length;
  char fill[IO_SIZE];
  ...
  */
  ...
  if ((file = my_create(name, CREATE_MODE, create_flags, MYF(0))) >= 0) {
    ...
    length = next_io_size(
        (ulong)(IO_SIZE + key_length + reclength + create_info->extra_size));
    ...
    create_info->table_options |=
        HA_OPTION_LONG_BLOB_PTR; // Use portable blob pointers
...
bool st_table_list::prep_check_option(THD *thd, uint8 check_opt_type) {
  ...
  for (TABLE_LIST *tbl = ancestor; tbl; tbl = tbl->next_local) {
    ...
    if (tbl->view &&
        tbl->prep_check_option(thd, ((check_opt_type == VIEW_CHECK_CASCADED)
                                         ? VIEW_CHECK_CASCADED
                                         : VIEW_CHECK_NONE))) {
      DBUG_RETURN(TRUE);
    }
  ...
  if (check_option) {
    ...
    if (!check_option->fixed && check_option->fix_fields(thd, &check_option) ||
...
int st_table_list::view_check_option(THD *thd, bool ignore_failure) {
  if (check_option && check_option->val_int() == 0) {
    ...
    if (ignore_failure) {
      ...
      my_error(ER_VIEW_CHECK_FAILED, MYF(0), view->view_db.str,
               view->view_name.str);
...
bool st_table_list::set_insert_values(MEM_ROOT *mem_root) {
  if (table) {
    if (!table->insert_values && !(table->insert_values = (byte *)alloc_root(
...
bool Natural_join_column::check_grants(THD *thd, const char *name,
                                       uint length) {
  GRANT_INFO *grant;
  const char *db_name;
...
void Field_iterator_natural_join::set(TABLE_LIST *table_ref) {
  ...
  */
  column_ref_it =
      new List_iterator_fast<Natural_join_column>(*(table_ref->join_columns));
...
void Field_iterator_table_ref::set_field_iterator() {
  ...
  */
  if (table_ref->is_join_columns_complete) {
    ...
    DBUG_ASSERT(table_ref->is_natural_join || table_ref->nested_join ||
