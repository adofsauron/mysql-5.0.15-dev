...
#ifdef THREAD
#define lock_append_buffer(info) pthread_mutex_lock(&(info)->append_buffer_lock)
...
int init_io_cache(IO_CACHE *info, File file, uint cachesize, enum cache_type type, my_off_t seek_offset,
                  pbool use_async_io, myf cache_myflags)
{
  uint min_cache;
  my_off_t end_of_file = ~(my_off_t)0;
  DBUG_ENTER("init_io_cache");
  DBUG_PRINT("enter", ("cache: 0x%lx  type: %d  pos: %ld", (ulong)info, (int)type, (ulong)seek_offset));
  ...
  if (type != READ_NET && type != WRITE_NET)
  {
    ...
    {
      uint buffer_block;
      cachesize = (uint)((ulong)(cachesize + min_cache - 1) & (ulong) ~(min_cache - 1));
      if (cachesize < min_cache)
        cachesize = min_cache;
      buffer_block = cachesize;
      if (type == SEQ_READ_APPEND)
        buffer_block *= 2;
      if ((info->buffer = (byte *)my_malloc(
               buffer_block, MYF((cache_myflags & ~MY_WME) | (cachesize == min_cache ? MY_WME : 0)))) != 0)
      {
...
my_bool reinit_io_cache(IO_CACHE *info, enum cache_type type, my_off_t seek_offset,
                        pbool use_async_io __attribute__((unused)), pbool clear_cache)
{
  DBUG_ENTER("reinit_io_cache");
  DBUG_PRINT("enter", ("cache: 0x%lx type: %d  seek_offset: %lu  clear_cache: %d", (ulong)info, type,
                       (ulong)seek_offset, (int)clear_cache));

  /* One can't do reinit with the following types */
  DBUG_ASSERT(type != READ_NET && info->type != READ_NET && type != WRITE_NET && info->type != WRITE_NET &&
              type != SEQ_READ_APPEND && info->type != SEQ_READ_APPEND);

  /* If the whole file is in memory, avoid flushing to disk */
  if (!clear_cache && seek_offset >= info->pos_in_file && seek_offset <= my_b_tell(info))
  {
...
#ifdef HAVE_AIOWAIT
  if (use_async_io && !my_disable_async_io && ((ulong)info->buffer_length < (ulong)(info->end_of_file - seek_offset)))
  {
...
int _my_b_read(register IO_CACHE *info, byte *Buffer, uint Count)
{
  ...
  { /* Fill first intern buffer */
    ...
    if ((read_length = my_read(info->file, Buffer, (uint)length, info->myflags)) != (uint)length)
    {
      info->error = (read_length == (uint)-1 ? -1 : (int)(read_length + left_length));
      DBUG_RETURN(1);
    }
    Count -= length;
    Buffer += length;
  ...
  else if ((length = my_read(info->file, info->buffer, (uint)max_length, info->myflags)) < Count || length == (uint)-1)
  {
    if (length != (uint)-1)
...
static int lock_io_cache(IO_CACHE *info, my_off_t pos)
{
  ...
  total = s->total;
  s->count--;
  while (!s->active || s->active->pos_in_file < pos) pthread_cond_wait(&s->cond, &s->mutex);

  if (s->total < total && (!s->active || s->active->pos_in_file < pos))
    return 1;
...
int _my_b_read_r(register IO_CACHE *info, byte *Buffer, uint Count)
{
  ...
  while (Count)
  {
    ...
    length = (length <= info->read_length) ? length + IO_ROUND_DN(info->read_length - length)
                                           : length - IO_ROUND_UP(length - info->read_length);
    if (info->type != READ_FIFO && (length > (uint)(info->end_of_file - pos_in_file)))
...
int _my_b_seq_read(register IO_CACHE *info, byte *Buffer, uint Count)
{
  ...
  /* pos_in_file always point on where info->buffer was read */
  if ((pos_in_file = info->pos_in_file + (uint)(info->read_end - info->buffer)) >= info->end_of_file)
  ...
  */
  ...
  { /* Fill first intern buffer */
    ...
    length = (Count & (uint) ~(IO_SIZE - 1)) - diff_length;
    if ((read_length = my_read(info->file, Buffer, (uint)length, info->myflags)) == (uint)-1)
    {
      info->error = -1;
...
int _my_b_async_read(register IO_CACHE *info, byte *Buffer, uint Count)
{
  ...
  memcpy(Buffer, info->read_pos, (size_t)(left_length = (uint)(info->read_end - info->read_pos)));
  Buffer += left_length;
  org_Count = Count;
  Count -= left_length;
  ...
  {                                /* wait for read block */
    ...
    if (info->aio_result.result.aio_errno)
    {
      if (info->myflags & MY_WME)
        my_error(EE_READ, MYF(ME_BELL + ME_WAITTANG), my_filename(info->file), info->aio_result.result.aio_errno);
      my_errno = info->aio_result.result.aio_errno;
      info->error = -1;
      return (1);
    }
    if (!(read_length = (uint)info->aio_result.result.aio_return) || read_length == (uint)-1)
    {
    ...
    next_pos_in_file = (info->pos_in_file + (uint)(info->read_end - info->request_pos));
  ...
  if (Count)
  {
    ...
    { /* Small block, read to cache */
      if ((read_length = my_read(info->file, info->request_pos, read_length, info->myflags)) == (uint)-1)
        return info->error = -1;
      ...
      { /* Didn't find hole block */
        if (info->myflags & (MY_WME | MY_FAE | MY_FNABP) && Count != org_Count)
          my_error(EE_EOFERR, MYF(ME_BELL + ME_WAITTANG), my_filename(info->file), my_errno);
        info->error = (int)(read_length + left_length);
        return 1;
      }
    }
    ...
    { /* Big block, don't cache it */
      if ((read_length = my_read(info->file, Buffer, (uint)Count, info->myflags)) != Count)
      {
  ...
  if (max_length)
  {
    ...
    DBUG_PRINT("aioread", ("filepos: %ld  length: %ld", (ulong)next_pos_in_file, (ulong)max_length));
    if (aioread(info->file, read_buffer, (int)max_length, (my_off_t)next_pos_in_file, MY_SEEK_SET,
                &info->aio_result.result))
    { /* Skip async io */
      my_errno = errno;
      DBUG_PRINT("error", ("got error: %d, aio_result: %d from aioread, async skipped", errno,
...
int my_block_write(register IO_CACHE *info, const byte *Buffer, uint Count, my_off_t pos)
{
  ...
  if (pos < info->pos_in_file)
  {
    ...
    if (pos + Count <= info->pos_in_file)
      return my_pwrite(info->file, Buffer, Count, pos, info->myflags | MY_NABP);
  ...
  if (info->type == WRITE_CACHE || append_cache)
  {
    ...
    if ((length = (uint)(info->write_pos - info->write_buffer)))
    {
      ...
      */
      ...
      if (my_write(info->file, info->write_buffer, length, info->myflags | MY_NABP))
        info->error = -1;
      else
        info->error = 0;
      if (!append_cache)
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < num_loops; i++)
  {
    ...
    if (my_b_append(&sra_cache, buf, 4) || my_b_append(&sra_cache, block, block_size))
      die("write failed");
 ...
 wonder how that happened");
  printf("Final size of %s is %s, wrote %d bytes\n", fname, llstr(status.st_size, llstr_buf), total_bytes);
