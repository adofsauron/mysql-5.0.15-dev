...
double my_strtod(const char *str, char **end_ptr, int *error)
{
  ...
  *error = 0;
  ...
  if ((next_char == 'e' || next_char == 'E') && dec_digits + ndigits != 0 && str < end - 1)
  {
  ...
  if ((exp = (neg_exp ? exp + digits_after_dec_point : exp - digits_after_dec_point)))
  {
    double scaler;
    if (exp < 0)
    {
    ...
    if (exp + ndigits >= MAX_DBL_EXP + 1 && result)
    {
      /*
        ...
        simpler
      */
      if (exp + ndigits > MAX_DBL_EXP + 1 || result >= MAX_RESULT_FOR_MAX_EXP)
      {
        if (neg_exp)
          result = 0.0;
        else
