...
bool mysql_rename_tables(THD *thd, TABLE_LIST *table_list)
{
  ...
  */
  ...
  if ((ren_table = rename_tables(thd, table_list, 0)))
  {
    ...
    for (table = table_list; table->next_local != ren_table; table = table->next_local->next_local)
      ;
...
static TABLE_LIST *rename_tables(THD *thd, TABLE_LIST *table_list, bool skip_error)
{
  ...
  for (ren_table = table_list; ren_table; ren_table = new_table->next_local)
  {
    ...
    sprintf(name, "%s/%s/%s%s", mysql_data_home, new_table->db, new_alias, reg_ext);
    ...
    sprintf(name, "%s/%s/%s%s", mysql_data_home, ren_table->db, old_alias, reg_ext);
    unpack_filename(name, name);
    ...
    switch (frm_type)
    {
      case FRMTYPE_TABLE:
      {
        ...
        if ((table_type = get_table_type(thd, name)) == DB_TYPE_UNKNOWN)
          ...
          rc = mysql_rename_table(table_type, ren_table->db, old_alias, new_table->db, new_alias);
        break;
      }
      case FRMTYPE_VIEW:
        /* change of schema is not allowed */
        if (strcmp(ren_table->db, new_table->db))
          my_error(ER_FORBID_SCHEMA_CHANGE, MYF(0), ren_table->db, new_table->db);
        else
