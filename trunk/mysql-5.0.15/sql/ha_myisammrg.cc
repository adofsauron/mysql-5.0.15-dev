...
handlerton myisammrg_hton = {
  "MRG_MYISAM",       SHOW_OPTION_YES, "Collection of identical MyISAM tables",
  DB_TYPE_MRG_MYISAM, NULL,            0, /* slot */
  0,                                      /* savepoint size. */
...
int ha_myisammrg::index_read(byte *buf, const byte *key, uint key_len,
...
int ha_myisammrg::index_read_last(byte *buf, const byte *key, uint key_len) {
  statistic_increment(table->in_use->status_var.ha_read_key_count,
                      &LOCK_status);
  int error =
      myrg_rkey(file, buf, active_index, key, key_len, HA_READ_PREFIX_LAST);
  table->status = error ? STATUS_NOT_FOUND : 0;
  return error;
}
...
THR_LOCK_DATA **ha_myisammrg::store_lock(THD *thd, THR_LOCK_DATA **to,
                                         enum thr_lock_type lock_type) {
  MYRG_TABLE *open_table;

  for (open_table = file->open_tables; open_table != file->end_table;
       open_table++) {
...
int ha_myisammrg::create(const char *name, register TABLE *form,
  ...
  *pos = 0;
  DBUG_RETURN(myrg_create(fn_format(buff, name, "", "", 2 + 4 + 16),
                          table_names, create_info->merge_insert_method,
                          (my_bool)0));
}
...
void ha_myisammrg::append_create_info(String *packet) {
  ...
  for (first = open_table = file->open_tables; open_table != file->end_table;
       open_table++) {
    LEX_STRING db, name;
    split_file_name(open_table->table->filename, &db, &name);
    if (open_table != first)
